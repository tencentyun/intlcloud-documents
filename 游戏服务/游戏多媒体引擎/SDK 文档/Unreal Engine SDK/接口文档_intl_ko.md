## 소개
Tencent Cloud 게임 멀티미디어 엔진 SDK의 사용을 환영합니다. Unreal Engine 개발자가 Tencent Cloud 게임 멀티미디어 엔진 제품 API를 디버깅하고 연결하는 데 도움이 되도록 여기에 Unreal Engine 개발에 적합한 액세스 기술 문서를 소개합니다.
...
```
### 메시지 전송
Delegate 방법을 채택한 인터페이스는 응용프로그램에 콜백 알림을 발송하는 데 사용됩니다. 메시지 유형은 ITMG_MAIN_EVENT_TYPE를 참조하며 data는 Windows 플랫폼에서 json 문자열 형식입니다. key-value에 대한 자세한 내용은 설명 문서를 참조하십시오.
...
### 시스템 복원
시스템에 다시 시작이 발생하는 동시에 엔진에 Resume을 알려야 합니다.
#### 함수 프로토타입
...
## 음성 채팅 방 관련 인터페이스
초기화한 후, SDK로 방 입장을 호출하여 방에 입장해야만 음성 채팅을 진행할 수 있습니다.
...
```
ITMGContext virtual void EnterRoom(int roomId, ITMG_ROOM_TYPE roomType, const char* authBuff, int buffLen)//일반 방 입장 인터페이스
```
|매개변수     |유형         |의미|
| ------------- |:-------------:|-------------|
| roomId			|int   		|방 번호, 32비트만 지원	|
...
| audioMode    		|int    	|0은 전역 음석을 표시하고 1은 팀 은성을 표시합니다.		|
...
### 멤버 상태 변화
이 이벤트는 상태가 변화해야 알리며, 상태가 변화하지 않는 상황에서는 알리지 않습니다. 멤버 상태를 실시간으로 획득하려면 상위 계층에서 알림을 수신할 때 캐시를 하십시오. 이벤트 메시지는 ITMG_MAIN_EVNET_TYPE_USER_UPDATE이고 그 중 data는 event_ID 및 user_list 2개의 정보를 포함하며 OnEvent 함수에서 정보 event_ID를 판단해야 합니다.
...
|ITMG_MAIN_EVENT_TYPE_ROOM_DISCONNECT    		       |방이 네트워크 등으로 인해 끊김 메시지|
...
- 대부분의 게임 앱은 EnableAudioCaptureDevice/EnableAudioSend와 EnableAudioPlayDevice/EnableAudioRecv를 동시 호출해야 합니다.
...
### 오디오 엔진 캡처과 재생 복원
이 인터페이스를 호출하여 오디오 엔진 캡처와 재생을 복원합니다. 이 인터페이스는 동기화 인터페이스로 방 입장 이후에만 유효합니다.
#### 함수 프로토타입  
...
### 마이크의 소프트웨어 음량 설정
이 인터페이스는 마이크의 소프트웨어 음량 설정에 사용됩니다. 매개변수 volume은 마이크의 소프트웨어 음량 설정에 사용되며, 값이 0이면 무성임을, 값이 100이면 음량이 커지지도 작아지지도 않음을 의미합니다. 기본값은 100입니다.
#### 함수 프로토타입  
```
...
### 스피커 실시간 음량 획득
이 인터페이스는 스피커 실시간 음량 획득에 사용됩니다. 반환 값은 int 유형이며, 스피커 실시간 음량임을 의미합니다.
#### 함수 프로토타입  
```
...
### 스피커의 소프트웨어 음량 설정
이 인터페이스는 스피커의 소프트웨어 음량 설정에 사용됩니다.
매개변수 volume은 스피커의 소프트웨어 음량 설정에 사용되며, 값이 0이면 무성 을, 값이 100이면 음량이 커지지도 작아지지도 않음을 의미합니다. 기본값은 100입니다.
...
### 반주 재생 시작
이 인터페이스를 호출하여 반주 재생을 시작합니다. M4A, AAC, WAV, MP3 모두 4가지 형식을 지원합니다. 이 API를 호출하면 음량이 리셋됩니다.
...
| loopBack  	|bool	|믹스하여 발송 여부, 일반적으로 TRUE로 설정하며 즉, 다른 사람도 반주를 들을 수 있습니다.	|
...
해당 인터페이스는 자신이 반주를 들을 수 있는지 여부 설정에 사용합니다.
#### 함수 프로토타입  
```
...
|SetVoiceType 		|음성변조 효과|
|GetEffectsVolume	|효과음 재생 음량 획득|
|SetEffectsVolume 	|효과음 재생 음량 설정|


### 효과음 재생
이 인터페이스는 효과음 재생에 사용됩니다. 매개변수에서의 효과음 ID는 앱에서 관리해야 하며, 하나의 독립 파일을 유일하게 식별합니다. 파일은 M4A, AAC, WAV, MP3 모두 4가지 형식을 지원합니다.
#### 함수 프로토타입  
```
...
|ITMG_MAIN_EVENT_TYPE_ROOM_DISCONNECT		|방이 네트워크 등으로 인해 끊김 메시지	|
|ITMG_MAIN_EVENT_TYPE_CHANGE_ROOM_TYPE		|방 유형 변화 이벤트		|
|ITMG_MAIN_EVENT_TYPE_MIC_NEW_DEVICE    	|추가 마이크 장치 메시지		|
|ITMG_MAIN_EVENT_TYPE_MIC_LOST_DEVICE    	|분실 마이크 장치 메시지		|
|ITMG_MAIN_EVENT_TYPE_SPEAKER_NEW_DEVICE	|추가 스피커 장치 메시지		|
|ITMG_MAIN_EVENT_TYPE_SPEAKER_LOST_DEVICE	|분실 스피커 장치 메시지		|
...
|ITMG_MAIN_EVNET_TYPE_PTT_SPEECH2TEXT_COMPLETE	|음성 문자로 전환 완료			|
