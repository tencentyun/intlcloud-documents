...
// Set the SDK logging level. For more information on each level, see <a href="https://imsdk-1252463788.file.myqcloud.com/IM_DOC/Web/SDK.html#setLogLevel">setLogLevel API Description</a>.
...
tim.on(TIM.EVENT.MESSAGE_RECEIVED, function(event) {
  // A newly pushed one-to-one chat message, group chat message, group tip, or group system notification is received. You can traverse `event.data` to obtain the message list and render it to the UI.
...
tim.on(TIM.EVENT.MESSAGE_READ_BY_PEER, function(event)) {
  // SDK has received a notification indicating that the opposite end has read the message. This is the read receipt. Before using it, you need to upgrade the SDK version to V2.7.0 or higher. Only one-to-one chat conversations are supported.
