...
This API is used to create a text message. It returns a message instance. If you need to send a text message, call the [sendMessage](https://imsdk-1252463788.file.myqcloud.com/IM_DOC/Web/SDK.html#sendMessage) API to send this message instance.
...
let message = tim.createTextMessage({
  ...
  // The message priority, which is applicable to group chats (supported by v2.4.2 or later). If messages in a group exceed the frequency limit, the backend delivers high-priority messages first. For more information, see https://intl.cloud.tencent.com/document/product/1047/33526.
  // Valid enumerated values: TIM.TYPES.MSG_PRIORITY_HIGH, TIM.TYPES.MSG_PRIORITY_NORMAL (default), TIM.TYPES.MSG_PRIORITY_LOW, and TIM.TYPES.MSG_PRIORITY_LOWEST
...
| `conversationType` | `String` | The conversation type. Valid values: `TIM.TYPES.CONV_C2C` and `TIM.TYPES.CONV_GROUP`. |
| `payload` | `Object` | The message content container. |
| `onProgress` | `function` | A callback function used to query the upload progress. |

`payload` has the following properties:

| Name | Type | Description |
| ---- | ---------------------------- | ------------------------------------------------------------ |
| file | `HTMLInputElement` or `Object` | This is used to select a DOM node or file object of the image in a web app, or the `success` callback parameter for the `wx.chooseImage` API of a Mini Program. The SDK reads the data contained in this parameter and uploads the image. |
...
<span class="hljs-built_in">document</span>.getElementById(<span class="hljs-string">'testPasteInput'</span>).addEventListener(<span class="hljs-string">'paste'</span>, <span class="hljs-function"><span class="hljs-keyword">function</span>(<span class="hljs-params">e</span>) </span>{
  ...
  <span class="hljs-keyword">if</span> (clipboardData &amp;&amp; clipboardData.files &amp;&amp; clipboardData.files.length &gt; <span class="hljs-number">0</span>) {
    ...
    <span class="hljs-comment">// After the image message is successfully sent, the content pointed to by `file` may be cleared by the browser. If you have additional rendering requirements, copy the data in advance.</span>
    fileCopy = file.slice();
  }

  <span class="hljs-keyword">if</span> (<span class="hljs-keyword">typeof</span> file === <span class="hljs-string">'undefined'</span>) {
    <span class="hljs-built_in">console</span>.warn(<span class="hljs-string">'file is undefined. Please check the compatibility of the code or browser.'</span>);
...
wx.chooseImage({
  ...
  count: 1, // Only one image can be selected. The SDK does not support the sending of multiple images at a time.
...
| Name | Type | Description |
| ---- | -------- | -------------------- |
| file | `Object` | The file information obtained after recording. |

**Mini Program example**


<pre><code><span class="hljs-comment">// Example: use the official WeChat RecorderManager to record a voice message. For more information, see <a href="https://developers.weixin.qq.com/minigame/dev/api/media/recorder/RecorderManager.start.html">RecorderManager.start(Object object)</a>.</span>
...
<span class="hljs-comment">// 2.2 Listen for the recording completed event. After recording is completed, call createAudioMessage to create a voice message instance.</span>
...
| Name | Type | Description |
| ------ | ------------------ | ------------------------------------------------------------ |
| file | `HTMLInputElement` | This is used to select a DOM node or file object of the file in a web app. The SDK reads the data contained in this parameter and uploads the file. |
...
<pre><code class="language-javascript"><span class="hljs-comment">// Example: use a custom message to implement dice rolling.</span>
...
| Name | Type | Description |
| ------- | -------- | -------------------- |
| `index` | `Number` | The emoji index, which is customized by the user. |
...
| `options` | `Object` | `optional` | The message sending option (that is, the message content container). |
...
<span class="hljs-selector-tag">tim</span><span class="hljs-selector-class">.sendMessage</span>(message, {
  ...
  <span class="hljs-attribute">offlinePushInfo</span>: {
    ...
    <span class="hljs-attribute">androidOPPOChannelID</span>: <span class="hljs-string">''</span> <span class="hljs-comment">// channelID for offline push configured on OPPO mobile phones that run Android 8.0 or later.</span>
  }
});</code></pre>
