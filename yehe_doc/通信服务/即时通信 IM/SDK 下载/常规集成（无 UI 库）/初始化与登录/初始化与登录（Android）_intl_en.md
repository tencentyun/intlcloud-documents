...
### SDKAppID
SDKAppID indicates the app ID. It is a unique ID that the IM service uses to distinguish customer accounts. We recommend that you apply for a new SDKAppID for every independent app to automatically isolate messages between SDKAppIDs.
...
### SDKConfig
The `V2TIMSDKConfig` parameter is used for initialization configuration of the SDK. It is often used to set the log level, that is, [setLogLevel](http://doc.qcloudtrtc.com/im/classcom_1_1tencent_1_1imsdk_1_1v2_1_1V2TIMSDKConfig.html#a033c4e90397236427f3dd65038df8033). The following table describes possible log levels.
...
- IM SDK logs are stored by default in `/sdcard/tencenet/imsdklogs/<Application package name>` for versions earlier than 4.8.50, or in  `/sdcard/Android/data/<Application package name>/files/log/tencent/imsdk` for version 4.8.50 or later.
...
### Listener
[V2TIMSDKListener](http://doc.qcloudtrtc.com/im/classcom_1_1tencent_1_1imsdk_1_1v2_1_1V2TIMSDKListener.html) is used to monitor the network status and user information changes.
...
> If the `onUserSigExpired` callback is received, it indicates that the UserSig used for login has expired. In this case, you need to update the UserSig and then log in again. If you continue to use the expired UserSig, the SDK falls into an endless login loop.
...
- When the IM SDK triggers an `onKickOffline` callback. That is, when the current user is forced offline, the message stating "You have already logged in to the SDK on another device using the current account. Are you sure you want to log in again?" can be displayed on the UI. In this case, you can select "Yes" to log in again.
...
### Logout
To log out of the SDK, call the [logout](http://doc.qcloudtrtc.com/im/classcom_1_1tencent_1_1imsdk_1_1v2_1_1V2TIMManager.html#a0398924fa1b62a8f5cc9b51673273b48) function.
