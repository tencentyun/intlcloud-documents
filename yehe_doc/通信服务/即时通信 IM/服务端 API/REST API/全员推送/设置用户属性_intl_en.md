## Feature Description
This API is used by the admin to set attributes for users. Each request can set attributes for a maximum of 100 users. Before using this API, be sure to define app user attributes.
...
| Attrs | Object | Required | Attribute set. Each attribute is a key-value pair, with the key being the attribute name and the value being the user’s corresponding attribute value. A user attribute value cannot exceed 50 bytes. |
...
| 90033 | Attributes are invalid. |
| 91000 | An internal service error occurs. Please try again. |

## API Debugging Tool
To debug this API, you can use the [Online RESTful API Debugging Tool](https://avc.qcloud.com/im/APITester/APITester.html#v4/all_member_push/im_set_attr).
