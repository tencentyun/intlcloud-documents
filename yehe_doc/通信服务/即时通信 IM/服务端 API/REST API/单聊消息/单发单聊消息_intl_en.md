...
#### The admin sends messages to another account and forbids callback for the messages
...
| MsgTimeStamp | Integer |Optional | Message timestamp; UNIX timestamp (unit: second). |
...
| MsgTime | Integer | Message timestamp; UNIX timestamp. |
...
| 90992 | An internal service error occurred. Please try again. If this error code is returned for all requests and the app has configured third-party callback, check whether the app server returns the callback result to the IM backend server normally. |
