...
>?Due to the internal implementation, AVChatRoom (audio-video group) only supports modifying the member profiles of admins and the group owner. Error 10007 will be returned if you try to modify the profiles of ordinary group members.
...
Set group member custom fields. `AppMemberDefinedData` is not available by default and needs to be enabled in the [IM console](https://console.cloud.tencent.com/im) before use. For details, see the description table for request packet fields.
```
{
    "GroupId": "@TGS#2CLUZEAEJ", // Target group (required)
    "Member_Account": "bob", // Target group member (required)
...
| AppMemberDefinedData | Array | Optional | Group member custom field. This field is not available by default and needs to be enabled in the [IM console](https://console.cloud.tencent.com/im). For details, see [Group System](https://intl.cloud.tencent.com/document/product/1047/33529).  |
