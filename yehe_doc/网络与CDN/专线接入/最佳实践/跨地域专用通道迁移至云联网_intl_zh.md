...
2. 查看 VPC 路由配置[](id:1-2)
  ...
  4. 单击左侧目录的**路由表**，选择 VPC 所在地域，选择 VPC，单击**路由表 ID**。
 5. 进入即可查看 VPC 路由表配置详情![]()。
3. 查看专用通道配置[](id:1-3)
  1. 登录 [专线接入控制台](https://console.cloud.tencent.com/dc/dc)。
  8. 单击左侧目录中的**专用通道**，单击**专用通道 ID**，进入专用通道详情页。
...
#### 步骤四：创建云联网实例[](id:2-4)
1. 登录 [云联网控制台](https://console.cloud.tencent.com/vpc/ccn)，单击**+新建**，输入云联网实例名称，选择计费模式、服务质量、限速方式和关联实例私有网络，单击**确定**。
![]()
