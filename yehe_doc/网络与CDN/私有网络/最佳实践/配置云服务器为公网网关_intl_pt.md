...
## Pré-requisitos
- Você está logado no [console do CVM](https://console.cloud.tencent.com/cvm/index).
...
1. Faça login no [console do CVM](https://console.cloud.tencent.com/cvm/index) e selecione [**EIP**](https://console.cloud.tencent.com/cvm/eip) na barra lateral esquerda.
...
>
1. [Crie uma tabela de rotas personalizada](https://intl.cloud.tencent.com/document/product/215/35236).
...
Para obter mais informações, consulte [Gerenciamento de tabela de rotas](https://intl.cloud.tencent.com/document/product/215/35236).
![](https://main.qcloudimg.com/raw/9b2d9537e7aa0c00428ef112db300d73.png)

### Etapa 4: configure o gateway público
1. [Faça login no CVM de gateway público](https://intl.cloud.tencent.com/document/product/213/5436) e execute as etapas abaixo para habilitar o encaminhamento de rede e o proxy NAT.
 1. Execute o seguinte comando para criar o script `vpcGateway.sh` em `usr/local/sbin`.
```
vim /usr/local/sbin/vpcGateway.sh
```
 2. Pressione **i** para alternar ao modo de edição e adicione o seguinte código no script.
```
#!/bin/bash
echo "----------------------------------------------------"
echo " `date`"
...
```
 ...
 4. Execute o seguinte comando para definir a permissão do script.
```
...
2. Defina o RPS do gateway público.
 1. Execute o seguinte comando para criar o script `set_rps.sh` em `usr/local/sbin`.
```
