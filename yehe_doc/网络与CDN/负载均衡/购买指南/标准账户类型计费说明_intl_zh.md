...
实例费用的计费周期为按小时计费，每小时结算一次，使用不足一小时将按照一小时进行扣费。
...
:::
::: 共享带宽包[](id:bag)
多 IP 聚合计费，按月结算，适用于大规模业务，且不同使用公网的实例间可形成流量错峰的场景，价格详情请参见 [共享带宽包 - 常规 BGP 带宽包](https://intl.cloud.tencent.com/zh/document/product/684/15254#bgp)。
:::
</dx-accordion>
...
总费用 = 实例费 + 公网网络费 = 0.686美元 +0.24美元 = 0.926美元，于下一小时（10:00:00 - 10:59:59）进行结算并扣费。

#### 示例2（公网网络费计费模式为共享带宽包）：
假设您在广州地域的 09:00:00 - 09:59:59 时间内，使用了按量计费且公网网络费的计费模式为共享带宽包的负载均衡，则：
- 实例费：实例费单价 × 使用时长 = 0.686美元/天 × 1天 = 0.686美元。
- 公网网络费：由于 <a href="https://intl.cloud.tencent.com/zh/document/product/684/15254">共享带宽包费用</a> 是按月结算，因此在 09:00:00 - 09:59:59 时间内仅结算实例费。
总费用 = 实例费 = 0.686美元，于下一小时（10:00:00 - 10:59:59）进行结算并扣费。
:::
::: 性能容量型实例
#### 示例1（公网网络费计费模式为按流量计费）：
假设您在广州地域的 09:00:00 - 09:59:59 时间内，使用了按量计费且公网网络费的计费模式为按流量计费的负载均衡，且从负载均衡到公网的总流量为2GB，且每小时平均消耗的性能容量单位 LCU 数为 2 个，则：
- 实例费：每小时实例费 × 使用时长 =0.686美元/天 × 1天 = 0.686美元。
- 公网网络费：每单位流量费 × 总流量 = 0.12美元/GB × 2GB = 0.24美元。
- 性能容量 LCU 费 = 2个 LCU × 0.0072美元/个/小时 × 1小时 = 0.0144美元。
总费用 = 实例费 + 公网网络费 = 0.686美元 + 0.24美元 + 0.0144美元 = 0.9404美元，于下一小时（10:00:00 - 10:59:59）进行结算并扣费。
...
- 公网网络费：由于 <a href="https://intl.cloud.tencent.com/zh/document/product/684/15254">共享带宽包费用</a> 是按月结算，因此在 09:00:00 - 09:59:59 时间内仅结算实例费。
- 性能容量 LCU 费 = 2个 LCU × 0.0072美元/个/小时 × 1小时 = 0.0144美元。
总费用 = 实例费 + 性能容量单位 LCU 费 = 0.7004美元，于下一小时（10:00:00 - 10:59:59）进行结算并扣费。
