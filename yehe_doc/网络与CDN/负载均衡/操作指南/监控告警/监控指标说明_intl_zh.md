...
## 负载均衡实例维度
>?入带宽利用率和出带宽利用率监控指标仅标准账户类型支持，判断账户类型的方式请参见 [判断账户类型](https://intl.cloud.tencent.com/document/product/684/15246)。
...
|SNAT 并发连接数 | 个  | 在统计周期内，负载均衡的 SNAT IP 每分钟的并发连接数。|
|SNAT 端口使用率 | %  | 在统计周期内，负载均衡的 SNAT IP 的端口使用率。端口使用率 = SNAT 并发连接数 / (SNAT IP 数 * 55000 * RS 个数）。|
|SNAT失败数 | 个  | 在统计周期内，负载均衡的 SNAT IP 与后端 RS 每分钟建立连接的失败次数。|
...
## 相关文档
[公网负载均衡监控指标](https://intl.cloud.tencent.com/document/product/248/10997)
