...
>?此功能处于内测阶段，如需使用，请 [提交工单](https://console.cloud.tencent.com/workorder/category)。
...
- **降低抖动和丢包**
公网链路的传输性能不稳定，如跨境问题，会导致网络的抖动和丢包，影响服务体验。而 Anycast CLB 的传输性能稳定，它将客户端请求就近接入腾讯云后，通过腾讯云的内网专线进行跨地域传输，解决了公网抖动和丢包的问题。
- **高可靠**
...
### Anycast 发布域
Anycast 发布域是加速 IP 地址发布的地点，即 Anycast CLB 的 VIP 所发布的 POP 接入点，客户端会接入最近的 POP 点。目前 Anycast CLB 支持同时发布在：北京、上海、广州、中国香港、多伦多、硅谷、法兰克福、弗吉尼亚、莫斯科、新加坡、首尔、孟买、曼谷和东京。

### Anycast CLB 所属地域
与普通负载均衡的地域概念一致，Anycast CLB 所属地域是购买 Anycast CLB 时选择的地域，也指后端云服务器所在的地域。目前 Anycast CLB 已覆盖绝大部分地域。
- 亚太地区：北京、上海、广州、中国香港、新加坡、首尔、孟买、曼谷、东京。
- 欧美地区：多伦多、硅谷、法兰克福、弗吉尼亚、莫斯科。
...
## 操作指南
### 前提条件
本功能内测中，操作前请确保您的 [提交工单](https://console.cloud.tencent.com/workorder/category) 已通过。
...
>? 内网负载均衡实例绑定弹性公网 IP 的限制说明请参见 [使用限制](https://intl.cloud.tencent.com/document/product/214/44336)。
5. 在弹出的“绑定弹性公网 IP”对话框，选择刚才创建的加速 IP，单击**提交**。
![]()
5. 内网负载均衡绑定 Anycast 加速 IP 后，该负载均衡即可提供 Anycast 负载均衡服务。更多负载均衡配置请参见 [负载均衡监听器概述](https://intl.cloud.tencent.com/document/product/214/6151)。
![](https://main.qcloudimg.com/raw/f8e1334deb5679de7b5330208782a5e4.png)


## 相关文档
[内网负载均衡实例绑定 EIP](https://intl.cloud.tencent.com/document/product/214/44336)