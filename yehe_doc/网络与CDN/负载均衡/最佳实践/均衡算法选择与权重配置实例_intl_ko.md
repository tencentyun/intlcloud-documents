...
- **원리**
라운드 로빈 스케쥴링은 폴링을 기반으로 다른 서버에 대한 요청을 스케쥴링하는 것입니다. 즉, 시스템은 각 스케쥴링에서 서버 i를 선택하기 위해 i = (i + 1) mod n을 수행합니다. 가중 라운드 로빈 스케쥴링은 서로 다른 서버의 성능 불균형을 해결할 수 있습니다. 가중치를 사용하여 서버의 처리 성능을 나타내고 폴링을 통해 가중치별로 다른 서버에 대한 요청을 스케쥴링합니다. 가중치가 높은 서버는 연결을 더 빨리 수신하지만 가중치가 낮은 서버보다 더 많은 연결을 처리해야 합니다. 동일한 가중치를 가진 서버는 동일한 수의 연결을 처리합니다.
- **장점**
이 알고리즘은 단순성과 높은 실용성이 특징입니다. 모든 연결의 상태를 기록하지 않는 상태 비저장 스케쥴링 알고리즘입니다.
- **단점**
가중 라운드 로빈 스케쥴링은 비교적 간단하지만 요청의 서비스 시간이 크게 변경되거나 각 요청이 다른 시간을 소비하는 시나리오에는 적합하지 않습니다. 이러한 시나리오에서는 라운드 로빈 스케쥴링으로 인해 서버 간에 부하 분산 불균형이 발생할 수 있습니다.
- **적용 시나리오**
이 알고리즘은 각 요청이 기본적으로 최고의 로딩 성능으로 백엔드에서 동일한 시간을 소비하는 시나리오에 적합합니다. 일반적으로 HTTP 서비스와 같은 비지속 연결 서비스에서 사용됩니다.
- **권장 사항**
각 요청이 백엔드에서 기본적으로 동일한 시간을 소비한다는 것을 알고 있는 경우(예시: 리얼 서버에서 처리되는 요청이 동일하거나 유사한 유형임) 가중 라운드 로빈 스케쥴링을 사용하는 것이 좋습니다. 각 요청 간의 시간 차이가 작은 경우 이 알고리즘은 소비량이 적고 효율이 높으며 순회가 필요 없기 때문에 이 알고리즘을 사용하는 것이 좋습니다.
