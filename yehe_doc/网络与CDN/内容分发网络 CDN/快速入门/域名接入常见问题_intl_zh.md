...
您可以参考 [CNAME 配置](https://www.tencentcloud.com/document/product/228/3121) 文档中的操作说明，在您的 DNS 服务商处绑定 CNAME。
...
CDN 节点会按照 [节点缓存过期配置](https://intl.cloud.tencent.com/document/product/228/38424) 缓存资源，若 CDN 节点的缓存未过期，则不会主动回源站更新资源。
...
WordPress 涉及到登入（后台登入目录 /wp-admin）、接口等动态请求，如果缓存设置不得当会造成登录异常，建议您将相应的动态文件类型的缓存时间设置为不缓存。
...
如果您需要停止加速，可以在 CDN 控制台中关闭加速服务，关闭加速域名后，即可删除加速域名，具体操作可以参见 [域名操作](https://intl.cloud.tencent.com/document/product/228/5736)。如果您关闭加速域名后，无法执行删除，可能是域名当前状态还在关闭中，或者您当前用户身份为协作者身份，协作者的操作权限由 CDN 服务的创建者主账号创建配置，需要分配对应域名的删除权限才可以操作。
