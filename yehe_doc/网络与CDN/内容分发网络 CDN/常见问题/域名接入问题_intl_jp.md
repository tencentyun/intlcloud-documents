...
6. 同じアカウントでは、複数のネストされたドメイン名を追加できます。たとえば、`*.example.com`、`*.path.example.com`、`a.path.example.com`は、同じアカウントで同時に追加できます。ドメイン名の設定、アクセストラフィックの統計は優先度別に統計できます。一致性が高いほど、優先度が高くなります。たとえば、`a.path.example.com`へのアクセスは、`a.path.example.com`のドメイン名構成が適用されます。`b.path.example.com`へのアクセスは、`*.path.example.com`のドメイン名構成が適用されます。`c.example.com`へのアクセスは、`*.path.com`の構成が適用されます。アクセストラフィックの統計は同様です。
