CDN은 자가 진단 툴을 제공합니다. 특정 URL에 액세스 오류가 발생할 경우 이 툴을 이용해 자가 점검을 진행할 수 있습니다. 자가 진단 과정에는 도메인에 연결된 DNS 리졸브 확인, 링크 품질 확인, 노드 상태 확인, 원본 서버 확인, 데이터 액세스 일관성 등 일련의 진단 항목이 포함되어 있어 문제를 진단하고 해결 방법을 제시합니다.
