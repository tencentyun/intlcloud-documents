...
지정한 리포트 차원에 따라 해당 지역의 트래픽 분포 그래프를 표시합니다. 소스 클라이언트 IP로 방문자 소재한 성(省)을 식별하고 지도, 목록을 표시하여 고객이 서비스 사용자 지역 분포 상황을 알기 쉽도록 합니다. 중국 내 지역은 성별로, 중국 외 지역은 지역별로 통계를 집계합니다. 
