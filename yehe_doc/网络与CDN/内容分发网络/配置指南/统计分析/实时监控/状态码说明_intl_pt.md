...
A tabela abaixo explica os códigos de status internos do CDN.
...
| 403    | A solicitação foi rejeitada                             | Verifique se os controles de acesso, como a lista de bloqueio/permissões do referencial, a lista de bloqueio/permissões de IP ou a autenticação estão configurados. |
| 413    | O comprimento do conteúdo da solicitação POST excedeu o limite                    | Verifique o tamanho do conteúdo da solicitação POST do cliente (o tamanho máximo é 32 MB por padrão).           |
...
| 503    | O controle de frequência do COS é acionado                          | Verifique a configuração do cache ou se o servidor de origem do COS retorna no-cache/no-store.                 |
...
| 514    | A frequência de acesso de IP excedeu o limite                       | Verifique a configuração de controle de frequência de acesso de IP no console do CDN.                                 |
...
| 537    | Tempo limite de recepção de dados do servidor de origem na solicitação HTTPS             | Verifique a estabilidade do servidor de origem comercial.                                         |
