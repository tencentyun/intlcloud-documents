...
>!
> 1.当面は中国本土のみをサポートし、独自の証明書は設定されず、[画像の最適化]（https://cloud.tencent.com/document/product/228/43121）機能が有効になっていないドメイン名はバージョン管理が有効となります。
>2. それぞれの環境で、1回につき1つのバージョンのみデプロイできます。
>3. URL更新機能は本番環境とシミュレーション環境の両方で機能し、ディレクトリ更新およびURLプリフェッチ機能は本番環境でのみ機能します。 パージとプリフェッチ機能の説明については、[パージとプリフェッチ]（https://intl.cloud.tencent.com/document/product/228/6299）をご参照ください。
> 4. [ネットワーク帯域幅の制限の設定]（https://intl.cloud.tencent.com/document/product/228/7541）などのデータ/使用量状況の監視機能は、本番環境でのみ機能します。
