After a flow log is created for the ENI, you can store and analyze the network traffic in real time, making it fit for troubleshooting, compliance audit, security and other use cases. This document describes how to create a flow log in the private network.
...
## Directions
1. Log in to the [VPC console](https://console.cloud.tencent.com/vpc/vpc?rid=1) and select **Diagnostic Tools** > **Flow Logs** on the left sidebar.
...
## Result Validation
After 15 minutes, locate the flow log you’ve created on the **Flow Logs** page and click **Check** in the **Operation** column to access the **Search and Analysis** page. Select a time range and search for the IP of the CVM B. The result is the same as the response received by the CVM A.
![]()
