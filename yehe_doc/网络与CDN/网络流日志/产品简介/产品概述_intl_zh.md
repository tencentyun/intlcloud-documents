...
您可以为弹性网卡等创建网络流日志，采集传入/传出的流量。创建流日志后，您可以在 [日志服务（CLS）](https://intl.cloud.tencent.com/document/product/614) 中查看和检索数据。若需对流日志数据进行生命周期管理，则可以在日志服务（CLS）中将指定流日志投递其他云产品进行分析或存储，例如投递至 COS 存储桶中。
