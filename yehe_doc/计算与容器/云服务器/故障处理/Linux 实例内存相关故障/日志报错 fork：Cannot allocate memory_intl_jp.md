...
## 考えられる原因
スレッド数の限度を超えたことが原因と考えられます。システム内部のスレッド総数が`pid_max`に達すると、新しいスレッドが作成されたときに「fork：Cannot allocate memory」というエラーが報告されます。

## 解決方法
1. [処理手順](#ProcessingSteps)を参照して、インスタンスのメモリ使用率が高すぎないかどうかを確認します。
2. スレッド数が不足していないか確認し、スレッド総数`pid_max`の構成を変更します。 
...
3. 以下のコマンドを実行して、システム内部のスレッド総数を確認します。
```
pstree -p | wc -l
```
スレッド総数が`pid_max`に達すると、システムは新しいスレッドを作成するときに「fork：Cannot allocate memory」というエラーを報告します。
>?`ps -efL`コマンドを実行して、スレッドの起動が多いプログラムを特定します。
>
4. `/etc/sysctl.conf`構成ファイルの`kernel.pid_max`値を65535に変更して、スレッドの数を増やします。変更が完了すると、下図のようになります。
