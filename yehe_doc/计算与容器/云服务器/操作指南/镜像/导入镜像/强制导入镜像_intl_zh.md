## 操作场景
当用户的 Linux 镜像因为某些原因无法 [安装 cloudinit](https://intl.cloud.tencent.com/document/product/213/12587) 时，可以通过**强制导入镜像**功能完成镜像的导入。由于强制导入的镜像没有安装 cloudinit，如果用户使用强制导入镜像进行导入，腾讯云无法对用户的云服务器进行初始化配置。因此，用户使用强制导入镜像进行导入时，需要根据腾讯云提供的配置文件，自行设置脚本，对云服务器进行配置。本文档指导用户如何在强制导入镜像的前提下，对云服务器进行配置。
...
## 限制条件
- 镜像仍需要满足 [导入镜像](https://intl.cloud.tencent.com/document/product/213/4945) 中关于 Linux 镜像导入的镜像的限制（cloudinit 除外）。
- 导入镜像的系统分区未满。
- 导入的镜像不能存在可以被远程利用的漏洞。
