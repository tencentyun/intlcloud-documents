...
## VPA 优势
相较于 [自动伸缩功能 HPA](https://intl.cloud.tencent.com/document/product/457/32424)，VPA 具有以下优势：
...
- VPA 与 HPA 不可同时在 CPU 和内存预留上运行。如需同时运行 **VPA** 与 **HPA**，则 HPA 需使用除 CPU 和内存以外的指标，详情可参见 [在 TKE 上使用自定义指标进行弹性伸缩](https://intl.cloud.tencent.com/document/product/457/38941)。
...
- VPA 性能尚未在大型群集中进行测试。
- VPA 对 Pod 资源 Request 的建议值可能会超出可用资源（例如节点资源上限、空闲资源或资源配额），并导致 Pod 处于 Pending 状态无法被调度。同时使用 VPA 与 [Cluster Autoscaler](https://intl.cloud.tencent.com/document/product/457/35900) 可以部分解决此问题。
...
## 前提条件
- 已创建容器服务 TKE 集群。如果您还未创建集群，请参考 [快速创建一个标准集群](https://intl.cloud.tencent.com/document/product/457/40029)。
- 已使用命令行工具 Kubectl 连接集群。如果您还未连接集群，请参考 [连接集群](https://intl.cloud.tencent.com/document/product/457/30639)。
...
#### 解决方案
1. 如果您没有通过集群中的云服务器执行命令，建议您在云服务器中下载 Autoscaler 项目，并执行完整的 [部署 VPA](#VPA) 操作。如需为您的云服务器连接集群，详情可参见 [连接集群](https://intl.cloud.tencent.com/document/product/457/30639)。
...
2. **更换标签及推送**。
    将 vpa-admission-controller、vpa-recommender、vpa-updater 的镜像更换标签后推送到您的镜像仓库中。上传镜像操作详情可参见 [容器镜像服务个人版快速入门](https://intl.cloud.tencent.com/document/product/1051/38866)。
3. **更改 YAML 镜像地址**。
