...
由于 CDN 本质上是进行内容的加速，无法识别出请求是否为恶意，因此无法对恶意请求进行处理，来避免此类流量的产生。为了避免此类恶意域名给您带来过多损失，当您的域名遭受攻击且带宽超出 10Gbps 时，您可以 [提交工单](https://console.qcloud.com/workorder/category) 进行申诉，腾讯云 CDN 计算带宽超出 10Gbps 部分的费用，并返还给您。
