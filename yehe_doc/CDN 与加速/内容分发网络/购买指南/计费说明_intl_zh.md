...
| :------: | :----------------------------------------------------------: | :----------------------------------------------------------: | ---------------- | :----------------------------------------------------------: |
| 带宽计费 | 按每日的峰值带宽计费。每5分钟统计一个带宽数据，每日得到288个值，取其中的最大值 | 后付费按日结算<br />前一天00:00:00 - 23:59:59产生的总消耗，会在第二天18:00前进行计算扣费 | 阶梯到达模式     | 参见 [计费详情](#pay-by-bandwidth) |
| 流量计费 |          按每日从腾讯云 CDN 节点流出的实际流量计费           | 后付费按日结算<br />前一天00:00:00 - 23:59:59产生的总消耗，会在第二天18:00前进行计算扣费 | 月度阶梯累进模式 | 参见 [计费详情](#pay-by-traffic) |
...
|    计费模式    |                             描述                             |                           适用场景                           |                           计费详情                           |
| :------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
| 日峰带宽取月均 | CDN 每日带宽统计点共288个，对所有有效天的峰值取平均，得到计费带宽，再根据商务签订的合同价格计算费用。 | CDN 月消费金额大于2万美元或预期超过2万美元的用户，请提交工单进行商务洽谈。 | 参见 [计费详情](#billed-by-monthly-average-of-daily-bandwidth-values) |
|    月95带宽    | CDN 每日带宽统计点共288个，从当月1号起，每一个有效日（产生消耗）的所有统计点进行排序，去掉前5%的统计点，剩下的最大的统计点，即为计费带宽，再根据合同价格计算费用。 |                                                              | 参见 [计费详情](#billed-by-monthly-95th-percentile-bandwidth) |
