...
>3. リフレッシュ機能は、本番環境とシミュレーション環境の両方で機能します。
...
>! 新しいバージョンを送信した後、設定項目のエラーメッセージを送信した場合（設定項目の送信に成功しなかった可能性があります）、このバージョンが引き続き生成されます（エラーのある設定項目はデフォルトで元の内容になります）ので、新たに作成する必要はありません。バージョン管理ページに戻ってこのバージョンを編集し、設定の調整を続けることができます。
...
バージョン管理が有効になっているドメイン名では、次の手順でバージョンをパブリッシュすることをお勧めします。

1. シミュレーション環境でテストまたはパブリッシュするバージョンをオンラインにする：バージョン操作列の【その他】>【シミュレーション環境にパブリッシュ】をクリックします。
2. シミュレーション環境にデプロイされたバージョンをテストする：CDNはIPv4のIPをドメイン名に割り当て、クライアントのHOSTSを変更し、ドメイン名をこのIPに指定してテストを実行します。
...
オフラインになった後、ドメイン名を再度オンラインにすることができます。本番環境の【オンライン】をクリックすると、本番環境とシミュレーション環境において、環境に表示されるバージョンが同時にオンラインになります。
