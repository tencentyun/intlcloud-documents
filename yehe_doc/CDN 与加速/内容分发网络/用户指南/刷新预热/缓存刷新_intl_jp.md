...
> ?更新が完了した後、ノード上の対応するリソースには有効なキャッシュがありません。ユーザーがアクセス要求を再度開始すると、ノードは必要なリソースをオリジンサーバーから取得し、ノード上にキャッシュします。このため、大量の更新タスクをサブミットすると、かなり多くのキャッシュがクリアされて、back-to-originリクエストが激増し、オリジンサーバーに大きな負荷がかかってしまいます。
...
[CDN コンソール](https://console.cloud.tencent.com/cdn)にログインし、左側のディレクトリの【パージとプリフェッチ】をクリックし、ページに入ってから必要に応じて【URL 更新】および【ディレクトリを更新する】をサブミットします。
...
- 1つのアカウントの1日あたりのURL更新の上限は10000件であり、1回にサブミットできるURL更新の上限は1000件です。海外でのアクセラレーションをアクティブにしているお客様の場合は、1日あたりの海外でのURL更新の上限は10000件となり、国内での上限枠と別々になっています。
...
- 1つのアカウントの1日あたりのディレクトリ更新の上限は100件であり、1回にサブミットできるディレクトリ更新の上限は20件です。海外でのアクセラレーションをアクティブにしているお客様の場合は、1日の海外でのディレクトリ更新の上限は100件となり、国内での上限枠と別々になっています。
- 更新タスクをサブミットする際に、`http://`または`https://`プロトコル識別子を付け加える必要があります。
