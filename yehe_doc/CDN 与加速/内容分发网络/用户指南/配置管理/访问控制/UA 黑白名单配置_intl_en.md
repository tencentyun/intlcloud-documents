...
- Blocklist and allowlist cannot be set at the same time. Please either set blocklist rules or allowlist rules.
- Maximum number of blocklist/allowlist rules: 10
- Rules support the wildcard `*`. Please separate multiple values with `|`.
- Supported effect types: all content, specified file type, file directory, and specified file. Regular matching is currently not supported.
...
When the switch is off, click **Add Rule** to add blocklist/allowlist rules one by one:
...
It matches the blocklist and a 403 error will be directly returned.
