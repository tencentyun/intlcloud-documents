...
### 設定を有効にする
スイッチをクリックし、リンク不正アクセス防止のタイプを選択してリストに入力します。空の referを許可するかにチェックを入れて、【確定】をクリックすれば、リンク不正アクセス防止設定を有効にすることができます。
...
+ リンク不正アクセス防止は、ワイルドカードマッチングをサポートします。つまり、*.qq.comが設定されている場合、`www.qq.com`と`a.qq.com`の両方がマッチします。
