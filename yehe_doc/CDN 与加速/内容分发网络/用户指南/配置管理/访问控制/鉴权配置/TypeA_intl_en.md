...
## Notes:
**Cache hit rate**
If you have enabled the TypeA authentication mode for a domain name, the access URL will carry the authentication parameter. When a CDN node caches the resource, it will automatically ignore the corresponding parameter and thus not affect the cache hit rate.
...
For example, the TypeA configuration here is as: "Authentication Parameter: `sigh`"; "Authentication Scope: `jpg`"; then the `sign` parameter will be automatically filtered for JPG files even though the configuration is as "All Files: Not filter" in **Cache Configuration** -> **Cache Key Rule Configuration**.
