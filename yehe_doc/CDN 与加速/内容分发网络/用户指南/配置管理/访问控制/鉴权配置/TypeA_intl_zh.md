...
**自定义有效时间**：通过请求中携带的 timestamp，加上配置的有效时间，与当前时间进行对比，判定请求是否过期，若过期则直接返回403，有效时间单位为秒。
...
>!因配置后会自动忽略对应的参数，即会过滤配置的鉴权参数，所以会影响鉴权范围内文件的缓存键，且此处的优先级高于【缓存配置 - 缓存键规则配置】处的缓存键规则。
