...
+ Content-Disposition: it activates download in the client and sets the default filename of the downloaded file.
...
#### Content-Language
`Content-Language` specifies the code of the language used by the webpage and can be set as follows:
...
`Access-Control-Allow-Origin` is used to solve the problem of cross-origin permissions of resources. Its value specifies the origins that can access the resource. You can also set the wildcard "\*" to allow all origins to access the resource. The common configuration is as follows:
- `Access-Control-Allow-Origin: *`
...
+ Custom request headers are used.
