...
>- 批量发送内置自动 warm up 功能，一开始批量发送邮件时，控制台会智能判断发信域名/ IP 的信誉度并分配当日最大发信量。当发信超过当日分配的最大发信量时，将会自动中断邮件发送，剩余未发送邮件进入队列缓存中，并将延迟至当前时刻24小时后自动发送。未预热好的域名/ IP 日发信配额参见 [标准版 warm up 计划](https://intl.cloud.tencent.com/document/product/1084/43285)。
