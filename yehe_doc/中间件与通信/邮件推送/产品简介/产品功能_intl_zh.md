[](id:senderConfig)
## 发件人配置选项
邮件推送目前提供了两种发送电子邮件的方式，包括腾讯云 SES 控制台和腾讯云 SES  API。您可以使用 SES 命令行界面（SES  CLI）或使用 [SES 开发工具包（SDK）来访问 API](https://intl.cloud.tencent.com/document/product/1084/39387)。
...
### 功能说明
自动 warm up 分为 [标准版规则（默认）](#default) 和 [自定义规则](#customize) 两档。
