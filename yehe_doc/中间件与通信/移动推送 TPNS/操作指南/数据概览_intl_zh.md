...
- 昨日活跃用户数（DAU）：昨日 SDK 累计联网的去重设备数（应用前台在线或后台进程在线时 SDK 会连接至 TPNS 服务器）。
...
- 昨日联网设备数：昨日 SDK 累计连接TPNS服务器的去重设备数（应用前台在线或后台进程在线时，SDK 会连接至 TPNS 服务器）
...
- 日活跃用户数（DAU）：SDK 每日累计联网的去重设备数（应用前台在线或后台进程在线时 SDK 会连接至 TPNS 服务器）。
- 通知栏开启数：终端通知栏开启的设备数。
