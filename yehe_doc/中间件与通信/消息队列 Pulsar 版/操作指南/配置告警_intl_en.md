...
TDMQ for Pulsar allows you to monitor the resources created under your account, so that you can keep track of the status of your resources in real time. You can configure alarm rules for monitoring metrics. When a monitoring metric reaches the set alarm threshold, Cloud Monitor will notify you of exceptions in time via email, SMS, WeChat, phone, etc.
...
3. On the **Alarm Policy** page, select a policy type and instance and set the alarm rule and notification template.
   - **Policy Type**: select **TDMQ alarm**.
   - **Alarm Object**: select the TDMQ for Pulsar resource for which to configure the alarm policy.
