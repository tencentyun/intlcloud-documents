...
2. Pilih **Global SMS** > **Body Templates** (SMS Global > Template Isi) di bilah sisi kiri dan klik **Create Body Template** (Buat Template Isi).
3. Atur parameter berikut sesuai kebutuhan dan sesuai dengan [standar tinjauan template isi]:
...
1. Pada halaman [Template Isi](https://console.cloud.tencent.com/smsv2/isms-template), Anda dapat melihat informasi template isi.
 ...
 - Status/Reason (Status/Alasan): status template isi, yang meliputi **pending review** (menunggu tinjauan), **rejected** (ditolak), dan **approved** (disetujui). Jika status template isi adalah **rejected** (ditolak), Anda dapat mengeklik **View Details** (Lihat Detail) untuk melihat alasan atau saran mendetail.
 - Application Time (Waktu Pengajuan): waktu saat template isi dibuat.
...
1. Pada halaman [Template Isi](https://console.cloud.tencent.com/smsv2/isms-template), klik **Delete** (Hapus) pada baris template isi target.
