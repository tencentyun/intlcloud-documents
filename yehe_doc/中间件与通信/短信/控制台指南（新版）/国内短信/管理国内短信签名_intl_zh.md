...
3. 结合实际情况和
   ...
   - 签名类型：短信签名的类型。
     企业认证用户可选择【公司】、【App】、【网站】、【公众号、小程序】、【商标】或【政府/机关事业单位/其他机构】。
   ...
   - 授权委托：仅当【签名用途】设置为他用时，出现该参数。
     i. 下载并填写 [授权委托书](https://sms-1258344699.cos.ap-guangzhou.myqcloud.com/Declaration%20of%20Authorisation%20(SMS%20Signature).docx)，授权委托书中被授权方需为当前腾讯云账号实名认证的个人/企业名称全称，授权方需为签名所属主体公司名全称。
     ii. 在授权委托书上加盖授权方公章。
     iii. 上传加盖授权方公章的授权委托书照片或扫描件，支持上传 jpg 或 png 格式的图片，图片不超过5Mb。
   - 申请说明：申请【App】、【网站】以及【公众号或小程序】类签名时，需按 签名审核标准 填写对应内容。申请其他签名时，该参数选填。
...
- 内容：短信签名的实际内容，已审核通过的签名内容可指定为使用 API 或 SDK 发送短信时的`sign`参数值，未经指定则默认使用第一个审核通过的签名内容。
   - 状态/原因：签名的状态，包括【待审核】、【未通过】以及【已通过】。若签名处于【未通过】状态，可单击【查看失败原因并修改】了解详细的未通过原因或建议。
- 申请时间：创建签名的时间。
