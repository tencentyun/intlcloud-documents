...
![](https://main.qcloudimg.com/raw/47341a0f76425641a0c4b20f513912fd.png)
 ...
 - 时间粒度：根据消息量的大小，选取汇聚消息的时间间隔，时间间隔为5 - 60分钟不等。为保证转储性能，聚合文件数量与Partition数量，partition_max 设置数值有关，具体详见文档底部产品限制说明。
 - 存放Bucket：对不同的 topic，选取相应的 COS 中 Bucket，则请求消息会自动在 Bucket 下创建 instance-id/topic-id/date/timestamp为名称的文件路径进行存储。相关路径如无法满足业务需要，请创建完成后在云函数 CkafkaToCosConsumer 下自行修改。
...
通用转储模板常见转储应用场景有 [Elasticsearch](https://intl.cloud.tencent.com/product/es)、[MySQL](https://intl.cloud.tencent.com/product/cdb)、PostgreSQL 等。
...
- 使用 COS 消息转储必须至少拥有一个 VPC 网络环境，如在创建时选择基础网络请参考 [路由接入方式](https://intl.cloud.tencent.com/document/product/597/32555)  绑定 VPC 网络。
