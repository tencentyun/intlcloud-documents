...
该任务指导您在使用消息队列 CKafka 时，通过控制台配置路由接入规则，增强对公网/内网传输中的用户网络访问控制。关于公网的访问请参考 [配置 ACL 策略](https://intl.cloud.tencent.com/document/product/597/39084)。

>?
>- 消息队列 CKafka 支持 SASL_SSL 和 SASL_PLAINTEXT 认证方式，其中 SASL_SSL 接入方式仅专业版实例支持，需要 [提交工单](https://console.cloud.tencent.com/workorder/category) 申请使用（具体下发过程：提交工单申请 ->  下发证书到 broker 机器 -> 提供给用户一个证书供客户端消费）。
