...
> ?
> 
> - Kafka 提供了多种安全认证机制，主要分为 SSL 和 SASL2 大类，其中 SASL/PLAIN 是基于账号密码的认证方式，比较常用。CKafka 支持 SASL_PLAINTEXT 认证（参见 [添加路由策略-公网域名接入](https://intl.cloud.tencent.com/document/product/597/32555)）。
...
- **预设规则：**预设一套策略，后续创建 Topic 的时候可以自动应用这套规则。
  ...
  后续处理：完成授权后，用户可以通过 SASL 接入点接入消息队列 CKafka 并使用 PLAIN 机制消费消息（参见 <a href="https://www.tencentcloud.com/document/product/597/40049">SDK 文档</a>）。
