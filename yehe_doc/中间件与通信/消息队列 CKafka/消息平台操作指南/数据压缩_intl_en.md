...
## Message Format
Currently, Kafka supports two message formats, i.e., v1 and v2 (introduced in v0.11.0.0). CKafka supports version 0.9, 0.10, 1.1, 2.4, and 2.8.
...
- Zero Copy: This feature can prevent costly data copy in kernel state when data is transmitted in disks or over networks to implement fast data transmission.
...
As shown by analysis, this is because that the source data content varies by business, resulting in different performance of the compression algorithm. Therefore, we recommended users who are concerned about CPU metrics use the more stable Snappy compression algorithm.
...
// Note that different versions have different configurations. Currently, versions 0.9 and below do not support compression, and versions 0.10 or later do not support Gzip compression.
