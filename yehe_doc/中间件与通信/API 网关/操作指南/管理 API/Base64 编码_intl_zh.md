...
## 注意事项
- 对于每次成功触发 Base64 编码的请求，API 网关不仅会对请求体进行 Base64 编码，还会把 isBase64Encoded 字段的值设置为 True 一起传递给云函数，该字段可用于通知云函数本次请求是否经过 Base64 编码（后端对接云函数的结构体请参考 [API 网关传递给后端的结构体](https://intl.cloud.tencent.com/document/product/628/38896)）。
- 由于触发器限制，API 网关单次传递给云函数的请求内容不能大于 6MB。因此，本功能只适用于传递 Base64 编码后小于 6MB 的文件，传递大于 6MB 的文件请您参考 [上传文件](https://intl.cloud.tencent.com/document/product/628/38852) 采用 COS 方式上传。
