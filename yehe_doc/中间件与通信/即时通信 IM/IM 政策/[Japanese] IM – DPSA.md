...
本モジュールは、お客様がインスタントメッセージング（「**機能**」）を使用する場合に適用されます。このモジュールは、https://intl.cloud.tencent.com/document/product/301/17347 （「**DPSA**」）にあるデータプライバシーおよびセキュリティ契約に組み込まれています。本モジュールで使用されているが定義されていない用語は、DPSAで定義された意味を持つものとします。DPSA と本モジュールの間に矛盾が生じた場合、その矛盾の範囲内で本モジュールが適用されるものとします。
