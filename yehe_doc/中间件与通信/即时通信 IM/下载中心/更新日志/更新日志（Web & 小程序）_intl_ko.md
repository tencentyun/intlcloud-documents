...
Windows WeChat이 미니 프로그램 및 uni-app 패키지 Android app(일부 기기)에 액세스할 때 로그인 실패 발생.
...
- 오디오/비디오 그룹의 새 구성원이 그룹에 가입하기 전에 기록 메시지를 볼 수 있도록 함(기능을 사용하려면 플래그십 에디션 패키지를 활성화해야 함).
...
- Tencent Cloud IM 업로드 플러그인 [tim-upload-plugin](https://www.npmjs.com/package/tim-upload-plugin) 지원. 이 플러그인은 보다 안전한 파일 업로드를 가능하게 하고 Web, WeChat, QQ, Baidu, Toutiao 및 Alipay 미니 프로그램 플랫폼을 지원하며 26KB에 불과합니다. 자세한 내용은 [registerPlugin](https://web.sdk.qcloud.com/im/doc/en/SDK.html#registerPlugin)을 참고하십시오.
...
WeChat 개발자 툴의 기본 라이브러리 2.14.1에 [tim-wx-sdk.js](https://www.npmjs.com/package/tim-wx-sdk)를 가져올 때 컴파일 오류 발생.
...
- WeChat, QQ, Baidu, Toutiao 및 Alipay 미니 프로그램 플랫폼 지원(현재 Baidu, Toutiao 및 Alipay 미니 프로그램 플랫폼에서 COS에 업로드해야 하는 이미지, 비디오 또는 파일 메시지 또는 기타 메시지는 보낼 수 없습니다).
...
- WeChat 미니 프로그램 플러그인 환경에서 실행 지원.
...
**오류 수정**
- WeChat 미니프로그램이 백그라운드에서 오랜 시간 머물다가 포그라운드로 나갈 때 메시지 간헐적 손실.
