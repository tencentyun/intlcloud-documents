...
- 미니프로그램의 프런트/백그라운드 전환과 같은 시나리오에서 가끔 발생하는 그룹 대화 손실 문제.
...
- [TIM.TYPES.KICKED_OUT_REST_API](https://web.sdk.qcloud.com/im/doc/en/module-TYPES.html#.KICKED_OUT_REST_API) 유형 추가, REST API kick 지원.
...
- 일부 모델에서 미니프로그램을 디버깅할 때 발생한 `Cannot add property markTimeline, Object is not extensible` 문제.
...
uni-app 패키지 Android app(일부 기기)에 액세스할 때 로그인 실패 발생.
...
- .image 이미지를 보내기 위한 Alipay 미니프로그램 지원 추가.
...
- Baidu 미니프로그램 및 기타 플랫폼에 대한 uni-app 패키지 앱에서 발생하는 `__wxConfig is not defined` 문제.
...
- 오디오/비디오 그룹(AVChatRoom)이 [콘솔](https://console.cloud.tencent.com/im)에서 생성되고 그룹 소유자가 지정된 후 [Sending System Messages in a Group](https://intl.cloud.tencent.com/document/product/1047/34958)을 위해 REST API에서 보낸 메시지는 그룹 소유자가 그룹에 가입한 후 그룹 소유자 측에서 반복됨.
...
- Tencent Cloud IM 업로드 플러그인 [tim-upload-plugin](https://www.npmjs.com/package/tim-upload-plugin) 지원. 이 플러그인은 보다 안전한 파일 업로드를 가능하게 하고 Web, Baidu, Toutiao 및 Alipay 미니 프로그램 플랫폼을 지원하며 26KB에 불과합니다. 자세한 내용은 [registerPlugin](https://web.sdk.qcloud.com/im/doc/en/SDK.html#registerPlugin)을 참고하십시오.
...
마이크로 개발자 툴의 기본 라이브러리 2.14.1에 [tim-wx-sdk.js](https://www.npmjs.com/package/tim-wx-sdk)를 가져올 때 컴파일 오류 발생.
...
- Baidu, Toutiao 및 Alipay 미니 프로그램 플랫폼 지원(현재 Baidu, Toutiao 및 Alipay 미니 프로그램 플랫폼에서 COS에 업로드해야 하는 이미지, 비디오 또는 파일 메시지 또는 기타 메시지는 보낼 수 없습니다).
...
[TIM.create](https://web.sdk.qcloud.com/im/doc/en/TIM.html#.create) API는 `oversea` 매개변수를 추가합니다. 이 매개변수가 `true`로 설정되면 SDK는 간섭을 피하기 위해 중국 본토 외부의 도메인 이름 사용.
...
- 오디오/비디오 그룹은 오랜 시간 연결이 끊긴 후 네트워크가 다시 연결되거나 미니프로그램이 백그라운드에서 오랜 시간 실행된 후 포그라운드로 전환되는 경우 메시지를 가져오지 못하는 간헐적 문제.
...
**오류 수정**
- [콘솔](https://console.cloud.tencent.com/im)에서 오디오/비디오 그룹(AVChatRoom)을 만들고 그룹 소유자를 지정한 후 그룹 소유자가 그룹에 가입한 후 다른 그룹 구성원이 보낸 메시지가 그룹 소유자 측에서 반복됨.
...
### 2.2.0 @2019.11.21
**새로운 기능**
- 미니 프로그램은 [createVideoMessage](https://web.sdk.qcloud.com/im/doc/en/SDK.html#createVideoMessage) API를 통한 비디오 메시지 생성 및 전송 지원. 비디오 메시지는 플랫폼 간에 동기화 가능. 최신 버전의 [TUIKit 및 SDK](https://intl.cloud.tencent.com/document/product/1047/33996)로 업데이트해야 함.
...
### 2.1.1 @2019.10.18
**새로운 기능**
미니 프로그램은 [오디오 메시지 전송](https://web.sdk.qcloud.com/im/doc/en/SDK.html#createAudioMessage) 지원. 오디오 메시지는 플랫폼 간에 동기화 가능. 최신 버전의 [TUIKit 및 SDK](https://intl.cloud.tencent.com/document/product/1047/33996)로 업데이트해야 함.
...
**새로운 기능**
- Web & 미니 프로그램은 [오디오 메시지](https://web.sdk.qcloud.com/im/doc/en/Message.html#.AudioPayload) 수신 지원.
- Web & 미니 프로그램은 [비디오 메시지](https://web.sdk.qcloud.com/im/doc/en/Message.html#.VideoPayload) 수신 지원.
...
**오류 수정**
- 미니 프로그램에서 이미지 메시지를 보낼 때 화면 깜빡거림.
- JPG 또는 기타 이미지 전송 실패.
