...
- 친구 설명 수정 후 계정의 각 터미널에서 친구 정보 업데이트 콜백이 되지 않는 문제 수정
...
- 울티메이트 버전 활성화 시 AVChatRoom 라이브 그룹 입장 이전의 20개의 메시지 반환 지원
...
- 울티메이트 버전 Community 지원 추가(최대 10만명)
...
_ Windows SDK에서 새 메시지 콜백이 트리거될 때 클라이언트 스레드가 SDK 논리 스레드를 차단할 수 있는 문제 수정
...
- 그룹 이름/공지 사항/소개에서 민감한 단어 필터링에 대한 지원 추가
...
- 친구 추천 삭제
- APNS는 닉네임 푸시
- 그룹 이름에서 민감한 단어를 필터링하기 위한 지원 추가
- 그룹 공지사항에서 민감한 단어를 필터링하기 위한 지원 추가
...
- 메시지의 광고 및 민감한 단어 필터링에 대한 지원 추가
