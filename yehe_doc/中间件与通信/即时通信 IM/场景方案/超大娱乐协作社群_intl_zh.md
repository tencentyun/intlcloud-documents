...
## 终端 SDK 集成指南
>!社群（Community）话题（Topic）功能仅 IM 终端 SDK 6.2.2363 增强版及以上版本支持，需 [购买旗舰版](https://www.tencentcloud.com/document/product/1047/34577) 并 [申请开通](https://intl.cloud.tencent.com/document/product/1047/44322) 后方可使用。

下文以 Android 为例，介绍社群话题的接口功能。

### 下载并配置 Demo 源码快速体验
您可参见 [快速入门](https://intl.cloud.tencent.com/document/product/1047/45914)，快速体验 IM 的功能（社群话题的 UI 组件正在开发中，敬请期待）。
下面从 API 调用的角度来介绍下社群话题的使用：

### 社群话题的 API 使用
1. 首先调用接口 createGroup 创建支持话题的社群，可以参见 [社群管理](https://intl.cloud.tencent.com/document/product/1047/48172) “创建社群”步骤来实现。
2. 然后通过调用 getJoinedCommunityList 接口，可以获取到创建和加入的该类社群列表。
>!社群用来管理群成员，但不可以在社群中收发消息。社群的其他功能可以参考“其他管理接口”列表。
>
3. 成功创建社群后，可以在社群下调用 createTopicInfoCommunity 创建多个话题，参见 [话题管理]https://intl.cloud.tencent.com/document/product/1047/48172) 中“创建话题”步骤实现。
4. 话题的管理还包括“删除话题”、“修改话题信息”、“获取话题列表”、“监听话题回调”功能。同时，话题也是可以供用户交流的地方，因此可以进行收发消息，相关接口可以参见 [话题消息](https://intl.cloud.tencent.com/document/product/1047/48172) 介绍。

## Web  SDK 集成指南
>!社群（Community）话题（Topic）功能仅 IM Web SDK v2.19.1 及以上版本支持，需 [购买旗舰版](https://www.tencentcloud.com/document/product/1047/34577) 并 [申请开通](https://intl.cloud.tencent.com/document/product/1047/44322) 后方可使用。
>
社群话题的接口功能如下：
### 下载并配置 Demo 源码快速体验
您可参见 [快速入门](https://intl.cloud.tencent.com/document/product/1047/45912)，快速体验 IM 的功能。
下面从 API 调用的角度来介绍下社群话题的使用：

### 社群话题的 API 使用
1. 首先调用接口 createGroup 创建支持话题的社群，可以参见 [社群管理](https://intl.cloud.tencent.com/document/product/1047/48171) “创建社群”步骤来实现。
2. 然后通过调用 getJoinedCommunityList 接口，可以获取到创建和加入的该类社群列表。社群用来管理群成员，但不可以在支持话题的社群中收发消息。社群的其他功能可以参考普通群组API。
3. 成功创建社群后，可以在社群下调用 createTopicInfoCommunity 创建多个话题，参见 [话题管理](https://intl.cloud.tencent.com/document/product/1047/48171) “创建话题”步骤实现。
...
- [集成 SDK（Web & 小程序 & uni-app）](https://intl.cloud.tencent.com/document/product/1047/34309)
