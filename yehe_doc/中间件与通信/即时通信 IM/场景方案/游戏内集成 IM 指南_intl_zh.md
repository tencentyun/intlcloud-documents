...
>
>  - **用户资料**：用户资料包含标配字段和自定义字段。自定义字段可见下文的拓展资料。目前即时通信 IM 支持的用户资料标配字段可参见 [用户资料标配资料字段](https://intl.cloud.tencent.com/document/product/1047/33520)。
...
>  </tr>
>  </tbody></table>
>  更详细内容请参见 <a href="https://intl.cloud.tencent.com/document/product/1047/33521">好友标配字段</a>。
>  - **自定义资料**：自定义资料字段可以通过即时通信 [IM 控制台](https://console.cloud.tencent.com/im) > 应用配置 > 功能配置 申请，提交申请后，自定义资料字段将在5分钟内生效。
>   - 更多关于用户资料管理，请参见 [用户资料管理](https://intl.cloud.tencent.com/document/product/1047/33520)。
>   - 更多关于好友关系链自定义资料管理，请参见 [好友自定义字段](https://intl.cloud.tencent.com/document/product/1047/33521)。
>
>  #### IM 用户资料存储限制
>  - **业务特性限制**
...
>   - 其他调用频率：最高200次/秒
>
>  更多关于使用限制请参见 [使用限制](https://intl.cloud.tencent.com/document/product/1047/34381)。
>
>  ### 邮件系统
>
...
>  ```
>  关于更多有关内容请参见 [Unity-接收消息](https://intl.cloud.tencent.com/document/product/1047/48573)。
...
>  </thead>
>  <tbody><tr>
>  <td><a href="https://intl.cloud.tencent.com/document/product/1047/34919">单发单聊消息</a></td>
>  <td>向指定账号发消息，接收方看到的发送着不是管理员，而是管理员指定的账号</td>
>  <td>向某个特定用户发送消息，如段位赛奖励等</td>
>  </tr>
>  <tr>
>  <td><a href="https://intl.cloud.tencent.com/document/product/1047/34920">批量发单聊消息</a></td>
>  <td>支持一次最多对500个用户单发消息，最高调用频率为200次/秒</td>
>  <td>向某些特定用户发送消息，因无需创建组使用比较灵活，但若发送用户较多则需要分批发送</td>
>  </tr>
>  <tr>
>  <td><a href="https://intl.cloud.tencent.com/document/product/1047/34959">在群组中发送普通消息</a></td>
>  <td>向群组发送普通消息，需要将用户添加到同一个组内</td>
>  <td>向较多用户发送消息可以系统创建群组发送普通消息，群组最大人数限制为 社交群(community) 10万人</td>
>  </tr>
>  <tr>
>  <td><a href="https://intl.cloud.tencent.com/document/product/1047/37165">全员推送服务</a></td>
>  <td>向 App 内全员推送消息，可指定用户标签和属性发送消息</td>
>  <td>向 App 内全员推送消息或人数非常多且有特征属性时可使用，如活动推送邮件等</td>
>  </tr>
>  </tbody></table>
>  <dx-alert infotype="explain" title="">
>  “全员推送” 为 IM 旗舰版功能，需 [购买旗舰版](https://www.tencentcloud.com/document/product/1047/34577) 并在 [控制台](https://console.cloud.tencent.com/im/login-message) > 功能配置 > 登录与消息 > 全员推送设置，打开开关后方可使用。
>  </dx-alert>
...
>  });
>  ```
>  更多拉取历史消息内容，请参见 [历史消息-Unity](https://intl.cloud.tencent.com/document/product/1047/48871)。
>
>  #### 邮件未读计数
>
...
>  });
>  ```
>  更多内容请参见 [Unity-会话未读消息数](https://intl.cloud.tencent.com/document/product/1047/48846)。
>
>  #### 全员邮件
...
>
>  更多相关全员推送的内容请参见 [全员推送](https://intl.cloud.tencent.com/document/product/1047/37166)。
>
>  #### 邮件有效期
>  对于历史邮件，邮件存储有效期为：体验版/专业版为7天，旗舰版为30天，专业版和旗舰版支持延长时长。对于历史存储的更多内容请参见 [历史消息存储时长配置](https://intl.cloud.tencent.com/document/product/1047/34419)。
>  此外，在服务端发送消息时，可通过在请求包内设置 `MsgLifeTime` 设置消息离线保存时长（最长为7天）。若该字段为0，则表示消息只发在线用户，不离线保存。 更多内容请参见 [服务端 API](https://intl.cloud.tencent.com/document/product/1047/34919)。
>
>  ### 临时组队
>
...
>  }
>  ```
>  >? App 中 同时存在的所有群组数量最多为10万，若超过10万则需要支付一定费用。详情请参见 [价格说明](https://intl.cloud.tencent.com/document/product/1047/34350)。更多关于服务端创建群组请参见 [创建群组](https://intl.cloud.tencent.com/document/product/1047/34895)。
...
>  }
>  ```
>  更多内容请参见 [增加群成员](https://intl.cloud.tencent.com/document/product/1047/34921)。
...
>  }
>  ```
>  更多相关内容请参见 [群组满员之后回调](https://intl.cloud.tencent.com/document/product/1047/34376)。
...
>  }
>  ```
>  >? 要启用回调，必须配置回调URL，并打开本条回调协议对应的开关。配置方法参见 [第三方回调配置](https://intl.cloud.tencent.com/document/product/1047/34520) 文件。更多相关内容请参见 [新成员入群之后回调](https://intl.cloud.tencent.com/document/product/1047/34372)。
>  >
>  - **游戏途中退出组队**：当玩家自主退出游戏或由于网络问题退出游戏时，服务端可以通过 `群成员离开之后回调`通知组内其他玩家有人退出群聊，也可以对因网络问题退出游戏的玩家发送消息。[群成员离开之后回调](https://intl.cloud.tencent.com/document/product/1047/34373) 的请求 URL 示例如下：
>  ```
>  https://www.example.com?SdkAppid=$SDKAppID&CallbackCommand=$CallbackCommand&contenttype=json&ClientIP=$ClientIP&OptPlatform=$OptPlatform
>  ```
>  请求包示例如下：
...
>  }
>  ```
>  - **游戏结束后解散群聊**：游戏结束后，服务端可直接解散群聊。[解散群聊](https://intl.cloud.tencent.com/document/product/1047/34896) 的请求 URL 示例如下：
>  ```
...
>  }
>  ```
>  更多详细信息请参见 [获取群详细资料](https://intl.cloud.tencent.com/document/product/1047/34961)。
...
>  >? 该接口不支持直播群（AVChatRoom）。更多详细信息请参见 [获取群成员详细资料](https://intl.cloud.tencent.com/document/product/1047/34948)。
>
>
>  #### 队内成员获取队内情况
>
...
>  ```
>  >?
>  >- 详细内容请参见 [Unity-获取群成员资料](https://intl.cloud.tencent.com/document/product/1047/50320) 。
>  >- 其他关于群信息如`群满员开始游戏`、`有成员加入/退出组队`等可参见  [组队场景](#Team_scene) 并使用回调通知到群内。
>  >- 更多有关群组选择和关于群组的其他事项，请参见 [群组系统](https://intl.cloud.tencent.com/document/product/1047/33529) 。更多有关**群组管理的控制台指南**，请参见 [群组管理控制台指南](https://intl.cloud.tencent.com/document/product/1047/34578)。
>
>  ### 敏感信息过滤
>
...
>
>  腾讯云 IM 提供 9 种基本消息类型，包含文本消息、表情消息、地理位置消息、图片消息、语音消息、文件消息、短视频消息、系统通知和自定义消息。其中除了自定义消息之外的其他消息格式已经确定，只需要填入相应的信息即可。更多关于消息类型的具体描述请参见 [消息类型](https://intl.cloud.tencent.com/document/product/1047/33523)，关于消息类型的格式请参见 [消息格式描述](https://intl.cloud.tencent.com/document/product/1047/33527)。
>
>  服务端可通过 [单发单聊消息](https://intl.cloud.tencent.com/document/product/1047/34919) 、 [批量发单聊消息](https://intl.cloud.tencent.com/document/product/1047/34920) 、 [在群组中发送普通消息](https://intl.cloud.tencent.com/document/product/1047/34959) 向用户发送自定义消息。在群组中发送普通消息的基础请求包示例如下：
>
>  ```json
>  {
>      "GroupId": "@TGS#2C5SZEAEF",
...
>  ### 组内语音/视频聊天
>
>  游戏内的语音/视频聊天也是一项很重要的功能。腾讯云 IM 提供给予即时通信 IM 和 [实施音视频品视频 TRTC](https://www.tencentcloud.com/document/product/647) 音视频通话功能（TUICallKit），为 IM 聊天提供实施的音频通话或视频通话。
>
>  >? 
...
>  >- 更多关于 TUICallKit 的内容，请参见 [含 UI 集成方法-音视频通话](https://www.tencentcloud.com/document/product/1047/50025)。
>
>
>  ### 游戏聊天室类型
>
...
>
>  >?
>  >- 关于好友聊天和私信聊天的单聊权限控制请参见 [单聊消息权限控制](https://intl.cloud.tencent.com/document/product/1047/33523#.E5.8D.95.E8.81.8A.E6.B6.88.E6.81.AF.E6.9D.83.E9.99.90.E6.8E.A7.E5.88.B6%EF%BC%89)。
>  >- 更多群组系统相关内容请参见 [群组系统](https://intl.cloud.tencent.com/document/product/1047/33529)。
