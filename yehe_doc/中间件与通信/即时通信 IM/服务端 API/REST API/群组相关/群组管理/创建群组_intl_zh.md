...
>- App 中同时存在的所有群组类型群数量加和超过10万，需要支付一定费用。详情请参考 [价格说明](https://intl.cloud.tencent.com/document/product/1047/34350)。
>- 因为用户帐号可以同时加入的群组数量是有限制，用户已加入群数量达到上限时会造成加群/建群失败，详情请参考 [价格说明](https://intl.cloud.tencent.com/document/product/1047/34350)。
...
- **基础形式**
创建群，Owner_Account 字段是选填的，如果需要指定群主，请确保 [已经导入](https://intl.cloud.tencent.com/document/product/1047/34953) 群主 UserId 。不填则该群没有群主。
```json
{
...
| 字段 | 类型 | 属性 |说明|
|---------|---------|---------|---------|
| Owner_Account | String | 选填 |群主 ID（需是 [已导入](https://intl.cloud.tencent.com/document/product/1047/34953) 的帐号）。填写后自动添加到群成员中；如果不填，群没有群主；成员使用 AVChatroom（直播群）时，必须每次调用进群操作 |
