## Feature Overview
 The app admin can use this API to update (set/increment/decrement) group counters.
...
```
{
    ...
    "Mode": "Set"  // `Set`, `Increase`: Increment, `Decrease`: Decrement
}
```
...
| Mode   | String | No | `Increase` and `Decrease` indicate incrementing or decrementing the original value respectively. If the `Key` does not exist, create one with `Value` being 0 and then increase or decrease the value. `Set` indicates setting `Value` corresponding to the `Key`. |
