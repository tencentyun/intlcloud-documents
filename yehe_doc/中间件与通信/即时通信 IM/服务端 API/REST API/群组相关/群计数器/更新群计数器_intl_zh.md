...
```
{
    ...
    "Mode": "Set"  // Set:设置; Increase:递增; Decrease:递减;
}
```
...
| GroupCounter | Array | 必填 | 群计数器列表，Key 为群计数器的键，Key 数量不超过20个，Key 长度不超过128个字节；Value 为群计数器的值，最大支持 64 位有符号整型； |
| Mode   | String | 选填 | Increase/Decrease 表示基于原来的 Value 进行递增递减，如果 Key 不存在，则先创建一个 Value 为0的 Key，然后递增递减。Set 表示直接设置 Key 对应的Value |
...
| GroupCounter | Array | 更新的群计数器最新键值列表 |
