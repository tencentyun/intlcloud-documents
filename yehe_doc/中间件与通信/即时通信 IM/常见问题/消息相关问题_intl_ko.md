...
현재 IM은 [TPNS](https://intl.cloud.tencent.com/product/tpns)에서 제공하는 벤더 jar 패키지를 사용합니다. [IM 오프라인 푸시(Android)](https://intl.cloud.tencent.com/document/product/1047/34336) 문서를 참고하여 관련 종속성 패키지를 교체하면 이 문제를 해결할 수 있습니다.
...
상기 방법으로 C2C 메시지, 그룹 메시지 관련 문제를 파악할 수 없다면 다음의 경우를 확인하십시오.
...
- APNs
오프라인 푸시(iOS) 문서를 참고하여 다음 사항을 확인하십시오.
...
- Android
오프라인 푸시 문서를 참고하여 다음 사항을 확인하십시오.
...
3. 알림 음소거를 활성화했는지 확인합니다. iOS의 경우 사용자 지정 푸시 알림음 설정 방법을, Android의 경우 글로벌 오프라인 푸시 구성 설정 방법을 참고하십시오.
4. 메시지가 ‘sendOnlineMessage’ API를 통해 전송된 온라인 메시지인지, 또는 RESTful API를 통해 푸시할 때 ‘MsgLifeTime’이 ‘0’으로 설정되었는지 확인합니다.
5. 메시지에 오프라인 푸시 비활성화 식별자가 설정되었는지 확인합니다. iOS의 경우 오프라인 메시지 속성 사용자 정의 문서를, Android의 경우 단일 메시지 오프라인 푸시 구성 설정 방법을 참고하십시오.
