...
IM terminal users need to obtain the latest messages at any time. However, due to the limited performance and battery power of mobile devices, when the app is running in the background, IM recommends that you use the system-grade push channels (APNs) provided by Apple for message notifications to avoid excessive resource consumption caused by maintaining a persistent connection. Compared with third-party push, APNs provides more stable system-grade persistent connections, enabling users to receive push messages at any time and greatly reducing resource consumption.
<dx-alert infotype="notice" title="">
- If you want to continue to receive IM message notifications if, without proactive logout, the app switches to the background, the mobile phone screen is locked, or the app process is killed by a user, you can enable IM offline push.
- If the app proactively calls the `logout` API to log out or is forced to log out due to multi-device login, you cannot receive offline push messages even if IM offline push is enabled.
</dx-alert>
...
1. [Apply for APNs certificates](#step-1.3A-apply-for-an-apns-certificate).
...
1. Log in to the [Apple Developer](https://developer.apple.com/account/) website and click **Certificates, Identifiers & Profiles** in the right pane or **Certificates, IDs & Profiles** in the left sidebar to enter the **Certificates, IDs & Profiles** page.
...
3. You can register an AppID by following the steps below or add the `Push Notification` service to your existing AppID.
   ...
   > ? Your `Bundle ID` cannot contain the wildcard `*`. Otherwise, you will be unable to use the remote push service.
...
7. Check **Push Notifications** to activate the remote push service.
...
2. In the **Apple Push Notification service SSL Certificates** window, you can see two SSL certificates: **Development SSL Certificate** and **Production SSL Certificate**.
   <img src="https://main.qcloudimg.com/raw/bd55cffb96e80b505e70db33c73e27dd.jpg" style="zoom:50%;" />

3. <span id="step3"></span>Click **Create Certificate** under **Development SSL Certificate** to create a certificate. You will be prompted that a Certificate Signing Request (CSR) is required.
   <img src="https://main.qcloudimg.com/raw/637ce37ec54ca5a4bf3006b527572da5.jpg" style="zoom:50%;" />

4. Open **Keychain Access** on Mac. Choose **Keychain Access** > **Certificate Assistant** > **Request a Certificate From a Certificate Authority**.
...
6. Go back to the Apple Developer page shown in [step 3](#step3) and click **Choose File** to upload the `*.certSigningRequest` file.
...
9. Repeat steps 1-8 to generate and download the production SSL certificate locally.
   ...
   > ? The production SSL certificate is in fact a Sandbox+Production certificate that can apply to both the development and production environments.
...
10. Double-click and open the downloaded development SSL certificate and production SSL certificate to import them to Keychain Access.
...
3. Click **Add Certificate** on the right of **iOS Native Offline Push**.
4. Choose the certificate type, upload an iOS certificate (.p12), set the certificate password, and click **OK**.
...
> ? Different apps need to use different push certificates, and you need to [apply for APNs certificates](#step-1.3A-apply-for-an-apns-certificate) for each app and [configure offline push](#Configuring Offline Push).
...
- If you want to customize the badge number, follow the steps below:
 1. Use the app to call the [- (void)setAPNSListener:(id<V2TIMAPNSListener>)apnsListener](https://im.sdk.qcloud.com/doc/zh-cn/categoryV2TIMManager_07APNS_08.html#a62e1694cf9e1d65b76f90064cbcbb683) API to configure listening.
 2. Use the app to implement the [- (uint32_t)onSetAPPUnreadCount](https://im.sdk.qcloud.com/doc/zh-cn/protocolV2TIMAPNSListener-p.html#a164265ae900e0ddeb6d6393786a548ba) API and return the badge number to be customized internally.
...
When calling [sendMessage](https://im.sdk.qcloud.com/doc/zh-cn/categoryV2TIMManager_07Message_08.html#a3694cd507a21c7cfdf7dfafdb0959e56) to send messages, set the `iOSSound` field in [offlinePushInfo](https://im.sdk.qcloud.com/doc/zh-cn/interfaceV2TIMOfflinePushInfo.html) to the name of the audio file.
...
When calling [sendMessage](https://im.sdk.qcloud.com/doc/zh-cn/categoryV2TIMManager_07Message_08.html#a3694cd507a21c7cfdf7dfafdb0959e56) to send messages, set the `AndroidSound` field in [offlinePushInfo](https://im.sdk.qcloud.com/doc/zh-cn/interfaceV2TIMOfflinePushInfo.html) to the name of the audio file.
...
> * For Mi ringtone setting, see https://dev.mi.com/console/doc/detail?pId=1278%23_3_0. 
...
When calling [sendMessage](https://im.sdk.qcloud.com/doc/zh-cn/categoryV2TIMManager_07Message_08.html#a3694cd507a21c7cfdf7dfafdb0959e56) to send messages, set the `title` and `desc` fields in [offlinePushInfo](https://im.sdk.qcloud.com/doc/zh-cn/interfaceV2TIMOfflinePushInfo.html). Once set, the `title` content will be added to the default push content and `desc` will be displayed as the push content.
...
// `receiver` and `groupID` cannot be empty at the same time, and only one of them can be set.
[[V2TIMManager sharedInstance] sendMessage:msg receiver:receiver groupID:groupID priority:V2TIM_PRIORITY_DEFAULT onlineUserOnly:NO offlinePushInfo:pushInfo progress:nil succ:^{
...
When calling [sendMessage](https://im.sdk.qcloud.com/doc/zh-cn/categoryV2TIMManager_07Message_08.html#a3694cd507a21c7cfdf7dfafdb0959e56) to send messages, set the `ext` field in [offlinePushInfo](https://im.sdk.qcloud.com/doc/zh-cn/interfaceV2TIMOfflinePushInfo.html). When the user receives an offline push message and starts the app, the `ext` field can be obtained from the `AppDelegate -> didReceiveRemoteNotification` system callback, and the user is redirected to the UI as specified by `ext`. For more information, see [TUIKitDemo](https://github.com/TencentCloud/TIMSDK/blob/master/iOS/Demo/TUIKitDemo/AppDelegate%2BPush.m).
...
The offline push for custom messages is different from that for common messages. Since we are not able to resolve the content of custom messages and determine what to push, we simply do not push custom messages by default. If you want to enable offline push for custom messages, set the `desc` field in [offlinePushInfo](https://im.sdk.qcloud.com/doc/zh-cn/interfaceV2TIMOfflinePushInfo.html) when calling [sendMessage](https://im.sdk.qcloud.com/doc/zh-cn/categoryV2TIMManager_07Message_08.html#a3694cd507a21c7cfdf7dfafdb0959e56). Then, the content of `desc` will be displayed by default for offline push.

### How do I disable the receiving of offline push messages?

To disable the receiving of offline push messages, set the `config` parameter of the [setAPNS](https://im.sdk.qcloud.com/doc/zh-cn/categoryV2TIMManager_07APNS_08.html#a6aecbdc0edaa311c3e4e0ed3e71495b1) API to `nil`. This feature is supported from v5.6.1200.
