...
2. 当单聊/群聊收到新消息，会话列表需要更新时，可以通过设置会话未读消息总数变更的回调 `SetConvTotalUnreadMessageCountChangedCallback` 中的 `total_unread_count` ([c#](https://comm.qq.com/im/sdk/unity_plus/_site_en/api/com.tencent.imsdk.unity.callback.ConvTotalUnreadMessageCountChangedCallback.html)) 获取到消息未读数。
3. 根据 `ConvInfo` 的 `conv_recv_opt` ([c#](https://comm.qq.com/im/sdk/unity_plus/_site_en/api/com.tencent.imsdk.unity.types.ConvInfo.html#com_tencent_imsdk_unity_types_ConvInfo_conv_recv_opt)) 判断获取到的消息接收选项为 `kTIMRecvMsgOpt_Not_Notify` 时显示小红点而非消息未读数。
