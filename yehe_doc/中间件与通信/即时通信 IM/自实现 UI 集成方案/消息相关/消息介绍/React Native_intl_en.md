...
In the Tencent Cloud IM SDK, the message class is `V2TimMessage` ([TS](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimMessage.html)), which will be frequently used for message sending and receiving.
...
| needReadReceipt           | Whether the message requires a read receipt        | To use it, you need to purchase the Ultimate edition plan. For more information, see [sendMessageReadReceipts](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#sendMessageReadReceipts). |
| priority | Message priority | It applies only to group messages and can be set by calling the `sendMessage` API. |
| groupAtUserList | List of users tagged in the group message | It applies only to group messages and stores the `userID`. For more information, see [createTextAtMessage](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#createTextAtMessage). |
...
| textElem | Text message storage element | It needs to be created and entered when a text message is sent. When the message is received and `elemType` is identified as `TEXT`, the content can be parsed from `textElem`. For more information, see [sendMessage](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#sendMessage) / [onRecvNewMessage](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimAdvancedMsgListener.html#onRecvNewMessage). |
...
| mergerElem | Merged message storage element | It needs to be created and entered when a merged location message is sent. When the message is received and `elemType` is identified as `MERGER`, the content can be parsed from `mergerElem`. For more information, see [createForwardMessage](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#createForwardMessage). |
| groupTipsElem | Group tip message storage element | Currently, a group tip message can be sent only by the server. When the message is received by the SDK and `elemType` is identified as `GROUP_TIPS`, the content can be parsed from `groupTipsElem`. For more information, see [onRecvNewMessage](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimAdvancedMsgListener.html#onRecvNewMessage). |
