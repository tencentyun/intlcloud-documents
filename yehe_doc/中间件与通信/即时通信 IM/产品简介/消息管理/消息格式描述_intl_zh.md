...
|Desc|String|自定义消息描述信息。当接收方为 iOS 或 Android 后台在线时，做离线推送文本展示。<br>若发送自定义消息的同时设置了 [OfflinePushInfo.Desc](https://intl.cloud.tencent.com/document/product/1047/33527) 字段，此字段会被覆盖，请优先填 OfflinePushInfo.Desc 字段。<br><dx-alert infotype="explain" title="说明">
当消息中只有一个 TIMCustomElem 自定义消息元素时，如果 Desc 字段和 OfflinePushInfo.Desc 字段都不填写，将收不到该条消息的离线推送，需要填写 OfflinePushInfo.Desc 字段才能收到该消息的离线推送。
</dx-alert>|
