...
|Flutter|Flutter2 & dart 2.12以降のバージョン|サポート|-|サポート|
...
>?現在使用している中国のサイトは、中国でのデータストレージのみをサポートしています（サービスはグローバルに利用可能です）。インド、シンガポール、韓国、ドイツなどの海外のデータストレージサイトを展開する必要がある場合は、[IMグローバルサイト](https://intl.cloud.tencent.com/login?s_url=https%3A%2F%2Fconsole.intl.cloud.tencent.com%2Fim%3Ffrom%3D15211)にアクセスしてください。
### アカウント機能
...
| 音声メッセージ     | 音声データは秒単位で時間の長さ情報を提供してください   |
...
|グループTipsメッセージ|グループに出入りするメンバー、グループの説明情報の変更、グループメンバープロフィールの変更など、システムの通知メッセージです|
...
| 機能タイプ           | 機能の説明       |
| ------------------ | ---------------- |
| ユーザープロフィールの設定   | ユーザーが自分のニックネーム、検証方法、プロフィール画像、性別、年齢、署名、場所などのプロフィールを設定します |
| ユーザープロフィールの取得  | ユーザーが自分やフレンド、知らない人に関するプロフィールを確認します    |
| フィールドごとのユーザープロフィールの取得 | 特定のフィールドごとにユーザープロフィールを取得します       |
| カスタムユーザープロフィール     | 最大20件のカスタムユーザープロフィールフィールド      |
...
| すべてのフレンドの取得         | すべてのフレンドの取得は、デフォルトでは基本プロフィールのみを取得します       |
...
| フレンドカスタムプロフィールの設定   | 最大20件のフレンドカスタムフィールドです              |
...
| フレンドプロフィール変更のシステム通知 | フレンドプロフィールの変更時には、システムから通知を受信します               |
...
- コミュニティ(Community)：作成後、自由に出入りでき、最大10万人をサポートします。またメッセージ履歴保存、ユーザー検索グループIDをサポートし、グループ追加申請を開始した後、管理者の承認なしにグループに参加できます。
>?
...
<table>
   <tr>
      ...
      <th width="0px" style="text-align:center">友だちワークグループ(Work)</td>
			 <th width="0px" style="text-align:center">知らない人とのソーシャルグル(Public)</td>
...
<td>デフォルトでは、グループ名、グループ通知、グループ紹介、グループプロフィール画像、グループ所有者を変更すると、通知が送信され、ローミングデータが保存されます。グループ発言禁止の変更については、デフォルトで通知が無効になっています。<a href="https://console.cloud.tencent.com/im/qun-setting">コンソール</a>による設定</td>がサポートされています。
<td>デフォルトでは、グループ名、グループ通知、グループ紹介、グループプロフィール画像、グループ所有者を変更すると、通知が送信され、ローミングデータが保存されます。グループ発言禁止の変更については、デフォルトで通知が無効になっています。<a href="https://console.cloud.tencent.com/im/qun-setting">コンソール</a>による設定</td>がサポートされています。
<td>デフォルトでは、グループ名、グループ通知、グループ紹介、グループプロフィール画像、グループ所有者を変更すると、通知が送信され、ローミングデータが保存されます。グループ発言禁止の変更については、デフォルトで通知が無効になっています。<a href="https://console.cloud.tencent.com/im/qun-setting">コンソール</a>による設定</td>がサポートされています。
<td>グループ名、グループ通知、グループ紹介、グループプロフィール画像、グループ所有者を変更すると、通知は送信されるが、ローミングデータは保存されません。グループ発言禁止の変更通知が無効になっています。</td>
<td>デフォルトでは、グループ名、グループ通知、グループ紹介、グループプロフィール画像、グループ所有者を変更すると、通知が送信され、ローミングデータが保存されます。グループ発言禁止の変更については、デフォルトで通知が無効になっています。<a href="https://console.cloud.tencent.com/im/qun-setting">コンソール</a>による設定</td>がサポートされています。
</tr>
<tr>
...
>?申請するときは、Tencent Cloudルートアカウントにログインしてください。
