...
| 기능 유형       | 설명    |
| --- | --- |
| 단일 플랫폼 로그인     | 사용자는 한 번에 Android, iPhone, iPad, Windows, Mac 및 Web 플랫폼 중 하나에서만 온라인 상태가 될 수 있음              |
| 듀얼 플랫폼 로그인(기본 설정) | 사용자는 Android, iPhone, iPad, Windows 또는 Mac 플랫폼과 Web 플랫폼에서 동시에 온라인 상태일 수 있음            |
| 트리플 플랫폼 로그인     | 사용자는 Android, iPhone 또는 iPad 플랫폼, Windows 또는 Mac 플랫폼 및 Web 플랫폼에서 동시에 온라인 상태일 수 있음 |
| 멀티 플랫폼 로그인     | 사용자는 Android, iPhone 또는 iPad 플랫폼, Windows 또는 Mac 플랫폼 및 Web 플랫폼에서 동시에 온라인 상태일 수 있음           |

> ?[IM 콘솔](https://console.cloud.tencent.com/im)에 로그인하여 타깃 앱에 대한 **애플리케이션 구성**을 클릭하여 **기능 구성** 페이지를 열어 다중 장치 로그인을 구성할 수 있습니다.
...
| 멀티 디바이스 동기화 | 메시지는 여러 장치에서 동기화되고 수신됩니다 |
| 기록 메시지 | 로컬 및 클라우드 기록 메시지가 모두 지원됩니다 |
...
| 메시지 포워딩 | 사용자는 메시지를 다른 사용자나 그룹에 포워딩할 수 있습니다 |
| @기능    | 그룹 내의 @메시지와 일반 메시지는 본질적인 차이는 없으며, @대상자가 메시지를 받았을 때 UI에서 특별 처리함 |
| 입력 상태 표시기 | 이 기능은 온라인 메시징 시나리오에서 구현할 수 있습니다 |
...
| 블록리스트 추가     | 사용자 차단 기능. 기존 친구인 경우 친구 관계 해제    |
| 블록리스트에서 제거 | 블록리스트에서 사용자 제거 |
| 블록리스트 가져오기 | 사용자의 블록리스트 가져오기 |
| 비고 | 친구에 대한 설명 추가 |
| 친구 사용자 정의 프로필 설정 | 최대 20개의 친구 사용자 정의 필드 |
| 친구 그룹 생성 | 그룹 생성 시 사용자를 동시 지정하여 그룹화 가능. 한 사용자를 여러 그룹에 추가 가능. |
| 친구 목록 삭제 | 친구 그룹 삭제 |
| 친구 검증 | 친구 일괄 검증 지원 |
| 블록리스트 검증 | 블록리스트 일괄 검증 지원 |
| 그룹에 친구 추가     | 친구 그룹에 친구 추가                |
| 그룹에서 친구 제거     | 친구 그룹에서 친구 제거              |
| 친구 목록 이름 변경 | 친구 그룹 이름 변경 |
| 친구 목록 정보 가져오기 | 특정 친구 목록 가져오기                   |
...
> - 커뮤니티(Community) 기능은 v5.8.1668 이상의 기본 SDK 인핸스드 버전 및 Web SDK v2.17.0 이상에서만 지원됩니다. [플래그십 버전 구매](https://buy.cloud.tencent.com/avc?from=17182) 후 **[콘솔](https://console.cloud.tencent.com/im)** > **기능 설정** > **그룹 설정** > **그룹 기능 설정** > **커뮤니티**에서 활성화해야 합니다.
...
<table>
   ...
   <tr>
      ...
      <td ><li>체험판: 20명/그룹</li><li>프로 버전: 기본 설정값: 200명/그룹, 최대 확장 가능 규모: 2000명/그룹</li><li>플래그십 버전: 기본 설정값: 2000명/그룹, 최대 확장 가능 규모: 6000명/그룹</li></td>
                  <td><li>체험판: 20명/그룹</li><li>프로 버전: 기본 설정값: 200명/그룹, 최대 확장 가능 규모: 2000명/그룹</li><li>플래그십 버전: 기본 설정값: 2000명/그룹, 최대 확장 가능 규모: 6000명/그룹</li></td>
                              <td ><li>체험판: 20명/그룹</li><li>프로 버전: 기본 설정값: 200명/그룹, 최대 확장 가능 규모: 2000명/그룹</li><li>플래그십 버전: 기본 설정값: 2000명/그룹, 최대 확장 가능 규모: 6000명/그룹</li></td>
                                          <td>무제한</td><td>체험판 및 프로 버전은 지원하지 않으며, 플래그십 버전 기본 설정값은 100000명/그룹</td>
   </tr>
   <tr>
     <td>그룹 정보 수정</td>
     <td>
...
| 금일 1:1 메시지량 | 당일 1:1 총 메시지량 |
| 금일 일반 그룹 메시지량 | 당일 오디오 비디오 그룹 외 총 메시지량 |
| 금일 오디오 비디오 그룹 메시지량 | 당일 오디오 비디오 그룹 총 메시지량  |
