...
- 完備された[グループ管理](https://intl.cloud.tencent.com/document/product/1047/33530)機能：グループの作成/解散、参加者管理、グループプロフィール管理、参加者プロフィール管理など。
...
- 一般的なユースケースに基づき、**フレンドワークグループ(Work)**、**知らない人とのソーシャルグループ(Public)**、**臨時ミーティンググループ(Meeting)**、**ライブストリーミンググループ(AVChatRoom)**および**コミュニテ(Community)** という5つのグループタイプがデフォルトで設定されています。
- 拡張可能なグループ参加者上限数：
...
>- コミュニティ（Community）は、エンターテインメントとコラボレーションのための新しいツールです。同じコミュニティで、異なる分類、トピックを分けられるほか、メッセージを相互に分離し、階層的なコミュニケーションを実現します。また、極めて大規模なメンバーを収納し、同じセットの友達関係の使用を可能にして、ユーザーが独特なソーシャル拡張ルートを見つけられるようにサポートします。さらに、効率的なユーザー運営ツールとして利用し、メンバーをグループ別に分類して、メンバーグループの表示、発言、および管理などの権限を設定することもできます。趣味による友達作り、ゲームソーシャル、フォロワーの運営、組織管理などのシナリオに適用できます。
>- コミュニテ(Community)機能は、端末SDK  5.8.1668拡張バージョン以降でのみサポートされています。Web SDK 2.17.0以降のバージョンの場合、[Ultimate版を購入](https://www.tencentcloud.com/document/product/1047/34577)して、[有効化を申請](https://intl.cloud.tencent.com/document/product/1047/44322)した後にのみ使用できます。
...
| グループ参加者のロール | 説明      | 管理権限         |
| ------------ | -------------- | ------------ |
| 一般参加者     | 管理権限をもたないグループ参加者      |   友だちワークグループ(Work)では、一般参加者はグループプロフィール修正の権限を有します|
| 管理者       | グループマスターが任命し、グループマスターに協力してグループを管理するグループ参加者。一定の管理権限を有します。 |<ul style="margin:0;"><li>グループ基本プロフィールの修正</li><li>一般参加者をグループから追放</li><li>一般参加者に対するミュート（その参加者の一定時間の発言を禁止します）</li><li>その他ユーザーのグループ参加申請の審査承認</li></ul>友だちワークグループ(Work)はデフォルトでは管理者の設定をサポートしていません|
...
| グループタイプ   |適用ケース    |
|--------|---------|
| フレンドワークグループ(Work)| 一般的なWeChatグループに似ています。作成後はすでにグループ内にいるフレンドのみを招待して参加させることができ、かつ被招待者側の同意またはグループマスターの承認は不要です。旧バージョンのPrivateと同じです  |
...
|--- |-- |-- |-- |-- |-- |
|利用可能な参加者のロール|グループマスター<br>一般参加者<br>App管理者|グループマスター<br>管理者<br>一般参加者<br>App管理者|グループマスター<br>管理者<br>一般参加者<br>App管理者|グループマスター|グループマスター<br>管理者<br>一般参加者<br>App管理者|
|グループ基本プロフィールの変更権限|一般参加者|グループ管理者 <br>グループ所有者<br>App管理者|グループ所有者<br>App管理者|グループ所有者<br>App管理者|管理者<br>グループ所有者<br>App管理者|
...
>- 友だちワークグループ(Work)については、一般参加者はグループの名前、概要、公告、グループプロフィール画像URLのみ修正可能です。その他のグループの基本プロフィールは修正できません。
...
|機能項目|フレンドワークグループ(Work)|知らない人とのソーシャルグル(Public)|臨時ミーティンググループ(Meeting)|ライブストリーミンググループ(AVChatRoom)|コミュニティ(Community)|
|--- |-- |-- |-- |-- |-- |
...
>- フレンドワークグル(Work)、知らない人とのソーシャルグループ(Public)、臨時ミーティンググループ(Meeting)およびコミュニティ(Community)はメッセージ履歴保存機能が備わっています。デフォルトで、無料で7日間保存（フラッグシップ版はデフォルトで30日）しますが、より長い期間保存したい場合は、[コンソール](https://console.cloud.tencent.com/im)でメッセージ保存期間を修正できます。メッセージ保存期間の延長には、付加価値サービスの費用がかかります。詳細については、[料金説明](https://intl.cloud.tencent.com/document/product/1047/34350)をご参照ください。


### 一括インポートと自動回収の違い

|機能項目|フレンドワークグル(Work)/ 知らない人とのソーシャルグループ(Public)/ 臨時ミーティンググループ(Meeting)/コミュニティ(Community)|ライブストリーミンググループ(AVChatRoom)|
|--- |--- |--- |
...
>!グループ回収機能を有効にする必要がある場合、[チケットテンプレート](https://intl.cloud.tencent.com/document/product/1047/44322)でチケットを提出して申請できます。設定すると、グループタイプによって非アクティブグループをクリアできます（グループが非アクティブであるということは、グループ内でコメントがなく、参加者の変更もないことを指します）。

## グループデータ構造の紹介
### グループ基本プロフィール[](id:GroupBaseInfoFilter)

|フィールド名|タイプ|説明|備考|
|--- |--- |--- |--- |
|GroupId|String|グループの一意性ID|読み取り専用<br>グループIDは、App内での一意性が保証され、その形式はプレフィックスが@TGS#となります。また、AppでのカスタムグループIDの設定が可能です。|
|Type|String|グループタイプ|読み取り専用<br>デフォルトで次のグループタイプをサポートします：フレンドワークグループ(Work)、知らない人とのソーシャルグループ(Public)、臨時ミーティンググループ(Meeting)、ライブストリーミンググル(AVChatRoom)、コミュニティ(Community)。詳細については、[グループタイプの説明](#GroupType)をご参照ください。<br>旧バージョンのSDKには、Private、ChatRoomおよびBChatRoomタイプも含まれていますが、使用を推奨しません |
...
|InfoSeq|Integer|グループプロフィールを変更する度にこの値が増加|読み取り専用|
...
|MaxMemberNum|Integer|最大参加者数|省略時のデフォルト値：課金パッケージの上限。例えば、体験版であれば20です。パッケージをアップグレードした場合、修正するグループ基本プロフィールに応じてこのフィールドを対応する数値に修正する必要があります|
...
### グループ参加者プロフィール[](id:SelfInfoFilter)
...
|NameCard|String|グループネームカード|読み取り/書き込み可能。最長50バイト、調整不可|
...
例えば、Appでグループ内にGroupLevelというフィールドを拡張する必要があり、そのValueが数字で、当該グループのレベル情報の記録に使用するとします。仮にレベル情報はAppバックエンドで計算する必要があるとしたならば、当該フィールドの最小書き込み権限は「App管理者が書き込み可能」でなければなりません。また当該フィールドはグループの公開プロフィールにする必要があるため、その最小読み取り権限は「いずれの人（非参加者を含む）も読み取り可能」でなければなりません。
...
グループメンバーディメンションのカスタムフィールドを設定する前に、先にグループタイプを指定する必要があります。ただしライブストリーミンググループ(AVChatRoom)およびそれを参照したグループタイプについては、全グループ参加者のプロフィールを保存しないため、グループメンバーディメンションのカスタムフィールドをサポートしていません。
