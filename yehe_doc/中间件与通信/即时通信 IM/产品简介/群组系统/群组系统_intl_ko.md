...
- 자주 사용되는 시나리오에 맞춰 기본적으로**업무 그룹(Work)**, **공개 그룹(Public)**, **회의 그룹(Meeting)**및**라이브 방송 그룹(AVChatRoom)** 4가지 그룹 유형이 구성되어 있으며, 비즈니스 니즈에 따라 [사용자 정의 그룹 유형](https://intl.cloud.tencent.com/document/product/1047/33529)을 구성할 수 있습니다. 
- 그룹 인원 제한:
  - 라이브 방송 그룹 외 그룹 인원 제한은 유료로 최대 6000명까지 확장 가능합니다. 자세한 내용은 [요금 안내](https://intl.cloud.tencent.com/document/product/1047/34350)를 참고하십시오.
...
- [사용자 정의 그룹 유형](https://intl.cloud.tencent.com/document/product/1047/33529)
...
| 관리자       | 일정한 그룹 관리 권한을 갖고 있는 그룹 구성원으로 그룹 소유자가 임명함. |<ul style="margin:0;"><li>그룹 프로필 정보 수정</li><li>일반 그룹 구성원 그룹에서 내보내기</li><li>일반 그룹 구성원 음소거(일정 시간 동안 발언 금지)</li><li>다른 사용자의 그룹 참여 신청 심사</li></ul>업무 그룹(Work)은 기본적으로 관리자 설정을 지원하지 않습니다. |
...
| 그룹유형   |적용 시나리오   |
|--------|---------|
| 업무 그룹(Work)| 일반 WeChat 그룹과 유사하며, 사용자가 그룹 구성원인 친구의 초대를 받아 그룹에 입장할 수 있습니다. 초대 받은 사람의 동의나 그룹 소유자의 승인이 필요 없습니다.  |
...
|기능 항목|업무 그룹(Work)|공개 그룹(Public)|회의 그룹(Meeting)|라이브 방송 그룹(AVChatRoom)|
|--- |-- |-- |-- |-- |
|구성원 역할|그룹 소유자<br>일반 구성원<br>App 관리자|그룹 소유자<br>관리자<br>일반 구성원<br>App 관리자|그룹 소유자<br>관리자<br>일반 구성원<br>App 관리자|그룹 소유자<br>일반 구성원<br>App 관리자|
|그룹 프로필 정보 수정 권한|일반 구성원|그룹 관리자 <br>그룹 소유자<br>App 관리자|그룹 소유자<br>App 관리자|App 관리자|
|정보 가져오기 가능한 구성원|전체 구성원| 전체 구성원|전체 구성원|상위 300개 그룹 구성원|
|그룹 해산 권한|App 관리자|그룹 소유자 및 App 관리자|그룹 소유자 및 App 관리자|그룹 소유자 및 App 관리자|
...
|기능 항목|업무 그룹(Work)|공개 그룹(Public)|회의 그룹(Meeting)|라이브 방송 그룹(AVChatRoom)|
|--- |-- |-- |-- |-- |
|그룹 ID 일치 검색을 통한 그룹 참여|미지원|지원|지원|지원|
|그룹 정보 퍼지 검색을 통한 그룹 참여|미지원|미지원|미지원|미지원|
|그룹 참여 신청|미지원|지원. 그룹 소유자 또는 관리자 승인 필요.|지원. 승인 필요 없음.|지원. 승인 필요 없음.|
|구성원의 다른 사용자 그룹 참여 초대|지원|미지원|미지원|미지원|
...
|기능 항목|업무 그룹(Work)|공개 그룹(Public)|회의 그룹(Meeting)|라이브 방송 그룹(AVChatRoom)|
|--- |--- |--- |--- |--- |
|관리자 지정|미지원|지원|지원|미지원|
|그룹 소유자 그룹 퇴장|지원. 퇴장 후 그룹은 소유자 없음 상태로 전환.|미지원|미지원|미지원|
|그룹 구성원 ‘내보내기’ |지원. 그룹 소유자가 내보내기 가능.|지원. 그룹 소유자 및 관리자가 내보내기 가능. 관리자는 일반 그룹 구성원만 내보내기 가능.|지원. 그룹 소유자 및 관리자가 내보내기 가능. 관리자는 일반 그룹 구성원만 내보내기 가능.|미지원. ‘음소거’ 기능으로 유사 효과 구현 가능.|
|그룹 구성원 ‘음소거’ 처리|미지원|지원. 그룹 소유자 및 관리자 ‘음소거’ 권한 보유. 관리자는 일반 그룹 구성원만 음소거 가능.|지원. 그룹 소유자 및 관리자 ‘음소거’ 권한 보유. 관리자는 일반 그룹 구성원만 음소거 가능.|지원. 그룹 소유자 ‘음소거’ 권한 보유.|
|오프라인 그룹 구성원 정기 삭제|지원. 기본값: 비활성화.|지원. 기본값: 비활성화.|지원. 기본값: 비활성화.|미지원|
...
|기능 항목|업무 그룹(Work)/공개 그룹(Public)/회의 그룹(Meeting)|라이브 방송 그룹(AVChatRoom)|
|--- |--- |--- |
|인원 제한|<ul style="margin:0;"><li>체험판: 20명/그룹</li><li>프로 버전: 기본 200명/그룹. [부가 가치 서비스](https://intl.cloud.tencent.com/document/product/1047/34350#zz) 구매 시 최대 2000명/그룹까지 확장 가능.</li><li>울티메이트 버전: 기본 2000명/그룹. [부가 가치 서비스](https://intl.cloud.tencent.com/document/product/1047/34350#zz) 구매 시 최대 6000명/그룹까지 확장 가능.</li></ul>|무제한|
|그룹 수량 제한|<ul style="margin:0;"><li>체험판: 최대 100개. 해산된 그룹 제외.</li><li>프로 버전 또는 울티메이트 버전: 무제한</li></ul>|<ul style="margin:0;"><li>체험판: 최대 10개. 해산된 그룹 제외.</li><li>프로 버전: 최대 50개. 해산된 그룹 제외. [부가 가치 서비스](https://intl.cloud.tencent.com/document/product/1047/34350#zz) 구매 시 라이브 방송 그룹 생성 수량 무제한.</li><li>울티메이트 버전: 무제한.</li></ul>|
...
|기능 항목|업무 그룹(Work)|공개 그룹(Public)|회의 그룹(Meeting)|라이브 방송 그룹(AVChatRoom)|
|--- |--- |--- |--- |--- |
|읽지 않은 메시지 통계|지원|지원|미지원|미지원|
|그룹 참여 전 메시지 기록 조회|미지원|미지원|지원|미지원|
|메시지 기록 저장|지원|지원|지원|미지원|
|구성원 변경 알림|지원|지원|미지원|지원|
|그룹 생성 후, 메시지를 발송하여 활성화|필요|불필요|불필요|불필요|
|기본 메시지 수신 옵션| 온/오프라인 푸시 수신 |온/오프라인 푸시 수신|온라인 푸시 메시지만 수신|온라인 푸시 메시지만 수신|
|사용자가 게스트 신분(로그인 필요 없음)으로 그룹 메시지 수신|미지원|미지원|미지원|지원|
...
>- 업무 그룹(Work), 공개 그룹(Public) 및 회의 그룹(Meeting)은 메시지 기록 저장 기능이 있으며 무상으로 7일(울티메이트 버전은 기본 30일)동안 저장할 수 있습니다. 저장 기간을 연장이 필요하시면, [콘솔](https://console.cloud.tencent.com/im)에서 메시지 저장 시간을 수정할 수 있습니다. 메시지 기록 저장 시간 연장은 유료 부가 가치 서비스로 자세한 내용은 [요금 안내](https://intl.cloud.tencent.com/document/product/1047/34350)를 참고하십시오. 
...
|그룹 자동 삭제 시간(초)|그룹 소유자의 그룹 해산 또는 모든 구성원 퇴장의 경우를 제외하고 백엔드에서 그룹을 삭제하지 않음. |그룹 소유자의 그룹 해산 또는 모든 구성원 퇴장의 경우를 제외하고 백엔드에서 그룹을 삭제하지 않음.|

>!그룹 삭제 기능을 활성화하려면 [IM 설정 변경 요청 티켓 템플릿](https://intl.cloud.tencent.com/document/product/1047/37275)에 따라 티켓을 제출하여 신청하십시오. 설정 후에는 그룹 유형에 따라 미활성 그룹(발언자, 구성원 변경이 없는 그룹)을 삭제합니다.
...
|Type|String|그룹 유형|읽기 전용<br>기본적으로 업무 그룹(Work), 공개 그룹(Public), 회의 그룹(Meeting), 라이브 방송 그룹(AVChatRoom) 유형 지원을 지원합니다. 자세한 내용은 [그룹 유형 소개](#GroupType)를 참고하십시오.<br>구버전 SDK에 포함된 Private, ChatRoom 및 BChatRoom 유형은 사용을 권장하지 않습니다. |
...
실제 사용 과정 중 IM이 제공하는 [그룹 유형](#GroupType)이 요구 사항을 충족하지 못하면, [IM 설정 변경 요청 티켓 템플릿](https://intl.cloud.tencent.com/document/product/1047/37275)에 따라 관련 정보를 준비하고, [티켓 제출](https://console.cloud.tencent.com/workorder/category)을 통해 그룹 유형 수정 또는 사용자 정의 그룹 유형 추가를 신청하십시오.
예를 들어, 업무 시나리오에서 사용되는 그룹이 업무 그룹(Work)과 유사하지만, 그룹 내 모든 구성원이 모두 최고 수준의 관리 권한 및 그룹 참여 전 메시지 기록 조회 권한을 보유해야 하는 경우, 다음 방안을 선택할 수 있습니다.
- 업무 그룹(Work)을 선택하고 [티켓 제출](https://console.cloud.tencent.com/workorder/category)을 통해, ‘그룹 참여 전 메시지 기록 조회 가능 여부’ 및 ‘일반 구성원에게 내보내기 권한 부여 여부’ 옵션 활성화를 신청합니다. 
- [티켓 제출](https://console.cloud.tencent.com/workorder/category)을 통해 새로운 그룹 유형(OAGroup)을 정의합니다. 업무 그룹(Work)을 참고 유형으로 지정하고 ‘그룹 참여 전 메시지 기록 조회 가능 여부’ 및 ‘일반 구성원에게 내보내기 권한 부여 여부’ 옵션을 활성화합니다.  

>!
>- 그룹 유형 추가 시, 한 그룹 유형을** 참고 유형**으로 지정해야 합니다. 구 SDK 버전의 BChatRoom 유형은 참고 유형으로 지정할 수 없습니다.
>- 새로운 그룹 설정이 완료되면 티켓 신청 시 지정한 수정 사항이 적용 되고, 그 외의 특성은 참고 유형과 동일하게 적용됩니다.
