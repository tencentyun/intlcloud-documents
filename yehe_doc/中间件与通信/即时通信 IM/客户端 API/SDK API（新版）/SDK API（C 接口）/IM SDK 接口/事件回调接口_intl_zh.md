...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMRecvNewMsgCallback | 新消息回调函数，请参考 [TIMRecvNewMsgCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgReadedReceiptCallback | 消息已读回执回调，请参考 [TIMMsgReadedReceiptCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |

>?发送方发送消息，接收方调用接口 [TIMMsgReportReaded](https://intl.cloud.tencent.com/document/product/1047/34391) 上报该消息已读，或者调用 [TIMMsgSendMessageReadReceipts](https://intl.cloud.tencent.com/document/product/1047/34391) 发送群消息已读回执，发送方 IM SDK 会通过此接口设置的回调抛出。
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgRevokeCallback | 消息撤回通知回调，请参考 [TIMMsgRevokeCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |

>?发送方发送消息，接收方收到消息。此时发送方调用接口 [TIMMsgRevoke](https://intl.cloud.tencent.com/document/product/1047/34391) 撤回该消息，接收方的 IM SDK 会通过此接口设置的回调抛出。
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgExtensionsChangedCallback | 设置消息扩展信息更新的回调，请参考 [TIMMsgExtensionsChangedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgExtensionsDeletedCallback | 设置消息扩展信息删除的回调，请参考 [TIMMsgExtensionsDeletedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgElemUploadProgressCallback | 文件上传进度回调，请参考 [TIMMsgElemUploadProgressCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMGroupAttributeChangedCallback | 设置群组属性变更回调，请参考 [TIMGroupAttributeChangedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMGroupCounterChangedCallback | 设置群计数器变更回调，请参考 [TIMGroupCounterChangedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMGroupTipsEventCallback | 群消息回调，请参考 [TIMGroupTipsEventCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMConvEventCallback | 会话事件回调，请参考 [TIMConvEventCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMConvTotalUnreadMessageCountChangedCallback | 会话未读消息总数变更的回调，请参考[TIMConvTotalUnreadMessageCountChangedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMConvTotalUnreadMessageCountChangedByFilterCallback | 按会话 filter 过滤的未读消息总数变更的回调，请参考[TIMConvTotalUnreadMessageCountChangedByFilterCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMNetworkStatusListenerCallback | 连接事件回调，请参考 [TIMNetworkStatusListenerCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMKickedOfflineCallback | 踢下线回调，请参考 [TIMKickedOfflineCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMUserSigExpiredCallback | 票据过期回调，请参考 [TIMUserSigExpiredCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |

>?用户票据可能会存在过期的情况，如果用户票据过期，此接口设置的回调会调用。[TIMLogin](https://intl.cloud.tencent.com/document/product/1047/34390) 也将会返回70001错误码。开发者可根据错误码或者票据过期回调进行票据更换。
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMOnAddFriendCallback | 添加好友回调，请参考 [TIMOnAddFriendCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMOnDeleteFriendCallback | 删除好友回调，请参考 [TIMOnDeleteFriendCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMUpdateFriendProfileCallback | 更新好友资料回调，请参考 [TIMUpdateFriendProfileCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMFriendAddRequestCallback | 好友添加请求回调，请参考 [TIMFriendAddRequestCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMFriendApplicationListDeletedCallback | 好友申请删除回调，请参考 [TIMFriendApplicationListDeletedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMFriendApplicationListReadCallback | 设置好友申请已读的回调，请参考 [TIMFriendApplicationListReadCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMFriendBlackListAddedCallback | 设置黑名单新增的回调，请参考 [TIMFriendBlackListAddedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMFriendBlackListDeletedCallback | 设置黑名单删除的回调，请参考 [TIMFriendBlackListDeletedCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMLogCallback | 日志回调，请参考 [TIMLogCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |

>?设置日志监听的回调之后，IM SDK 内部的日志会回传到此接口设置的回调。开发者可以通过接口 [TIMSetConfig](https://intl.cloud.tencent.com/document/product/1047/34388) 配置哪些日志级别的日志回传到回调函数。
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgUpdateCallback | 消息更新回调，请参考 [TIMMsgUpdateCallback](https://intl.cloud.tencent.com/document/product/1047/34550) |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
