...
This API is used to send a new message. (This API is to be disused. Please use the `TIMMsgSendMessage` API.)
...
| conv_type | enum TIMConvType | Conversation type. For more information, see [TIMConvType](https://intl.cloud.tencent.com/document/product/1047/34551). |
| json_msg_param | const char\* | JSON string of the message. |
| cb | TIMCommCallback | Callback function for indicating whether the custom message is saved. For more information about the callback function definition, see [TIMCommCallback](https://intl.cloud.tencent.com/document/product/1047/34550). |
| user_data | const void\* | User-defined data. The IM SDK only transfers the user data to the callback function `cb` without processing the data. |
...
| Parameter | Type | Description |
|-----|-----|-----|
| json_identifier_array | const char\* | List of UserIDs. |
...
This API is used to search for local messages.
