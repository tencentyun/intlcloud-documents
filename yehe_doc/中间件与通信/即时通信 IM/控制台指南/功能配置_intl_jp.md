...
フラッグシップ版で複数プラットフォームでログインを選択する場合、Web端末の同時オンライン可能数は最大で10台です。Android、iPhone、iPad、Windows、Macプラットフォームでは同時オンライン可能デバイスの最大数は3台です。
</dx-alert>
...
>?この機能は**フラッグシップ版ユーザーのみに開放**され、フラッグシップ版ユーザーでない場合は、アップグレードしてご使用ください。機能の正常な使用を保証するために、**端末** **SDK**を**5.9.0バージョン以上**に更新し、**Web SDK**を**2.16.0バージョン以降に更新してください**。
...
### グループ既読メッセージ通知の設定
1. **ログインとメッセージ**画面で、**グループ既読メッセージ通知の設定**の右側にある編集をクリックします。
2. ポップアップ表示された**グループ既読メッセージ通知の設定**のダイアログボックスで、メッセージ通知をサポートするグループタイプを設定します。
3. **保存**をクリックし、設定を保存します。
>?この機能は**フラッグシップ版ユーザーのみに開放**され、フラッグシップ版ユーザーでない場合は、アップグレードしてご使用ください。グループ既読メッセージ通知の機能は、**端末SDK 6.1.2155以上のバージョン**をサポートします。グループ容量が200未満のフレンドワークグループ（Work）、知らない人とのソーシャルグループ（Public）および臨時ミーティンググループ（Meeting）などに適しています。
...
### 直近の連絡先のプル個数設定
**ログインとメッセージ**画面で、**直近の連絡先のプル個数設定**の領域でクラウドからプルされた直近の連絡先の個数を設定できます。クライアントはデフォルトでクラウドから最近の連絡先を100個プルでき、クラウドから連絡先を最大500個プルするように設定できます。
>?
>- 直近の連絡先のプル個数設定は**フラッグシップ版**のみ使用でき、フラッグシップ版ユーザーでない場合は、アップグレードしてご使用ください。
>- 直近の連絡先のプル個数設定は**端末SDK 5.1.1、Web SDK 2.0****以上のバージョン**のみをサポートします。古いバージョンのユーザーが設定する必要がある場合は、SDKバージョンをアップグレードしてください。
