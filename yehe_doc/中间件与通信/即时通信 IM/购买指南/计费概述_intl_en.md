...
- **MAU**: The number of unique users that log in to IM app in a given month. A user that logs in repeatedly in the same month counts as only one MAU.
...
- **Audio/video call feature**:
  - **It is currently in beta, with a 60-day free edition provided**.
