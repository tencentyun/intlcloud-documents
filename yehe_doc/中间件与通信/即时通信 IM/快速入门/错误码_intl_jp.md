...
| ------ | ------------------------------------------------------------ |
| 0      | エラーなし。                                                     |
| 6015   | 実行中。インターフェース呼出しの制御を管理しておいてください。例えば、初めての初期化操作がコールバック前である場合、後続の初期化操作でこのエラーコードが返されます。|
...
| 7002   | TinyIdが空です。内部エラー。UserIDが存在し、かつ、有効になっているかチェックしてください。        |
...
| 7008   | APIの呼出し頻度が上限を超えています。ご確認して、頻繁にインターフェースを呼び出さないでください。                     |
| 7009   | 実行中に終了します。たとえば、ログイン中、unInitを呼び出してSDKの利用を停止します。|
| 7010   | データベースへの操作に失敗しました。                                          |
| 7011   | データベースに検索したデータが存在しません。                                  |
| 7012   | SDK内部で発生するはずがない内部エラー                                |
| 7013   | このインターフェースの使用をサポートしません。フラッグシップ版のパッケージへアップグレードしてください。 |
| 7014   | 不正なリクエスト。インターフェースの呼出しが要求を満たしているかチェックしてください。                       |
| 7015   | SDKのローカル内容審査で、センシティブワードが命中されました。                               |
...
| 6031   | ファイルのアップロードに失敗しました。アップロードする画像を正常に開けるか確認してください。 ||
| 6009   | ファイルのダウンロードに失敗しました。ネットワーク、またはファイルや音声が期限切れになっていないかをチェックしてください。現在、リソースファイルの保存は7日間です。 |
...
| 8004   | Cloud Imageはポルノ検出などが原因でサムネイルへの変換に失敗しました。|
| 8005   | マージする転送メッセージのネストレベルが上限の100レベルを超えています。 |
| 8006   | メッセージの変更に矛盾が発生しました。変更を要求したメッセージはすでに他の人によって変更されています。|
| 8010   | シグナリングのリクエストIDが無効か、またはすでに処理されています。 |
| 8011   | シグナリングをリクエストする権限がありません。例えば、自分以外が行った招待をキャンセルする権限です。 |
| 8012   | シグナリング招待がすでに存在しています。|
| 8020   | メッセージをキャンセルする時、キャンセルするメッセージが存在しないか、すでに送信されています。キャンセルに失敗しました。|
| 8021   | このメッセージがキャンセルされたため、送信に失敗しました。|

### グループのエラーコード

| エラーコード | 説明                                                         |
| ------ | ------------------------------------------------------------ |
| 8501   | グループID（グループの唯一の識別子）が不正です。カスタムグループIDは、48バイト以内の印刷可能なASCII文字(0x20-0x7e)を使用する必要があり、プレフィックスに@TGS#を設定することはできません（サーバーにデフォルトで割り当てられたグループIDとの混同を避けるため）。 |
...
| 8507   | グループ参加者数の制限超過。グループ作成および参加者招待時に、指定した参加者数が制限を超過しています。最大グループ参加者数は、プロフェッショナル版は200人で、付加価値サービスにより2000人まで拡張できます。フラッグシップ版は2000人で、付加価値サービスにより6000人まで拡張できます。オーディオビデオチャットルームおよびオンライン参加者向けに放送する大規模グループは無制限です。 |
...
| 8511   | グループ属性インターフェースの操作制限：バックグラウンドでは、インターフェースに対する追加、削除、変更は1秒に5回までとし、SDKでは、インターフェースの検索は5秒に20回までとします。      |
| 8512   | グループオンラインメンバー数を取得するインターフェースの操作制限：SDKでは、インターフェースの検索は60秒に1回とします。       |
| 8513   | グループプロファイルを取得するインターフェースの操作制限：SDKでは、インターフェースの検索は1秒に1回とします。       |
| 8514   | 参加したグループのリストを取得するインターフェースの操作制限：SDKでは、インターフェースの検索は1秒に1回とします。       |
| 8515   | グループメンバーをマークすることはできません。       |
...
| 9521   | リクエストパケットが送信待ちキューに入っており、一部のデータが送信済みで、送信待ちの部分にタイムアウトが発生しました。アップリンクの帯域幅が不足している可能性があるため、ネットワークに正常に接続しているかを確認してください。エラーコードをコールバックすると、ネットワークに接続していることが検出されます。 |
...
| 60020  | プロフェッショナル版が期限切れになり、利用停止しています。[IM購入ページ](https://buy.cloud.tencent.com/avc)にログインして再購入してください。購入して5分後に有効になります。|
|60021  |RestAPI呼び出し元IPが不正です。|
| 80002  | メッセージ送信パケットが長すぎます。現在サポートしているメッセージパケット長は最大8kです。パケットサイズを小さくしてからもう一度お試しください。 |
| 80003  | シングルチャットのメッセージを送信する前にコールバックが失敗するか、グループチャットメッセージを送信する前にコールバックが失敗するか、パケット応答がタイムアウトし、メッセージがデリバーされない。コンソールで[イベントが発生する前のコールバックタイムアウトの処理ポリシー](https://intl.cloud.tencent.com/document/product/1047/34354)を自己構成できます。|
...
| 70398  | アカウント数が制限を超えています。100を超えるアカウントを作成したい場合は、アプリケーションをプロフェッショナル版にアップグレードしてください。         |
...
| 72000  | アプリケーションは体験版で、デイリーアクティブユーザー数（DAU）が無料利用枠を超えています。この制限を解除する場合、アプリケーションをプロフェッショナル版またはフラッグシップ版にアップグレードしてください。      |
| 72001  | コンソールでユーザーステータスクエリおよびステータス変更通知の設定を有効にしてください。具体的な操作方法については、[操作ガイド](https://intl.cloud.tencent.com/document/product/1047/49562)をご参照ください      |
...
| 51012  | トップに表示できる最大セッション数を超えています。トップに表示できる最大セッション数は50で、変更不可です。       |
...
| 10006  | 操作頻度の制限。呼び出しの頻度を減らしてお試しください。                         |
...
| 10035  | オーディオビデオチャットルームとオンラインメンバーの放送型大規模グループはメッセージの削除をサポートしていません。               |
| 10036  | オーディオビデオチャットルームの作成数が制限を超えています。[料金説明](https://intl.cloud.tencent.com/document/product/1047/34350) をご参照のうえ、前払いパッケージ「IMオーディオビデオチャットルーム」をご購入ください。 |
...
|10058|体験版で100グループの制限を超えており、購入することで作成可能なグループ数を拡張する必要があることを表します。|
...
| エラーコード | 説明                                           |
|---------|---------|
| -195  | グーグルプッシュがプッシュオブジェクトを作成することに失敗しました。これは通常、証明書のエラーによって発生したと考えられます。|
| -196  | パラメータがありません。intentを指定したが、intentの値を設定していません。|
| -197  | 証明書エラー。証明書関連のパラメータをご確認ください。|
| -198  | メーカーレスポンスエラー。|
| -199  | メーカーネットワーク異常。|
| -200  | IMバックグラウンドネットワークエラー。|
| -201  | 正当性チェックにパスしませんでした。例えば、tokenエラーまたは証明書IDエラー。|
| -202  | メーカーにプッシュする前に、オブジェクトの作成に失敗しました。|
| -203  | HUAWEIへの1日あたりのプッシュ制限。|
...
| 6207   | アカウントの認証に失敗しました（TinyId変換失敗）。UserIDが存在し、かつ、有効になっているかチェックしてください。。|
...
>!問題を依然として解決できない場合、[AIBot](https://cloud.tencent.com/act/event/smarty-service) または[チケットの提出](https://console.cloud.tencent.com/workorder/category)で使用したインターフェース、エラーコードとエラーメッセージを技術サポーターに提供してください。
