...
You can call the `Login` API ([c#](https://comm.qq.com/im/doc/unity/en/api/loginOrlogout/Login.html)) to log in.
...
* Your ticket expires when the user is online: You might receive the `UserSigExpiredCallback` callback ([c#](https://comm.qq.com/im/doc/unity/en/callback/UserSigExpiredCallback.html)) when users are online. In this case, you need to generate a new `user_sig` to log in again.
* A user is kicked offline: When a user is kicked offline, the IM SDK will notify you through the `KickedOfflineCallback` callback ([c#](https://comm.qq.com/im/doc/unity/en/callback/KickedOfflineCallback.html)). In this case, you can prompt the user and call `Login` to log in again.
...
After logging in successfully, call `GetLoginUserID` ([c#](https://comm.qq.com/im/doc/unity/en/api/loginOrlogout/GetLoginUserID.html)) to get the `UserID`.
...
Call `GetLoginStatus` ([c#](https://comm.qq.com/im/doc/unity/en/api/loginOrlogout/GetLoginStatus.html)) to get the login status. If a user is logged in or logging in, don't call the login API frequently. The IM SDK supports the following login statuses:
...
When you call the `Login` API ([c#](https://comm.qq.com/im/doc/unity/en/api/loginOrlogout/Login.html)) to log in, if the limit specified by the multi-client login policy for your account is exceeded, a newly logged in instance will kick out an earlier one.
The one kicked out will receive `KickedOfflineCallback` ([c#](https://comm.qq.com/im/doc/unity/en/callback/KickedOfflineCallback.html)).
...
In special cases, for example, if you use the IM SDK only after entering a specific UI and no longer use it after exiting the UI, you can call the `Logout` API ([c#](https://comm.qq.com/im/doc/unity/en/api/loginOrlogout/Logout.html)) to log out of the SDK, after which you will no longer receive new messages. Note that in this case, you also need to call `Uninit` ([c#](https://comm.qq.com/im/doc/unity/en/api/IMSDKInit/Uninit.html)) after the logout to uninitialize the SDK.
...
## Account Switch
Call `Login` ([c#](https://comm.qq.com/im/doc/unity/en/api/loginOrlogout/Login.html)) to switch between accounts in the application.
