## Feature Description
* The API for pulling historical messages is in the `V2TIMManager` and `V2TIMManager+Message (for iOS and macOS)` and `V2TIMMessageManager (Android)` classes.
...
This API supports pulling historical messages by page. For more information, see [Pulling by page](#advance_page).
...
| Parameter | Definition | Valid for One-to-One Chat | Valid for Group Chat | Required | Description |
| --- | --- | --- | --- | --- |--- |
...
option.setGetType(V2TIMMessageListGetOption.V2TIM_GET_CLOUD_OLDER_MSG); // Pull cloud messages in reverse chronological order
 ...
 *
 * If `onlineUserOnly` is set to `false` in the `invite` operation, a custom message will be generated for each signaling operation (including `invite`, `cancel`, `accept`, `reject`, and `timeout`).
 * The message will be delivered to the user via `V2TIMAdvancedMsgListener` > `onRecvNewMessage` and can be pulled from historical messages. To customize the display text based on the signaling information, call the following API to get the signaling information.
 *
...
To minimize the impact of such a limit mechanism on the business layer, take the following measures to reduce invalid messages:
* You can use online messages, that is, set `onlineUserOnly` to `YES/true` when sending messages.
* For group messages, use targeted group messages to specify the receiver.
