...
2. 发送方调用 `MsgSendMessage` ([c#](https://comm.qq.com/im/sdk/unity_plus/_site_en/api/com.tencent.imsdk.unity.TencentIMSDK.html#com_tencent_imsdk_unity_TencentIMSDK_MsgSendMessage_System_String_com_tencent_imsdk_unity_enums_TIMConvType_com_tencent_imsdk_unity_types_Message_System_Text_StringBuilder_com_tencent_imsdk_unity_callback_ValueCallback_com_tencent_imsdk_unity_types_Message__)) 接口创建一条 @ 文本消息，创建消息对象 `Message`，并在其中指定需要 @ 的成员，然后将刚才创建的 @ 消息对象发送出去。
...
1. 在加载和更新会话处，需要调用 `ConvInfo` 的 `conv_group_at_info_array` ([c#](https://comm.qq.com/im/sdk/unity_plus/_site_en/api/com.tencent.imsdk.unity.types.ConvInfo.html#com_tencent_imsdk_unity_types_ConvInfo_conv_group_at_info_array)) 接口获取会话的 @ 数据列表。
2. 通过列表中 `GroupAtInfo` 对象的 `conv_group_at_info_at_type` ([c#](https://comm.qq.com/im/sdk/unity_plus/_site_en/api/com.tencent.imsdk.unity.types.GroupAtInfo.html#com_tencent_imsdk_unity_types_GroupAtInfo_conv_group_at_info_at_type)) 接口获取 @ 数据类型，并更新到当前会话的 @ 信息。
