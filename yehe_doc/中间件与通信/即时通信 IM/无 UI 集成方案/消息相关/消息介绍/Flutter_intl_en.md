## Message Class
In the Tencent Cloud IM SDK, the message class is `V2TIimessage` ([Dart](https://pub.dev/documentation/tencent_im_sdk_plugin_platform_interface/latest/models_v2_tim_message/V2TimMessage-class.html)), which will be frequently used for message sending and receiving.
...
| needReadReceipt | Whether the message requires a read receipt | It is set by the sender and valid for the Ultimate edition on v6.1 or later. For more information, see [Read Receipt](https://comm.qq.com/en_doc_site/flutter/api/manager_v2_tim_message_manager/V2TIMMessageManager/sendMessageReadReceipts.html). |
| priority | Message priority | It applies only to group messages and can be set by calling the `sendMessage` API. |
| groupAtUserList | List of users tagged in the group message | It applies only to group messages and stores the `userID`. For more information, see [createTextAtMessage method](https://comm.qq.com/en_doc_site/flutter/api/manager_v2_tim_message_manager/V2TIMMessageManager/createTextAtMessage.html). |
