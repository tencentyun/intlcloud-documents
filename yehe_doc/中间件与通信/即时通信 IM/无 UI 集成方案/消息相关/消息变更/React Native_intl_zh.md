...
会话参与者可以调用 `modifyMessage`([TS](https://comm.qq.com/im/doc/RN/en/Api/V2TIMMessageManager/modifyMessage.html)) 接口对会话里的消息做二次修改。
...
会话的参与者调用 `addAdvancedMsgListener` ([TS](https://comm.qq.com/im/doc/RN/en/Api/V2TIMMessageManager/addAdvancedMsgListener.html)) 添加高级消息监听器。

当会话里的消息被修改后，会话的参与者都会收到 `onRecvMessageModified`([TS](https://comm.qq.com/im/doc/RN/en/Callback/OnRecvMessageModified.html)) 回调，回调里面会携带修改之后的消息对象。
