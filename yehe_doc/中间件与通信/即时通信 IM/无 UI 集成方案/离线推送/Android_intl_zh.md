- ## 概述
  ...
  登录腾讯云 [即时通信 IM 控制台](https://console.qcloud.com/avc) ，添加各个厂商推送证书，并将您在步骤一中获取的各厂商的 AppId、AppKey、AppSecret 等参数配置给 IM 控制台的推送证书，以 Google 为例:
  ...
  ### 步骤4：配置厂商推送规则
  -  **应用离线参数配置**
  步骤二推送证书添加成功之后，IM 控制台会为您分配一个证书 ID，请您填充到  [PrivateConstants](https://github.com/TencentCloud/TIMSDK/blob/master/Android/TUIKit/TUIOfflinePush/tuiofflinepush/src/main/java/com/tencent/qcloud/tim/tuiofflinepush/PrivateConstants.java)  的配置参数里，该证书配置给 IM 控制台的 ID 会在注册推送服务和上报 token 时使用, 以 Google 为例：
  **推送证书 ID 如下：**
  ...
  **填充的参数如下：**
  ...
  -  **清单文件配置厂商推送权限相关**
  清单文件中需要添加各个厂商的推送规则，可以参见 TUIKitDemo [清单文件](https://github.com/TencentCloud/TIMSDK/blob/master/Android/Demo/app/src/main/AndroidManifest.xml)相关配置，以 Google 为例：
  ```
  ...
  ```
  以 Google FCM 为例，在注册结果回调处保存注册成功后的 token，并调用 [setOfflinePushConfig](https://im.sdk.qcloud.com/doc/zh-cn/classcom_1_1tencent_1_1imsdk_1_1v2_1_1V2TIMOfflinePushManager.html#a494d6cafe50ba25503979a4e0f14c28e) 接口上报给后台。
  ```
  ...
  Disacord交流群：[点击加入](https://discord.com/invite/8EmN2ma25W)
