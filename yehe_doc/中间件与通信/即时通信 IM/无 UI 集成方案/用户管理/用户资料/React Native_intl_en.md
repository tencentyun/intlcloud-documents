...
Call `addFriendListener` ([TS](https://comm.qq.com/im-react-native-doc/classes/FriendshipManager__________.V2TIMFriendshipManager.html#addFriendListener)) to add a relationship chain event listener.

To stop receiving relationship chain events, call `removeFriendListener` ([TS](https://comm.qq.com/im-react-native-doc/classes/FriendshipManager__________.V2TIMFriendshipManager.html#removeFriendListener)) to remove the relationship chain event listener.
...
Call the `getUsersInfo` API ([TS](https://comm.qq.com/im-react-native-doc/classes/BaseManager______.V2TIMManager.html#getUsersInfo)) and enter a user's `UserID` for the `userIDList` parameter to query the user's profile.

Call the `setSelfInfo` API ([TS](https://comm.qq.com/im-react-native-doc/classes/BaseManager______.V2TIMManager.html#setSelfInfo)) to modify a user's profile.
After the profile is modified successfully, you will receive the `onSelfInfoUpdated` callback ([TS](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimSDKListener-1.html#onSelfInfoUpdated)).
...
Call the `getUsersInfo` API ([TS](https://comm.qq.com/im-react-native-doc/classes/BaseManager______.V2TIMManager.html#getUsersInfo)) and enter the `UserID` of a non-friend user for the `userIDList` parameter to query the profile of the non-friend user.

> ? The profile of a non-friend user cannot be modified.

### Querying and modifying a friend's profile

Call the `getFriendsInfo` API ([TS](https://comm.qq.com/im-react-native-doc/classes/FriendshipManager__________.V2TIMFriendshipManager.html#getFriendsInfo)) to query the profile of the specified friend. The relationship between the user and the friend can be obtained through the `relation` field of the `V2TIMFriendInfoResult` in the callback:
...
Call the `setFriendInfo` API ([TS](https://comm.qq.com/im-react-native-doc/classes/FriendshipManager__________.V2TIMFriendshipManager.html#setFriendInfo)) to modify the information of a friend such as remarks.
