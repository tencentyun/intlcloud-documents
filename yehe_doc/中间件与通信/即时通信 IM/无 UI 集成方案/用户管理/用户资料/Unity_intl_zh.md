...
## 用户资料管理
### 查询和修改自己的资料
您可以调用 `ProfileGetUserProfileList` ([c#](https://comm.qq.com/im/doc/unity/en/api/UserApi/ProfileGetUserProfileList.html)) 接口查询个人资料，其中参数 `friendship_getprofilelist_param_identifier_array` 需填入自己的 UserID。

您可以调用 `ProfileModifySelfUserProfile` ([c#](https://comm.qq.com/im/doc/unity/en/api/UserApi/ProfileModifySelfUserProfile.html)) 接口修改个人资料。
...
### 查询非好友用户资料
您可以调用 `ProfileGetUserProfileList` ([c#](https://comm.qq.com/im/doc/unity/en/api/UserApi/ProfileGetUserProfileList.html)) 接口查询非好友资料，其中参数 `friendship_getprofilelist_param_identifier_array` 填入非好友的 UserID 即可。

> ? 不能修改非好友的资料。

### 查询和修改好友资料
您可以调用 `FriendshipGetFriendsInfo` ([c#](https://comm.qq.com/im/doc/unity/en/api/FriendshipApi/FriendshipGetFriendsInfo.html)) 接口查询指定的好友资料，从回调信息中通过 `FriendInfoGetResult` 的 `friendship_friend_info_get_result_relation_type` 字段可以得到该用户与自己的关系：
...
您可以调用 `FriendshipModifyFriendProfile` ([c#](https://comm.qq.com/im/doc/unity/en/api/FriendshipApi/FriendshipModifyFriendProfile.html)) 接口修改好友备注等资料。
