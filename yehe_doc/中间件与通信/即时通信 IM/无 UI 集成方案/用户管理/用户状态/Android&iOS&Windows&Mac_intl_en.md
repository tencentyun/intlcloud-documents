...
The Chat SDK on v6.3 or later provides the user status management feature. Each user has two statuses:
...
Call `getUserStatus` with `userIDList` containing only your own userID to query your own status.

> ? 
> 1. To query your own status, you don't need to upgrade to the Ultimate edition or enable the feature in the console.
> 2. This API can be called an unlimited number of times.
...
Call the `subscribeUserStatus` API ([Android](https://im.sdk.qcloud.com/doc/en/classcom_1_1tencent_1_1imsdk_1_1v2_1_1V2TIMManager.html#a9c6deb154d0042d5472ec55cfe0962bb) / [iOS and macOS](https://im.sdk.qcloud.com/doc/en/interfaceV2TIMManager.html#a20e6cbe409549e0d2ff62be76914e0b3) / [Windows](https://im.sdk.qcloud.com/doc/en/classV2TIMManager.html#a6485ab30f0025998ffeecb78490332f5)) to subscribe to the status of the specified user. By default, the Chat SDK supports subscribing to the statuses of up to 200 users. After this limit is exceeded, the earliest subscribed user statuses will be removed.
...
V2TIMManager.getInstance().subscribeUserStatus(useridList, new V2TIMCallback() {
    @Override
    public void onSuccess() {
		// Subscribed to the user status successfully
    }

    @Override
    public void onError(int code, String desc) {
		// Failed to subscribe to the user status
    }
});
```
:::
...
V2TIMManager.getInstance().unsubscribeUserStatus(useridList, new V2TIMCallback() {
    @Override
    public void onSuccess() {
		// Unsubscribed from the user status successfully
    }

    @Override
    public void onError(int code, String desc) {
		// Failed to unsubscribe from the user status
    }
});
```
:::
...
Notifications on all these status changes can be called back through `onUserStatusChanged` ([Android](https://im.sdk.qcloud.com/doc/en/classcom_1_1tencent_1_1imsdk_1_1v2_1_1V2TIMSDKListener.html#a94251d1971d7b6692b3278ed0d42b73e) / [iOS and macOS](https://im.sdk.qcloud.com/doc/en/protocolV2TIMSDKListener-p.html#a8b086c14a9505990c7f5ac053bf45cd6) / [Windows](https://im.sdk.qcloud.com/doc/en/classV2TIMSDKListener.html#a432b98566a4adb3a001ce754788b54e8)). However, these notifications are triggered in different ways for different user types.
...
1. If you have enabled automatic notifications of friends' statuses in the [Chat console](https://console.cloud.tencent.com/im), when the status of a user's friend changes, the `onUserStatusChanged` callback will be automatically triggered.
...
To get the change in a non-friend user's status, you can only call `subscribeUserStatus` to subscribe to the status. When the user's status changes, the `onUserStatusChanged` callback will be triggered.
For the use limits of `subscribeUserStatus`, refer to [Subscribing to the User Status](#subscribe).
