...
初始化 SDK 前，您需要初始化一个 [`LogLevelEnum`](https://comm.qq.com/im/doc/RN/en/Enum/LogLevelEnum.html) 对象。该对象用于对 SDK 进行日志级别设置。
...
从 4.7.1 版本开始，IM SDK 的日志开始采用 xlog 模块进行输出。xlog 日志默认是压缩的，需要使用 Python 脚本进行解压。
...
我们建议您在调用 initSDK 时传入 `V2TimSDKListener`([Details](https://comm.qq.com/im/doc/RN/en/Interface/Listener/V2TimSDKListener.html)) 接口添加 SDK 事件监听器，在对应回调中做一些逻辑处理。
...
操作完上述步骤后，您可以调用 `initSDK`([Details](https://comm.qq.com/im/doc/RN/en/Api/V2TIMManager/initSDK.html)) 进行 SDK 初始化。
...
反初始化需要操作一个步骤：调用反初始化接口 `unInitSDK`([Details](https://comm.qq.com/im/doc/RN/en/Api/V2TIMManager/unInitSDK.html))
