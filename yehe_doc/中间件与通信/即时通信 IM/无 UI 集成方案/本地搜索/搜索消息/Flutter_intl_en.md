...
The message search parameter class is `V2TimMessageSearchParam` ([dart](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Class/Message/V2TimMessageSearchParam.html)). When searching for messages, the SDK will execute different search logics based on the object settings.
...
| messageTypeList      | Set of the message types to be searched for                           | If it is left empty, messages of all the supported types (excluding `V2TIMFaceElem` and `V2TIMGroupTipsElem`) will be searched for. For more information on the values of other types, see `V2TIMElemType` ([dart](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Class/Message/V2TimMessage.html#elemtype)). |
...
The message search result class is `V2TIMMessageSearchResult` ([dart](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Class/Message/V2TimMessageSearchResult.html)). The parameters are as described below:
...
Here, `messageSearchResultItems` is a list that contains the `V2TIMMessageSearchResultItem` objects ([dart](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Class/Message/V2TimMessageSearchResultItem.html)). The parameters are as described below:
...
When a user enters a keyword in the search box to search for messages, you can call `searchLocalMessages` ([dart](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Api/V2TIMMessageManager/searchLocalMessages.html)) to search for local messages of the IM SDK.
...
In general, if you call the `createCustomMessage(data)` API ([dart](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Api/V2TIMMessageManager/createCustomMessage.html)) to create a custom message, the message cannot be found, as the SDK will save it as a binary data stream.
...
If you don't need offline push, you can use the `disablePush` in the `V2TIMOfflinePushInfo` parameter of the `sendMessage` API ([dart](https://comm.qq.com/im/doc/flutter/en/SDKAPI/Api/V2TIMMessageManager/sendMessage.html)).
