...
The message search parameter class is `V2TimMessageSearchParam` ([TS](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimMessageSearchParam-1.html)). When searching for messages, the SDK will execute different search logics based on the object settings.
...
| messageTypeList | Set of the message types to be searched for | If it is left empty, messages of all the supported types (excluding `V2TIMFaceElem` and `V2TIMGroupTipsElem`) will be searched for. For more information on the values of other types, see `V2TIMElemType` ([TS](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimMessage-1.html#elemType)). |
...
The message search result class is `V2TIMMessageSearchResult` ([TS](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimMessageSearchResult-1.html)). The parameters are as described below:
...
Here, `messageSearchResultItems` is a list that contains the `V2TIMMessageSearchResultItem` objects ([TS](https://comm.qq.com/im-react-native-doc/interfaces/interface.V2TimMessageSearchResultItem.html)). The parameters are as described below:
...
When a user enters a keyword in the search box to search for messages, you can call `searchLocalMessages` ([TS](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#searchLocalMessages)) to search for local messages of the IM SDK.
...
In general, if you call the `createCustomMessage(data)` API ([TS](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#createCustomMessage)) to create a custom message, the message cannot be found, as the SDK will save it as a binary data stream.

To enable a user to find a custom message, you need to call the `createCustomMessage(data, description, extension)` API [TS](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#createCustomMessage) to create and send the custom message and put the text to be searched for in the `description` parameter.
...
If you don't need offline push, you can use the `disablePush` in the `V2TIMOfflinePushInfo` parameter of the `sendMessage` API ([TS](https://comm.qq.com/im-react-native-doc/classes/MessageManager__________.V2TIMMessageManager.html#sendMessage)).
