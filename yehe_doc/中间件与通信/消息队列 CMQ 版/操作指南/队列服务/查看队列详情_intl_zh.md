...
- 队列属性：单击右上角**修改配置**可修改队列属性。
  ...
  | 属性                   | 说明                                                         |
  | ---------------------- | ------------------------------------------------------------ |
  | 消息生命周期           | 队列的 msgRetentionSeconds 属性，消息在本队列中的保留时间，从发送到该队列开始经过此参数指定的时间后，该消息会变得可被删除（不一定会被及时清除）；单位为秒，有效值范围1分钟到15天。消息生命周期详情参考 [消息生命周期](https://intl.cloud.tencent.com/document/product/1111/42987)。 |
