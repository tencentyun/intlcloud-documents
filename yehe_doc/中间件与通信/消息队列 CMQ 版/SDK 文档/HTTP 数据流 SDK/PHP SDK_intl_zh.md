...
目前创建消息队列可以在控制台，或者通过云 API 方式来创建，使用云API 需要安装相关 SDK，SDK 安装可参见 PHP SDK 3.0安装使用。
</dx-alert>
2. 添加依赖。
<dx-codeblock>
:::  shell
...
### 操作步骤
1. 准备所需资源，创建主题订阅和订阅者。
   1. 创建主题订阅，创建主题订阅可通过控制台，或云 API 进行创建，使用云 API 需要安装相关 SDK，SDK 安装可参见 PHP SDK 3.0安装使用。
   <dx-codeblock>
   :::  php
      $cred = new Credential($secretId, $secretKey);
      $httpProfile = new HttpProfile();
