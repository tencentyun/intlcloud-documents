...
- In GSE, a queue is a group of server fleets running in regions of a game asset package. It can be used to create a cross-region fleet group, allowing you to place game server sessions in any fleet in the queue to minimize latency for a better player experience. It also enables you to make the most of your fleet capacity for new games in order to achieve higher availability.
...
- In App Flow, a message refers to the data structure passed between each connector instance and processor instance in the integration flow.
...
### Message Filter Tag
TDMQ for CMQ allows consumers to filter messages by tags, which ensures that they only consume messages that are relevant to them. After a tag is added, subscribers will receive only messages with the tag. If no tag is set, all messages will be sent to all subscribers.
...
- In CLS, a topic refers to the basic management that corresponds to an application or service. It is the smallest management unit for log collection, indexing, and delivery. There can be multiple log topics in a logset.
