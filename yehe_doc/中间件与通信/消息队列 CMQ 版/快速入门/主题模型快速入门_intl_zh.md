...
2. 在左侧导航栏选择**主题订阅**，选择好地域，单击**新建**，填写主题名称。
	...
	- 消息过滤类型：
		- 标签：CMQ 提供生产、订阅的消息标签匹配能力，可用于消息过滤。详细规则参见 [标签键匹配功能说明](https://intl.cloud.tencent.com/document/product/1111/43003)。
		- 路由匹配：Binding key、Routing key 是组合使用的，完全兼容 rabbitmq topic 匹配模式。发消息时配的 Routing key 是客户端发消息带的。创建订阅关系时配的 Binding key 是 topic 和 订阅者 的绑定关系。详细规则请参见 [路由键匹配功能说明](https://intl.cloud.tencent.com/document/product/1111/43004)。
	- 资源标签：选填，标签可以帮助您从各种维度方便地对 TDMQ CMQ 版资源进行分类管理，具体使用方法可参见 [标签管理](https://intl.cloud.tencent.com/document/product/1111/43007)。
...
2. 选择页面上方的**订阅者**页签，单击**新建**，填写订阅者相关信息。
   ...
   - 添加订阅者标签：添加订阅者时，需增加 FilterTag。增加 FilterTag 后，该订阅者仅能收到带该 FilterTag 的消息，单个订阅者最多可添加5个 tag。只要其中某个 tag 能匹配 Topic 的过滤标签，订阅者即可收到该次 Topic 投递的消息，若消息不带任何标签，则该订阅者无法收到该类型消息。
     - 标签：详细规则参见 [标签键匹配功能说明](https://intl.cloud.tencent.com/document/product/1111/43003)。
     - 路由匹配：详细规则请参见 [路由键匹配功能说明](https://intl.cloud.tencent.com/document/product/1111/43004)。
...
<td>API 调用地址，在 <a href = "https://console.cloud.tencent.com/tdmq">TDMQ CMQ 版控制台</a>的<b>主题订阅</b> > <b>API 请求地址</b>处复制。
