...
>! 安卓端的应用内消息展示依赖 WebView 框架，请留意配置 WebView 数据目录，否则可能引起程序崩溃，详情请参见 [应用内消息展示](https://intl.cloud.tencent.com/document/product/1024/30715)。
>


### iOS 端设置允许应用内消息展示

iOS 端 SDK 默认允许应用内消息展示，更多应用内消息的设置请参考 [iOS 接口文档](https://intl.cloud.tencent.com/document/product/1024/30727)。
...
在 Rest API 可选参数中增加设置`supply_inapp_msg`参数为`true`，可实现应用内消息补推，具体参考 [PushAPI 参数说明](https://intl.cloud.tencent.com/document/product/1024/33764)。
推送示例如下：
```
{
   "audience_type": "all",
