...
After the Tencent Push Notification Service is stopped, the application will not be able to push messages to devices through Tencent Push Notification Service. To receive messages pushed by Tencent Push Notification Service again, you must call the `startXGWithAccessID:accessKey:delegate:` method again to restart the Tencent Push Notification Service.
...
### Querying the Tencent Push Notification Service token
...
> - This API should be called after `xgPushDidRegisteredDeviceToken:error:` returns a success.
> - One application can have up to 10,000 custom tags. One device token can be bound to a maximum of 100 custom tags (to increase this limit, [submit a ticket](https://console.cloud.tencent.com/workorder/category)). One custom tag can be bound to an unlimited number of device tokens.
> 
