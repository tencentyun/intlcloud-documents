...
The account, tag, and user attribute features in this document are applicable to **SDK v1.2.9.0 and later**. For **SDK v1.2.7.2** and earlier, see [API Documentation](https://intl.cloud.tencent.com/document/product/1024/30727).
...
This API is used to launch Tencent Push Notification Service by using the information of the application registered at the official website.
...
```objective-c
/// @note Tencent Push Notification Service SDK1.2.7.2+
...
## Terminating Tencent Push Notification Service
...
After Tencent Push Notification Service is stopped, the application will not be able to push messages to devices through it. To receive messages pushed by it again, you must call the `startXGWithAccessID:accessKey:delegate:` method again to restart Tencent Push Notification Service.
...
>! The API for obtaining the token should be called after `xgPushDidRegisteredDeviceToken:error:` returns a success.
...
If there is no account of this type, this API will add a new one; otherwise, it will overwrite the existing one. (Newly added in Tencent Push Notification Service SDK v1.2.9.0+)
...
```objective-c
/// @note Tencent Push Notification Service SDK1.3.2.0+
...
This API is used to delete all accounts of a specified account type. (Newly added in Tencent Push Notification Service SDK v1.2.9.0+)
...
> - This API should be called after `xgPushDidRegisteredDeviceToken:error:` returns a success.
> - One app can have up to 10,000 custom tags. One device token can be bound to up to 100 custom tags. If you want to increase this limit, contact [customer service](https://intl.cloud.tencent.com/support). One custom tag can be bound to an unlimited number of device tokens.
> 
...
```Objective-C
/// Tencent Push Notification Service network connected
/// `_launchTag` clear flag. For example, set this tag to `YES` during cold or hot start.
- (void)xgPushNetworkConnected {
    if (_launchTag) {
        /// -1: Does not clear the notification bar; 0: Clears the notification bar.
...
```
/// @note Tencent Push Notification Service SDK1.2.4.1+
