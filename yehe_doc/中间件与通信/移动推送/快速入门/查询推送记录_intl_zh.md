## 操作场景
本文档指导您如何在移动推送 TPNS 控制台查询推送记录（消息 ID、标题、内容、发送时间和操作）。
...
![](https://main.qcloudimg.com/raw/e8f1cd83d57f62d5ff622502bee20b35.png)
 - TPNS 通道
   ...
   - 实际发送：在计划发送设备数中，通过TPNS通道对进程在线终端下发成功的有效设备数。
   - 抵达：消息成功抵达的设备数。
   - 点击：消息成功点击的设备数。
   - 清除：消息被清除的设备数。
   - 抵达率：抵达/实际发送。
...
>?
>1. 厂商通道不会返回“清除”。
>2. 抵达数据仅供参考，华为通道和魅族通道抵达回调需要自行配置，可参考 [厂商通道抵达回调获取指南](https://intl.cloud.tencent.com/document/product/1024/35246)。
...
按不同指标：
- 实际发送数：指定时间内 APNs 通道或 TPNS 通道接收到消息的设备数。
