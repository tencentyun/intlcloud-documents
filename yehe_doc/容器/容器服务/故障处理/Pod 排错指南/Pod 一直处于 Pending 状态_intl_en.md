...
- No usable nodes are available in the same availability zone as the drained node
...
#### Analysis
If a node has taints for which the Pod has no corresponding tolerations, the Pod will not be scheduled to that node. You can use `kubectl describe node <node-name>` to query existing node taints, as shown below:
``` bash
...
```
- Solution 2: add corresponding tolerations
  > The following uses a Pod named `nginx` as an example to describe how to add tolerations:
  >
...
## Relevant Operations
### Adding taints<span id="addTaints"></span>
...
> A taint called `node.cloudprovider.kubernetes.io/uninitialized` is added to a new node automatically and subsequently removed when the node is successfully initialized. This is to prevent Pods from being scheduled to an uninitialized node.
