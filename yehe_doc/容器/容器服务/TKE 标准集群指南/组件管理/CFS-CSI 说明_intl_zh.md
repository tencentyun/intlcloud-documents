...
CFS 接入简单，您无需调节自身业务结构，或者是进行复杂的配置。只需三步即可完成文件系统的接入和使用：创建文件系统，启动服务器上文件系统客户端，挂载创建的文件系统。通过 CFS-CSI 扩展组件，您可以快速在容器集群中通过标准原生 Kubernetes 使用 CFS，详情请参见 [CFS 使用场景](https://intl.cloud.tencent.com/document/product/582/9129)。

## 限制条件
- CFS 自身限制可参见 [CFS 系统限制](https://intl.cloud.tencent.com/document/product/582/9135)。
