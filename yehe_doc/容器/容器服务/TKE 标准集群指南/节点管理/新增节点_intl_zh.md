...
![](https://main.qcloudimg.com/raw/54bbd92b00eea36efaf6de36c850b7eb.png)
  - **CAM角色**：可为本批次创建的所有节点绑定相同的 CAM 角色，赋予节点该角色绑定的授权策略。详情请参见[ 管理实例角色](https://intl.cloud.tencent.com/document/product/213/45917)。
...
- 若高 IO 型、高性能 HCC 类机型需要对 NVMe 类型数据盘进行挂载，建议通过为数据盘设置文件系统卷标的方式单独添加进集群，不要和其他机型一起操作。
...
- 如果您填写的挂载点不存在，系统会为您创建对应目录，不会报错。
 ...
 - 登录方式：
     - 设置密码：请根据提示设置对应密码。
    - 立即关联密钥：密钥对是通过一种算法生成的一对参数，是比常规密码更安全的登录云服务器的方式，具体详情可参阅 [SSH 密钥](https://intl.cloud.tencent.com/document/product/213/6092)。
