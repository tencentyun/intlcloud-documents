...
4. 单击该权限组 ID，进入其详情页，可进行权限组规则的添加、编辑或删除操作。如果权限组中没有添加规则，则会允许全部。详情请参见 [ 添加权限组规则](https://intl.cloud.tencent.com/document/product/582/10951)。
...
4. 单击**新建**，进入“新建 StorageClass” 页面，参考以下信息进行创建。如下所示：
   ...
   - **可用区**：表示当前地域下支持使用文件存储的可用区，每个地域下不同可用区所适用的存储类型不完全一致，请参考 [ 可用地域 ](https://intl.cloud.tencent.com/document/product/582/35772) 进行选择。
...
>- 不指定 PersistVolume。详情请参见 [查看 PV 和 PVC 的绑定规则](https://intl.cloud.tencent.com/document/product/457/37770)。
> 
