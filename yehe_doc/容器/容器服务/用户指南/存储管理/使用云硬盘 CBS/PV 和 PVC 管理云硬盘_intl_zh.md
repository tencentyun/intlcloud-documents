...
由于静态创建云硬盘类型的 PV 时，需要绑定同类型可用 StorageClass，请参考 [创建 StorageClass](https://intl.cloud.tencent.com/document/product/457/36158#create) 完成创建。
...
主要参数信息如下：
   ...
   - **读写权限**：云硬盘只支持单机读写。
   - **StorageClass**：按需选择合适的 StorageClass。本文以选择在 [通过控制台创建 StorageClass](#create) 步骤中创建的 `cbs-test` 为例。
   - **容量**：根据实际需求进行设置。云硬盘容量最小值由云硬盘产品规格决定。
