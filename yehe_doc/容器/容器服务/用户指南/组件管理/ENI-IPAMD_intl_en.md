...
ENI-IPAMD runs on specific nodes or the master and is a GRPC server to implement IP resource allocation management, the StatefulSet controller for IP address reservation, and the node controller for host secondary ENI allocation management.

**When VPC-CNI network mode is selected in cluster, ENI-IPAMD will be installed automatically.** VPC-CNI network mode has the following characteristics:
...
- In VPC-CNI mode, StatefulSet supports pods with static IP addresses. The IP addresses of these pods do not change after the pods are restarted or migrated. Therefore, they are suitable for scenarios such as access restriction based on source IP addresses and log querying based on IP addresses.
