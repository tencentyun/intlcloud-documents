...
>?节点池功能目前已全量发布。如果您的集群中创建了伸缩组，现在仍可以继续使用这些伸缩组。但请注意，节点池全量后容器服务 TKE 不会对旧版伸缩组进行新功能迭代，请您尽快使用 TKE 提供的转换功能（通过  [控制台](https://intl.cloud.tencent.com/document/product/457/35901)）将伸缩组转换成节点池。
...
## 节点池弹性伸缩原理
节点池级别的弹性伸缩包含 Kubernetes 提供的 cluster autoscaler（CA）功能及腾讯云弹性伸缩服务提供的 AutoScaling（AS）能力两个层级的扩缩容策略。
> ?
