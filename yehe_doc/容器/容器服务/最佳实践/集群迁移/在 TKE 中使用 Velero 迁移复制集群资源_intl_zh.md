...
在需要被迁移的集群和目标集群上都安装 Velero 实例，并且两个集群的 Velero 实例指向相同的腾讯云 [对象存储 COS](https://intl.cloud.tencent.com/document/product/436) 位置，流程如下：
...
- 已 [注册腾讯云账号](https://intl.cloud.tencent.com/register)。
- 已开通腾讯云 [对象存储 COS](https://console.cloud.tencent.com/cos5) 服务。
- 已有需要被迁移的 TKE 集群（以下称作集群 A），已创建迁移目标的 TKE 集群（以下称作集群 B），创建 TKE 集群请参见 [创建集群](https://intl.cloud.tencent.com/document/product/457/30637)。
- 集群 A 和 集群 B 都需要安装 Velero 实例（1.5版本以上），并且共用同一个对象存储 COS 存储桶作为 Velero 后端存储，安装步骤请参见 [配置存储和安装 Velero](https://intl.cloud.tencent.com/document/product/457/38939#.E9.85.8D.E7.BD.AE.E5.AD.98.E5.82.A8)。
...
本示例需要 Velero 1.5以上版本且启用 [Restic](https://velero.io/docs/v1.5/restic/) 来备份持久卷数据，请确保在安装 Velero 阶段开启 `--use-restic` 和 `--default-volumes-to-restic` 参数，安装步骤请参见 [配置存储和安装 Velero](https://intl.cloud.tencent.com/document/product/457/38939#.E9.85.8D.E7.BD.AE.E5.AD.98.E5.82.A8)。
