...
If you choose to use the HTTP-01 verification method, you must use an ingress to support the verification process. cert-manager will automatically modify the rules of an existing ingress or automatically add a new ingress to open the temporary HTTP path needed for verification. When performing HTTP-01 verification on an Issuer configuration, if the ingress `name` is specified, cert-manager automatically modifies the rules of the specified ingress to open the temporary HTTP path required for verification. If `class` is specified, cert-manager automatically adds an ingress , as detailed in the example below.
