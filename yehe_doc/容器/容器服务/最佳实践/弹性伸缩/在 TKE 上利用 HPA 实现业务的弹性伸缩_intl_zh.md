...
以 Deployment 资源类型的工作负载为例，创建一个单副本数，服务类型为  Web 服务的 “hpa-test” 工作负载。在容器服务控制台创建Deployment 类型工作负载方法请参见 [Deployment 管理](https://intl.cloud.tencent.com/document/product/457/30662)。
本示例创建结果如下图所示： 
...
再通过工作负载的网络出口带宽监控可以观察到在16:21时网络出口带宽增至大概196Kbps，已经超过 HPA 设定的网络出口带宽目标值，进一步证明此时触发 HPA  [扩缩容算法](https://intl.cloud.tencent.com/document/product/457/32424)，扩容了一个副本数来满足设定的目标值，故工作负载的副本数量变成了2个。如下图所示：
