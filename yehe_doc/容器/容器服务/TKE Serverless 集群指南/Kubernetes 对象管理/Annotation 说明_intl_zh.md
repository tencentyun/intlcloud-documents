...
超级节点上运行的 Pod 默认会根据 request 与 limit 自动计算出底层资源的规格，用户可以参考官方文档 [指定资源规格](https://intl.cloud.tencent.com/document/product/457/36161)，也可以通过给 Pod 添加注解去指定 Pod 需要的计算资源规格。示例如下：
...
### 开启竞价模式
如需开启 [竞价模式](https://intl.cloud.tencent.com/document/product/457/46967)，可在 Pod 上添加如下注解：
...
超级节点上提供 [镜像缓存能力](https://intl.cloud.tencent.com/document/product/457/44484)，即提前创建好镜像缓存实例，自动将需要的镜像下载下来并创建云盘快照，后续创建 Pod 开启镜像缓存，这样就会根据镜像名自动匹配镜像缓存实例的快照，直接使用快照里面的镜像内容，避免重复下载，加快 Pod 启动速度。
...
## 加载内核模块
如需加载 [TOA 内核模块](https://intl.cloud.tencent.com/document/product/608/18945)，可在 Pod 上添加如下注解：
...
超级节点支持通过开源的日志采集组件将日志采集到 kafka。如果采集的是容器内文件，或者采集时进行了 [多行合并](https://intl.cloud.tencent.com/document/product/457/40216)，则需合理配置单行大小。默认设置的单行大小是 2M，超过 2M 则会丢弃该行日志。日志的单行大小可通过如下注解调整：
