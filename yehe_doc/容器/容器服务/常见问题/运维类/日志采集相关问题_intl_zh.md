...
- **检查所选的日志 topic 是否开启了索引**，索引配置是使用日志服务进行检索分析的必要条件。若未开启，则无法查看日志。配置索引的详细操作，请参见 [日志服务配置索引](https://intl.cloud.tencent.com/document/product/614/39594)。
...
- 若采用环境变量的方式开启 EKS 日志采集，并选择角色授权，在**创建角色时需要选择 CVM 载体**，详情请参见 [EKS 日志采集](https://intl.cloud.tencent.com/document/product/457/37907)。
...
当客户程序的日志数据跨占多行时（例如 Java 程序日志），不能使用换行符 `\n` 作为日志的结束标识符。为了能让日志系统明确区分开每条日志，需要配置具有首行正则表达式的 configmap，当某行日志匹配了预先设置的正则表达式，则认为是一条日志的开头，而在下一个行首出现则作为该条日志的结束标识符。详情可参见 [EKS 日志采集实现多行日志合并](https://intl.cloud.tencent.com/document/product/457/40216)。
...
在应用输出日志时，应注意容器内应用尽量输出至 stdout。如果您的程序日志输出至容器的文件，需要有相应的手段来管理日志文件的滚动和清理，或者挂载 volume 持久化存储，否则20G临时空间会被占满，详情可参见 [Pod 临时存储](https://intl.cloud.tencent.com/document/product/457/34050)。

若无管理日志文件的滚动和清理手段，推荐您进行以下操作：
- 挂载 volume 持久化存储，详情可参见 [存储管理](https://intl.cloud.tencent.com/document/product/457/37769)。
- 开启 EKS 日志采集，详情可参见 [EKS 日志采集](https://intl.cloud.tencent.com/document/product/457/37907)。
