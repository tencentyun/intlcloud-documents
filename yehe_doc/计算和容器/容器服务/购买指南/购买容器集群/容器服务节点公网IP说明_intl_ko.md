...
### 1단계: NAT Gateway 생성
1. [VPC 콘솔](https://console.cloud.tencent.com/vpc/vpc?rid=1)에 로그인하고 왼쪽 사이드바에서 [[NAT Gateway](https://console.cloud.tencent.com/vpc/nat?rid=1)]를 클릭합니다.
2. ‘NAT Gateway’ 관리 페이지에서 **생성**을 클릭합니다.
3. ‘NAT Gateway 생성’ 팝업 창에서 다음 매개변수를 입력합니다.
 - 게이트웨이 이름: 사용자 정의.
...
4. **생성**을 클릭하여 NAT Gateway 생성을 완료합니다.
...
1. 왼쪽 사이드바에서 [[라우팅 테이블](https://console.cloud.tencent.com/vpc/route?rid=1)]을 클릭합니다.
2. 라우팅 테이블 목록에서 Internet에 액세스해야 하는 서브넷과 연결된 라우팅 테이블 ID/이름을 클릭합니다.
3. ‘라우팅 정책’ 섹션에서 **새 라우팅 정책**을 클릭합니다.
3. ‘라우팅 추가’ 페이지에서 **Destination**을 입력하고 **다음 홉 유형**에 대해 **NAT Gateway**를 선택하고 **다음 홉**에 대해 생성된 NAT Gateway의 ID를 선택합니다.
4. **확인**을 클릭합니다.
...
사용 방법: 자세한 내용은 [EIP 운영 가이드](https://intl.cloud.tencent.com/document/product/215/4958#.E6.93.8D.E4.BD.9C.E6.8C.87.E5.8D.97)를 참고하십시오.
