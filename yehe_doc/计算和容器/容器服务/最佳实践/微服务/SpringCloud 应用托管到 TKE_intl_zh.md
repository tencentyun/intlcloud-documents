...
腾讯云 [容器镜像服务 TCR](https://intl.cloud.tencent.com/products/tcr)（Tencent Container Registry，TCR），提供了个人版和企业版两种镜像仓库。两者区别如下：
...
PiggyMetrics 是一个 Dubbo 容器化的 Demo 项目，因此容器镜像服务个人版完全满足需求。但对于企业用户，推荐使用 [企业版 TCR](https://console.cloud.tencent.com/tcr)。如需使用镜像仓库，请见 [镜像仓库基本操作](https://intl.cloud.tencent.com/document/product/1051/35488)。  



### TSW 介绍

腾讯云微服务观测平台 TSW（Tencent Service Watcher，TSW）提供云原生服务可观察性解决方案，能够追踪到分布式架构中的上下游依赖关系，绘制拓扑图，提供服务、接口、实例、中间件等多维度调用观测。  
...
```如下图所示：
![](https://qcloudimg.tencent-cloud.cn/raw/e8d4f229984a2b8222865f0629165f60.png)
</dx-alert>
...
3. 在 [我的镜像](https://console.cloud.tencent.com/tke2/registry/user/self?rid=1) 中可以查看上传的所有镜像。  
<dx-alert infotype="explain" title=" ">
...
2. 创建完成后，在容器服务控制台 的 [集群管理](https://console.cloud.tencent.com/tke2/cluster) 页面可以看到新建的集群信息。本文新建的集群名称为 piggyMetrics。
...
3. 单击**下一步**，进入“日志解析方式”，其中本文示例 PiggyMetrics 使用单行文本方式。了解更多 CLS 支持的日志格式，请参见 [采集文本日志](https://intl.cloud.tencent.com/document/product/614/32287) 文档。  
:::
::: 方式2：使用\sCRD
用户还可通过自定义资源定义（CustomResourceDefinitions，CRD）的方式配置日志采集规则。PiggyMetrics 使用了容器文件路径的采集方式，日志格式为单行文本。下文是 account-service 日志采集具体的配置 YAML。了解更多的 CRD 采集配置请参见 [使用 CRD 配置日志采集](https://intl.cloud.tencent.com/document/product/457/43936) 文档。  
<dx-codeblock>
:::  yaml
...
如下图所示：
![](https://qcloudimg.tencent-cloud.cn/raw/1639d04a87a1b66ce10ef222fdbad466.png)
...
3. 单击服务名称 statistics-service 进入该服务的信息页，单击**接口观测**可以查看到接口 `{PUT}/{accountName}` 抛出了 NestedServletException 异常，从而导致该接口不可用。
