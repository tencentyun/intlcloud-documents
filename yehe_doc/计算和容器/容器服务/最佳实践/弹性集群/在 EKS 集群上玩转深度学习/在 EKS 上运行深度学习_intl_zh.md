...
本文将在上一篇文档 [构建深度学习容器镜像](https://intl.cloud.tencent.com/document/product/457/42059) 基础上继续操作，利用自建集群，在 EKS 上运行深度学习任务。
...
请参见 [创建集群](https://intl.cloud.tencent.com/document/product/457/34048) 文档创建 EKS 集群。
>?由于需要运行 GPU 训练任务，在创建集群时，请注意选择的容器网络所在区的支持资源，选择支持 GPU 的可用区，如下图所示：
![](https://qcloudimg.tencent-cloud.cn/raw/b589e6f916b4caca2c41d2266b9e470d.png)


### 创建 CFS 文件系统（可选）

容器将在任务结束后，自动删除容器并且释放资源。因此为了实现对模型和数据的持久化存储，建议通过挂载外部存储的方式持久存储数据。目前支持 [云硬盘 CBS](https://intl.cloud.tencent.com/document/product/362)、[文件存储 CFS](https://intl.cloud.tencent.com/document/product/582)、[对象存储 COS](https://intl.cloud.tencent.com/document/product/436) 等方式。
...
3. 在“详细设置”页面进行相关配置，CFS 类型信息与配置细节可参见 [创建文件系统及挂载点](https://intl.cloud.tencent.com/document/product/582/9132) 文档。如下图所示：
![](https://qcloudimg.tencent-cloud.cn/raw/51332f14b4aa30b19127a1da6bb12dbb.png)
...
![](https://qcloudimg.tencent-cloud.cn/raw/04c5089b337db6d92fc0bd086b995c1b.png)
...
本文任务以 MNIST 手写数字识别数据集，加两层 CNN 为例，相关示范镜像为上一章 [自建镜像](https://console.cloud.tencent.com/tke2/registry/user/self/detail/tag?rid=5&reponame=carltk%2Ftensorflow-model)，如需自定义镜像，请参见 [深度学习容器镜像构建](https://intl.cloud.tencent.com/document/product/457/42059) 文档。以下提供两种创建任务的方式。

<dx-tabs>
::: 控制台操作指引
由于深度学习任务的性质，本文以部署 Job 节点为例。如何部署 Job 请参见 [Job 管理](https://intl.cloud.tencent.com/document/product/457/30665) 文档。
以下提供控制台的部署范例：

1. 在**数据卷（选填）**配置项中，选择 NFS 盘，并输入上述步骤创建的 CFS 名称和 IPv4地址。如下图所示：
![](https://qcloudimg.tencent-cloud.cn/raw/3c08cc3dc7b9382f71fc4aeb368034ff.png)
2. 在**实例内容器**中的**挂载点**配置项里，选择数据卷，并配置挂载点。
[](id:precautions)
>!
>- 因为数据集可能需要联网下载，所以需要配置对集群的外网访问。详情请参见常见问题 [公网访问相关](https://intl.cloud.tencent.com/document/product/457/42062)。
>- 选择 GPU 型号后，在填写 request 和 limit 时需要为容器分配符合 [资源规格](https://intl.cloud.tencent.com/document/product/457/34057) 的 CPU 和内存，实际填写并不严格要求精确到个位。
...
- 在 YAML 文件中需使用 Annotations 声明资源分配，详情请参见 [Annotation 说明](https://intl.cloud.tencent.com/document/product/457/36162)。同样需要注意的是不同 GPU 对应不同的 CPU、内存选项，建议按需填写。
- 此处数据卷使用的是 NFS 。若需使用其他数据卷进行持久化存储，请参见 [其他存储卷使用说明](https://intl.cloud.tencent.com/document/product/457/30678)。
...
3. 选择**工作负载** > **Job**，在 Job 列表中单击上述步骤创建的 Job。
	- 选择**事件**页签在查看事件
	- 选择**日志**页签查看日志，如下图所示：
	![](https://qcloudimg.tencent-cloud.cn/raw/beab0951b83f90695d70ab5fa98bcde5.png)
...
因为 EKS 即用即消的特性，导致如果需要查看日志，必须当且仅当 Pod 处于 Running 状态时才可查看。解决方法请参见常见问题 [日志采集相关](https://intl.cloud.tencent.com/document/product/457/42063)。
...
- 创建 TKE 节点时，选择带有 GPU 的节点。详情请参见 [新建 GPU 云服务器](https://intl.cloud.tencent.com/document/product/457/30656) 文档。
