...
## Notes
1. You have activated [EKS](https://console.cloud.tencent.com/tke2/ecluster/startUp?rid=1).
...
TKE Resilience Chart mainly consists of a virtual node manager, scheduler, and toleration controller as detailed below:

| Alias | Component Name | Description |
| -------------------- | -------------- | -------------------------------------------------------------------- |
| eklet                | Virtual node manager | It manages the lifecycle of PodSandboxes and provides APIs related to native kubelet and nodes.  |
...
5. Workload scale-out:
  ...
  - If the number of actually created workload replicas reaches N and the settings of the global and local switches for the **"successfully scheduled"** behavior are satisfied, workloads in `pending` status will be scaled out to EKS.
6. Workload scale-in:
  - For TKE Kubernetes Distro, instances in EKS will be scaled in preferentially.
  - For K8s community edition, workloads will be scaled in randomly.
7. Scheduling rule restrictions:
  - DaemonSet Pods cannot be scheduled to virtual nodes. This feature is available only in TKE Kubernetes Distro. In K8s community edition, DaemonSet Pods will be scheduled to virtual nodes but `DaemonsetForbidden` will be displayed.
  - Pods in `kube-system` and `tke-eni-ip-webhook` namespaces cannot be scheduled to virtual nodes.
  ...
  - Pods with fixed IP enabled cannot be scheduled to virtual nodes.
8. Virtual nodes support custom DNS configuration: after you add the `eks.tke.cloud.tencent.com/resolv-conf` annotation to a virtual node, `/etc/resolv.conf` in the generated CVM instance will be updated to the custom content. **Note:** the original DNS configuration on the virtual node will be overwritten, and your custom configuration will prevail.
```
...
```
>? For more information on the regions and AZs where EKS is available, please see [Regions and AZs](https://intl.cloud.tencent.com/zh/document/product/457/41125). 
...
```
You can see that one virtual node has been deployed in the cluster.
```bash
# kubectl get node
...
### Creating test case
Create a demo application `nginx-deployment`, which has four replicas (three in EKS and one in the local cluster). Below is the sample YAML configuration:
```bash
apiVersion: apps/v1
kind: Deployment
metadata:
...
Check the scale-in feature. As a TKE Kubernetes Distro cluster is used, EKS instances will be scaled in preferentially. Here, the number of application replicas is adjusted from 4 to 3.
```bash
