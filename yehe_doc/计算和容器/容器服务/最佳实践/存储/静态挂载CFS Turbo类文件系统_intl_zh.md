...
CFS 存储类型请参考 [文件存储类型及性能规格](https://intl.cloud.tencent.com/document/product/582/33745)。
...
### 使用 CFS Turbo 存储卷
1. 创建 CFS Turbo 文件系统，具体操作请参见 [创建文件系统](https://intl.cloud.tencent.com/document/product/582/9132)。
...
spec:
  ...
  storageClassName: ""
```
>! 使用 `lustre` 协议挂载 CFS Turbo 卷需预先在集群节点内根据操作系统内核版本安装对应客户端，详情请参考 [在 Linux 客户端上使用 CFS Turbo 文件系统](https://intl.cloud.tencent.com/zh/document/product/582/40298)。
...
```
>! 在 CFS 控制台创建的 Turbo 类型存储仅支持通过私有协议挂载，若需要 NFS 协议挂载请参考 [在 CFS Turbo NFS 协议下对接 TKE](https://intl.cloud.tencent.com/zh/document/product/582/42255)。
