...
2. 针对节点数大于80的集群，建议安装 NodeLocal DNSCache，详情参见：[在 TKE 集群中使用 NodeLocal DNS Cache](https://intl.cloud.tencent.com/document/product/457/34061)
...
4. 可以通过在控制台 [安装 DNSAutoScaler 组件](https://intl.cloud.tencent.com/document/product/457/39122)，来实现自动调整 CoreDNS 副本数（要注意提前配置好平滑升级），组件的默认配置如下：
```
data:
  ladder: |-
    {
...
关于更多 NodeLocal DNSCache 的介绍及如何在 TKE 集群中部署 NodeLocal DNSCache 的具体步骤，参见：[在 TKE 集群中使用 NodeLocal DNS Cache](https://intl.cloud.tencent.com/document/product/457/34061)
...
>! 在配置完成后，需要等待5min，再继续后面的步骤；如果业务有使用 UDP 服务，请 [提交工单](https://console.intl.cloud.tencent.com/workorder/category)  来寻求帮助。
...
## 配置自定义域名解析
详情参见：[在 TKE 中实现自定义域名解析](https://intl.cloud.tencent.com/document/product/457/39125)
