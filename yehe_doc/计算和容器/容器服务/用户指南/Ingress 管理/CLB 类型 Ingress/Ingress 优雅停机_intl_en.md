...
>!This is only effective in the [direct access mode](https://intl.cloud.tencent.com/document/product/457/36837). Please check whether your cluster supports direct access.
...
The following is an example of using an annotation to indicate the use of graceful shutdown. For detailed Ingress annotations, please see [Ingress Annotation](https://intl.cloud.tencent.com/document/product/457/40675).
...
metadata: 
  annotations: 
    ...
    ingress.cloud.tencent.com/enable-grace-shutdown: "true"  # Indicate the use of graceful shutdown
  name: my-Ingress
spec: 
  selector: 
    app: MyApp
...
1. The Pod is deleted, and the status is set to `Terminating`.
2. kube-proxy updates the forwarding rule and removes the Pod from the endpoint list of the Ingress, and new traffic will no longer be forwarded to the Pod.
...
1. **Use `preStop`**
To implement graceful termination, you must process the SIGTERM signal in your business code. The main logic is to stop accepting new traffic, continue to process existing traffic, and quit after all connections are closed. For more information, please see [Go by Example: Signals](https://gobyexample.com/signals).
...
For more information on how to configure `preStop`, please see <a href="https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#lifecycle-1">Lifecycle</a>.
