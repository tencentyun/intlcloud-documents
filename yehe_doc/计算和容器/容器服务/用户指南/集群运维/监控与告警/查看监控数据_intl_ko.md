...
- [클러스터 지표 조회](#check1)
- [노드 지표 조회](#check2)
- [노드 내 Pod 지표 조회](#check3)
- [워크로드 지표 조회](#check4)
- [워크로드 내 Pod 지표 조회](#check5)
- [Pod 내 Container 지표 조회](#check6)

## 전제 조건
TKE 콘솔에 로그인하고 [[클러스터](https://console.cloud.tencent.com/tke2/cluster?rid=1)] 관리 페이지로 이동합니다.
...
2. **노드 관리**를 펼쳐 일반 노드와 Master&Etcd 노드에 대한 모니터링 정보를 확인합니다.
 - **노드**>**모니터링**을 선택하여 **노드 모니터링** 페이지로 이동하고 모니터링 정보를 확인합니다. 아래 이미지를 참고하십시오.
![](https://main.qcloudimg.com/raw/c1c1fca1e108f8479b50a895c2d2d0b5.png)
 - **Master&Etcd**>**모니터링**을 선택하면 **Master&Etcd 모니터링** 페이지로 이동하여 모니터링 정보를 확인할 수 있습니다. 아래 이미지를 참고하십시오.
...
2. **노드 관리**>**노드**를 선택하여 노드 목록 페이지로 이동합니다.
3. 필요에 따라 노드에서 Pod의 지표를 조회하는 방법을 선택합니다.
 - 노드 목록에서 Pod 지표 확인하기.
    1. **모니터링**을 클릭하여 **노드 모니터링** 페이지로 이동합니다.
    2. **Pod**를 클릭하고 확인할 노드로 **소속 노드**를 선택합니다. 이렇게 하면 해당 노드의 Pod 모니터링 지표 대조표를 확인할 수 있습니다. 아래 이미지를 참고하십시오.
![](https://main.qcloudimg.com/raw/e58301ce2675d2de018a867c31fcfb69.png)
 - 노드 세부 정보 페이지에서 Pod 지표 확인하기.
    ...
    2. **Pod 관리** 탭을 선택하고 **모니터링**을 클릭하여 해당 노드의 Pod 모니터링 지표 대조표를 확인합니다. 아래 이미지를 참고하십시오.
![](https://main.qcloudimg.com/raw/269631374cbe8ff955de4d691c53772c.png)

<span id="check4"></span>
### 워크로드 지표 조회
1. 확인할 클러스터의 ID/이름을 선택하여 클러스터 관리 페이지로 이동합니다.
2. **워크로드**>**모든 워크로드 유형**을 선택합니다. 예를 들어 **Deployment**를 선택하여 Deployment 관리 페이지로 이동합니다.
3. **모니터링**을 클릭하여 워크로드 모니터링 정보를 확인합니다. 아래 이미지를 참고하십시오.
...
4. **Pod 관리** 탭을 선택하고 **모니터링**을 클릭하여 해당 워크로드의 Pod 모니터링 지표 대조표를 확인합니다. 아래 이미지를 참고하십시오.
![](https://main.qcloudimg.com/raw/bda91f0b0fb55cf87cf7258cc471ae1f.png)

<span id="check6"></span>
### Pod에 있는 Container의 지표 조회
1. 워크로드 세부 정보 페이지로 이동하려면 [워크로드 내 Pod 지표 조회](#target)의 [1단계](#first) - [3단계](#third)을 참고하십시오.
2. **Pod 관리** 탭을 선택하고 **모니터링**을 클릭하여 **Pod 모니터링** 페이지로 이동합니다.
3. **Container**를 클릭하고 조회할 Pod로 **소속 Pod**를 선택합니다. 이렇게 하면 해당 Pod의 Container에 대해 모니터링되는 지표의 플롯을 볼 수 있습니다. 아래 이미지를 참고하십시오.
