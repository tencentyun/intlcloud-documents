...
상태 확인은 다음 유형으로 나뉩니다.
- **활성 검사**: 컨테이너의 활성 상태 여부를 확인합니다. 이는 ps를 사용하여 프로세스가 있는지 여부를 확인하는 것과 유사합니다. 활성 확인 실패 시 클러스터가 컨테이너를 재시작합니다. 활성 확인이 성공되면 작업이 수행되지 않습니다.
- **준비 상태 확인**: 컨테이너가 사용자 요청을 처리할 준비가 되었는지 확인합니다. 예를 들어 애플리케이션이 서비스를 제공하기 위해 데이터 디스크 마운트 또는 외부 모듈 시작에 의존하기 때문에 시작하는 데 시간이 걸리는 경우 Kubernetes는 준비 상태 프로브를 사용하여 애플리케이션 시작이 완료되었는지 확인합니다. 준비 프로브가 실패하면 준비 프로브가 성공할 때까지 해당 컨테이너에 대한 요청이 차단됩니다.
...
```
다음은  [TKE 콘솔](https://console.cloud.tencent.com/tke2)을 사용하여 상태 확인을 설정하는 방법입니다.
 1. 클러스터의 ‘Deployment’ 페이지에서 **생성**을 클릭합니다.
 2. ‘Workload 생성’ 페이지로 이동하여 ‘포드의 컨테이너’ 섹션에서 **고급 설정**을 선택합니다.
  3. ‘컨테이너 상태 확인’에 대해 **활성 확인**을 선택하고 다음 매개변수를 설정합니다.
   - **확인 방법**: ‘명령 확인 실행’을 선택합니다.
   - **명령 실행**: 다음을 입력합니다.
   ```
sh 
/data/scripts/health_check.sh 
...
- **시작 대기 시간**: 단위는 초. 컨테이너가 시작된 후 프로브를 시작하기 전에 대기할 시간을 지정합니다. 예를 들어 시작 지연이 5로 설정된 경우 컨테이너가 시작된 후 5초 후에 상태 확인이 시작됩니다.
- **간격**: 단위는 초. 상태 확인 빈도를 지정합니다. 예를 들어 간격을 10으로 설정하면 상태 확인이 10초마다 한 번씩 실행됩니다.
- **응답 시간 초과**: 단위는 초. 상태 확인을 위한 시간 초과 기간을 지정합니다. TCP 포트 프로브, HTTP 요청 프로브, 실행 명령 확인에 대한 TCP 연결 시간 초과 기간, HTTP 요청 응답 시간 초과 기간 및 실행 명령 시간 초과 기간을 각각 나타냅니다.
- **정상 임계값**: 단위는 횟수. 컨테이너가 정상으로 결정되기 전에 연속적인 상태 확인이 성공한 시간을 지정합니다. 예를 들어 정상 임계값이 3으로 설정된 경우 프로브가 연속 3회 성공하는 경우에만 컨테이너가 정상으로 간주됩니다.
>! 활성 상태 확인의 경우 정상 임계값은 1일 수 있습니다. 다른 값은 유효하지 않습니다.
- **비정상 임계값**: 단위는 횟수. 컨테이너가 비정상으로 결정되기 전에 연속적인 상태 확인 실패 횟수를 지정합니다. 예를 들어 비정상 임계값이 3으로 설정된 경우 프로브가 연속적으로 세 번 실패하는 경우에만 컨테이너가 비정상으로 간주됩니다.
