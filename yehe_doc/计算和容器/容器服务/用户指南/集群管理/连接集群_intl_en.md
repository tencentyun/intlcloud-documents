...
TKE will upgrade the cluster access from July 20, 2022 (UTC +8). After the upgrade, a new architecture will be used when you enable public/private network access for a cluster. Existing clusters that have enabled the feature are not affected. Please re-enable cluster access during July 20 to August 10 to migrate to the new architecture. For details, see [Enabling Cluster Access](#CreateClusterEndpoint) and [Obtaining Kubeconfig](#installKubectl).
