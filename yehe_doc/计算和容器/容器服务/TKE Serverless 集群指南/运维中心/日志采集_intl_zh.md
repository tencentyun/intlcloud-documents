...
5. 参考以下信息进行日志消费端配置，您可选择 CLS 或 Kafka 作为日志消费端。如下图所示：
  - 推荐选择 [日志服务 CLS](https://intl.cloud.tencent.com/product/cls) 为消费端，并选择日志集和日志主题。若无合适的日志集，请参考 [配置日志服务 CLS 作为日志消费端](#配置CLS作为日志消费端)。
   - 若选择 Kafka 为消费端，请参考 [配置 Kafka 作为日志消费端](#配置Kafka作为日志消费端)。
...
>! API 密钥对应的用户需具备访问日志服务 CLS 的权限。若无 API 密钥，需新建 API 密钥。详情请参见 [访问密钥](https://intl.cloud.tencent.com/document/product/598/34227)。
