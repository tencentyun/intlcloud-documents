## 작업 시나리오
본 문서는 컨테이너 클러스터에서 Node.js Hello World 서비스를 빠르게 생성하는 방법을 설명합니다. Docker 이미지를 빌드하는 방법에 대한 자세한 내용은 [docker 이미지를 빌드하는 방법](https://intl.cloud.tencent.com/document/product/457/9115)을 참고하십시오.
...
#### Docker 이미지 생성
>?Docker 이미지에 대한 자세한 내용은 [docker 이미지를 빌드하는 방법](https://intl.cloud.tencent.com/document/product/457/9115)을 참고하십시오.
>
...
#### qcloud 이미지 레지스트리에 이미지 업로드
>!이미지를 업로드하기 위한 전제 조건:
>- [내 이미지](https://console.cloud.tencent.com/tke2/registry/user/space)에서 네임스페이스를 생성 완료해야 합니다.
>- [Tencent Cloud Registry](https://intl.cloud.tencent.com/document/product/457/9117)에 로그인 해야합니다. 이미지 사용에 대한 자세한 내용은 [이미지 레지스트리 사용 설명서](https://intl.cloud.tencent.com/document/product/457/9117)를 참고하십시오.

다음 명령을 순서대로 실행하여 이미지를 qcloud 이미지 레지스트리에 업로드합니다.
```shell
...
```
>?
>- 명령의 IMAGEID를 [이미지 확인](#search)에서 기록한 IMAGEID로 바꿉니다.
...
2. ‘클러스터 관리’ 페이지에서 클러스터의 ID를 클릭하여 서비스를 생성하고 클러스터의 워크로드 ‘Deployment’ 페이지로 이동합니다. 그런 다음 다음 그림과 같이 [생성]을 클릭합니다.
![](https://main.qcloudimg.com/raw/bacaf92e14b7c342db6b3179c2ae5e8f.png)
3. ‘Workload 생성’ 페이지에서 지침에 따라 워크로드의 기본 정보를 지정합니다. 아래 이미지를 참고하십시오.
![](https://main.qcloudimg.com/raw/e2d083fececab9d1f84dd82f3850537a.png)
 - **워크로드 이름**: 워크로드의 이름을 입력합니다. 이 예시에서 이름은 helloworld입니다.
 - **설명**: 관련 워크로드 정보를 지정합니다.
 - **태그**: key = value 키 값 쌍을 지정합니다. 이 예시에서 기본값은 k8s-app = **helloworld**입니다.
 - **네임스페이스**: 필요에 따라 네임스페이스를 선택합니다.
 - **유형**: 필요에 따라 유형을 선택합니다.
 - **볼륨**: 마운트할 워크로드 볼륨을 설정합니다. 자세한 내용은 [Volumne 관리](https://intl.cloud.tencent.com/document/product/457/30678)를 참고하십시오.
4. 지침에 따라 ‘Pod의 컨테이너’를 설정합니다. 아래 이미지를 참고하십시오. 
컨테이너 이름을 입력합니다. 이 예시에서 이름은 helloworld입니다.
![](https://main.qcloudimg.com/raw/27b651e922b4a3afb925326ed8393bd0.png)
[이미지 선택]을 클릭하고 나타나는 대화 상자에서 [내 이미지]를 클릭합니다. 다음 그림과 같이 검색창을 이용하여 helloworld 이미지를 찾은 후 [확인]을 클릭합니다.
![](https://main.qcloudimg.com/raw/86a18f657b75d338ab3c084710c3ba10.png)
주요 매개변수 정보는 다음과 같습니다.
 - **이미지 태그(Tag)**: 기본 값 v1을 사용합니다.
...
5. ‘포드 수’ 섹션에서 지침에 따라 서비스에 대한 포드 수를 설정합니다. 아래 이미지를 참고하십시오.
![](https://main.qcloudimg.com/raw/6cc62e4c9118b83f7c4552a55f4c4cf0.png)
 - **수동 조정**: 포드 수를 지정합니다. 이 예에서는 1로 설정되어 있습니다. ‘+’ 또는 ‘-’를 클릭하여 포드 수를 변경할 수 있습니다.
 - **자동 조정**: 설정된 조건 중 하나라도 충족되면 자동으로 pod 수를 조정합니다. 자세한 내용은 [서비스 자동 스케일 다운](https://intl.cloud.tencent.com/document/product/457/32424)을 참고하십시오.
6.   지침에 따라 워크로드에 대한 액세스 설정(Service)을 구성합니다. 아래 이미지를 참고하십시오.   
![](https://main.qcloudimg.com/raw/57b97c8877fd3ac116c71fad4bf416f2.png)
 - **Service**: ‘활성화’를 선택합니다.
 - **서비스 액세스**: ‘공용 네트워크 액세스’를 선택합니다.
...
7. [Workload 생성]을 클릭하여 Hello World 서비스를 생성합니다.
...
2. Hello World 서비스가 속한 클러스터의 ID를 클릭하고 [서비스]>[Service]를 선택합니다.
...
컨테이너를 생성할 수 없는 경우 [이벤트 FAQ](https://intl.cloud.tencent.com/document/product/457/8187)를 참고하십시오.
