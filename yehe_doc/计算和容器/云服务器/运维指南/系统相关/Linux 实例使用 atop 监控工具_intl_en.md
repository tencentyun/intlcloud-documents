...
-**c**: Filter by CPU utilization of the process in descending order.
-**m**: Filter by memory usage of the process in descending order.
-**d**: Filter by disk usage of the process in descending order.
-**a**: Filter by resource usage of the process in descending order 
...
- **ATOP line**: CVM name, information sampling date and point in time.
- **PRC line**: Overall operation status of the process.
 - sys and user: The ratio between CPU service time in process kernel mode and that in process user mode.
 - #proc: Total number of processes.
 - #zombie: Number of zombie processes.
 - #exit: Number of processes exited during the ATOP sampling period.
-**CPU line**: Usage of overall CPU configuration (Deem multi-core CPUs as a complete CPU resource). All values of the fields on the CPU line are added up to N00%, and N is equal to the number of CPU cores.
 ...
 - idle: The ratio between the CPU's idle time and the sampling period. 
...
- **CPL line**: CPU loading conditions.
 ...
 - csw: It indicates the number of context switches.
 - intr: It indicates the number of interruptions.
...
- **PAG line**: Paging of virtual memory
 - swin and swout: Number of memory pages swapped in and out.
- **DSK line**: Disk usage. A column of information shows the usage of each disk device. Add a line of DSK information if there is sdb device.
