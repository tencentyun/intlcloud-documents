## 概要
このドキュメントでは、Windows Cloud Virtual Machine（CVM）のOSをアクティブ化する方法について説明します。
...
3. powershellウィンドウでは、次のコマンドを順番に実行して、OSをアクティブにします。
```
slmgr /upk
```
```
...
## 関連する質問
Windows OSがアクティブ化されていない一部のシナリオでは、ハイエンドマシンのシステムメモリは2GBに制限され、残りのメモリは「ハードウェア用に予約されたメモリ」の形式で制限されます。その理由は、`HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\ProductOptions`レジストリが破損されるためです。次のコマンドを実行して、システムを再度アクティブにするかどうかを決定できます。
```
(Get-ItemProperty HKLM:\SYSTEM\CurrentControlSet\Control\ProductOptions\).ProductPolicy.count
```
 - 返された結果が56184などの10000レベルの値である場合、システムを再度アクティブにする必要はありません。
 - 返された結果が「非アクティブ値：1960」の場合、次の方法を参照して解決してください。
<dx-tabs>
::: 方法1
1. 次のコマンドを実行して、システムをアクティブにします。
```
slmgr.vbs /ipk <ProductKey>
```
<dx-alert infotype="explain" title="">
`<ProductKey>`実際のOSのバージョンに応じて置き換えてください。詳細については、[ProductKey](#ProductKey)をご参照ください。
</dx-alert>
