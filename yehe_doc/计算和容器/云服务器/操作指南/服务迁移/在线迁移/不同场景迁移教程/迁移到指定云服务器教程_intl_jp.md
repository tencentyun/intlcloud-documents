...
このドキュメントでは、オンラインマイグレーションコンソールを使用して、ソースサーバー上のシステム、アプリケーションプログラムなどを自分で作成したコンピュータルーム(IDC)やクラウドプラットフォームなどのソース環境から指定されるCVMに移行する操作手順についてご紹介します。
...
4. オンラインマイグレーションコンソールに移動し、移行タスクを作成します。
    ...
    例えば、Linux移行元サーバーをTencent Cloud広州リージョンのCVM1にマイグレーションする場合、移行タスク設定は下図のように行います：
