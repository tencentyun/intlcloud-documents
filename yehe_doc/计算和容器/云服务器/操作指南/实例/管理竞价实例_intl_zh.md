...
- **云 API**：[RunInstance 接口](https://intl.cloud.tencent.com/zh/document/product/213/33237) 已增加竞价实例相关参数。
...
1. 登录 [云服务器购买页面](https://buy.intl.cloud.tencent.com/cvm?regionId=1&projectId=-1)。
...
- **固定价格**
当前阶段采用固定折扣模式，您必须设置参数为大于等于当前市场价格，市场价格详细请参见 [当前竞价实例支持哪些地域和实例类型及规格](https://intl.cloud.tencent.com/document/product/213/17817)。
...
:::
::: 云\sAPI
RunInstance 接口内的 [InstanceMarketOptionsRequest](https://intl.cloud.tencent.com/zh/document/api/213/15753?from_cn_redirect=1) 参数可指定使用竞价实例模式和配置相关信息。
* **同步接口**：目前 RunInstance 提供的是一次性的同步请求接口，即申请失败（库存不足、请求价格低于市场价格）则马上返回失败，且不再继续申请。
* **固定价格**：当前阶段采用固定折扣模式，您必须设置参数为大于等于当前市场价格，市场价格详细请参见 [当前竞价实例支持哪些地域和实例类型及规格](https://intl.cloud.tencent.com/document/product/213/17817)。
