...
- 导出镜像到 COS 存储桶，会产生以下费用：
 - **[存储容量费用](https://intl.cloud.tencent.com/document/product/436/40099)**：镜像存储在 COS 存储桶，会产生存储容量费用。COS 会计算对象大小，按照目标对象的存储类型和所属地域计费。
 - **[请求费用](https://intl.cloud.tencent.com/document/product/436/40100)**：导出镜像到 COS 存储桶，会产生写请求费用。COS 会计算写请求次数，并收取请求费用。
 - **[流量费用](https://intl.cloud.tencent.com/document/product/436/33776)**：导出镜像到 COS 存储桶，会产生上行流量。COS 会计算流量大小，内网上行流量、外网上行流量均免费。
- 从 COS 存储桶下载镜像，会产生以下费用：
 - **[请求费用](https://intl.cloud.tencent.com/document/product/436/40100)**：从 COS 存储桶下载镜像，会产生写请求费用。COS 会计算写请求次数，并收取请求费用。
 - **[流量费用](https://intl.cloud.tencent.com/document/product/436/33776)**：从 COS 存储桶下载镜像，会产生下行流量。COS 会计算流量大小，内网下行流量免费、外网下行流量收费。
