...
CBSの拡張完了後、 [拡張パーティションおよびファイルシステム（Windows）](https://intl.cloud.tencent.com/document/product/362/31601)または[拡張パーティションおよびファイルシステム（Linux）](https://intl.cloud.tencent.com/document/product/362/31602)を実行し、拡張部分の容量を既存のパーティション内に区分するか、または独立した新しいパーティションにフォーマットする必要があります。
...
5. 「拡張パーティションおよびファイルシステム」の手順で、注意事項を確認し、【調整開始】をクリックすれば完了です。
6. 対象のクラウドサービスのオペレーティングシステムタイプに応じて、 [拡張パーティションおよびファイルシステム（Windows）](https://intl.cloud.tencent.com/document/product/362/31601)または[拡張パーティションおよびファイルシステム（Linux）](https://intl.cloud.tencent.com/document/product/362/31602)を実行し、拡張部分の容量を既存のパーティション内に区分するか、または独立した新しいパーティションにフォーマットする必要があります。

#### CBSコンソールで拡張する
1. [CBSコンソール](https://console.cloud.tencent.com/cvm/cbs)にログインします。
2. 対象のCBSの【その他】>【拡張】を選択します。
3. 必要な新しい容量サイズ（現在のサイズ以上である必要があります）を選択します。
4. 支払いを完了します。
5. 対象のクラウドサービスのオペレーティングシステムタイプに応じて、 [拡張パーティションおよびファイルシステム（Windows）](https://intl.cloud.tencent.com/document/product/362/31601)または[拡張パーティションおよびファイルシステム（Linux）](https://intl.cloud.tencent.com/document/product/362/31602)を実行し、拡張部分の容量を既存のパーティション内に区分するか、または独立した新しいパーティションにフォーマットする必要があります。
...
## システムディスクの拡張
拡張タイプがシステムディスクのCBSの場合、以下の2つの方法で拡張することができます。
...
4. 「拡張パーティションおよびファイルシステム」の手順で、注意事項を確認し、「強制シャットダウンに同意する」を選択した後、【調整開始】をクリックすれば完了です。
...
#### システムの再インストールで拡張する
CVMで[システムの再インストール](https://intl.cloud.tencent.com/document/product/213/4933) 操作を行う場合、同時にシステムディスクを拡張することもできます。
...
![](https://main.qcloudimg.com/raw/03d38f34651d317176c50f1ed3a03f30.png)
 ...
 - 含まない場合、対象のクラウドサービスのオペレーティングシステムタイプに応じて、手動でファイルシステムとパーティションを拡張する必要があります。[拡張パーティションおよびファイルシステム（Linux）](https://intl.cloud.tencent.com/document/product/362/31602)を実行し、拡張部分の容量を既存のパーティションに区分するか、新しい独立したパーティションにフォーマットする必要があります。

[](id:confirmwindowsConfig)
#### Windowsインスタンスのcloudinit設定を確認する
