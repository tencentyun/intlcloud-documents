## 스팟 인스턴스란 무엇인가요?
스팟 인스턴스(Spot)는 할인된 가격과 시스템 중단 메커니즘을 특징으로 하는 CVM 인스턴스의 새로운 모드입니다. 즉, 할인된 가격으로 스팟 인스턴스를 구매할 수 있지만 시스템에서 자동으로 다시 회수할 수 있습니다. 스팟 인스턴스를 사용할 때 작업은 콘솔 작업, 원격 로그인, 서비스 배포 및 VPC와의 연결을 포함하여 종량제 CVM 인스턴스로 수행하는 작업과 거의 동일합니다.
...
## 현재 단계의 특수 정책
- **고정 할인**: 모든 스팟 인스턴스는 동일한 사양의 종량제 인스턴스 가격에서 80% 할인된 고정 할인으로 제공됩니다. 일부 유형의 인스턴스에 대한 할인은 약간 조정될 수 있습니다.
...
- **모든 리전에서 사용 가능**: 스팟 인스턴스는 거의 모든 Tencent Cloud 리전에서 사용할 수 있습니다. 스팟 인스턴스가 지원하는 인스턴스 유형은 종량제 인스턴스가 지원하는 유형과 동일합니다. 지원되는 최신 리전 및 인스턴스 유형은 [스팟 인스턴스 - 지원되는 리전 및 인스턴스 유형](https://intl.cloud.tencent.com/document/product/213/17817#.E5.BD.93.E5.89.8D.E7.AB.9E.E4.BB.B7.E5.AE.9E.E4.BE.8B.E6.94.AF.E6.8C.81.E5.93.AA.E4.BA.9B.E5.9C.B0.E5.9F.9F.E5.92.8C.E5.AE.9E.E4.BE.8B.E7.B1.BB.E5.9E.8B.E5.8F.8A.E8.A7.84.E6.A0.BC.EF.BC.9F)을 참고하십시오.

## 제품 특징
### I. 비용 효율성
![](https://main.qcloudimg.com/raw/8179ef6629ac0a0b4b9c3c9cd6f80ffa.png)
스팟 인스턴스는 종량제 인스턴스 가격에서 최대 90% 할인된 가격으로 판매됩니다.

- **할인 범위**: 스팟 인스턴스는 동일한 사양의 종량제 인스턴스 가격에서 최대 90% 할인된 가격으로 판매됩니다.
- **할인 제외**: 할인은 CVM CPU 및 메모리에만 적용됩니다. 시스템 디스크, 데이터 디스크, 대역폭 및 유료 이미지를 포함한 기타 CVM 관련 청구 항목은 할인 대상에서 제외됩니다.
- **가격 변동**: 할인은 일정 기간 동안 안정적입니다. 그러나 가용존에서 대량 구매가 있는 경우 가격이 변동될 수 있습니다. 현재 할인율은 80%로 고정되어 있습니다.
...
- **가격**: 시장 가격이 최고 입찰가보다 높으면 스팟 인스턴스가 다시 회수됩니다. 그러나 현 단계에서는 시장 가격이 고정될 것입니다.
- **재고로 인한 중단**: 스팟 인스턴스가 부족할 경우 Tencent Cloud는 부족에 따라 스팟 인스턴스를 재회수합니다. 공급이 충분해지면 다시 스팟 인스턴스를 신청할 수 있습니다.
...
## 제한
- **할당량**: 스팟 인스턴스 할당량은 가용존에서 소유한 모든 스팟 인스턴스의 총 vCPU 코어 수를 제한합니다. 현재 각 계정은 각 가용존에서 최대 50개의 vCPU 코어를 가질 수 있습니다. 더 높은 할당량을 원하시면 [티켓 제출](https://console.cloud.tencent.com/workorder/category)을 통해 신청하십시오.
- **작업 제한1**: 구성 업그레이드 및 다운그레이드는 스팟 인스턴스에 대해 지원되지 않습니다.
- **작업 제한2**: 스팟 인스턴스의 청구 방법을 월간 구독으로 전환할 수 없습니다.
