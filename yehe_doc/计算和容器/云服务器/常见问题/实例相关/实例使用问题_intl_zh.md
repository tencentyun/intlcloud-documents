...
### 如何查询 Linux 实例的账号和密码？
云服务器实例的账号和密码会在创建成功后将以 [站内信](https://console.cloud.tencent.com/message) 的方式发送到您的腾讯云账户上。Linux 系统的默认管理员账号是 root。
...
腾讯云服务器提供的是常规服务器，不是多媒体服务器，默认不提供声卡和显卡组件，所以在系统中无法添加声卡和显卡。
