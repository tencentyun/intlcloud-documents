...
## Solution
1. Check the memory utilization and total number of processes as instructed in [Steps](#ProcessingSteps).
...
2. Check whether the number of processes exceeds the limit as instructed in [Log Error "fork: Cannot allocate memory"](https://intl.cloud.tencent.com/document/product/213/40502). If not, proceed to the next step.
...
vim /etc/sysctl.conf
```
5. Press **i** to enter the edit mode and modify the `vm.min_free_kbytes` configuration item. If it doesn't exist, simply add it in the configuration file.
