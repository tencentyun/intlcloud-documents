## Problem
When I tried to log in to the CVM via VNC, the error message “Cannot allocate memory” appeared.
![](https://main.qcloudimg.com/raw/0a31fdd909701c27c9923b2fff24668a.png)

## Cause[](id:PossibleCauses)
There are too many huge pages. The default huge page size is 2048 KB. The number of huge pages is stated in `/etc/sysctl.conf`. If there are 1280 huge pages (as shown below), 2.5 GB of memory are taken. In case of low instance specification, there may not be enough memory for proper system running, and you cannot enter the system after restarting it.
![](https://main.qcloudimg.com/raw/1978a0b2a85fc828674f720c108c48a3.png)


## Solution
1. Perform the [troubleshooting procedure](#ProcessingSteps) to check whether the total number of processes exceeds the limit. 
2. Modify the configurations of huge pages as needed.


## Steps[](id:ProcessingSteps)
1. Check whether the total number of processes exceeds the limit as instructed in [Log Error “fork: Cannot allocate memory”](https://intl.cloud.tencent.com/document/product/213/40502). If not, proceed to the next step.
...
vim /etc/sysctl.conf
```
5. Press **i** to enter the edit mode and reduce the value of `vm.nr_hugepages` as needed.
