...
<img src="https://qcloudimg.tencent-cloud.cn/raw/15f9b82876e48db915cab4232d686640.png" style="width:100%"/>
...
<td>Web 서비스는 일반적으로 개인 웹 사이트, 블로그, 미니프로그램 및 대형 전자 상거래 웹 사이트 등을 포함하며 컴퓨팅, 스토리지, 메모리 같은 리소스에 대한 수요가 균일하여 서비스에 필요한 설정이 가능한 표준형 인스턴스를 추천합니다. </td>    
...
<td>CPM 2.0은 재 가상화 능력을 갖추고 있으며 오프라인 개인 클라우드를 Tencent Cloud로 마이그레이션할 수 있어 내포된 가상화로 인한 성능 부하를 걱정할 필요가 없습니다. </td>
...
<img src="https://qcloudimg.tencent-cloud.cn/raw/2f7f44ae615893f69a72ca64e2fa67fb.png" style="width:100%"/>
...
- **[클라우드 모니터링](https://intl.cloud.tencent.com/document/product/248/32799)**
클라우드 리소스에 대해 클라우드 모니터링 알람을 설정하는 것은 원활한 서비스 운영에 매우 중요한 역할을 합니다. 클라우드 모니터링을 사용하여 클라우드 서비스 리소스 사용률, 응용 프로그램 성능 및 클라우드 서비스의 운영 상황을 전체적으로 파악할 수 있습니다. 또한 클라우드 모니터링은 지표 모니터링, 사용자 정의 알람, 리전 간/프로젝트 간 인스턴스 그룹화, 사용자 정의 모니터링, 시각화 Dashboard 및 Prometheus 호스팅 서비스 등의 기능을 지원합니다. 이를 통해 클라우드 서비스에 돌발 상황이 발생한 경우 적시에 해결 할 수 있어 시스템 안정성과 운영 효율을 높이며 운영 비용을 절감할 수 있습니다. 
- **[CLB](https://intl.cloud.tencent.com/document/product/214/524)**
서비스의 단일 노드 실행 리스크를 방지하고자 하는 경우, CLB를 설정할 수 있습니다. CLB 서비스는 버츄얼 서비스 주소(VIP)를 설정하여 같은 리전의 다수의 CVM 리소스를 성능과 가용성이 우수한 하나의 애플리케이션 서비스 풀로 가상화하는 것입니다. 애플리케이션에 지정된 방식에 따라 클라이언트의 네트워크 요청을 CVM 풀로 배포할 수 있습니다.
CLB 서비스는 CVM 풀의 CVM 인스턴스 상태를 점검합니다. 비정상 상태의 인스턴스를 자동으로 격리하고 CVM의 단일 노드 문제를 해결하며 애플리케이션의 전반적인 서비스 성능을 향상시킵니다.
