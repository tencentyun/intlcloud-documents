...
- 다음 설정에는 Tencent Cloud 미러 이미지 소스가 사용되었습니다. Tencent Cloud 미러 이미지 소스는 MariaDB 공식 웹사이트 소스와 동시 업데이트 되었습니다. MariaDB 10.4 버전 소스가 유효하지 않은 문제가 있을 수 있습니다(본문은 CentOS 7.6에 MariaDB 10.4.22 버전 설치를 예시로 설명). [MariaDB 공식 웹사이트](https://downloads.mariadb.org)에서 다른 버전 및 운영 체제의 MariaDB 설치 정보를 얻을 수 있습니다.
