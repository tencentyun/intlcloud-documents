...
```
For more information on the permissions obtained during quick authorization, please see [scf_QcsRole role permission list](#list).
...
You can run the `scf credentials` command to quickly set the persistent storage of the global key information. This command must be configured under the created scf project. Please make sure that you have created a project with `serverless.yml` through `scf init` or manually.
...
### scf_QcsRole role permission list[](id:list)
...
| QcloudAccessForscfRole | This policy can be associated with the Serverless Framework (scf) service role (scf_QCSRole) for scf' quick experience feature to access other Tencent Cloud service resources. It contains permissions of CAM-related operations. |
