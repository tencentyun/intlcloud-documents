...
您可以在云函数中，通过编写代码，快速连接您本地的数据库或者腾讯云云数据库，本文介绍如何使用已有 SDK 在云函数代码中连接 [MySQL](https://intl.cloud.tencent.com/document/product/236/5147) 云数据库，并实现对数据库的插入、查询等操作。同时还支持已有 SDK 连接 [TDSQL-C](https://intl.cloud.tencent.com/document/product/1098/40615) 或 [TDSQL MySQL版](https://intl.cloud.tencent.com/document/product/1042/33311) 数据库，您可按需进行相应操作。

>? 您也可以通过 Serverless Framework 组件完成数据库与函数的部署，详情请参见 [Serverless 组件](https://intl.cloud.tencent.com/zh/document/product/1040/33163)。


## 前提条件

- 已 [注册腾讯云账号](https://intl.cloud.tencent.com/register) 并 [完成实名认证](https://console.cloud.tencent.com/developer/auth)。
- 网络环境打通：
...
按照以下步骤创建 VPC 和子网，详情可参见 [快速搭建私有网络](https://intl.cloud.tencent.com/document/product/215/31891)。
...
以下步骤将以 [云原生数据库 TDSQL-C](https://intl.cloud.tencent.com/document/product/1098/40626) 为例，指导您如何快速创建 MySQL 数据库。

>? 其它类型数据库请参考相关产品文档：
>- [云数据库 MySQL](https://intl.cloud.tencent.com/document/product/236/37785)
>- [云数据库 PostgreSQL](https://intl.cloud.tencent.com/document/product/409/40724) 
>- [云数据库 Redis](https://intl.cloud.tencent.com/document/product/239/37712)

1. 登录TDSQL-C 购买页，选择所要部署的地域、可用区以及数据库规格等信息等，单击**立即购买**。
2. 购买成功后，返回集群列表，待集群状态显示为“运行中”，即可正常使用。如下图所示：
![](https://qcloudimg.tencent-cloud.cn/raw/f6c763f68f429af94d452c6fc8347174.png)
3. 单击集群 ID，进入集群详情页面。可以对您的数据库集群进行配置修改、账号管理、安全组设置等操作，详情请参见 [管理 TDSQL-C 集群](https://intl.cloud.tencent.com/document/product/1098/40628)。如下图所示：
...
<dx-alert infotype="explain" title="">
使用已有 SDK，需要先安装依赖包，详情请参见 [依赖安装](https://intl.cloud.tencent.com/document/product/583/34879)。
</dx-alert>
```js
...
您可以参考以下代码示例创建云函数并为函数配置与示例代码对应的 [环境变量](https://intl.cloud.tencent.com/document/product/583/32748)：
...
为方便使用，云函数团队将 Node.js 和 Python 连接池相关代码封装为 SCF DB SDK for MySQL，通过该 SDK，您可以在云函数代码中连接 [MySQL](https://intl.cloud.tencent.com/document/product/236/5147)、[TDSQL-C](https://intl.cloud.tencent.com/document/product/1098/40615) 或 [TDSQL MySQL版](https://intl.cloud.tencent.com/document/product/1042/33311) 数据库，并实现对数据库的插入、查询等操作。
...
>2. 其他 Node.js 版本可参考 [依赖安装](https://intl.cloud.tencent.com/document/product/583/34879) 安装 `scf-nodejs-serverlessdb-sdk` 进行使用。
...
- 云数据库 MySQL 提供 [数据库代理](https://intl.cloud.tencent.com/document/product/236/42048)功能，通过代理集群中转访问数据库的主从节点，进行读写分离，将读请求转发至只读实例，降低主库的负载。
