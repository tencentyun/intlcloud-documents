...
3. 选择服务和操作。
 ...
 - **资源描述**：`*`
 - **条件（可选）**：置空
4. 编辑策略名称和备注（建议修改为便于理解的策略名称），单击**完成**。

### 步骤3：把策略关联到用户/用户组
1. 在“[策略](https://console.cloud.tencent.com/cam/policy)”管理页面，单击新建策略右侧的**关联用户/组/角色**，弹出关联提示框。
...
3. 选择服务和操作。
 ...
 - **资源描述**：`*`
 - **条件（可选）**：置空
4. 请编辑策略名称和备注（建议修改为便于理解的策略名称），并对策略内容进行修改，替换成 [SCF 策略语法](https://intl.cloud.tencent.com/document/product/583/38177) 中策略示例的代码。
