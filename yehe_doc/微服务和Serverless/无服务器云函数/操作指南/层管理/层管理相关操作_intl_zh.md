...
2. 参考 [部署函数](https://intl.cloud.tencent.com/document/product/583/32741) 将本地函数代码打包上传，打包时执行以下命令排除 `node_modules` 文件夹。
   ``` shell
...
2. 参考 [部署函数](https://intl.cloud.tencent.com/document/product/583/32741) 将本地函数代码打包上传，已经上传到层中的文件无需跟随函数代码再次进行上传。
3. 参考 [绑定云函数](#bind) 步骤，将已创建的层绑定至部署好的函数。 
4. 完成上述步骤后，即可开始在函数中引用层中的文件。
 ``` python
 # -*- coding: utf8 -*-
