...
在地域维度，每个账号的**弹性并发的扩容速度上限（函数 burst）默认限制为500个/分钟**，即该地域下所有函数在1分钟内累计最多可以启动500个新的并发实例。如在1分钟内已经达到了当前限制，则将无法再启动新的并发实例，持续到下1分钟。在此期间有新的并发扩容请求，将会产生扩容受限错误（429 ResourceLimit），详情可参见 [云函数状态码](https://intl.cloud.tencent.com/document/product/583/35311)。
...
云函数平台提供函数粒度的并发管理能力，详情可参见 [并发管理](https://intl.cloud.tencent.com/document/product/583/39464)。
