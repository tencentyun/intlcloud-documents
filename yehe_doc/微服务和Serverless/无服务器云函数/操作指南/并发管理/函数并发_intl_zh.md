...
- **并发配额**：目前在账号、函数维度进行并发限制配置时的额度，配额按内存为单位进行设置或调整，同一时刻运行的函数实例不可超过并发配额。实际可启动的并发实例数，根据配额除以函数的配置内存得到。
...
目前云函数为用户提供两个层次的并发配额管理能力：

```
账号并发
    |- 函数保留并发
```
>? **版本预置并发**为预先启动实例的能力，不属于并发配额管理能力。详情可参见 [预置并发](https://intl.cloud.tencent.com/document/product/583/37704)。
...
同时账号的可用配额，即可由函数共享的并发额度，会随着保留并发的配置而减少，最低会减少到12800MB。即账号维度的可配置额度会留存12800MB额度不可分配到保留并发或预置并发中，用于未配置保留或预置的函数运行。
