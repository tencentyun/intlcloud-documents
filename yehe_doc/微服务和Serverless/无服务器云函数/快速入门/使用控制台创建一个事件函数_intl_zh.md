...
4. 函数名称默认填充，可根据需要自行修改。函数配置保持默认，并单击【完成】。
...
::: 触发器配置测试
在“触发管理”页中，查看触发器详情。
1. 触发器创建成功后，会在该函数的“触发管理”页面生成访问路径。
...
:::
::: 查看监控
在“函数管理”页中，选择已创建函数的【监控信息】，即可查看函数调用次数/运行时间等情况。
...
## 遇到问题？
非常抱歉您在使用时出现问题，您可以 [提交工单](https://console.cloud.tencent.com/workorder/category) 来寻求帮助。
