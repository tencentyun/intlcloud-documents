...
本文为您介绍如何使用 API 网关集成云函数]，将实时音视频 TRTC 房间的主播音视频进行混流录制，录制完毕后上传到COS存储，提供开箱即用、灵活便捷、可编程的直播录制能力。云函数默认提供 512MB 内存来存储录制文件，如果您需要更大的存储空间，可以选择使用 [CFS 挂载能力](https://intl.cloud.tencent.com/document/product/583/37497)。工作流程如下图所示： 
...
| 参数名称  | 类型   | 必选 | 描述                                                         |
| --------- | ------ | ---- | ------------------------------------------------------------ |
| SecretId  | String | 否   | 腾讯云账号的 SecretId。详情请参见 [访问管理](https://intl.cloud.tencent.com/document/product/598/34228)。 |
| SecretKey | String | 否   | 腾讯云账号的 SecretKey。详情请参见 [访问管理](https://intl.cloud.tencent.com/document/product/598/34228)。 |
...
- 使用 RoomId 的用户停止录制，需要调用 [移除用户](https://intl.cloud.tencent.com/zh/document/product/647/34268) 接口。
- 使用 StrRoomId 的用户停止录制时，需要调用 [移除用户（字符串房间号）](https://intl.cloud.tencent.com/document/product/647/39630)接口。
...
6. 如需使用 [CFS 挂载能力](https://intl.cloud.tencent.com/document/product/583/37497)，由于 CFS 只能私有网络访问，因此必须将云函数的 VPC 配置在与 CFS 在同一个私有网络下。如下图所示：
...
2. 填写 Demo 名称，单击**创建**完成应用创建。您可以根据自己的客户端选择模板试运行，例如 [跑通Demo(桌面浏览器)](https://intl.cloud.tencent.com/document/product/647/35607)。
...
1. 参考 [跑通Demo(桌面浏览器)](https://intl.cloud.tencent.com/document/product/647/35607)，用户user_00001与user_00002进入一个 TRTC 房间。
...
6. 切换至 [实时音视频控制台](https://console.cloud.tencent.com/trtc/monitor)，在“监控仪表盘”页面单击房间 ID，查看所有在房间中的用户，其中一个观众即为录制观众。
7. 如需在录制过程中停止录制，可以调用 [移除用户](https://intl.cloud.tencent.com/zh/document/product/647/34268) 或者 [移除用户（字符串房间号）](https://intl.cloud.tencent.com/document/product/647/39630) 将用户提出房间。
