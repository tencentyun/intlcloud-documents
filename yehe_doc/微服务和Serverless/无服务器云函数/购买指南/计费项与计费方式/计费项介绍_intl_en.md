...
- **Basic package fee:** A basic package tier is automatically granted, and the fee is automatically deducted on a monthly basis (0.06 USD/day). This fee will be charged in the following cases: 1) The user is a three-month new user. 2) The user has valid subscriptions or resource packages. 3) No resource usage, function invocation and outbound traffic incurred i the last month. 
...
For the unit price of each billable item, see [Product Pricing](https://intl.cloud.tencent.com/document/product/583/12281). Here, the public network outbound traffic is in GB.
