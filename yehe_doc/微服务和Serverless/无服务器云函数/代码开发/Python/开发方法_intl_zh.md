...
event 的具体内容，根据不同触发器或调用来源而变化。详细数据结构说明请参见 [触发器相关说明](https://intl.cloud.tencent.com/document/product/583/9705)。
...
```
其中 `errorCode` 字段标识错误为代码错误，`errorMessage` 字段标识错误具体说明，`stackTrace` 字段输出错误堆栈信息，`statusCode` 字段标识具体错误。`statusCode` 具体说明请参见 [云函数状态码](https://intl.cloud.tencent.com/document/product/583/35311)。
