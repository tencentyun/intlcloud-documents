...
3. 在**新建事件规则** > **事件模式**页，配置告警规则。 
	本文以**流计算 Oceanus TaskManager CPU 负载过高**事件告警配置为例，如下图所示。您可以选择指定的事件告警类型，也可以选择全部告警事件。详细事件匹配规则请参见 [管理事件规则](https://intl.cloud.tencent.com/document/product/1108/42288)。
