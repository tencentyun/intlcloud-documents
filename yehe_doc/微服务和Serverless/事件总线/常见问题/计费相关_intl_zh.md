...
### 2. 自定义事件集如何计费？
若您自行创建**自定义事件集**，将按您投递到自定义事件集的事件数量计费。具体收费标准请参见 [产品定价](https://intl.cloud.tencent.com/document/product/1108/43288)。
...
### 4. 冻结费用是什么逻辑？
事件总线按照小时推量到计费平台，按天进行实际扣费，在新创建 [自定义事件集](https://intl.cloud.tencent.com/document/product/1108/42285) 时，会先提前冻结0.64美元，删除事件集后会将金额解冻退还；如果账号内余额不足0.64美元，会导致创建事件集失败。
