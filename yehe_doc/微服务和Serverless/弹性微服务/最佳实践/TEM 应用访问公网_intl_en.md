...
Applications running on TEM usually need public network access, and also require allowlist access in the scenarios such as mini programs. In these cases, the application should have a fixed public IP.
...
#### Steps
<dx-steps>
- Deploy the applications in TEM.
- Create a NAT Gateway instance in the NAT Gateway console and bind an EIP.
- Create a route table pointing to the NAT Gateway and associate a subnet in the VPC console.
- Verify whether the TEM applications can access the public network.
- (Optional) Query the public network access IP addresses.
</dx-steps>
...
### Step 5: (optional) query the public network access IP addresses
