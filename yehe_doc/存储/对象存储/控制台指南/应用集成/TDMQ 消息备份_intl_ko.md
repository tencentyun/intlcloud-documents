...
TDMQ 메시지 백업은 Tencent Cloud COS가 [SCF](https://intl.cloud.tencent.com/document/product/583)를 기반으로 제공하는 기능이며, TDMQ 메시지를 COS에 저장합니다. TDMQ 메시지를 저장하면 데이터 분석 및 다운로드 등의 작업을 편리하게 진행할 수 있습니다.

TDMQ는 Apache 오픈 소스 프로젝트 [Pulsar ](https://pulsar.apache.org/) 기반의 자체개발 파이낸스급 분산형 메시지 미들웨어로, 높은 일치성, 신뢰성 및 동시 접속량이 특징입니다.
...
2. 왼쪽 메뉴에서 [애플리케이션 통합]을 클릭하고 [TDMQ 메시지 백업]을 찾습니다.
...
5. 팝업 창에서 다음과 같이 정보를 설정합니다.
 - **함수 이름**: 함수의 고유 식별자로, 생성 후에는 변경할 수 없습니다. [SCF 콘솔](https://console.cloud.tencent.com/scf/list?rid=1&ns=default)에서 해당 함수를 조회할 수 있습니다.
 - **연결 버킷**: TDMQ 메시지를 저장할 COS 버킷입니다.
 - **Interval**: 메시지 양에 따라 메시지를 취합할 시간 간격을 선택하며, 시간 간격은 5~15분으로 설정할 수 있습니다. 단일 메시지 파일은 최대 5,000개로 제한되며, 메시지 파일은 최대 500MB로 제한됩니다.
...
6. [다음 단계]를 클릭하여 TDMQ 설정을 진행합니다. 설정 항목에 대한 설명은 다음과 같습니다.
 ...
 - **주제 선택**: 메시지 소스의 주제를 선택합니다.
 - **구독 선택**: 해당하는 주제 구독을 선택합니다. 기존 구독이 만족스럽지 않다면 TDMQ 콘솔의 [소비자 관리](https://console.cloud.tencent.com/tdmq/topic-subscription?rid=1&clusterId=pulsar-5rqmwvrxv8&env=default&topic=zetest)로 이동하여 구독을 생성할 수 있습니다.
 - **시작 위치**: 이전 메시지의 시작 위치
 ...
 - **액세스 주소**: 반드시 VPC 내부 네트워크 액세스 주소여야 하며, TDMQ 클러스터는 VPC에 액세스해야 합니다.
...
생성한 함수에 다음과 같은 작업을 진행할 수 있습니다.
 - [로그 조회]를 클릭해 TDMQ 메시지 백업의 과거 실행 기록을 조회합니다. 백업 시 오류가 보고된 경우, [로그 조회]를 클릭하고 빠르게 SCF 콘솔에 리디렉션하여 로그 오류 상세 내용을 조회할 수 있습니다.
 - [편집]을 클릭하여 TDMQ 메시지 백업 규칙을 변경합니다.
 - [삭제]를 클릭하여 사용하지 않는 TDMQ 메시지 백업 규칙을 삭제합니다.
