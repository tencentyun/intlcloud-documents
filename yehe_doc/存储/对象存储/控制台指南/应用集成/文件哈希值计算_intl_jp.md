...
ファイルハッシュ値の計算は、Tencent CloudのCloud Object Storage（COS）が[Serverless Cloud Function（SCF）](https://intl.cloud.tencent.com/document/product/583)をベースとしてユーザー向けに提供する整合性チェックソリューションです。ファイルがCOSにアップロードされると、ハッシュ値が自動的に計算され、ファイルのカスタムヘッダーに追加されるため、ファイルの整合性を確認しやすくなります。
...
- ファイル名のハッシュ値の計算は、広州、上海、北京、成都、中国香港、シンガポール、ムンバイ、トロント、シリコンバレーなど、SCFがリリースされているすべてのリージョンでサポートされています。その他のサポートリージョンについては、[SCF製品ドキュメント](https://intl.cloud.tencent.com/document/product/583)をご参照ください。
- ハッシュ値の計算中にエラーが報告された場合、作成した関数の右側にある【ログの確認】をクリックすると、SCFコンソールにジャンプし、ログエラーの詳細を確認することができます。
...
2. 左側ナビゲーションで、【アプリケーション統合】をクリックし、【データ処理】タブの下にある【ファイルハッシュ値の計算】を見つけます。
3. 【ファイルハッシュ値の計算】をクリックし、ルールの設定ページに進みます。
>! SCFサービスをアクティブ化していない場合は、[SCFコンソール](https://console.cloud.tencent.com/scf)に移動してSCFサービスをアクティブ化し、プロンプトに従ってサービス権限承認を行えば完了です。
>
4. 【ハッシュルールの設定】をクリックし、ポップアップウィンドウに以下の情報を設定します。
 ...
 - **イベントタイプ**：イベントとは、SCFをトリガーする操作のことです。アップロード操作を例に取ると、アップロード方法は、`PUT Object`インターフェースか`POST Object`インターフェースを呼び出して行います。選択したイベントが【Putメソッドによって作成】された場合、`PUT Object`インターフェースを介してアップロードされたファイルのみがハッシュ値の計算をトリガーします。
...
新規作成した関数に対しては、次のような操作も可能です。
 - 【ログの確認】をクリックして、ファイルハッシュ値の計算関数の履歴を確認します。関数でエラーが発生した場合は、【ログの確認】をクリックしてSCFコンソールにすばやくジャンプし、ログエラーの詳細を確認することもできます。
 - 【編集】をクリックして、ファイルハッシュ値の計算ルールを変更します。
 - 【削除】をクリックして、不要になったファイルハッシュ値の計算関数を削除します。
