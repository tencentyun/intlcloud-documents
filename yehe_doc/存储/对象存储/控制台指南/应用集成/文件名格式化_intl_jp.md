...
ファイル名のフォーマットは、Tencent CloudのCloud Object Storage(COS)が[Serverless Cloud Function（SCF）](https://intl.cloud.tencent.com/document/product/583)をベースとしてユーザー向けに提供するファイルリネームソリューションです。ファイルがCOSにアップロードされると、命名ルールに従って自動的にファイル名をリネームすることができます。既存のファイル名を置き換えるための変数プレースホルダーの使用もサポートしています。
...
- ファイル名のフォーマットは、広州、上海、北京、成都、中国香港、シンガポール、ムンバイ、トロント、ソウル、シリコンバレーなど、SCFがリリースされているすべてのリージョンでサポートされています。その他のサポートリージョンについては、[SCF製品ドキュメント](https://intl.cloud.tencent.com/document/product/583)をご参照ください。
...
2. 左側ナビゲーションバーで、**アプリケーションの統合**をクリックします。
3. **データ処理**タブの**基本処理**の下から、**ファイル名のフォーマット**を見つけます。
4. **フォーマットルールの設定**をクリックし、ルール設定ページに進みます。
...
7. 設定を追加し、**確定**をクリックしてしばらくすると、関数が追加されたことを確認できます。

新規作成した関数に対しては、次のような操作が可能です。
 - **ログ**をクリックして、SCFコンソールにすばやくジャンプし、ログエラーの詳細を確認します。
 - **詳細**をクリックして、設定したフォーマット済みの名前関数ルールの詳細を確認します。
 - **編集**をクリックして、ファイル名のフォーマットルールを変更します。
 - **削除**をクリックして、不要になったファイル名フォーマット関数を削除します。
