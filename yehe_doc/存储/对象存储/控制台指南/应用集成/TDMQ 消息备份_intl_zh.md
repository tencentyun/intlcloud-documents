...
TDMQ 消息备份是腾讯云对象存储（Cloud Object Storage，COS）基于 [云函数（Serverless Cloud Function，SCF）](https://intl.cloud.tencent.com/document/product/583) 为用户提供的 TDMQ 消息转存至 COS 的功能，可以协助用户将 TDMQ 消息进行转存以便于对数据进行分析与下载等操作。

TDMQ 是基于 Apache 开源项目 [Pulsar ](https://pulsar.apache.org/) 自研的金融级分布式消息中间件，具备跨城高一致、高可靠、高并发的特性。
...
6. 单击【下一步】，进行 TDMQ 配置，配置项说明如下：
 ...
 - **访问地址**：必须为 VPC 内网访问地址，TDMQ 集群需接入 VPC。
