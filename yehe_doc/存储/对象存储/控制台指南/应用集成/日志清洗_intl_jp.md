...
ログクリーンアップは、Tencent CloudのCloud Object Storage（COS）が[Serverless Cloud Function（SCF）](https://intl.cloud.tencent.com/document/product/583)をベースにしてユーザー向けに提供するログファイル処理ソリューションです。ユーザーが[ログ管理サービス](https://intl.cloud.tencent.com/document/product/436/16920)を有効にするか、ログファイルを自分でアップロードすると、COSによりあらかじめ設定されたSCFが自動的にトリガーされます。関数であらかじめ指定したSQL検索ステートメントによって、ファイル内のログ情報はフィルタリングしてクリーンアップされ、その結果は指定したバケットとそのパスに配布されます。
...
2. 左側ナビゲーションで、**アプリケーション統合**をクリックし、**データ処理**タブの下にある**ログクリーンアップ**を見つけます。
...
5. ポップアップしたウィンドウで、以下の情報を設定します。
	...
	- **イベントタイプ**：イベントとは、SCFをトリガーする操作のことです。アップロード操作を例に取ると、アップロード方法は、`PUT Object`インターフェースか`POST Object`インターフェースを呼び出して行われます。選択したイベントが**Putメソッドによって作成**された場合、`PUT Object`インターフェースを介してアップロードされたログファイルのみがログクリーンアップをトリガーします。
...
6. **次のステップ**をクリックし、ログクリーンアップの設定を行います。設定項目の説明は次のとおりです。
	...
	- **SQL式**：検索したいステートメントを入力します。当社が提供するSQLテンプレートを選択することもできます。SQL関連のコマンドの説明については、[SQL関数](https://intl.cloud.tencent.com/document/product/436/32474)をご参照ください。
...
新規作成した関数に対しては、次のような操作が可能です。
 - **ログの確認**をクリックして、ログクリーンアップの履歴を確認します。ログクリーンアップでエラーが発生した場合は、**ログの確認**をクリックしてSCFコンソールにすばやくジャンプし、ログエラーの詳細を確認することもできます。
 - **編集**をクリックして、ログクリーンアップルールを変更します。
 - **削除**をクリックし、使用しないログクリーンアップルールを削除します。
