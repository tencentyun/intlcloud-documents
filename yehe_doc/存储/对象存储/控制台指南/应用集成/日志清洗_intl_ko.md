...
- 로그 클리닝 기능은 COS 데이터 인덱스 인터페이스를 사용합니다. 제한 사항에 대한 자세한 내용은 [Select 개요](https://intl.cloud.tencent.com/document/product/436/32472)를 참조하십시오.
...
- COS의 로그 클리닝 기능은 SCF 서비스에 종속됩니다. SCF 서비스는 사용자에게 [무료 한도](https://intl.cloud.tencent.com/document/product/583/12282)를 제공하며 무료 한도를 초과한 부분에 대해서는 [SCF 서비스 가격](https://intl.cloud.tencent.com/document/product/583/12281)에 따라 청구됩니다.

## 작업 순서

1. [COS 콘솔](https://console.cloud.tencent.com/cos5)에 로그인합니다.
2. 왼쪽 메뉴에서 [애플리케이션 통합]을 클릭한 후 [데이터 처리] 탭에서 [로그 클리닝]을 찾습니다.
...
5. 팝업 창에서 다음과 같이 정보를 설정합니다.
   <img src="https://main.qcloudimg.com/raw/d2818dba1adf7b4aceffdfb0a85e916b.png" width="550px"  />
	...
	- **이벤트 유형**: 이벤트란 SCF를 트리거하는 작업입니다. 업로드 작업을 예로 들면, 업로드 방식으로 `PUT Object` 인터페이스를 호출하거나 `POST Object` 인터페이스를 호출할 수 있으며, 이벤트를 [Put 방법 생성]으로 선택하면 `PUT Object` 인터페이스를 통해 업로드하는 로그 파일만 로그 클리닝을 트리거합니다.
>!파일을 간편 업로드, 멀티파트 업로드, 리전 간 복제 등 다양한 방식을 통해 버킷에 업로드하는 경우 [모두 생성] 이벤트를 선택하십시오.
	- **트리거 조건**: 로그 파일을 어떤 경로에 업로드할 때 SCF가 트리거되는 조건을 의미합니다. 특정 접두사를 선택하는 경우 로그 파일을 특정 접두사의 경로에 업로드할 때 SCF가 트리거되며, 모든 버킷을 선택하는 경우 버킷의 임의 위치에 로그 파일을 업로드할 때 항상 트리거됩니다.
...
6. [다음 단계]를 클릭하여 로그 클리닝을 설정합니다. 설정 항목에 대한 설명은 다음과 같습니다.
   ![](https://main.qcloudimg.com/raw/acc8cad18eafe1f619c97223e58727a3.png)
	- **원본 파일 설정**: 실제 상황에 따라 인덱스가 필요한 파일 유형, 행/열 세퍼레이터, 리스트 헤더, 압축 포맷을 선택합니다.
		- **SQL 표현식**: 실행할 인덱스 명령을 작성합니다. Tencent Cloud에서 제공하는 SQL 템플릿을 사용할 수도 있으며, SQL 관련 명령어에 대한 설명은 [SQL 함수](https://intl.cloud.tencent.com/document/product/436/32474)를 참조하십시오.
...
8. 설정을 추가한 후 [확인]을 클릭하면 추가된 함수를 확인할 수 있습니다.
   ...
   - [로그 조회]를 클릭하여 로그 클리닝한 이전의 실행 상황을 확인할 수 있습니다. 로그 클리닝 시 오류가 발생한 경우 [로그 조회]를 클릭하여 빠르게 SCF 콘솔로 리디렉션해 로그 오류에 대한 상세 정보를 확인할 수 있습니다.
   - [편집]을 클릭하여 로그 클리닝 규칙을 수정할 수 있습니다.
   - [삭제]를 클릭하여 사용하지 않는 로그 클리닝 규칙을 삭제할 수 있습니다.
