...
> ?
> - 現在、INTELLIGENT_TIERINGストレージタイプは、北京、上海、広州、重慶リージョンでのみサポートされています。このタイプをアップロードしたい場合は、まず対応するリージョンのバケットに対して、[INTELLIGENT_TIERINGストレージ設定の有効化](https://intl.cloud.tencent.com/document/product/436/38306)を行ってください。
> - ディープアーカイブストレージタイプは、北京、上海、広州、成都リージョンでのみサポートされています。
...
1. [COSコンソール](https://console.cloud.tencent.com/cos5)にログインし、【バケットリスト】をクリックしてバケットリストに進みます。オブジェクトをストレージしたいバケットを選択し、バケットのファイルリストページに進みます。ファイルリストで【ファイルのアップロード】をクリックします。
![](https://main.qcloudimg.com/raw/1f499717168f5559337899f635e19a13.png)

2. ファイルのアップロードインターフェースで【ファイルの選択】または【フォルダの選択】をクリックすると、単一または複数のローカルファイル（またはフォルダ）を選択してアップロードすることができます。選択を完了した後、そのまま【アップロード】をクリックするか、【次のステップ】をクリックすると、オブジェクトの属性を設定した後、アップロードすることができます（ステップ3参照）。
![](https://main.qcloudimg.com/raw/8489d2bae2ba778bac87386093cd51e7.png)

3.（オプション）アップロードするファイルのストレージタイプ、アクセス権限、サーバー側の暗号化、オブジェクトタグおよびメタデータ（入力しなくても可）を設定し、【アップロード】をクリックします。 設定の説明は次のとおりです。
- **ストレージタイプ**：異なるユースケースに基づき、オブジェクトごとに異なるストレージタイプを設定することができます。デフォルトのストレージタイプは標準ストレージです。ストレージタイプの説明については、[ストレージタイプ](https://intl.cloud.tencent.com/document/product/436/6222?lang=en&pg=#storage-classes)をご参照ください。
...
> ?複数ファイルのアップロードをサポートしていないブラウザもありますので、IE10以上、Firefox、Chromeなどの主流ブラウザを使用することをお勧めします。

4. 【アップロード】をクリックすると、ページ右上の「タスク完了」で現在のアップロードの進捗状況を確認することができます。アップロードが完了すると、「ファイルリスト」ページで先ほどアップロードしたオブジェクトを確認できます。
