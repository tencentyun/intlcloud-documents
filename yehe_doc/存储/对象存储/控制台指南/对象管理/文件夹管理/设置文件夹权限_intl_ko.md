...
COS 콘솔을 통해 폴더에 액세스 권한을 설정하고 폴더 내용에 대해 지정된 작업을 수행할 사용자를 지정할 수 있습니다. 권한 설정 시 [최소 권한 원칙](https://intl.cloud.tencent.com/document/product/436/32972)에 따라 데이터 자산 보안을 강화할 것을 권장합니다.
...
폴더 권한은 본질적으로 객체 차원 기반의 액세스 권한 설정이며, 이 권한 우선순위는 버킷의 액세스 권한보다 상위에 있습니다. 객체 스토리지 COS는 객체 설정에 두 가지 권한 유형을 지원합니다.
...
- 사용자 권한: 루트 계정은 기본적으로 객체에 대한 모든 권한(즉, 전체 제어)을 갖습니다. 또한 COS는 서브 계정 추가를 지원해 데이터 읽기, 데이터 쓰기, 읽기 권한, 쓰기 권한, 심지어 전체 제어가 가능한 최고 권한을 갖습니다.

## 작업 순서

1. [COS 콘솔](https://console.cloud.tencent.com/cos5)에 로그인한 뒤, 왼쪽 메뉴바에서 [버킷 리스트]를 클릭하여 버킷 리스트 페이지로 이동합니다.
2. 폴더가 있는 버킷을 찾아서 버킷 이름을 클릭하여 버킷 관리 페이지로 이동합니다.
	 ![버킷 관리 페이지 화면 캡처](https://main.qcloudimg.com/raw/b3917dd3aff063f14016d9f1b280276b.png)
3. [파일 리스트] 탭 아래 권한 설정할 폴더를 찾은 뒤, 오른쪽 작업 열 아래 [권한 설정]을 클릭합니다.
	 ![파일 리스트 페이지 화면 캡처](https://main.qcloudimg.com/raw/2d5efff0532cec0e1fbec063f983e611.png)
4. 업무 필요에 따라 폴더 권한을 설정할 수 있습니다. 권한 설정 설명은 다음과 같습니다.
<table>
   <tr>
      <th>권한 유형</th>
      <th>설정 페이지</th>
...
5. 권한 설정을 완료한 후, 오른쪽에 [저장]을 클릭하면 권한 설정이 저장됩니다.
