...
> ?
> - 目前智能分层存储类型仅支持北京、上海、广州和重庆地域，如需上传该类型，请先为对应地域的存储桶 [开启智能分层存储配置](https://intl.cloud.tencent.com/document/product/436/38306)。
...
1. 登录 [对象存储控制台](https://console.cloud.tencent.com/cos5)，单击【存储桶列表】，进入存储桶列表。选择需要存储对象的存储桶，进入存储桶的文件列表页面。在文件列表中，单击【上传文件】。
![](https://main.qcloudimg.com/raw/1f499717168f5559337899f635e19a13.png)

2. 在上传文件界面中，单击【选择文件】或【选择文件夹】，可选择单个或多个本地文件（或文件夹）上传，选择完成后您可直接单击【上传】，或单击【下一步】设置对象属性后再上传（见第3步）。
![](https://main.qcloudimg.com/raw/8489d2bae2ba778bac87386093cd51e7.png)

3.（可选）设置待上传文件的存储类型、访问权限、服务端加密、对象标签和元数据（可不填），然后单击【上传】。配置说明如下：
- **存储类型**：根据不同的应用场景，您可以为不同的对象设置不同的存储类型，默认存储类型为标准存储。关于存储类型的介绍，请参见 [存储类型](https://intl.cloud.tencent.com/document/product/436/6222?lang=en&pg=#storage-classes)。
...
4. 单击【上传】后，您可以在页面右上方的“任务已完成”中查看当前的上传进度，上传完成后，您将可以在存储桶“文件列表”页查看到刚才已上传的对象。
