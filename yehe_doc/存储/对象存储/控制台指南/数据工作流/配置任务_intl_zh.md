...
![](https://qcloudimg.tencent-cloud.cn/raw/9b039ab7eb963c16e50d988e82203f48.png)
 ...
 - **队列回调 URL**：与队列绑定的回调地址信息，可前往公共配置的队列中进行配置。
...
5. 选择**媒体处理**页签，选择**转码处理 > 音视频转码**任务类型，单击**创建任务**，并按照如下信息进行配置：
 ...
 - **目标文件名**：您可在此处指定输出文件的文件名。
 - **队列**：目前只支持默认队列 queue-1，队列相关说明请查看 [队列与回调](https://intl.cloud.tencent.com/document/product/436/46412)。
 - **队列回调 URL**：与队列绑定的回调地址信息，可前往 公共配置的队列中进行配置。
...
![](https://qcloudimg.tencent-cloud.cn/raw/9b7322d5facc1b48cef33f9a1a77c729.png)
 ...
 - **选择模板**：您可选择指定模板。
 - **目标格式**：目前控制台开启视频截帧任务默认输出 JPG 格式截图，若在模板中开启了截帧压缩可输出 HEIF、TPG 格式。使用视频截帧 API 可选择输出 JPG 或 PNG 格式截图，详情请查看视频截帧 API 文档。
...
![](https://qcloudimg.tencent-cloud.cn/raw/d06366d7bea0a7b3d612af4f1bd65090.png)
 ...
 - **队列**：目前只支持默认队列 queue-1，队列相关说明请查看队列文档。
