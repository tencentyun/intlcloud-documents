...
>현재 지원하는 버킷 암호화 방식은 SSE-COS 암호화입니다(COS가 키를 호스팅하여 관리하는 서버 암호화 방식). 서버 암호화에 관한 내용은 [서버 암호화 개요](https://intl.cloud.tencent.com/document/product/436/18145?lang=en)를 참조하십시오.


## 작업 순서
#### 신규 버킷 생성 시 암호화 설정

[버킷 생성](https://intl.cloud.tencent.com/document/product/436/13309) 시, 다음 이미지와 같이 버킷 암호화를 추가할 수 있습니다.
...
2. 왼쪽의 [보안 관리] > [서버 암호화]를 클릭합니다. [서버 암호화] 설정 항목을 찾은 뒤 [편집]을 클릭하여 현재 상태를 '활성화'로 변경합니다.
![](https://main.qcloudimg.com/raw/e2863ba89860f15464870ac198b5335f.png)
3. 지정한 암호화 방식을 선택한 뒤 [저장]을 클릭하면 버킷 암호화 설정이 완료됩니다.
