...
您可以通过对象存储（Cloud Object Storage，COS）控制台，为存储桶添加策略，以允许或禁止某个账号、某个来源 IP（或 IP 段）访问策略所设定的 COS 资源。关于存储桶策略概述和策略示例的相关信息，请参见 [访问策略语言概述](https://intl.cloud.tencent.com/document/product/436/18023) 和 [存储桶策略示例](https://intl.cloud.tencent.com/document/product/436/18031)。下面将为您详细介绍如何添加存储桶策略。
...
3. 单击**权限管理**，找到 **Policy 权限设置**，COS 提供添加存储桶策略的方式为 **[图形设置](#1)** 和 **[策略语法](#2)**，操作步骤请参见如下。您可以选其中一种方式添加存储桶策略。关于配置项的更多说明，请参见 [访问策略语言概述](https://intl.cloud.tencent.com/document/product/436/18023)。
...
![自定义策略](https://qcloudimg.tencent-cloud.cn/raw/d7b01ebbfd480190b02e6af986ca4972.png)
