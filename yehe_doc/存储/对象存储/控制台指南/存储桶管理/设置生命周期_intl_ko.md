...
라이프사이클 관리 기능을 활용하여, 지정한 객체의 스토리지 유형을 정기적으로 전환하거나 삭제함으로써 비용 절감의 효과를 볼 수 있습니다. COS는 설정한 규칙에 따라 지정한 객체의 스토리지 유형을 설정 시간에 맞춰 자동으로 전환 및 삭제하는 기능을 제공합니다. 이에 관한 자세한 내용은 [라이프사이클 개요](https://intl.cloud.tencent.com/document/product/436/17028) 문서를 참조하십시오.
...
4. 왼쪽의 [기본 설정] > [라이프사이클] 설정 항목을 클릭한 뒤 [규칙 추가]를 클릭합니다.
...
- **현재 버전 파일 관리**: 현재 버전의 객체 관리 옵션을 활성화하여 현재 버전의 객체를 전환하거나 삭제할 수 있습니다. 버킷 내 객체를 표준 스토리지에서 표준IA 스토리지, CAS, DEEP ARCHIVE로 전환할 수 있으며, 객체 만료 후에는 삭제도 가능합니다.
   스토리지 유형을 핫에서 콜드로 나열하면 **표준 스토리지** > **표준IA 스토리지** > **INTELLIGENT TIERING** > **CAS** > **DEEP ARCHIVE**와 같습니다. 핫에서 콜드 방향으로만 스토리지 유형을 전환할 수 있으며, 반대 방향으로는 전환이 불가능합니다. 시간은 COS에서 파일이 수정된 시점부터 계산되며, 객체를 수정하는 작업은 객체를 새로 업로드하는 것으로 간주합니다.
- **이전 버전 파일 관리**: 이전 버전의 객체 관리 옵션을 활성화하여 이전 버전의 객체를 전환하거나 삭제할 수 있습니다. 해당 옵션을 활성화하지 않은 경우, 최신 버전 객체만 처리하도록 기본값이 설정됩니다.
- **이전 버전이 없는 삭제 마커 정리**: 객체의 최신 버전에 삭제 마커(Delete Marker)가 되어 있고, 해당 객체의 이전 버전이 모두 삭제된 상태에서 해당 옵션을 활성화하면 삭제 마커도 삭제됩니다. 해당 옵션은 **현재 버전 파일 관리**의 만료 삭제 기능과 동시에 활성화할 수 없습니다.
