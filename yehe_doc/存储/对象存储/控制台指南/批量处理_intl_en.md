...
<table>
   ...
   <tr>
      <td>VersionId</td>
      <td>Object version ID. If versioning is enabled for a bucket, COS will assign a version number to each object added to the bucket. If you do not want to inventory the latest version of an object, you can specify a particular version ID</td>
   </tr>
</table>
...
3. If an inventory Etag automatically appears, you have selected the correct inventory file. Then, click **Next** into the “Operation Configuration” page, select one of the following two job types, and configure accordingly:
  - **Batch Data Copy**:
    - Destination Bucket: select the bucket that is to store the object copies.
  ...
  - **Restored archived objects in batches**：
    - Restoration Mode: you can opt for one of the two modes, standard or bulk. For more information on restoration modes, see [Restoring an Archived Object](https://intl.cloud.tencent.com/document/product/436/30961).
...
4. Click **Next** to enter the "Other Configurations" page where you should configure the following:
...
6. Once completed, find the newly created job in the job list, and click **Pending** > **Confirm** under **Status**. To cancel the job, click **Cancel Job** under **Operation**.
