...
<table>
   ...
   <tr>
      ...
      <td><ul  style="margin: 0;"><li>종량제: 공중망 다운스트림 트래픽(GB) x GB당 단가</li></ul></td>
   </tr>
   <tr>
      <td>공중망 업스트림 트래픽</td>
   ...
   <tr>
      ...
      <td><ul  style="margin: 0;"><li>종량제: CDN Origin-pull 트래픽(GB) x GB당 단가</li></ul></td>
   </tr>
   <tr>
      <td>리전 간 복제 트래픽</td>
      ...
      <td><ul  style="margin: 0;"><li>종량제: 리전 간 복제 트래픽(GB) x GB당 단가</li></ul></td>
   </tr>
   <tr>
      <td>글로벌 가속 트래픽</td>
      ...
      <td><ul  style="margin: 0;"><li>종량제: 글로벌 가속 트래픽(GB) x GB당 단가</li></ul></td>
   </tr> 
</table>
...
|-----|--------|------|
|       종량제         |   <ul  style="margin: 0;"><li>공중망 다운스트림 트래픽 요금</li><li>CDN Origin-pull 트래픽 요금</li><li>리전 간 복제 트래픽 요금 </li><li> 글로벌 가속 트래픽 요금</li></ul>        |<ul  style="margin: 0;"><li>일 결산</li><li>트래픽 요금 = GB당 단가 x 일일 누적 트래픽</li></ul>|
...
### 예시: 스탠다드 스토리지 용량 요금 + 스탠다드 스토리지 요청 요금 + 공중망 다운스트림 트래픽 요금

2020년 11월 1일에 사용자 A가 10GB 객체를 스탠다드 스토리지 클래스의 광저우 리전에 있는 버킷에 업로드하여 100개의 요청을 생성했으며, 11월 2일과 11월 3일에 사용자 A는 CDN이 비활성화된 공중망을 통해 10GB의 객체를 로컬 파일 시스템에 다운로드하여 매일 100회의 요청을 생성하였고, 이러한 작업 외에 사용자 A는 다른 작업을 수행하지 않았다고 가정할 때, 다음과 같이 스토리지 사용량, 요청 및 트래픽 요금이 매일 정산됩니다.

- 스탠다드 스토리지 사용 요금: 2020년 11월 2일부터 매일 정산됩니다.
- 스탠다드 스토리지 요청 요금: 2020년 11월 2일, 3일, 4일에 일별 결산됩니다.
- 트래픽 요금: 2020년 11월 3일과 4일에 일별 결산됩니다.


요금 분석:

- 스탠다드 스토리지 사용 요금 = 0.024 USD/GB/월 / 30 x 10GB x 30일 = 0.24 USD.
- 스탠다드 스토리지 요청 요금 = 0.002 USD/만 회 x 100회 / 10000 x 3 = 0.00006 USD.
- 공중망 다운스트림 트래픽 요금 = 0.1USD/GB x 10GB x 2 = 2USD


요약하면, 11월 사용자 A의 총 청구 금액은 0.24 + 0.00006 + 2 = 2.24006 USD입니다.
