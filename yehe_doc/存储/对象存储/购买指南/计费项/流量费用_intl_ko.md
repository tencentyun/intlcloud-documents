...
2020년 11월 1일에 사용자 A가 10GB 객체를 STANDARD 스토리지 클래스의 광저우 리전에 있는 버킷에 업로드하여 100개의 요청을 생성했으며, 11월 2일과 11월 3일에 사용자 A는 CDN이 비활성화된 공중망을 통해 10GB의 객체를 로컬 파일 시스템에 다운로드하여 매일 100회의 요청을 생성하였고, 이러한 작업 외에 사용자 A는 다른 작업을 수행하지 않았다고 가정할 때, 다음과 같이 스토리지 사용량, 요청 및 트래픽 요금이 매일 정산됩니다.
