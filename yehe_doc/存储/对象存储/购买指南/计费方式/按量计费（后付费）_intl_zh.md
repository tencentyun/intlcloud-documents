按量计费（后付费）方式是对象存储（Cloud Object Storage，COS）默认计费方式，支持 [所有地域](https://intl.cloud.tencent.com/document/product/436/6224)。根据用户的存储容量、请求、流量等计量项的具体用量进行计费，对用户账户按日或者按月进行扣费结算。


## 产品定价

关于 COS 的按量计费定价，请参见 [产品定价](https://intl.cloud.tencent.com/pricing/cos)。
