...
其中：
- 用户客户端：即用户手机 App。
- COS：[腾讯云对象存储](https://intl.cloud.tencent.com/product/cos)，负责存储 App 上传的数据。
- CAM：[腾讯云访问管理](https://intl.cloud.tencent.com/product/cam)，用于生成 COS 的临时密钥。
...
按照最小权限原则，建议您按照自己的需求，通过 Policy 控制临时密钥的权限范围。服务器下发一个完全读写权限的密钥，一旦被攻击，可能导致其他用户的数据泄露。具体的配置方法请参见 [临时密钥生成及使用指引](https://intl.cloud.tencent.com/document/product/436/14048)。
...
更多关于 iOS 如何向 COS 上传和下载文件，请参见 iOS SDK [快速入门](https://intl.cloud.tencent.com/document/product/436/11280)。
...
#### iOS
iOS 的完整示例工程请参见 [COS iOS Demo](https://github.com/tencentyun/qcloud-sdk-ios-samples/tree/master/OOTB-XML)。
