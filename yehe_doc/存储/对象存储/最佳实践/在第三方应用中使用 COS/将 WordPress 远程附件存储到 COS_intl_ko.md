...
- Tencent Cloud의 [CDN](https://intl.cloud.tencent.com/product/cdn)과 결합하여 이미지 첨부 파일 조회 속도를 향상함으로써 웹 사이트 액세스 속도가 최적화됩니다.
...
| 섬네일 업로드하지 않음        | 선택 시 상응하는 섬네일 파일을 업로드하지 않습니다. 선택하지 않는 것을 권장합니다.                   |
| 로컬 백업하지 않음    | 선택 시 로컬에 원본 파일을 보관하지 않습니다. 선택하지 않는 것을 권장합니다.                       |
