...
#### Sample: Restricting the VPC ID to aqp5jrc1

The policy in this sample allows the sub-account with ID 100000000002 that belongs to the root account with ID 100000000001 (APPID: 1250000000) to access the bucket `examplebucket-1250000000`, on condition that the VPC ID is `aqp5jrc1`.
...
#### Sample 2: Allowing users to get objects of the latest version
...
#### Sample 3: Disallowing users from deleting objects uploaded before versioning is enabled
...
#### Sample 1: Restricting the maximum value of the request header Content-Length
...
#### Sample 2: Restricting the minimum value of the request header Content-Length
...
#### Sample 1: Restricting Content-Type of PutObject requests to "image/jpeg"
...
The bucket policy in this sample is to restrict that object upload requests (`PutObject`) must carry the `Content-Type` header and with the value `image/jpeg`.
...
#### Sample 1: Restricting the GetObject request parameter response-content-type to be "image/jpeg"
...
#### Sample 1: Restricting download requests to use HTTPS
...
#### Sample 2: Denying any non-HTTPS request
...
### Allowing setting specified storage class (cos:x-cos-storage-class)
...
### Allowing setting specified bucket/object ACL (cos:x-cos-acl)
...
#### Sample 1: The object ACL must be set to private in a PutObject request
...
<span id="prefix"></span>
### Allowing listing objects in specified directory only (cos:prefix)
...
#### Sample 1: Allowing listing only objects in a specified directory of the bucket
...
### Requiring x-cos-forbid-overwrite header in object upload requests (cos:x-cos-forbid-overwrite)
...
#### Sample 1: Restricting the value of `x-cos-forbid-overwrite` to true in object upload requests (PutObject)

Assume that the root account with UIN 100000000001 that owns the bucket `examplebucket-1250000000` uses the following bucket policy to restrict the sub-account with UIN 100000000002 from overwriting objects with the same names when uploading objects. The policy requires that all `PutObject` requests carry the `x-cos-forbid-overwrite` header with the value `"true"`.
