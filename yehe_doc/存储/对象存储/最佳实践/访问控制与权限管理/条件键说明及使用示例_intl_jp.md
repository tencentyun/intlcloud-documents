...
### オブジェクトをアップロードする際に必ずx-cos-forbid-overwriteヘッダーを含める（cos:x-cos-forbid-overwrite）

#### リクエストヘッダーx-cos-forbid-overwrite

同名ファイルへの上書きを禁止します。同名ファイルをアップロードすると既存のファイルに上書きする場合がありますが、リクエストヘッダーx-cos-forbid-overwriteを使用すると、オブジェクトのアップロード時に同名ファイルへの上書きを禁止することができます。

#### 条件キーcos:x-cos-forbid-overwrite

条件キー`cos:x-cos-forbid-overwrite`を使用すると、アップロードリクエストに必ずリクエストヘッダー`x-cos-forbid-overwrite`を使用するよう制限することができます。それにより、ユーザーが既存のオブジェクトを上書きする可能性があるリクエストを送信することを禁止できます。

条件オペレーター：string_equal、string_not_equal。

#### 事例1：PutObjectによるオブジェクトのアップロード時に必ずx-cos-forbid-overwriteをtrueに設定するよう制限する

ルートアカウント（uin:100000000001）がバケットexamplebucket-1250000000を所有し、サブユーザー（uin:100000000002）がオブジェクトをアップロードする際に既存の同名オブジェクトを上書きできないよう制限する必要があるとします。以下のポリシーによって、サブユーザーのPutObjectリクエストの送信時に必ず`x-cos-forbid-overwrite`ヘッダーを含め、かつその値を必ず文字列の`"true"`とするよう制限します。
