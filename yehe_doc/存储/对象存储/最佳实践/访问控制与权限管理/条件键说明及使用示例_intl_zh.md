在使用访问策略授予权限时，您可以指定策略的 [生效条件](https://intl.cloud.tencent.com/document/product/436/18023)，例如限制用户访问来源、上传文件的存储类型等。
...
#### 请求头部 Content-Length
RFC 2616中定义的 HTTP 请求内容长度（字节），在 PUT 和 POST 请求中经常使用。详情见[请求头部列表](https://intl.cloud.tencent.com/document/product/436/7728)
...
RFC 2616中定义的 HTTP 请求内容类型（MIME），例如`application/xml`或`image/jpeg`，详情请参见 [请求头部列表](https://intl.cloud.tencent.com/document/product/436/7728)。
...
#### 请求参数 reponse-content-type

GetObject 接口支持加入请求参数`reponse-content-type`，用于设置响应中 Content-Type 头部的值。

#### 条件键 cos:response-content-type

使用条件键`cos:response-content-type`，您可以对请求的是否必须携带请求参数`reponse-content-type`参数值做限制。
...
使用请求头部`x-cos-acl`，您可以在上传对象、创建存储桶时指定访问控制列表（ACL），或修改对象、存储桶 ACL；关于 ACL 的相关介绍可参见 [ACL概述](https://intl.cloud.tencent.com/document/product/436/30583)。
...
#### 示例1：限制 PutObject 上传对象时必须设置 x-cos-forbid-overwirte 为 true

假设主账号（uin:100000000001）拥有存储桶 examplebucket-1250000000，需要限制子用户（uin:100000000002）上传对象时不可覆盖原有同名对象。以下策略限制子用户发起 PutObject 请求必须携带`x-cos-forbid-overwirte`头部且值必须为字符串`"true"`。
