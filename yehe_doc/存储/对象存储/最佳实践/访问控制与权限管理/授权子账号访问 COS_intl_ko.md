...
기업 간 또는 기업 내 여러 팀 간에는 Tencent Cloud COS 리소스에 대해 팀별 또는 직원별로 서로 다른 액세스 권한을 부여해야 합니다. 사용자는 Cloud Access Management(CAM)를 통해 버킷 또는 객체에 서로 다른 작업 권한을 설정함으로써 각 팀별 또는 직원별로 상호 협업할 수 있습니다.
...
3. [사용자 정의 생성]을 선택하고 [**액세스 가능한 리소스 및 수신 정보**] 유형을 선택한 후 [다음 단계]를 클릭합니다.
4. 필요에 따라 사용자 관련 정보를 입력합니다.
 - **사용자 정보 설정**: 서브 계정 이름(예: Sub_user) 및 서브 계정의 이메일을 입력합니다. 서브 계정에 이메일을 추가하여 Tencent Cloud에서 발송하는 WeChat 바인딩 이메일을 획득해야 합니다.
 - **액세스 방식**: 프로그램 액세스 및 Tencent Cloud 콘솔 액세스를 선택하고, 다른 설정은 필요에 따라 선택합니다.
...
3. 실제 필요에 따라 **빈 템플릿**을 선택하여 권한 부여 정책을 사용자 정의하거나 COS와 연결할 **시스템 템플릿**을 선택하고 [다음 단계]를 클릭합니다.
![](https://main.qcloudimg.com/raw/e1d43ba1ae7e2ae8bb22416c8c4b8127.png)
4. 기억하기 쉬운 정책 이름을 입력합니다. **빈 템플릿**을 선택한 경우 정책 구문을 입력해야 합니다. 자세한 내용은 [정책 예시](#정책 예시)를 참조하십시오. 정책 내용을 [정책 내용] 편집 창에 복사할 수 있으며, 입력한 정보가 정확한지 확인한 후 [완료]를 클릭합니다.
...
위 1단계에서 설정한 프로그램 액세스 및 Tencent Cloud 콘솔 액세스 방법을 다음과 같이 소개합니다.
...
서브 계정에 권한이 부여되면 [서브 계정 로그인 인터페이스](https://console.cloud.tencent.com/developer)에서 루트 계정의 ID, 서브 계정의 이름 및 비밀번호를 입력하여 콘솔에 로그인합니다. 로그인 후 [클라우드 서비스]에서 [COS]를 클릭하여 루트 계정의 스토리지 리소스에 액세스할 수 있습니다.

<span id="정책 예시"></span>
## 정책 예시
다음은 몇 가지 전형적인 시나리오의 정책 예시입니다. 사용자 정의 정책 설정 시 아래 참고 정책을 [정책 내용 편집] 입력창에 복사한 후 실제 상황에 따라 수정하여 사용할 수 있습니다. 더 많은 COS 주요 시나리오에서의 정책 구문은 [액세스 정책 언어 개요](https://intl.cloud.tencent.com/document/product/436/18023) 또는 [CAM 제품 문서](https://intl.cloud.tencent.com/document/product/598/11083) 의 **비즈니스 사례** 부분을 참조하십시오.
...
### 예시4: 서브 계정에 특정 IP 대역의 읽기 및 쓰기 권한 설정
아래는 IP 대역이 `192.168.1.0/24` 및 `192.168.2.0/24`인 주소에 대해서만 읽기 전용 권한을 부여하는 예시입니다.
효력 발생 조건 입력에 대한 더 자세한 정보는 [효력 발생 조건](https://intl.cloud.tencent.com/document/product/598/10608)을 참조하십시오.
```
{
    "version": "2.0",
    "statement": [
