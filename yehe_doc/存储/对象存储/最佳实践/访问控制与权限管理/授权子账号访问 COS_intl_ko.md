...
먼저 루트 계정, 서브 계정(사용자), 사용자 그룹 등 몇 가지 핵심 개념을 이해하고 있어야 합니다. CAM과 관련된 용어 및 설정에 대한 자세한 설명은 CAM [Glossary](https://intl.cloud.tencent.com/document/product/598/18564)를 참고하십시오.

#### 루트 계정
루트 계정은 개발자라고도 합니다. Tencent Cloud 계정에 가입하면 시스템에서 Tencent Cloud 서비스에 로그인할 수 있는 루트 계정 ID를 생성합니다. Tencent Cloud는 사용량을 기록하고 루트 계정을 기준으로 요금을 청구합니다.

기본적으로 루트 계정은 계정의 리소스에 대한 전체 액세스 권한을 가집니다. 루트 계정은 청구 정보에 액세스하고, 사용자 비밀번호를 변경하고, 사용자 및 사용자 그룹을 생성하고, 다른 Tencent Cloud 서비스 리소스에 액세스할 수 있습니다. 기본적으로 루트 계정만 이러한 리소스에 액세스할 수 있습니다. 다른 사용자는 루트 계정에 의해 인증된 후에만 액세스할 수 있습니다.

#### 서브 계정(사용자) 및 사용자 그룹
- 서브 계정은 루트 계정으로 생성된 객체입니다. Tencent Cloud 콘솔에 로그인할 수 있는 권한은 물론 ID 및 자격 증명이 있습니다.
- 기본적으로 서브 계정은 리소스를 소유하지 않습니다. 루트 계정으로 인증해야 합니다.
 - 하나의 루트 계정은 여러 개의 서브 계정(사용자)을 생성할 수 있습니다.
 - 하나의 서브 계정은 Tencent Cloud 리소스 관리를 지원하기 위해 여러 루트 계정에 속할 수 있습니다. 그러나 특정 시점에서 하나의 서브 계정은 Tencent Cloud 리소스를 관리하기 위해 하나의 루트 계정으로만 로그인할 수 있습니다.
- 서브 계정은 콘솔에서 개발자(루트 계정) 간에 전환할 수 있습니다.
 - 서브 계정이 콘솔에 로그인하면 기본 루트 계정에 속하며 루트 계정에서 부여한 액세스 권한을 가집니다.
 - 서브 계정이 하나의 루트 계정에서 다른 루트 계정으로 전환된 후에는 현재 루트 계정에서 부여한 액세스 권한만 갖게 되며 이전 계정에서 부여한 권한은 갖지 않습니다.
- 사용자 그룹은 동일한 기능을 가진 여러 사용자(서브 계정)의 모음입니다. 비즈니스 요구 사항에 따라 다양한 사용자 그룹을 생성하고 적절한 정책과 연결하여 다양한 권한을 부여할 수 있습니다.

## 작업 단계
서브 계정 생성, 서브 계정에 대한 권한 부여, 서브 계정으로 COS 리소스 액세스의 세 단계로 서브 계정에 COS 액세스 권한을 부여할 수 있습니다.

### 1단계: 서브 계정 생성
CAM 콘솔에서 서브 계정을 생성하고 액세스 권한을 부여할 수 있습니다. 자세한 작업 방법은 다음과 같습니다.
...
4. 필요에 따라 사용자 관련 정보를 입력합니다.
 - **사용자 정보**: 서브 계정의 사용자 이름(예: Sub_user) 및 이메일 주소를 설정합니다. Tencent Cloud에서 보낸 이메일을 수신하여 서브 계정과 WeChat 계정을 연결하려면 이메일 주소가 필요합니다.
 - **액세스 방식**: 프로그래밍 액세스 및 Tencent Cloud 콘솔 액세스를 선택하고, 기타 구성 항목은 필요에 따라 설정할 수 있습니다.
4. **다음**을 클릭하고 본인 인증을 시작합니다.
5. 서브 계정에 권한을 부여합니다. 제공된 정책 옵션으로 서브 계정에 COS 버킷 목록에 대한 액세스 권한 또는 읽기 전용 권한을 부여하는 것과 같은 간단한 정책을 구성할 수 있습니다. 보다 복잡한 정책을 설정하려면 [2단계: 서브 계정에 권한 부여](#.E6.AD.A5.E9.AA.A42.EF.BC.9A.E5.AF.B9.E5.AD.90.E8.B4.A6.E5.8F.B7.E6.8E.88.E4.BA.88.E6.9D.83.E9.99.90)로 진행하십시오.
6. 사용자 태그를 선택적으로 설정하고 **다음**을 클릭합니다.
...
서브 계정는 권한 부여 후 [서브 계정 로그인](https://cloud.tencent.com/login/subAccount?s_url=https%3A%2F%2Fconsole.cloud.tencent.com%2Fdeveloper) 페이지에서 루트 계정 아이디, 서브 계정 이름, 서브 계정 비밀번호를 입력하여 콘솔에 로그인할 수 있습니다. 그 다음 **제품**에서 **Cloud Object Storage**를 클릭하여 루트 계정에서 스토리지 리소스에 액세스할 수 있습니다.

<span id="정책 예시"></span>
## 정책 예시
다음과 같은 일반적인 샘플 정책이 여기에서 제공됩니다. 정책을 구성할 때 아래 코드를 참고하여 **정책 내용** 란에 복사 붙여넣기 하고 필요에 따라 수정하면 됩니다. 다른 일반적인 COS 시나리오에 대한 자세한 정책 구문은 [액세스 정책 언어 개요](https://intl.cloud.tencent.com/document/product/436/18023) 또는 [CAM 제품 문서](https://www.tencentcloud.com/document/product/598)의 **비즈니스 사용 사례** 부분을 참고하십시오.
