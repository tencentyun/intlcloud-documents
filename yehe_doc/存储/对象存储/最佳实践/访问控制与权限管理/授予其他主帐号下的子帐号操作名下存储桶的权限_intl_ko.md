...
5. 아래와 같이 양식을 작성합니다.
  ...
  - **필터**: 필터를 추가하거나 필요에 따라 비워둡니다.
6. **완료**를 클릭하여 루트 계정 B가 버킷에 대해 지정한 권한을 부여합니다.
7. 다른 버킷을 조작하기 위해 루트 계정 B에 권한을 부여해야 하는 경우 위 단계를 반복합니다.
...
>?루트 계정 B는 사전 설정된 정책이 아닌 사용자 정의 정책을 사용해서만 서브 계정 B0 권한을 부여할 수 있습니다.
3. 다음과 같이 작성합니다.
	- **정책 이름**: 중복되지 않고 의미가 있는 정책 이름을 사용자 정의합니다. 예: cos-child-account.
	- **비고**: 선택 가능하며, 필요에 따라 설명을 추가합니다.
	- **정책 내용**:
```shell
{
    "version": "2.0",
...
7. 그러면 인증이 완료되고 서브 계정 B0의 키를 사용하여 루트 계정 A의 버킷을 작업할 수 있습니다.
