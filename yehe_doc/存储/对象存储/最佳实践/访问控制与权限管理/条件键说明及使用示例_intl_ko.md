...
### 객체 업로드 요청에 x-cos-forbid-overwrite 헤더 필요(cos:x-cos-forbid-overwrite)

#### 요청 헤더 x-cos-forbid-overwrite

기존 파일과 이름이 중복된 파일을 업로드하면 기존 파일을 덮어씁니다. 요청 헤더 x-cos-forbid-overwrite를 사용하여 파일 업로드 중에 동일한 이름의 파일을 덮어쓰는 것을 금지할 수 있습니다.

#### 조건 키 cos:x-cos-forbid-overwrite

조건 키 `cos:x-cos-forbid-overwrite`를 사용하여 원래 객체를 덮어쓸 수 있는 업로드 요청을 방지하기 위해 객체 요청에 요청 헤더 `x-cos-forbid-overwrite`가 포함되도록 요구할 수 있습니다.

조건 연산자: string_equal, string_not_equal.
