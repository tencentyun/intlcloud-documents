>!
>当前 COS 已经通过底层索引打散机制实现高 QPS，如果有需要更高性能的 QPS 需求，可以 [联系我们](https://intl.cloud.tencent.com/contact-sales)。在日常组织文件过程中，我们仍然推荐您遵循本文档的要求，避免过于集中的索引存储方式。
...
腾讯云 COS 支持性能扩展，以支持更高的请求速率。如果您的请求是高 GET 请求负载，建议您搭配腾讯云 CDN 产品进行使用，详情请参见 [域名管理](https://intl.cloud.tencent.com/document/product/436/18424)。如果预计存储桶的综合请求速率会超过每秒30000个 PUT/LIST/DELETE 请求，建议您 [联系我们](https://intl.cloud.tencent.com/contact-sales)，以便于为工作负载做好准备，避免遇到请求的限制。
...
>!以下提供的所有方法，均是有可能提升单个存储桶访问性能的方法，如果您的业务典型负载超过每秒30000个请求，在执行以下方法的同时，您仍需 [联系我们](https://intl.cloud.tencent.com/contact-sales) 以便于为您的业务负载提前做好准备。
...
腾讯云 CDN 利用遍布全国和全球的边缘加速节点，向用户分发内容时可降低延时并提高速度，对于热点文件可以支持预拉热的方式进行缓存，从而减少回源到 COS 的 GET 请求数，详情请参见 [域名管理](https://intl.cloud.tencent.com/document/product/436/18424) 文档。
