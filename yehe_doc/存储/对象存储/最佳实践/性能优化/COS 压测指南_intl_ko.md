...
COSBench는 CentOS 7.0 이상 버전에서 실행됩니다.
...
3. s3-config-sample.xml 파일을 편집하여 작업 설정 정보를 추가합니다. 작업 설정 시 다음 다섯 단계를 주로 포함합니다.
 1.   init 단계: 버킷을 생성합니다.
 1.   prepare 단계: worker 스레드, main단계에서 읽어오기 위해 PUT 업로드 시 객체 크기를 지정합니다.
 1.   main 단계: worker 스레드, 읽기/쓰기 객체를 혼합하고, 실행 시간을 지정합니다.
 1.   cleanup 단계: 생성된 객체를 삭제합니다.
 1.   dispose 단계: 버킷을 삭제합니다.
...
6. 다음은 소속 리전이 베이징이며, 32코어, 내부 네트워크 대역폭 17Gbps인 CVM에 대한 업로드 및 다운로드 성능 테스트 예시로, 두 단계를 포함하고 있습니다.
