...
在运行该示例之前，请参考 [安装](https://intl.cloud.tencent.com/document/product/436/42230) 文档完成安装，注意执行 helm 命令加上参数 `--set webhook.enable=true` 开启 `webhook`，并检查 Fluid 各组件正常运行：
...
>? mountPoint 这里为了方便用户进行实验使用的是 Web UFS, 使用 COS 作为 UFS 可见 [使用 GooseFS 挂载 COS（COSN）](https://intl.cloud.tencent.com/document/product/436/41024)。
>
