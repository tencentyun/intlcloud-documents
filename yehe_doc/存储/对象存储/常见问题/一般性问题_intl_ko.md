...
Tencent Cloud COS에 대한 자세한 내용은 [Cloud Object Storage](https://www.tencentcloud.com/document/product/436)를 참고하십시오.


### COS 와 CFS의 차이점은 무엇인가요?

[Cloud Object Storage](https://www.tencentcloud.com/document/product/436)는 디렉터리 레이어 구조와 데이터 포맷 제한이 없으며 어떤 양의 데이터도 저장할 수 있습니다. 버킷 스토리지 용량에는 상한선이 없어 파티셔닝이 필요하지 않습니다. 데이터의 최종 일관성을 보장하기 위해 HA 배포를 지원하지만 파일 잠금과 같은 기능은 지원하지 않습니다. COS API는 HTTP/HTTPS 프로토콜을 통한 데이터 액세스를 지원하며 COS는 비즈니스에 통합할 수 있는 다양한 SDK 및 툴을 제공합니다. COS에 업로드된 객체는 URL을 통해 직접 액세스하거나 다운로드할 수도 있습니다.

[Cloud File Storage (CFS)](https://www.tencentcloud.com/document/product/582)는 일반적인 네트워크 파일 전송 프로토콜을 사용하고 파일 시스템을 생성하고 대규모 확장을 구현할 수 있지만 CVM에 마운트해야 합니다. 웹 사이트, 온라인 배포자 및 아카이브와 같은 광범위한 응용 프로그램에 대한 데이터를 저장할 수 있습니다. 높은 컴퓨팅 처리량과 매우 높은 가용성 및 지속성을 특징으로 하며 높은 동시성 또는 공유 스토리지를 요구하는 시나리오에도 적합합니다.

### COS와 CBS의 차이점은 무엇인가요?

[COS](https://www.tencentcloud.com/document/product/436)는 파일 시스템, 디렉토리 구조, 파일 수 또는 저장 용량에 제한이 없으며 Web API를 통해 관리 및 액세스해야 합니다. COS는 CVM 없이 독립적으로 사용할 수 있는 비즈니스 통합을 위한 다양한 SDK 및 툴을 제공합니다. COS는 방대한 양의 데이터에 대한 액세스를 지원하지만 밀리초 레벨의 응답 또는 임의 읽기/쓰기와 관련된 시나리오에는 적합하지 않습니다.

[CBS](https://www.tencentcloud.com/document/product/362)는 CVM 설치가 필요하며 파일 시스템의 파티션 혹은 포맷 후에 마운트하여 사용할 수 있습니다. CBS 유형과 사양 지표에 따라 IOPS 및 처리량 성능이 서로 다른 제품이 제공되므로 단독 디바이스로 여러 시나리오를 충족할 수 있습니다. 
...
COS는 폴더와 디렉터리 개념이 없지만 사용자들의 다양한 사용 습관을 고려해 기존 파일의 관리 디렉터리 구조를 차용했으며 콘솔 상에서 “폴더” 표시 방식을 본떴습니다. 자세한 사항은 [폴더와 디렉터리](https://intl.cloud.tencent.com/document/product/436/13324) 문서를 참고하십시오.
...
CI(Cloud Infinite)를 COS 콘솔과 통합하였기 때문에 이미지 확대/축소, 자르기, 워터마크 추가 등의 작업이 가능합니다. 자세한 사항은 [기본 이미지 처리](https://intl.cloud.tencent.com/document/product/436/36569) 문서를 참고하십시오.


### COS는 이미지 압축을 지원하나요?

COS는 비정형 데이터의 분산형 스토리지 서비스입니다. 서비스 자체는 이미지 압축을 지원하지 않습니다. 이미지 압축 처리는 [Cloud Infinite(CI)](https://www.tencentcloud.com/products/ci)를 참고하십시오.

### COS는 썸네일 기능을 제공하나요?

COS는 비정형 데이터의 분산형 스토리지 서비스입니다. 서비스 자체는 이미지 압축을 지원하지 않습니다. 썸네일 기능에 대해서는 [Cloud Infinite(CI)](https://www.tencentcloud.com/products/ci)를 참고하십시오.

### COS는 비디오 파일을 트랜스 코딩할 수 있나요?

COS는 비정형 데이터를 위한 분산 스토리지 서비스로 자체적으로 비디오 트랜스코딩을 지원할 수 없습니다. 동영상 파일을 트랜스코딩하는 방법은 [Cloud Infinite(CI)](https://www.tencentcloud.com/products/ci)를 참고하십시오.

### COS가 처리할 수 있는 오디오/비디오 파일 형식은 무엇입니까?

COS는 비정형 데이터를 위한 분산 스토리지 서비스로 자체적으로 오디오/비디오 파일 처리를 지원할 수 없습니다. mp4, avi, ts, hls, mp3 및 aac 파일과 같은 리치 미디어 파일을 처리하는 방법에 대한 자세한 내용은 [Cloud Infinite(CI)](https://www.tencentcloud.com/products/ci)를 참고하십시오.

### COS는 업로드된 파일의 자동 압축 해제를 지원하나요?
...
2021년 9월 COS 콘솔 업데이트 이후 버킷 길이 제한이 변경되었습니다. 새 정책에 따르면 통합된 전체 도메인은 최대 60자를 포함할 수 있으므로 버킷 이름의 길이는 **리전 약칭** 및 **APPID**의 문자 수에 영향을 받습니다. 기존 버킷 이름은 영향을 받지 않습니다. 특수한 상황으로 도메인 이름 길이를 확장해야 하는 경우 [고객센터](https://intl.cloud.tencent.com/contact-sales)로 문의하시기 바랍니다.
...
- 계정에 잔액이 부족해 금액이 연체될 경우(계정 잔액이 0보다 적은 경우) COS는 연체 24시간 후 서비스를 중단합니다.
