...
You can mount a bucket by using a temporary key (STS) by following the steps below:
...
>
Step 2: Run the COSFS command. Specify the role as `sts` by using the command option `-ocam_role=[role]`, and specify the key file's path by using `-opasswd_file=[path]`:
```shell
...
You can check the parameter options provided by COSFS by running the `cosfs --help` command, and check the COSFS version number by running `cosfs --version`.
...
When running the mounting command, you can specify a directory under in bucket as follows:
```shell
...
If you are not a root user, we recommend you create a `.passwd-cosfs` file in the `Home` directory, set the permission to 600, and then mount COSFS by running the mounting command. In addition, you can also specify the key file path by using the `-opasswd_file=path` option and set the permission to 600.

### Does COSFS support mounting over HTTPS?

Yes. Mounting methods over HTTP and HTTPS are as follows:
```shell
-ourl=http://cos.ap-guangzhou.myqcloud.com
-ourl=https://cos.ap-guangzhou.myqcloud.com
```

If the version of the NSS library that libcurl depends on is v3.12.3 or later (check the NSS version by running `curl -V`), you can mount the bucket over HTTPS by running the following command:
...
In a COSFS-mounted directory, you can create a file with a name not containing `/`. On a Unix-like system, the `/` character is a directory separator, so `/` is not allowed in filenames in a COSFS-mounted directory. In addition, when creating a file with a name containing special characters, make sure that such characters are not used by shell; otherwise, file creation may fail.
...
You can check whether the COSFS process exists by running the `ps ax|grep cosfs` command. If the COSFS process is down due to faulty operations, run the following command to remount it:
...
COSFS automatically sets the Content-Type of a file uploaded to COS by comparing the file's suffix against /etc/mime.types. When a Content-Type problem occurs, check whether the configuration file exists on the system. For Ubuntu, you can add it by using `sudo apt-get install mime-support`. For CentOS, you can add it by using `sudo yum install mailcap`. You can also create this file manually, where one line is added for each file format:
...
### I have set auto-amounting at startup for COSFS by using `/etc/fstab`, but the error "wrong fs type, bad option, bad superblock on cosfs" occurs when I run "mount -a". Why?
...
### I have copied a file to COSFS by running the rsync command and the progress has reached 100%, but I only see a temporary file on the server. Why?
...
Lower versions of the mounting tool do not support the `--no-canonicalize` option. Update the tool. We recommend you [download v2.17](https://cdn.kernel.org/pub//linux/utils/util-linux/v2.17/) and then mount it again. The installation command is as follows:
...
Step 2. Check whether the server time is correct by running the date command. If it is incorrect, correct it by running the date command and then mount it again, such as `date -s '2014-12-25 12:34:56'`.
...
You can mount a non-empty directory by using the `-ononempty` parameter, but we recommend you not do so because a problem may occur when the mount target and the original directory have files with the same path.
...
### What should I do if COSFS has a high CPU utilization and sends a large number of HEAD and LIST requests to COS during a certain period of time every day?
This is usually caused by scheduled disk scanning tasks on your server. A common disk scanning program on Linux is `updatedb`. You can add the COSFS mount target directory to the `PRUNEPATHS` configuration item in the configuration file `/etc/updatedb.conf` of `updatedb` to prevent disk scanning.
