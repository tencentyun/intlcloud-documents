...
The `masquerade_address` is a server address configured for the client. When the FTP server runs on a host that is mapped to a public IP through NAT, you need to configure the masquerade_address option as an FTP server public IP that the client can access in order to notify the client to use the IP for data communication with the server.
...
Therefore, in most cases, it is recommended to set `masquerade_address` to the IP used by the client for connecting to the server.
