...
Hadoop-COS는 Apache Hadoop, Spark, Tez 등 빅 데이터 연산 프레임워크의 통합을 지원하여 HDFS 액세스와 같이 Tencent Cloud COS에 저장된 데이터를 읽고 쓸 수 있습니다. 또한 Druid 등의 쿼리와 분석 엔진 Deep Storage도 지원됩니다.

### Hadoop을 자체 구축하였는데 Hadoop-COS jar 패키지는 어떻게 사용합니까?

Hadoop-COS pom 파일을 수정하여 버전이 Hadoop 버전과 동일하게 유지한 상태에서 컴파일합니다. 그리고 Hadoop-COS jar 패키지와 COS JAVA SDK jar 패키지를 hadoop/share/hadoop/common/lib 디렉터리에 넣습니다. 자세한 설정 방법은 [Hadoop 툴](https://intl.cloud.tencent.com/document/product/436/6884) 문서를 참조하십시오.


### Hadoop-COS 툴에 휴지통 메커니즘이 존재합니까?
HDFS의 휴지통 기능은 COS에 적용되지 않으며, Hadoop-COS를 사용하여 `hdfs fs` 명령어를 통해 COS 데이터를 삭제하는 경우 데이터가 cosn://user/${user.name}/.Trash 디렉터리로 이동되어 실제로 삭제 작업은 진행되지 않고 COS에 보관됩니다. 별도로 `-skipTrash` 매개변수를 사용해 휴지통 기능을 생략하고 직접 데이터를 삭제할 수도 있습니다. HDFS 휴지통과 유사하게 정기적으로 데이터를 삭제할 목적인 경우 객체 접두사가 `/user/${user.name}/.Trash/`인 객체에 라이프사이클 규칙을 설정할 수 있으며, 설정 가이드는 [라이프사이클 규칙 설정](https://intl.cloud.tencent.com/document/product/436/14605)을 참조하십시오.


## CosFileSystem 클래스를 찾을 수 없는 문제
### 로딩 시 CosFileSystem 클래스를 찾을 수 없다는 오류 Error: java.lang.RuntimeException: java.lang.ClassNotFoundException: Class org.apache.hadoop.fs.CosFileSystem not found가 표시됩니다.

**가능한 원인**
설정은 정확하게 로딩되었지만 hadoop classpath에 Hadoop-COS jar 패키지 위치가 포함되어 있지 않아 발생할 수 있습니다.

**해결 방법**
Hadoop-COS jar 패키지를 hadoop classpath에 로딩합니다.


### 정품 Hadoop을 사용하는데 CosFileSystem 클래스를 찾을 수 없다는 오류가 표시됩니다.

Hadoop-COS는 정품 Hadoop 버전과 Hadoop-COS 버전을 유지 보호합니다. 상응하는 fs.cosn.impl과 fs.AbstractFileSystem.cosn.impl 설정이 다를 수 있습니다.
- 정품 Hadoop 설정:
```xml
<property>
...
신규 버전은 속도 제한 fs.cosn.traffic.limit(b/s) 설정을 지원하며, 해당 설정은 tag가 5.8.3 이상인 버전에서만 지원합니다. [Github 웨어하우스](https://github.com/tencentyun/hadoop-cos)에서 확인할 수 있습니다.

## 멀티파트 문제

### Hadoop-COS에서 업로드하는 멀티파트 블록의 크기는 어떻게 합리적으로 설정합니까?
Hadoop-COS 내부에서 멀티파트 동시 업로드를 통해 대용량 파일을 처리하며, fs.cosn.upload.part.size(Byte) 설정으로 COS에 업로드하는 블록의 크기를 제어합니다.
