...
1단계: 임시 키 설정파일을 생성합니다. 예를 들어 /tmp/passwd-sts에 COSFS에 사용하는 명령어 선택 항목 -opasswd-file=\[path\]로 키 구성 파일을 지정합니다. 임시 키 관련 개념은 [임시 키 생성 및 사용 가이드](https://intl.cloud.tencent.com/document/product/436/14048) 문서를 참조하십시오. 임시 키 구성 파일 예시는 다음과 같습니다.
```shell
COSAccessKeyId=AKIDXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  #이하 임시 키 Id, Key와 Token 필드
...
```
이 중 COSFS는 COSAccessTokenExpire에 설정된 시간에 따라 키 파일에서 설정을 다시 로딩해야 할지 판단합니다.
...
>
2단계: COSFS 명령어를 실행합니다. 명령어 옵션 -ocam_role=[role]을 사용해 역할을 sts로 지정하고, -opasswd_file=[path]로 키 파일 경로를 지정합니다. 예시는 다음과 같습니다.
```shell
...
### COSFS에서 제공하는 마운트 매개변수 선택 항목 및 버전은 어떻게 조회하나요?

`cosfs --help` 명령어를 사용하여 COSFS에서 제공하는 매개변수 선택 항목을 조회할 수 있으며, `cosfs --version` 명령어를 사용하여 COSFS 버전을 조회할 수 있습니다.

### COSFS에 생성되는 로그는 어떻게 조회하나요?

CentOS의 경우 COSFS에 생성되는 로그는 /var/log/messages에 저장되며, Ubuntu의 경우 /var/log/syslog에 저장됩니다. 사용 중 문제가 발생할 경우 해당 시간의 로그를 보내주십시오.
...
libcurl에 종속되는 NSS 라이브러리는 3.12.3 이상 버전의 시스템(`curl -V` 명령어를 사용해 NSS 버전 조회)으로, HTTPS 방식으로 버킷을 마운트하는 경우 다음 명령어를 실행해야 합니다.
...
권한을 개방하려는 경우 마운트 시 `-oallow_other`를 지정합니다.

### COSFS 마운트 디렉터리에 파일 생성 시 이름 제한이 있습니까?

COSFS 마운트 디렉터리에 `/` 문자가 포함되지 않는 이름의 파일을 생성할 수 있습니다. Unix 시스템 상에서 `/` 문자는 디렉터리 세퍼레이터이므로 COSFS 마운트 디렉터리에 `/` 문자가 포함된 파일은 생성할 수 없습니다. 이외에도 특수문자가 포함된 파일 생성 시 특수 문자가 shell에 사용되어 파일 생성에 실패하는 문제가 발생할 수 있으니 유의해야 합니다.
...
`ps ax|grep cosfs` 명령어를 사용하여 COSFS 프로세스가 존재하는지 조회할 수 있습니다. COSFS 프로세스가 오작동으로 종료된 경우 다음 명령어를 실행하여 다시 마운트할 수 있습니다.
...
COSFS 프로세스가 오작동으로 종료되지 않은 경우 기기 상의 fuse 버전이 2.9.4 이하인지 확인합니다. libfuse가 2.9.4 버전 이하인 경우 COSFS 프로세스에 오류가 발생하여 종료될 수 있습니다. 이 경우 [COSFS 툴](https://intl.cloud.tencent.com/document/product/436/6883) 문서를 참조하여 fuse 버전을 업데이트하거나 COSFS 최신 버전 설치를 권장합니다.

### COSFS를 통해 업로드한 파일의 Content-Type이 "application/octet-stream"으로 변경되었습니다. 어떻게 해야 합니까?

COSFS는 /etc/mime.types에 따라 업로드 파일의 확장자명을 비교하여 COSFS에 업로드하는 파일의 Content-Type을 자동으로 설정합니다. Content-Type 문제가 발생하는 경우 시스템 상에 해당 구성 파일이 존재하는지 확인하십시오. Ubuntu인 경우 sudo apt-get install mime-support를 통해 추가할 수 있으며, CentOS인 경우에는 sudo yum install mailcap을 통해 추가할 수 있습니다. 또한 수동으로 해당 파일을 생성할 수 있으며 모든 파일 포맷에 대해 다음과 같이 라인을 추가합니다.
...
COS 제품 인증 정책이 변경되어 V1.0.0 이전 버전의 COSFS 툴을 사용하는 경우 정책 검증에 실패하게 됩니다. 최신 COSFS 툴을 설치하여 다시 마운트하십시오.
...
3. cp 명령어를 사용해 복사하고 -p 또는 -a 매개변수가 있는 경우, 해당 매개변수를 제거한 후 해당 명령어를 실행할 것을 권장합니다.
...
### /etc/fstab을 사용하여 COSFS 시작 시 자동 마운트를 설정했습니다. 그러나 mount -a 실행 시 "wrong fs type, bad option, bad superblock on cosfs" 오류가 발생합니다.
...
쓰기 과정에서 해당 마운트 포인트 Is 명령어의 결과로 파일 크기가 변경되며, 파일을 비활성화할 때 COSFS가 로컬 캐시 파일 상에 입력한 데이터를 COS에 업로드합니다. 업로드 실패한 경우에만 크기가 0인 파일을 볼 수 있으며, 이 경우 실패한 파일을 다시 복사하십시오.
...
### rsync 명령어를 사용하여 파일을 COSFS에 복사하고 진행률이 이미 100%로 표시되는데 서버에 임시 파일만 보입니다. 이유가 무엇인가요?
rsync 명령어가 마운트 디렉터리에 있어 임시 파일을 생성하게 되며, 진행률이 100%로 표시되는 것은 임시 파일이 로컬에 입력을 100% 완료했다는 의미입니다. 임시 파일 쓰기 완료 후에 COS에 업로드되며 다른 이름으로 변경하여 복사됩니다.
일반적으로 COSFS 마운트 디렉터리에 데이터를 복사하는 경우 rsync 명령어를 사용하여 cp 명령어에 비해 더 많은 시간이 소요될 수 있습니다.

### COSFS의 디스크 용량이 거의 꽉 찼습니다. 어떻게 처리해야 하나요?
...
docker 이미지 실행 시 매개변수 --privileged를 추가해야 합니다.
...
마운트 디렉터리 시간이 1970-01-01 08:00로 변경된 것은 정상적인 현상입니다. 마운트 포인트를 언마운트하면 마운트 디렉터리 시간이 마운트 전 시간으로 복구됩니다.

 ### 마운트 디렉터리가 비어 있지 않아도 되나요?
...
### COSFS 디렉터리에서 Is 명령어 실행 시 명령어 반환 시간이 오래 걸리는 이유는 무엇인가요?
마운트 디렉터리에 파일이 많은 상황에서 Is 명령어를 실행하려면, 디렉터리의 모든 파일에 대해 HEAD 작업을 진행해야 합니다. 따라서 디렉터리 시스템의 호출을 읽은 후 반환하려면 비교적 긴 시간이 소모됩니다.
>!불필요한 재시작이 발생할 수 있어 IO hung 활성화는 권장하지 않습니다.
>


### info 레벨의 로그를 사용하여 생성된 시스템 로그 파일이 스토리지 용량을 너무 많이 차지합니다. 어떻게 처리해야 하나요?
...
COSFS의 읽기/쓰기는 모두 디스크를 경유하여 전송됩니다. 공유 데이터 세트의 머신러닝 알고리즘에서 공유 데이터 읽기, 간단한 로그 백업 등 POSIX semantics로 COS에 액세스해야 하는 시나리오에 적합합니다. COSFS는 동시 업로드 및 다운로드 시 멀티 스레드로 가속합니다. 리전 내에서 내부 네트워크를 통해 6GB 파일 순차적으로 읽기에 소모되는 시간은 약 80s이며, 6GB 파일 순차적으로 쓰기에 소모되는 시간은 약 160s입니다. 일반적으로 SDK 및 멀티 스레드 등의 기술을 통해 더 좋은 성능을 구현할 수 있습니다.
...
### COSFS 경로에서 Is 명령어 실행 시 명령어 반환 시간이 오래 걸리는 이유는 무엇인가요?

마운트 디렉터리에 파일이 많은 상황에서 Is 명령어를 실행하려면, 디렉터리의 모든 파일에 대해 HEAD 작업을 진행해야 합니다. 따라서 디렉터리 시스템의 호출을 읽은 후 반환하려면 비교적 긴 시간이 소모됩니다. 불필요한 재시작이 발생할 수 있어 IO hung 활성화는 권장하지 않습니다.
