...
3. 현재 간편 업로드의 속도는 10MB에 달하며, 고급 API로 32개를 동시 업로드를 하는 경우 60MB에 달합니다. 만약 속도가 이 두 값보다 훨씬 낮다면 위의 두 사항을 참조하십시오.
4. WARN 로그에서 IOException이 출력되면 SDK가 재시작되므로 무시해도 됩니다. IOException의 원인이 너무 느린 네트워크 속도 때문이라면 해결 방법 1과 2를 참조하십시오.
...
COS Java SDK 구버전은 예를 들어 125로 시작하는 APPID만 지원합니다. 만약 130으로 시작하는 APPID를 사용하는 경우 SDK를 최신 버전으로 업데이트하는 것을 권장합니다. Java SDK의 다운로드 및 설치는 [Java SDK 문서](https://intl.cloud.tencent.com/document/product/436/10199)를 참조하십시오.
...
COS Java SDK 자체는 로컬 파일의 crc64 계산을 지원하지 않으며, 사용자의 서비스에서 직접 구현해야 합니다. [Java issues](https://github.com/tencentyun/cos-java-sdk-v5/issues/63)를 참조하여 방법을 확인하십시오.
