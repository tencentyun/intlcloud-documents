...
2. 请检查您设置的过期时间（experationDate）是否小于当前时间，若当前时间大于过期时间则会导致签名过期，您需要修改过期时间重新生成临时密钥；
...
COS 中的目录是虚拟的，实际是一个以 / 结尾的对象。您可以调用上传对象接口，创建一个以 / 结尾的对象键即是一个目录。可参考 [小程序 SDK 创建目录示例](https://intl.cloud.tencent.com/document/product/436/32457) 。
