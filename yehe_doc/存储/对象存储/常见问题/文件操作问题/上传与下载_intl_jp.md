...
中国大陸パブリッククラウドリージョンでの1バケットのデフォルトの帯域幅は15Gbit/s、その他のリージョンでは10Gbit/sです。帯域幅がこの閾値に達すると、リクエストによってトラフィックコントロールがトリガーされます。これ以上の帯域幅が必要な場合は、[お問い合わせ](https://intl.cloud.tencent.com/contact-sales)ください。

### ファイルをダウンロードせず、直接ブラウザでプレビューするにはどうすればよいですか。

バケットドメイン名の形式が`<BucketName-APPID>.cos.<Region>.myqcloud.com`であれば、それはXMLバージョンのドメイン名です。ブラウザが直接プレビューをサポートしているファイルタイプであれば、この形式のドメイン名に対応するオブジェクトリンクにアクセスすることで、ブラウザでのファイルプレビューが実現できます。

バケットドメイン名の形式が`<BucketName-APPID>.<region>.myqcloud.com`であれば、それはJSONバージョンのドメイン名です。JSONバージョンのドメイン名が対応するオブジェクトリンクは、ブラウザでアクセスするとダウンロードのポップアップが表示されます。ブラウザでファイルをプレビューしたい場合は、2つの方法があります。

1. COSコンソールを[新バージョンコンソール](https://console.cloud.tencent.com/cos5)にアップグレードし、XMLバージョンのドメイン名に対応するオブジェクトリンクを使用してアクセスします（強く推奨）。
2. カスタムドメイン名をバインドし、静的ウェブサイトを有効にし、カスタムドメイン名を使用してアクセスします。ドキュメントは[JSONバージョンのドメイン名管理](https://intl.cloud.tencent.com/document/product/436/18424)および[JSONバージョンの静的ウェブサイトの設定](https://intl.cloud.tencent.com/document/product/436/14984)をご参照ください。

#### 事例：

北京リージョンのexamplebucket-1250000000バケットルートディレクトリ下のpicture.jpgファイルを例に説明します。

- オブジェクトアドレスが`https://examplebucket-1250000000.cos.ap-beijing.myqcloud.com/picture.jpg`形式の場合は、このアドレスをそのまま使用してブラウザでpicture.jpgファイルをプレビューできます。
- オブジェクトアドレスが`https://examplebucket-1250000000.cosbj.myqcloud.com/picture.jpg`形式の場合、直接ブラウザでオブジェクトをプレビューしたい場合は次の2つの方法があります。
  1. COSコンソールを[新バージョンコンソール](https://console.cloud.tencent.com/cos5)にアップグレードし、XMLバージョンのドメイン名に対応するオブジェクトリンクを使用してアクセスします（強く推奨）。
  2. カスタムドメイン名をバインドし、静的ウェブサイトを有効にし、カスタムドメイン名を使用してアクセスします。ドキュメントは[JSONバージョンのドメイン名管理](https://intl.cloud.tencent.com/document/product/436/18424)および[JSONバージョンの静的ウェブサイトの設定](https://intl.cloud.tencent.com/document/product/436/14984)をご参照ください。

### ファイルをプレビューせず、直接ブラウザでダウンロードするにはどうすればよいですか。

[COSコンソール](https://console.cloud.tencent.com/cos5)でカスタムHeaders内のContent-Dispositionパラメータ値をattachmentに設定します。コンソールの操作ガイドは[カスタムHeaders](https://intl.cloud.tencent.com/document/product/436/13361)をご参照ください。
...
- コンソール方式：操作手順については[オブジェクトのアップロード](https://intl.cloud.tencent.com/document/product/436/13321)をご参照ください。
- API/SDK方式：COSではプログラミングによってAPIまたはSDKインターフェースを複数回呼び出す方式でファイルのバッチ操作を行うことができます。詳細については、[オブジェクトアップロードAPIインターフェース](https://intl.cloud.tencent.com/document/product/436/10111)および[SDKの概要](https://intl.cloud.tencent.com/document/product/436/6474)をご参照ください。
- ツール方式：[COSCMDコマンドラインツール](https://intl.cloud.tencent.com/document/product/436/10976)および[COSBrowserツール](https://intl.cloud.tencent.com/document/product/436/11366)を使用してバッチ操作を実現できます。
