...
COS(Cloud Object Storage)의 액세스 엔드포인트는 지능형 DNS 확인을 사용합니다. 인터넷(다른 ISP 포함)을 통한 COS 액세스의 경우 COS에 액세스할 수 있는 최적의 연결을 감지하고 선택합니다. COS에 액세스하기 위해 Tencent Cloud에 서비스를 배포한 경우 동일한 리전 내 액세스는 자동으로 사설망 주소로 지정됩니다. 사설망에서는 리전 간 액세스가 지원되지 않으며 COS 엔드포인트는 기본적으로 공중망 주소로 확인됩니다.
...
동일한 리전 내의 Tencent Cloud 제품은 기본적으로 사설망을 통해 서로 액세스하므로 트래픽 비용이 발생하지 않습니다. 따라서 비용을 절약하기 위해 다른 Tencent Cloud 제품을 구매할 때 동일한 리전을 선택하는 것이 좋습니다.
