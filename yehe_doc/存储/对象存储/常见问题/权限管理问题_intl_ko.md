...
### 버킷 생성 시 오류가 보고되고 권한 설정이 올바르지 않은 경우 어떻게 해야 하나요?

이 경우 버킷이 공개 읽기/비공개 쓰기 또는 공개 읽기/쓰기 권한으로 설정되어 루트 계정의 ACL 규칙 수가 점유된 것입니다. 루트 계정의 ACL 규칙 수가 최댓값에 도달하고, 최댓값을 조정할 수 없으므로 오류가 보고됩니다.
