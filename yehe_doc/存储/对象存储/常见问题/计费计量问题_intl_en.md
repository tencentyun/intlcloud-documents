...
Log in to the Tencent Cloud console, select **Billing Center** > **[Bill Overview](https://console.cloud.tencent.com/expense/bill/overview)**, and view the billing statistical period of your account as shown in the red box in the following figure, which can be billing cycle or deduction cycle.
...
Your account is billed by deduction cycle and was upgraded from monthly settlement to daily settlement on September 1, 2022, and you downloaded the **bill details (L3)** and **usage details** of COS in the [Billing Center](https://console.cloud.tencent.com/expense/bill/summary) on September 30.

Taking the **COS STANDARD storage usage** billable item as an example, the bill contained the storage usage fees for August and September.

1. In the **bill details (L3)**:
(1) Pay-as-you-go monthly settlement: The monthly bill was generated on September 1 for storage usage fees incurred in the entire August (from 00:00 on August 1 to 23:59 on August 31).

Therefore, the **monthly COS STANDARD storage usage** of the `asp` bucket from August 1 to August 30, 2022 was 2732799.233 GB (the sum of two monthly records), as shown in column Y in the following figure:
...
Therefore, the **daily COS STANDARD storage usage** of the `asp` bucket from September 1 to September 29, 2022 changed every day, and the total amount of daily storage usage by September 29 was 79784325.88 GB, as shown in column Y in the following figure:
![](https://qcloudimg.tencent-cloud.cn/raw/24d87c453c639d6860ab4484a15235cd.png)

2. In the **usage details**:
(1) Daily usage details: They contain the daily usage from 00:00 on August 1 to 23:59 on September 29.

Therefore, the **daily COS STANDARD storage usage** of the `asp` bucket from August 1 to September 29, 2022 was displayed in the daily usage details, as shown in column E in the following figure:
Here:
...
![](https://qcloudimg.tencent-cloud.cn/raw/4bbf13ac895d2a7193d19beac1c00c9f.png)
  - From September 1 to September 29, the fees were settled daily. As the statistics collection logics were the same, the **daily storage usage** in the usage details was basically the same as the **daily storage usage** in the bill details (L3). The total of the **daily storage usage** in the usage details was 79784325.65 GB.
![](https://qcloudimg.tencent-cloud.cn/raw/b69e0b0bcd4ba79b10c237e24a623667.png)

(2) Cost allocation by month: The total storage usage was the sum of the monthly storage usage of the bucket from 00:00 on August 1 to 23:59 on August 31 and the daily storage usage from 00:00 on September 1 to 23:59 on September 29.

Therefore, the sum of the **daily and monthly COS STANDARD storage usage** of the `asp` bucket from August 1 to September 29, 2022 was displayed in the monthly usage details (5306487.1688193 GB), as shown in column F in the following figure:
![](https://qcloudimg.tencent-cloud.cn/raw/490b5f63e147f0846c7d1cf1cf917a11.png)

As the statistics collection logics were different, the storage usage in the monthly usage details for cost allocation was different from that in the bill details (L3).


In conclusion, as the statistics collection logics were different, the usage amounts in the monthly usage details for cost allocation was different from that in the bill details (L3). This was normal and happened only in the first month after the upgrade from monthly settlement to daily settlement for billing by deduction cycle.
