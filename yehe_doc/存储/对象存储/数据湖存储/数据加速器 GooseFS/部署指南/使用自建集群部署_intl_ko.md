...
1. 먼저 [GooseFS의 이진법 배포 패키지 다운로드](https://downloads.tencentgoosefs.cn/goosefs/1.3.0/release/goosefs-1.3.0-bin.tar.gz)를 합니다.
...
동일하게, GooseFS 클러스터 실행 시 마운트를 할 수 있습니다. 
...
- `conf/masters`와  `conf/workers`의 호스트 이름 혹은 IP 주소 설정을 하고, 최종 프로덕션 환경의 모든 구성의 설정을 완료해야 합니다. 
- ‘scripts’ 디렉터리로 이동하여  `install.properties` 구성 파일을 꼼꼼하게 입력하고 ‘root’ 계정으로 전환 혹은 ‘sudo’ 신분을 사용을 통해 `config_ssh.sh`를 실행하여 클러스터의 ‘비밀번호 없이 로그인’ 설정을 완료합니다. 
- ‘비밀번호 없이 로그인’ 설정 완료 후 `validate_env.sh` 툴을 실행하여 전체 클러스터 설정 상태를 검증합니다. 
- ‘root’ 계정 혹은 ‘sudo’ 신분으로 `install.sh` 스크립트를 실행하여 설치 배포 실행 후, 모든 프로세스가 완료될 때까지 기다립니다. 
...
1. [GooseFS의 이진법 배포 패키지 다운로드](https://downloads.tencentgoosefs.cn/goosefs/1.3.0/release/goosefs-1.3.0-bin.tar.gz)를 합니다. 
...
```
`goosefs.master.hostname` 을 단일 master 노드의 hostname 혹은 ip로 설정합니다. `goosefs.master.mount.table.root.ufs`는 지정 GooseFS 루트 디렉터리가 마운트한 유닉스 파일 시스템 (UFS) 경로 URI 입니다. 주의: 해당 URI는 반드시 Master와 Worker 노드 모두 액세스 할 수 있어야 합니다. 
...
|  프로세스  |  설명  |
|-----|-----|
|GooseFSMaster   | RPC 포트는 9200으로, Web 포트는 9201으로 기본 설정되어 있습니다.   |
|  GooseFSWorker  | RPC 포트는 9203으로, Web 포트는 9204으로 기본 설정되어 있습니다.     |
|  GooseFSJobMaster    |    RPC 포트는 9205으로, Web 포트는 9206으로 기본 설정되어 있습니다.   |
|  GooseFSProxy           |   Web 포트는 9211으로 기본 설정되어 있습니다.   |
| GooseFSJobWorker       |    RPC 포트는 9208으로, Web 포트는 9210으로 기본 설정되어 있습니다.  |
