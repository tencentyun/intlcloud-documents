...
1. 로컬 디렉터리를 생성하고 입력한 다음(필요에 따라 다른 디렉터리를 선택할 수도 있음) [goosefs-1.4.0-bin.tar.gz](https://downloads.tencentgoosefs.cn/goosefs/1.4.0/release/goosefs-1.4.0-bin.tar.gz)를 다운로드합니다.
```
$ cd /usr/local
$ mkdir /service
$ cd /service
...
$ cd goosefs-1.4.0
```
압축 해제 후 gooseFS goosefs-1.4.0의 홈 디렉터리가 생성됩니다. 본문에서는 `${GOOSEFS_HOME}`을 이 홈 디렉터리의 절대 경로로 사용합니다.
