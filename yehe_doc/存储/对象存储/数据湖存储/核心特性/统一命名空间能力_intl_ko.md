...
GooseFS 통합 네임스페이스(NameSpace) 기능은 투명한 네이밍 메커니즘을 통해 다양한 기본 스토리지 시스템 액세스 의미 체계를 통합하여 사용자에게 데이터 관리에 대한 통합된 인터랙티브 뷰를 제공합니다.

GooseFS는 로컬 파일 시스템, Tencent Cloud Object Storage(COS), Tencent Cloud HDFS(Cloud HDFS, CHDFS) 등 다양한 기본 스토리지 시스템을 통합 네임스페이스 기능을 통해 연결하고, 이러한 기본 스토리지 시스템과 통신하며, 상위 레이어 서비스를 위한 통합 액세스 인터페이스 및 파일 프로토콜을 제공합니다. 서비스측은 GooseFS의 액세스 인터페이스를 사용하여 다른 기본 스토리지 시스템에 저장된 데이터에 액세스할 수 있습니다.

![](https://main.qcloudimg.com/raw/6ef8e2899bbd704f7c05c7d0526624b1.png)

상기 이미지는 통합 네임스페이스가 어떻게 작동하는지 보여줍니다. GooseFS를 사용하여 네임스페이스 명령 create ns를 생성하여 COS 및 Cloud HDFS의 지정된 파일 디렉터리를 GooseFS에 마운트한 다음 gfs://의 통합 schema를 통해 데이터에 액세스할 수 있습니다. 자세한 설명은 다음과 같습니다.
...
- 파일이 GooseFS에 캐시되지 않은 경우, 파일이 로컬 파일 시스템에 캐시되지 않았기 때문에 gfs:// 형식을 통한 액세스는 실패하게 됩니다. 하지만 기본 스토리지 시스템의 네임스페이스를 통해 액세스할 수 있습니다.

## 통합 네임스페이스 기능 사용

ns 작업을 통해 GooseFS에 네임스페이스를 생성하고, 기본 스토리지 시스템을 GooseFS에 매핑할 수 있습니다. 현재 지원되는 기본 스토리지 시스템에는 COS, 클라우드 HDFS, 로컬 HDFS 등이 있습니다. 네임스페이스 생성 작업은 Linux 파일 시스템에서 파일 볼륨을 마운트하는 것과 유사합니다. 네임스페이스 생성 후, GooseFS는 클라이언트에 통합 액세스 의미 체계를 갖춘 파일 시스템을 제공할 수 있습니다. 현재 GooseFS 네임스페이스의 작업 명령어 집합은 다음과 같습니다.
...
| CACHE(2) | <li>메타데이터 액세스 동작: 캐시가 히트되면 메타데이터는 Master를 따르며, 메타데이터는 언더 스토리지부터 능동적으로 동기화되지 않습니다. </li><li>데이터 스트림 액세스 동작: 데이터 스트림의 ReadType은 CACHE 정책을 사용합니다. </li> | Once | CACHE | 낮음 | 히트: 높음<br/>미스: 낮음 | NO |
...
>! 네임스페이스 생성 시 캐시 정책을 지정하는 것 외에도 사용자는 파일을 읽기/쓰기할 때 지정된 파일에 대해 ReadType 또는 Write_Type을 설정하거나 properties  프로파일을 통해 전역 캐시 정책을 구성할 수도 있습니다. 여러 정책이 동시에 존재하는 경우 우선순위는 사용자 정의 우선순위 >  Namespace 읽기/쓰기 정책 > 프로파일의 전역 캐시 정책 구성입니다. 이 중 읽기 정책의 경우 사용자 정의 ReadType과 Namespace의 DirReadPolicy 조합이 적용됩니다. 즉, 데이터 스트림 읽기 정책은 사용자 정의 ReadType을 채택하고 메타데이터는 Namespace 정책을 채택합니다. 
>
...
conf/goosefs-site.properties 프로파일에서 메타데이터 동기화 주기를 수정하여 메타데이터 동기화 주기를 관리할 수 있으며, 구성 매개변수는 다음과 같습니다.
...
- GooseFS 클러스터가 위치한 컴퓨터실과 기본 스토리지 간의 물리적 거리가 멀수록, 메타데이터 동기화 딜레이가 길어집니다.
...
<table>
   ...
   <tr>
      ...
      <td rowspan=3>동일한 디렉터리에 따라 다른 업데이트 주기를 구성하면, 메타데이터 동기화의 부담을 완화할 수 있습니다.</td>
   </tr>
   <tr>
