...
- 确保已经开通元数据加速，并且进行了正确的环境部署和 HDFS 协议配置。具体部署和配置，详情请参见 [使用 HDFS 协议访问已开启元数据加速器的存储桶](https://intl.cloud.tencent.com/document/product/436/46198)。
...
public class Demo {
			private static FileSystem initFS() throws IOException {
				...
				// 配置项可参见 https://intl.cloud.tencent.com/document/product/1106/41965
				// 以下配置是必填项
			...
			conf.set("fs.cosn.trsf.fs.ofs.impl", "com.qcloud.chdfs.fs.CHDFSHadoopFileSystemAdapter");
				...
				// 其他可选配置项请参见 https://intl.cloud.tencent.com/document/product/1106/41965 
...
> - 对于 EMR 环境，如果您按照 [使用 HDFS 协议访问已开启元数据加速器的存储桶](https://intl.cloud.tencent.com/document/product/436/46198) 逐步操作，那么 Hadoop common 包通常在 `/usr/local/service/hadoop/share/hadoop/common/` 目录下，元数据加速桶依赖的 Jar 包通常在 `/usr/local/service/hadoop/share/hadoop/common/lib/` 目录下。
> 
