...
<!--建议使用子账号密钥或者临时密钥的方式完成配置，提升配置安全性。为子账号授权时请遵循[最小权限指引原则](https://intl.cloud.tencent.com/document/product/436/32972)。-->
<property>
...
| fs.cosn.bucket.region               | 格式形如 ap-beijing               | 请填写待访问存储桶的地域信息，枚举值请参见 [地域和访问域名](https://intl.cloud.tencent.com/document/product/436/6224) 中的地域简称，例如：ap-beijing、ap-guangzhou 等。兼容原有配置：fs.cosn.userinfo.region。 |
...
>?
>- POSIX 访问方式下除通用配置项外，还需要添加以下配置内容。POSIX 访问方式的 [其他可选配置项](https://intl.cloud.tencent.com/document/product/1106/41965) 添加"fs.cosn.trsf." 前缀即可用于访问元数据加速桶。
...
S3协议访问方式需要设置如下配置，其他可选项请参见 [Hadoop-cos 配置项](https://intl.cloud.tencent.com/document/product/436/6884)。
