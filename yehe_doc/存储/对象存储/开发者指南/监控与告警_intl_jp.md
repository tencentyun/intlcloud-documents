...
COSの読み取り/書き込みリクエスト量、トラフィックなどのデータは[BCM](https://intl.cloud.tencent.com/document/product/248) によって集計され、表示されます。COSコンソールまたはBCMの[コンソール](https://console.cloud.tencent.com/monitor)上で、COSの読み取り/書き込みリクエスト量、トラフィックなどの詳細な監視データを確認することができます。

>?ここでは主に、COSコンソールで統計データを取得するケースについて述べます。データインターフェースを使用してより詳細な情報を取得する方法についてお知りになりたい場合は、BCMインターフェースをご利用ください。詳細については、[BCM](https://intl.cloud.tencent.com/document/product/248)製品ドキュメントをご参照ください。
...
| MazStdReadRequests   | マルチAZ標準ストレージ読み取りリクエスト     | マルチAZ標準ストレージタイプの読み取りリクエスト回数です。リクエスト回数はリクエストコマンドの送信回数に基づいて計算されます | 回   | appid、bucket |
| MazStdWriteRequests  | マルチAZ標準ストレージ書き込みリクエスト     | マルチAZ標準ストレージタイプの書き込みリクエスト回数です。リクエスト回数はリクエストコマンドの送信回数に基づいて計算されます | 回   | appid、bucket |
...
| MazIaReadRequests    | マルチAZ低頻度ストレージ読み取りリクエスト     | マルチAZ低頻度ストレージタイプの読み取りリクエスト回数です。リクエスト回数はリクエストコマンドの送信回数に基づいて計算されます | 回   | appid、bucket |
| MazIaWriteRequests   | マルチAZ低頻度ストレージ書き込みリクエスト     | マルチAZ低頻度ストレージタイプの書き込みリクエスト回数です。リクエスト回数はリクエストコマンドの送信回数に基づいて計算されます | 回   | appid、bucket |
...
| MazItReadRequests    | マルチAZ INTELLIGENT_TIERINGストレージ読み取りリクエスト | マルチAZ INTELLIGENT_TIERINGストレージタイプの読み取りリクエスト回数です。リクエスト回数はリクエストコマンドの送信回数に基づいて計算されます | 回   | appid、bucket |
| MazItWriteRequests   | マルチAZ INTELLIGENT_TIERINGストレージ書き込みリクエスト | マルチAZ INTELLIGENT_TIERINGストレージタイプの書き込みリクエスト回数です。リクエスト回数はリクエストコマンドの送信回数に基づいて計算されます | 回   | appid、bucket |
...
| MazStdStorage                | マルチAZ標準ストレージ-ストレージ容量           | MB   | appid、bucket |
| SiaStorage                   | 低頻度ストレージ-ストレージ容量                 | MB   | appid、bucket |
| MazIaStorage                 | マルチAZ低頻度ストレージ-ストレージ容量           | MB   | appid、bucket |
...
| MazItFreqStorage             | マルチAZ INTELLIGENT_TIERINGストレージ-高頻度階層ストレージ容量 | MB   | appid、bucket |
| MazItInfreqStorage           | マルチAZ INTELLIGENT_TIERINGストレージ-低頻度階層ストレージ容量 | MB   | appid、bucket |
...
| MazStdObjectNumber           | マルチAZ標準ストレージ-オブジェクト数           | 個   | appid、bucket |
| IaObjectNumber               | 低頻度ストレージ-オブジェクト数                 | 個   |appid、bucket |
| MazIaObjectNumber            | マルチAZ低頻度ストレージ-オブジェクト数           | 個   |appid、bucket |
...
| MazItFreqObjectNumber        | マルチAZ INTELLIGENT_TIERINGストレージ_高頻度階層オブジェクト数 | 個   | appid、bucket |
| MazItInfreqObjectNumber      | マルチAZ INTELLIGENT_TIERINGストレージ_低頻度階層オブジェクト数 | 個   | appid、bucket |
...
| MazStdMultipartNumber        | マルチAZ標準ストレージ-ファイルフラグメント数         | 個   | appid、bucket |
| IaMultipartNumber            | 低頻度ストレージ-ファイルフラグメント数               | 個   |appid、bucket |
| MazIaMultipartNumber         | マルチAZ低頻度ストレージ-ファイルフラグメント数         | 個   | appid、bucket |
| ItFrequentMultipartNumber    | INTELLIGENT_TIERING-高頻度ファイルフラグメント数           | 個   |appid、bucket |
| MazItFrequentMultipartNumber | マルチAZ INTELLIGENT_TIERING-高頻度ファイルフラグメント数     | 個   |appid、bucket |
...
- **アラート設定**：監視指標の閾値を設定します。監視データがアラート条件をトリガーした時点で、BCMが当該グループに速やかにアラート情報を送信することができます。詳細については、[アラートの概要](https://intl.cloud.tencent.com/document/product/248/6126)および[モニタリングアラートの設定](https://intl.cloud.tencent.com/document/product/436/39104)をご参照ください。
