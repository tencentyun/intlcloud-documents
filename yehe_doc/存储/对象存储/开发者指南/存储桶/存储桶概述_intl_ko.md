...
버킷(Bucket)은 객체의 저장 장치로써 객체를 보관하는 '컨테이너'로 이해할 수 있으며, '컨테이너'는 용량에 제한이 없습니다. 객체는 폴더나 목록의 개념 없이 수평적 구조로 버킷에 보관되므로, 사용자가 객체를 단일 버킷에 보관할지 여러 개의 버킷에 보관할지 선택할 수 있습니다.

>?하나의 루트 계정은 최대 200개의 버킷을 생성할 수 있으나, 각 버킷 내 객체 수량에는 제한이 없습니다.


## 버킷 이름 생성 규칙

버킷의 이름은 버킷 이름(BucketName)과 APPID를 하이픈 '-'로 연결한 구조로 생성됩니다. 예를 들어, `examplebucket-1250000000`에서 'examplebucket'은 사용자 정의 문자열이고, '1250000000'은 시스템 생성 문자열(APPID)입니다. API와 SDK의 예시에서 버킷의 이름은 `<BucketName-APPID>`의 포맷으로 생성됩니다.


- 버킷 이름(BucketName): 사용자가 수동으로 입력한 문자열로 이뤄지며, 이름 생성 규칙은 다음과 같습니다.
 - 알파벳 소문자와 숫자 [a~z, 0~9], 하이픈 '-' 및 그 조합만 지원합니다.
 - 사용자 정의 문자열은 1~50개의 문자를 지원합니다.
...
예를 들어, 업무가 화난 지역에 분포되어 있는 경우 광저우 리전을 선택해 버킷을 생성하면 객체의 업로드 및 다운로드 속도를 향상시킬 수 있습니다. 리전에 관한 자세한 내용은 [리전 및 액세스 도메인](https://intl.cloud.tencent.com/document/product/436/6224) 문서를 참조하십시오.
...
버킷은 기본적으로 공용 권한과 사용자 권한의 두 가지 권한을 제공합니다.

### 공용 권한
공용 권한은 '개인 읽기/쓰기', '공개 읽기와 개인 쓰기', '공개 읽기/쓰기'를 포함합니다. 액세스 권한은 객체 스토리지 콘솔에 있는 버킷의 [권한 관리]를 통해 수정할 수 있습니다. 액세스 권한에 관한 자세한 내용은 [액세스 제어 기본 개념](https://intl.cloud.tencent.com/document/product/436/30581)을 참조하십시오.
...
사용자는 Tencent Cloud 콘솔, 툴, API, SDK 등 다양한 방식으로 버킷 및 설정 속성을 관리할 수 있습니다. 예를 들어 버킷 설정은 정적 웹 사이트 호스팅 및 버킷의 액세스 권한 설정에 사용됩니다. 아래 예시는 일부 기능에 대한 설정 가이드이며, 버킷 기능 설정에 대한 자세한 내용은 [버킷 개요](https://intl.cloud.tencent.com/document/product/436/13312)를 참조하십시오. 
...
- 객체 스토리지는 수평적 구조로 객체를 저장하며 폴더 개념이 없습니다. 자세한 내용은 [객체 개요](https://intl.cloud.tencent.com/document/product/436/13324) 문서의 '폴더 및 목록' 부분을 참조하십시오.
