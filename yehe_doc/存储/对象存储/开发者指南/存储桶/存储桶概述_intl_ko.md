...
버킷의 이름은 버킷 이름(BucketName)과 APPID를 하이픈 '-'으로 연결한 구조로 생성됩니다. 예를 들어, `examplebucket-1250000000`에서 'examplebucket'은 사용자 정의 문자열이고, '1250000000'은 시스템 생성 문자열(APPID)입니다. API와 SDK의 예시에서 버킷의 이름은 `<BucketName-APPID>`의 포맷으로 생성됩니다.


- 버킷 이름(BucketName): 사용자가 수동으로 입력한 문자열로 이뤄지며, 이름 생성 규칙은 다음과 같습니다.
 - 알파벳 소문자와 숫자 [a~z, 0~9], 하이픈 '-' 및 그 조합만 지원됩니다.
 - 버킷 이름의 최대 글자 수는  **[리전 약칭](https://intl.cloud.tencent.com/document/product/436/6224)** 및 **APPID** 글자 수의 영향을 받으며, 구성된 완전한 요청 도메인 이름은 최대 60자입니다. 예를 들어 요청 도메인 이름 '123456789012345678901-1250000000.cos.ap-beijing.myqcloud.com'의 글자 수 합계는 총 60자입니다.
...
### 공용 권한
공용 권한은 '개인 읽기/쓰기', '공개 읽기와 개인 쓰기', '공개 읽기/쓰기'를 포함합니다. 액세스 권한은 객체 스토리지 콘솔에 있는 버킷의 [권한 관리]를 통해 수정할 수 있습니다. 액세스 권한에 관한 자세한 내용은 [액세스 제어 기초 개념](https://intl.cloud.tencent.com/document/product/436/30581)을 참고하십시오.
...
- 객체 스토리지는 수평적 구조로 객체를 저장하며 폴더 개념이 없습니다. 자세한 내용은 [객체 개요](https://intl.cloud.tencent.com/document/product/436/13324) 문서의 '폴더와 디렉터리' 부분을 참고하십시오.
