...
如果是第一次创建**需开启元数据加速**的存储桶，需要按照提示进行相应的**授权**操作，单击授权完成后，将自动开启 HDFS 协议，并且看到默认的存储桶挂载点信息。
...
2. 在计算集群配置 `core-site.xml`，具体配置指引请参见 [挂载 CHDFS](https://intl.cloud.tencent.com/document/product/1106/41965)。
...
- 登录 [COS 控制台](https://console.cloud.tencent.com/cos)，查看存储桶文件列表，明确文件和目录是否一致。
...
2. 在 HDFS 协议的 HDFS 鉴权模式下，选择 Ranger 鉴权模式，配置上 Ranger 相应的地址信息即可。
 ...
 - 如果遇到 Ranger 相关问题，可参见 [CHDFS 常见问题](https://intl.cloud.tencent.com/document/product/1106/41958)。

### 自建 Hadoop/CDH 等环境

1. 配置 Ranger 服务，通过 Ranger 服务以 HDFS 协议访问 COS，详情请参见 [CHDFS Ranger 权限体系解决方案](https://intl.cloud.tencent.com/document/product/1106/41973) 文档。
2. 在 HDFS 协议的 HDFS 鉴权模式下，选择 Ranger 鉴权模式，配置上 Ranger 相应的地址信息即可。
 - 如果是 CHDFS，在 core-site.xml 中新增配置项：fs.ofs.ranger.enable.flag，设置为：true。
 - 如果是 COSN，在 core-site.xml 中新增配置项：fs.cosn.credentials.provider，设置为：org.apache.hadoop.fs.auth.RangerCredentialsProvider。
 - 如果遇到 Ranger 相关问题，可参见 [CHDFS 常见问题](https://intl.cloud.tencent.com/document/product/1106/41958)。
...
3. 默认情况下，采用原生 `POSIX ACL` 方式进行鉴权，如果需要使用 `Ranger 鉴权`，可参考 [Ranger 相关原理和实践访问](https://intl.cloud.tencent.com/document/product/1106/41973)。
