...
COS 액세스 요청 시 COS 인증을 거쳐야만 리소스 작업이 가능합니다. 따라서 COS 액세스 요청은 자격 식별 가능 여부에 따라 익명 요청과 서명 요청의 2가지 유형으로 나뉩니다.

- 익명 요청: Authorization 또는 관련 매개변수를 가지고 있지 않거나, 관련 문자가 사용자의 신분 특징을 식별할 수 없는 경우, 익명 요청으로 간주되어 인증을 진행합니다.
- 서명 요청: 서명된 요청은 HTTP 헤더 또는 요청 패킷에 Authorization 필드가 포함돼야 합니다. 해당 필드의 내용은 Tencent Cloud의 보안 자격 증명 SecretID, SecretKey 및 요청의 일부 특징 값을 결합한 것이며, 암호화 알고리즘으로 생성됩니다.

COS SDK 호출을 사용할 경우 보안 자격 증명만 설정하면 요청을 발송할 수 있습니다. REST API 호출을 사용할 경우 [서명 요청](https://intl.cloud.tencent.com/document/api/436/7778) 문서를 참조해 직접 요청 서명을 계산하거나 COS 서명 툴을 통해 직접 생성해야 합니다.
...
루트 계정 로그인 후, CAM의 [Tencent Cloud API 키(https://console.cloud.tencent.com/cam/capi) 페이지를 통해 루트 계정 보안 자격 증명 SecretID와 SecretKey를 관리하고 획득할 수 있습니다. 다음은 키의 예시입니다.
...
액세스 키는 고유한 계정을 표시하는 데 사용할 수 있습니다. 키 서명을 사용하여 요청을 발송하면 Tencent Cloud는 요청 발송자의 신분을 식별하고 인증한 후, 신분, 리소스, 작업, 조건 등을 검증해 이 작업 실행의 허가 여부를 판단합니다.

>!루트 계정 키는 계정에 속한 모든 리소스의 모든 작업 권한을 가지고 있습니다. 키가 유출되면 클라우드 리소스 손실 가능성이 있으므로, 서브 계정을 생성하고 적절히 권한을 할당할 것을 강력히 권장합니다. 서브 계정의 키 생성을 요청하여 리소스에 액세스 및 관리하십시오.

### 서브 계정의 보안 자격 증명

사용자 및 클라우드 리소스를 다각도로 관리해야 하는 경우, 루트 계정에 속한 여러 개의 서브 계정을 생성한 후 리소스 권한을 여러 계정이 나누어 관리하도록 할 수 있습니다. 서브 계정 생성에 대한 자세한 내용은 CAM의 [사용자 관리](https://intl.cloud.tencent.com/document/product/598/13674) 관련 문서를 참조하십시오.
...
역할은 가상 신분으로, 영구 키를 가지고 있지 않습니다. Tencent Cloud CAM은 임시 보안 자격 증명을 생성하기 위한 STS API를 제공합니다.
사용 방법 및 예시에 대한 자세한 내용은 [역할 사용](https://intl.cloud.tencent.com/document/product/598/19419) 문서를 참조하거나 STS API 문서를 참조하여 임시 보안 자격 증명 생성 방법을 확인하십시오. 임시 보안 자격 증명에는 일반적으로 **제한 정책**(작업, 리소스, 조건) 및 **제한 시간**(시작 및 종료 유효 시간)만 포함되어 있으므로, 생성하는 임시 보안 자격 증명은 자체 배포하거나 직접 사용할 수 있습니다.
...
해당 인터페이스는 또한 `expiration` 필드를 통해 임시 보안 자격 증명의 유효 시간을 반환하며, 이는 해당 시간 내에 해당 보안 자격 증명을 사용해야만 요청을 발송할 수 있음을 의미합니다.
...
[리전 및 액세스 도메인](https://intl.cloud.tencent.com/document/product/436/6224) 문서에 REST API 호출에 사용할 수 있는 도메인 리스트가 있습니다.
...
Tencent Cloud 객체 스토리지 COS의 액세스 도메인은 스마트 DNS 리졸브를 사용하므로, 인터넷을 통해 서로 다른 ISP 환경에서 COS에 액세스할 수 있는 최적의 링크를 검증 및 지정합니다. Tencent Cloud에서 COS 액세스를 위한 서비스를 배포할 경우, 리전 내 액세스는 자동으로 내부 네트워크 주소로 지정되며, 리전 간에는 일시적으로 내부 네트워크 액세스가 지원되지 않고 외부 네트워크 주소로 리졸브 됩니다.

### 내부 네트워크 액세스 판단 방법

리전 내 Tencent Cloud 제품 액세스는 자동으로 내부 네트워크를 통해 연결되며, 발생하는 내부 네트워크 트래픽은 과금되지 않습니다. 따라서 Tencent Cloud의 여러 제품을 구매할 때 최대한 같은 리전을 선택하여 비용을 절약하는 것을 권장합니다.

>! 퍼블릭 클라우드 리전과 금융 클라우드 리전 내부 네트워크는 서로 통신되지 않습니다.

내부 네트워크의 액세스 여부 확인은 다음 방법을 참조하십시오.

Tencent CVM을 통한 COS 액세스의 경우, 내부 네트워크를 이용하여 COS에 액세스했는지 판단하려면 CVM에서 `nslookup` 명령어를 사용하여 COS 도메인을 리졸브해 볼 수 있습니다. 내부 IP가 반환되면 CVM과 COS 간에 내부 네트워크로 액세스한 것이며, 반환되지 않으면 외부 네트워크로 액세스한 것입니다.
...
여기서 `10.148.214.13`와 `10.148.214.14`의 두 IP는 내부 네트워크를 통해 COS에 액세스했음을 의미합니다.
...
>!인터넷을 통해 액세스하면 다양한 인터넷 환경의 영향을 받을 수 있습니다. 액세스가 원활하지 않을 경우 로컬 네트워크 링크를 진단하거나 현지 ISP에 문의하여 피드백을 받으십시오.
...
리전 내 Tencent Cloud VPC 네트워크를 통해 COS에 액세스하는 경우 ICMP 프로토콜의 `ping` 또는 `traceroute` 등의 툴을 사용한 연결성 테스트가 불가능할 확률이 높습니다. 기본 연결 테스트의 `telnet` 명령어를 사용하여 테스트를 진행할 것을 권장합니다.

`psping` 또는 `tcping` 등의 툴을 사용하여 직접 액세스 도메인의 80 포트에 대한 딜레이 시간 테스트를 시도해 볼 수도 있습니다. 테스트 전, `nslookup` 명령어를 통해 액세스 도메인이 내부 네트워크 주소에 정확하게 리졸브되었는지 조회 및 확인하십시오.
