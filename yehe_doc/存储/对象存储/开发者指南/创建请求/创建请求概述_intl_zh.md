...
如果您使用 COS SDK 访问，只需配置好您的安全凭证即可发起请求。使用 REST API 访问，则需要参见 [请求签名](https://intl.cloud.tencent.com/document/api/436/7778) 文档自行计算请求签名或通过COS 签名工具直接生成。
...
使用方法和示例详情请参阅 [使用角色](https://intl.cloud.tencent.com/document/product/598/19419) 文档，或参阅STS API文档查看生成临时安全凭证的方式。临时安全凭证通常只包括**有限策略**（操作、资源、条件）和**有限时间**（有效起止时间），因此生成的临时安全凭证可自行分发或直接使用。
