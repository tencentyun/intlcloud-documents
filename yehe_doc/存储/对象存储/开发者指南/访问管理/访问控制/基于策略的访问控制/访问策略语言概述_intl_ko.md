...
region |리전 정보입니다. Tencent Cloud COS가 지원하는 [사용 가능 리전](https://intl.cloud.tencent.com/document/product/436/6224)을 참조하십시오.|필수
 account |리소스 소유자의 루트 계정 정보이며, 리소스 소유자를 설명하는 두가지 방식을 지원합니다.<br>첫번째 방식은 uin 방식, 즉 루트 계정의 qq 번호이며 `uin/${OwnerUin}`로 나타납니다(예시: uin/100000000001).<br>또 다른 방식은 uid 방식, 즉 루트 계정의 APPID이며 `uid/${appid}`로 나타납니다(예시: uid/1250000000). COS의 리소스 소유자는 루트 계정의 개발사 APPID와 같은 uid 방식을 통합 사용합니다.|필수
...
루트 계정이 익명 사용자를 허용하고 액세스 출처 IP가 101.226.\*\*\*.185/101.226.\*\*\*.186인 경우 화남 리전의 버킷이 examplebucket-1250000000인 객체에 대해 GET(다운로드)과 HEAD 작업을 실행하여 인증할 필요가 없습니다. 자세한 내용은 [권한 제한 설정 관련 사례](https://intl.cloud.tencent.com/document/product/436/12514)를 참조하십시오.
