...
Tencent Cloud 계정 생성을 시작으로 COS의 권한 및 실명 인증 프로세스는 다음 5단계를 거쳐야 합니다. Tencent Cloud 계정 생성, COS 서비스 활성화, 인증 ID 생성, ID에 대한 권한 설정 그리고 액세스 및 실명 인증을 시작합니다. 
...
COS는 [버킷 정책](https://intl.cloud.tencent.com/document/product/436/18031), [사용자 정책(CAM 정책)](https://intl.cloud.tencent.com/document/product/436/45236), [버킷 ACL](https://intl.cloud.tencent.com/document/product/436/30583) 및 [객체 ACL](https://intl.cloud.tencent.com/document/product/436/30583)을 포함한 다양한 권한 설정 방식을 지원하며, 사용 시나리오에 따라 적절한 인증 방식을 선택할 수 있습니다.

#### 5단계: 액세스 및 실명 인증 시작

콘솔, API 요청, SDK 등을 통해 COS에 액세스할 수 있습니다. 보안상의 이유로 버킷은 기본적으로 개인 읽기이며 어떤 방법을 사용하든 실명 인증이 필요합니다. 콘솔의 경우 계정 비밀번호를 사용하여 로그인합니다. API 요청과 SDK 모두에 대해 사용자는 키(ak/sk)를 사용하여 실명 인증을 해야 합니다.

## COS 실명 인증 방식

![](https://qcloudimg.tencent-cloud.cn/raw/6e44d339c032f4920c5c9c6cea76c4c2.png)

기본적으로 COS 버킷은 비공개이며 COS에 액세스하려면 키(영구 키, 임시 키)를 통한 실명 인증 또는 사전 서명된 URL을 사용하여 액세스합니다. 특별한 경우에는 버킷을 개방하여 공개 읽기로 할 수도 있는데 이는 위험한 작업이며 모든 사용자는 실명 인증 없이 객체 URL을 통해 직접 객체를 다운로드할 수 있습니다.
...
영구 키는 SecretId와 SecretKey를 포함합니다. 각 루트 계정과 서브 계정은 두 쌍의 영구 키를 생성할 수 있습니다. 영구 키는 계정의 영구 ID를 나타내며 삭제하지 않으면 오랫동안 유효합니다. 자세한 내용은 [영구 키를 사용하여 COS 액세스](https://intl.cloud.tencent.com/document/product/436/45241)을 참고하십시오.
...
프로그래밍에 익숙하지 않은 사용자가 사용자 정의 유효 기간이 있는 사전 서명된 URL 가져오기에 적합합니다. 자세한 내용은 서명 툴 사용 문서를 참고하십시오.
- 서명 링크 자체 스티칭 
...
각 언어 SDK는 사전 서명된 URL의 생성 및 업로드 방법을 제공하며, 생성 방법은 [사전 서명된 승인 업로드](https://intl.cloud.tencent.com/document/product/436/14114)를 참고하십시오. 익숙한 개발 언어를 선택하십시오.
- 방법2: 서명 링크 자체 스티칭
