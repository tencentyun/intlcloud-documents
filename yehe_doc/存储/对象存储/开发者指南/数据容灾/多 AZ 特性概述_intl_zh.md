...
多 AZ（Available Zone）是指由 [腾讯云对象存储](https://www.tencentcloud.com/products/cos) 推出的多 AZ 存储架构，这一存储架构能够为用户数据提供数据中心级别的容灾能力。
...
> ?
> - 对象存储多 AZ 特性目前仅支持北京、广州、上海地域，其他公有云地域后续将陆续支持。
> - 使用对象存储多 AZ 特性，产生的存储容量费用相对较高，详情请参见 [产品定价](https://buy.intl.cloud.tencent.com/price/cos?lang=en&pg=)。
...
<tr>
<td>支持的地域</td>
<td colspan=2>请参见 <a href="https://intl.cloud.tencent.com/document/product/436/30925">存储类型概述</a> 文档</td>
</tr>
<tr>
...
1. 创建存储桶，并在**创建时**开启多 AZ 配置。存储桶的创建指引，可参见 [创建存储桶](https://intl.cloud.tencent.com/document/product/436/13309) 文档。
...
> - 如果您希望将存量数据存储于多 AZ 存储桶中，可以新建一个开启多 AZ 配置的存储桶，同时使用 COS Batch 的批量复制功能，将存量存储桶中的文件批量复制到新存储桶中。COS Batch 的使用指引，可参见 [批量处理](https://intl.cloud.tencent.com/document/product/436/32956) 操作文档。
