...
>- 数据万象在每个账户在首次产生该服务用量后，将发放一个用量为6000次，有效期为2个月的免费额度，超出用量后将正常计费。费用由数据万象收取，详细的计费说明请参见数据万象 [计费与定价](https://intl.cloud.tencent.com/document/product/1045/33431)。
>- 盲水印功能适用于所有公有云地域。
...
- 数据万象在每个账户在首次产生该服务用量后，将发放一个用量为6000次，有效期为2个月的免费额度，超出用量后将正常计费，详情请参见数据万象 [免费额度](https://intl.cloud.tencent.com/contact-sales)。
- 文字盲水印当前支持数字[0 - 9]及英文大小写[A - Z，a - z]。
- 盲水印可抵抗裁剪、涂抹、变色等多种图片盗取攻击，防盗效果与原图大小、攻击程度相关。如需详细咨询请 [联系我们](https://intl.cloud.tencent.com/contact-sales)。

## 使用方法

### 使用对象存储控制台

您可以使用对象存储控制台开启盲水印，详情请参见设置盲水印 控制台指南文档。

### 使用 REST API

您可以使用 API 添加或提取盲水印，详情请参见 [盲水印](https://intl.cloud.tencent.com/document/product/1045/43029) API 文档。
