...
호출 인터페이스를 통해 COS의 모니터링 데이터를 조회할 수 있으며, COS의 모니터링 항목은 다음과 같습니다. COS의 모니터링 인터페이스에 대한 자세한 내용은 [COS 모니터링 지표](https://intl.cloud.tencent.com/document/product/248/37269) 문서를 참조하십시오.

## 모니터링 지표

> ?COS는 범용 리전을 사용합니다. 따라서 버킷이 어떤 리전에 소속되어 있든 COS 모니터링 지표 데이터 풀링 시, Region은 항상 '광저우' 리전으로 선택하십시오.
> - [API Explorer](https://console.cloud.tencent.com/api/explorer?Product=monitor&Version=2018-07-24&Action=DescribeBaseMetrics)를 사용해 데이터 풀링 시, Region 필드는 항상 '화남리전(광저우)'으로 선택하십시오.
...
| -------------------- | ------------------------ | ------------------------------------------------------------ | ---- | ------------- |
| StdReadRequests      | 표준 스토리지 읽기 요청           | 표준 스토리지 유형의 읽기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
| StdWriteRequests     | 표준 스토리지 쓰기 요청           | 표준 스토리지 유형의 쓰기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨  | 회   | appid, bucket |
| MazStdReadRequests   | 다중AZ 표준 스토리지 읽기 요청     | 다중AZ 표준 스토리지 유형의 읽기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
| MazStdWriteRequests  | 다중AZ 표준 스토리지 쓰기 요청     | 다중AZ 표준 스토리지 유형의 쓰기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨  | 회   | appid, bucket |
| IaReadRequests       | 표준IA 스토리지 읽기 요청           | 표준IA 스토리지 유형의 읽기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
| IaWriteRequests      | 표준IA 스토리지 쓰기 요청           | 표준IA 스토리지 쓰기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
| MazIaReadRequests    | 다중AZ 표준IA 스토리지 읽기 요청     | 다중AZ 표준IA 스토리지 유형의 읽기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
| MazIaWriteRequests   | 다중AZ 표준IA 스토리지 쓰기 요청     | 다중AZ 표준IA 스토리지 유형의 쓰기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
...
| MazItReadRequests    | 다중AZ INTELLIGENT TIERING 읽기 요청       | 다중AZ INTELLIGENT TIERING 유형의 읽기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
| MazItWriteRequests   | 다중AZ INTELLIGENT TIERING 쓰기 요청       | 다중AZ INTELLIGENT TIERING 유형의 쓰기 요청 횟수. 요청 횟수는 요청 명령 발송 횟수에 따라 계산됨 | 회   | appid, bucket |
...
| ---------------------------- | --------------------------------- | ---- | ------------- |
| StdStorage                   | 표준 스토리지 - 스토리지 용량                 | MB   | appid, bucket |
| MazStdStorage                | 다중AZ 표준 스토리지 - 스토리지 용량           | MB   | appid, bucket |
| SiaStorage                   | 표준IA 스토리지 - 스토리지 용량                 | MB   | appid, bucket |
| MazIaStorage                 | 다중AZ 표준IA 스토리지 - 스토리지 용량           | MB   | appid, bucket |
...
| MazItFreqStorage             | 다중AZ INTELLIGENT TIERING - 고빈도 레이어 스토리지 용량 | MB   | appid, bucket |
| MazItInfreqStorage           | 다중AZ INTELLIGENT TIERING - 저빈도 레이어 스토리지 용량 | MB   | appid, bucket |
| ArcStorage                   | CAS - 스토리지 용량                 | MB   | appid, bucket |
| DeepArcStorage               | DEEP ARCHIVE - 스토리지 용량             | MB   | appid, bucket |
| StdObjectNumber              | 표준 스토리지 - 객체 수                 | 개   | appid, bucket |
| MazStdObjectNumber           | 다중AZ 표준 스토리지 - 객체 수           | 개   | appid, bucket |
| IaObjectNumber               | 표준IA 스토리지 - 객체 수                 | 개   | appid, bucket |
| MazIaObjectNumber            | 다중AZ 표준IA 스토리지 - 객체 수           | 개   | appid, bucket |
...
| MazItFreqObjectNumber        | 다중AZ INTELLIGENT TIERING_고빈도 레이어 객체 수 | 개   | appid, bucket |
| MazItInfreqObjectNumber      | 다중AZ INTELLIGENT TIERING_저빈도 레이어 객체 수 | 개   | appid, bucket |
| ArcObjectNumber              | CAS 객체 수                  | 개   | appid, bucket |
| DeepArcObjectNumber          | DEEP ARCHIVE 객체 수              | 개   | appid, bucket |
| StdMultipartNumber           | 표준 스토리지 - 파일 조각 수               | 개   | appid, bucket |
| MazStdMultipartNumber        | 다중AZ 표준 스토리지 - 파일 조각 수         | 개   | appid, bucket |
| IaMultipartNumber            | 표준IA 스토리지 - 파일 조각 수               | 개   | appid, bucket |
| MazIaMultipartNumber         | 다중AZ 표준IA 스토리지 - 파일 조각 수         | 개   | appid, bucket |
| ItFrequentMultipartNumber    | INTELLIGENT TIERING - 고빈도 파일 조각 수           | 개   | appid, bucket |
| MazItFrequentMultipartNumber | 다중AZ INTELLIGENT TIERING - 고빈도 파일 조각 수     | 개   | appid, bucket |
| ArcMultipartNumber           | CAS - 파일 조각 수               | 개   | appid, bucket |
...
> ?
> 1. 3xx, 4xx, 5xx 상태 코드에 관한 자세한 내용은 [에러 코드표](https://intl.cloud.tencent.com/document/product/436/7730)를 참조하십시오.
...
| 지표 이름(영어)   | 지표 이름(한글)     | 지표의 의미                                                     | 단위 | 차원          |
| ------------ | -------------- | ------------------------------------------------------------ | ---- | ------------- |
| StdRetrieval | 표준 데이터 읽기량 | 표준 데이터를 읽을 때 발생하는 트래픽. 표준 스토리지의 공인 네트워크 다운스트림 트래픽, 내부 네트워크 다운스트림 트래픽, CDN Origin-pull 트래픽의 총합 | B    | appid, bucket |
| IaRetrieval  | 표준IA 데이터 읽기량 | 표준IA 데이터를 읽을 때 발생하는 트래픽. 표준IA 스토리지의 공인 네트워크 다운스트림 트래픽, 내부 네트워크 다운스트림 트래픽, CDN Origin-pull 트래픽의 총합 | B    | appid, bucket |
...
- **알람 설정**: 모니터링 지표 임계값을 설정하고, 모니터링 데이터가 알람 조건을 충족하면 클라우드 모니터링이 즉시 알람 정보를 관련 그룹에 전송합니다. 자세한 내용은 [알람 개요](https://intl.cloud.tencent.com/document/product/248/6126) 및 [모니터링 알람 설정](https://intl.cloud.tencent.com/document/product/436/39104)을 참조하십시오.
