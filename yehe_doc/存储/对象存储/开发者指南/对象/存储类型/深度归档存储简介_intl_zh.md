...
1. 使用控制台上传
   在 [COS 控制台](https://console.cloud.tencent.com/cos5) 进入存储桶【文件列表】，单击【上传文件】，然后选择本地文件进行上传，在设置对象属性中将存储类型选择为【归档存储存储】，详情请参见 [上传对象](https://intl.cloud.tencent.com/document/product/436/13321)。
...
- **请求 QPS 限制**：最多为100次。
- **地域限制**：仅支持北京、上海、广州公有云地域。
- **存储桶限制**：深度归档存储类型不支持开启了多 AZ 配置的存储桶。
- **操作限制**：无法通过生命周期、COPY 操作转换为其他存储类型，不支持追加上传对象为深度归档存储类型。
