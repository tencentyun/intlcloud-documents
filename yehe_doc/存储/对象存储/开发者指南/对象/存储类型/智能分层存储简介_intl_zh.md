...
> - 智能分层存储类型为独立的存储类型，使用时将产生智能分层存储容量费用，更多计费信息请参见 [产品定价](https://intl.cloud.tencent.com/pricing/cos)。
> 
...
>?
>- 标准存储、低频存储容量费用在不同公有云地域有不同的定价，具体定价可参见 [产品定价](https://intl.cloud.tencent.com/pricing/cos)。
...
>
2. 智能分层对象监控费用按存储的文件数来计算，每月每万个监控对象会收取0.025美元。
