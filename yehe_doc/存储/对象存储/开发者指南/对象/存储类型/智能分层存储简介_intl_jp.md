...
- **安定性・永続性**：INTELLIGENT_TIERINGストレージでは、標準ストレージと同等の低レイテンシー、高スループットを体感していただくことができます。また、INTELLIGENT_TIERINGストレージはイレイジャーコーディングによる冗長ストレージ方式を採用し、99.999999999%（イレブンナイン）のデータ信頼性を実現しています。また、データの分割ストレージ、読み取り/書き込み同時実行により、業務可用性は99.95%に達しています。マルチAZアーキテクチャにはINTELLIGENT_TIERINGの特性が同期されているため、設計上のデータ信頼性は99.9999999999%（トゥエルブナイン）に、設計上の業務可用性は99.995%に達します。
