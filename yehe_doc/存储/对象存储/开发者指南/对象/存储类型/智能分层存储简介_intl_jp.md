...
>!
> - INTELLIGENT_TIERINGストレージタイプは現在、北京、南京、上海、広州、成都、重慶、東京、シンガポールリージョンでのみサポートしています。
> - INTELLIGENT_TIERINGストレージタイプは独立したストレージタイプであり、ご利用の際はINTELLIGENT_TIERINGストレージ容量料金が発生します。課金に関するより詳しい情報については、[製品価格](https://intl.cloud.tencent.com/pricing/cos)をご参照ください。
> 
...
- **安定性・永続性**：INTELLIGENT_TIERINGストレージでは、標準ストレージと同等の低レイテンシー、高スループットを体感していただくことができます。また、INTELLIGENT_TIERINGストレージはイレージャーコーディングによる冗長ストレージ方式を採用し、99.999999999%（イレブンナイン）のデータ信頼性を実現しています。また、データの分割ストレージ、読み取り/書き込み同時実行により、業務可用性は99.95%に達しています。
...
データをINTELLIGENT_TIERINGストレージタイプとしてCOSに保存するには、初めにバケットでINTELLIGENT_TIERING設定を有効にする必要があります。有効にした後、オブジェクトをアップロードする際に**ストレージタイプ**をINTELLIGENT_TIERINGストレージタイプに指定すれば利用できます。
...
1. 初めに、REST APIを使用してバケットのINTELLIGENT_TIERINGストレージを有効にします。次のAPIドキュメントをご参照ください。
...
現在COSがリリースしているSDKはすべて、INTELLIGENT_TIERINGストレージタイプの使用をサポートしています。具体的な方法は、ファイルをアップロードする際にStorageClassパラメータをINTELLIGENT_TIERINGに設定し、INTELLIGENT_TIERINGストレージへの直接転送を実現するものです。詳細については、[SDKの概要](https://intl.cloud.tencent.com/document/product/436/6474)をご参照ください。
...
- **設定の制限**：最初に設定した後は変更できません。変更が必要な場合は、[お問い合わせ](https://intl.cloud.tencent.com/contact-sales)ください。
...
>?
>- 標準ストレージ、低頻度ストレージ容量料金はパブリッククラウドリージョンごとに金額が異なります。具体的な価格については、[製品価格](https://intl.cloud.tencent.com/pricing/cos)をご参照ください。
...
>
2. INTELLIGENT_TIERINGオブジェクト監視料金は保存されているファイル数に基づいて計算されます。1か月のオブジェクト1万個あたりの監視料金は0.025米ドルです。
...
>! 64KB未満のINTELLIGENT_TIERINGタイプのファイルは常に標準層に保存されます。このため、64KB未満のファイルについては必要に応じ、標準/低頻度/アーカイブ/ディープアーカイブなどのストレージタイプに直接アップロードすることをお勧めします。こうすることでコストが削減できます。
>
