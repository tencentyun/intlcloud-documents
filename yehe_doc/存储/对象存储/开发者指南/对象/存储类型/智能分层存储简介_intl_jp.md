...
- **最短保存期間の制限**：INTELLIGENT_TIERINGストレージタイプのオブジェクトの最短保存期間は30日間です。30日未満の場合は30日として課金されます。
...
>! INTELLIGENT_TIERINGには64KBの最小ストレージユニット制限が設けられており、64KB未満のファイルは常に標準層に保存されます。このため、64KB未満のファイルについては必要に応じ、標準/低頻度/アーカイブ/ディープアーカイブなどのストレージタイプに直接アップロードすることをお勧めします。こうすることでコストが削減できます。
>
