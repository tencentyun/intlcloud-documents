...
1. Uploading through the console
   In the [COS console](https://console.cloud.tencent.com/cos5), go to **File List**, and click **Upload Files**. Select a local file to upload, and set the object’s storage class to **DEEP ARCHIVE**. For more information, see [Uploading Objects](https://intl.cloud.tencent.com/document/product/436/13321).
...
- **QPS**: has an upper limit of 100.
- **Regions**: only Beijing, Shanghai and Guangzhou Public Cloud regions.
- **Buckets**: DEEP ARCHIVE cannot be used for MAZ-enabled buckets.
- **Operations**: an object in DEEP ARCHIVE cannot be transitioned to another storage class through lifecycle or any COPY operations. It is also not allowed to upload objects directly to DEEP ARCHIVE using the APPEND Object API.
