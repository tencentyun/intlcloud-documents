...
>オブジェクトタグ機能は課金項目です。価格の詳細については、[製品価格](https://intl.cloud.tencent.com/document/product/436/6239)のドキュメントをご参照ください。

## 利用方法

### COSコンソールの使用

CDNコンソールによってオブジェクトタグを管理できます。詳細については、[オブジェクトタグの設定](https://intl.cloud.tencent.com/document/product/436/35664)をご参照ください。
...
- タグキーは`qcs:`、`project`、`プロジェクト`などで始めることはできません。これらはシステム予約済みタグキーであり、ユーザーによる作成は禁止されています。
...
- UTF-8形式で表される文字、スペース、数字[0-9]および特殊文字`+ - = ._ : / @`をサポートしています。
- タグ値の長さの範囲は1～127文字とします（UTF-8形式を使用）。
