...
```shell
타깃 버킷/경로 접두사{YYYY}/{MM}/{DD}/{time}_{random}_{index}.gz
```

로그는 5분마다 생성되어 한 줄씩 기록됩니다. 모든 기록은 여러 개의 필드를 포함하고 각 필드는 빈칸으로 분할됩니다. 주의할 점은 단일 로그 파일의 최대 크기가 256MB이며, 5분 이내 발생된 로그양이 256MB을 초과할 경우 로그는 여러 개의 로그 파일로 분할된다는 것입니다. 현재 지원하는 로그 필드는 다음과 같습니다.
