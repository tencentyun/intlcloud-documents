...
低頻度ストレージ、INTELLIGENT_TIERINGストレージ、アーカイブストレージおよびディープアーカイブストレージにはそれぞれ、オブジェクトの最小占有容量の制限が設けられています。例えば、低頻度ストレージに64KB未満のファイルをアップロードした場合は、64KBとして計算されます。Tencent Cloud COSはファイルのサイズをチェックせず、指定されたルールに従って無条件にオブジェクトの切り替え操作を実行します。 

>? ライフサイクルは0バイトのオブジェクトに対しては切り替え操作を実行しません。
