...
- #### 最终一致性
如果对同一组的对象配置了多条规则，且存在冲突性情况（不含过期删除配置），对象存储会优先执行沉降至最冷存储类型的规则。例如，规则 A 配置了90天后**沉降到低频存储**，规则 B 配置了90天后**沉降到归档存储**，且上述规则都命中了同一对象 test.txt，则优先执行规则 B。
...
- #### 最终一致性
如果对同一组的对象配置了多条规则，且存在冲突性情况，对象存储会以最短过期时间为准执行，且**过期删除的执行效力大于转换存储类型**。例如，规则 C 配置了180天后**沉降到低频存储**，规则 D 配置了180天后**删除对象**，且上述规则都命中了同一对象 test.txt，则优先执行规则 D。
