...
핫 데이터는 업로드 후 보통 짧은 시간 내 대량 액세스로 이슈가 증가하는데, 일정 시간이 지나면 이슈는 점차 낮아지거나 실시간 액세스가 필요 없게 됩니다. 라이프사이클 규칙을 통해 30일 전의 데이터를 표준IA 스토리지로 전환하고 60일 전의 데이터를 아카이브로 전환할 수 있습니다. 이 과정을 데이터 전환이라고 합니다.
...
- 데이터 전환: 생성된 객체를 지정 시간 후 표준IA 스토리지 유형, 인텔리전트 티어링, 아카이브 및 딥 아카이브 유형으로 전환할 수 있습니다.
...
퍼블릭 클라우드 리전이 지원되며 금융 클라우드 리전은 표준IA 스토리지 유형으로의 데이터 전환만 지원합니다.

#### 단방향 원칙

데이터 전환은 단방향 즉, 표준 스토리지>표준IA 스토리지>인텔리전트 티어링>아카이브>딥 아카이브만 허용하고, 등급 뛰어넘기 전환(예시: 표준 스토리지>표준IA 스토리지)도 지원하지만 역방향은 지원하지 않습니다. [PUT Object - Copy](https://intl.cloud.tencent.com/document/product/436/10881)(아카이브/딥 아카이브가 아닌 유형 대상) 또는 [POST Object restore](https://intl.cloud.tencent.com/document/product/436/12633)(아카이브/딥 아카이브 유형에만 적용됨) 호출을 통해서만 콜드 스토리지 유형의 데이터를 핫 스토리지 유형으로 복구할 수 있습니다.

#### 최종 일관성

같은 그룹에 속한 객체에 여러 규칙을 설정하고, 충돌적 상황(만료 삭제 설정 불포함)이 존재할 경우, COS는 우선적으로 가장 콜드한 스토리지 유형의 규칙으로 전환합니다. 예를 들어, 같은 그룹에 **표준IA 스토리지로 전환** 및 **아카이브로 전환** 규칙을 설정하면 우선적으로 **아카이브로 전환** 규칙을 수행합니다.
...
표준IA 스토리지 및 인텔리전트 티어링 유형은 최소 30일, 아카이브 유형은 최소 90일, 딥 아카이브 유형은 최소 180일 저장하여, 데이터 전환 또는 삭제 수행 시 별도의 스토리지 요금이 발생하지 않도록 주의하십시오. Tencent Cloud COS는 30/90/180일 이하의 라이프사이클 설정은 검사하지 않으므로 정확한 설정은 귀하의 요구에 따라 수행됩니다.

예를 들어, 아카이브의 객체가 30일 미만일 때 전환되면, 객체는 당일 아카이브 유형 요금을 발생시키는 동시에 표준IA 스토리지 유형은 30일째까지만 과금하게 됩니다. 아카이브 객체가 90일 미만일 때 만료 삭제되면, 객체는 계속 아카이브 유형으로 90일째까지 과금됩니다. 데이터가 딥 아카이브로 전환되는 것과 같은 이치입니다.

#### 크기 제한

표준IA 스토리지, 인텔리전트 티어링, 아카이브 및 딥 아카이브 유형별 객체 최소 크기 제한이 있습니다. 예를 들어 64KB보다 작은 객체가 표준IA 스토리지에 업로드되면 64KB로 계산됩니다. 비용 절감을 위해 라이프사이클은 64KB 미만 객체에 대해 스토리지 유형 전환 작업을 수행하지 않습니다. 

>? 라이프사이클은 64KB 미만 객체에 대해 스토리지 유형 전환 작업을 수행하지 않습니다.
>
