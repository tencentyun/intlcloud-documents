...
- 접두사: 지정된 접두사가 있는 객체에 규칙이 적용됩니다.
- 버전 관리: 최신 버전이 아닌 객체에 규칙이 적용됩니다.
- 마커 삭제: 객체의 모든 기록 버전이 지워지면 삭제 마커를 제거하도록 지정할 수 있습니다.
- 불완전한 멀티파트 업로드: 불완전한 멀티파트 업로드 작업을 처리합니다.

### 시간

- 일 수 기준: 객체의 마지막 수정 날짜를 기준으로 객체에 정의된 작업을 수행할 시기를 지정할 수 있습니다.
- 특정 날짜 기준: 정의된 작업을 수행할 날짜를 지정할 수 있습니다.
...
- #### 최종 일관성
동일한 객체에 대해 여러 규칙이 구성되고 서로 충돌하는 경우(만료 시 삭제 구성 제외) COS는 객체를 coldest 스토리지 클래스로 전환하는 규칙을 실행합니다. 예를 들어 규칙 A와 B가 각각 90일 후에 **객체를 STANDARD_IA 및 ARCHIVE로 전환**하도록 구성되어 있고 둘 다 동일한 객체 test.txt에 도달하면 규칙 B가 실행됩니다.
...
- #### 최종 일관성
동일한 객체에 여러 규칙이 구성되어 있고 서로 충돌하는 경우 COS는 만료 시간이 가장 빠른 규칙을 실행하며 **만료 시 삭제 구성이 객체 전환 구성보다 우선합니다**. 예를 들어, 규칙 C와 D가 각각 180일 후에 **객체를 STANDARD_IA로 전환**하고 **객체를 삭제**하도록 구성되어 있고 둘 다 동일한 객체 test.txt에 도달하면 규칙 D가 실행됩니다.
