...
| Freeze             | Request.Conf | This field can be used to set automatic freezing for images based on the moderation score. It takes effect only if the image moderated in `input` is an `object`. | Container | No |
...
| :----------------- | :----- | :----------------------------------------------------------- |
| Code              | String    | Error code, which will be returned only in case of a failure. For more information, see [Error Codes](https://www.tencentcloud.com/document/product/1045/33700). |
...
| ForbidState  | Integer   | If you set automatic freezing during request, this field indicates the status of the image. `0`: not frozen; `1`: frozen. |
