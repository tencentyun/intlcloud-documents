...
- 调用接口会产生图片审核费用 和 [COS 读请求费用](https://intl.cloud.tencent.com/document/product/436/40100)。
...
| large-image-detect | 对于超过大小限制的图片是否进行压缩后再审核，取值为： 0（不压缩），1（压缩）。默认为0。<br/>注：压缩最大支持32MB的图片，且会收取图片压缩费用。对于 GIF 等动态图过大时，压缩时间较长，可能会导致审核超时失败。 | Int    | 否       |
