...
This API (GET Bucket Referer) is used to read the allowlist or the blocklist of Bucket Referer.
...
| DomainList | RefererConfiguration | Sets a list of domain names used in the blocklist/allowlist. You can configure multiple domain names by using prefix match. Domain names and IPs with ports are supported. Wildcard "*" is supported for second-level and third-level domain names. | Container | Yes |
| Domain | DomainList | Sets a single domain name used in the blocklist/allowlist, such as `www.qq.com/example`, `192.168.1.2:8080`, and `*.qq.com`. | String | Yes |
