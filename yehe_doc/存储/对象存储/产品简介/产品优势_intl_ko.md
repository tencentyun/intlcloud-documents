...
Tencent Cloud COS는 데이터간 멀티 구성 및 멀티 디바이스 잔여 스토리지를 제공하여 사용자 데이터에 대한 원격 재해 복구 및 리소스 격리 기능을 제공합니다. 객체별로 최대 99.9999999999%의 데이터 지속성을 제공하여 다른 스토리지보다 뛰어난 내구성을 자랑합니다.
...
COS는 CDN 가속, CI 이미지 처리, 멀티미디어 트랜스 코딩, 파일 미리보기 등의 모듈을 포함한 기타 Tencent Cloud 제품과 연동이 가능하며, ‘저장+처리’ 통합 솔루션을 제공합니다. 또한 빅 데이터를 연산하는 데이터 풀로 활용 가능해 빅 데이터 분석과 연산을 위한 데이터 소스를 제공하고, SCF 서비스와 결합하여 이벤트 알림 및 자동 프로세스를 구현할 수 있습니다. 
