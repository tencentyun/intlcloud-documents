...
根据访问频度的高低，COS 提供四种对象的存储级别：标准存储（多 AZ）、标准存储、低频存储、归档存储。

>默认存储类型为标准存储。

#### 标准存储（多 AZ）

**适用场景**：手机图片、重要文件、商业数据、敏感信息

- 标准存储（多 AZ）为用户提供了高数据持久性，高可用性、高性能的对象存储服务。
- 标准存储（多 AZ）将数据存储于同一城市的不同机房，保障用户业务稳定性不受单机房故障影响。


#### 标准存储
**适用场景**：热点视频、社交图片、移动应用、游戏程序、静态网站。
...
对比项	| 标准存储（多 AZ）  |标准存储|	低频存储|	归档存储
---|---|---|----|----
数据持久性|  99.9999999999%|   	99.999999999%	|99.999999999%|	99.999999999%
服务可用性	|  99.99% |   99.95%	|99.9%	|99.9%
响应	|  毫秒级	|   毫秒级	|毫秒级	|需提前申请恢复
对象最小计量大小|	 128KB  |   按对象实际大小计算|	64KB|	64KB
最短计费时间	|   不限制	| 不限制	|30天	|90天
支持地域|	当前仅支持广州、北京地域 |  全部地域	|全部地域|	只适用于公有云地域（暂不支持南京地域）
存储费用	|较高| 标准	|较低	|极低
数据取回费用	|不收取数据取回费用  |不收取数据取回费用|	较低	|较高
