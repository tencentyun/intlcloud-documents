...
## 高级接口（推荐）
该小节主要讲述由 COS 提供的封装了上传和复制操作的高级接口，用户只需要设置相应的参数，该接口内部会根据文件大小决定是进行简单上传（复制）还是分片上传（复制），使用接口前请确认已完成了 [快速入门](https://intl.cloud.tencent.com/document/product/436/8629) 中指引的初始化步骤。
