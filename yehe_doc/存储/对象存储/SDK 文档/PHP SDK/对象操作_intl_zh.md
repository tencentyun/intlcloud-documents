...
| StorageClass         | String      | 文件的存储类型，例如 STANDARD、STANDARD_IA、ARCHIVE，默认值：STANDARD。更多存储类型，请参见 [存储类型概述](https://intl.cloud.tencent.com/document/product/436/30925)   | 否       |
...
| StorageClass         | String | 文件的存储类型，例如 STANDARD、STANDARD_IA、ARCHIVE。更多存储类型请参见 [存储类型概述](https://intl.cloud.tencent.com/document/product/436/30925) | 无     |
...
| Metadata             | Array       | 用户自定义的文件元信息                                       | 无     |
| StorageClass         | String      | 文件的存储类型，例如 STANDARD 、 STANDARD_IA 、 ARCHIVE，默认值：STANDARD。更多存储类型请参见 [存储类型概述](https://intl.cloud.tencent.com/document/product/436/30925) | 无     |
...
该小节主要讲述由 COS 提供的封装了上传和复制操作的高级接口，用户只需要设置相应的参数，该接口内部会根据文件大小决定是进行简单上传（复制）还是分块上传（复制），使用接口前请确认已完成了 [快速入门](https://intl.cloud.tencent.com/document/product/436/12266) 中指引的初始化步骤。
...
| -------- | ------ | ---------------------------------- | -------- |
| Progress         | Function      | 进度条回调，参数为总大小($totolSize), 已上传大小($uploadedSize) | 否       |
| PartSize         | Int      | 最小分块文件大小，默认为50M | 否       |
...
| options 参数 | 类型   | 描述                               | 是否必填 |
| -------- | ------ | ---------------------------------- | -------- |
| Progress         | Function      | 进度条回调，参数为总大小($totolSize)，已上传大小($downloadedSize) | 否       |
| PartSize         | Int      | 最小分块文件大小，默认为50M | 否       |
| Concurrency         | Int      | 并发度，默认为10 | 否       |
| ResumableDownload         | Bool      | 是否开启断点续传，默认为False | 否       |
