...
생성한 사전 서명된 URL은 프로토콜명(HTTP 혹은 HTTPS)을 포함하며 해당 프로토콜명은 사전 서명을 요청한 COS(Cloud Object Storage) 클라이언트가 설정한 프로토콜과 동일해야 합니다.
...
| bucketName      | 구조 함수 또는 set 메소드 | 버킷 이름. 버킷의 이름 생성 포맷은 BucketName-APPID이며, 자세한 내용은 [이름 생성 규칙](https://intl.cloud.tencent.com/document/product/436/13312)을 참고하십시오. | String |
...
| expiration      | set 메소드            | 서명 만료일, 서명 만료 시간을 설정할 수 있으며 기본값은 1시간입니다.              | Date                    |
| contentType     | set 메소드            | 서명 요청의 Content-Type                                | String                  |
| contentMd5      | set 메소드            | 서명 요청의 Content-Md5                                 | String                  |
| responseHeaders | set 메소드            | 서명 다운로드 요청 중 덮어쓰고 반환활 HTTP 헤더                      | ResponseHeaderOverrides |
...
// 이 요청의 헤더를 입력합니다. Host 헤더는 자동으로 완성되며 입력할 필요가 없습니다.
...
영구 키를 사용하여 만료되지 않으며 서명이 있는 다운로드 URL을 생성하는 예시 코드는 다음과 같습니다.
...
GeneratePresignedUrlRequest req =
        new GeneratePresignedUrlRequest(bucketName, key, HttpMethodName.GET);
// 서명 만료 시간을 매우 먼 시간으로 설정합니다(예: 이 경우 3000년 12월 31일).
...
// clientConfig에 region, https(기본값: http), 타임아웃 시간, 프록시 등을 설정하는 set 메소드가 포함되어 있습니다. 사용 시 소스 코드 또는 FAQ의 Java SDK 부분을 참고 바랍니다.
Region region = new Region("COS_REGION");
ClientConfig clientConfig = new ClientConfig(region);
// https 프로토콜을 사용하는 URL을 생성할 경우, 이 행을 설정하길 권장합니다.
// clientConfig.setHttpProtocol(HttpProtocol.https);
...
// 이 요청의 header를 입력합니다. Host 헤더는 자동으로 완성되며 기타 헤더만 입력합니다.
...
COSSigner 클래스는 COS 서명을 구성하는 메소드를 제공합니다. 이는 모바일 SDK에 배포하는 데 사용되며, 파일을 업로드 및 다운로드할 수 있습니다. 서명의 경로는 배포 후 작업할 key와 매칭되어야 합니다.
...
| headerMap    | 서명할 HTTP Header map. 전달된 Host, Content-Type, Content-Md5 및 x로 시작하는 header만 서명을 진행합니다. | Map            |
