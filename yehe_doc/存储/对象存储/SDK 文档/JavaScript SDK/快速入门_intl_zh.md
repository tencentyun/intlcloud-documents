...
由于固定密钥放在前端会有安全风险，正式部署时我们推荐使用临时密钥的方式，实现过程为：前端首先请求服务端，服务端使用固定密钥调用 STS 服务申请临时密钥（具体内容请参见 [临时密钥生成和使用指引](https://intl.cloud.tencent.com/document/product/436/14048) 文档），然后返回临时密钥到前端使用。
...
// 关于地域的详情见 https://intl.cloud.tencent.com/document/product/436/6224
...
强烈推荐使用高级上传接口uploadFile，自动针对小文件使用简单上传，大文件使用分块上传，性能更好。详情请参见 [高级上传](https://intl.cloud.tencent.com/document/product/436/43861) 文档。
若使用临时密钥方式，需同时授权`简单上传对象`和`分块上传`的权限。请参考[授权指引](https://intl.cloud.tencent.com/document/product/436/30580)。
...
cos.getObjectUrl({
    ...
    if (err) return console.log(err);
     /* 通过指定response-content-disposition=attachment实现强制下载 */
