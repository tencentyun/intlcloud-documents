...
JavaScript SDK는 객체 URL과 사전 서명된 URL 요청을 가져오는 인터페이스를 제공합니다.
...
> ?서명 과정이 비동기 획득인 경우, callback을 통해 서명이 있는 Url을 획득해야 합니다.
...
cos.getObjectUrl({
    ...
    var downloadUrl = data.Url + (data.Url.indexOf('?') > -1 ? '&' : '?') + 'response-content-disposition=attachment'; // 강제 다운로드 매개변수 추가
    window.open(downloadUrl); // 새 창에서 url을 엽니다. 현재 창에서 열어야 하는 경우, 숨겨진 iframe을 사용해 다운로드하거나 a 태그 download 속성을 사용해 다운로드를 보조합니다.
});
```
...
cos.getObjectUrl({
    ...
    // Url 획득 후, 프런트 엔드는 이와 같이 ajax 업로드가 가능합니다.
    var xhr = new XMLHttpRequest();
    xhr.open('PUT', data.Url, true);
    xhr.onload = function (e) {
        console.log('업로드 성공', xhr.status, xhr.statusText);
    };
    xhr.onerror = function (e) {
        console.log('업로드 오류', xhr.status, xhr.statusText);
    };
    xhr.send(file); // file은 업로드할 파일 객체입니다.
});
```
...
| Method  | 작업 방법. 예시: GET, POST, DELETE, HEAD 등 HTTP 방법. 기본값: GET | String  | 아니요   |
...
| 매개변수 이름 | 매개변수 설명                                                     | 유형   |
| ------ | ------------------------------------------------------------ | ------ |
| err    | 요청 과정에서 오류 발생 시 반환되는 객체에는 네트워크 오류와 작업 오류가 포함됩니다. 요청 완료 시 빈칸으로 표시되며, 자세한 내용은 [에러 코드](https://intl.cloud.tencent.com/document/product/436/7730) 문서를 참조하십시오. | Object |
