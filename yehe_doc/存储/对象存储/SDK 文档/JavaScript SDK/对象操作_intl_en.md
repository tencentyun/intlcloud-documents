...
| - - ETag | MD5 checksum of the file,<br>such as `"22ca88419e2ed4721c23807c678adbe4c08a7880"`. **Note that double quotation marks are required at the beginning and the end** | String |
...
| - - StorageClass |Storage class of the object. Enumerated values: `STANDARD`, `STANDARD_IA` and `ARCHIVE`. For more information, please see [Storage Class](https://intl.cloud.tencent.com/document/product/436/30925) | String |
...
| - x-cos-storage-class | Storage class of the object; enumerated values: `STANDARD` (default), `STANDARD_IA`, `ARCHIVE`. `STANDARD` is not display if returned | String |
...
> !This API is used to read object content. To download a file using your browser, you should first get a download URL through the `cos.getObjectUrl` method. For more information, please see [Pre-signed URL](https://intl.cloud.tencent.com/document/product/436/31540).
...
| - OptionsForbidden | Indicates whether the `OPTIONS` request is forbidden. It is `true` if HTTP status code 403 was returned | Boolean |
...
| - - StorageClass | Storage class of the part; enumerated values: `STANDARD`, `STANDARD_IA`, `ARCHIVE` | String | No |
...
This API is used to upload a part after a multipart upload was initialized. It can upload up to 10,000 parts of 1 MB to 5 GB for a multiple upload.
