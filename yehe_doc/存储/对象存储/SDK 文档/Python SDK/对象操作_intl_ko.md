...
| Prefix   |  기본값 null. 객체 키를 필터링해 prefix로 시작하는 객체와 매칭  | String  |  아니요|
| Delimiter   |   기본값 null. 세퍼레이터 설정(예: `/`을 설정해 폴더 시뮬레이션)  | String|  아니요|
| Marker   |  기본적으로 UTF-8 이진법 순서로 나열. 반환된 객체 리스트의 시작 위치 표시  | String  |  아니요|
| MaxKeys   | 반환되는 최대 객체 수. 기본값은 최대 1000개  | Int  |  아니요|
| EncodingType   |   기본적으로 인코딩하지 않으며, 반환값의 인코딩 방식을 규정. 옵션값: url  | String  | 아니요|

#### 반환 결과 설명

가져온 객체의 메타 정보(dict 포맷):
...
| Delimiter   |   기본값 null. 세퍼레이터 설정(예: `/`을 설정해 폴더 시뮬레이션)  | String|
...
| Prefix   |  기본값 null. 객체 키를 필터링해 prefix로 시작하는 객체와 매칭  | String  |  아니요|
| Delimiter   |   기본값 null. 세퍼레이터 설정(예: `/`을 설정해 폴더 시뮬레이션)  | String|  아니요|
| KeyMarker   |  기본적으로 UTF-8 이진법 순서로 나열. 반환된 객체 리스트의 key 시작 위치 표시  | String  |  아니요|
| VersionIdMarker|  기본적으로 UTF-8 이진법 순서로 나열. 반환된 객체 리스트의 VersionId 시작 위치 표시  | String  |  아니요|
| MaxKeys   | 반환되는 최대 객체 수. 기본값은 최대 1000개  | Int  |  아니요|
| EncodingType   |   기본적으로 인코딩하지 않으며, 반환값의 인코딩 방식을 규정. 옵션값: url  | String  | 아니요|
...
# secret_id, secret_key, region을 포함한 사용자 속성을 설정합니다.
...
# 디렉터리 생성
# COS에서 디렉터리는 경로가 ‘/’로 끝나는 특수 object입니다. Put Object 인터페이스를 호출하면 됩니다.
try:
...
| GrantFullControl |권한을 부여받은 계정에 모든 권한 부여. 포맷: `id="OwnerUin"`|String|아니요|
|GrantRead |권한을 부여받은 계정에 읽기 권한 부여. 포맷: `id="OwnerUin"`  |String|아니요|
|  StorageClass  |  객체의 스토리지 유형 설정. STANDARD, STANDARD_IA, ARCHIVE가 있으며, 기본값은 STANDARD. 스토리지 유형에 대한 자세한 내용은 [스토리지 유형 개요](https://intl.cloud.tencent.com/document/product/436/30925) 참조 | String | 아니요 |
...
| Bucket   | 버킷 이름은 BucketName-APPID로 구성  | String  |  예 |
| Key   |  객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임  |String  | 예 |
| VersionId   | 버전 제어 활성화 후 지정 객체의 버전  | String  | 아니요 |
| IfModifiedSince   | 지정 시간 이후 수정될 경우에만 반환. 시간 포맷: GMT | String  | 아니요 |
...
|  Bucket  | 버킷 이름은 BucketName-APPID로 구성 | String  |  예 |
|  Key  |  객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임 | String  | 예 |
|  Range  |  다운로드할 객체의 범위 설정. 포맷: bytes=first-last  | String  |  아니요 |
|  IfMatch  |  ETag가 지정된 콘텐츠와 일치해야 반환 |String  | 아니요 |
|  IfModifiedSince  |   지정 시간 이후 수정될 경우에만 반환. 시간 포맷: GMT | String  | 아니요 |
|  IfNoneMatch  |  ETag가 지정 콘텐츠와 불일치해야 반환 | String  | 아니요 |
|  IfUnmodifiedSince  |  객체 수정 시간이 지정 시간보다 이르거나 같을 경우에만 반환. 시간 포맷: GMT | String  | 아니요|
...
다운로드한 객체의 Body 및 메타 정보(dict 포맷):
...
<table>
   ...
   <tr>
      <td>Body</td>
      <td>객체 콘텐츠 다운로드. get_raw_stream() 방법으로 파일 스트림을 얻고, get_stream_to_file(local_file_path) 방법으로 객체 콘텐츠를 지정 로컬 파일에 다운로드</td>
      <td>StreamBody</td>
   </tr>
   <tr>
      <td>ETag</td>
...
|  Bucket  | 버킷 이름은 BucketName-APPID로 구성 | String|  예 |
|  Key  | 객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임 | String| 예 |
|  CopySource  | Bucket, Key, Region, VersionId를 포함한 원본 객체의 경로 복사 |  Dict | 예 |
|  CopyStatus  |  옵션값은 Copy와 Replaced. Copy로 설정하는 경우 설정된 사용자 메타데이터 정보를 무시하고 바로 복사. Replaced로 설정하는 경우 설정된 메타 정보에 따라 메타데이터 수정. 타깃 경로와 루트 경로가 같은 경우 반드시 Replaced로 설정 | String| 예 |
| ACL |객체 ACL 설정. 예: private, public-read |String| 아니요|
| GrantFullControl |지정 계정에 객체에 대한 모든 권한 부여. 포맷: `id="OwnerUin"` |String|아니요|
|GrantRead |지정 계정에 객체의 읽기 권한 부여. 포맷: `id="OwnerUin"`|String|아니요|
|  StorageClass  |  객체의 스토리지 유형 설정. STANDARD, STANDARD_IA가 있으며, 기본값은 STANDARD. | String|  아니요 |
|  Expires  | Expires 설정 | String| 아니요 |
|  CacheControl  | 캐시 정책. Cache-Control 설정 | String|  아니요 |
|  ContentType  | 콘텐츠 유형. Content-Type 설정 | String |  아니요 |
...
| LastModified | 객체의 최종 수정 시간 복사|String|
...
| -------------- | -------------- |---------- | ----------- |
| Bucket  | 버킷 이름은 BucketName-APPID로 구성 | String |  예 |
| Delete  | 삭제된 반환 결과 방식과 타깃 Object 설명 | Dict | 예 |
| Object  | 삭제 예정인 타깃 Object 정보 설명 | List | 예 |
| Key     | 객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임| String|아니요|
| VersionId | 버전 제어 활성화 후 타깃 객체의 버전 넘버 | String |아니요|
| Quiet   |삭제한 반환 결과 방식 지정. true, false 중 선택 가능. 기본값은 false. true로 설정할 경우 실패 오류 정보만 반환. false로 설정할 경우 성공과 실패 정보를 모두 반환|String|아니요|
...
|Bucket|버킷 이름은 BucketName-APPID로 구성|String| 예|
|key |객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임|String|예|
|RestoreRequest| 검색한 임시 객체의 규칙| Dict|예|
|Days| 임시 객체의 만료 시간| Int|예|
|CASJobParameters| 복구 유형의 설정 정보| Dict|아니요|
|Tier| 복구 객체의 모드. CAS 유형의 데이터를 복구할 경우 Expedited(고속), Standard(표준), Bulk(일괄) 세 가지 모드 중 선택할 수 있음. DEEP ARCHIVE 유형의 데이터를 복구할 경우 Standard(표준), Bulk(일괄) 중 선택할 수 있음| String|아니요|
...
|Bucket|버킷 이름은 BucketName-APPID로 구성|String| 예|
|key |객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임|String|예|
|Expression|SQL 표현식. 요청할 인덱스 작업을 나타냄| String|예|
|ExpressionType| 표현식 유형. 본 항목은 확장 항목으로 현재 SQL 표현식과 SQL 매개변수만 지원| String|예|
|InputSerialization| 인덱스할 객체의 포맷. 자세한 내용은 [요청 예시](https://intl.cloud.tencent.com/document/product/436/32360#.E8.AF.B7.E6.B1.82) 참조| Dict|예|
|OutputSerialization| 인덱스 결과의 출력 포맷. 자세한 내용은 [요청 예시](https://intl.cloud.tencent.com/document/product/436/32360#.E8.AF.B7.E6.B1.82) 참조| Dict|예|
|RequestProgress| QueryProgress 쿼리 진행률 정보 반환 여부. COS Select를 선택할 경우 주기적으로 쿼리 진행률 반환| Dict|아니요|

#### 반환 결과 설명
객체의 Body 및 메타 정보(dict 포맷):
```python
{
    'Body': EventStream(),
    'ETag': '"9a4802d5c99dafe1c04da0a8e7e166bf"',
...
| -------------- | -------------- |---------- | ----------- |
| Bucket   | 버킷 이름은 BucketName-APPID로 구성  | String  | 예|
| Prefix   |  기본값 null. 멀티파트 업로드 키를 필터링해 prefix로 시작하는 멀티파트 업로드와 매칭  | String  |  아니요|
| Delimiter   |   기본값 null. 세퍼레이터 설정| String|  아니요|
| KeyMarker   |  UploadIdMarker와 함께 사용하며, 멀티파트 업로드 시작 위치 표시  | String  |  아니요|
| UploadIdMarker   |  KeyMarker와 함께 사용하며, 멀티파트 업로드 시작 위치 표시. KeyMarker를 지정하지 않으면 UploadIdMarker가 무시됨| String  |  아니요|
| MaxUploads   |반환되는 멀티파트 업로드 최대 수. 기본값은 최대 1000개  | Int  |  아니요|
| EncodingType   |   기본적으로 인코딩하지 않으며, 반환값의 인코딩 방식을 규정. 옵션값: url  | String  | 아니요|

#### 반환 결과 설명

멀티파트 업로드한 정보 가져오기(dict 포맷):
...
| Bucket  | 버킷 이름은 BucketName-APPID로 구성 | String |  예 |
| Key  | 객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임 | String |  예|
| Body  | 로컬 파일 스트림 또는 입력 스트림 등 멀티파트 업로드의 콘텐츠 | file/bytes |  예|
...
| UploadId  | 멀티파트 업로드의 식별 ID | String |  예|
|  CopySource  | Bucket, Key, Region, VersionId를 포함한 원본 객체의 경로 복사 |  Dict | 예 |
|CopySourceRange| 복사할 원본 객체의 범위(포맷: bytes=first-last). 지정하지 않으면 원본 객체 전체 복사|String|아니요|
|CopySourceIfMatch| 원본 객체의 Etag와 주어진 값이 동일한 경우에만 복사|String|아니요|
|CopySourceIfModifiedSince| 지정된 시간 이후 원본 객체가 수정되면 복사|String|아니요|
|CopySourceIfNoneMatch| 원본 객체의 Etag와 주어진 값이 동일하지 않은 경우에만 복사|String|아니요|
|CopySourceIfUnmodifiedSince| 지정된 시간 이후 원본 객체가 수정되지 않으면 복사|String|아니요|
...
| 매개변수 이름   | 매개변수 설명   |유형 |
| -------------- | -------------- |---------- |
| ETag |멀티파트의 MD5 값 복사|String|
| LastModified |멀티파트의 최종 수정 시간 복사|String|
| x-cos-copy-source-version-id | 원본 객체의 버전 넘버 | String |
...
|key |객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임 |String| 예|
|UploadId |멀티파트 업로드의 식별 ID|String| 예|
|MaxParts |반환되는 멀티파트의 최대 수. 기본값은 최대 1000개|Int| 아니요|
|PartNumberMarker |기본값: 0. 첫 파트부터 멀티파트로 나열하며, PartNumberMarker의 다음 멀티파트부터 나열|Int| 아니요|
|EncodingType |기본적으로 인코딩하지 않으며, 반환값의 인코딩 방식을 규정. 옵션값: url|String|아니요|

#### 반환 결과 설명

업로드한 모든 멀티파트의 정보(dict 포맷):
...
| IsTruncated   |  반환된 멀티파트의 잘림 여부 표시  | String|
...
|  StorageClass  |  객체의 스토리지 유형 설정. STANDARD, STANDARD_IA, ARCHIVE가 있으며, 기본값은 STANDARD. 스토리지 유형에 대한 자세한 내용은 [스토리지 유형 개요](https://intl.cloud.tencent.com/document/product/436/30925) 참조 | String |
|  Part |ETag, PartNumber, Size, LastModified 등 업로드된 멀티파트 관련 정보 | String |
...
|  Key  | 객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임 | String  |   예|
|  UploadId  | 멀티파트 업로드의 식별 ID | String  |   예|
|  MultipartUpload  |모든 멀티파트의 ETag와 PartNumber 정보 |  Dict |   예|

#### 반환 결과 설명

패키징된 객체의 관련 정보(dict 포맷):
...
| 매개변수 이름   | 매개변수 설명   |유형 |
| -------------- | -------------- |---------- |
|  ETag  |	병합된 객체의 고유 태그값. 객체 콘텐츠의 MD5 검증값이 아니므로, 객체 고유성 검사에만 사용할 수 있음. 객체 콘텐츠 검증이 필요한 경우 업로드 과정에서 단일 멀티파트의 ETag 값 검증이 가능함 |  String |
|  Bucket  |버킷 이름은 BucketName-APPID로 구성 |  String |
...
|  Bucket  | 버킷 이름은 BucketName-APPID로 구성 | String |  예 |
|  Key  | 객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg`에서 객체 키는 doc/pic.jpg임 | String |  예 |
|  LocalFilePath  | 로컬 파일의 경로명 |  String |  예 |
|  PartSize  | 멀티파트 업로드의 멀티파트 크기. 기본값: 1MB |  Int |  아니요 |
|  MAXThread  | 멀티파트 업로드의 동시 업로드 수량. 기본적으로 5개 스레드로 멀티파트 업로드 |  Int |  아니요 |
|progress_callback |  업로드 진행률 콜백 함수. 이 함수를 사용자 정의하여 업로드 진행률 확인 가능| Func| 아니요 |
| EnableMD5 | SDK의 Content-MD5 계산 여부. 기본적으로 비활성화 상태이며, 활성화 시 업로드 시간 증가|Bool | 아니요|
| ACL |객체 ACL 설정. 예: private, public-read |String| 아니요|
| GrantFullControl |권한을 부여받은 계정에 모든 권한 부여. 포맷: `id="OwnerUin"`|String|아니요|
|GrantRead |권한을 부여받은 계정에 읽기 권한 부여. 포맷: `id="OwnerUin"`  |String|아니요|
...
|  Bucket  | 버킷 이름은 BucketName-APPID로 구성 | String  |  예 |
|  Key  |  객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인`examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg` 에서 객체 키는 doc/pic.jpg임 | String | 예 |
|  DestFilePath  | 파일을 다운로드하는 로컬 타깃 경로명 |  String |  예 |
|  PartSize  | 멀티파트 다운로드의 멀티파트 크기. 기본값: 20MB |  Int |  아니요 |
|  MAXThread  | 멀티파트 다운로드의 동시 다운로드 수량. 기본적으로 5개 스레드로 멀티파트 다운로드 |  Int |  아니요 |
...
start_prefix = 'data/'
# COS 종속 세퍼레이터 '/'로 디렉터리 semantics 시뮬레이션
# 기본 null 세퍼레이터를 사용해 디렉터리에 있는 모든 하위 노드를 나열할 수 있으며, 로컬 디렉터리 재귀와 유사한 효과 구현
# delimiter를 '/로 설정하면 프로그램에서 재귀 처리할 서브 디렉터리가 필요함
delimiter = ''
...
def listCurrentDir(prefix):
    ...
    # delimiter를 '/로 설정하면 재귀 처리할 서브 디렉터리가 필요함
...
# 로컬 디렉터리에 파일 업로드. 로컬 디렉터리에 동일한 이름의 파일이 있는 경우 덮어쓰기
...
|  Key  |  객체 키(Key)는 객체의 버킷 내 고유 식별자. 예: 객체의 액세스 도메인`examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg` 에서 객체 키는 doc/pic.jpg임 | String | 예 |
|  CopySource  | Bucket, Key, Region, VersionId를 포함한 원본 객체의 경로 복사 |  Dict | 예 |
 |  CopyStatus  |복사 상태. 옵션값: Copy, Replaced | String | 아니요 |
 |  PartSize  |멀티파트 다운로드의 멀티파트 크기. 기본값: 10MB |  Int |  아니요 |
 |  MAXThread  | 멀티파트 다운로드의 동시 다운로드 수량. 기본적으로 5개 스레드로 멀티파트 다운로드 |  Int |  아니요 |

#### 반환 결과 설명
5GB 미만 파일은 copy_object의 반환 결과이고, 5GB 이상 파일은 complete_multipart_upload의 반환 결과입니다(dict 포맷).
...
클라이언트 암호화는 중요 데이터를 보관하는 고객에게 적합합니다. 클라이언트 암호화는 일부 업로드 속도를 제한하고 SDK 내부에서 직렬 방식을 사용해 멀티파트 업로드를 진행합니다.
...
```python
# 사용자 개인정보를 초기화합니다(SECRET_ID, SECRET_KEY).
...
# 객체를 업로드합니다. 암호화되지 않은 클라이언트의 put_object 기능은 모두 호환됩니다. 자세한 내용은 put_object를 참조하십시오.
...
# 객체를 다운로드합니다. 암호화되지 않은 클라이언트의 get_object 기능은 모두 호환됩니다. 자세한 내용은 get_object를 참조하십시오.
response = client_for_aes.get_object(
                        Bucket='examplebucket-1250000000',
                        Key='exampleobject')

# 멀티파트 업로드합니다. 암호화되지 않은 클라이언트의 멀티파트 업로드와 호환됩니다. 마지막 part를 제외한 각 part의 크기는 16바이트의 배수여야 합니다.
