...
COSBrowser 모바일 버전은 리전별 버킷 리스트를 보여줍니다. 리전별 버킷을 확인하고 하단의 리소스를 클릭하면 생성된 버킷을 확인할 수 있습니다.
...
1. 버킷 리스트 페이지로 이동하여 우측 상단의 <b>+</b>를 클릭합니다.
...
1. 버킷 리스트 페이지에서 대상 버킷 오른쪽의 <b>...</b>를 클릭합니다.
2. 팝업된 작업 리스트에서 **자세히**를 클릭하여 버킷의 기본 정보를 확인합니다.
기본 정보에는 버킷 이름, 리전, 생성 시간이 포함됩니다.
...
1. 버킷 리스트 페이지로 이동하여 대상 버킷의 오른쪽에 있는<b>...</b>을 클릭합니다.
...
>? 글로벌 가속을 활성화해도 기존 기본 버킷 도메인 이름에는 영향을 미치지 않으며 계속 사용할 수 있습니다.


#### 작업 순서

1. 버킷 리스트 페이지로 이동하여 대상 버킷의 오른쪽에 있는 <b>...</b>을 탭합니다.
...
버킷에 파일을 업로드할 도메인 이름을 지정할 수 있습니다. 이 이름은 일단 구성되면 파일 업로드에 먼저 사용됩니다.


<span id="SetDownloadDomainName"></span>
### 다운로드 도메인 이름 설정

버킷에서 파일 다운로드를 위한 도메인 이름을 지정할 수 있습니다. 이 도메인 이름은 일단 구성되면 파일 다운로드에 먼저 사용됩니다.
