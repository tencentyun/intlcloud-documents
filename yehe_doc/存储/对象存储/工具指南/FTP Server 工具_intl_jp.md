...
COS FTP Serverは、ファイルのアップロード、ダウンロード、削除、作成など、FTPプロトコルを介してCOSのオブジェクトやディレクトリを直接操作することができます。FTP ServerツールはPythonで実装されており、インストールがより簡単です。
...
>?FTP Serverツールは現在、中断からの再開機能サポートしていません。

## 使用の開始

#### システム環境

- オペレーティングシステム：Linux、Tencent CentOSシリーズ[CVM](https://intl.cloud.tencent.com/document/product/213)を推奨します。当面の間、Windowsはサポートしません。
- psutilが依存するLinuxシステムパッケージ：python-devel（またはpython-dev、Linuxディストリビューションの名称によって異なります）、`yum install python-devel`や`aptitude install python-dev`といったLinuxのパッケージ管理ツールによって追加されます。
...
1. Linuxコマンドラインでコマンド`ftp [ipアドレス] [ポート番号]`を使用して、COS FTP Serverに接続します。例えば、次のコマンドです。
```sh
...
2. 上記のコマンドを実行すると、**Name**と**Password**という入力すべき項目が表示されますので、COS FTP Serverの設定項目ftp_login_user_nameとftp_login_user_passwordに設定した内容を入力すると、接続が完了します。
...
2. FileZillaクライアントでCOS FTP Serverのアクセス情報を設定したら、【クイック接続】をクリックします。
