...
Hadoop 2.x 사용자는 [cos-distcp-1.3-2.8.5.jar 패키지](https://cos-sdk-archive-1253960454.file.myqcloud.com/cos-distcp/cos-distcp-1.3-2.8.5.jar)를 다운로드하여 jar 패키지의 [MD5 인증값](https://cos-sdk-archive-1253960454.file.myqcloud.com/cos-distcp/cos-distcp-1.3-2.8.5-md5.txt)에 따라 다운로드한 jar 패키지가 완벽한지 확인합니다.

Hadoop 3.x 사용자는 [cos-distcp-1.3-3.1.0.jar 패키지](https://cos-sdk-archive-1253960454.file.myqcloud.com/cos-distcp/cos-distcp-1.3-3.1.0.jar)를 다운로드하여 jar 패키지의 [MD5 인증값](https://cos-sdk-archive-1253960454.file.myqcloud.com/cos-distcp/cos-distcp-1.3-3.1.0-md5.txt)에 따라 다운로드한 jar 패키지가 완벽한지 확인합니다.
...
|         --dest=LOCATION          | 붙여넣는 타깃 디렉터리 지정, HDFS 또는 COS 경로 가능<br> 예시: --dest=cosn://examplebucket-1250000000/user/logs |   없음   |    예    |
|       --srcPattern=PATTERN | 정규식으로 원본 디렉터리에 있는 파일 필터링<br>예시: `--srcPattern='.*.log'`<br>**주의: `*`가 shell에 의해 해석되지 않도록 매개변수는 따옴표로 묶어야 합니다.** | 없음 | 아니요 |
...
| --multipartUploadChunkSize=VALUE | 플러그 인에서 COS에 파일 전송 시 멀티파트 크기 지정. COS는 최대 10000개의 파트 수를 지원하며 파일 크기에 따라 파트 크기를 조정할 수 있습니다. 단위: MB, 기본값: 8MB<br>예시: --multipartUploadChunkSize=20 |  8MB   |    아니요    |
|    --cosServerSideEncryption     | 파일을 COS로 업로드할 때, SSE-COS를 암호화/복호화 알고리즘으로 사용 지정</br>예시: --cosServerSideEncryption | false  |    아니요    |
...
|         --skipMode=MODE          | 파일 복사 전 원본 파일과 타깃 파일의 일치 여부를 검증하며, 일치할 경우 건너뜁니다. none(인증 안함), length (길이), checksum(CRC 값), length-checksum(길이 + CRC 값)을 선택할 수 있습니다.</br>예시: --skipMode=length |  none  |    아니요    |
|         --checkMode=MODE         | 파일 복사 완료 시 원본 파일과 타깃 파일의 일치 여부를 검증하며, 일치하지 않을 경우 복사를 중지합니다. none(인증 안함), length (길이), checksum(CRC 값), length-checksum(길이 + CRC 값)을 선택할 수 있습니다.<br/>예시: --checkMode=length-checksum |  length  |    아니요    |
...
|      --diffOutput=LOCATION       | 변경 파일 리스트를 출력할 디렉터리 지정. 해당 출력 디렉터리는 반드시 비어 있어야 합니다.<br/>예시: --diffOutput=/diff-output |   없음   |    아니요    |
...
```
위 명령어 중 `${version}`은 COSDistCp의 버전입니다. 예를 들어 1.0 버전의 COSDistCp jar 패키지 이름은 cos-distcp-1.0.jar입니다.
...
COSDistCp는 기본적으로 파일 복사 실패 시 5번 재시도하며, 실패 파일 정보를 /tmp/${randomUUID}/output/failed/ 디렉터리에 입력합니다. 여기서 ${randomUUID}는 랜덤 문자열입니다.
...
다음 명령어를 실행하여 실패 파일 리스트에 따라 다시 마이그레이션을 실행합니다.
```plaintext
...
다음 명령어를 통해 MapReduce 작업의 로그를 획득하여 파일 복사 실패 이유를 확인할 수 있습니다. 여기서 application_1610615435237_0021는 애플리케이션 ID입니다.
```
...
매개변수 `--srcPattern`로 명령을 실행합니다. `/data/warehouse/logs` 디렉터리의 확장자가 .log인 로그 파일만 동기화합니다. 예시는 다음과 같습니다.
...
매개변수 `--deleteOnSuccess`로 명령을 실행합니다. `/data/warehouse` 디렉터리의 파일을 HDFS에서 COS로 동기화한 후 즉시 원본 디렉터리에서 해당 파일을 제거합니다.

```plaintext
hadoop jar cos-distcp-${version}.jar --src /data/warehouse --dest cosn://examplebucket-1250000000/data/warehouse --deleteOnSuccess
```

>!해당 옵션을 지정하면 파일 1개 마이그레이션 완료 즉시 대응되는 원본 파일을 삭제합니다. 전체 마이그레이션이 완료된 후 원본 파일을 삭제하는 것이 아니므로 신중하게 사용하십시오.

### 단일 파일의 읽기 대역폭 제한

매개변수 `--bandWidth`로 명령을 실행합니다. 단위는 MB이며, 마이그레이션 파일별 읽기 대역폭을 10MB/s로 제한할 경우, 예시는 다음과 같습니다.
...
매개변수 `--cosChecksumType`으로 명령을 실행합니다. 기본값은 CRC32C이며, CRC32C와 CRC64를 선택할 수 있습니다.
...
`--skipMode` 옵션은 파일 복사 전 원본 파일과 타깃 파일의 일치 여부를 검증하는 데 사용하며, 일치할 경우 건너뜁니다. none(인증 안함), length(길이), checksum(CRC 값), length-checksum(길이 + CRC 값)을 선택할 수 있습니다.

원본과 타깃 파일 시스템의 검증 및 알고리즘이 일치하지 않는 경우에는 원본 파일의 새로운 검사합을 계산합니다. 원본이 HDFS일 경우 다음 방식으로 HDFS 원본의 COMPOSITE-CRC32C 검증 알고리즘 지원 여부를 확인할 수 있습니다.
...
매개변수 `--outputCodec`으로 명령을 실행합니다. 해당 매개변수를 통해 HDFS의 데이터를 실시간으로 압축해 COS에 백업할 수 있어 스토리지 비용을 절약할 수 있습니다. 매개변수 옵션값은 keep, none, gzip, lzop, snappy가 있으며, none은 타깃 파일을 압축하지 않은 상태로 저장하고, keep은 원본 파일의 압축 상태를 유지합니다. 예시는 다음과 같습니다.
...
>!여기에서 keep을 제외한 모든 값은 먼저 파일을 압축 해제한 후 타깃 압축 유형으로 변환합니다. 따라서 keep을 제외하고는 압축 매개변수 등이 일치하지 않아 타깃 파일과 원본 파일이 동일하지 않을 수 있으나 압축 해제 후 파일은 동일합니다.
...
`--srcPrefixesFile` 매개변수를 사용하여 해당 파일을 지정하여 마이그레이션 명령어를 실행합니다.
...
>!위 명령어의 증분 마이그레이션은 파일 크기가 변경된 파일만 동기화할 수 있으며 파일 내용이 변경된 파일은 동기화할 수 없습니다. 파일 내용이 변경된 경우 --diffMode 사용 예시를 참조하여 파일 CRC에 따라 변경된 파일 리스트를 확인하십시오.

### CRC에 따라 변경 파일 리스트 획득 및 증분 마이그레이션

매개변수 `--diffMode`와 `--diffOutput`으로 명령을 실행합니다.
- `--diffMode`의 옵션값은 length와 length-checksum이 있습니다.
...
다음 예시는 파일 크기 및 CRC 값에 따라 원본 파일과 타깃 파일이 동일한지 여부를 검증합니다.
...
5. 읽기 권한이 없는 등의 사유로 diff 작업에 실패하는 경우 DIFF_FAILED로 기록
...
### 환경에서 Hadoop-COS를 설정하지 않은 경우, COSDistCp는 어떻게 실행합니까?
...
### 복사 결과에 일부 파일 복사에 실패했다고 표시됩니다. 어떻게 처리해야 합니까?

COSDistCp는 복사 과정에서 IOException이 나타날 경우 5번 재시도하며, 5번의 재시도에도 복사에 실패하는 경우 실패한 파일 정보를 `/tmp/${randomUUID}/output/failed/` 디렉터리에 입력합니다. 여기서 ${randomUUID}는 랜덤 문자열이며, 복사에 실패하는 주요 원인은 다음과 같습니다.
...
로그 정보 기록에 원본 파일이 존재하지 않고 원본 파일을 확실하게 생략 가능한 경우, 다음 명령어를 통해 SRC_MISS를 제외한 변경 파일 리스트를 획득할 수 있습니다.
```
hadoop fs -getmerge /tmp/${randomUUID}/output/failed/ failed-manifest
grep -v '"comment":"SRC_MISS"' failed-manifest |gzip > failed-manifest.gz
```
SRC_MISS를 제외한 실패 파일이 존재하는 경우 `/tmp/${randomUUID}/output/logs/` 디렉터리의 오류 로그 정보 및 풀링 애플리케이션 로그를 종합하여 원인은 진단할 수 있습니다. 예를 들어, yarn 애플리케이션의 로그를 풀링하는 경우 다음 명령어를 사용할 수 있습니다.
```
yarn logs -applicationId application_1610615435237_0021 > application_1610615435237_0021.log
```
