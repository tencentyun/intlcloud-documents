Java Development Kit (JDK) is the SDK for Java. This document takes JDK 1.8 as an example to describe how to install and configure JDK on Windows and Linux.
...
Right-click and select **PC** > **Properties** > **Advanced system settings** > **Environment Variables** > **System variables** > **Create**, and configure the programs.
...
#### 4. Testing
Click **Start (or press Win+R) > Run (type `cmd`) > OK (or press Enter)**, type the `javac` command, and press Enter to test. If the following is displayed, the environment variable is configured successfully.
