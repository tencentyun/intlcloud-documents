...
> ?
> - 로컬 문자 포맷을 UTF-8로 유지하십시오. 그렇지 않을 경우 중국어 버전의 파일 작업 시 오류가 발생합니다.
...
| -t               | 임시 키 token으로, 임시 키 사용 시 설정해야 하며 x-cos-security-token 헤더를 설정해야 합니다. | 문자열 | 아니오       |
| -b               | 지정한 버킷 이름이며, 버킷 이름은 BucketName-APPID 포맷으로 생성됩니다. [이름 생성 규칙](https://intl.cloud.tencent.com/document/product/436/13312)을 참조하십시오. 처음 설정 사용 시 COS 콘솔에서 버킷을 생성해야 하며 툴 설정에 사용합니다. | 문자열 | 예       |
...
>- 구성 파일 중 `anonymous` 항목에서 True, False를 선택할 수 있으며, 익명 모드의 사용 여부를 나타냅니다. 즉, 서명이 비어 있습니다.
...
>?
>- `-b <BucketName-APPID>` 매개변수를 통해 버킷 이름을 지정하며, 버킷의 이름 생성 포맷은 BucketName-APPID입니다. 입력하는 버킷 이름은 반드시 해당 포맷이어야 합니다.
...
### 구성 파일 및 로그 파일 경로 지정 명령어

사용자가 구성 파일의 경로를 지정하지 않을 경우 기본 구성 파일 경로인 `~/.cos.conf`를 사용하며, 로그 파일 경로를 지정하지 않을 경우 기본 로그 파일 경로인 `~/.cos.log`를 사용합니다.

>?
>- `-c <conf_path>` 매개변수를 통해 버킷 이름을 지정하며, 버킷의 이름 생성 포맷은 BucketName-APPID입니다. 입력하는 버킷 이름은 반드시 해당 포맷이어야 합니다.
>- `-l <log_conf>`를 통해 Region을 지정하며, 버킷의 소속 리전을 지정할 수 있습니다.

- 명령어 포맷
```plaintext
coscmd -c <conf_path> -l <log_conf> <action> ...
```
- 작업 예시 - 구성 파일 경로를 /data/home/cos_conf로, 로그 출력 경로를 /data/home/cos_log로 지정하고, 이름이 examplebucket이며 소속 리전이 베이징인 버킷 생성
```plaintext
...
```
 >!-H 매개변수를 사용하여 HTTP header 설정 시, JSON 포맷을 유지해야 합니다. 예시: `coscmd upload -H "{'x-cos-storage-class':'Archive','Content-Language':'zh-CN'}" <localpath> <cospath>`. 헤더에 대한 자세한 내용은 [PUT Object](https://intl.cloud.tencent.com/document/product/436/7749) 문서를 참조하십시오.
...
```
- 작업 예시 - D 드라이브 doc 폴더의 .txt 및 .doc 확장명을 가진 파일은 업로드 생략
```plaintext
...
> - COSCMD 멀티파트 업로드 시 모든 파트에 MD5 검증을 실행합니다.
...
```
 >!`-s` 또는 `--sync` 매개변수를 사용하여 폴더 다운로드 시 로컬에 존재하는 동일한 파일(다운로드하는 파일이 COSCMD의 upload 인터페이스를 통해 업로드되고, 파일이 `x-cos-meta-md5` 헤더를 가지고 있어야 함)을 건너뜁니다.
...
```
 >!폴더 다운로드 시 `--ignore` 매개변수를 사용하면 특정 파일을 생략할 수 있으며 `--invlude` 매개변수를 사용하면 특정 파일을 필터링할 수 있고, shell 와일드카드 규칙을 지원합니다. 여러 개의 규칙을 지원하며, 쉼표 `,`로 구분합니다. 특정 확장명을 생략하는 경우 마지막에 `,`를 입력하거나 큰따옴표 `""`를 추가해야 합니다.
...
```
>!'<>'의 매개변수를 삭제할 COS 파일 경로(cospath)로 대체하고, 툴에서 사용자에게 삭제 진행 여부를 안내해 작업을 수행합니다.
...
> ?
> - '<>'의 매개변수를 복사할 COS 파일 경로(sourcepath)와 붙여 넣을 COS 파일 경로(cospath)로 대체합니다.
> - sourcepath 포맷: `<BucketName-APPID>.cos.<region>.myqcloud.com/<cospath>`
> - -d 매개변수를 사용하여 `x-cos-metadata-directive` 매개변수를 설정할 수 있으며, Copy 및 Replaced로 선택할 수 있고 기본값은 Copy입니다.
> - -H 매개변수를 사용하여 HTTP header로 설정 시, JSON 포맷을 유지해야 합니다. 예시: `coscmd copy -H -d Replaced "{'x-cos-storage-class':'Archive','Content-Language':'zh-CN'}" <localpath> <cospath>`. 헤더에 대한 자세한 내용은 [PUT Object - Copy](https://intl.cloud.tencent.com/document/product/436/10881) 문서를 참조하십시오.
...
> - sourcepath 포맷: `<BucketName-APPID>.cos.<region>.myqcloud.com/<cospath>`
> - -d 매개변수를 사용하여 `x-cos-metadata-directive` 매개변수를 설정할 수 있으며, Copy 및 Replaced로 선택할 수 있고 기본값은 Copy입니다.
> - -H 매개변수를 사용하여 HTTP header로 설정 시, JSON 포맷을 유지해야 합니다. 예시: `coscmd move -H -d Replaced "{'x-cos-storage-class':'Archive','Content-Language':'zh-CN'}" <localpath> <cospath>`. 헤더에 대한 자세한 내용은 [PUT Object - copy](https://intl.cloud.tencent.com/document/product/436/10881) 문서를 참조하십시오.
...
> !
>- '<>'의 매개변수를 버전 제어가 필요한 상태(status)로 대체합니다.
