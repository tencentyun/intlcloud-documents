...
| excelRow | A value of `1` indicates to put all rows on one page for formatting. Default value: 0. | Int | No |
| excelCol            | A value of `1` indicates to put all columns on one page for formatting. Default value: 0. | Int | No |
...
| imageDpi    | Renders the image according to the specified DPI. This parameter works together with `scale`. Value range: 96–600. Default value: 96. The width of the output image must be less than 65500 px. | Int | No |
...
| Content-Type | Different values will be returned based on the actual image format, such as `image/jpeg` and `image/webp`. It is okay to keep the current value in case of an exception. | String |
