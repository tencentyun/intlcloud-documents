...
- Blind watermarking is a paid service, which needs to be activated on the bucket's configuration page. For detailed pricing, see Image Processing Fees.
...
| level    | Blind watermark level, which only takes effect if `type` is set to `2`. Valid values: 1 (default), 2, 3. The larger the value, the better the blind watermarking effect. | Int  |  No |
...
| ------------------------- | ------------------ | --------- |
| UploadResult              | Upload result | Container |
