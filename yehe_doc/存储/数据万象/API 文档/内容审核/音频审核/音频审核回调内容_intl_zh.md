...
| 节点名称（关键字） | 描述                                                         | 类型    |
| :----------------- | :----------------------------------------------------------- | :------ |
| Code               | 错误码，只有 State为 Failed 时返回。详情请查看 [错误码列表](https://intl.cloud.tencent.com/document/product/1045/33700)。 | String  |
...
| Category           | 该字段表示审核命中的具体审核类别。注意：该字段可能返回空。   | String  |
| SubLabel           | 该字段表示审核命中的具体子标签。注意：该字段可能返回空。     | String  |
| Label              | 本次审核的综合结果标签，如果命中了敏感关键词，该字段返回对应的关键词。 | String  |
