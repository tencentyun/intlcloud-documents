...
[标签](https://intl.cloud.tencent.com/document/product/651) 是腾讯云提供的用于标识云上资源的标记，是一个键-值对（Key-Value）。标签可以帮助您从各种维度（例如业务，用途，负责人等）方便的对云资源进行分类管理。需要注意的是，腾讯云不会使用您设定的标签，标签仅用于您对腾讯云资源的管理。本文档指导您对文件存储资源进行编辑标签的操作。
...
- **标签键限制**：只能为`数字`、`字母`、`+=.@-`，且标签键长度最大为255个字符。
...
>? 创建文件系统的相关配置项说明，可参见 [创建文件系统及挂载点](https://intl.cloud.tencent.com/document/product/582/9132) 文档。
