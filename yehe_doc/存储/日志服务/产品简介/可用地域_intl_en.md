...
1. If another Tencent Cloud service is integrated with CLS, try to select a logset in the same region as the service. Tencent Cloud services in the same region can access each other over the private network, which effectively reduces latency and improves access speed.
...
3. CLS request domain names divide into private domain names and public domain names:
	- A private domain name is in the format of `${region}.cls.tencentyun.com`, which is valid only for access requests from the same region, that is, CVM or Tencent Cloud services access the CLS service in the same region through the private domain name.
