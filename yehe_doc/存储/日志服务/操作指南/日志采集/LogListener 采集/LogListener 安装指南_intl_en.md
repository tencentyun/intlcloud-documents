...
```shell
wget https://loglistener-1254077820.cos.ap-shanghai.myqcloud.com/loglistener-linux-x64-2.4.1.tar.gz && tar -zxvf loglistener-linux-x64-2.4.1.tar.gz -C /usr/local && cd /usr/local/loglistener-2.4.4/tools && ./loglistener.sh install
```
...
> ? The operation commands described in this document are only applicable to LogListener 2.2.4 and above. For commands on lower versions, please see [Operation guide of earlier LogListener versions](https://intl.cloud.tencent.com/document/product/614/35674).

### 1. View LogListener version
...
### 3. Manage LogListener processes

```shell
/etc/init.d/loglistenerd (start|restart|stop) # Start, restart, stop
```

### 4. View LogListener process status
...
## LogListener Uninstallation

Run the uninstallation command in the `loglistener/tools` directory as an admin:

```shell
./loglistener.sh uninstall
```

## Manual LogListener Update
