...
On the **Collection Configuration** page, enter the collection rule name and enter the **Collection Path** according to the **log collection path format** as shown below:
...
> - We recommend you configure the collection path as `log/*.log` and rename the old file after log rotation `log/*.log.xxxx`.
...
#### Configuring single-line - full regular expression mode
...
- The time attribute of a log is defined as follows:
 ...
 For more information on the time format, see [Configuring Time Format](https://intl.cloud.tencent.com/document/product/614/32942).
