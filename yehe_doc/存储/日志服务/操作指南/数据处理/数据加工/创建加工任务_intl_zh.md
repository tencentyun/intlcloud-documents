...
## 费用说明
数据加工会产生相关的费用，例如：您的日志主题在广州，原始数据是100GB，经过数据加工（假定您是过滤场景）之后，数据变成50GB，写入目标主题（不开启索引），则您的相关费用是：  
- 数据加工费：100GB × 0.15元/GB =15元  
- 目标主题的写流量：50GB × 0.3（压缩率） × 0.18元/GB =2.7元  
- 目标主题的存储费：50GB × 0.3（压缩率） × 0.0115元/GB  = 0.17元  

总计17.87元。 国内大部分地域可参考这个计费，特殊地域参考 [产品定价](https://intl.cloud.tencent.com/document/product/614/37510)。
