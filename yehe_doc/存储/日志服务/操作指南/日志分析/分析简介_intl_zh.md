...
| 指标                                     | 限制说明                                                     | 备注                        |
| ---------------------------------------- | ------------------------------------------------------------ | --------------------------- |
| 单个统计字段值（Value）最大长度          | 最大长度为32KB，超出部分将无法写入 es。                             | 若单词大小超过32k，无法写入 ES，不便排查丢弃的日志内容。                           |
| 非聚类分析（不含 group by 子句）结果条数 | 每次分析返回结果条数最大1000条。                               | limit 默认100条，最大1000条。 |
| 聚类分析（含 group by 子句）结果条数     | 每次分析返回结果条数最大2000条。                                | limit 默认100条，最大2000条。 |
