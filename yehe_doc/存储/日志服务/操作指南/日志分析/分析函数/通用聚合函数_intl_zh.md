...
| count_if(KEY)        | 统计满足指定条件的日志条数。                      | `* | SELECT count_if(remote_addr) AS UV`                    |
| geometric_mean(KEY)  | 计算目标列的几何平均数。                          | `* | SELECT geometric_mean(request_time) AS request_time`     |
| max(KEY)             | 查询 x 中的最大值。                                 | `* | SELECT MAX(request_time) AS max_request_time`           |
| max_by(x,y)          | 返回 y 为最大值时对应的 x 值。                        | `* | SELECT MAX(request_method, request_time) AS method`     |
