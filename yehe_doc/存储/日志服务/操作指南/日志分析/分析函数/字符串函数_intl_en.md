This document introduces basic syntax and examples of string functions.
...
| concat_ws(key0, ..., keyN)           | Concatenates strings with `key0` as the delimiter. If `string0` is `null`, the result is `null`, and the `null` values in string1, ..., stringN are skipped. The return value is of the VARCHAR type. | Concatenates strings with the value of `remote_addr` as the delimiter:</br> `*\ | SELECT concat_ws(remote_addr, host, time_local) ` |
| concat_ws(\*string0\*, array(varchar)) | Concatenates strings with `string0` as the delimiter. If `string0` is `null`, the result is `null`, and the `null` values in the array are skipped. The return value is of the VARCHAR type.</br> Note: the `array (varchar)` parameter in this function is an array not a string. | Concatenate strings with the `Android` string as the delimiter:</br> `*\ | SELECT concat_ws('Android', split(http_user_agent, '/')) ` |
...
| length(key)                          | Returns the length of a string. The return value is of the INT type. | Return the length of the `http_user_agent` string:</br> `*\ | SELECT length(http_user_agent)      ` |
| levenshtein_distance(key1, key2)     | Returns the the Levenshtein distance between the `key1` and `key2` strings. The return value is of the BIGINT type.       | Return the Levenshtein distance between the `remote_addr` and `http_protocol` strings:</br> `*\ | SELECT levenshtein_distance(remote_addr, http_protocol)            ` |
...
| position(substring IN key)           | Returns the position of `substring` in a string. Positions start with 1. If the position is not found, `0` is returned. This function takes the special syntax `IN` as a parameter and it references the strpos() function. The return value is of the BIGINT type. | Return the position of the 'G' characters in `http_method`:</br> `*\ | select position('G' IN http_method)      ` |
...
| word_stem(word)                      | Returns the stem of `word` in the English language. The return value is of the VARCHAR type.                  | Return the English word of 'Mozila':</br> `*\ | SELECT word_stem('Mozilla')      ` |
...
- Extract the values of the 2nd to 5th characters from the value of the **remote_user** field:
```
...
- Split the value of the **http_protocol** field into substrings using a slash (/) and return the collection of the substrings:
```
