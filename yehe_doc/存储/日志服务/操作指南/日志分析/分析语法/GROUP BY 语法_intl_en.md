This document introduces the usage and examples of the `GROUP BY` syntax.
...
| Aggregate function | The `GROUP BY` syntax is usually used together with aggregate functions such as `MIN`, `MAX`, `AVG`, `SUM`, and `COUNT`. For more information, please see [Aggregate Function](https://intl.cloud.tencent.com/document/product/614/38728). |
...
> 
- Calculate PV by the time granularity of 5 minutes:
...
For example, the following statement is used to perform modulo alignment at an interval of 5 minutes:
```
