...
| 401                       | AuthFailure.SecretIdNotFound | The key does not exist. Check if the key has been deleted or <br>disabled in the console, and if not, check if the key is correctly entered. Note that there should be no space before or after the key. |
...
| 401                       | AuthFailure.UnauthorizedOperation         | Unauthorized request. For more information about the authentication, see the [CAM documentation](https://intl.cloud.tencent.com/zh/document/product/598)  |
