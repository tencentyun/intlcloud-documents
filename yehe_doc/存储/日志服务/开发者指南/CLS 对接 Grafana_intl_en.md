...
```
For more information on how to install more plugins, please see [Grafana plugins](https://grafana.com/grafana/plugins?type=panel).
...
```
>? 
> - If your CVM instance is not on CentOS, please confirm the location of the Grafana plugin directory first and then enter the directory for installation.
> - If the version of Grafana to be installed is below 7.0, you don't need to modify the Grafana configuration file, and you can skip the following steps and directly [configure a log data source](#ConfigLogDataSource).
> 
2. Open the `grafana.ini` configuration file on the machine where Grafana has been deployed.
...
```
2. On the left sidebar, select the **Settings** icon to enter the **Data Sources** page.
...
<tr><td>Log panel</td><td>Log panel is used to show log search result. Query syntax supports searching by keyword, fuzzy match. For more information, see [Syntax and Rules](https://intl.cloud.tencent.com/document/product/614/30439). e.g. status:400</td><td>limit: it specifies the number of log search results to be returned.</td><tr>
<tr><td>Table panel</td><td>Table panel will automatically show the results of whatever columns and rows your query returns</td><td>None</td><tr>
