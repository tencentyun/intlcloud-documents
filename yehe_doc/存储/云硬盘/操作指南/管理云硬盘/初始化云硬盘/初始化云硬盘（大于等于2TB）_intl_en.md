## Scenario
This document walks you through the scenario of how to initialize cloud disks by taking a cloud disk with a capacity of not less than 2 TB as an example. For more information, see [Initialization Scenarios](https://intl.cloud.tencent.com/document/product/362/31596).
...
## Prerequisites
You have [mounted a cloud disk](/doc/product/362/5745) to your CVM.
...
4. Execute the following command to create a new mounting target.
```
mkdir <Mounting point>
```
Take the newly created mounting target `/data` as an example:
```
mkdir /data
```
5. Execute the following command to mount the newly created partition to the newly created mounting target.
```
...
8. Run the following command to back up the `/etc/fstab` file to a directory, `/home`, for example:
```
cp -r /etc/fstab /home
```
...
```
<Device information> <Mounting target> <File system format> <File system installation option> <File system dump frequency> <File system check sequence at launch>
```
...
lsblk
 ```
The returned information is similar to what is shown in the figure below, which indicates that the current CVM has two disks, `/dev/vda`, the system disk, and `/dev/vdb`, the newly added data disk.
...
4. Enter `p`, and press Enter to view the current disk partition format.
 The returned information is similar to what is shown in the following figure:
...
lsblk
 ```
The resulting response is similar to what is shown in the following figure, you can now see the new partition “/dev/vdc1”.
...
```
 ...
 The formatting takes a while, please pay attention to the system’s running status, and do not exit.
13. Execute the following command to create a new mounting target.
```
mkdir <Mounting point>
```
Take the newly created mounting target `/data/newpart2` as an example:
```
mkdir /data/newpart2
```
14. Execute the following command to mount the newly created partition to the newly created mounting target.
```
...
17. Run the following command to back up the `/etc/fstab` file, in this example, to the /home directory:
```
cp -r /etc/fstab /home
```
