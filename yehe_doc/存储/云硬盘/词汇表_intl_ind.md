...
### Elastic cloud disks (Disk cloud elastis)

Disk cloud elastis memiliki siklus pemakaian (siklus penagihan) yang terpisah dan dapat dipasang dan dilepas di antara berbagai instans CVM di wilayah dan zona ketersediaan yang sama. Namun, disk ini tidak dapat dipasang secara bersamaan di beberapa CVM.



### Non-elastic cloud disks (Disk cloud non-elastis)
...
Rollback mampu memulihkan program atau data ke status benar terakhir ketika terjadi kesalahan pemrosesan. Terdapat dua jenis rollback, yaitu rollback program dan rollback data.
...
### Snapshot chain (Rantai snapshot)

Rantai shapshot adalah rantai hubungan yang terdiri atas semua snapshot dari disk yang sama, dan setiap simpul mewakili satu snapshot dari disk tersebut.
...
### Cloud disk snapshots (Snapshot disk cloud)
...
### Incremental snapshot (Snapshot tambahan)
