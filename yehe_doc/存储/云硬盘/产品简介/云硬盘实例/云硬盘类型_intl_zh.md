...
>!
>-  增强型 SSD 云硬盘当前仅在广州三区、广州四区、上海二区、上海三区、上海五区、北京三区、北京四区、成都一区、重庆一区、南京一区、南京二区、清远一区开放售卖，后续将逐步增加售卖可用区。
...
**高性能云硬盘**、**SSD 云硬盘**和**增强型 SSD 云硬盘**的性能指标分别为：

|        性能指标       |               增强型 SSD 云硬盘            |SSD 云硬盘                                                    | 高性能云硬盘                                                 | 
| -------------- | ------------------------------|------------------------------ | ------------------------------------------------------------ | 
| 随机 IOPS      | 最大随机 IOPS = 1800 + 存储容量（GB）× 50<br/>且最大随机 IOPS 不超过50000 | 最大随机 IOPS = 1800 + 存储容量（GB）× 30<br/>且最大随机 IOPS 不超过26000 | 最大随机 IOPS = 1800 + 存储容量（GB）× 8<br>且最大随机 IOPS 不超过6000 |
| 吞吐量（MB/s） | 最大吞吐量 = 120 + 存储容量（GB）× 0.5<br/>且最大吞吐量不超过350MB/s | 最大吞吐量 = 120 + 存储容量（GB）× 0.2<br/>且最大吞吐量不超过260MB/s | 最大吞吐量 = 100 + 存储容量（GB）× 0.15<br>且最大吞吐量不超过150MB/s |
| 时延| 0.3 - 1ms|0.5 - 3ms| 0.8 - 4ms| 
