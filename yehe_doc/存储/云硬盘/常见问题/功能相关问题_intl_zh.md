...
### 对象存储和云硬盘的区别是什么？
- [对象存储](https://intl.cloud.tencent.com/zh/document/product/436)：具备无文件系统、目录结构、文件数量和空间上限的特性，需通过 Web API 接口管理和访问存储，提供了 SDK 和工具等集成，可以不依托云服务器单独使用。对象存储支持大规模数据的访问，但不适合毫秒级响应或随机读写的场景。
- [云硬盘](https://intl.cloud.tencent.com/zh/document/product/362)：需要搭配云服务器，且需进行文件系统分区或格式化后，才可以被挂载使用。云硬盘不同的类型，具备不同的性能指标可满足不同使用场景。
