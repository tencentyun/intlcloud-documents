...
Jika kapasitas VG LVM Anda tidak memenuhi kebutuhan, Anda dapat membeli disk cloud elastis, memasangkannya ke instans CVM, kemudian menambahkannya ke VG LVM untuk meningkatkan kapasitasnya.

## Building LVM (Membuat LVM)
...
1. [Log in to the Linux CVM instance] (Login ke instans CVM Linux) (https://intl.cloud.tencent.com/document/product/213/5436) sebagai pengguna root.
...
``` Pembuatan PV yang berhasil akan tampak seperti gambar berikut:
...
```plaintext
vgextend [VG name] (nama VG) [Path of the new PV] (Path PV baru)
...
```plaintext
lvcreate [-L <LV size> (ukuran LV)][-n <LV name> (nama LV)] <VG name> (nama VG).
