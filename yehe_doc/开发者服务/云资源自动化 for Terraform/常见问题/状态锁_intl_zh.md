...
当有进程执行 [上锁步骤](https://www.tencentcloud.com/document/product/1172/52394) 中的步骤4、5、6、7时，如果其他进程尝试操作同一 Backend，则会抛出 Lock Error，示例如下：
``` text
╷
...
如在执行 [上锁步骤](https://www.tencentcloud.com/document/product/1172/52394) 的步骤3、4时，由于进程中断而退出，标签锁未解开，此时再次进行 terraform 操作会报标签无法创建的异常：
``` text
│ Error: Error acquiring the state lock
│
...
此时，Terraform 没有相关命令释放这个锁，您需要手动删除 tencentcloud-terraform-lock:xxxxxxxxx 标签。您可通过控制台，或调用云 API 接口 [DeleteTag](https://www.tencentcloud.com/document/product/651/46534) 进行删除。控制台操作步骤如下：
