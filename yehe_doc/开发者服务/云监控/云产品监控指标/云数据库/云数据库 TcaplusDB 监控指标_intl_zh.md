...
| Writelatency  | 平均写时延 | 平均写入数据时延 | 微秒 | TableInstanceId、ClusterId |
| Comerror     | 一般错误率 | 表格一般操作的错误比例 | %  | TableInstanceId、ClusterId |
...
| Writecu | 实际写容量单位 | 对表格实际写入的容量单位数量 | 个/秒   | TableInstanceId、ClusterId |
| Readcu     | 实际读容量单位 | 对表格实际读取的容量单位数量 | 个/秒  | TableInstanceId、ClusterId |
