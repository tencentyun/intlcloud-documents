...
| DataReadIOPS    | 写入带宽   | 文件系统平均每秒钟写入的数据量 | KB/s | FileSystemId |

### 读写次数类

| 指标英文名       | 指标中文名 | 指标含义                         | 单位  | 维度         |
| ---------------- | ---------- | ---------------------------- | ----- | ------------ |
| DataReadIOPS | 读取 IOPS   | 文件系统平均每秒钟读操作次数 | 次/秒 | FileSystemId |
| DataWriteIOPS    | 写入 IOPS   | 文件系统平均每秒钟写操作次数 | 次/秒 | FileSystemId |
...
> ?每个指标的统计粒度（Period）取值不一定相同，可通过 [DescribeBaseMetrics](https://intl.cloud.tencent.com/document/product/248/33882) 接口获取每个接口支持的统计粒度。
