...
We recommend you reinstall the agents first as instructed in [Installing CVM Agents](https://intl.cloud.tencent.com/document/product/248/6211) and wait 3 minutes before checking again whether the monitoring data is restored. If an error occurs during the reinstallation or the monitoring data is not restored, troubleshoot the problem as detailed below.

## Causes and Solutions

| Cause | Solution |
| ------------------------------- | ------------------------------------- |
| The agents are not installed/started | Troubleshoot by referring to [Step 1](#step1) |
| The reporting domains cannot be resolved | Troubleshoot by referring to [Step 2](#step2) |
| The agents failed to get the UUID | Troubleshoot by referring to [Step 3](#step3) |
...
### Step 1. Check whether the agents have been installed or started[](id:step1)
...
If not, install the agents as instructed in [Installing CVM Agents](https://www.tencentcloud.com/document/product/248/6211).
...
### Step 2. Check reporting domains[](id:step2)
...
1. If you use a self-built/third-party DNS service, we recommend you not add the private network DNS provided by Tencent Cloud in `/etc/resolv.conf`. For more information, see [Private Network Access](https://intl.cloud.tencent.com/document/product/213/5225).
...
7. Run `services.msc`. Then, right-click the QCloud BaradAgent Monitor and QCloud Stargate Manager services and click **Restart the service**.
   ...
   If there is still no monitoring data after the restart, uninstall and reinstall the agents as instructed in [Installing CVM Agents](https://intl.cloud.tencent.com/document/product/248/6211).
   </dx-alert>
   :::
   </dx-tabs>
...
### Step 4. Check the CVM instance log[](id:step4)
...
The agents may fail to report data if the CPU usage, memory usage, or bandwidth utilization of the CVM instance is too high.

**Troubleshooting methods:** 

- High CPU usage: For detailed troubleshooting directions, see [CVM CPU/Memory Usage Is Too High](https://intl.cloud.tencent.com/document/product/248/36204).
...
- High bandwidth utilization: For detailed troubleshooting directions, see [CVM Bandwidth Utilization Is Too High](https://intl.cloud.tencent.com/document/product/248/36207).
