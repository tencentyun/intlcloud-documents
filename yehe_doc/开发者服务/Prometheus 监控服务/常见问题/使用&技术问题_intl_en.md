...
You can use Prometheus to sustain this scenario, but we recommend you not expose too many metrics in the exporter, as the exporter itself and the Prometheus agent use a lot of memory. Only expose key metrics, and do not use parameters such as user ID and URL in metric labels.
...
### After the TMP instance is rebooted, the amount of generated data soars. Is this normal?
Retries will be performed after instance rebooting, so it is normally if the data volume seems to fluctuate greatly in a short period of time.
