...
2. 创建 Prometheus 托管实例，详情请参见创建实例。
3. 通过实例列表进入到 Prometheus 实例的管理页面。
4. Prometheus 预聚合规则 (Recording Rule)，详情请参见预聚合概述。

## 操作步骤

### 新建规则

1. 通过实例管理页面的左侧菜单**预聚合** > **新建**打开规则新建页面，根据具体实际需求调整规则的表达式及需要聚合生成的新指标名，如下图，具体术语请参见预聚合概述。
