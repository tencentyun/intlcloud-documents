...
<tr>
<td class="div-td-el">聚合 API 出错</td>
...
<td class="div-td-el">absent(sum(up{job="kube-controller-manager"}) by (cluster_id) &gt; 0)</td>
<td>15m</td>
...
<td class="div-td-el">increase(kubelet_certificate_manager_client_expiration_renew_errors[5m]) &gt; 0</td>
<td>15m</td>
...
<tr>
<td>副本数和 HPA 不匹配</td>
<td class="div-td-el">(kube_hpa_status_desired_replicas{job=~".*kube-state-metrics"} != kube_hpa_status_current_replicas{job=~".*kube-state-metrics"}) and changes(kube_hpa_status_current_replicas[15m]) == 0</td>
<td>15m</td>
...
<tr>
<td>网卡接收出错</td>
<td class="div-td-el">increase(node_network_receive_errs_total[2m]) &gt; 10</td>
<td>1h</td>
<td>网卡接收数据出错</td>
</tr>
<tr>
