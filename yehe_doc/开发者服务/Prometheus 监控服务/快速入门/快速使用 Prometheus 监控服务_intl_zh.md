...
>?对于已创建腾讯云 [容器服务 TKE](https://www.tencentcloud.com/document/product/457) 的用户，您可以在 [ Prometheus 监控服务控制台](https://console.cloud.tencent.com/monitor/prometheus) 创建 Prometheus 实例并安装 Prometheus 监控插件对其进行监控，同时 Prometheus 监控服务集成 Grafana 及预定义 Dashboard 来查看不同维度的性能指标数据。

## 前提条件

创建腾讯云容器服务 [托管版集群](https://intl.cloud.tencent.com/document/product/457/30637)。
...
2. 单击**新建**，进入新建购买页，可根据自己的实际情况购买对应的实例，详情请参见 [创建实例](https://intl.cloud.tencent.com/document/product/1116/43170)。
...
3. 在集群监控页面进行下列操作。
 - 关联集群：将集群和 Prometheus 实例关联，参考指引 [关联集群](https://intl.cloud.tencent.com/document/product/457/46731)。
 - 数据采集配置：支持通过控制台新增或 Yaml 文件配置两种方式，配置新的数据采集规则来监控您的业务数据。
 - 精简基础监控指标：选择需要上报的指标，避免不必要的费用支出。参考指引 [精简基础监控指标](https://intl.cloud.tencent.com/document/product/457/47004) 。
