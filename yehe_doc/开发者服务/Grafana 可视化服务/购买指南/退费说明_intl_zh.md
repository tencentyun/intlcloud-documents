为更加方便您使用 Grafana 可视化服务，如果您在购买包年包月 Grafana 可视化服务后有任何不满意，我们支持以下退货退款。
- 您可享受**1台** Grafana 实例5天无理由退还，您支付的有效金额将返还到您的腾讯云账户。
- 除此之外，您还可享受普通退还，扣除您已使用的费用，退款金额将按购买支付使用的现金和赠送金支付比例退还至您的腾讯云账户。

以上均可在 [Grafana 可视化服务控制台](https://console.cloud.tencent.com/monitor/grafana) 自助操作，详情可参见 [自助退还操作](#step1)。
...
## 5天无理由退还
- 对于单个账号而言，Grafana 实例新购之日起5天之内（含5天），可支持**1台** Grafana 实例5天无理由退还。
...
## 普通退还
如果您已经享用5天无理由退还，我们还支持您的包年包月 Grafana 实例可在任意时间内通过控制台自助退还。
...
1. 登录 [Grafana 监控服务控制台](https://console.cloud.tencent.com/Grafana) ，在实例列表里选择要退还的实例，在页面右侧的操作一栏下选择**更多** > **实例状态** > **销毁/退还**。
2. 在弹出的对话框中进行退还确认。
