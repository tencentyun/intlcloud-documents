...
| 11 | Field rule | You can explicitly set the shardkey through `tcaplusservice.tcaplus_sharding_key`. In principle, it must be a subset of the intersection of all index key fields. If you do not explicitly set it, the system will use the intersection of the index key sets as the shardkey by default. The setting of the shardkey is subject to the backend data distribution. You need to evaluate whether the values of the field used as the shardkey are discrete. You are not recommended tp set a field with limited values such as gender and weekday as the shardkey. |
