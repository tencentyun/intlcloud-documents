...
- **读 CU**：以每秒1次4KB单行读操作为1个预留读 CU（Read Capacity Unit，RCU，[读能力单元](https://intl.cloud.tencent.com/document/product/1016/36557)）。
- **写 CU**：以每秒1次4KB单行写操作为1个预留写 CU（Write Capacity Unit，WCU，[写能力单元](https://intl.cloud.tencent.com/document/product/1016/36557)）。
...
| 硅谷     | 0.006289            | 0.002              | 0.0055              |
| 弗吉尼亚 | 	0.006289            | 0.002              | 0.0055              |
| 法兰克福 | 0.006            | 0.0022              | 0.0057              |
| 新加坡       | 0.0061            | 0.0025              | 0.0061              |
| 东京         | 0.0055            | 0.0019              | 0.0055             |
...
| ---------- | ------------ | ------------ | ------------------------------------------------------------ |
| 1          | 80           | 26           | (1GB × 0.0052美元/GB/天 + 80 RCU × 0.0019美元/RCU/天 + 26 WCU × 0.0048美元/WCU/天) × 1天 = 0.282美元 |

例如由于业务推广访问量增长，应用程序与该集群交互增多，读写访问频率上涨，当日峰值达到了1000 RCU/s、300 WCU/s，数据总量达到1.5GB。那么当日的费用应该为：

| 容量（GB） | 读请求（CU） | 写请求（CU） | 费用计算                                                     |
| ---------- | ------------ | ------------ | ------------------------------------------------------------ |
| 1.5        | 1000         | 300          | (1.5GB × 0.0052美元/GB/天 + 1000 RCU × 0.0019美元/RCU/天 + 300 WCU × 0.0048美元/WCU/天) × 1天 = 3.3478美元 |
...
| 弗吉尼亚 | 0.66               | 81.07696089                 |
| 硅谷     | 0.66               | 88.18275585                |
| 法兰克福 | 0.86285714              | 95.78907542                  |
| 新加坡       | 0.75714286                | 94.16675515                  |
| 东京         | 0.86285714              | 91.75491703                  |
...
接入层当日费用 = 0.5 × 4 = 2美元
存储层当日费用 = 64.28571429 × 2 = 128.57美元

当日总费用为：2美元 + 128.57美元 = 130.57美元
