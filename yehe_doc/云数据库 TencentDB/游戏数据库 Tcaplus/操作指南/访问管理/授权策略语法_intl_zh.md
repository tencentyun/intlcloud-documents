...
**region**：[地域信息](https://intl.cloud.tencent.com/document/product/213/6091)，如 ap-shanghai。若指定特定资源，则无需填写 region。
**account**：资源拥有者的主帐号信息，如 uin/164256472。
**resource**：各产品的具体资源详情，如集群为 cluster/19168929215 或者 cluster/\*；包括集群，表格组，表格是级联关系，下表描述了 TcaplusDB 能够使用的资源和对应的资源描述方法。


| 资源   | 授权策略中的资源描述方法                                     |
| ------ | ------------------------------------------------------------ |
| 集群    | qcs::tcaplusdb:$region:$account:cluster/$clusterId           |
| 表格组 | qcs::tcaplusdb:$region:$account:cluster/$clusterId/tablegroup/$tablegroupId |
| 表格    | qcs::tcaplusdb:$region:$account:table/$tableId |

例如，您可以使用特定集群（集群 ID 为19168929215）语句中指定它，如下所示：
```
"resource":[ "qcs::tcaplusdb::uin/164256472:cluster/19168929215"]
```

您还可以使用 * 通配符指定属于特定账户的所有集群，如下所示：
```
"resource":[ "qcs::tcaplusdb::uin/164256472:cluster/*"]
```
...
```
"resource":["qcs::tcaplusdb::uin/164256472:cluster/19168929215","qcs::tcaplusdb::uin/164256472:cluster/21168929215"]
```
