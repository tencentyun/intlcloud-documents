본 문서에서는 DTS의 데이터 마이그레이션 기능을 통해 전용 게이트웨이, VPN 게이트웨이로 자체구축한 MySQL에서 TencentDB for MySQL로 데이터를 마이그레이션하는 것에 대해 소개합니다. DTS는 구조 마이그레이션, 전체 데이터 마이그레이션 및 증분 데이터 마이그레이션을 지원하며, 서비스를 중단하지 않고도 TencentDB for MySQL로 원활하게 데이터를 마이그레이션할 수 있습니다.
...
- 마이그레이션 예정인 소스 자체구축 MySQL이 포함되어 있는 로컬 네트워크는 전용/VPN 게이트웨이를 통해 Tencent Cloud에 연결되어 있습니다.
...
-  증분 동기화: DTS는 전체 데이터 마이그레이션을 바탕으로 소스 자체구축 MySQL 데이터베이스의 binlog 정보를 읽어와 분석하고, 소스 자체구축 MySQL의 증분 업데이트를 타깃 TencentDB for MySQL로 동기화합니다. 증분 데이터 동기화를 통해 서비스를 중단하지 않고 자체구축 애플리케이션을 Tencent Cloud로 원활하게 마이그레이션할 수 있습니다.
...
<tr>
<td rowspan=10>소스 라이브러리 설정</td>
<td>소스 라이브러리 유형</td><td>‘MySQL’을 선택합니다. </td></tr>
<tr>
<td>서비스 제공업체</td><td>‘일반’을 선택합니다. </td></tr>
<tr>
<td>액세스 유형</td><td>‘전용 게이트웨이 액세스’ 또는 ‘VPN 엑세스’를 선택합니다. </td></tr>
<tr>
<td>소속 리전</td><td>소스 라이브러리 소속 리전은 DTS 서비스 아웃바운드 리전입니다. 자체구축 인스턴스에서 가장 가까운 리전을 선택하십시오. </td></tr>
<tr>
<td>VPC 전용 게이트웨이/ VPN 게이트웨이</td><td><br>전용 게이트웨이 액세스 시 VPC 전용 게이트웨이만 지원됩니다. 게이트웨이 연결 네트워크 유형을 확인해 주십시오. <br>VPN 게이트웨이, VPN 게이트웨이를 통해 액세스한 VPN 게이트웨이 인스턴스를 선택하십시오. </td></tr>
<tr>
