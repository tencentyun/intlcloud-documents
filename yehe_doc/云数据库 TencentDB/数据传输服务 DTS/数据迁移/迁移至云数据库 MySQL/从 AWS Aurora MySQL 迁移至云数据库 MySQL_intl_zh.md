...
本文主要介绍通过 DTS 数据迁移功能从 AWS Aurora MySQL 迁移数据至腾讯云数据库 MySQL。DTS 支持结构迁移、全量数据迁移以及增量数据迁移，可以实现在不停服的情况下，平滑迁移数据到腾讯云数据库 MySQL。
<span id="qttj"></span>
## 前提条件
- 已 [创建云数据库 MySQL](https://intl.cloud.tencent.com/document/product/236/37785)，支持版本：MySQL 5.6、MySQL 5.7。
...
-	增量同步：在全量数据迁移的基础上，DTS 会读取并解析源端 AWS Aurora MySQL 数据库的 binlog 信息，将源端 AWS Aurora MySQL 中的增量更新同步到目标云数据库 MySQL。通过增量数据同步完成应用在不停服的情况下平滑迁移到腾讯云。
...
```
上述命令将 binlog 日志的保存设置为24小时，最大可设置为168个小时，即7天。
-	AWS Aurora MySQL 的 binlog 日志需处于开启状态，且 binlog_format 需设置为row；MySQL 为5.6及以上版本时， binlog_row_image 需设置为 full。
...
<tr>
<td>服务提供商</td><td>选择“AWS”。</td></tr>
<tr>
<td>数据库版本	</td><td>选择 Aurora MySQL 5.6 或 Aurora MySQL 5.7。</td></tr>
<tr>
<td>接入类型</td><td>选择“公网”。</td></tr>
<tr>
<td>所属地域</td><td>源库所属地域为 DTS 服务出口地域，请选择离 AWS Aurora MySQL 最近的一个地域即可。</td></tr>
<tr>
<td>主机地址</td><td>AWS Aurora MySQL 访问 IP 地址，您可以在 AWS Aurora MySQL 的基本信息页面，获取数据库的访问地址。</td></tr>
<tr>
<td>端口</td><td>AWS Aurora MySQL 访问端口，默认为3306。</td></tr>
<tr>
<td>帐号</td><td>AWS Aurora MySQL 的数据库帐号，帐号权限需要满足要求。</td></tr>
<tr>
<td>密码</td><td>AWS Aurora MySQL 的数据库帐号的密码。</td></tr>
<tr>
