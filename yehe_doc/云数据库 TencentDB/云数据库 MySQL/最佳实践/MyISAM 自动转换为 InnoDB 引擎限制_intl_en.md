This document describes how to troubleshoot when a table creation error is reported after the MyISAM has automatically converted to InnoDB.

## Background
TencentDB for MySQL supports InnoDB storage engine by default, and MySQL 5.6 and later versions no longer support MyISAM or Memory engines. For more information, please see [Database Storage Engines](https://intl.cloud.tencent.com/document/product/236/9535).
...
## Solution
1. The table creation SQL statement that triggers an error is as follows:
```
...
2. Add an index, and the SQL statement should be modified as follows:
```
 create table t_complexkey
 ( 
 id int(8) AUTO_INCREMENT, 
...
3. Run the following command to show the table structure:
```
