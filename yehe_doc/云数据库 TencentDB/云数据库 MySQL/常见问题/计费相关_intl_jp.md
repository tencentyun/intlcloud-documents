...
MySQLはリージョンにより一定の無料バックアップスペースを贈与します。無料バックアップスペースの容量はユーザーの所在リージョンにおけるすべての高可用性エディションとファイナンスエディションのインスタンス（マスターインスタンス、ディザスタリカバリインスタンスを含む）の合計ストレージスペースです。 

フリークォータ以外のバックアップスペースについては、中国大陸は0.000127ドル/GB/時間で課金します。ほかのリージョンの価格については、[MySQL価格電卓](https://buy.cloud.tencent.com/price/cdb/calculator)をご参照ください。
