...
- **내부 네트워크 연결**: CVM을 통해 CDB의 내부 네트워크 주소에 직접 연결합니다. 이 방식은 내부 고속 네트워크를 사용하므로 네트워크 속도가 빠르고 딜레이가 낮습니다.
 - CVM과 데이터베이스의 계정이 동일해야 하며, 동일한 [VPC](https://intl.cloud.tencent.com/zh/document/product/215/535) 내(동일한 리전)에 있거나 기본 네트워크가 동일해야 합니다.
...
## Windows CVM에서 액세스
1. Windows CVM에 로그인한 뒤, <a href="https://intl.cloud.tencent.com/zh/document/product/213/10516" target="_blank">Windows CVM의 빠른 구성</a>을 참조 바랍니다.
...
5. MySQL Workbench를 연 다음, [Database]>[Connect to Database]를 선택하여, MySQL 데이터베이스 인스턴스의 내부 네트워크(혹은 외부 네트워크) 주소, 사용자 이름 및 비밀번호를 입력한 후 [확인]을 클릭해 로그인합니다.
 ...
 - Username: root로 기본 설정되어 있으며, 외부 네트워크로 연결했다면 간편한 관리 및 제어를 위해 단독 [계정 생성](https://intl.cloud.tencent.com/zh/document/product/236/31900)하시길 권장합니다.
 - Password: Username에 상응하는 비밀번호이며, 비밀번호를 잊어버렸을 경우 [비밀번호 재설정](https://intl.cloud.tencent.com/zh/document/product/236/31901)을 통해 변경할 수 있습니다.
...
## Linux CVM에서 액세스
1. Linux CVM에 로그인한 뒤, <a href="https://intl.cloud.tencent.com/zh/document/product/236/31901" target="_blank">Linux CVM의 빠른 구성</a>을 참조 바랍니다.
...
```
    ...
    2. 'Enter password:'가 출력된 후, MySQL 인스턴스 root 계정의 비밀번호를 입력합니다. 비밀번호를 잊어버렸을 경우 [비밀번호 재설정](https://intl.cloud.tencent.com/zh/document/product/236/31901)을 통해 변경할 수 있습니다.
...
```
      ...
      - username: 외부 네트워크로 연결한 사용자 이름으로 변경되며, 외부 네트워크로 연결했다면 간편한 관리 및 제어를 위해 단독 [계정 생성](https://intl.cloud.tencent.com/zh/document/product/236/31901)하시길 권장합니다.
    2. 'Enter password:'가 출력된 후, 외부 네트워크로 연결한 사용자 이름과 상응하는 비밀번호를 입력합니다. 비밀번호를 잊어버렸을 경우 [비밀번호 재설정](https://intl.cloud.tencent.com/zh/document/product/236/31901)을 통해 변경할 수 있습니다.
...
## 부록 2: 네트워크 연결성 인증 방법
telnet 명령어를 사용하여 빠르게 문제를 진단하고, 네트워크 연결성 문제를 파악하시길 바랍니다. 자세한 내용은 [telnet 명령어](https://intl.cloud.tencent.com/zh/document/product/236/31901)를 참조 바랍니다.

telnet으로 CDB의 네트워크 액세스가 정상임을 확인한 후에도, CVM에서 명령 라인을 통해 CDB에 로그인할 때 오류 메시지가 출력된다면, [인스턴스 연결 관련 문제](https://intl.cloud.tencent.com/zh/document/product/236/11278?from_cn_redirect=1#sytyzysjk)를 참조 바랍니다.

## 부록 3: 인스턴스 연결 불가 관련 문제
인스턴스 연결 불가와 관련된 문제가 발생했다면, [원클릭 연결성 진단 툴](https://intl.cloud.tencent.com/zh/document/product/236/11278?from_cn_redirect=1#sytyzysjk)을 사용하여 문제를 진단하고, 결과 보고에 따라 [인스턴스 연결 불가 관련 문제 해결]에서 적합한 솔루션을 찾아보시길 권장합니다.
