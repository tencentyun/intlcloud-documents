...
2. Select configuration items on the purchase page based on your actual needs, confirm that everything is correct, and click **Buy Now**.
 ...
 >The port 3306 needs to be opened for the MySQL instance through the inbound rule of the security group. The MySQL instance uses private network port 3306 by default and supports customizing the port. If the default port is changed, the new port should be opened in the security group.
