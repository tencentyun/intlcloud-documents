...
インスタンス価格計算式：インスタンス価格=メモリ規格費用+ストレージスペース費用。



## インスタンス仕様（メモリ容量）価格
> この製品のサブスクリプションモード課金モードは内部テスト中です。価格ドキュメントは参考として、最終価格は請求書をご参照ください。購入のご希望がある場合、[業務担当者までお問い合わせ](https://intl.cloud.tencent.com/contact-sales)ください。
>
### サブスクリプション料金
...
<th colspan="4" style="text-align: center;">月額料金（USD/月）</th>
</tr>
</thead>
<tbody><tr>
<td>構成タイプ</td>
<td>データベースタイプ</td>
<td>インスタンス仕様</td>
<td align="center">上海、広州、北京、南京、清遠</td>
<td>成都、重慶</td>
<td>シンガポール、香港（中国）、台北（中国）、トロント、ムンバイ</td>
<td>東京、ソウル、ロシア、バンコク、フランクフルト、シリコンバレー、バージニア</td>
</tr>
<tr>
<td>高可用性エディション-マスターインスタンス</td>
<td>MySQLマスターインスタンス</td>
<td>1コア、1000MBのメモリ</td>
...
<tr>
<td>シングルノード高IOエディション</td>
<td>MySQL読取専用インスタンス</td>
<td>1コア、1000MBのメモリ</td>
...
1. 上記の価格はインスタンス仕様の構成費用のみであり、ストレージスペース、バックアップスペース、帯域幅の価格は含まれていません。
3.この表のすべての価格は新規購入価格であり、更新と構成調整時の価格は異なる場合があります。
4.公式価格は状況に応じて適切に調整する場合があります。データが長期的に有効なわけではなく、詳細な価格については公式サイトをご参照ください。
...
<th colspan="5" style="text-align: center;">月額料金/USD/GB/月</th>
</tr>
</thead>
<tbody><tr>
<td>データベースタイプ</td>
<td>上海、広州、北京、南京、清遠</td>
<td>成都、重慶</td>
<td>香港（中国）、台北（中国）、シンガポール、ソウル、トロント、ムンバイ、バンコク</td>
<td>フランクフルト、シリコンバレー、バージニア</td>
<td>東京、モスクワ</td>
</tr>
<tr>
<td>高可用性エディション インスタンス</td>
<td>0.101408451</td>
<td>0.101408451</td>
<td>0.169014085</td>
<td>0.112676056</td>
...
1. 上記の価格はストレージスペースの費用のみであり、インスタンス仕様と帯域幅の価格は含まれていません。
2.この表のすべての価格は新規購入価格であり、更新と構成調整時の価格は異なる場合があります。
3.公式価格は状況に応じて適切に調整する場合があります。データが長期的に有効なわけではなく、詳細な価格については公式サイトをご参照ください。
...
バックアップスペースはサブスクリプション課金モードをサポートしていません。従量課金制の価格をご参照ください。
...
#### 課金計算式
**総費用 = メモリ規格費用 + ストレージスペース費用 + バックアップスペース費用 + トラフィック費用（今現在無料**
...
<tr>
<td>ストレージスペース費用</td>
<td>【購入】ページで選択したディスク容量は、従量課金の階層価格を適用します。価格の詳細については、<a href="https://buy.cloud.tencent.com/price/cdb/overview" target="_blank">製品の価格設定</a>をご参照ください。</td>
</tr>
<tr>
<td>バックアップスペース費用</td>
<td>TencentDB for MySQLは、リージョンに基づいて一定の無料バックアップスペースを提供します。無料バックアップスペースの大きさはユーザーの所在地域におけるすべての高可用性インスタンス（マスターインスタンス、ディザスタリカバリインスタンスを含む）のストレージスペースの合計とします。<br>無料のバックアップスペースを超える場合の費用については、<a href="https://intl.cloud.tencent.com/document/product/236/32344" target="_blank">バックアップスペースの費用に関する説明</a>をご参照ください。</td>
</tr>
<tr>
<td>トラフィック費用</td>
...
### サブスクリプション課金
広州地域の場合：
- 広州三区：4コア、8000MBのメモリ、500GBのディスク、有効期間1ヶ月のサブスクリプション課金モードのMySQL（高可用性エディション）を購入しました。
- 広州四区：4コア、8000MBのメモリ、200GBのディスク、有効期間1ヶ月のサブスクリプション課金モードのMySQL（高可用性エディション）を購入しました。

総費用 = メモリ規格費用 + ストレージスペース費用 + バックアップスペース費用

##### メモリ規格費用 + ストレージスペース費用
費用は次のように計算します。
メモリ規格費用 + ストレージスペース費用 = 2 × 816元/月 + (500GB + 200GB) × 0.72元/GB/月 × 1ヶ月 = 2136元
...
顧客は広州地域での合計バックアップストレージスペースが700Gを超えた場合（例えば、データバックアップは800G、ログバックアップは100Gである）、課金スペース = 800 + 100 - 700 = 200GBとなります。現時点でユーザーがこの200GBのためにバックアップスペース費用を支払う必要があります。今後ストレージスペースを超えた場合も同様とします。

バックアップスペース料金（時間）= 200GB（バックアップスペースを超えると実際のシナリオによって変化することがあります）× 0.0008元/GB/時間

### 従量課金
広州地域で、4コア、8000MBのメモリ、500GBのディスク、有効期限400時間の従量課金MySQLインスタンスを購入した場合：
第1階層料金：（0.0250 USD/GB/時間×8GB+500GB × 0.0003）×96時間=33.6 USD
第2階層料金：（0.0200 USD/GB/時間×8GB+500GB × 0.0003）×264時間=81.84 USD
第3階層料金：（0.0150 USD/GB/時間×8GB+500GB × 0.0003）×40時間=10.8 USD
インスタンス料金 = 第1階層料金 + 第2階層料金 + 第3階層料金 = 126.24 USD

## よくある質問
#### 私のインスタンスはサブスクリプション課金モードですが、他の料金がかかっているのは風邪ですか。
バックアップスペースが無料の限度額を超えたか確認してください。無料の限度額を超えたバックアップスペースは課金されます。
バックアップスペースの使用情報は、[MySQLコンソール](https://console.cloud.tencent.com/cdb/backup)のデータベースバックアップページで表示できます。バックアップスペース課金の詳細については、[バックアップスペース課金について](https://cloud.tencent.com/document/product/236/36263)をご参照ください。
