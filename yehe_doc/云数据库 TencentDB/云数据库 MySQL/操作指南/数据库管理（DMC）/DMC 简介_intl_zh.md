## 概述
DMC（Database Management Console）是腾讯云的一款数据库管理工具，提供库表级操作、实例会话管理、实时监控、InnoDB 锁等待、SQL 窗口为一体的数据库管理服务。

目前支持云数据库 MySQL 、云数据库 MariaDB 及云原生数据库 TDSQL-C（兼容 MySQL 版）。此外，DMC 还内嵌了 PhpMyAdmin（PMA）管理工具，以便用户对数据库进行快捷、高效的管理操作。

DMC 有如下两版控制台：
- DMC 管理（旧版）：提供库表级操作、实例会话、实时监控、InnoDB 锁等待等功能。
- DMC 管理（新版）：提供库表级操作、实例监控、实例会话、SQL 窗口等功能。
