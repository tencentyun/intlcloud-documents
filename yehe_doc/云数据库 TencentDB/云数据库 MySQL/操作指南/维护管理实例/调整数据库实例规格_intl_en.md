...
>?You can view disk space on the instance details page in the [MySQL console](https://console.cloud.tencent.com/cdb), or receive disk alarms with the [Alarm Policies (Cloud Monitor)](https://intl.cloud.tencent.com/document/product/236/8457).
...
- You can adjust the configuration of a TencentDB for MySQL instance and its associated read-only replicas only when they are in normal status (running) and are not executing any tasks.
...
It may be because the used storage capacity has exceeded the maximum capacity of the hard disk. To downgrade you instance, you need to clean up data first and make sure the remaining available capacity accounts for more than 20% of the total capacity or over 50 GB.
...
The time it takes depends on the data volume, data replication, and read requests of your instance.
