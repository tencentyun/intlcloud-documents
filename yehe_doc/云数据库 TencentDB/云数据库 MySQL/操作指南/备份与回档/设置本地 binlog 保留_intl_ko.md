...
- 생성된 binlog는 시스템의 [자동 백업 기능](https://intl.cloud.tencent.com/document/product/236/37796)을 통해 COS에 백업됩니다. 백업 후의 binlog는 설정된 로컬 binlog 보관 정책에 따라 삭제되며, 사용 중인 binlog는 오류 방지를 위해 삭제할 수 없습니다. 이에 따라 정리 과정에서 일정 수준의 딜레이가 발생합니다. 설정 후 조금만 기다려 주십시오.

## 작업 순서
1. [MySQL 콘솔](https://console.cloud.tencent.com/cdb)에 로그인한 후, 인스턴스 리스트 페이지에서 인스턴스 이름을 클릭하여 인스턴스 관리 페이지로 이동합니다.
2. 인스턴스 관리 페이지에서 [백업 복구] 페이지를 선택하고, [로컬 Binlog 설정]을 클릭합니다.
3. 팝업된 대화 상자에서 보관 시간 및 용량 사용률을 입력하고, 오류가 없는지 확인한 다음 [확인]을 클릭합니다.
>?가용 용량 보호를 활성화하면, 사용률이 80%를 초과하거나 남은 용량이 5GB 미만인 경우 binlog가 강제 삭제됩니다. 총 용량의 사용률이 80% 이하로 떨어지고 남은 용량이 5GB 이상이 될 때까지 가장 오래된 Binlog부터 차례로 삭제됩니다.
>
...
로컬 binlog 기본 보관 시간은 120시간이며, 기본 용량 사용률은 30% 이하입니다. 가용 용량 보호 기능은 기본적으로 활성화되어 있습니다.
