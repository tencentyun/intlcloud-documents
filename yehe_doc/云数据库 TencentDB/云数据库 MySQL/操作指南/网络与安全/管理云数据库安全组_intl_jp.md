## 操作シナリオ
[セキュリティグループ](https://intl.cloud.tencent.com/zh/document/product/213/12452) は、フィルタリング機能を含んだ、一種のバーチャルファイアウォールの状態です。単体または複数のクラウドデータベースのネットワークアクセス制御を設定するために使用します。また、ネットワークセキュリティを隔絶させるための、Tencent Cloudが提供する重要な手段になります。セキュリティグループは論理上グループ分けしたもので、同一リージョン内で同じネットワークセキュリティを隔絶させたいというニーズがあるクラウドデータベースインスタンスを、同じセキュリティグループ内に追加できます。クラウドデータベースはCVMなどとセキュリティグループリストをシェアして、セキュリティグループ内でルールに基づいてマッチングします。ルールと制限の詳細は [セキュリティグループ詳細説明](https://intl.cloud.tencent.com/document/product/215/38750)をご参照ください。

>?
>- TencentDB for MySQLのセキュリティグループは現在Virtual Private Cloud（VPC）プライベートネットワークアクセスと外部ネットアクセスによるネットワーク制御にのみサポートしており、基本ネットワークのネットワーク制御はサポートしていません。
>- 広州、上海、中国香港、北京、成都、重慶、ソウル、東京リージョンのみデータベース外部ネットワークアクセスセキュリティグループ機能に対応していて、その他のリージョンは対応していません。
>- クラウドデータベースは自発的にトラフィックをアウトバウンドしていないことから、アウトバウンドルールはクラウドデータベースに対して有効にはなりません。
...
3. ポップアップダイアログボックスで、次の設定を完了し、確認後に【OK】をクリックします。
 - テンプレート：セキュリティグループの中のデータベースインスタンスがデプロイする必要があるサービスを基に、適切なテンプレートを選択し、セキュリティグループルールの構成を簡素化します。次の表で示します。
<table>
...
<table>
	...
	<tr><td>単一のIPv6アドレスまたはIPv6アドレス範囲</td><td>CIDRで表記（例えば、<code>FF05::B5</code>、<code>FF05:B5::/60</code>、<code>::/0</code>または<code>0::0/0</code>。この内、<code>::/0</code>または<code>0::0/0</code>はすべての IPv6 アドレスに一致することを示しています）。</td></tr>
	<tr><td>セキュリティグループIDの参照。次のセキュリティグループのIDを参照できます：<ul  style="margin: 0;"><li>セキュリティグループ ID</li><li>その他のセキュリティグループ</li></ul>
</td><td><ul  style="margin: 0;"><li>現在のセキュリティグループは、セキュリティグループと関連するCVMを示します。</li><li>その他のセキュリティグループは、同一リージョンの同一プロジェクト内のもう一つのセキュリティグループIDであることを示します。</li></ul>
</td></tr>
	<tr><td>参照 <a href="https://intl.cloud.tencent.com/document/product/215/31867">パラメータテンプレート</a> における IPアドレスオブジェクトまたはIPアドレスグループオブジェクト</td><td>-</td></tr>
</table>
 - プロトコルポート：プロトコルタイプとポート範囲を入力。 [パラメータテンプレート](https://intl.cloud.tencent.com/document/product/215/31867) のプロトコルポートまたはプロトコルポートグループの参照もできます。
  >?TencentDB for MySQLとの接続では、3306プロトコルポートが開かれていること。
 - ポリシー：デフォルトでは「許可」を選択しています。
