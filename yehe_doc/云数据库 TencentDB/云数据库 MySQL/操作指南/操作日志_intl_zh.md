...
您可以在控制台的操作日志页面查看实例的慢日志明细、错误日志明细、回档日志，及下载慢日志。也可以在命令行界面（CLI）或者使用云数据库 API 查看、下载数据库日志，请参见 [查询备份日志](https://intl.cloud.tencent.com/document/product/236/15842) 与 [查询二进制日志](https://intl.cloud.tencent.com/document/product/236/15843)。
...
## 操作步骤
1. 登录 [云数据库 MySQL 控制台](https://console.cloud.tencent.com/cdb)，在实例列表中，单击实例名或“操作”列的【管理】，进入实例管理页面。
2. 选择【操作日志】页，可选择查看实例的慢日志明细、错误日志明细、回档日志，及下载慢日志。
<table>
<thead>
<tr>
<th>功能项</th>
...
3. 在【慢日志下载】页，您可单击操作列的【下载】下载慢日志。
4. 在弹出的对话框中，选择如下任一方式下载：
>?若日志大小为0KB，则无法下载。
>
方式一：单击【本地下载】直接下载。
方式二：复制下载地址，并登录到与该云数据库在同一 VPC 下的 Linux 云服务器中，运用 wget 命令进行下载。
