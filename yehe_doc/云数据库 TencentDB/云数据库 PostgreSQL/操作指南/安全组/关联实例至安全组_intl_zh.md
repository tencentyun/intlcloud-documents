...
## 为主实例/只读实例绑定安全组
1. 登录 [PostgreSQL 控制台](https://console.cloud.tencent.com/postgres)，在实例列表，单击需要绑定安全组的实例名，进入实例管理页面。
...
## 为只读实例组绑定安全组
1. 登录 [PostgreSQL 控制台](https://console.cloud.tencent.com/postgres)，在实例列表，找到只读实例组管理的只读实例，单击实例名，进入只读实例管理页面。
...
1. 登录 [PostgreSQL 控制台](https://console.cloud.tencent.com/postgres)，在实例列表，单击实例名，进入实例管理页面。
2. 在实例管理页面，选择【安全组】页。
