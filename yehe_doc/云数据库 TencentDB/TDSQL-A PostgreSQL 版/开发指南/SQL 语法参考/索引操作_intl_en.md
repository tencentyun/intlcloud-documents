...
CREATE INDEX
```
You cannot create a unique index for non-shardkey fields.

#### Unique index must carry shardkey
```
...
CREATE INDEX
postgres=# INSERT INTO t_sex SELECT t,'Male' FROM generate_series(1,1000000) as t;           
INSERT 0 1000000
postgres=# INSERT INTO t_sex SELECT t,'Female' FROM generate_series(1,100) as t;           
INSERT 0 100
postgres=# ANALYZE t_sex ;
ANALYZE
postgres=# EXPLAIN SELECT * FROM t_sex WHERE sex ='Female';      
                  QUERY PLAN                  
----------------------------------------------------------------------------------
 Remote Fast Query Execution (cost=0.00..0.00 rows=0 width=0)
  Node/s: dn001, dn002
...
\# The index does not take effect if `sex = 'Male'` is configured
postgres=# EXPLAIN SELECT * FROM t_sex WHERE sex ='Male';   
              QUERY PLAN               
-------------------------------------------------------------------
 Remote Fast Query Execution (cost=0.00..0.00 rows=0 width=0)
  Node/s: dn001, dn002
  -> Seq Scan on t_sex (cost=0.00..9977.58 rows=500539 width=8)
     Filter: (sex = 'Male'::text)
(4 rows)

\# Connect to DN, and you can see that index occupies large space and has high intensity
...
DROP INDEX
postgres=# CREATE INDEX t_sex_sex_idx on t_sex (sex) WHERE sex='Female'; 
CREATE INDEX
postgres=# ANALYZE t_sex;
ANALYZE
postgres=# EXPLAIN SELECT * FROM t_sex WHERE sex ='Female';    
...
CREATE INDEX
```
The column storage mode does not support GiST indexes.
...
CREATE INDEX
```
The column storage mode does not support GIN indexes.
...
CREATE INDEX
```
The column storage mode does not support JSONB indexes.
...
Time: 2.980 ms
```
The column storage mode does not support array indexes.

### Index of any field in `btree_gin`
```
...
# Combined query of three fields
```
...
#### Notes on multi-field index usage
The `or` query condition `bitmap scan` supports up to two different field conditions.
```
...
If all returned fields are in the index file, only the index needs to be scanned, which reduces the I/O overheads.
```
...
Time: 1.564 ms
```
Its update performance is better than that of a single-field multi-index file.
Single-Field:
```
...
A multi-field index has lower performance than an independent single-field index when querying a non-first field.
Multiple-Field:
```
