...
>?全实例监控功能目前支持云数据库 MySQL（不含基础版）、云原生数据库 TDSQL-C（TDSQL-C for MySQL）。
...
## 切换监控指标
1. 登录 [DBbrain 控制台](https://console.cloud.tencent.com/dbbrain/analysis)，在左侧导航选择【监控告警】>【智能监控】，在上方选择对应数据库，然后选择【全实例监控】页。
2. 通过上方的下拉菜单搜索控件，选择需要查看的监控指标，支持 MySQL/TDSQL-C 云监控提供的所有监控指标，全实例监控视图根据所选指标排序展示。
...
## 搜索实例
1. 登录 [DBbrain 控制台](https://console.cloud.tencent.com/dbbrain/analysis)，在左侧导航选择【监控告警】>【智能监控】，在上方选择对应数据库，然后选择【全实例监控】页。
2. 在全实例监控页，选择 MySQL 数据库时，可按实例 ID、实例名和内网 IP 进行模糊搜索；选择 TDSQL-C 数据库时，可按集群 ID、集群名、实例 ID、实例名和接入点地址进行模糊搜索。
