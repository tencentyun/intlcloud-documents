...
>?`metric` 命名限制请参见 [系统限制](https://intl.cloud.tencent.com/document/product/1100/40910)。
...
| time     | 否   | map  | 时间列相关配置，用于存储数据入库的唯一时间，例如`{"name": "timestamp", "format":  "epoch_second"}`，若不填则系统默认格式为`{"name": "timestamp", "format":  "epoch_millis"}` |
