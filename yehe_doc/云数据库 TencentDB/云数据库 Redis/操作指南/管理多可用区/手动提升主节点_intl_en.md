...
A multi-AZ deployed instance in the standard architecture can have only one master node. You can promote a replica node to master node, and the master node will be automatically demoted to replica node simultaneously.

Process of promoting to master node/AZ:
...
Process of promoting to master node group:
1. Promote all nodes in the node group to master nodes. (If all of the nodes are already master nodes, skip the step.)
2. Promote the node group to master node group. Note that if a server-level failover occurs after the promotion is completed, some of the nodes in the node group will be demoted to replica nodes and promoted back to master nodes.
...
- Promoting nodes will trigger a master-replica switch. During the switch, database access will be affected a few seconds to 3 minutes, and BLPOP, BRPOP, BRPOPLPUSH, SUBSCRIBE, and other blocking commands may fail one or more times before they can be executed successfully.
- If the promotion fails, please try again.
- After promoting a replica node to master node, your business may access resources across multiple AZs, increasing the service response latency and decreasing [QPS](https://intl.cloud.tencent.com/document/product/239/31950#qps.2F.E5.B9.B6.E5.8F.91).
