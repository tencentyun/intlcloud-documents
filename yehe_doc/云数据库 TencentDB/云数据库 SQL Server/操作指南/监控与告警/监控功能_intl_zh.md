...
|请求数<br>（Batch Requests/sec）|平均每秒的请求数（次/秒）|max|参考业务需要|
...
|磁盘 IOPS<br>（IOPS）|磁盘读写次数（次/秒）|max|<br>< 30% 好<br>< 60% 正常<br>> 60% 需关注|
...
|每秒钟 SQL 做全表扫描数目<br>（Full Scans/sec）|磁盘读写次数（次/秒）|avg|< 30% 好<br>< 60% 正常<br>> 60% 需关注|
