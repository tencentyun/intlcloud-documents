## Exporting Data
TDSQL supports exporting data with mysqldump. Before export, you need to set the `net_write_timeout` parameter (`set global net_write_timeout=28800`) in the [TDSQL Console](https://console.cloud.tencent.com/dcdb). Note that command line utilities are not authorized by TDSQL, so you can set the parameter only in the console.
```
