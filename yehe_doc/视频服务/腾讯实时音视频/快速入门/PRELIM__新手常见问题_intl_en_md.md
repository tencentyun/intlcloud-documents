...
- In the call mode, each room can accommodate up to 300 concurrent users, and up to 50 of them can turn on their cameras or mics.
...
- **Real-time communication** (keywords: multi-person interaction, UDP-based proprietary protocol, low latency)
 The main application scenarios for real-time communication (RTC) are audio/video interaction and low-latency live streaming. It uses UDP-based proprietary protocols and can keep the latency as low as 100 ms. Typical applications include QQ calls, VooV Meeting, and online group classes. Tencent Real-Time Communication (TRTC) is supported by mainstream platforms including iOS, Android, Windows, and WeChat Mini Program. It can communicate with WebRTC and relay live streams to CDNs through on-cloud stream mixing.
...
- You can find the latest version number of the SDK on the release notes page.
  ...
  - For desktop browsers, please see [Release Notes (Desktop Browser)](https://intl.cloud.tencent.com/document/product/647/39779).
