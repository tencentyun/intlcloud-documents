...
TRTCMeeting는 오픈 소스 Class로, Tencent Cloud의 두 가지 클로즈드 소스 SDK에 종속됩니다. 자세한 구현 방법은 [다중 사용자 화상 회의(Android)](https://intl.cloud.tencent.com/document/product/647/37283)를 참조하십시오.
...
| [muteRemoteAudio](#muteremoteaudio) | 특정 사용자 오디오 차단 |
| [muteRemoteVideoStream](#muteremotevideostream) | 특정 사용자 비디오 스트림 차단 |
...
| [setVideoBitrate](#setvideobitrate) | 비트레이트 설정 |
...
[TRTCMeeting](https://intl.cloud.tencent.com/document/product/647/37283) 단일 항목 객체를 가져옵니다.
```java
...
[TRTCMeeting](https://intl.cloud.tencent.com/document/product/647/37283) 단일 항목 객체를 폐기합니다.
>?인스턴스 폐기 후에는 외부에 캐시된 TRTCMeeting 인스턴스를 다시 사용할 수 없으며, 다시 [sharedInstance](#sharedInstance)를 호출해 새로운 인스턴스를 획득해야 합니다.
...
1. [주최자]가 `createMeeting()`을 호출하여 회의를 생성하면, 회의 방 생성 여부가 ActionCallback을 통해 주최자에게 통지됩니다.
...
원격 비디오 화면을 재생을 중단합니다. `onUserVideoAvailable()`이 false로 콜백되면 해당 인터페이스를 호출합니다.
```java
...
|-----|-----|-----|
| userId | String | 사용자 ID |
| fillMode | int  | 화면 꽉 채우기 또는 원본에 맞추기 모드가 있으며, 기본값은 화면 꽉 채우기(FILL)입니다. 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloud__android.html#ab4197bc2efb62b471b49f926bab9352f)를 참조하십시오. |
...
|-----|-----|-----|
| userId | String | 사용자 ID |
| rotation | int  | 시계 방향 회전 각도이며, 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloud__android.html#a87fd1307871debc7c051de4878eb6d69)를 참조하십시오. |
...
| 매개변수 | 유형 | 의미 |
|-----|-----|-----|
| resolution | int | 비디오 해상도이며, 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloudDef__android.html#aa3b72c532f3ffdf64c6aacab26be5f87)를 참조하십시오. |
...
| 매개변수 | 유형 | 의미 |
|-----|-----|-----|
| bitrate | int | 비트 레이트입니다. SDK는 타깃 비트레이트에 따라 인코딩하며, 네트워크가 불안정한 상태에서만 자체적으로 비디오 비트레이트를 줄입니다. 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloudDef__android.html)를 참조하십시오. |
...
| 매개변수 | 유형 | 의미 |
|-----|-----|-----|
| type | int | 화면 모드로, 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloud__android.html#aa353b5cf5662c43252eb8e5132f041c1)를 참조하십시오. |
...
| 매개변수 | 유형 | 의미 |
|-----|-----|-----|
| mute | boolean | 오디오를 음소거/음소거 취소합니다. 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloud__android.html#a37f52481d24fa0f50842d3d8cc380d86) 참조하십시오. |
...
| 매개변수 | 유형 | 의미 |
|-----|-----|-----|
| trtcAudioRecordingParams | TRTCCloudDef.TRTCAudioRecordingParams | 녹음 매개변수 입니다. 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloudDef__android.html#classcom_1_1tencent_1_1trtc_1_1TRTCCloudDef_1_1TRTCAudioRecordingParams)를 참조하십시오. |
...
>? 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloud__android.html#aa6671fc587513dad7df580556e43be58)를 참조하십시오.
...
>? 자세한 내용은 [TRTC SDK](http://doc.qcloudtrtc.com/group__TRTCCloudListener__android.html#aba07d4191391dadef900422521f34e5b)를 참조하십시오.
...
| 매개변수 | 유형 | 의미 |
|-----|-----|-----|
| userId | String | 새로 방에 입장한 사용자 ID |
