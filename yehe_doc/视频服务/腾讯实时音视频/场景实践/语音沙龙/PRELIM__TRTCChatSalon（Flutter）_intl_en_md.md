...
- The room owner can approve a listener’s request to speak as well as remove a speaker.
...
| [getRoomMemberList](#getroommemberlist) | Gets information of all users in room. |
| [getArchorInfoList](#getarchorInfolist) | Gets speaker list in room. |
| [getUserInfoList](#getuserinfolist) | Gets user information of specified `userId`. |
...
| [agreeToSpeak](#agreetospeak)   | Approves request to speak (called by room owner) |
| [refuseToSpeak](#refusetospeak) | Rejects request to speak (called by room owner) |
...
## Mic-on Request Signaling Event Callbacks

| API                                             | Description                                                         |
| -------------------------------------- | ---------------------------------------- |
| [onRaiseHand](#onraisehand) | A listener requested to speak. |
| [onAgreeToSpeak](#onagreetospeak)   | The room owner approved the listener’s request to speak. |
...
This API is used to terminate the `TRTCChatSalon` singleton object.
...
| Parameter    | Type   | Description                                                                                                                    |
| --------- | --------- | ------------------------------------------------------------ |
| roomId | int | Room ID. You need to assign and manage the IDs in a centralized manner. Multiple `roomID` values can be aggregated as a chat salon room list. Currently, Tencent Cloud does not provide chat salon room list management services. Please manage the list on your own. |
...
2. The user selects a chat salon, and call `enterRoom` with the room ID passed in to enter.
3. After room entry, the user can call `getArchorInfoList` to get the speaker list and `getRoomMemberList` the user list. The user list minus the speaker list is the listener list.
...
This API is used to get the list of anchors in the room.
...
| Parameter    | Type   | Description                                                                                                                    |
| ---------- | ------------------ | ------------------------------------------------------------ |
| userIdList | List | List of user IDs to obtain. If this parameter is `null`, the information of all users in the room is obtained. |
...
Calling this API will immediately modify the seat list. The viewer needs to call `raiseHand` first to apply to the anchor and call this API after receiving `onAgreeToSpeak`.
...
This API is used to send a request to speak.
...
This API is used to approve a request to speak (called by room owner).
...
Another user logged in to the same account, and the current user was kicked out.
...
| mute       | bool   | Seat status. Default value: unmuted. |
...
Whether the anchor muted a speaker.
...
| Parameter        | Type    | Description                                                                                                      |
| ---------- | ------ | -------------- |
| userId | String | ID of the listener who entered |
| userName | String | Nickname |
| userAvatar | String | Profile photo address |
...
## Raise-Hand Signaling Event Callbacks
...
The room owner approved the request to speak.
