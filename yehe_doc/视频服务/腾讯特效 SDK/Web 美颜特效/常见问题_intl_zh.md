...
可以，SDK 可以作为一个中间渲染处理器，支持多种输入和输出源。
...
一般是因为签名已过期，请重新计算直播推流签名，推流 URL 拼装方法请参见 [自主拼装直播 URL](https://intl.cloud.tencent.com/document/product/267/38393)。

[](id:q7)
### getEffectList 拉取素材资源时报错是什么原因？
一般是调用时机错误导致的报错，注意**在 SDK `created` 回调或之后调用 `getEffectList`，此时就可以根据素材数据完成业务交互逻辑**。
