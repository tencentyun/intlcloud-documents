...
2. Demo의 SDK 파일을 실제로 사용하는 SDK용 파일로 교체합니다. 구체적 작업은 다음 단계를 따르십시오.
   ...
   - xmagic 모듈의 `../src/main/jniLibs`에 있는 모든 .so 파일을 SDK 패키지의 jniLibs에 있는 .so 파일로 교체합니다(arm64-v8a 및 armeabi-v7a에 대한 .so 파일을 가져오려면 jinLibs 폴더에 있는 ZIP 파일의 압축을 풀어야 합니다).
...
[](id:step3)
## 3단계: SDK 인터페이스 통합
Demo 프로젝트의 TCVideoRecoredActivity 클래스를 참고하십시오.
1. **인증:**
```java
 //인증 시 주의사항 및 오류 코드 내용은 https://intl.cloud.tencent.com/document/product/1143/45385을(를) 참고하십시오.
...
VideoCustomProcessListener 인터페이스의 `onTextureCustomProcess(int textureId, int width, int height)` 메소드에 다음 코드를 추가합니다.
```java
if (isPause == 0 && mXMagic !=null) {
    return mXMagic.process(textureId, width, height);
}
...
```
6. **SDK 뷰티 필터 패널을 호스팅하는 레이아웃을 레이아웃에 추가합니다.**
```xml
 <RelativeLayout
...
자세한 내용은 Demo 프로젝트의  `TCVideoRecordActivity.initXMagic();` 메소드를 참고하십시오.
