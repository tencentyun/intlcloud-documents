...
腾讯实时音视频提供了完备的云端录制能力，可以通过旁路推流使用 [云直播](https://intl.cloud.tencent.com/document/product/267) 的能力为您提供全程的云端录制功能，并将录制下来的文件存储到 [云点播](https://intl.cloud.tencent.com/document/product/266) 平台，方便用户进行回放。
...
- **多路音视频录制成一个文件**
即将房间中的多路音视频先进行 [云端混流](https://intl.cloud.tencent.com/document/product/647/34618)，再录制混合后的画面，实现“一次通话（或一场直播）只生成一个文件”。
...
在多人会议和在线教育等场景下，可能更期望将一堂课程（或一次会议）只录制出一个文件，并且需要该文件中包含所有用户的画面和声音。您可以开启录制的同时，使用 [云端混流](https://intl.cloud.tencent.com/document/product/647/34618) 服务将同房间中的 userA、userB 和 userC 的画面和声音混合成一路 （A + B + C），录制过程请参见 [原理解析](#mixRecord)。
...
### 步骤6：删除录制文件
腾讯云点播系统提供了一系列 REST API 来管理其上的音视频文件，您可以通过 [删除媒体 API](https://intl.cloud.tencent.com/document/product/266/31764) 删除某个指定的文件。
REST 请求示例：
```
...
- [Web 浏览器](https://intl.cloud.tencent.com/document/product/266/14424)
