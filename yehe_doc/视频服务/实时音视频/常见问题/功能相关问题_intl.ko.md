...
라이브 방송 시나리오에서 호스트와 시청자의 역할을 설정할 수 있습니다. 호스트 역할 TRTCRoleAnchor은 멀티미디어 업스트림/다운스트림 권한을 가지고 있으며 최대 50명까지 동시 접속을 지원합니다. 시청자 TRTCRoleAudience는 멀티미디어 다운스트림 권한만을 가지고 있으며 동시 접속 가능 인원은 최대 10만 명입니다.
...
- TRTCAppSceneLIVE: 비디오 저딜레이 라이브 방송, 10만 인터랙션 강의, 비디오 라이브 방송 PK, 화상 소개팅, 인터랙션 강의, 원격 교육, 초대형 회의 등에 적합한 비디오 ILVB입니다.
- TRTCAppSceneAudioCall: 일대일 음성 통화, 300인 음성 회의, 음성 채팅, 음성 회의, 온라인 마피아 게임 등에 적합한 음성 통화 시나리오입니다.
- TRTCAppSceneVoiceChatRoom: 음성 저딜레이 라이브 방송, 음성 라이브 방송 마이크 연결, 음성 채팅, 노래방, FM 라디오 등에 적합한 음성 ILVB입니다.


[](id:que9)
### TRTC에서 지원하는 플랫폼에는 어떤 것이 있나요?
iOS, Android, Windows(C++), Windows(C#), Mac, 데스크톱 브라우저, Electron, WeChat 미니프로그램과 같은 플랫폼을 지원합니다. 자세한 내용은 [플랫폼 지원](https://intl.cloud.tencent.com/document/product/647/35078)을 참조하십시오.
...
- [데스크톱 브라우저 > join](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/Client.html#join)
...
릴레이 라이브 방송을 활성화하고자 할 경우, 중국 당국의 규정에 따라 ICP비안이 있어야 재생 도메인을 사용할 수 있습니다. 자세한 내용은 [CDN 라이브 방송 시청](https://intl.cloud.tencent.com/document/product/647/35242)을 참조하십시오.

[](id:que17)
### TRTC는 대략 어느 정도 딜레이되나요?
글로벌 end to end로 평균 300ms 미만입니다.

[](id:que18)
### TRTC는 액티브 콜 기능을 지원하나요?
신호 터널을 결합해야 합니다. 예를 들어 [인스턴트 메시지 IM](https://intl.cloud.tencent.com/zh/document/product/1047) 서비스의 사용자 정의 메시지를 사용하여 호출할 수 있습니다. [SDK](https://intl.cloud.tencent.com/document/product/647/34615) 소스 코드의 시나리오 Demo 사례를 참조하십시오.
...
자체 서버 녹화(녹음/녹화)를 지원합니다. 필요한 경우 [Submit Ticket](https://console.cloud.tencent.com/workorder/category)을 통해 고객센터로 문의하시면 SDK 및 관련 가이드를 제공합니다.
...
- 라이브 방송 모드에서는 단일 방에 최대 10만 명의 시청자가 온라인 시청을 할 수 있으며, 최대 50명의 호스트가 카메라 또는 마이크를 활성화할 수 있습니다.


[](id:que44)
### TRTC는 라이브 방송 시나리오 유형의 애플리케이션을 어떻게 실행하나요?
TRTC는 온라인 라이브 방송 시나리오 전용으로 10만 명 저딜레이 ILVB 솔루션을 출시했습니다. 이를 통해 호스트와 마이크가 연결된 호스트의 최저 딜레이 시간은 200ms 이내, 일반 시청자의 딜레이 시간은 1s 이내로 유지됩니다. 또한 약한 네트워크를 강력하게 보완하는 기술은 복잡한 모바일 네트워크 환경에 적합합니다.
...
[](id:que46)
### TRTC SDK는 배경 음악 재생 시 순환 재생을 지원하나요? 배경 음악 재생 진행률 조정을 지원하나요?  
...
있습니다. TRTC는 onRemoteUserEnterRoom/onRemoteUserLeaveRoom을 사용하여 방 멤버의 방 출입을 수신합니다(멀티미디어 업스트림 권한이 있는 사용자만 트리거 가능).
...
현재는 로컬 음악만 지원하며, 먼저 로컬에 다운로드한 후 playBGM()을 호출하여 재생할 수 있습니다.
...
- stopLocalPreview는 로컬 비디오 수집을 중지합니다. 이 인터페이스를 호출하면 로컬 및 원격 화면에 블랙 스크린 현상이 나타납니다.
...
[](id:que62)
### TRTC에는 권장하는 화질(비트 레이트, 해상도, 프레임 레이트) 관련 매개변수 설정이 있나요?
...
[](id:que64)
### TRTC는 방에 대한 권한 인증을 지원하나요(예: 회원만 입장할 수 있는 시나리오 여부)? 
...
[](id:que67)
### TRTC란 무엇인가요?
TRTC는 Tencent Cloud 서비스를 통해 Tencent가 다년간 네트워크와 멀티미디어 기술 분야에서 쌓아온 성과를 바탕으로 한 그룹 멀티미디어 통화 및 저딜레이 ILVB 시나리오를 공개하고, 개발자들이 저비용, 저딜레이, 고품질을 특징으로 한 멀티미디어 인터랙션 솔루션을 빠르게 구축할 수 있도록 돕습니다. 자세한 설명은 [제품 개요](https://intl.cloud.tencent.com/document/product/647/35078)를 참조하십시오.
...
[](id:que69)
### TRTC는 어떻게 시작하나요?
TRTC는 각 플랫폼의 Demo 소스 코드를 제공하여 단 몇 분 만에 사용자만의 미니 애플리케이션을 구축할 수 있습니다. 자세한 내용은 [신규 사용자 가이드](https://intl.cloud.tencent.com/document/product/647/39386)를 참조하십시오.

[](id:que70)
### TRTC는 클라우드 녹화 및 재생을 어떻게 구현하나요?
자세한 내용은 [클라우드 녹화 및 재생 구현](https://intl.cloud.tencent.com/document/product/647/35426)을 참조하십시오.
