...
[](id:r1)
### 如何录制单路流？
- 在控制台配置成**全局自动录制**，推流时会自动录制房间里的每条单路流，具体请参见 [全局自动录制](https://intl.cloud.tencent.com/document/product/647/35426)。
- 在控制台配置成**指定用户录制**，需要录制的流在进房时设置下TRTCParams中的userDefineRecordId参数就可以，具体请参见 [指定用户录制（SDK API）](https://intl.cloud.tencent.com/document/product/647/35426)。
- 如果不希望全局自动录制，同时平台也不支持SDK API，可以通过 [云直播](https://intl.cloud.tencent.com/products/css) 的 [直播录制](https://intl.cloud.tencent.com/document/product/267/31563) 进行单独的录制。

[](id:r2)
### 如何录制混流？
- 如果配置的**全局自动录制**，混流也会自动录制。
- 如果配置的**指定用户录制**，并使用服务器 REST API 触发混流，在混流接口中指定参数 OutputParams.RecordId，可以开启混流录制，具体请参见  [混流参数-OutputParams](https://intl.cloud.tencent.com/document/product/647/36760)。
- 如果配置的**指定用户录制**，并使用客户端 SDK API 触发混流，主播在进房时设置了 TRTCParams 中的 userDefineRecordId 参数混流就会录制，具体请参见  [指定用户录制（SDK API）](https://intl.cloud.tencent.com/document/product/647/35426)。
...
[](id:r5)
### 录制文件和回调什么时候产生？
- 录制完成5分钟后录制文件会转存到 [云点播](https:/intl.cloud.tencent.com/product/vod) 平台并触发回调。
...
[](id:r6)
### 录制文件怎么获取？
- 可以通过点播控制台手动查找和点播 REST API 查找，具体请参见 [查找录制文件](https://intl.cloud.tencent.com/document/product/647/35426)。
- 可以通过回调方式及时获取录制文件下载地址，具体请参见  [接收录制文件](https://intl.cloud.tencent.com/document/product/647/35426)。
...
[](id:r7)
### 没有收到录制回调如何排查？
- 首先通过控制台查看录制文件是否生成，如果没有生成，可以按照前面方法进行初步排查。文件查找请参见  [查找录制文件](https://intl.cloud.tencent.com/document/product/647/35426)。
- 如果生成了录制文件但没有收到回调，可以先检查回调是否已经正确配置。回调配置请参见 [接收录制文件](https://intl.cloud.tencent.com/document/product/647/35426)。
