...
2. 確認したいSDKAppIDに対応する【アプリケーション情報】をクリックし、さらに【すぐに作業を開始】タブをクリックします。
3. 【ステップ2 UserSigを発行するためのキーを取得】のタグを開けば、UserSig計算用の暗号化キーが取得できます。
4. 【キーのコピー】をクリックします。これでキーがクリップボードにコピーされます。


### キーをクエリするとき、パブリックキーおよびプライベートキーの情報しか取得できませんが、キーはどうしたら取得できますか？
...
|  適用プラットフォーム | ファイルソースコードリンク | ファイル相対パス |
|:---------:|:---------:|:---------:|
...
サーバー側がUserSigを計算するメソッドを採用することで、UserSig計算用のキーが漏洩しないことを最大限保障できます。なぜなら、1つのAppをハッキングするよりも1台のサーバーを攻撃する難度の方が高いからです。
...
| 言語バージョン | 署名アルゴリズム | 主要関数 | ダウンロードリンク |
|:---------:|:---------:|:---------:|:---------:|
