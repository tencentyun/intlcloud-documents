[](id:que1)
### 2대의 모바일에서 동시에 Demo를 실행할 경우 서로의 화면이 보이지 않는 이유는 무엇인가요?
두 대의 휴대폰에서 Demo를 실행할 경우 각자 다른 UserID를 사용해야 합니다. TRTC에서는 동일 UserID(SDKAppID가 다를 경우 제외)를 2개의 단말기에서 동시에 사용할 수 없습니다.

[](id:que2)
### 방화벽에 어떤 제한이 있나요?
SDK는 UDP 프로토콜을 통해 멀티미디어를 전송하므로, UDP를 차단하는 기업용 네트워크에서는 사용할 수 없습니다. 이와 유사한 문제가 발생하는 경우 [기업용 방화벽 제한 대응](https://intl.cloud.tencent.com/document/product/647/35164)을 참조하여 진단 및 해결하십시오.

[](id:que3)
### iOS 패키징에 Crash가 실행됩니다.
iOS14 이상의 debug 모드 문제인지 진단하십시오. 자세한 내용은 [공식 홈페이지 설명](https://flutter.cn/docs/development/ios-14#launching-debug-flutter-without-a-host-computer)을 참조하십시오.

[](id:que4)
### iOS에서 비디오가 표시되지 않습니다. (Android는 정상)
항목 'nfo.plist' 중 'io.flutter.embedded_views_preview' 값이 YES인지 확인하십시오.
