...
- ILVB(키워드: 마이크 연결, PK)
 ILVB는 일종의 비즈니스 형태로, 호스트와 관객 사이에서 인터랙션 마이크를 연결하고, 호스트와 호스트 사이에서 인터랙션 PK를 진행하는 일종의 라이브 방송 형태입니다.
...
[TRTC 콘솔](https://console.cloud.tencent.com/rav)에 로그인하여 생성한 애플리케이션을 클릭한 후 [Account Information]를 클릭하면 서버 상태를 확인할 수 있습니다.
...
<span id="que8"></span>
### 크로스 룸 마이크 연결은 어떻게 해야 합니까(호스트 PK)?
...
[TRTC 콘솔](https://console.cloud.tencent.com/trtc)에서 자동 릴레이 푸시 스트리밍을 활성화했는지 확인합니다.
...
1. [TRTC 콘솔](https://console.cloud.tencent.com/trtc)에 로그인하여 [자동 릴레이 푸시 스트리밍]을 활성화하고 [클라우드 녹화 실행]을 비활성화합니다.
...
1. 관객 수 증가(Client -> Server) 신규 관객이 들어왔을 경우 방의 관객 수가 +1 되어야 한다는 의미로 앱의 관객 측에서는 방 입장 시 Server에 누적 요청을 1회 발송합니다.
2. 관객 수 감소(Client -> Server) 관객이 방에서 나갔을 경우 방의 관객 수가 -1 되어야 한다는 의미로 앱의 관객 측에서는 방에서 나갈 시 Server에 차감 요청을 1회 발송합니다.
...
4. 시작할 때 오디오만 있다가 도중에 비디오로 전환되었을 경우 녹화 템플릿이 달라 비디오 시간대의 녹화 파일만이 생성되었거나 오디오 시간대의 녹화 파일만이 생성되었을 것입니다.
...
- TRTC는 클라우드 QOS 트래픽 제어 Policies를 통해 네트워크 상태에 따라 비트 레이트와 해상도를 동적으로 조정합니다. 네트워크 상태가 좋지 않은 경우 비트 레이트가 낮아져 해상도가 떨어질 수 있습니다.
...
<span id="que36"></span>
###  게스트 초대 연결 시 게스트 방 번호는 어떻게 알립니까?
