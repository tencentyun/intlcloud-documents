...
휴대폰 로컬 저장은 지원하지 않습니다. 녹화 후 비디오 파일은 VOD 플랫폼에 저장되며 자체적으로 다운로드하여 휴대폰에 저장할 수 있습니다. 
...
해당 기능은 UI 레이아웃 로직으로 SDK에서 UI상의 표시 프로세스를 제한하지 않습니다. 공식 홈페이지 Demo에서 화면 전후 스택 및 아홉 개 칸 레이아웃 모드의 예시 코드를 제공하고 있으며, 플로팅 창, 화면 크기 변경, 화면 드래그를 지원합니다. 자세한 내용은 [공식 Demo](https://github.com/tencentyun/TRTCSDK)를 참고하십시오.
...
[](id:que49)
### TRTC SDK는 연결 해제 및 재연결을 지원합니까?
SDK는 사용자 연결 끊김 시 무한 재접속 메커니즘을 지원하며, 구체적인 연결 상태와 연결 과정에서의 처리 로직은 다음과 같습니다.
다음 이미지는 채널에 참여하는 사용자 Userid1로부터 수신된 수신 콜백 이벤트, 연결 중단 및 방에 다시 참여하는 프로세스를 보여줍니다.
![](https://qcloudimg.tencent-cloud.cn/raw/893ec522169d27a7b9a0e2dc4b09c7d0.png)
**자세한 설명**:

- T1: 사용자가 'enterRoom' 인터페이스에 대한 호출을 시작하여 방 입장 요청을 시작합니다.
- T2: `onEnterRoom` 콜백을 수신합니다.
- T3: 클라이언트는 네트워크 문제로 인해 네트워크 연결이 끊겼고 SDK는 방 재입장을 시도합니다.
- T4: 연락처가 8초 동안 서버에 연결되지 않으면 'onConnectionLost' 연결 끊김 콜백을 수신합니다.
- T5: 이어 3초 후에 서버에 연결되지 않고 'onTryToReconnect' 재시도 콜백을 수신합니다.
- T6: 이후 24초마다 'onTryToReconnect' 재시도 콜백을 수신합니다.
- T7: 연결 끊김 중 아무 때나 재연결에 성공하고, `onConnectionRecovery` 복구 콜백을 수신합니다.
