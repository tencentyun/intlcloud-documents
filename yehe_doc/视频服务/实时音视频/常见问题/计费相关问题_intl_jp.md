...
[](id:que9)
### 画面共有はどのように課金されますか。
画面共有は、画面シェアとも呼ばれますが、**単独のビデオストリーム**です。ユーザーが**画面共有でビデオストリームを購読し、ビデオ画面を受信すると、ビデオ時間に応じて課金されます**。

#### 課金の例：
マンツーマン授業のシーンで、教師Aと学生Bがどちらもカメラをオンにしており、画面解像度はどちらもSD（単価：1.99米ドル/分）です。教師Aは画面共有によって教材のPPTを再生し、画面解像度はHD（単価：3.99米ドル/分）です。授業時間は全部で30分であり、かつその中で教師Aと学生Bは、最初から最後まで互いのオーディオビデオストリーミングを閲覧したとします。

#### 分析：
- **Aによる使用量および料金**：
Aによる料金 = SDビデオ時間単価 × SDビデオ時間 = 1.99米ドル/1000分 ×（30分 / 1000）=0.0597米ドル
- **Bによる使用量および料金**：
Bによる料金 = SDビデオ時間単価 × SDビデオ時間 + HDビデオ時間単価 × HDビデオ時間=1.99米ドル/1000分 ×（30分/1000）+ 3.99米ドル/1000分 ×（30分/1000）=0.1794米ドル

このTRTCルームで発生した料金総額は、ユーザーAによる料金 + ユーザーBによる料金 = 0.2391米ドルです
...
- 後払いを開始していない状態で、無料お試しパッケージを使い切った、または期限切れとなり、サービスが自動的に停止された場合：[パッケージを購入](https://buy.cloud.tencent.com/trtc)することによりサービスを再度アクティブ化するか、または直接[後払いの開始](https://intl.cloud.tencent.com/document/product/647/41979)を行うことができます。
