...
1. [호스트] `createMeeting()`을 호출하고 `roomId`를 전송하여 회의를 생성하면, 회의 방 생성 여부가 `ActionCallback` 을 통해 호스트에게 통지됩니다. 
2. [호스트] `startCameraPreview()`를 호출하여 카메라 미리보기를 시작할 때 뷰티 필터 매개변수를 조정할 수 있습니다.
3. [호스트] `startMicrophone()`을 호출해 마이크 수집을 시작합니다.
...
1. [참석자] `enterMeeting()` 을 호출하고 `roomId`를 전송하면 회의 방에 입장됩니다.
2. [참석자] `startCameraPreview()`를 호출해 카메라 미리보기를 시작하고 `startMicrophone()`을 호출해 마이크 수집을 시작합니다.
3. [참석자] `onUserVideoAvailable` 이벤트를 수신하면 `startRemoteView()`가 호출되고 참석자 `userId`가 전송되어 재생이 시작됩니다.
