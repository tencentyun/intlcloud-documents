...
3. **UserSig 발급**
	...
	- 디버깅 및 실행 단계에서 userSig 발급 방법은 [디버깅 및 실행 단계에서 UserSig를 계산하는 방법은 무엇입니까?](https://intl.cloud.tencent.com/document/product/647/35166#how-do-i-calculate-.3Ccode.3Eusersig.3C.2Fcode.3E-during-debugging-and-running.3F)를 참고하십시오.
	- 프로덕션 환경에서 userSig 발급 방법은 [정식 실행 단계에서 UserSig는 어떻게 계산하나요?](https://intl.cloud.tencent.com/document/product/647/35166#how-do-i-calculate-.3Ccode.3Eusersig.3C.2Fcode.3E-during-production.3F)를 참고하십시오.

[](id:step2)

### 2단계: TUIRoom 컴포넌트 다운로드 및 복사
1. [Github](https://github.com/tencentyun/TUIRoom)을 클릭하여 TUIRoom 저장소 코드를 클론 또는 다운로드합니다.
2. 기존 Vue 3 + TS 프로젝트를 엽니다. 해당 프로젝트가 없으면 다음 스크립트를 사용하여 Vue 3.0 + VITE + TS 템플릿 프로젝트를 생성할 수 있습니다.
```bash
...
1. TUIRoom 컴포넌트를 'App.vue' 컴포넌트와 같은 페이지로 가져옵니다.
	...
	- 호스트와 참석자는 [init](#init) 메소드를 통해 TUIRoom 컴포넌트에 응용 프로그램 및 사용자 데이터를 초기화할 수 있습니다. 호스트는 [createRoom](#createroom) 메소드를 통해 방을 만들고 입장할 수 있습니다. 참석자는 [enterRoom](#enterroom) 메소드를 통해 호스트가 만든 방에 참여할 수 있습니다.
```javascript
<template>
...
app.mount('#app');
```
3. **SVG 아이콘을 로드하도록 svg-icon 컴포넌트 구성**
TUIRoom은 모든 아이콘 자산(SVG 파일)을 `src/TUIRoom/assets/icons/svg` 디렉터리에 배치합니다. 'vite.config.ts' 파일에서 SVG 아이콘 로딩 방식을 설정해야 합니다.
...
```
4. **element-plus 컴포넌트의 주문형 가져오기 구성**
...
[2단계](#step2)의 스크립트를 사용하여 Vue + TS 프로젝트를 생성하는 경우 다음을 수행해야 합니다.
