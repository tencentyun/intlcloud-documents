...
サポートしています。以前の関連機能を実行するためにバックグラウンドに入る必要がある場合は、現在のプロジェクトを選択できます。 **Capabilities** の下に**Background Modes** を設定して **ON**を開いて **Audio，AirPlay and Picture in Picture** にチェックを入れます。下図に示すとおりです。
