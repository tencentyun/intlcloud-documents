`TRTCKaraokeRoom` includes the following features which are based on Tencent Real-Time Communication (TRTC) and Instant Messaging (IM).
...
`TRTCKaraokeRoom` is an open-source class depending on two closed-source Tencent Cloud SDKs. For the specific implementation process, see [Karaoke (Android)](https://intl.cloud.tencent.com/document/product/647/41941).
