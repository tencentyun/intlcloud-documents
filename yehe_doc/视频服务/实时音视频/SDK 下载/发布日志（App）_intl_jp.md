...
>?以前のバージョンの更新履歴については[その他](https://intl.cloud.tencent.com/document/product/647/39426)をクリックしてください。
...
- Windows：カメラの互換性を最適化し、一部のデバイスで収集したフレームレートが設定値と一致しなかったり、オンに失敗したりする問題を解決しました。
- iOS：互換性が向上し、cocos2Dなどの他のレンダリングコンポーネントと組み合わせて使用するときのコンフリクトが軽減されます。
- Android：アップストリームでカメラをオフにしてからオンにし、再生側でオフにする前の最後のフレームが表示されてから正常に表示されるという問題を修正しました。
...
- Android&iOS：このバージョンは国家プライバシーセキュリティ基準の規定に適合し、Tencent社内での複数製品に対して検証しました。
...
**機能追加**
- すべてのプラットフォーム：外部オーディオデバイスの異常検出を追加しました。onStatisticsコールバックを登録した後、 [TRTCLocalStatistics](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCTypeDef__cplusplus.html#structtrtc_1_1TRTCLocalStatistics)のaudioCaptureStateを使用して、長期間のミュート、音割れ、異常な中断といった問題をリアルタイムに検出できます。
- Windows：ユーザー定義キャプチャはRGBA形式でのビデオデータの入力をサポートします。

**品質の最適化**
- すべてのプラットフォーム：BGMリソース管理を最適化し、メモリ使用量を速やかに解放します。
- すべてのプラットフォーム：プッシュ側がバックグラウンドに戻ってビデオのアップロードを一時停止すると、再生側は[onUserVideoAvailable(false)](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloudCallback__cplusplus.html#a091f1c94ff1e2bc39c36e9d34285e87a)の通知を速やかに受信できます。
...
**問題の修正**
- iOS：一部のデバイスにバックグラウンドOpenGLレンダリングcrashが偶発する問題を修正しました。
...
- C# ：最新バージョンのAPIインターフェースに同期してアップグレードしました。
...
- すべてのプラットフォームはTXDeviceManagerデバイス管理タイプを追加しました。
...
**最適化**
- Androidはソフトウェアデコード、ハードウェアデコードのロジック切り替えを最適化しました。
- WindowsはSystem loopbackオーディオ収集音質およびエコー除去効果を最適化しました。
- Windowsはオーディオデバイス選択ロジックを最適化し、無音声の割合を低下させます。
- Windowsは二者間通話するときの切断効果を最適化しました。
- すべてのプラットフォームは、マニュアル受信モードでロールを切り替え時のインスタントブロードキャスティング効果を最適化しました。
- すべてのプラットフォームは、オーディオ受信ロジックを最適化して音声効果を向上しました。
- すべてのプラットフォームはsendCustomCmdMsgの信頼性を最適化しました。

**修正**
- iOSは、muteLocalVideoの呼び出しによりローカルビデオのレンダリングが一時停止する問題を修正しました。
- iOSは、フォアグラウンドとバックグラウンドの切り替え時に、システムコンポーネントの呼び出しに時折不具合が生じる問題を修正しました。
...
- Android：一部の携帯電話で長時間Appをバックグラウンドに切り替えた後、再度フォアグラウンドに戻したとき、時折音声画像が同期しない問題を修正しました。
- Android：バックグラウンドに切り替えた後にマイクがリリースされない問題を修正しました。
...
- Windows：画面共有が主経路を経由するのをサポートすると同時に、 startLocalPreview をコールしてローカルプレビューを有効にします。
- Windows： SDK内部の再生バッファに起因するオーディオディレイが大きくなる問題を低減しました。
...
- iOS：フォアグラウンドとバックグラウンドの切り替え時に、一部のiPadビデオレンダリングでメインスレッドが時々動かなくなってしまう問題を修正しました。
...
- Windows：カメラおよびマイクを抜いた後、再度差し込んでもデバイスが自動的にアクティブにならない問題を修正しました。
...
- Cloud MCU Mixの背景画像の設定サポートを追加しました（監督管理の必要性から、ピクチャはまずTRTCコンソールを経由してからアップロードしてください）。
- Cloud MCU MixはA+B=>CおよびA+B=>Aの2種類のモードのサポートを追加しました。
...
- Mix streamingのプリセットテンプレートのユーザビリティを最適化しました。
- Mix streamingを最適化し、成功率を向上させました。
...
- 3A起動ポリシーを最適化しました。
-  mcu Mix streamingのユーザビリティを向上させました。
...
- インターフェースgetBeautyManagerを追加して、美顔、画像加工効果インターフェースを集めました。
...
- CDNおよびMix streamingのリツイートを最適化しました。
