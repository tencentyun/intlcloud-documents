...
**新機能：**
- すべてのプラットフォーム：ロールのスムーズな切り替えをサポートし、ロールの切り替えによってオーディオビデオの再生が一時的に中断されることはありません。
- iOS：ステレオオーディオのキャプチャをサポートします。
- Android：Android 10以降では、オーディオを再生するためのキャプチャシステムをサポートします（startSystemAudioLoopback）。

**機能の最適化：** 
- すべてのプラットフォーム：音楽シーンのエコーキャンセル能力を最適化すると、音質効果がより自然になります。
- すべてのプラットフォーム：ロールの切り替え+muteLocalAudioの音質と起動効果を最適化します。
- すべてのプラットフォーム：onSpeedTestコールバックの帯域幅予測を最適化します。
- iOS：メモリヒープの問題を回避するために、メモリ管理を最適化します。
- Android：一部のモデルの携帯電話でのインイヤ・モニタリングのディレーを最適化します。
- Windows：ビデオダウンリンク時のビデオレンダリングリンクの性能を最適化します。
- Windows：リークとエコーの問題を効果的に回避するために、ステレオキャプチャのロジックを最適化します。

**バグ修正：**
- すべてのプラットフォーム：退室コールバック(onExitRoom)のreason異常の問題を修正しました。
- すべてのプラットフォーム：アップリンクカスタムビデオが送信されるときにタイムスタンプが等しい場合の黒い画面の問題を修正しました。
- すべてのプラットフォーム：最初にmuteLocalAudio、次にstartLocalAudioオーディオ時のcrashの問題を修正しました。
- すべてのプラットフォーム：3Aシーンを手動で設定せずに、カスタムオーディオキャプチャを開くと、3Aが開くという問題を修正しました。
- すべてのプラットフォーム：オーディオカスタムレンダリングで時々発生するノイズの問題を修正しました。
- iOS：logパス(setLogDirPath)が途中で設定され、サンドボックスが変更された場合のメモリリークの問題を修正しました。
- iOS&Mac：システムオーディオサービスが異常な場合、BGM継続シーンのクラッシュの問題を修正しました。
- Android：時々発生するBluetoothイヤホンの再接続の問題を修正しました。
- Android：一部の携帯電話で時々発生する音が出ない問題を修正しました。
- Android：Redmiなどの一部のモデルでイヤホンを繰り返し接続したり取り外したりすることによって引き起こされるクラッシュの問題を修正しました。
- Windows&iOS：スクリーンキャプチャ失敗の問題を修正しました。
- Windows：VODプレーヤーがイメージを開いた後、vodを閉じるとクラッシュが発生するという問題を修正しました。
- Windows：ブロードキャストptsはgenerateCustomPtsを使用せず、複数のブロードキャスト再生によりptsがフォールバックする問題を修正しました。
- Windows：無効画面に画像を表示することを選択した後、時々発生するクラッシュの問題を修正しました。
...
- すべてのプラットフォーム：キャスター入退室通知コールバック([onRemoteUserEnterRoom](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloudDelegate__ios.html#a390831928a4d2a7977c4c1572da8be58) / [onRemoteUserLeaveRoom](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloudDelegate__ios.html#afa7d16e1e4c66d938fc2bc69f3e34c28))の速度を最適化します。
- Windows：画面共有の性能を最適化しました。フィルターウィンドウが設定されていない場合、性能は2倍になります。

**バグ修正：** 

- iOS&Mac：BGM再生を開始するとき、時々発生する[onComplete](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TXAudioEffectManager__ios.html#a08ab9a6d9100f133420e44ae5eb99ee6) コールバックエラーの問題を修正しました。
- Android：ネットワークモジュールによって引き起こされるクラッシュの問題を修正しました。
- すべてのプラットフォーム：SEIが異常に送信される問題を修正しました。


### Version 9.9 @ 2022.05.06

**機能の最適化：** 
- Windows：ビデオリンクを最適化し、性能のオーバーヘッドを削減しました。
- Windows：タブるチャネルの効果を維持するために、Systemloopbackキャプチャ前処理を最適化しました。
- Mac：キャプチャボリュームが高すぎるために発生する音の飛び出しの問題を最適化し、音質体験を向上させました。
- Mac：画面共有（補助チャネル）の画質を向上させました。
- Android：キャプチャのディレーを最適化し、インイヤ・モニタリング体験を向上させました。

**バグ修正：** 
- Android：ルーム番号が21億を超える番号をサポートしないという問題を修正しました。


### Version 9.8 @ 2022.04.21

**新機能：**
- Windows：「ヘヴィメタル」や「ロリ声」などの音響効果インターフェースを追加しました。詳細については`ITXAudioEffectManager.setVoiceChangerType`をご参照ください。
- Windows：ローカルの画面が一時停止されている間に代替画像を設定してプッシュすること、すなわち代替画像のプッシュをサポートします。

**機能の最適化：** 

すべてのプラットフォーム：ビデオシーンにおける性能を最適化します。

**バグ修正：** 
- Mac：システムサウンドカードのオーディオ録音時にドライバーのインストールが失敗する問題を修正しました。
- すべてのプラットフォーム：ローカル画面共有（サブストリーム）時にカスタムレンダリングが無効になる問題を修正しました。


### Version 9.7 @ 2022.04.06

**機能の最適化：** 
- iOS&Android：Music音質の効果を最適化します。
<dx-alert infotype="explain" title="Tips：">
すべてのプラットフォームでAPIインターフェース[TRTCCloud.startLocalAudio(TRTCAudioQualityMusic)](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloud__android.html#a1dadf09b10a2d128e4cef11707934329)を使用してMusic音質を有効にすることができます。以下も同様です。
</dx-alert>
- Windows：Music音質での再生効果を最適化し、音質へのダメージを軽減します。
- Windows：一部のプロ向けのサウンドカードの互換性問題を改善し、音質の向上に有効です。
- Windows：サードパーティプロセスリミックスの最適化により、サードパーティプロセスリミックスでより多くのシーンに対応できます。

**バグ修正：**
- すべてのプラットフォーム：CDN再生時にビデオ画面がちらつく問題を修正しました。
- iOS&Android：ライブストリーミング再生の受話器・スピーカーの切り替えが無効になる問題を修正しました。
- iOS&Android：APIを介してMusicの音質を設定した場合に、期待通りの音質が得られない問題を修正しました。
- iOS：ソフトエンコーディングで時々発生するメモリリークを修正しました。
- iOS：ローカルビデオ画面のレンダリングで時々発生する最初のフレームコールバックがない問題を修正しました。
- Windows：画面共有モードで時々発生するマウスキャプチャの異常なクラッシュを修正しました。
- Windows：Musicの音質でスピーカーが再生しない問題を修正しました。
- Windows：一部のカメラがstartCameraDeviceTestを正しくオンにできない問題を修正しました。
...
-すべてのプラットフォーム：サードパーティライブラリのコンプライアンス改造を完了し、国内、海外のコンプライアンス要件を満たすようにしました。
- すべてのプラットフォーム：TRTC SDKのサイズを最適化しました。データの詳細は次の表に示すとおりです：
<table>
<thead>
<tr>
<th>プラットフォーム</th>
<th><strong>最適化前</strong></th>
<th><strong>最適化後</strong></th>
</tr>
</thead>
<tbody><tr>
<td>Android</td>
...
すべてのプラットフォーム：既知の問題を修正し、安定性を高めました。

**機能の最適化：**
- iOS: フィルライトの下で偶発的に現れるソラリゼーションの問題を修正しました。
- Mac: テクスチャのアップロードをサポートし、性能を高めました。
- Android：美顔などの前処理プロセスを最適化し、ローエンドのキャプチャカードのラグ問題を修正しました。
- Windows：Live V1からV2インターフェースにアップグレードされ、より安定したLiveコンポーネントを提供します。
- Windows：ローエンドでのグラフィックカードの互換性を向上させました。

>?以前のバージョンの更新履歴については[その他](https://cloud.tencent.com/document/product/647/46907)をクリックしてください。

### Version 9.5 @ 2022.01.11

**故障の修復：**
- すべてのプラットフォーム：APIの使いやすさが向上し、一部のAPI固有の呼び出しタイミングによってカスタムレンダリングの再生がブラックスクリーンになる問題を修正しました。
...
**機能の最適化：**
- すべてのプラットフォーム：通話シナリオの脆弱なネットワーク環境でのスムーズさを最適化します。
...
**機能の最適化**
- すべてのプラットフォーム：脆弱なネットワーク状況におけるビデオの秒速開始スピードを最適化しました。
- すべてのプラットフォーム：脆弱なネットワークの制御ポリシーを最適化しました。同じシナリオでも、よりスムーズです。
- すべてのプラットフォーム：スピードテスト機能を最適化し、現在のネットワーク帯域幅に対する検査をサポートしました。
- すべてのプラットフォーム：TCP伝送プロトコルに対するサポートを最適化しました。複雑なネットワーク環境により良く対応します。
...
- iOS： VODPlayerが再生を減速するcrashを修正しました。
...
**品質の最適化**
- Android：オンラインライブストリーミングエンコードの品質を最適化し、ビデオ画面をさらに鮮明にしました。
- Windows：エコー除去アルゴリズムを最適化し、エコー除去効果をさらに向上させます。

**問題の修正**
- iOS： VODPlayerとTRTCを同時に使用しているときにオーディオ再生に時々不具合が生じる問題を修正しました。
...
**問題の修正**
- iOS：一部に時々現れるアプリケーションの強制終了に起因するクラッシュの問題を修正しました。
- Android：収集したフレームレートがやや高いときに現れる美顔効果の不具合の問題を修正しました。
...
**修正**
- iOSは、muteLocalVideoの呼び出しによりローカルビデオのレンダリングが一時停止する問題を修正しました。
- iOSは、フロントエンドとバックエンドの切り替え時に、システムコンポーネントの呼び出しに時折不具合が生じる問題を修正しました。
- iOSは、音響効果を起動時に、インイヤーモニタリングのオーディオが途切れ途切れになる問題を修正しました。
- Androidは、通話音量の音響効果を切断したときに、電話が中断され 効果音が再生を止めない問題を修正しました。
- Androidは、時折オーディオ収集の開始エラーが起こる問題を修正しました。
- Windowsは、時折ローカルビデオレンダリングでスクリーンが黒くなる問題を修正しました。
- Windowsは、進捗で退出時にcrashが起こることがある問題を修正しました。
- Windowsは、Bluetoothイヤホンのサポートを最適化してBluetoothイヤホンに音声が出ない問題を修正しました。
- Windowsは、画面共有が終了したときに焦点が集まる問題を修正しました。
- すべてのプラットフォームは、状態コールバックのパケットロス率の統計に不具合がある問題を修正しました。
...
- iOS：一部判明しているメモリ漏出の問題を修正しました。
- iOS：replaykitの拡張によって、時折スクリーンキャプチャの終了後にcrashが現れる問題を修正しました。
- iOS：シミュレータ環境のコンパイルの問題を解決しました。
- Android：一部の携帯電話で長時間Appをバックエンドに切り替えた後、再度フロントエンドに戻したとき、時折音声画像が同期しない問題を修正しました。
...
- Android：Bluetoothイヤホンが断絶してから再接続した後に、オーディオの収集と再生に不具合が生じる問題を修正しました。
- Android：既知のcrashの問題を修正しました。
- Windows：64ビットSDKで何度も画面共有を起動するとcrashすることがある問題を修正しました。
...
- iOS：内部スレッドモデルを最適化して、30チャネル以上で同時再生するシナリオでの操作の安定性を向上させました。
...
- すべてのプラットフォーム：脆弱なネットワーク環境でmuteLocalVideoおよびmuteLocalAudioインターフェースをコールするときに、再生に時々不具合が生じるBUGを修正しました。
- iOS：再生オーディオ効果がローエンドのiPhoneまたはiPadで失敗することがあるBUGを修正しました。
- iOS：iPad Proの画面共有の画面に変形、伸びが生じる問題を修正しました。
- iOS： ユーザーの権限拒否後にも、App内のスクリーンがスクリーンレコーディングの権限申請の表示を繰り返しポップアップし続ける問題を修正しました。
...
- Windows：enterRoom および exitRoom の入退室をクイックコールするとき、再生側に無音声を時々生じさせるBUGを修正しました。
- Windows：SDKはVisual Stuido 2010プロジェクトのコンパイルの互換性の問題を修正しました。
...
- Mac：画面共有は、指定ウィンドウにある指定エリアの共有についてサポートを開始しました。

**修正**

- すべてのプラットフォーム：退室していない状況で同一のルームに入室するとき、SDK が onEnterRoom のコールバックをトリガーしないBUGを修正しました。
- すべてのプラットフォーム：スクリーンを黒くする可能性があり、時々数種類の内部BUGが起きる問題を修正しました。
- すべてのプラットフォーム：事前にstartRemoteSubStreamViewをコールすると、スクリーンの画面共有を正常に表示できない問題を修正しました。
- Windows：既知のいくつかのハンドルおよびGDI漏出を修正しました。
- Windows：複数の既知のcrashの問題を修正しました。
- Windows：カメラおよびマイクを抜いた後、再度差し込んでもデバイスが自動的に有効にならない問題を修正しました。
- iOS： iOS 10のBGMインターフェースが、特定ルールのファイルパスに伝送されたときにクラッシュすることがあるBUGを修正しました。
- Android：クイックenterRoomおよびexitRoomを頻繁に行った後に時々音がしない問題を修正しました。
- Android：スクリーンキャプチャプッシュによりスクリーンが時々黒くなる問題を修正しました。
...
- クラウドMCUミクスストリーミングの背景画像の設定サポートを追加します（監督管理の必要性から、ピクチャはまずTRTCコンソールを経由してからアップロードしてください）。
- クラウドMCUミクスストリーミングはA+B=>CおよびA+B=>Aの2種類のモードのサポートを追加しました。
- リアルタイム状態でonStatisticsをコールバックし、再生バッファ時に長いフィールド jitterBufferDelayを追加しました。
...
- 再生側ネットワークジッター評価アルゴリズムを最適化して再生ディレイを低減しました。
- Android SDK のエンドツーエンドマイク接続通話の遅延を低減しました。
- インイヤーモニタリングレイテンシーをさらに最適化しました。
- 再生viewを動的に切り替えるときに画面が黒くなる問題を最適化しました。

**修正**

- 1個の関数で連続してplayBGMおよびpauseBGMを呼び出した後、再生しても無効になる問題を修正しました。
- 退室後にonEnterRoomコールバックを時々受診する問題を修正しました。
...
-  Androidバージョンでスクリーンの縦横の切り替え時にローカルスクリーンが少し光る問題を修正しました。
- 一部のAndroid携帯電話がカスタマイズしたビデオを配信しても正常にエンコードできない問題を修正しました。
- オーディオ処理時にデータパッケージの処理で、時折クラッシュが1カ所起こる問題を修正しました。
...
- オーディオでは過度のノイズ低減サポートを追加しました。 setAudioQuality(TRTCAudioQualitySpeech) によって有効にすることができます。
- 音響効果ファイルはassetパッケージの音響効果ファイルをサポートします。
- ローカルビデオ解像度を向上させました。
- 再生側カスタムレンダリングは、テクスチャをサポートして性能オーバーヘッドを低減しました。
- カメラの収集解像度によるロジック選択を最適化して、画角効果を向上させました。
- エコー処理効果を最適化しました。
...
- カスタマイズしたビデオ収集時に、 SDK内部のOpenGLのコンテキストエラーがあって時々crashする問題を修正しました。
- 入室前setLocalVideoRenderListenerカスタムレンダリングのコールバックがアクティブにならない問題を修正しました。
- 横型スクリーンモードで前後のカメラの切り替えによって、再生側画面が逆さまになる問題を修正しました。
- 入室前にstartLocalPreviewを呼び出して、入室後に再生側のスクリーン表示に不具合が生じる確率が高いという問題を修正しました。
- ハードウェアエンコーダに時々生じるcrashを修正しました。
- ローカルオーディオレコーディングが時々断続的に途切れるBugを修正しました。
- プッシュ（muteLocalVideo，muteLocalAudio）を一時停止したときに、強制終了またはcrashが発生した後に再入室すると、再生側がオーディオ、ビデオを自動再生できない問題を修正しました。
...
ロールの切り替え時に、カスタムストリームIDが時々タイミングよく有効にならない問題を修正しました。
...
- 通話音量はデフォルトではANS、AGCを有効にして、通話モードでの音質を向上させました。
- ミクスストリーミングのプリセットテンプレートのユーザビリティを最適化しました。
- ミクスストリーミングを最適化し、成功率を向上させました。

**修正**

- 入室して頻繁にAGCをアクティブにするとき、処理した音声がすべてゼロ（0）になる問題を修正しました。
- 速度測定によってその他のAPI呼び出しへの応答が遅くなる問題を修正しました。
- システムコールに遮断された後、上り音量が2倍になり音声にノイズが入る問題を修正しました。
...
- 入退室を複数回繰り返すことによるメモリ漏出の問題を修正しました。
...
-  HTTPコンポーネントが時々crashする問題を修正しました。
- 音響効果の再生が時々コールバックを完了していない問題を修正しました。
- 入室失敗後に時々回復できない問題を修正しました。
...
ライブストリーミングモード（TRTCAppSceneLIVE）は「低遅延大型ルーム」機能を追加しました：

- オーディオ、ビデオの最適化に特化したUDPプロトコルを採用して弱いネットワークを防止する機能を強化しました。
...
- ライブストリーミングTXLivePlayerがバッファロジックの再生を最適化してラグ率を低減しました。
- 先にmuteLocalVideoをしてから再生側画面を削除する回復速度を最適化しました。
- 高ディレイおよび高パケットロスのネットワーク環境下のQoEアルゴリズムを最適化して、弱いネットワークに対する耐性を強化しました。
- デコーダーの性能を最適化して、ローエンドのAndroid携帯電話のディレイが次第に高くなるBugを修正しました。
- 音量評価アルゴリズム（enableAudioVolumeEvaluation）を最適化して、音量評価の感度をさらによくしました。
...
- enterRoomが時々コールバックしないBugを修正しました。
- オーディオ収集の閉鎖後、再生しても音声が出ないBugを修正しました。
- 除去後にローカルのレンダリングviewを再追加した後にスクリーンが緑色になるBugを修正しました。
- レンダリングコールバック(setRemoteVideoRenderDelegate)のカスタマイズで、リモート画面の解像度が540P以上（540Pを含む）のとき、コールバックを10回しか行わないBugを修正しました。
...
- MixストリーミングsetMixTranscodingConfig APIの設定コールバック関数を追加しました。
- エンタープライズ版でのデカ目、小顔、フェイスシェイプおよび動的エフェクトの付属機能を追加しました。

**最適化**

- 弱いネットワーク環境下での流暢性を向上させました。
...
**修正**
- 音声のハンズフリー切り替えが機能しない Bugを修正しました。
...
- Android 9.0システムでDemoをアクティブにした後、ポップアップする問題を修正しました。
- 音量調節ボタンが視聴者側音量を調整できない問題を修正しました。
...
**追加**
- フィルター濃度設定インターフェース setFilterConcentration() を追加しました。
...
シンプル音声通話シナリオ（人狼ゲームなど）でのRelayed Push機能を修正して、TRTCParamのbussInfoフィールドを合わせて使用してください。
...
- CDNの転送/プッシュおよびミクスストリーミングの実装を最適化しました。
...
- 全く新しいQoSアルゴリズムを採用して、ラグ率をさらに低くし、流暢性をさらに高くしました。
- 全く新しいAudioコンポーネントを採用して、各種ネットワーク状況下での音質を高度に最適化しました。
- 高・低画質ストリームのデュアルチャンネルエンコード機能（WindowsおよびMacデバイスでのみアクティブにすることを推奨）をサポートします。
- CDN転送/プッシュおよびミクスストリーミング機能をサポートします。
