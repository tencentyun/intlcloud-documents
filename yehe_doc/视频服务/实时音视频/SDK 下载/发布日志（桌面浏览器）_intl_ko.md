...
버전 번호 `major.minor.patch`의 자세한 규칙은 다음과 같습니다.
  ...
  - minor: 마이너 버전 번호로, 각 마이너 버전 번호 간에는 인터페이스가 호환되며 인터페이스 신규 추가 또는 최적화 시 해당 필드의 숫자가 증가합니다.
  - patch: 패치 번호로, 기능 개선 또는 오류 복구 시 해당 필드의 숫자가 증가합니다.

>!
> - 제품의 안정성 및 온라인 지원을 위해 항상 최신 버전을 유지하시기 바랍니다.
> - 버전 업데이트에 대한 주의 사항은 [업데이트 가이드](https://trtc-1252463788.file.myqcloud.com/web/docs/tutorial-00-info-update-guideline.html)를 참조하십시오.
...
- [Client.setTurnServer](https://trtc-1252463788.file.myqcloud.com/web/docs/Client.html#setTurnServer)에서 멀티 turn server 설정 지원
- userId 검증 로직 최적화
...
- [LocalStream.setVideoProfile](https://trtc-1252463788.file.myqcloud.com/web/docs/LocalStream.html#setVideoProfile) 인터페이스에서 동적 호출 지원
- 대시 보드 데이터 리포트 로직 최적화
- 자동 재생 제한 시 처리 로직 최적화. 자세한 내용은 [자동 재생 제한 처리 제안](https://trtc-1252463788.file.myqcloud.com/web/docs/tutorial-21-advanced-auto-play-policy.html)을 참조하십시오.
- 디바이스 연결/해제 자동 복구 실패 시 처리 로직 최적화. 자세한 내용은 [DEVICE_AUTO_RECOVER_FAILED](https://trtc-1252463788.file.myqcloud.com/web/docs/module-ErrorCode.html#.DEVICE_AUTO_RECOVER_FAILED)를 참조하십시오.
...
폐기된 인터페이스 삭제: setDefaultMuteRemoteStreams. [client.unsubscribe](https://trtc-1252463788.file.myqcloud.com/web/docs/Client.html#unsubscribe) 인터페이스로 대체하여 사용하십시오.

**Improvement**

- 방 입장 인터페이스 roomId 매개변수 검증 로직 최적화. 자세한 내용은 [인터페이스 문서](https://trtc-1252463788.file.myqcloud.com/web/docs/Client.html#join) 및 [업데이트 가이드](https://trtc-1252463788.file.myqcloud.com/web/docs/tutorial-00-info-update-guideline.html)를 참조하십시오.
- peer-join 및 peer-leave 이벤트 공지 시기 최적화

**Bug Fixed**

방 퇴장 시 간혹 `Cannot read property 'isConnected' of null` 오류가 발생하는 문제 수정
...
- 모든 플랫폼에서 문자열 방 번호 지원. 자세한 내용은 [TRTC.createClient의 userDefineRecordId 매개변수](https://trtc-1252463788.file.myqcloud.com/web/docs/TRTC.html#.createClient)를 참조하십시오.
...
- 인터넷이 끊긴 후 재연결 시 간혹 실패 오류가 발생하는 문제 수정
...
- 디바이스 복구 실패 시 표시하는 에러 코드 추가: [DEVICE_AUTO_RECOVER_FAILED](https://trtc-1252463788.file.myqcloud.com/web/docs/module-ErrorCode.html#.DEVICE_AUTO_RECOVER_FAILED)

**Bug Fixed**

- Chrome/87 버전에서 간혹 블랙 스크린 현상이 발생하는 문제 수정
...
데스크톱 Firefox M56 이상 및 데스크톱 Edge M80 이상 지원

**Breaking Change**

[TRTC.checkSystemRequirements](https://trtc-1252463788.file.myqcloud.com/web/docs/TRTC.html#.checkSystemRequirements)에서 상세한 점검 결과 반환. 자세한 내용은 [인터페이스 문서](https://trtc-1252463788.file.myqcloud.com/web/docs/TRTC.html#.checkSystemRequirements) 및 [업데이트 가이드](https://trtc-1252463788.file.myqcloud.com/web/docs/tutorial-00-info-update-guideline.html)를 참조하십시오.
...
- iOS WeChat 탑재 브라우저에서 방 입장 및 풀 스트림이 불가능한 문제 수정
...
- TRTC.checkSystemRequirements 점검 로직 최적화
- Safari 화면 공유 지원. 자세한 내용은 [화면 공유 사용 튜토리얼](https://trtc-1252463788.file.myqcloud.com/web/docs/tutorial-16-basic-screencast.html)을 참조하십시오.
...
- WebSocket 신호 터널 재연결 로직 최적화로 연결 안정성 향상
- 로그 출력 로직 최적화
...
- replaceTrack을 사용해 업스트림 오디오 채널로 전환한 후 getLocalVideoStats 인터페이스에서 undefined를 반환하는 문제 수정
...
- Chrome 56에서 방 입장 시 오류가 발생하는 문제 수정
...
- 오류 안내 정보 최적화
- 푸시 스트림 수집 디바이스 변경 점검 시 푸시 스트림 상태 자동 복구

**Bug Fixes**

unpublish 완료 후 곧바로 publish 재시도 실패 오류가 발생하는 문제 수정

## Version 4.6.1 @ 2020.07.28

**Improvement**

- [TRTC.isScreenShareSupported](https://trtc-1252463788.file.myqcloud.com/web/docs/TRTC.html#.isScreenShareSupported) Safari는 화면 공유를 지원하지 않습니다.
- subscribe & unsubscribe 인터페이스의 매개변수 검증 로직 개선
- 네트워크 품질 로그 추가

**Bug Fixes**

- 미디어 디바이스에 대한 라이선스가 없고 TRTC.createStream 인터페이스에서 전달한 디바이스 ID가 빈 문자열일 때 SDK에서 OverconstrainedError를 보고하는 문제 수정
...
휴대폰 QQ 브라우저 점검 추가, 휴대폰 QQ 브라우저는 현재 TRTC 데스크톱 브라우저 SDK 지원 불가
...
- webrtc 지원 여부를 능동적으로 점검하는 기능 추가
- sdp 응답 메커니즘 최적화
- 리포트 로직 최적화
...
- 구버전 Chrome의 WebAudio Context 수 제한 문제 수정
...
- App에서 SDK의 LocalStream.setVideoProfile() 인터페이스를 통해 비디오 속성(해상도, 프레임 레이트, 비트 레이트)을 완벽하게 제어할 수 있으며, 구버전의 Tencent Cloud 콘솔을 통한 '화면 설정(Spear Role)'을 지원하지 않습니다.
