...
- All platforms: supported mixing video-only streams, allowing for more flexible stream mixing control.

**Quality improvement**
- macOS: the `startSystemAudioLoopback` API supported dual audio channels.
...
- All platforms: improved the adaptability to poor network conditions across the audio-video link. Smooth audio and video can be delivered even when the packet loss rate reaches 70%.
...
- iOS: fixed the issue where `snapvideoshot` causes lag with CAAnimation.
- iOS & macOS: fixed the black screen issue when the same view is used display camera and screen sharing images in turn.
- iOS: fixed the blurry screen issue on iPhone 6S when a third-party beauty filter component is used.
- iOS: fixed the issue where, in cases where TRTC and VOD are used at the same time, the app occasionally crashes after VOD is stopped.
...
**New features**
- All platforms: added statistics on remote video lag to `onStatistics`.
...
**Quality improvement**
- All platforms: optimized the audio processing algorithm for when earphones are used to deliver better audio quality.
- Android: optimized the audio pre-processing algorithm to reduce the impact of AEC, ANS, and AGC on audio quality.

**Bug fixing**
- iOS: fixed occasional crash when the app is force killed.
...
- iOS: fixed the issue where the app occasionally freezes when a component is called during foreground-background switch.
- iOS: fixed intermittent in-ear monitoring audio when audio effects are enabled.
- Android: fixed the issue where audio effects do not stop when an incoming call switches the phone to the call volume mode.
...
- All platforms: added audio lag information `audioTotalBlockTime` and `audioBlockRate` to `TRTCRemoteStatistics`.
...
- iOS: fixed occasional noise when users use earphones, even though the default audio quality settings are used.
...
- Android: fixed occasional lip-sync errors on certain phones after the app remains in the background for a long time and is switched to the foreground again.
- Android: fixed the issue where the mic is not released after the app is switched to the background.
...
**Optimization**
- iOS: allowed using TXVodPlayer and TRTC at the same time with AEC enabled.
...
- Android: supported using TXVodPlayer and TRTC at the same time with AEC enabled.
...
- iOS & Android: improved volume and audio quality when TXVodPlayer and TRTC are used at the same time.
...
- iOS: fixed the issue where the app keeps requesting screen recording permission after the user denies.
...
- Windows: fixed the occasional issue where no audio is played when a user uses `enterRoom` and `exitRoom` to enter and exit the room.
...
- Added the `jitterBufferDelay` field, which indicates the playback buffer time, to the real-time status callback `onStatistics`.
...
- Fixed encoding failure when custom videos are published on certain Android phones.
...
- Added the new audio effect management API `TXAudioEffectManager` to offer more diverse audio effects while continuing to support the legacy APIs.
- Added the `minVideoBitrate` option to the video encoding parameter `setVideoEncoderParam`. This option is recommended for scenarios that are demanding on image quality.
...
- Supported 128 kbps high-quality stereo sound throughout the link, which can be set using the `setAudioQuality(TRTCAudioQualityMusic)` API.
...
- Supported playing back multiple tracks of background music at the same time. This feature can be used in karaoke scenarios, where vocal and accompaniment are separate tracks. Loop playback of background music is also supported now.
...
- Fixed the issue where, when the user publishing streams entered the room again after the pausing of stream pushing (muteLocalVideo，muteLocalAudio) causes crash or forces the app to close, streams are not played back automatically at the player end.
...
- Optimized visual effect APIs such as filter and green screen and aggregated them into the `TXCBeautyManager` class, so that they share a calling method now.
...
- Added the `setRemoteSubStreamViewRotation` API, which can be used to adjust the rotation degree of played back substream video.
- Added a global volume mode setting API `setSystemVolumeType(TRTCSystemVolumeTypeVOIP)`, which can ensure that call volume is used throughout the process. This is mainly to prevent the switch from Bluetooth earphones to the built-in mic during audio capturing.
...
- Added the audio effect API, which can be used to play short audio effects.
...
- Fixed incorrect rotation degree for local preview.
...
- Adopted a UDP protocol optimized for audio/video communication, allowing the SDK to adapt perfectly to poor network conditions.
