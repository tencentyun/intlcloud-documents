...
**새로운 기능:**
- 모든 플랫폼: 역할 전환 시 오디오 및 비디오 재생이 일시적으로 중단되지 않도록 원활한 역할 전환 지원이 추가되었습니다.
- iOS: 스테레오 오디오 캡처 지원이 추가되었습니다.
- Android: Android 10 이상(startSystemAudioLoopback)에서 캡처 시스템에 의한 오디오 재생 지원이 추가되었습니다.

**기능 최적화:** 
- 모든 플랫폼: 음악 시나리오에서 에코 제거 기능을 최적화하여 음질을 보다 자연스럽게 만들었습니다.
- 모든 플랫폼: 역할 전환 + muteLocalAudio 호출 시 음질 및 시작 효과를 최적화했습니다.
- 모든 플랫폼: 대역폭 예측 onSpeedTest에 대한 콜백을 최적화했습니다.
- iOS: 힙 메모리 문제를 피하기 위해 메모리 관리를 최적화했습니다.
- Android: 일부 휴대폰에서 인이어 모니터링 지연을 최적화했습니다.
- Windows: 영상 수신 시 영상 렌더링 연동 성능을 최적화하였습니다.
- Windows: 에코 문제를 효과적으로 방지하기 위해 스테레오 캡처 로직을 최적화했습니다.

**오류 수정:**
- 모든 플랫폼: 방 퇴장(onExitRoom)에 대한 콜백의 reason 예외가 수정되었습니다.
- 모든 플랫폼: 사용자 정의 영상 전송 시 타임스탬프가 같을 때 검은 화면이 나오는 현상이 수정되었습니다.
- 모든 플랫폼: startLocalAudio 전에 muteLocalAudio가 호출될 때 발생하는 오디오 crash를 수정했습니다.
- 모든 플랫폼: 3A를 수동으로 설정하지 않고 사용자 지정 오디오 캡처를 활성화하면 3A가 켜지는 현상이 수정되었습니다.
- 모든 플랫폼: 사용자 지정 오디오 렌더링에서 간헐적으로 발생하는 노이즈 문제가 수정되었습니다.
- iOS: log 경로가 중간(setLogDirPath)으로 설정되고 샌드박스가 변경될 때 메모리 유출이 수정되었습니다.
- iOS&Mac: 시스템 오디오 서비스가 비정상적일 때 연속 BGM 재생 시나리오에서 크래쉬가 수정되었습니다.
- Android: 간헐적으로 블루투스 헤드셋이 지속적으로 재연결되는 문제가 수정되었습니다.
- Android: 특정 휴대폰에서 간헐적으로 오디오가 들리지 않는 문제가 수정되었습니다.
- Android: Redmi 등 특정 폰에서 이어폰을 반복적으로 꽂았다 뺐다 할 때 발생하는 크래쉬를 수정하였습니다.
- Windows&iOS: 화면 캡처 오류가 수정되었습니다.
- Windows: 미러링을 켠 후 vod 플레이어를 끄면 crash가 발생하는 문제를 수정했습니다.
- Windows: generateCustomPts가 pts 스트리밍에서 사용되지 않아 다중 스트림으로 인해 pts가 롤백될 수 있는 문제가 수정되었습니다.
- Windows: 정지 기능이 활성화된 후 간헐적으로 크래쉬가 발생하는 문제를 수정했습니다.
...
**문제 수정**
- iOS: 간혹 App 강제 종료 시 크래쉬가 발생하는 문제가 수정되었습니다.
- Android: 프레임 레이트 샘플링이 높을 경우 뷰티 필터 효과에 오류가 발생하는 문제가 수정되었습니다.
- Windows: 고 DPI 환경에서 화면 공유 시 간혹 크래쉬가 발생하는 문제가 수정되었습니다.
...
- iOS: 간혹 replaykit 확장 녹화 종료 시 크래쉬가 발생하는 문제가 수정되었습니다.
...
- 전체 플랫폼: 간혹 발생하는 크래쉬 문제 중 일부가 해결되어 SDK 안정성이 향상되었습니다.
...
- iOS: 서브 방(TRTCCloud.createSubCloud)에서 muteRemoteAudio 호출 시 크래쉬가 발생하는 문제가 수정되었습니다.
- iOS: 간혹 렌더링 중 크래쉬가 발생하는 문제가 수정되었습니다.
- iOS: 프론트엔드/백엔드 전환 시 일부 iPad에서 비디오 랜더링 중 간혹 메인 스레드가 크래쉬되는 문제가 수정되었습니다.
...
- iOS: iOS 10에서 배경음악 인터페이스를 특정 규칙의 파일 경로로 이동했을 때 크래쉬가 발생하는 버그가 수정되었습니다.
...
- 오디오 처리 시 간혹 데이터 패키지에서 크래쉬가 발생하는 문제가 수정되었습니다.
...
- 사용자 정의 비디오 샘플링 시 간혹 SDK 내부 OpenGL 컨텍스트 오류 크래쉬가 발생하는 문제가 수정되었습니다.
...
- 하드 코더에서 간혹 크래쉬가 발생하는 문제가 수정되었습니다.
- 로컬 오디오 녹화 시 간혹 끊김 현상이 발생하는 버그가 수정되었습니다.
- 푸시 스트림 중지(muteLocalVideo, muteLocalAudio) 시 강제 종료 또는 크래쉬 후 다시 방에 입장하면 오디오/비디오가 자동 재생되지 않는 문제가 수정되었습니다.
...
- 간혹 HTTP 컴포넌트에서 크래쉬가 발생하는 문제가 수정되었습니다.
...
- 안정성이 개선되고, 간혹 발생하는 크래쉬 문제가 해결되었습니다.
