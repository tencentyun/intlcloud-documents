...
**새로운 기능**
- 전 플랫폼: 외부 연결 오디오 기기에 대한 이상 검증이 추가되었습니다. onStatistics 콜백 등록 후, [TRTCLocalStatistics](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCTypeDef__cplusplus.html#structtrtc_1_1TRTCLocalStatistics)의 audioCaptureState로 장시간 음소거, 파음, 이상 끊김 문제를 실시간으로 검증할 수 있습니다.
- Windows: 사용자 정의 수집은 RGBA 포맷의 비디오 데이터 입력을 지원합니다.

**품질 최적화**
- 전 플랫폼: BGM 리소스 관리를 최적화하여 메모리 점유율을 즉시 릴리스합니다.
- 전 플랫폼: 푸시 스트리밍측에서 백그라운드 퇴장 후 비디오 업스트림을 일시 중지하면 재생측에서 [onUserVideoAvailable(false)](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloudCallback__cplusplus.html#a091f1c94ff1e2bc39c36e9d34285e87a) 공지를 즉시 수신할 수 있습니다.
- Mac: 화면 공유 시 마우스의 CPU 및 메모리 점유율 최적화

**문제 수정**
- Android : setRemoteViewFillMode 일부 모델에 간혹 적용되지 않는 문제를 수정했습니다.
- iOS/Mac: 사용자 정의 뷰티 필터 중지 시 메모리 리소스가 릴리스되는 문제를 수정했습니다.
...
- iOS&Mac: 높은 CPU 시나리오에서 사용자 정의 오디오 수집 적합성이 최적화되었습니다.
- iOS&Android: 비디오 녹화 시 서브 채널을 통한 공유로 데스크톱 버전 정렬을 지원합니다.
- Mac: Apple M1 아키텍처에 대한 네이티브 지원이 추가되었습니다.
- Windows: 메모리 할당 로직 최적화로 안정성을 높였습니다.


## Version 8.5 @ 2021.03.24
**새로운 기능**
-  Mac: 화면 공유 기능을 최적화하여 타깃 창을 공유하면서 다른 창을 지정하여 함께 공유할 수 있습니다. 이와 관련한 내용은 API [addIncludedShareWindow](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloud__ios.html#a2e101f0ff00c8752eea1fa9a1a432233)를 참조하십시오.
-  모든 플랫폼: 재생 기능을 추가하여 [TXVodPlayer](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TXVodPlayer__android.html#classcom_1_1tencent_1_1rtmp_1_1TXVodPlayer)를 사용해 TRTCCloud와 바인딩하고, VOD가 현재 재생중인 콘텐츠를 TRTC의 서브 채널 푸시 스트림을 통해 공유합니다.
-  모든 플랫폼: 서브 채널 사용자 정의 수집 기능을 추가하였습니다. 이와 관련한 내용은 API [sendCustomVideoData](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__ITRTCCloud__cplusplus.html#aeeff994b8a298fa4948a11225312f629)를 참조하십시오.
...
- Windows: 슬라이드 쇼 창을 선택하여 화면 공유 시 슬라이드 쇼 창으로의 자동 전환을 지원합니다.
- 모든 플랫폼: 상태 콜백으로 end-to-end 딜레이를 증가시킵니다.

**문제 수정**
- iOS: 일부 장치에서 간혹 백그라운드 OpenGL 렌더링 중 발생하는 crash 문제가 최적화되었습니다.
- iOS: 스크린 화면 정지 시 재생 중인 화면 공유를 재생할 수 없는 문제가 최적화되었습니다.
...
**품질 최적화**
- 모든 플랫폼: clubhouse와 같은 음성 라이브 방송 시나리오에 더욱 적합하도록 [Music](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloudDef__ios.html#ga865e618ff3a81236f9978723c00e86fb) 모드에서의 음질이 최적화되었습니다.
...
-  iOS: snapvideoshot으로 인해 CAAnimation 애니메이션 랙이 발생하는 문제를 했습니다.
-  iOS&Mac: 동일한 View에서 카메라와 스크린 공유 화면을 번갈아 표시할 때 스크린 공유 화면의 블랙 스크린 문제를 수정했습니다.
-  iOS: iPhone 6s에서 3rd party 뷰티 필터 모듈 사용 시 화면이 뿌옇게 되는 문제를 수정했습니다.
...
- iOS & Android & Mac: 오디오 모듈 최적화를 통해 [enableCustomAudioCapture](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloud__ios.html#ab8f8aaa19d70c6a2c9d62ecceb6e974d)로 캡처한 오디오 데이터를 SDK에 전송하여 처리할 때 SDK가 에코 억제 및 노이즈 감소 효과를 유지할 수 있도록 하였습니다.
...
- iOS: VODPlayer 감속 재생 시 크래쉬 발생 문제가 수정되었습니다.
- iOS: 가끔 방에 입장한 후 헤드셋에서 오디오가 재생되는 문제가 수정되었습니다.
- iOS & Android: 에코 제거 및 잡음 억제 효과가 최적화되어 인이어 모니터링에서도 잔향 효과를 들을 수 있게 되었습니다.
...
- Android: 비디오 렌더링 모듈 TXCloudVideoView가 `addVideoView(new TextureView(getApplicationContext()))` 인터페이스를 지원하여 TextureView를 로컬 렌더링에 사용합니다.
- Android: 사용자 정의 렌더링 콜백이 RGBA 포맷의 비디오 데이터를 지원합니다.
- Windows: 로컬 카메라 수집과 원격 비디오 스트림 화면 캡처 재생을 지원합니다. [ITRTCCloud.snapshotVideo](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloud__ios.html#a3769ecbff6c0c4ee7cc5e4b40aaafe96)를 참조하십시오.
...
**품질 최적화**
- Android: 온라인 라이브 방송 인코딩 품질이 최적화되어 더욱 선명한 영상을 제공합니다.
...
- 모든 플랫폼에 API TRTCCloud.switchRoom이 추가되어 샘플링을 정지하지 않고도 방을 전환할 수 있게 되었습니다.
...
- 모든 플랫폼에 API TRTCCloud.setLocalRenderParams 로컬 이미지 설정의 렌더링 설정이 추가되었습니다.

**최적화**
- Android에서 소프트웨어/하드웨어 디코딩 전환 로직이 최적화되었습니다.
- Windows에서 System loopback 오디오 샘플링 음질 및 에코 제거 효과가 최적화되었습니다.
- Windows에서 오디오 디바이스 선택 로직이 최적화되어 Silent rate가 감소했습니다.
- Windows에서 동시 음성 제거 효과가 최적화되었습니다.
- 모든 플랫폼에서 수동 수신 모드에서 역할 전환 시 발생하는 바로 재생 효과가 최적화되었습니다.
...
- Windows에서 프로세스 종료 시 간혹 크래쉬가 발생하는 문제가 수정되었습니다.
- Windows에서 블루투스 이어폰 지원이 최적화되어 블루투스 이어폰에서 소리가 나지 않는 문제가 수정되었습니다.
...
**추가**
- Mac: 화면 공유가 선택된 창의 필터링 기능을 지원합니다. 공유하지 않을 창을 제거함으로써 보다 개선된 사생활 보호를 제공합니다.
- Windows: 화면 공유의 ‘공유 중’ 알림창 프레임 색상과 프레임 크기 설정 기능을 지원합니다.
- Windows: 화면 공유 기능으로 전체 화면을 공유할 때 고성능 모드 활성화를 지원합니다.
- 모든 플랫폼: 사용자 정의 암호화 기능을 지원하여 인코딩 후의 멀티미디어 데이터를 노출된 C 인터페이스를 통해 2차 처리할 수 있게 되었습니다.
- 모든 플랫폼: TRTCRemoteStatistics에 오디오 랙 정보 콜백 `audioTotalBlockTime`과 `audioBlockRate`가 추가되었습니다.

**최적화**
- iOS: 오디오 모듈의 실행 속도가 최적화되어 첫 번째 오디오 프레임을 더 빨리 샘플링하여 전송할 수 있게 되었습니다.
- Windows: 시스템 루프의 에코 제거 알고리즘이 최적화되어 시스템 루프백(SystemLoopback) 활성화 시 에코 제거 능력이 강화되었습니다.
- Windows: 화면 공유 기능의 창 샘플링 차폐 방지 기능이 최적화되어 창 필터 설정 기능을 지원합니다.
- Android: 대부분의 Android 모델에서 인이어 모니터링 효과가 최적화되어 인이어 모니터링 딜레이가 적정 수준까지 감소되었습니다.
- Android: Music 모드(startLocalAudio일 시에 지정)에서의 P2P 딜레이가 최적화되었습니다.
...
- iOS: 가끔 replaykit 확장 녹화 종료 시 크래쉬가 발생하는 문제가 수정되었습니다.
- iOS: 시뮬레이터 환경에서의 컴파일 문제가 해결되었습니다.
- Android: 일부 모바일 기기가 App에서 장기간 백그라운드로 전환된 이후에 다시 프론트로 전환될 경우 가끔 오디오-비디오 동기화가 이루어지지 않는 문제가 수정되었습니다.
...
- 모든 플랫폼: 가끔 발생하는 크래쉬 문제 중 일부가 해결되어 SDK 안정성이 향상되었습니다.
...
- Windows: 가상 카메라 e2eSoft Vacm이 호환됩니다.
...
- Windows: 오디오 실행 로직이 최적화되어 재생 상황에 한정하여 마이크를 점용하지 않습니다.



**수정**
- iOS: iPhone SE에서 오디오 재생 음량이 작은 문제가 수정되었습니다.
- iOS: 하위 방(TRTCCloud.createSubCloud)에서 muteRemoteAudio 호출 시 크래쉬가 발생하는 문제가 수정되었습니다.
- iOS: 가끔 렌더링 중 크래쉬가 발생하는 문제가 수정되었습니다.
...
- Mac: 화면 공유의 지정 영역 면적이 0일 경우 크래쉬가 발생하는 문제가 수정되었습니다.
- Android: READ_PHONE_STATE 권한이 설정되지 않았을 경우 Android5.0 기기에서 크래쉬가 발생하는 문제가 수정되었습니다.
- Android: 블루투스 이어폰을 제거한 후 재연결했을 때, 오디오 샘플링 및 재생 시 오류가 발생하는 문제가 수정되었습니다.
- Android: 알려진 크래쉬 문제가 수정되었습니다.
- Windows: 64비트 SDK에서 화면 공유 기능을 여러 번 활성화할 경우 크래쉬가 발생하는 문제가 수정되었습니다.
- Windows: 일부 시스템에서 OpenGL을 사용할 경우 크래쉬가 발생하는 문제가 수정되었습니다.
...
- iOS: 내부 스레드 모델이 최적화되어 30번 경로 이상에서 동시 재생 시나리오의 실행 안정성이 향상되었습니다.
- iOS & Android: Audio 모듈 성능이 최적화되어 SOF의 샘플링 딜레이가 향상되었습니다. 최신 버전에서는 첫 번째 오디오 프레임을 더 빨리 획득할 수 있습니다.
- iOS & Android: VOD 플레이어와 TRTC를 동시에 사용할 경우의 음량 크기 및 음질이 최적화되었습니다.
- iOS & Android: wav 오디오 포맷의 배경 음악과 효과음 파일 지원이 추가되었습니다.
- Windows: 일부 저가형 카메라의 과도하게 높은 CPU 사용률 문제가 최적화되었습니다.
- Windows: 멀티 USB 카메라와 마이크에 대한 호환성 최적화로 디바이스의 실행 성공률 향상
- Windows: 카메라와 마이크 기기의 선택 정책이 최적화되어 카메라 또는 마이크 사용 중 소켓으로 인해 샘플링 오류가 발생하는 문제를 피할 수 있게 되었습니다.

**수정**

- 모든 플랫폼: 네트워크가 약한 경우, muteLocalVideo 및 muteLocalAudio 인터페이스 호출 시 가끔 재생 오류가 발생하는 버그가 수정되었습니다.
- iOS: 저가형 iPhone 또는 iPad에서 오디오 재생 시 가끔 재생에 실패하는 버그가 수정되었습니다.
- iOS: iPad Pro에서 화면 공유 기능으로 인해 공유된 화면이 늘어나는 문제가 수정되었습니다.
- iOS: 사용자가 권한을 거절했음에도 App 내부 화면 기여가 계속해서 화면 녹화 권한 신청을 팝업하는 문제를 수정했습니다.
- Windows: 노트북 또는 데스크톱이 장기간 휴면 상태일 때, 나가기 [onExitRoom](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__ITRTCCloudCallback__cplusplus.html#a0a45883a23a200b0e9ea38fdde1da4bd) 이벤트 공지가 콜백되지 않는 문제를 수정했습니다.
- Windows: Music 음질 모드에서 시스템 오디오 루프백 [stopSystemAudioLoopback](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__ITRTCCloud__cplusplus.html#aab0258238e4414c386657151d01ffb23)을 활성화하면 에코가 누출되는 문제를 수정했습니다.
...
- Windows: 현재 Windows PC를 전역 음소거로 설정할 수 있는 [setCurrentMicDeviceMute](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__ITRTCCloud__cplusplus.html#a8a8badf62eee1021f9315f11df0f597f) 인터페이스를 추가하였습니다.
...
- Mac: 현재 Mac을 전역 음소거로 설정할 수 있는 [setCurrentMicDeviceMute](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloud__ios.html#a88569e62fe75b7ea98cc012169f22bfe) 인터페이스를 추가하였습니다.
...
- Windows: 알려진 크래쉬 문제 다수가 수정되었습니다.
- Windows: 카메라와 마이크를 뺀 후 다시 꽂았을 때 자동으로 장치가 활성화되지 않는 문제가 수정되었습니다.
- iOS: iOS 10에서 배경 음악 인터페이스를 특정 규칙의 파일 경로로 이동했을 때 크래쉬가 발생하는 버그가 수정되었습니다.
...
- 인이어 모니터링 딜레이 시간이 한층 더 최적화되었습니다.
...
- 비디오 인코딩 매개변수 setVideoEncoderParam에 minVideoBitrate 옵션이 추가되었습니다. 화질 요구사항이 높은 라이브 방송 클라이언트에게 권장합니다.
...
- 카메라 샘플링 해상도 선택 로직이 최적화되어 시각 효과가 향상되었습니다.
- 에코 처리 효과가 최적화되었습니다.
...
- 사용자 정의 비디오 샘플링 시 가끔 SDK 내부 OpenGL 컨텍스트 오류 크래쉬가 발생하는 문제가 수정되었습니다.
...
- 하드코더에서 가끔 크래쉬가 발생하는 문제가 수정되었습니다.
- 로컬 오디오 녹화 시 가끔 끊김 현상이 발생하는 버그가 수정되었습니다.
- 푸시 스트리밍 일시 중지(muteLocalVideo, muteLocalAudio) 시 강제 종료 또는 크래쉬 후 다시 방에 입장하면 멀티미디어가 자동 재생되지 않는 문제가 수정되었습니다.
...
Android에 핸드폰 녹화 기능이 추가되었습니다. 해당 기능은 핸드폰을 사용한 라이브 방송을 지원합니다.

**최적화**

- 통화 시나리오에서 중저가 Android 핸드폰의 성능 소모량이 최적화되어 통화 품질이 향상되었습니다.
...
- C++ STL 기초 라이브러리의 완전한 정적 컴파일
- 통화 음량이 기본적으로 ANS, AGC를 활성화하여 통화 모드에서의 음질이 향상되었습니다.
- 혼합 스트림 사전 설정 템플릿 활용성이 최적화되었습니다.
...
- 시스템에 의해 전화가 중단된 이후 업스트림 음량이 2배로 증가하고 노이즈가 섞이는 문제가 수정되었습니다.
...
- 약한 네트워크에서의 지터 방지 기능이 최적화되어 네트워크가 약한 환경에서 오디오가 보다 원활해졌습니다.
...
- API: pauseAudioEffect, resumeAudioEffect 음향효과가 추가되어 일시 중지/복구 제어 기능을 지원합니다.
- API: setBGMPlayoutVolume, setBGMPublishVolume, BGM이 추가되어 로컬 재생과 푸시 스트림 오디오 믹싱을 지원합니다.
- API: setRemoteSubStreamViewRotation 서브 채널 비디오 재생이 추가되어 렌더링 각도 회전 조정을 지원합니다.
...
- 시나리오 TRTCAppSceneAudioCall이 추가되었습니다. enterRoom 시 설정할 수 있습니다. 해당 시나리오에서 음성 통화와 관련된 TRTC SDK에 대한 전반적 최적화가 이루어졌습니다.
- 시나리오 TRTCAppSceneVoiceChatRoom이 추가되었습니다. enterRoom 시 설정할 수 있으며, TRTC SDK를 활성화하면 음성 인터렉션 채팅방 시나리오를 최적화할 수 있습니다.

**최적화**

- 비디오 스트리밍 중 중단에 대한 녹화 서비스의 저항력을 최적화하여 원격 녹화 파일의 완전성을 확보했습니다.
- 일부 모델에서 하드웨어 디코딩 시 오디오/비디오 동기화가 이루어지지 않는 문제가 최적화되었습니다.
- 비디오 화면이 1080P 고해상도 샘플링을 지원하여 핸드폰 라이브 방송을 PC로 시청하는 시나리오에서의 화면 해상도가 향상되었습니다.
- 에러 코드 최적화로 방 입장 에러 코드가 간소화되었습니다.
- 바로 재생 속도가 느린 문제가 최적화되었습니다.

**수정**

- 가끔 HTTP 모듈에서 크래쉬가 발생하는 문제가 수정되었습니다.
...
- 엔터프라이즈 버전에 피부 미백, 치아 미백, 주름 개선, 눈밑 지방 제거 등과 같은 신규 포토샵 기능이 추가되었습니다.
- onRemoteUserEnterRoom / onRemoteUserLeaveRoom 콜백이 추가되었습니다. 마이크를 켜지 않은 호스트의 라이브 방송 입장/퇴장 공지를 지원합니다.
...
- 방 입장 에러 코드가 통합됩니다. onEnterRoom 콜백을 통해 result&lt;0 방 입장 오류가 표시됩니다.
...
- 멀티미디어 최적화를 위한 UDP 프로토콜을 채용함으로써 약한 네트워크에 대한 저항 능력이 강화되었습니다.
- 평균 시청 딜레이를 1초로 만들어 시청자와 호스트 사이의 적극적인 인터렉션이 가능해졌습니다.
- 하나의 방에 최대 10만 명이 입장할 수 있게 되었습니다.

**최적화**

- 약한 네트워크 환경에서의 오디오-비디오 동기화가 이루어지지 않는 버그가 최적화되었습니다.
- onStatistics 상태 콜백이 최적화되었습니다. 콜백이 존재하는 스트림에 한합니다.
- 라이브 방송 TXLivePlayer 재생 버퍼 로직이 최적화되어 랙 비율이 감소했습니다.
- 선 muteLocalVideo 이후 재생 취소 시 화면 복구 속도가 최적화되었습니다.
- 딜레이와 패킷 손실률이 높은 네트워크 환경에서의 QoE 알고리즘이 최적화되어 약한 네트워크에 대한 저항력이 강화되었습니다.
- 디코더 성능을 최적화하여 초저가형 Android 핸드폰에서 딜레이가 점차 증가하는 버그를 수정했습니다.
- 오디오 평가 알고리즘(enableAudioVolumeEvaluation)이 최적화되어 음량 평가가 더욱 빠르게 이루어집니다.
- 영상 통화(TRTCAppSceneVideoCall) 모드에서의 QoE 알고리즘이 최적화되어 1v1 통화 모드의 경우 약한 네트워크 환경에서의 원활함이 더욱 향상되었습니다.
...
- 오디오 수집 비활성화 이후 재생을 해도 소리가 들리지 않는 버그가 수정되었습니다.
...
- 엔터프라이즈 버전에 눈 확대, 안면 축소, V라인, 움직이는 스티커 기능이 추가되었습니다.

**최적화**

- 약한 네트워크 환경에서의 유창성이 향상되었습니다.
- 음량 크기 콜백 알고리즘이 최적화되어 음량 콜백 수치가 더욱 합리적으로 변경되었습니다.
...
- Android에서 마이크 권한을 비활성화한 이후 오류 콜백이 오지 않는 버그가 수정되었습니다.
...
- 사용자 정의 오디오 데이터 콜백 인터페이스: TRTCCloud >> setAudioFrameDelegate가 추가되었습니다. 다음을 지원합니다.
...
- 크로스 룸 통화 기능인 connectOtherRoom이 추가되었습니다. 이미 존재하는 두 개의 TRTC 방을 서로 연결하는 기능으로, 라이브 룸의 호스트 PK에 사용됩니다.
...
- TRTCCloud의 다중 인스턴스 기능을 취소하고 생성 모드를 단일 항목 모드로 변경하여 다수의 TRTCCloud 인스턴스가 서로의 네트워크 리소스를 점유하는 일이 없도록 하였습니다.
...
- CDN 푸시와 혼합 스트림 구현이 최적화되었습니다.
- 방 입장이 라이브 방송과 영상 통화 시나리오로 구분됩니다.
- 안정성이 개선되고, 가끔 발생하는 크래쉬 문제가 해결되었습니다.
...
- 새로운 Audio 모듈을 채용하여 각종 네트워크 상황에서의 오디오 품질이 대폭 최적화되었습니다.
- 대형/소형 스트리밍 듀얼 채널 인코딩 기능을 지원합니다(Windows와 Mac 기기에서만 활성화할 것을 권장).
