버전 번호 `major.minor.patch`의 자세한 규칙은 다음과 같습니다.
  - major: 메이저 버전 번호로, 중요한 버전 재구성 시 해당 필드의 숫자가 증가하며 일반적으로 각 메이저 버전 간의 인터페이스는 호환되지 않음.
  - minor: 마이너 버전 번호로, 각 마이너 버전 번호 간에는 인터페이스가 호환되며 인터페이스 추가 또는 최적화 시 해당 필드의 숫자가 증가함.
  - patch: 패치 번호로, 기능 개선 또는 오류 복구 시 해당 필드의 숫자가 증가함.

>!
> - 제품의 안정성 및 온라인 지원을 위해 항상 최신 버전을 유지하시기 바랍니다.
> - 버전 업데이트에 대한 주의 사항은 [업데이트 가이드](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-00-info-update-guideline.html)를 참고하시기 바랍니다.

## Version 4.11.4 @2021.08.20

**Improvement**

- oppo & vivo 내장 브라우저에서 H.264 지원 감지 정확도 개선.
- 자동 복구 수집 로직 추가(디바이스 수집 이상 경고 시 트리거).
- subscribe 인터페이스의 초과 시간 로직 추가. [API_CALL_TIMEOUT](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/module-ErrorCode.html#.API_CALL_TIMEOUT) 에러 코드 참고.

**Bug Fixed**

- 일부 iOS Safari 이전 버전에서 간혹 스트림을 가져오지 못하는 문제 수정.
- 디바이스 전환 후 음소거 상태가 정확하지 않은 문제 수정.
- 방 입장 시간 초과 후 방 입장 인터페이스를 다시 호출할 때 간혹 발생하는 이상 경고 문제 수정.
- 원격 푸시 스트림 취소 후, 오디오 및 비디오 플레이어가 제 시간에 폐기되지 않는 문제 수정.

## Version 4.11.3 @2021.07.30

**Improvement**

- publish & subscribe 인터페이스 이상 경고 처리 로직 최적화.
- 믹싱 플러그인의 복구 전략 최적화.

**Bug Fixed**

- 간혹 Peer-leave 알림이 정확하지 않은 문제 수정.

## Version 4.11.2 @2021.07.23

**Improvement**

- 연결 성공률을 향상시키기 위해 turn server 스케줄링 지원.
- [Client.getRemoteMutedState](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/Client.html#getRemoteMutedState) 원격 소형 푸시 스트림 여부를 표시하기 위해 새로운 속성인 hasSmall 추가.

**Bug Fixed**

- LocalStorage가 비활성화된 경우 SDK를 사용할 수 없는 문제 수정.
- 간헐적으로 publish가 비정상적일 때 인터페이스가 rejected되지 않는 문제 수정.
...
- 뷰티 필터 플러그 인 지원. [뷰티 필터 활성화](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-28-advanced-beauty.html) 참고.
- 데이터 통계 정확도 최적화.
...
대형/소형 스트림 지원. [대형/소형 스트림 전송 활성화](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-27-advanced-small-stream.html) 튜토리얼 참고.
...
- 품질 Statistics 로직을 최적화하고 서버 API를 지원하여 통화 품질 데이터 획득.
- [ClientEvent.NETWORK_QUALITY](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/module-ClientEvent.html#.NETWORK_QUALITY) 이벤트 rtt 및 loss 데이터 반환.
- 반복 호출의 이상 경고를 방지하기 위해 인터페이스 인증 로직 최적화.
- 시간 소모적인 오디오 재생을 줄이기 위해 재생 로직 최적화.
...
- 인터페이스 [Client.startPublishCDNStream](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/Client.html#startPublishCDNStream)을 추가하여 Tencent Cloud CDN 및 서드 파티 CDN에 스트림 푸시 전송.
- 인터페이스 [Client.stopPublishCDNStream](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/Client.html#stopPublishCDNStream)을 추가하여 Tencent Cloud CDN 및 서드 파티 CDN에 스트림 푸시 중지.
...
- 클라우드 혼합 스트림에서 프리셋 레이아웃 모드 지원. 자세한 내용은 [Client.startMixTranscode](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/Client.html#startMixTranscode) 인터페이스 참고.
- 음량 콜백 지원. 자세한 내용은 [Client.enableAudioVolumeEvaluation](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/Client.html#enableAudioVolumeEvaluation) 인터페이스 참고.
...
- 자동 재생 제한 시 처리 로직 최적화. 자세한 내용은 [자동 재생 제한 처리 제안](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-21-advanced-auto-play-policy.html)을 참고.
- 디바이스 연결/해제 자동 복구 실패 시 처리 로직 최적화. 자세한 내용은 [DEVICE_AUTO_RECOVER_FAILED](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/module-ErrorCode.html#.DEVICE_AUTO_RECOVER_FAILED)를 참고.
...
- 방 입장 인터페이스 roomId 매개변수 검증 로직 최적화. 자세한 내용은 [인터페이스 문서](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/Client.html#join) 및 [업데이트 가이드](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-00-info-update-guideline.html) 참고.
...
폐기된 인터페이스 삭제: setDefaultMuteRemoteStreams, [TRTC.createClient의 autoSubscribe 매개변수](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/TRTC.html#.createClient) 인터페이스로 대체하여 사용.
...
- 모든 플랫폼에서 문자열 방 번호 지원. 자세한 내용은 [TRTC.createClient의 useStringRoomId 매개변수](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/TRTC.html#.createClient) 참고.
- 자동 구독 비활성화 지원. 자세한 내용은 [TRTC.createClient의 autoSubscribe 매개변수](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/TRTC.html#.createClient) 참고.
...
- Safari 화면 공유 지원. 자세한 내용은 [화면 공유 사용 튜토리얼](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-16-basic-screencast.html) 참고.
...
- [TRTC.isScreenShareSupported](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/TRTC.html#.isScreenShareSupported) Safari는 화면 공유 미지원.
...
휴대폰 QQ 브라우저 검증 추가. 휴대폰 QQ 브라우저에서는 현재 TRTC 데스크톱 브라우저 SDK 미지원.
...
- V8 최적화 후 서드 파티 라이브러리로 인해 발생하는 문제 수정.
...
클라이언트 이벤트 바인딩 취소 인터페이스 Client.off() 추가.
...
- 오디오 비트 레이트 조정 설정 추가. 개발자는 LocalStream.setAudioProfile()을 통해 오디오 속성을 설정할 수 있으며, 현재 두 가지 Profile(standard, high) 지원.
...
- App에서 SDK의 LocalStream.setVideoProfile() 인터페이스를 통해 비디오 속성(해상도, 프레임 레이트, 비트 레이트)을 완벽하게 제어 가능. 구버전의 Tencent Cloud 콘솔을 통한 ‘화면 설정(Spear Role)’ 미지원.
- SDK는 Stream 객체에 멀티미디어 플레이어를 캡슐화하여 멀티미디어 재생을 완벽하게 제어 가능.
- 원격 스트림에 대한 구독 및 구독 취소 기능 제공. 개발자는 Client.subscribe()/unsubscribe() 인터페이스를 통해 원격 스트림의 오디오, 비디오, 멀티미디어 데이터 스트림 수신을 효율적으로 제어 가능.
