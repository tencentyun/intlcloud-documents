애플리케이션 생성 완료 후, [기능 설정]을 통해 현재 애플리케이션에 대한 [릴레이 푸시 스트림](#bypass), [클라우드 녹화](#record), [고급 권한 제어](#purview) 기능을 활성화할 수 있으며, 해당 페이지에서 설정을 수정한 후에는 약 5분 후 설정이 적용됩니다.
...
2. [릴레이 푸시 스트림 설정]에서 [릴레이 푸시 스트림 활성화]버튼을 우측으로 클릭 설정합니다.
3. 팝업된 [릴레이 푸시 스트림 기능 비활성화] 창에서 **리스크 설명을 잘 읽어주시고**, [릴레이 푸시 스트림 기능 비활성화]를 클릭하면 비활성화가 완료됩니다.
...
2. [기능 설정]>[클라우드 녹화 활성화]에서 오른쪽에 있는 [편집]을 클릭하여 클라우드 녹화 설정 수정 페이지로 이동합니다.
