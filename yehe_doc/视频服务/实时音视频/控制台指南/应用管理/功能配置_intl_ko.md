애플리케이션 생성 완료 후 [기능 설정]에서 현재 애플리케이션의 [릴레이 푸시 스트림](#bypass), [클라우드 녹화](#record), [고급 권한 제어 기능](#purview)을 활성화할 수 있으며, 해당 페이지의 모든 기능 설정이 변경되고 적용되는 데 약 5분이 소요됩니다.
...
- 릴레이 푸시 스트림은 [CDN 라이브 방송 시청 실행](https://intl.cloud.tencent.com/document/product/647/35242) 시, CSS에서 라이브 방송 보기에 따라 발생하는 다운스트림 트래픽/대역폭 관련 요금을 부과하는데 사용되며, 자세한 내용은 [CSS>트래픽/대역폭 과금](https://intl.cloud.tencent.com/document/product/267/2818) 설명을 참고하십시오.
- [클라우드 녹화 및 재생](https://intl.cloud.tencent.com/document/product/647/35426)에 릴레이 푸시 스트림을 사용할 경우, 녹화 및 녹화 파일 저장 등의 요금이 발생합니다. 자세한 내용은 [클라우드 녹화 및 재생>관련 요금](https://intl.cloud.tencent.com/document/product/647/35426#.E7.9B.B8.E5.85.B3.E8.B4.B9.E7.94.A8) 설명을 참고하십시오.
- [CSS 콘솔](https://console.cloud.tencent.com/live/domainmanage)에서 릴레이 푸시 스트림에 사용하는 푸시 스트림 도메인(`xxxx.livepush.myqcloud.com`)에 녹화, 트랜스 코딩, 음란물 감지 화면 캡처, 워터마크 등의 유료 기능 템플릿을 바인딩한 경우, 릴레이 푸시 스트림 사용 시 템플릿에 해당하는 [부가 서비스 요금](https://intl.cloud.tencent.com/document/product/267/2819)이 발생합니다.

[](id:open_bypass)
### 릴레이 푸시 스트림 기능 활성화
1. TRTC 콘솔에 접속하여 [[애플리케이션 관리](https://console.cloud.tencent.com/trtc/app)]를 선택합니다.
2. 기능 설정 변경이 필요한 애플리케이션을 선택하고 타깃 애플리케이션 라인에서 [기능 설정]을 클릭합니다.
...
- **특정 스트림 릴레이**: ‘특정 스트림 릴레이’를 선택한 후, 클라우드 믹스 트랜스 코딩 서비스가 필요하지 않은 경우 클라이언트 측 SDK API [startPublishing](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloud__cplusplus.html#a7cbe48ea2cd3fb05a5b10350b6d81265)을 호출하여 릴레이 푸시를 시작합니다. 서비스가 필요한 경우 클라우드 믹스 트랜스 코딩 문서의 지침에 따라 클라우드 믹스 트랜스 코딩 후 릴레이를 자동으로 시작합니다.
...
3. 팝업된 [릴레이 푸시 스트림 기능 비활성화] 창에서 **리스크 설명을 꼼꼼히 읽은 후** [릴레이 푸시 스트림 기능 비활성화]를 클릭하면 비활성화가 완료됩니다.
...
### 주의 사항
- TRTC 서비스는 릴레이 푸시 스트림을 통해 [CSS](https://intl.cloud.tencent.com/document/product/267)로 전 과정 클라우드 녹화 기능을 제공하며 녹화한 파일을 [VOD](https://intl.cloud.tencent.com/document/product/266) 플랫폼에 저장합니다.
- 녹화 기능은 CSS 서비스를 이용하여 LVB의 라이브 방송 녹화 요금이 발생하며, 당월 라이브 방송 녹화 피크 동시 접속 수를 청구 기준으로 합니다. 자세한 과금 규정은 [CSS>라이브 방송 녹화 과금 설명](https://intl.cloud.tencent.com/document/product/267/39605)을 참고하십시오.
...
[](id:open_record)
### 클라우드 녹화 기능 활성화
TRTC의 클라우드 녹화는 방 안 모든 사용자의 멀티미디어 스트림을 하나의 독립된 파일로 녹화할 수 있습니다. 클라우드 녹화 기능 활성화 방법에 대한 자세한 가이드는 [클라우드 녹화 및 재생](https://intl.cloud.tencent.com/document/product/647/35426#open)을 참고하십시오.

[](id:change_record)
### 클라우드 녹화 설정 변경
>! 클라우드 녹화 설정을 변경하는 경우 현재 온라인에서 실행되고 있는 작업 데이터에 영향을 미칠 수 있습니다. 리스크를 확인한 후 신중하게 변경하시기 바랍니다.

1. [[애플리케이션 관리](https://console.cloud.tencent.com/trtc/app)]를 클릭하여 클라우드 녹화 설정 변경이 필요한 애플리케이션을 선택한 후, 타깃 애플리케이션 라인에서 [기능 설정]을 클릭합니다.
...
4. 실제 상황에 따라 [설정 정보](https://intl.cloud.tencent.com/document/product/647/35426#recordType)를 수정하고 [확인]을 클릭하면 변경 사항이 저장됩니다.
...
3. 비활성화 후 미치는 영향을 자세히 읽어주십시오. [클라우드 녹화 비활성화]를 클릭하면 클라우드 녹화가 비활성화됩니다.
...
[](id:purview)
## 고급 권한 제어 기능
특정 방에 입장 제한 또는 마이크 켜짐 제한 기능을 추가하고 싶은 경우, 즉, 지정된 사용자만 입장 또는 마이크를 켤 수 있도록 허용하고 싶지만 클라이언트 판단 권한에 대한 크래킹 공격이 염려될 경우 [고급 권한 제어 기능 활성화](https://intl.cloud.tencent.com/document/product/647/35157)를 고려해볼 수 있습니다.
...
2. [기능 설정]>[고급 권한 제어 기능]에서 [고급 권한 제어 기능 비활성화] 오른쪽에 있는 버튼을 클릭합니다.
...
- 클라우드 믹스 스트림 트랜스 코딩 시 사용자 정의 배경 이미지 설정이 필요한 경우 리소스 관리에서 해당하는 이미지 리소스를 추가할 수 있습니다. 자세한 방법은 [리소스 관리](https://intl.cloud.tencent.com/document/product/647/39081)를 참고하십시오.
