...
2. ルームID または右側の【通話詳細の確認】をクリックして、[通話詳細](https://intl.cloud.tencent.com/zh/document/product/647/39070)画面に入ります。
...
### ネットワーク状況の分析<span id="internet"></span>
...
#### ネットワークのパケットロス<span id="packet_loss"></span>
...
#### ビットレート<span id="bit_rate"></span>

正常な状況下では、ビデオと音声のビットレートは変動範囲±10%以内の曲線です。**ビットレートの急激な低下、または30%を超える変動が出現した場合は、現在のネットワークに輻輳またはジッターが発生していることを表します**。
> ! 画面共有の画面のGOPは長いため（5秒～10秒）、正常な状況では安定した周期のピークとボトムのある曲線となり、キーフレームがある時は、対応するピークがあります。

- ビデオ上りビットレートとネットワークパケットロス。
- 画面共有のビットレート曲線には安定した周期のピークがあります。

#### フレームレート<span id="frame_rate"></span>
...
### デバイス状態の確認<span id="equipment"></span>
...
ビデオと画面共有の解像度は補助的な情報で、主にRelayed live streamingおよびレコーディングファイルを再生するための判断に用いられます。ビデオの解像度に変動がある場合、[CDN](https://intl.cloud.tencent.com/zh/product/cdn) 経由でRelayed live streamingを視聴する視聴者またはビデオ再生を視聴する視聴者（特にWebユーザー）には、画面のフリーズ、ぼやけるなどの再生装置との互換性による問題が存在する可能性があります。
