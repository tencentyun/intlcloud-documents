...
The `[TRTCCalling](https://www.npmjs.com/package/trtc-calling-js)` component is an open-source class based on two closed-source SDKs: Tencent Cloud Real-Time Communication (TRTC) and Instant Messaging (IM). It supports one-to-one and group audio/video calls. For detailed instructions how to implement it, please see [Real-Time Video Call (Web)](https://intl.cloud.tencent.com/document/product/647/38927).
...
| [reject({inviteID, isBusy, callType})](#reject(.7Binviteid.2C-isbusy.2C-calltype.7D))      | Declines a call invitation. |
...
| [setMicMute(isMute)](#setmicmute(ismute))                                                     | Mutes the mic. |
...
This API is used to subscribe to an event dispatched by the component. For details about the events, please see the [event list](#event).
...
| userSig | String | Tencent Cloud's proprietary security signature. For how to calculate it, please see [FAQs > UserSig](https://intl.cloud.tencent.com/document/product/647/35166). |
...
| -------------- | ------ | ------------------------------------------------- |
| UserID | String | User ID |
| videoViewDomID | String | The video tag of this DOM ID node is removed to stop playing back the video |
...
| Parameter | Type | Description |
| ------ | ------- | -------------------------------------------- |
| isMute | Boolean | <li/>`true`: turns the mic off <li/>`false`: turns the mic on. |
...
| Parameter | Type | Description |
| ------ | ------- | -------------------------------------------- |
| profile | String | <li/>480p: 640x480 <li/>720p: 1280x720  <li/>1080p: 1920x1080  |

####  switchToAudioCall() 
This API is used to switch from video call to audio call.
>?  
>- This method is added starting from v0.10.0.
>- It can be used only during a one-to-one call.
...
| :-------------------------------------------: | :------------: | :-----------------------: |
|               [REJECT](#reject)               |     Inviter     |     The invitee declined the call.      |
...
|             [CALL_END](#call_end)             | Inviter and invitee |       The current call was ended        |
...
Trigger condition: a user joins the call.
...
Trigger condition: a user leaves the call.
...
The current call was ended.
Trigger condition: the current call is ended.
...
A user was kicked out of the room due to repeated login.
Trigger condition: a user logs in again on another page.
...
Trigger condition: a remote user turns on/off the camera.
...
Trigger condition: a remote user turns on/off the mic.
...
Trigger condition: the invitee declines the call, and the inviter receives the `REJECT` event callback.
...
The invitee didn't answer the call.
Trigger condition: the invitee does not answer the call after the timeout period set for the `call/groupCall` elapses, and the inviter receives the `NO_RESP` event callback.
...
The invitee was on a call and the line was busy.
Trigger condition: the invitee is already on another call, and the inviter receives the `LINE_BUSY` event callback.
...
Trigger condition: there is a call invitation, and the invitee receives the `INVITED` event callback.
...
The current call was canceled.
Trigger condition: the inviter cancels the call during calling, and the invitee receives the `CALLING_CANCEL` event callback.
...
The current call was not answered after timeout.
Trigger condition: the invitee does not answer the call after the timeout period set for the `call/groupCall` elapses, and the invitee receives the `CALLING_TIMEOUT` event callback.
...
| --------- | ----------- | ----------------------------- |
| 60001 | Method call failure | Failed to call `switchToAudioCall` |
| 60002 | Method call failure | Failed to call `switchToVideoCall` |
