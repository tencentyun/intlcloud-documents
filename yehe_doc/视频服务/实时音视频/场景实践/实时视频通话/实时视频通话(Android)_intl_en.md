...
| TRTCVideoCallHistoryActivity.java| UI for displaying historical contacts. |
...
1. Call `TRTCVideoCallImpl.sharedInstance(context)` to get the component instance.
...
1. Caller: the caller can call the `call()` method in `ITRTCVideoCall` and specify the callee's `userid` to initiate a video call request.
...
// 1. Initialize the `TRTCVideoCall` component
sCall.init();
...
1. Caller: to initiate a group video call, the caller needs to call the `groupCall()` function in `ITRTCVideoCall` and pass in the required user list (`userIdList`) and optional IM group ID (`groupId`).
