...
１．マウスを該当するカードまで移動し、【[Github](https://github.com/tencentyun/TRTCSDK/tree/master/Android)】をクリックしてGithub（または【[ZIP](https://liteavsdk-1252463788.cosgz.myqcloud.com/TXLiteAVSDK_TRTC_Android_latest.zip)】をクリック）にジャンプし、関連するSDK および付属のDemoのソースコードをダウンロードします。
...
| ファイルまたはフォルダ                     | 機能の説明                                                     |
| -------------------------------- | ------------------------------------------------------------ |
| TRTCVideoCallActivity.java       | ビデオ通話のメインインターフェースが表示され、このインターフェースで通話の応答と拒否が完了します。 |
| TRTCCallingEntranceActivity.java | 連絡先を選択するためのインターフェースを表示するために使用され、このインターフェースを介して登録済みユーザーを検索し、通話を開始できます。 |
| videolayout                      | ビデオ画像のレンダリングおよびレイアウトロジックを完了するために使用されます。
...
1. 発信者： `TRTCCalling` の `call()` メソッドをコールして通話リクエストを開始し、ユーザーID（userid）と通話タイプ（type）を渡して、通話タイプパラメータを`TYPE_VIDEO_CALL`に渡します。
2. 受信者：受信者がログイン状態の場合は、`onInvited()`という名称のイベント通知を受け取り、コールバックにおける `callType` のパラメータは発信者が入力した通話タイプであり、このパラメータを介して対応するインターフェースを起動します。受信者がログインしていない状態でも通話リクエストを受け取りたい場合は、[オフライン応答](#model.offline)をご参照ください。
3. 受信者：電話に対応したい場合、受信者は `accept()` 関数をコールすると同時に `openCamera()` 関数をコールして、自身のローカルCCDカメラを起動します。受信者は`reject()` をコールし、この通話を拒否することもできます。
4. 双方の音声およびビデオチャネルが確立した後、通話する双方は  `onUserVideoAvailable()`という名称のイベント通知を受け取り、相手方のビデオ画面を取得済みであることが表示されます。この時、双方のユーザーは`startRemoteView()`をコールし、リモートのビデオ画面を表示することができます。リモートの音声はデフォルトで自動再生に設定されています。
...
sCall.addDelegate(new TRTCCallingDelegate() {
    ...
    public void onInvited(String sponsor, final List<String> userIdList, boolean isFromGroup, int callType) {
        ...
        sCall.accept();
        // 通話リクエストを受け取った後、自身のカメラを起動することができます。
    ...
    public void onUserVideoAvailable(final String userId, boolean isVideoAvailable) {
        if (isVideoAvailable) {
            //相手方のビデオ画面を取得済みであることが表示され、相手方のビデオ影像が表示されます。
...
sCall.login(sdkappid, "aaa", usersig, new ActionCallback() {
    public void onSuccess() {
        //4. インスタンスコード：コンポーネントのログイン成功後にCCDカメラを起動し、ユーザー“aaa”を呼び出します。
        TXCloudVideoView localView = new TXCloudVideoView(mContext);
        mParentView.add(localView);
        sCall.openCamera(true, localView);
...
1. 発信者：多人数のビデオ通話では、`TRTCCalling` の `groupCall()` 関数をコールし 、ユーザーリスト（userIdList）、通話タイプ（type）、 IM グループ ID（groupId）を渡す必要があります。なお userIdList は入力必須パラメータ、通話タイプは`TYPE_VIDEO_CALL`に渡す入力必須パラメータで、groupId はオプションパラメータです。
2. 受信者：`onInvited()`という名称の イベント通知を介して呼び出しリクエストを受け取ることができます。
