...
|:-------:|:--------|
| AudioCallViewController.swift | Audio call homepage where calls can be answered or rejected. |
| AudioSelectContactViewController.swift | UI for selecting contacts. |
| AudioCallMainViewController.swift | UI for displaying historical contacts, where you can only initiate calls currently. |
| AudioCallViewController+Data.swift | User image rendering and layout logic during call. |

<span id="model"> </span>
## Implementing Custom UI
The source code folder `TRTCAudioCallDemo` contains two subfolders: `ui` and `model`. The `model` folder contains the implemented reusable open-source component `TRTCAudioCall`. You can find the API functions provided by this component in the `ITRTCAudioCallInterface.swift` file.
...
Apply for the mic permission by adding `Privacy - Microphone Usage Description` in the `info.plist` file.
...
1. The caller can call the `call()` method in `TRTCAudioCall` to initiate an audio call request.
2. The callee can call the `accept()` method to answer the call or call the `reject()` method to reject the call after receiving the `onInvited()` event.
3. The caller receives the `onUserEnter()` callback, which indicates that the callee has joined the call.
...
// `TRTCAudioCall.shared.accept`/`TRTCAudioCall.shared.reject` can be called to answer/reject the call
...
1. Caller: to initiate a group audio call, the caller needs to call the `groupCall()` function in `TRTCAudioCall` and pass in the required user list (`userIdList`) and optional group ID (`groupId`).
...
| setDelegate | Sets the `TRTCAudioCallDelegate` callback, through which users can get status notifications |
