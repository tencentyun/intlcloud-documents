...
The source code folder `trtcaudiocalldemo` contains two subfolders: `ui` and `model`. The `model` folder contains the implemented reusable open-source component `TRTCAudioCall`. You can find the API functions provided by this component in the `ITRTCAudioCall.java` file.
...
1. Caller: the caller can call the `call()` method in `ITRTCAudioCall` and specify the callee's `userid` to initiate an audio call request.
2. Callee: if already logged in, the callee will receive the `onInvited()` event notification. If you want the callee to receive call requests even when logged out, please see [Offline Answering](#model.offline).
3. Callee: to answer the call, the callee can call the `accept()` function. The callee can also call `reject()` to reject the call.
3. After the audio/video channel between the caller and callee is established, they will both receive the `onUserEnter()` event notification, which indicates that they have joined the call.
...
2. Callee: a callee can receive the request through the `onInvited()` event notification.
