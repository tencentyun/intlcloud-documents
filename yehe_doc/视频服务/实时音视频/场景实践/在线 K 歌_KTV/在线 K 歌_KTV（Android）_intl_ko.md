...
빠른 KTV 액세스 기능이 필요한 경우, Tencent Cloud에서 제공하는 App를 기반으로 설정을 적절히 수정하거나 TUIKaraoke 컴포넌트를 사용해 사용자 정의 UI 인터페이스를 구현할 수 있습니다.
...
2. 애플리케이션 명칭(예: `TestKaraoke`)을 입력한 후 [생성]을 클릭합니다.
...
[](id:ui.step2)
### 2단계: App 소스 코드 다운로드
[TUIKaraoke](https://github.com/tencentyun/TUIKaraoke/tree/main/Android/Source/src/main/java/com/tencent/liteav/tuikaraoke)를 클릭하여 이동한 후, 소스 코드를 Clone 또는 다운로드 합니다.
...
[](id:ui.step4)
### 4단계: App 실행
Android Studio(3.5 이후 버전)를 사용하여 소스 코드 프로젝트인 `TUIKaraoke`을 열고 [실행]을 클릭하면 즉시 해당 App이 디버깅됩니다.
...
|-------|--------|
| base | UI로 사용하는 기본 유형. |
| room | 메인 방 페이지, 방 주인 및 청취자를 위한 2개의 인터페이스 포함. |
| widget | 범용 컨트롤러. |
...
>
>![](https://main.qcloudimg.com/raw/206ba3492f4a2f18f36b1d5cba4e5558.jpg)

[](id:model)

## 사용자 정의 UI 인터페이스 구현

[소스 코드](https://github.com/tencentyun/TUIKaraoke/tree/main/Android/Source/src/main/java/com/tencent/liteav/tuikaraoke)의 Source 폴더에는 ui 폴더와 model 폴더가 포함되어 있으며 model 폴더에는 재사용할 수 있는 오픈 소스 컴포넌트 TRTCKaraokeRoom이 포함되어 있습니다. `TRTCKaraokeRoom.java` 파일에서 해당 컴포넌트가 제공하는 인터페이스 함수를 확인할 수 있으며 해당 인터페이스로 사용자 정의 UI 인터페이스를 구현할 수 있습니다.
...
[](id:model.step2)
### 2단계: 권한 및 난독화 규칙 설정
AndroidManifest.xml에서 App 권한을 설정합니다. SDK에는 다음 권한(6.0 이상 Android 시스템의 경우 스토리지 읽기 동적 권한 신청 필요)이 필요합니다.
<dx-codeblock>
::: java java
...
[](id:model.step4)
### 4단계: 컴포넌트 생성 및 로그인
1. `sharedInstance` 인터페이스를 호출하여 TRTCKaraoke 컴포넌트의 인스턴스 객체를 생성합니다.
...
3. 방 주인이 방 생성 후 `enterSeat`를 호출하여 자리에 입장합니다.
...
>!KTV 방 리스트에 포괄적인 정보가 충분히 포함되어 있다면 `getRoomInfoList` 호출 관련 단계는 건너뛸 수 있습니다.
4. 청취자가 하나의 KTV 방을 선택하고 `enterRoom`을 호출하여 방 번호를 전송하면 즉시 해당 방에 입장할 수 있습니다.
...
mTRTCKaraokeRoom.getRoomInfoList(roomList, new TRTCKaraokeRoomCallback.RoomInfoCallback() {
    ...
    public void onCallback(int code, String msg, List<TRTCKaraokeRoomDef.RoomInfo> list) {
        if (code == 0) {
            // 이 때 사용자의 UI 방 리스트 새로고침 가능
        }
    }
});

// 4. KTV 선택 후 roomId를 전송하여 방 입장
