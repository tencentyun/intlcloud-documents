TRTCChorusRoom은 Tencent Real-Time Communication(TRTC) 및 Instant Messaging(IM)을 기반으로 하며, 다음 기능을 지원합니다.
...
TRTCChorusRoom은 오픈 소스 Class로, Tencent Cloud의 두 가지 클로즈드 소스 SDK에 종속됩니다. 자세한 구현 방법은 [Chorus(iOS)](https://intl.cloud.tencent.com/document/product/647/42688)를 참고하십시오.
...
[TRTCChorus](https://intl.cloud.tencent.com/document/product/647/41940) 싱글톤 객체 가져오기.

```ObjectiveC
/**
* TRTCChorus 싱글톤 객체 가져오기
*
* - returns: TRTCChorus 인스턴스 
* - note: {@link TRTCChorus#destroySharedInstance()}를 호출하여 싱글톤 객체를 폐기할 수 있습니다.
*/
...
[TRTCChorus](https://intl.cloud.tencent.com/document/product/647/41940) 싱글톤 객체 폐기.

>?인스턴스 폐기 후에는 외부에 캐시된 TRTCChorus 인스턴스를 다시 사용할 수 없으며, 다시 [sharedInstance](#sharedInstance)를 호출해 새로운 인스턴스를 획득해야 합니다.

```ObjectiveC
/**
* TRTCChorus 싱글톤 객체 폐기
*
* - note: 인스턴스 폐기 후에는 외부에 캐시된 TRTCChorus 인스턴스를 다시 사용할 수 없으며, 다시 {@link TRTCChorus#sharedInstance()}를 호출해 새로운 인스턴스를 획득해야 합니다.
*/
...
[TRTCChorus](https://intl.cloud.tencent.com/document/product/647/41940) 이벤트 콜백, TRTCChorusDelegate에서 [TRTCChorus](https://intl.cloud.tencent.com/document/product/647/41940)의 각종 상태 알림을 확인할 수 있습니다.

```ObjectiveC
/**
* 모듈 콜백 인터페이스 설정
* 
* TRTCChorusDelegate를 통해 TRTCChorus의 다양한 상태 알림을 받아볼 수 있습니다.
*
* - parameter delegate 콜백 인터페이스
* - note: TRTCChorus의 이벤트 콜백은 기본적으로 Main Queue에서 사용자에게 콜백합니다. 이벤트 콜백이 있는 큐를 지정해야 할 경우 {@link TRTCChorus#setDelegateQueue(queue)}를 이용할 수 있습니다.
*/
- (void)setDelegate: (id<TRTCChorusDelegate>)delegate NS_SWIFT_NAME(setDelegate(delegate:));
```

>?setDelegate는 TRTCChorus의 콜백 프록시입니다.   
...
* 이벤트 콜백이 있는 큐 설정
*
* - parameter queue 큐입니다. 사용자가 지정한 queue에 TRTCChorus의 다양한 상태 공지 콜백을 배포합니다.
*/
...
| 매개변수  | 유형     | 의미         |
| ----- | ---------------- | ----------- |
| queue | dispatch_queue_t | 사용자가 지정한 스레드 큐에 TRTCChorus의 다양한 상태 공지를 배포합니다. |
...
| 매개변수     | 유형        | 의미          |
| --------- | ------------------- | ----------- |
| roomId    | int   | 방 아이디. 사용자가 할당하고 통합 관리합니다. 여러 개의 roomID를 1개의 음성 채팅방 리스트로 통합할 수 있습니다. Tencent Cloud는 현재 음성 채팅방 리스트 관리 서비스를 제공하지 않으므로 직접 관리하시기 바랍니다. |
...
1. 청취자가 귀하의 서버에서 최신 Chorus 방 리스트를 획득하며, 여기에는 여러 음성 채팅방의 roomId 및 방 정보가 포함될 수 있습니다.
...
| 매개변수    | 유형   | 의미      |
| -------- | --------- | -------------------- |
| musicId  | NSString   | 합창곡 musicID.  |
| startDelay  | NSInteger   | 합창곡 재생 지연 시간(초).  |
