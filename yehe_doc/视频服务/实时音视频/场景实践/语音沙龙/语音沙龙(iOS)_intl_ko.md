...
3. 방 주인은 사용자가 입장할 때 `onAnchorEnterSeat` 이벤트 공지를 수신하며, 이때 자동으로 마이크 수집이 활성화됩니다.
...
2. 청취자가 서비스 백그라운드에서 최신 음성 살롱 방 리스트를 가져옵니다.
...
1. `pickSeat`로 청취자 userId를 전송하면 특정 사용자의 마이크를 켤 수 있으며, 방 안에 있는 모든 사용자가 `onAnchorEnterSeat` 이벤트 공지를 수신합니다.
...
// 3. 청취자가 호스트가 되어, 마이크 위치를 공지하면 청취자가 실제로 마이크 켜기에 성공했는지 판단
func onAnchorEnterSeat(user: ChatSalonUserInfo) {
    // 마이크 켜기 이벤트 처리
}
:::
...
// 3. 청취자가 호스트가 되어 마이크 위치를 공지하면, 본인 여부 및 해당하는 프로세스 진행 여부 판단
func onAnchorEnterSeat(user: ChatSalonUserInfo) {
    // 마이크 켜기 이벤트 처리
}
:::
...
4. 방 주인이 `onInviteeAccepted` 이벤트 공지를 수신하고 `pickSeat`를 호출해 청취자의 마이크를 켭니다.
