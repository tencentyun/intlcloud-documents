...
1. TRTCChatSalon의 `sharedInstance` 클래스 메소드를 호출하여 TRTCChatSalon 인스턴스 객체를 생성합니다.
2. `setDelegate` 메소드를 호출해 모듈의 이벤트 콜백 공지를 등록합니다.
3. `login` 메소드를 호출해 모듈에 로그인하고, 다음 표를 참조하여 관련 매개변수를 입력합니다.
...
// 6. 방 입장 완료 후 onEnterRoomSeatListNotify 이벤트 공지 수신
...
// 3. 호스트가 마이크 위치를 통지하면, 여기에서 시청자가 실제로 마이크 켜기에 성공했는지 여부 판단
func onAnchorEnterSeat(user: ChatSalonUserInfo) {
    // 마이크 켜기 이벤트 처리
}
```
...
// 3. 호스트가 마이크 위치를 통지하면, 여기에서 본인 및 해당하는 프로세스 진행 여부 판단
func onAnchorEnterSeat(user: ChatSalonUserInfo) {
    // 마이크 켜기 이벤트 처리
}
```
...
- `sendRoomCustomMsg`를 통해 사용자 정의(신호) 메시지를 발송할 수 있으며, 해당 방에 있는 모든 호스트와 시청자가 `onRecvRoomCustomMsg` 콜백을 수신하게 됩니다.
  ...
  // eg: "CMD_DANMU": 댓글 자막 메시지 표시, "CMD_LIKE": 좋아요 메시지 표시
