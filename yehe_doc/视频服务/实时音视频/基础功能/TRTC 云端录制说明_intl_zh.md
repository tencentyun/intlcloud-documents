...
7.  主播2的一个音频M3U8索引文件；
8.  主播2的若干个音频TS切片文件；
<br>录制后台会把这些文件上传到您指定的云存储服务器，您的业务后台需要把这些录制文件拉取下来，并根据业务的需求对这些文件进行合并转码操作，我们会提供[音视频合并转码脚本]()（跳转到对应的位置）。
<br>
...
1.  混流后的的一个视频M3U8索引文件；
2.  混流后的若干个视频TS切片文件；
<br>录制后台会把这些文件上传到您指定的云存储服务器，您的业务后台需要把这些录制文件拉取下来，并根据业务的需求对这些文件进行合并转码操作，我们会提供[音视频合并转码脚本](TODO:跳转到转码脚本文档页面)（跳转到对应的位置）。
<br>
