TRTC Web SDK 屏幕分享支持度请查看 [浏览器支持情况](https://web.sdk.qcloud.com/trtc/webrtc/doc/en/tutorial-05-info-browser.html)。同时 SDK 提供 [TRTC.isScreenShareSupported](https://web.sdk.qcloud.com/trtc/webrtc/doc/en/TRTC.html#.isScreenShareSupported) 接口判断当前浏览器是否支持屏幕分享。
本文通过以下不同的场景介绍实现过程。

> !
> - Web 端暂不支持发布辅流，发布屏幕分享是通过发布主流实现的。远端屏幕分享流来自于 Web 用户时，[RemoteStream.getType()](https://web.sdk.qcloud.com/trtc/webrtc/doc/en/RemoteStream.html#getType) 返回值为 'main'，通常会通过 userId 来标识这是来自 Web 端屏幕分享流。
> - Native（iOS、Android、Mac、Windows 等）端支持发布辅流，并且发布屏幕分享是通过发布辅流实现的。远端屏幕分享流来自于 Native 用户时，[RemoteStream.getType()](https://web.sdk.qcloud.com/trtc/webrtc/doc/en/RemoteStream.html#getType) 返回值为 'auxiliary'。
...
## 屏幕分享参数配置
可设置的参数包括分辨率、帧率和码率，如果有需要可以通过 [setScreenProfile()](https://web.sdk.qcloud.com/trtc/webrtc/doc/en/LocalStream.html#setScreenProfile) 接口指定 profile，每个 profile 对应着一组分辨率、帧率和码率。SDK 默认使用 '1080p' 的配置。
...
>! 
>- 负责屏幕分享的 shareClient 需要关闭自动订阅，否则会出现重复订阅远端流的情况。请参见 [API说明](https://web.sdk.qcloud.com/trtc/webrtc/doc/en/TRTC.html#createClient)。
