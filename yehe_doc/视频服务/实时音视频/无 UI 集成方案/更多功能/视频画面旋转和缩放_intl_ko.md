...
- 가로 화면 모드는 어떻게 구현하나요? 예시: 그룹 멀티미디어 룸 App(Xiaoyu Yilian 등)은 일반적으로 가로 화면입니다.
...
<dx-alert infotype="explain">CSDN의 [iOS 가로 세로 화면 회전 및 기본 적응 방법](https://blog.csdn.net/DreamcoffeeZS/article/details/79037207)에서는 iOS 플랫폼의 화면 방향에 대한 개발 경험을 자세히 소개하고 있습니다.</dx-alert>
:::
::: Android 플랫폼
...
특히 [2단계](#step2)에서 TRTCVideoEncParam의 resMode 값은 다음과 같이 지정합니다.
...
| 인터페이스 함수 | 기능의 역할 |  비고 |
|---------|---------| ----- |
| setLocalViewRotation | 로컬 미리보기 화면의 시계 방향 회전 각도| 시계 방향으로 90도, 180도, 270도 3가지 방향의 회전 지원  |
| setLocalViewFillMode | 로컬 미리보기 화면의 채우기 모드 | 검은 테두리 자르기 또는 남기기|
...
해당 기능은 현재 0, 90, 180, 270도의 4가지 각도 회전을 지원합니다. 즉, 사용자의 휴대폰을 상하로 180도 뒤집어도 상대방이 보는 화면 방향은 그대로 유지됩니다. 또한 이러한 자체 적응은 인코더의 방향 조정을 기반으로 구현되므로 녹화된 영상, 그리고 H5에서 보이는 영상 화면 역시 기존 방향을 유지할 수 있습니다.
