...
>? 質問がある場合があるかもしれませんが、オーディオビデオストリームをリリースできるのはキャスターなら、すべてのユーザーにキャスターのロールで入室させることはできますか？答えは間違いなく「いいえ」です。その理由については、拡張機能ガイド[1つのルームに同時に存在するオーディオとビデオのチャネルはいくつですか？]をご参照ください。(#speed1)
