このドキュメントでは、主にTRTC Demo(Flutter)を素早く実行する方法をご紹介します。
...
- Flutter 2.0以降のバージョン。
- **Android端末向け開発：**
  - 以降Android Studio 3.5のバージョン。
  - 以降AppにはAndroid 4.1のバージョンのデバイスが必要です。
**iOS & macOS端末向け開発：**
  - Xcode 11.0以降のバージョン。
...
2. `TRTC-Simple-Demo/lib/debug/GenerateTestUserSig.dart`ファイルを検索して開きます。
...
2. Android Studio（3.5以降のバージョン）を使用して、ソースプロジェクトを開き、**実行**をクリックすれば完了です。
:::
::: iOS端末
1. iOSディレクトリ`podinstall`に移動します。
2. XCode（11.0以降のバージョン）を使用してソースディレクトリ配下の`/ios`プロジェクトを開き、Demoプロジェクトをコンパイルして実行します。
:::
::: Windows端末
...
### Android Manifest merge failedでコンパイルに失敗した場合はどうしますか。
