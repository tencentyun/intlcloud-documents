...
```
4. **SDK 설치 및 업데이트**
  - 단말 창에 다음 명령어를 입력하여 TRTC SDK를 실행합니다:
```
pod install
```
...
### 방법2: 수동 통합
1. [TRTC-SDK ](https://github.com/LiteAVSDK/TRTC_Mac/tree/main/SDK)의 Mac 버전을 다운로드합니다.
...
5. 다운로드한 SDK Framework와 필수 종속성을 'TXFFmpeg.xcframework', 'TXSoundTouch.xcframework', 'libc++.tbd', 'Accelerate.framework', 'SystemConfiguration.framework', 'MetalKit.framework' 순서로 추가합니다.  
추가하면 다음 이미지와 같아집니다.
...
### Objective-C 또는 Swift 인터페이스를 통해 TRTC SDK 참조
