...
[](id:step4)
### 4단계: 실행 가능한 프로그램 패키징
**빌드 툴**: 패키징 툴 `electron-builder`를 사용하는 것이 좋습니다. 다음 명령어를 실행하여 `electron-builder`를 설치할 수 있습니다.
```bash
$ npm install electron-builder@latest --save-dev
```
TRTC Electron SDK(즉, `trtc_electron_sdk.node`)를 올바르게 패키징하려면 다음 명령도 실행하여 `native-ext-loader`를 설치해야 합니다.
```bash
...
```
  ...
  - 로컬 개발 빌드의 경우 `native-ext-loader`가 `./node_modules/trtc-electron-sdk/build/Release`에 TRTC SDK를 로딩합니다. 자세한 내용은 [simple demo 설정](https://github.com/LiteAVSDK/TRTC_Electron/blob/main/TRTCSimpleDemo/vue.config.js)을 참고하십시오.

`package.json` 스크립트 구축 중, `--target_platform` 매개변수를 추가한 후 다음 단계를 진행합니다.

[](id:step6)
### 6단계: package.json 설정 수정
...
```
<table>
<thead><tr><th>매개변수</th><th>설명</th></tr>
</thead>
<tr>
<td>main</td>
...
[](id:q1)
### 방화벽에 어떤 제한이 있나요?
SDK는 UDP 프로토콜을 통해 멀티미디어를 전송하므로, UDP를 차단하는 기업용 네트워크에서는 사용할 수 없습니다. 이와 유사한 문제가 발생하는 경우 [방화벽 제한 처리](https://intl.cloud.tencent.com/document/product/647/35164)를 참고하십시오.
