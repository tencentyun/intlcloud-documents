...
- 長期割引：お客様のTRTCの月間消費額が3000米ドルを超える場合は、[リンク](https://intl.cloud.tencent.com/contact-us)にお問い合わせいただき、契約を締結することで長期割引が適用されます。
...
- サーバーAPI：より高度なニーズがある場合は、[サーバーAPI](https://cloud.tencent.com/document/product/647/49981)を通じて詳細な従量課金データを取得することもできます。問い合わせる時間が1日以下の場合は、5分単位のデータを返します。問い合わせ時間が1日より大きい場合は、1日ごとに集計したデータを返します。秒単位でカウントされます。
