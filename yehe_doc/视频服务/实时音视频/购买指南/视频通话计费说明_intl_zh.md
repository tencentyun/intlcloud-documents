...
[](id:Billing_items)
## 用量统计方式
实时音视频 TRTC 按 [房间](https://intl.cloud.tencent.com/document/product/647/37714) 内所有用户产生的 [视频时长](#v_duration) 和 [语音时长](#s_duration) 来统计视频通话服务的用量。
...
[](id:s_duration)
### 语音时长
视频通话场景下的语音时长 = 用户在 TRTC 房间内的总停留时长 - 接收视频画面时的停留时长。
...
- A 产生的用量及费用：
  ...
  - A 接收 B 的费用 = 语音时长单价 × 语音时长 = 0.99美元/千分钟 × (30分钟 / 1000）= 0.0297 美元
  - A 接收 C 的费用 = 超清视频时长单价 × 超清视频时长 = 14.99美元/千分钟 × (30分钟 / 1000) = 0.4497 美元
...
则该 TRTC 房间产生的**总费用**为 `用户 A 产生的费用 + 用户 B 产生的费用 + 用户 C 产生的费用 = 1.0782美元`。

[](id:estimate)
## 用量与费用预估
您可以使用 [TRTC 价格计算器](https://buy.intl.cloud.tencent.com/pricing/trtc) 预估自己业务将产生的视频通话用量和费用。
