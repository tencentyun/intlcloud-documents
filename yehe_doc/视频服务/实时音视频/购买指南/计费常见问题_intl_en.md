...
- Query via server API: You can also use a [server API](https://cloud.tencent.com/document/product/647/49981) to query your billable durations. If the time range specified for query is 1 day or shorter, the statistics returned are on a 5-minute basis. If it is longer than 1 day, the statistics returned are on a daily basis. The statistics are accurate to the second.
