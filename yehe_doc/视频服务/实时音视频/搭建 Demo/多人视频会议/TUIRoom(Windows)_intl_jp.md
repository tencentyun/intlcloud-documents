...
TUIRoom はオープンソースのClassであり、Tencent Cloudの2つのクローズドソースのSDKに依存しています。具体的な実装プロセスについては、 [多人数オーディオビデオルーム(Windows and macOS)](https://intl.cloud.tencent.com/document/product/647/44071)をご参照ください。
...
| [OnOrderedToExitSpeechkState](#onorderedtoexitspeechkstate)  | メンバーが発言の停止をリクエストされる場合のコールバック。         |
| [OnCallingRollStarted](#oncallingrollstarted)                | キャスター点呼を開始し、メンバーが受信する場合のコールバック。   |
...
[TUIRoomCore](https://cloud.tencent.com/document/product/647/63494) シングルトンオブジェクトを取得します。
```C++
...
[TUIRoomCore](https://cloud.tencent.com/document/product/647/63494) イベントコールバック。TUIRoomCoreCallbackを介して[TUIRoomCore](https://cloud.tencent.com/document/product/647/63494)の各種ステータス通知を取得できます。
...
| user_id | string | 現在のユーザーID。文字列タイプでは、英語のアルファベット（a-zとA-Z）、数字（0-9）、ハイフン（-）とアンダーライン（\_）のみ使用できます。 |
| user_sig | string | Tencent Cloudによって設計されたセキュリティ保護署名。取得方法については、 [UserSigの計算方法](https://intl.cloud.tencent.com/document/product/647/35166)をご参照ください。 |
...
美顔管理では、次の機能を使用できます。
- 「美顔スタイル」をスムースまたはナチュラルに設定します。「スムース」では、より強力な美肌補正効果が得られます。
