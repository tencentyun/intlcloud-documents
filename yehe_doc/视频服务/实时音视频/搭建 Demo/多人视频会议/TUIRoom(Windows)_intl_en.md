...
`TUIRoom` is an open-source class depending on two closed-source Tencent Cloud SDKs. For the specific implementation process, please see [Group Audio/Video Room (Windows)](https://intl.cloud.tencent.com/document/product/647/44071).
...
| [OnOrderedToExitSpeechkState](#onorderedtoexitspeechkstate)  | A member was asked to stop speaking.         |
...
| user_id | string | ID of the current user, which is a string that can contain only letters (a-z and A-Z), digits (0-9), hyphens (-), and underscores (\_) |
