## 효과
App을 [다운로드](https://intl.cloud.tencent.com/document/product/647/35076) 및 설치하여 그룹 멀티미디어 룸의 채팅방, 화면 공유, 뷰티 필터, 저지연 비디오 등 TRTC 기능 효과를 경험해 볼 수 있습니다.
...
3. RoomApp 소스 코드 프로젝트를 열고 프로그램을 컴파일 및 실행합니다.

[](id:step1_4)
### 4단계: App 소스 코드 수정
1. Module 계층은 TRTC 및 IM을 캡슐화하고 기본 논리 제어를 구현하며 UI 계층 호출을 용이하게 하는 기능적 인터페이스를 제공합니다.
2. App 모듈은 인터페이스의 기능적 구현을 ​​포함하는 UI 모듈로, 필요에 따라 2차 개발을 수행하거나 전체 UI를 교체할 수 있습니다.

## 애플리케이션 체험
>! 애플리케이션 체험 시 최소 2대의 디바이스가 필요합니다.
### 로그인 인터페이스
다음과 같이 방 번호와 사용자 이름을 입력하고 로그인합니다. 사용자 이름은 유일해야 하며 다른 사용자 이름과 중복되어서는 안 됩니다.
...
### 하단 툴바
마이크/카메라 셀프 제어, 화면 공유 열기, 참석자 목록, 채팅방 등의 기능이 있습니다.
...
### 채팅방
그룹 참석자는 채팅방에서 그룹 채팅할 수 있으며 호스트는 모든 참석자을 음소거/음소거 해제할 수 있습니다.
![](https://qcloudimg.tencent-cloud.cn/raw/9d927c98a595667d9f3f16d597abc77e.png)

### 방 퇴장
호스트의 방 퇴장에는 다음 두 옵션이 있습니다.
- 방 삭제. 모든 사람이 방에서 퇴장합니다.
- 혼자 방에서 퇴장하고, 호스트 권한을 다른 사용자에게 양도합니다.
...
VS에서 헤더 파일 디렉터리, 정적 라이브러리 디렉터리 및 종속 정적 라이브러리 파일을 설정합니다.
...
[](id:step2_6)
### 6단계: 방 삭제
1. 호스트가 DestoryRoom 인터페이스를 호출하여 방, IM 그룹 채팅을 삭제하고 TRTC 방에서 퇴장합니다.
2. 참석자는 그룹 삭제 및 TRTC 방 퇴장을 알리는 OnExitRoom 콜백 메시지를 받게 됩니다.

[](id:step2_7)
### 7단계: 방 양도
1. 호스트는 방 퇴장을 위해 TransferRoomMaster 인터페이스를 호출하여 다른 사람에게 방을 양도합니다.
2. 참석자는 방 호스트 변경을 알리는 OnRoomMasterChanged 콜백 메시지를 받게 됩니다.
3. 변경된 호스트는 호스트 권한을 획득하고 그룹 참석자을 제어할 수 있습니다.
...
3. 다른 참석자는 참석자 입장을 알리는 OnRemoteUserEnter 콜백을 수신합니다.
...
[](id:step2_10)
### 10단계: 화면 공유
1. StartScreenCapture 인터페이스를 호출하여 화면을 공유합니다.
2. 다른 참석자는 참석자가 화면을 공유하고 있음을 알리는 OnRemoteUserScreenVideoAvailable 콜백을 수신합니다.

>? 화면 공유 모듈은 창 선택 로직을 수행해야 합니다. App의 'ScreenShareWindow.h' 구현을 참고하십시오.

[](id:step2_11)
### 11단계: 텍스트 채팅 및 음소거 메시지 구현
