...
- Part 1. 서비스 활성화 및 Tencent Cloud에서 제공하는 Demo 실행 방법 소개.
- Part 2. TRTCCalling 컴포넌트를 사용한 영상 통화 기능의 빠른 구축 방법 소개.

## 환경 요건
최신 버전의 Chrome 브라우저를 사용하십시오. 현재 데스크톱 Chrome 브라우저가 TRTC Web SDK를 비교적 완벽하게 지원하므로, Chrome 브라우저를 사용한 체험을 권장합니다.

TRTCCalling은 다음 포트에 종속되어 데이터를 전송합니다. 해당 포트를 방화벽 얼로우리스트에 추가하고 설정을 완료하십시오. [공식 홈페이지 Demo](https://web.sdk.qcloud.com/component/trtccalling/demo/web/latest/index.html)에서 설정이 적용되었는지 확인할 수 있습니다.
  - TCP 포트: 8687
...
[](id:step2)
### 2단계: Demo 다운로드
1. 실제 비즈니스 니즈에 따라 Demo 소스 코드를 다운로드하거나 [TUICalling](https://github.com/tencentyun/TUICalling)에서 Demo 소스 코드를 다운로드합니다.
...
2. `Web/public/debug/GenerateTestUserSig.js` 파일을 찾아 엽니다.
...
### 3단계: 로그인 완료
login 함수를 호출하여 로그인 작업을 완료합니다. 매개변수에서 userID는 사용자 이름이고, userSig는 사용자 서명입니다. userSig의 계산 방식은 [userSig 계산 및 사용 방법](https://intl.cloud.tencent.com/document/product/647/35166)을 참고하십시오.
