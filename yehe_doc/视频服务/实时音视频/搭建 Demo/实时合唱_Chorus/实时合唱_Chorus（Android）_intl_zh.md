## 效果展示
您可以 [下载](https://intl.cloud.tencent.com/document/product/647/35076) 安装我们的 App 体验 Chorus 的能力，包括低延时K歌、麦位管理、收发礼物、文字聊天等 TRTC 在 Chorus 场景下的相关能力。
<table>
     <tr>
         <th>房主麦位操作</th>  
         <th>听众麦位操作</th>  
...
[](id:ui.step4)
### 步骤4：运行 App
使用 Android Studio（3.5以上的版本）打开源码工程`TUIChorus`，单击 **运行** 即可开始调试本 App。
...
[源码](https://github.com/tencentyun/TUIChorus/tree/main/Android/Source/src/main/java/com/tencent/liteav/trtcChorus) 中的 Source 文件夹包含两个子文件夹 ui 和 model，model 文件夹中包含可重用的开源组件 TRTCChorusRoom，您可以在`TRTCChorusRoom.java`文件中看到该组件提供的接口函数，并使用对应接口实现自定义 UI 界面。
<img src="https://main.qcloudimg.com/raw/7613bd7ec5b4e665f32ee5df69e5de85.png">

[](id:model.step1)
### 步骤1：集成 SDK
Chorus 组件 TRTCChorusRoom 依赖 TRTC SDK 和 IM SDK，您可以按照如下步骤将两个 SDK 集成到项目中。
...
[](id:model.step4)
### 步骤4：创建并登录组件
1. 调用 `sharedInstance` 接口可以创建一个 TRTCChorus 组件的实例对象。
...
<tr>
<td>userSig</td>
<td>腾讯云设计的一种安全保护签名，获取方式请参考 <a href="https://intl.cloud.tencent.com/document/product/647/35166">如何计算及使用 UserSig</a>。</td>
</tr>
<tr>
<td>callback</td>
...
2. 房主调用 `createRoom` 创建新的 Chorus 房间，此时传入房间 ID、上麦是否需要房主确认、麦位数等房间属性信息。
