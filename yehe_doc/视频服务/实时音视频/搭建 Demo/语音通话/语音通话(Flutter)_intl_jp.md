...
[model](https://github.com/tencentyun/TRTCFlutterScenesDemo/tree/main/lib/TRTCCallingDemo/model)ディレクトリ内の全ファイルをプロジェクトにコピーします。
```
...
1. 発信者：`TRTCCalling`の`call()`メソッドを呼び出して通話リクエストを開始し、ユーザーID（userid）と通話タイプ（type）を渡して、通話タイプパラメータを`TRTCCalling.typeAudioCall`に渡します。
