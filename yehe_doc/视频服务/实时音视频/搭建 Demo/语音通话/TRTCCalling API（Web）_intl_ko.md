...
| offlinePushInfo | Object | 사용자 정의 오프라인 메시지 푸시(옵션). **v1.0.0 및 이후 버전만 해당** |

offlinePushInfo 매개변수 (v1.0.0 및 이후 버전만 해당)

| 매개변수  | 유형   | 의미    |
| -------------------- | ------ | ------------------------- |
| title                | String | 오프라인 푸시 제목(옵션)                                   |
| description          | String | 오프라인 푸시 내용(옵션)     |
| androidOPPOChannelID | String | OPPO 휴대폰 시스템 8.0 및 이후 버전 오프라인 푸시의 채널 ID 설정(옵션) |
...
<dx-codeblock>
::: javascript javascript
trtcCalling.switchDevice({deviceType: 'audio', deviceId: deviceId}) // 디바이스 전환
:::
</dx-codeblock>
