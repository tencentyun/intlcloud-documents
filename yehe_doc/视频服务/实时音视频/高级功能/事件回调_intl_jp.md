...
<span id="deploy"></span>
## 構成情報
TRTCコンソールは、セルフサービスのコールバック構成情報をサポートしており、構成が完了した後、イベントコールバック通知を受信できます。


>!事前に次の情報を準備する必要があります。
>- **必須項目**：コールバック通知を受信するためのHTTP/HTTPSサーバーアドレス。
>- **オプション項目**：署名を計算するための暗号鍵で、大文字・小文字と数字で構成される最大32文字の鍵をカスタマイズできます。
...
| Role    | Number | [ロールタイプ](#role_type)（option：入退室時に付与）  |
| Reason  | Number | [具体的な原因](#reason) （option：入退室時に付与） |
...
|退室 | <li/>1：正常な退室<li/>2：タイムアウトによる退室<li/>3：ユーザーが別の端末に参加し、ローカルで退出<li/>4：ルームのユーザーが削除される<li/>5：マイク接続のキャンセル・退室<li/>6：強制終了|
