...
>- **オプション項目**：署名を計算するためのkeyで、大文字と小文字のアルファベットと数字で構成される最大32文字のkeyをカスタマイズできます。

## タイムアウト・リトライ
イベントコールバックサーバーがメッセージ通知を送信してから5秒以内にお客様のサーバーから応答を受信しない場合、通知は失敗とみなされます。最初の通知が失敗すると、直ちにリトライが行われます。その後の失敗は、メッセージがリトライせずに1分以上続くまで、** 10秒**の間隔でリトライを繰り返します。
...
>! 退室すると104イベントだけがコールバックされ、202イベントと204イベントがコールバックされません。104イベントは202イベントと204イベントを含むことに相当します。手動でビデオ/オーディオをオフにしないと、202イベント/204イベントをコールバックしません。
...
|退室 | <li/>1：正常な退室<li/>2：タイムアウト退出<li/>3：ルームユーザーの除去<li/>4：マイク接続取消後の退室<li/>5：強制終了<br>**注意：Androidシステムがプロセスの強制終了をキャプチャできず、バックグラウンドがタイムアウトするまで待って退出します。この場合、コールバックreason は2とします**|
