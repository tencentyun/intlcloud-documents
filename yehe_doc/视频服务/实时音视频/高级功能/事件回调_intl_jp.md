...
### 署名計算
署名はHMAC SHA256暗号化アルゴリズムによって計算されます。イベントコールバック受信サーバーがコールバックメッセージを受信した後、同様の方法で署名を計算します。同じとは、Tencent CloudのTRTCのイベントコールバックであり、偽造されていないことを意味します。 コールバックイベントの署名計算コードの原理と例については、[Signature Method v3](https://intl.cloud.tencent.com/document/product/647/34264)をご参照ください。署名の計算は次のとおりです。
```
