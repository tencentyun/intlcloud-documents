...
本功能目前免费开放内测中（后续若收费会提前通知），但接入的 RTMP 流会作为房间中的虚拟用户产生正常的通话费用，详情参见 [计费概述](https://intl.cloud.tencent.com/document/product/647/34610)。如果您有需要可以联系售前咨询：xunchenliu@tencent.com，提交申请加入白名单进行使用（需提供 SdkAppId）。

>! 不支持 RTMP 从 TRTC 拉流，如果需要旁路 CDN 直播观看，请参见 [实现 CDN 直播观看](https://intl.cloud.tencent.com/document/product/647/35242)。
...
```
	...
	- usersig 的生成规则，请参见 [UserSig 相关](https://intl.cloud.tencent.com/document/product/647/35166) （**请注意签名要在有效期内**）。
