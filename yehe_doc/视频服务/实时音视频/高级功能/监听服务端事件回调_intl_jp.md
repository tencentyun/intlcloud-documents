イベントコールバックサービスは、HTTP/HTTPSリクエストという形でサーバーへのTRTC業務でのイベントの通知をサポートします。イベントコールバックサービスは、ルームイベントグループ（Room Event）とメディアイベントグループ（Media Event）およびレコーディングイベントグループのいくつかのイベントを統合しています（クラウドレコーディング機能のコールバックイベントの説明については、[クラウドレコーディングと再生の実現](https://cloud.tencent.com/document/product/647/76497#.E5.9B.9E.E8.B0.83.E6.8E.A5.E5.8F.A3)をご参照ください）。お客様は、関連する設定情報をTencent Cloudに提供することで、このサービスをアクティブにすることができます。
...
<table id="header">
<thead><tr><th>フィールド名</th><th>値</th></tr></thead><tr>
...
<table id="body">
<thead><tr><th>フィールド名</th><th>タイプ</th><th>意味</th></tr>
</thead>
<tr>
...
>? レコーディングイベントグループに関する説明については、[クラウドレコーディングと再生の実現](https://cloud.tencent.com/document/product/647/76497#.E5.9B.9E.E8.B0.83.E6.8E.A5.E5.8F.A3)をご参照ください。
...
|退室 | <li/>1：正常な退室<li/>2：タイムアウトによる退室<li/>3：ルームのユーザーが削除される<li/>4：マイク接続のキャンセルによる退室<li/>5：強制終了<br>**注意：Androidシステムではプロセスの強制終了を捕捉できず、バックエンドのタイムアウトによる退室を待つしかありません。この場合のコールバックreasonは2となります**|
