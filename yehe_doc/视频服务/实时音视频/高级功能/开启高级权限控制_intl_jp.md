...
## 上級権限管理の起動
<span id="step1"></span>
### 手順1：TRTC コンソールで権限キーを起動します

1. Tencent CloudのTRTCコンソールで左側の[【アプリケーション管理】](https://console.cloud.tencent.com/trtc/app)をクリックします。
2. 右側のアプリケーションリストから高度な権限制御を有効にしたいアプリケーションをクリックし、【アプリケーション情報】ボタンをクリックします。
3. 「アプリケーション情報」ページの【権限キーをクリック】ボタンを開き、【確定】をクリックすれば、高度な権限制御を有効にすることができます。
...
try {
    ...
    params.put("privateMapKey", "xxxxx"); // 新たな privateMapKeyを入力します
