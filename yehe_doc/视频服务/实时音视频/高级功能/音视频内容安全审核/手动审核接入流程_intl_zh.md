...
<td>mix</td>
<td>否</td>
<td>取值时 true 为混流审核，false 为单流审核。不填默认是混流审核。<br>（混流审核下违规回调无法获取具体的违规用户，适用于房间维度的合规审核场景，单流审核可通过回调确定具体的违规 user_id。）</td>
</tr><tr>
<td>room_id_type</td>
<td>否</td>
...
3. 通过 [创建审核任务](https://intl.cloud.tencent.com/document/product/1139/46101) 接口，发起该房间的内容安全审核。
<table>
...
<td>否</td>
<td>String</td>
<td>公共参数，指定对应的海外地域，目前仅支持新加坡(ap-singapore)。</td>
</tr><tr>
<td>Tasks.N</td>
<td>是</td>
...
<td>否</td>
<td>String</td>
<td>可选参数，该字段表示回调签名的 key 信息，用于保证数据的安全性。 签名方法为在返回的 HTTP 头部添加 X-Signature 的字段，值为： seed + body 的 SHA256 编码和 Hex 字符串，在收到回调数据后，可以根据返回的body，用 <b>sha256(seed + body)</b>，计算出 <code>X-Signature</code> 进行验证。 具体使用实例请参见 <a href="https://intl.cloud.tencent.com/document/product/1139/46091">回调签名示例</a>。</td></tr>
<tr>
<td>CallbackUrl</td>
<td>否</td>
<td>String</td>
<td>可选参数，该字段表示接受审核信息回调的地址，格式为 URL 链接默认格式。配置成功后，审核过程中产生的违规音频片段将通过此接口发送。回调返回内容格式请参见 <a href="https://intl.cloud.tencent.com/document/product/1139/46101#.E7.A4.BA.E4.BE.8B2-.E5.9B.9E.E8.B0.83.E7.AD.BE.E5.90.8D.E7.A4.BA.E4.BE.8B">回调签名示例</a>。</td></tr>
</tbody></table>
...
以上示例为音频审核任务创建接口，如需发起视频审核任务，请调用 [视频审核任务](#appendix) 创建接口。
   ...
   - **CallbackUrl**
      该地址用于用户接口直播过程中和直播结束的审核结果，可以在控制台配置界面中设置，也可以通过接口传入。回调格式同 [查询任务详情接口](https://intl.cloud.tencent.com/document/product/1139/46100) 的输出参数。
   - **Tasks.N.Input.Url**
...
内容安全是以观众身份从 TRTC 拉取数据流进行审核的，TRTC 默认计费方式按订阅时长（拉流）计费，所以这里会产生费用，具体的收费请参见 [音视频计费时长说明](https://intl.cloud.tencent.com/document/product/647/42734)。
>!
...
### 5. 什么是 user_sig?
具体请参见 [user_sig 相关](https://intl.cloud.tencent.com/document/product/647/35166)。
...
>?更多问题，请参见 [TRTC 常见问题文档](https://intl.cloud.tencent.com/document/product/647/36057)。

[](id:appendix)

## 附录
- **音频审核服务**提供以下四个审核接口，通过单击链接查看详细接口文档：
     - [创建审核任务](https://intl.cloud.tencent.com/document/product/1139/46101 )
     - [结束审核](https://intl.cloud.tencent.com/document/product/1139/46097)
     - [查询审核任务列表](https://intl.cloud.tencent.com/document/product/1139/46096) 
     - [查询审核任务详情](https://intl.cloud.tencent.com/document/product/1139/46100)
- **视频审核服务**提供以下四个审核接口，通过单击链接查看详细接口文档：
     - [创建审核任务](https://intl.cloud.tencent.com/document/product/1140/46113)
     - [结束审核](https://intl.cloud.tencent.com/document/product/1140/46114)
     - [查询审核任务列表](https://intl.cloud.tencent.com/document/product/1140/46111)
     - [查询审核任务详情](https://intl.cloud.tencent.com/document/product/1140/46112)
