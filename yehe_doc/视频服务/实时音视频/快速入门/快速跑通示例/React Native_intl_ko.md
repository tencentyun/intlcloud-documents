...
<dx-alert infotype="notice">`android:hardwareAccelerated="false"`를 설정하지 마십시오. 하드웨어 가속을 비활성화한 후에는 상대방의 비디오 스트림을 렌더링할 수 없습니다.</dx-alert>
...
:::
::: iOS
1. Demo ios 디렉터리에서 `pod install`을 실행하여 종속 항목을 설치합니다.
2. Demo 디렉터리에서 Metros를 실행합니다.
```
npx react-native start
```
3. Demo 디렉터리에서 새 창을 열고 개발 디버깅을 실행(오류가 보고되면 xcode를 열어 컴파일 및 디버깅)합니다.
```
