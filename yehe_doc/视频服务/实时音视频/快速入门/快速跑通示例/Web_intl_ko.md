...
응용 시나리오가 지원되는 테이블에 없는 경우 [TRTC Web SDK 기능 테스트 페이지](https://web.sdk.qcloud.com/trtc/webrtc/demo/detect/index.html)에서 현재 환경이 WebView 환경 등 WebRTC 기능 지원 여부를 확인할 수 있습니다.
...
### URL 도메인 프로토콜 제한
브라우저 보안 정책의 제한으로 인해 WebRTC 기능을 사용하려면 페이지의 액세스 프로토콜에 대한 엄격한 요구 사항이 있습니다. 다음 테이블을 참고하여 애플리케이션을 개발 및 배포하십시오.
...
2. 다운로드한 소스 코드에서 `Web/base-js/js/debug/GenerateTestUserSig.js` 파일을 찾아 엽니다.
...
## 온라인 Demo 실행
온라인에서 Demo를 실행할 수 있는 방법도 제공합니다. `SDKAppID`와 `key`를 획득한 후, [빠른 demo 실행](https://web.sdk.qcloud.com/trtc/webrtc/demo/quick/index.html)을 클릭하여 열고, 페이지의 안내에 따라 실행합니다.
