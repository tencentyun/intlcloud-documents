...
UserSig란 Tencent Cloud가 설계한 일종의 보안 서명으로, 악성 해커가 귀하의 클라우드 서비스 사용권을 남용하는 것을 방지하기 위한 목적으로 설계되었습니다.
현재 Tencent Cloud의 TRTC, IM, MLVB 등의 서비스는 모두 해당 보안 메커니즘을 사용하고 있으며, 해당 서비스를 사용하려면 상응하는 SDK의 초기화 또는 로그인 함수에 SDKAppID, UserID, UserSig의 주요 정보를 제공해야 합니다.
이 중, SDKAppID는 귀하의 애플리케이션 식별에 사용되고, UserID는 귀사의 사용자 식별에 사용되며, UserSig는 이상의 두 정보를 기반으로 보안 서명을 계산합니다. UserSig는 **HMAC SHA256** 암호화 알고리즘으로 계산하여 산출되며, 해커가 UserSig를 위조하지 못하면 귀하의 클라우드 서비스 트래픽을 남용할 수 없습니다.
UserSig의 계산 원리는 다음 이미지와 같으며, 기본적으로 SDKAppID, UserID, ExpireTime 등 주요 정보에 대해 해시 암호화를 1회 진행하는 것입니다.
```Cpp
//UserSig 계산 공식, 이 중 secretkey는 usersig 계산용 암호화 키
...
- 통화 모드 시, 한 방에 최대 300명까지 동시 접속이 가능하며 최대 30명까지 동시에 카메라 또는 마이크를 활성화할 수 있습니다.
- 라이브 방송 모드 시, 한 방에 10만명 이상이 관중 자격으로 접속하여 시청할 수 있으며, 최대 30명까지 호스트 자격으로 카메라 또는 마이크를 활성화할 수 있습니다.

>? Web에서는 최대 20명이 동시에 카메라 또는 마이크를 활성화할 수 있도록 지원합니다.

<span id="que7"></span>
### TRTC는 라이브 방송 시나리오 애플리케이션을 어떻게 실행합니까?
TRTC는 온라인 라이브 방송 시나리오 전용으로 10만명 딜레이 시의 ILVB 솔루션을 출시하여, 호스트와 마이크가 연결된 호스트의 최저 딜레이 시간 200ms 이내, 일반 관중의 딜레이 시간 1s 이내로 보장합니다. 또한 약한 네트워크 보완 기술을 통해 복잡한 모바일 네트워크 환경에 대응합니다.
...
라이브 방송 시나리오(TRTCAppSceneLIVE와 TRTCAppSceneVoiceChatRoom)는 TRTCRoleAnchor(호스트)와 TRTCRoleAudience(관중)의 두 가지 역할을 지원합니다. 호스트 역할은 멀티미디어 데이터를 동시에 업스트림, 다운스트림할 수 있으며, 관중 역할은 다른 사람의 데이터를 다운스트림 재생만 할 수 있습니다. switchRole() 호출을 통해 역할을 교체할 수 있습니다.


<span id="que9"></span>
### TRTC 방에서 강퇴, 발언 금지, 음소거 기능을 지원합니까?  
지원합니다.
- 간단한 조작 명령인 경우 TRTC의 사용자 정의 명령 인터페이스 sendCustomCmdMsg를 사용해 개발자가 해당 제어 명령을 사용자 정의할 수 있으며, 제어 명령을 수신하는 통화 상대방이 해당 조작을 실행하면 됩니다. 예시: 강퇴는 하나의 강퇴 명령 정의로, 해당 명령을 수신한 사용자는 스스로 방을 퇴장하게 됩니다.
- 더 완벽한 조작 로직이 필요한 경우 개발자가 [인스턴트 메시징 IM](https://intl.cloud.tencent.com/document/product/1047)을 통해 관련 로직을 실현할 수 있도록 권장합니다. TRTC의 방과 IM 그룹을 매핑하고 IM 그룹에서 사용자 정의 정보를 수신/발신해 해당 조작을 실현하십시오.
...
지원합니다. 직접 통합 3자 데이터베이스를 지원하는 프로세스에 따라 SDK를 통합하면 됩니다. [Demo 실행(iOS&Mac)](https://intl.cloud.tencent.com/document/product/647/35086)을 참조하십시오.
...
- **RTRC**(키워드: 다중 사용자 인터랙션, UDP, 사유 프로토콜, 저 딜레이)
 TRTC(Real-Time Communication, RTC)의 주요 응용 시나리오는 멀티미디어 인터랙션과 저 딜레이 라이브 방송이며, UDP 기반의 사유 프로토콜을 사용하여 딜레이가 100ms까지 낮아집니다. 전형적인 시나리오로는 QQ 전화, Tencent 회의, Dabanke 등이 있습니다. Tencent Cloud TRTC는 전 플랫폼을 커버하며 클라우드 혼합 스트리밍 방식을 통해 릴레이 라이브 방송 화면을 CDN으로 전달합니다.
...
상응하는 RoomID, UserID를 통해 TRTC 콘솔의 [계기판 모니터링](https://console.cloud.tencent.com/trtc/monitor) 페이지에서 통화 품질을 확인할 수 있습니다.
...
- 해상도는 주로 부호율과 관련이 있습니다. SDK 부호율이 낮게 설정되어 있는지 확인하십시오. 해상도가 높은데 부호율이 낮은 경우 모자이크 현상이 쉽게 나타날 수 있습니다.
- TRTC는 클라우드 QOS 트래픽 제어 Policies를 통해 네트워크 상태에 따라 부호율과 해상도를 동적으로 조정합니다. 네트워크 상태가 좋지 않은 경우 부호율이 낮아져 해상도가 떨어질 수 있습니다.
- 방 진입 시 사용한 VideoCall 모드가 Live 모드인지 확인합니다. 통화 VideoCall 모드는 저 딜레이와 원활한 통화에 초점을 맞추므로, 네트워크가 약한 경우 화질을 포기하고 원활한 통화 상태를 확보합니다. 화질이 더 중요한 경우 Live 모드 사용을 권장합니다.
