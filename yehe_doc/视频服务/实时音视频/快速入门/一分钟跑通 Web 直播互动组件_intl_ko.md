본 문서에서는 Tencent Cloud의 Web 라이브 방송 인터랙션 구성 요소를 빠르게 실행하는 체험용 Demo를 소개합니다.
...
[Tencent Cloud 가입](https://intl.cloud.tencent.com/document/product/378/17985)에 계정이 있고 [실명 인증](https://intl.cloud.tencent.com/document/product/378/3629)이 완료되어야 합니다.
...
2. [애플리케이션 관리](https://console.cloud.tencent.com/trtc/app)에 들어가 [Create Application]을 클릭하고 애플리케이션 명칭(예: `testtrtc`)을 입력한 후 [Confirm]을 클릭합니다.
...
2. `TWebLive/dist/debug/GenerateTestUserSig.js` 문서를 열고 관련 매개변수를 설정합니다.
...
TWebLive는 카메라와 마이크를 사용하여 멀티미디어를 캡처합니다. 체험 과정에서 Chrome 브라우저에서 관련 안내를 수신할 수 있습니다. [허용]을 클릭하십시오.
...
|운영 체제 | 브라우저 유형 | 최저 버전 요구사항 | 수신(재생)| 발송(마이크 켜짐)|
|:-------:|:-------:|:-------:|:-------:|:-------:|
| Mac OS  | 데스크톱 Safari 브라우저 |  11+ | 지원 | 지원 | 
| Mac OS  | 데스크톱 Chrome 브라우저 |  47+ | 지원 | 지원 | 
| Windows  | 데스크톱 Chrome 브라우저 |  52+ | 지원 | 지원 | 
| Windows  | 데스크톱 QQ 브라우저 |  10.2 | 지원 | 지원 | 
| iOS | 모바일 Safari 브라우저 | 11.1.2 | 지원 | 지원 | 
| iOS | WeChat 내부 웹 페이지| 12.1.4 | 지원 | 미지원 | 
| Android | 모바일 QQ 브라우저| - | 미지원 | 미지원 | 
| Android | 모바일 UC 브라우저| - | 미지원 | 미지원 | 
| Android | WeChat 내부 웹 페이지| - | 미지원 | 미지원 | 
...
`"Join room failed result: 10006 error: service is suspended, if charge is overdue, renew it"` 오류가 발생하는 경우, TRTC 애플리케이션의 서버 상태가 사용 가능한 상태인지 확인하십시오.
