...
The `appScene` parameter specifies the application scenario of the SDK. In this document, `TRTCAppSceneLIVE` (online live streaming) is used as an example.
...
During the live streaming, the anchor may want to block local audio/video data for privacy reason. To this end, `muteLocalVideo` and `muteLocalAudio` can be called to block local video and audio capture, respectively.
...
`appScene` also needs to be specified as `TRTCAppSceneLIVE` (online live streaming), but the `role` field needs to be specified as `TRTCRoleAudience` (viewer).
