...
>! 注意すべき点としては、モバイル版のTRTC SDKは、デスクトップ版のように、「サブチャネルの共有」をサポートしていません。これは、iOSとAndroidシステムは、どちらもバックグラウンドで実行されているアプリに対し、カメラの使用権を制限しているからです。従って、サブチャネルの共有をサポートする意義はあまりありません。
...
2.お客様のプロジェクトにおいて、Broadcast Upload ExtensionのTargetを新規作成し、拡張コンポーネント用としてカスタマイズされた`TXLiteAVSDK_ReplayKitExt.framework`をSDK圧縮パッケージに統合します。
...
4.ポップアップされたフォームにDescriptionとIdentifierを記入します。そのうちIdentifierは、インターフェースにおいて対応するAppGroupパラメータに渡す必要があります。完了したら、【Continue】をクリックします。
...
2.ポップアップされたダイアログボックスに関連情報を記入し、**【Include UI Extension】にはチェック"を入れずに**【Finish】をクリックして作成を完了します。
...
4.新しく追加したTargetを選択し、【+ Capability】を順番にクリックして、【App Groups】をダブルクリックします。下図のとおりです。
 ...
 操作が完了すると、下図のようにファイルリストに`Target名.entitlements`という名前のファイルが生成されます。このファイルを選択し、+記号をクリックして上記の手順のApp Groupを記入すればOKです。
