...
위 정보에 따라, 공유 가능한 타깃을 나열하여 사용자가 선택할 수 있는 간단한 리스트 페이지를 구현할 수 있습니다(다음 이미지 참조).
...
## 화질 설정
`setSubStreamEncoderParam` 인터페이스에서 해상도, 비트 레이트, 프레임 레이트 등 공유 화면의 화질을 설정할 수 있습니다. 다음 값을 참조하십시오.
