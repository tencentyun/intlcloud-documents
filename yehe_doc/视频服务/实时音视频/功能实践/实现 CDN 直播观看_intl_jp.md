...
[マルチ画面合成](#mixCDN)を有効にすると、ミクスストリーミングをデコードしてエンコードする必要があるため、別途ミクスストリーミングトランスコード料金が発生します。ミクスストリーミングトランスコードは、解像度とトランスコード所要時間に応じて課金されます。キャスターが使用する解像度が高く、マイク接続時間が長いほど（通常、マイク接続シーンにのみミクスストリーミングトランスコードが必要）、料金が高くなります。詳細については、 [LVB > LVBトランスコード](https://intl.cloud.tencent.com/document/product/267/2818?lang=en&pg=#lvb-transcoding)をご参照ください。
