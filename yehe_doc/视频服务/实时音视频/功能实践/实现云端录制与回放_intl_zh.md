...
- **多路画面的混合**
  ...
  - [客户端 SDK API 混流方案](#recordSDKAPI)：可以直接在客户端发起混流，目前支持 iOS、Android、Windows、Mac 和 Electron 等平台，暂 Web 浏览器。
...
- **录制任务的开始**
由您的服务器调用 [StartMCUMixTranscode](https://intl.cloud.tencent.com/document/product/647/37761) ，并指定 `OutputParams.RecordId` 参数即可启动混流和录制。
```
...
>?关于云端混流的详细介绍，具体请参见 [云端混流转码](https://intl.cloud.tencent.com/document/product/647/34618#restapi)。
...
>! 建议使用 [专业版](https://intl.cloud.tencent.com/document/product/647/34615) TRTC SDK，专业版集合了 [超级播放器（Player+）](https://cloud.tencent.com/product/player)、[移动直播（MLVB）](https://intl.cloud.tencent.com/zh/product/mlvb) 等功能，由于底层模块的高度复用，集成专业版的体积增量要小于同时集成两个独立的 SDK，并且可以避免符号冲突（symbol duplicate）的困扰。
...
>!
>- 自2020年7月1日起首次在 TRTC 控制台创建应用的腾讯云账号，使用云端录制功能后产生的录制费用以 [云端录制计费说明](https://intl.cloud.tencent.com/document/product/647/38385) 为准。
