...
In the call modes, all users in a TRTC room are assigned to access servers and are in the role of an anchor. This means the users can speak to each other at any point in the call (up to 50 users can send upstream data at the same time). The modes are suitable for use cases such as online conferencing, but the number of users in each room is capped at 300.
...
When calling the [enterRoom()](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#enterRoom) API, you need to set a key parameter [`TRTCParams`](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCParams.html), which includes the following required fields:
...
[](id:step5)
### Step 5. Enables camera preview and mic capturing as an anchor
1.  Call [startLocalPreview()](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#startLocalPreview) as an anchor to enable local camera preview. The SDK will ask for camera access from the system.
2. Call `setLocalViewFillMode()` as an anchor to set the display mode of the local video image.
    -   `TRTCVideoFillMode.TRTCVideoFillMode_Fill` is the fill mode. The image may be scaled up or cropped, but there are no black bars.
    -   `TRTCVideoFillMode.TRTCVideoFillMode_Fit` is the fit mode. The image may be scaled down to fit the screen, but there may be black bars.
...
4.  Call [startLocalAudio()](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#startLocalAudio) as an anchor to enable the mic. The SDK will ask for mic access from the system.
...
1. Call [setBeautyStyle(style, beauty, white, ruddiness)](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#setBeautyStyle) as an anchor to enable beauty filters.
2. Parameter description:
    ...
    -   beauty: strength of the skin smoothing filter; value range: 0-9; 0 indicates that the filter is disabled. The larger the value, the higher the strength.
    -   white: strength of the skin brightening filter; value range: 0-9; 0 indicates that the filter is disabled. The larger the value, the higher the strength.
    -   ruddiness: strength of the rosy complexion filter; value range: 0-9; 0 indicates that the filter is disabled. The larger the value, the higher the strength. This parameter is not enabled for Windows for the time being.
...
3. After an anchor creates a room, the SDK will start encoding and uploading the video and audio data of the anchor and returns the [onEnterRoom(result)](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCallback.html#event:onEnterRoom) callback. If `result` is greater than 0, room entry is successful, and the value indicates the time (in milliseconds) the room entry takes; if `result` is less than 0, room entry fails, and the value is the error code of the failure.
...
[](id:step8)
### Step 8. Enter a room as audience

1. If `role` in [TRTCParams](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCParams.html) is set to **`TRTCRoleType.TRTCRoleAudience`**, the current user role is audience.
2. Call `enterRoom()` as audience to enter an audio/video room whose ID is the value of `roomId` in `TRTCParams` and set the `appScene` parameter:
...
3. Watch the anchor's video:
    - If a user knows the `userId` of the anchor of a room, after entering the room, the user can call [startRemoteView(userId, view)](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#startRemoteView) to display the anchor's video image.
    - Audience receive the [onUserVideoAvailable()](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCallback.html#event:onUserVideoAvailable) callback after room entry. Audience not knowing the `userId` of the anchor can obtain the information from the callback and then call [startRemoteView(userId, view)](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#startRemoteView) to display the anchor's video image.
...
1. Call [switchRole(TRTCRoleType.TRTCRoleAnchor)](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#switchRole) to switch to the anchor role (`TRTCRoleType.TRTCRoleAnchor`).
...
3.  Call [startLocalAudio()](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#startLocalAudio) to enable audio capturing by the mic.

<dx-codeblock>
::: javascript javascript
// Sample code: the audience’s mic is on
trtcCloud.switchRole(TRTCRoleType.TRTCRoleAnchor);
trtcCloud.startLocalAudio();
trtcCloud.startLocalPreview(frontCamera, view);

// Sample code: the audience’s mic is off
...
3. Users in both rooms can call [startRemoteView(userId, view)](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#startRemoteView) to display the video image of the anchor in the other room. Audio will be played automatically.

![Event Timeline for Anchor Competition](http://main.qcloudimg.com/raw/ac5b230340ebdab69998f95844fa61c1/%E4%B8%BB%E6%92%AD%E8%BF%9E%E9%BA%A6%E6%97%B6%E5%BA%8F%E5%9B%BE.png)
...
Call the [exitRoom()](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCloud.html#exitRoom) method to exit the room. The SDK needs to disable and release hardware such as the camera and mic during room exit, so the process may take a while. Room exit is completed only after the [onExitRoom()](https://web.sdk.qcloud.com/trtc/electron/doc/zh-cn/trtc_electron_sdk/TRTCCallback.html#event:onExitRoom) callback is received.
