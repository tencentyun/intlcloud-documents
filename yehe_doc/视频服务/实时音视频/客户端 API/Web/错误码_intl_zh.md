...
| SIGNAL_CAHNNEL_SETUP_FAILED        | 16385   | 0x4001 | 信令通道建立失败。通常与腾讯云账号有关，参见具体 [帐号相关错误信息](https://intl.cloud.tencent.com/document/product/647/41665)。       |
| SIGNAL_CHANNEL_ERROR               | 16386   | 0x4002 | 信令通道错误。         |
| ICE_TRANSPORT_ERROR                | 16387   | 0x4003 | ICE Transport 连接错误，即音视频数据传输通道错误。这主要是因为用户端的 UDP 端口异常（可能是用户电脑防火墙或路由器防火墙端口限制）引起，具体端口请参见 [端口白名单](https://intl.cloud.tencent.com/document/product/647/35164)。  |
| JOIN_ROOM_FAILED    | 16388   | 0x4004 | 进房失败。详细请参见 [进房失败相关错误信息](https://intl.cloud.tencent.com/document/product/647/41665)          |
...
| PLAY_NOT_ALLOWED    | 16451   | 0x4043 | 自动播放被禁止错误<ul style="margin:0"><li/>说明：调用 `play()` 播放音视频时，由于[自动播放策略](https://developers.google.com/web/updates/2017/09/autoplay-policy-changes)的限制导致无法自动播放<li/>处理建议：此时应该引导用户通过手势操作调用 `resume()` 继续进行播放，请参见  [自动播放受限处理建议](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/tutorial-21-advanced-auto-play-policy.html) |
...
| 70017  | 帐号系统 | 内部第三方票据验证超时，请重试，如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
...
| 70145  | 帐号系统 | 帐号类型错误        |
| 70169  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70201  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70202  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70203  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
...
| 70212  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70213  | 帐号系统 | 访问内部数据失败，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70214  | 帐号系统 | 验证内部票据失败    |
| 70221  | 帐号系统 | 登录状态无效，请使用 UserSig 重新鉴权   |
| 70222  | 帐号系统 | 内部错误，请重试     |
| 70225  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70231  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70236  | 帐号系统 | 验证 user signature 失败    |
| 70308  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70346  | 帐号系统 | 票据校验失败。       |
| 70347  | 帐号系统 | 票据因过期原因校验失败      |
| 70348  | 帐号系统 | 内部错误，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70362  | 帐号系统 | 内部超时，请重试。如果多次重试仍不成功，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
| 70401  | 帐号系统 | 内部错误，请重试     |
| 70402  | 帐号系统 | 参数非法。请检查必填字段是否填充，或者字段的填充是否满足协议要求     |
| 70403  | 帐号系统 | 发起操作者不是 App 管理员，没有权限操作      |
| 70050  | 帐号系统 | 因失败且重试次数过多导致被限制，请检查票据是否正确，一分钟之后再试   |
| 70051  | 帐号系统 | 帐号已被拉入黑名单，请[联系我们](https://intl.cloud.tencent.com/contact-us) |
...
| -10018 | 启用高级权限控制后，client.join 未携带 privateMapKey 参数，或者 privateMapKey 为 ' '，请参见 [开启高级权限设置](https://intl.cloud.tencent.com/document/product/647/35157) |
| -10019 | 启用高级权限控制后，client.join 携带的 privateMapKey 参数不符合规范，请参见 [开启高级权限设置](https://intl.cloud.tencent.com/document/product/647/35157) |
| -10020 | 服务端超时，请重试    |
...
| ICE/DTLS Transport connection failed   | 建立媒体传输通道时失败     | 请检查 [防火墙配置](https://intl.cloud.tencent.com/document/product/647/35164)         |
...
| RtcError: no valid ice candidate found      | TRTC Web SDK 在 STUN 打洞失败      | 请检查下 [防火墙配置](https://intl.cloud.tencent.com/document/product/647/35164)       |
