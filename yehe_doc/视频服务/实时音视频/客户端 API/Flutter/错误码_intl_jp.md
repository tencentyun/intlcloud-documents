...
|ERR_SERVER_INFO_SERVICE_SUSPENDED|-100013|サービスをご利用いただけません。Tencent Cloudアカウントの支払いが遅延していないかどうかご確認ください|
...
|ERR_CONNECT_OTHER_ROOM_AS_AUDIENCE|-3330|現在のロールは視聴者であり、ルーム間マイク接続をリクエストまたは切断することはできません。まずキャスターにswitchRole()してください|
...
|WARNING_RTMP_SERVER_RECONNECT|1102|ライブストリーミングのときにネットワークが切断しましたが、自動再接続が開始されました（自動再接続は連続3回を超えて失敗すると、それ以上は行われません）|
|WARNING_LIVE_STREAM_SERVER_RECONNECT|2103|ライブストリーミングのときにネットワークが切断しましたが、自動再接続が開始されました（自動再接続は連続3回を超えて失敗すると、それ以上は行われません）|
