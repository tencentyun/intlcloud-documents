...
<span id="que8"></span>
### 如何把碎片拼接起来？
目前腾讯云支持使用云端 API 接口拼接视频分片，API 详细用法可以参见 [编辑视频](https://intl.cloud.tencent.com/zh/document/product/266/34126)。 
...
2. [创建录制任务](https://intl.cloud.tencent.com/zh/document/product/267/37309) 和 [创建录制模板](https://intl.cloud.tencent.com/document/product/267/34223) 为两种录制发起方式，实际使用中按需选择其中一种即可。若同一直播流，配置录制模板的同时创建了录制任务，会导致重复录制。请检查是否已在控制台开启录制任务同时，调用 API 3.0的  [CreateLiveRecord](https://intl.cloud.tencent.com/zh/document/product/267/37309) 接口或 API 2.0的 [Live_Tape_Start](https://intl.cloud.tencent.com/zh/document/product/267/9567) 接口发起了录制任务。
