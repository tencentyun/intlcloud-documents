## 基于 RTC 协议的连麦方案
目前，在 连麦互动 - RTMP方案中，腾讯云移动直播 SDK 提供连麦互动组件 `MLVBLiveRoom` 用来帮助开发者快速实现连麦需求，为了更好的满足开发者针对连麦功能的需求，腾讯云新增了基于 RTC 协议的连麦方案，同时提供了更加简单灵活的 V2 接口。
...
| 价格     | [咨询商务](https://intl.cloud.tencent.com/contact-sales)                | 阶梯价格，详情请参见 [RTC 连麦方案怎么计算费用](#price)  |
...
### 步骤1：服务开通 
超低延时直播需要在开始接入前，先开通腾讯云 [**实时音视频**](https://intl.cloud.tencent.com/document/product/647) 服务，具体步骤如下：

1. 您需要 [注册腾讯云](https://intl.cloud.tencent.com/document/product/378/17985) 账号，并完成 [实名认证](https://intl.cloud.tencent.com/document/product/378/3629)，未完成实名认证的用户无法购买中国大陆的资源。
...
| 计费时长类型 | 拉流分辨率                   | 单价（美元/千分钟） |
| ------------ | ---------------------------- | ------------------- |
| 纯语音       | -                            | 0.99                |
| 标清 SD      | 不高于640 × 480（含）        | 1.99                |
| 高清 HD      | 640 × 480 - 1280 × 720（含） | 3.99                |
...
是的，目前 `V2TXLivePusher&V2TXLivePlayer` 是 腾讯云 TRTC协议实现，其基于 UDP 的超低延时的私有协议暂时还不支持同一台设备使用相同的 streamid 进行通信，同时考虑到用户的使用场景，所以暂时并未支持，后续会酌情考虑此问题的优化。
