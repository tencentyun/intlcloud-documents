媒体生产业务存在内容不可控性，如有存在不合规的内容，可能带来相关法律风险和品牌伤害。为确保业务健康，保障服务稳定，云点播提供了 [音视频审核](https://intl.cloud.tencent.com/document/product/266/33944)能力。
...
4. 云点播执行上传签名时对 `procedure` 参数配置的音视频审核任务(`ReviewAudioVideoTask`)。
5. 云点播通过 [音视频审核完成](https://www.tencentcloud.com/document/product/266/50677) 通知 App 后台审核结果。
...
## 图片审核
云点播提供的 [图片审核](https://www.tencentcloud.com/document/product/266/47138) 功能，可以对图片内容发起审核，并在审核结果中给出建议（建议复核和建议通过）。与音视频内容审核不同，图片审核是同步处理过程，发起任务后将立即获取结果。用户侧可以根据建议来决定是否发布图片：
- 审核结果为“block”时，嫌疑度很高，建议直接拦截图片。
- 审核结果为“pass”时，嫌疑度不高，建议直接通过。
- 审核结果为“review”时，嫌疑度较高，建议人工复核。

发起图片审核的方式：
1. 通过服务端 API [图片审核](https://www.tencentcloud.com/document/product/266/47138) 调用。
