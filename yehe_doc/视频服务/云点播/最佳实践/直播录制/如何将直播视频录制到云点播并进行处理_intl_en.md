...
## Prerequisites
- You have [signed up](https://intl.cloud.tencent.com/register) for a Tencent Cloud account and [logged in](https://intl.cloud.tencent.com/login) to it.
- You have activated both the CSS and VOD services. If not, go to [CSS](https://console.cloud.tencent.com/live/livestat) and [VOD](https://console.cloud.tencent.com/vod/overview) consoles to activate the services.
...
- **Creating a template in the console**:
...
![](https://qcloudimg.tencent-cloud.cn/raw/2c70d96915ff91297434a1415aea49d0.png)
	2. Click **Create Recording Template** and select the recording file type (select at least one). For more information on the configuration items, see [Creating a Recording Template](https://intl.cloud.tencent.com/document/product/267/34223).
...
#### Scheme 1. Global recording of a specified domain name 
...
- **Use cases**: Live show, e-commerce live streaming, online classroom, and video surveillance.
- **Steps**:
...
#### Scheme 2. Recording a specified stream
You can record a specific live stream by binding your live recording template to the specified live stream through an API.

- **Use cases**: Live events, live exhibitions, sports live streaming, and live streaming through co-anchoring.
- **Steps**: Create a recording rule through the [CreateLiveRecordRule](https://intl.cloud.tencent.com/document/product/267/30846) API by passing in the template ID of the recording template and the target domain name, path, and `StreamName` (exact match required). This will bind your recording template to the specified live stream.

#### Scheme 3. Recording a specified time period
...
- **Use cases**: News live streaming and live events.
- **Steps**: Create a recording task through the [CreateRecordTask](https://intl.cloud.tencent.com/document/product/267/37309) API by passing in the template ID of the recording template and the target domain name, path, `StreamName` (exact match required), and the start time and end time for recording.
-**Example**:
 1. In the simplest case, you only need to specify `StreamName`, `DomainName`, `AppName`, and `EndTime` to record live streams.
The following sample code creates a video recording task in .flv format from 8 AM to 10 AM, August 10, 2020, with 30-minute segments, and the recording files will be retained permanently.
Sample request:
```
...
The following sample code creates a recording task in .mp4 format from 8 AM to 10 AM, August 10, 2020, with 1-hour segments, and the recording files will be retained permanently.
...
#### Scheme 4. Real-time recording (mixed-stream recording is supported)
You can record any frame of a stream in real time through API calls.

- **Use cases**: Scenarios where only segments need to be recorded (or clipped after global recording), such as sporting event or game live streaming.
- **Steps**: Create a recording task through the [CreateRecordTask](https://intl.cloud.tencent.com/document/product/267/37309) API by passing in the template ID of the recording template and the target domain name, path, StreamName (exact match required), and the end time for recording.
-**Example**:
 ```
...
- **Steps**: Select the .aac format as the recording file type and bind the corresponding push address when creating a recording template.
>!A binding rule will take effect in about 5 to 10 minutes after creation. Any change of the binding rule will not affect live streams that are already being pushed and will apply only to new live streams.
...
After the live stream is ended, the recording file will be stored on the [VOD](https://console.cloud.tencent.com/vod/overview) platform.
...
#### Scheme 3. Live recording + adaptive bitrate streaming + video delivery acceleration + player

- **Use cases**: If you have high requirements for video security that cannot be met by using HLS encryption, you can use adaptive bitrate streaming and the Player SDK in combination to further enhance the video security. This scheme is highly suitable for online education and corporate training.
...
- **Steps**:
	...
	5. Configure the player: Select the adaptive bitstream generated for playback.
