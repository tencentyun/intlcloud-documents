...
- [VOD 플레이어](https://github.com/LiteAVSDK/Player_Flutter/blob/main/Flutter/docs/VOD 재생.md): `TXVodPlayerController`는 Android 및 iOS용 VOD 플레이어 SDK의 API를 캡슐화합니다. `TXVodPlayerController`를 통합하여 VOD 서비스를 개발할 수 있습니다. 자세한 코드 샘플은 `DemoTXVodPlayer`를 참고하십시오.
- [라이브 플레이어](https://github.com/LiteAVSDK/Player_Flutter/blob/main/Flutter/docs/라이브 재생.md): `TXLivePlayerController`는 Android 및 iOS용 라이브 플레이어 SDK의 API를 캡슐화합니다. 라이브 재생 서비스를 설정하는 데 사용할 수 있습니다. 자세한 코드 샘플은 `DemoTXLivePlayer`를 참고하십시오.
...
- [플레이어 컴포넌트](https://github.com/LiteAVSDK/Player_Flutter/blob/main/Flutter/docs/플레이어 컴포넌트.md): `SuperPlayerController`는 VOD와 라이브 방송을 결합한 플레이어 컴포넌트입니다. 현재 Beta 테스트 중이며 기능이 최적화되고 있습니다. 자세한 코드 샘플은 `DemoSuperplayer`를 참고하십시오.

## 빠른 통합

### pubspec.yaml 구성

1. LiteAVSDK_Player(기본 버전)를 통합합니다.
...
2. LiteAVSDK_Professional을 통합하려면,
...
2. iOS는
   ...
   에서 통합할 SDK를 지정합니다(기본적으로 `LiteAVSDK_Player`).

   ```xml
   pod 'TXLiteAVSDK_Player'        //LiteAVSDK_Player
   ```

3. LiteAVSDK_Professional 통합:
...
아직 License가 없는 경우 [문의하기](https://intl.cloud.tencent.com/contact-us)하여 License를 받으십시오.
...
v10.7.0부터 재생 기능을 사용하려면 {@link SuperPlayerPlugin#setGlobalLicense}를 호출하여 Licence를 설정해야 합니다(Licence는 한 번만 설정하면 됨). 그렇지 않으면 재생이 실패됩니다(검은 화면). 라이브 스트림 퍼블리싱 Licence, UGSV Licence 또는 비디오 재생 Licence를 사용하여 재생 기능을 활성화할 수 있습니다. Licence가 없는 경우 평가판 Licence를 무료로 신청하거나 공식 Licence를 구매할 수 있습니다.
...
플레이어 컴포넌트의 핵심 클래스는 `SuperPlayerVideo`이며, 'SuperPlayerView' 객체를 만들어 동영상을 재생할 수 있습니다.
v10.7.0부터 재생 기능을 사용하려면 {@link SuperPlayerPlugin#setGlobalLicense}를 호출하여 Licence를 설정해야 합니다(Licence는 한 번만 설정하면 됨). 그렇지 않으면 재생이 실패합니다(검은 화면). 라이브 스트림 퍼블리싱 Licence, UGSV Licence 또는 비디오 재생 Licence를 사용하여 재생 기능을 활성화할 수 있습니다. Licence가 없는 경우 평가판 Licence를 무료로 신청하거나 공식 Licence를 구매할 수 있습니다.
