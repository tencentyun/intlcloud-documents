...
### 6단계: 뒤로 가기 버튼 클릭 이벤트 수신[](id:step6)
...
| maxBufferSize | int | 재생 버퍼의 최대 크기(MB), 이 설정은 playableDuration에 영향을 주며, 값이 클수록 미리 버퍼링된 데이터가 많음 |
| maxPreloadSize | int | 최대 사전 로딩 버퍼 크기(MB) |
| firstStartPlayBufferTime | int | 첫 번째 버퍼링 중에 로딩할 비디오 데이터의 지속 시간(ms), 기본값: 100ms |
...
| enableRenderProcess | bool | 포스트 렌더링 및 포스트 프로덕션 기능 허용 여부, 기본값: 활성화, 초고해상도 플러그인이 활성화된 후 존재하는 경우 해당 플러그인이 기본적으로 로딩함 |
| preferredResolution | int | 우선적으로 재생에 사용되는 비디오의 해상도, preferredResolution = width * height|
...
| 매개변수 | 코드   | 설명               |
| ------ | ------ | ------------------ |
| NO_ERROR | 0 | 오류 없이 성공적으로 시작 |
| ERROR_PIP_LOWER_VERSION | -101 | android 버전이 너무 낮음, PiP 모드 미지원 |
| ERROR_PIP_DENIED_PERMISSION | -102 | PiP 모드 권한 미활성화 또는 현재 장치가 PiP를 미지원 |
| ERROR_PIP_ACTIVITY_DESTROYED | -103 | 현재 UI가 종료됨 |
