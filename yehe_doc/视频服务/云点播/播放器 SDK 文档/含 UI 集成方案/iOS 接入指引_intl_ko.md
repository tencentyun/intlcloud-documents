...
```
Player_Premium 버전에 종속해야 하는 경우 podfile에 다음 종속성을 추가할 수 있습니다.
...
2. `TXLiteAVSDK_Player_Premium.framework`를 프로젝트로 가져오고 `Do Not Embed`를 선택합니다.
...
아직 필요한 License가 없다면 <a href="https://cloud.tencent.com/document/product/881/74588">비디오 재생 License</a>의 안내에 따라 License를 받을 수 있습니다.
<br>License 정보를 획득한 후 SDK의 해당 인터페이스를 호출하기 전에 다음 인터페이스를 통해 License를 초기화하고 `- [AppDelegate application:didFinishLaunchingWithOptions:]`에서 다음 설정하는 것이 좋습니다.
```objective-c
...
* Player가 수동 재생 모드 `PLAY_ACTION_MANUAL_PLAY`로 설정된 경우 사용자가 **재생**을 탭한 후에만 비디오 재생이 시작됩니다. 썸네일은 첫 번째 비디오 프레임이 로딩될 때까지 표시됩니다.
...
VideoCacheView(캐시 선택 목록 보기)는 다른 해상도로 비디오를 선택하고 다운로드하는 데 사용됩니다. 왼쪽 상단에서 해상도를 선택한 후 다운로드할 비디오의 옵션을 클릭합니다. 확인 표시가 나타나면 다운로드가 시작된 것입니다. 아래 video download list 버튼을 클릭하면 VideoDownloadListView가 있는 Activity로 리디렉션됩니다.
...
| 매개변수           | 유형         | 설명                     |
| ---------------- | ------------ | ------------------------ |
| models           | NSArray      | 다운로드 목록의 비디오 데이터 모델   |
| SuperPlayerModel | currentModel | 재생 중인 비디오의 비디오 데이터 모델 |

VideoCacheListView(동영상 다운로드 목록)는 다운로드 중이거나 다운로드된 모든 동영상의 View 목록을 표시합니다. 이 버튼을 클릭하면 다운로드가 진행 중이면 일시 중지됩니다. 일시 중지된 경우 다시 시작됩니다. 완료되면 비디오가 재생됩니다.
...
```objective-c
// TXVodDownloadManager#getDownloadMediaInfoList API에서 가져온 데이터를 추가합니다
...
// 목록 항목은 클릭하여 재생, 길게 눌러 삭제 등과 같은 작업을 지원합니다.
- (void)longPress:(UILongPressGestureRecognizer *)longPress;  // 길게 누르기
```

### 9. 이미지 스프라이트 및 타임스탬프 정보
...
```objective-c
// 1단계: onPlayEvent 콜백에서 이미지 스프라이트 및 타임스탬프 정보 가져오기(동영상이 playWithModelNeedLicence를 통해 재생되는 경우에만 작동합니다)
[self.playerView playWithModelNeedLicence:playerModel];

// 2단계: playWithModelNeedLicence에 대한 VOD_PLAY_EVT_GET_PLAYINFO_SUCC 콜백 이벤트에서 키프레임 및 이미지 스프라이트 정보 가져오기
...
// 3단계: 획득한 타임스탬프 정보와 이미지 스프라이트를 UI에 표시
...
App 업그레이드 및 유지 관리 중에도 Demo 소스 코드를 정상적으로 사용할 수 있습니다.
