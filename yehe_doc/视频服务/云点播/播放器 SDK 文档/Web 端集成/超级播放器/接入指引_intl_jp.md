...
## 機能紹介
Tencent Cloud View Cube Web Super PlayerはHTML5の`<Video>`タグおよびFlashを介してビデオ再生を実現します。ブラウザがビデオ再生をサポートしていない場合に、ビデオ再生効果のマルチプラットフォーム統合体験を実現し、かつTencent Cloud Video on Demandサービスと組み合わせて、防犯リンクとHLS共通暗号化ビデオ再生などの機能を提供します。
...
player.loadVideoByID({
  fileID: '', // 再生したいビデオのfileIDを渡してください（必須）
  appID: '', // VODアカウントのappIDを渡してください（必須）
  psign:'' // Keyリンク不正アクセス防止の説明をご参照ください
});
```
...
### イメージ機能
イメージ機能をアクティブ化すると、ビデオ画面のイメージを反転させることができます。
...
### プログレスバーの表示
サーバーAPIを介してビデオにタイムスタンプを追加するには、プレーヤーのプログレスバーの表示を有効にします。
...
- HLS Master Playlistを再生する場合、プレーヤーの解像度選択機能は、特定のビットレートの選択か、またはネットワーク速度に応じた自動選択に変更されます。
...
### Refererリンク不正アクセス防止
有効にする手順については、[Refererリンク不正アクセス防止](https://intl.cloud.tencent.com/document/product/266/33985)をご参照ください。プレーヤーを初期化するには次のパラメータを追加する必要があります。
```
var player = TCPlayer('player-container-id', {
     fileID: '', // 再生したいビデオのfileIDを渡してください（必須）
     appID: '', // VODアカウントのappIDを渡してください（必須）
...
>- プレーヤーswfURLのドメイン名をホワイトリストに追加する必要があります。Flashモードで再生することができるのはRefererリンク不正アクセス防止が有効化されているビデオだけです。
...
### Keyリンク不正アクセス防止
有効にする手順については、[Keyリンク不正アクセス防止](https://intl.cloud.tencent.com/document/product/266/33986)をご参照ください。プレーヤーを初期化するには次のパラメータを追加する必要があります。
```
var player = TCPlayer('player-container-id', {
     fileID: '', // 再生したいビデオのfileIDを渡してください（必須）
     appID: '', // VODアカウントのappIDを渡してください（必須）
...
>!Refererリンク不正アクセス防止を同時に有効にする場合は、Refererリンク不正アクセス防止設定のサンプルコードをもとにパラメータを追加します。

### プレビュー機能
プレビュー機能を使用する場合は、まずKeyリンク不正アクセス防止を有効にする必要があります。有効にする手順については、[Keyリンク不正アクセス防止](https://intl.cloud.tencent.com/document/product/266/33986)をご参照ください。プレーヤーを初期化するには次のパラメータを追加する必要があります。
```
var player = TCPlayer('player-container-id', {
  fileID: '', // 再生したいビデオのfileIDを渡してください（必須）
  appID: '', // VODアカウントのappIDを渡してください（必須）
...
### ビデオ統計情報
右クリックしてビデオ統計パネルを開けば、ビデオのリアルタイム情報を表示することができます。
...
| 1008 | Without anti-leech information.             | 防犯リンクテストが追加されていません。 |
...
| 日時             | バージョン     | 更新内容|
|-----------------|--------- |-------------------------------------|
| 2020.7.10       |   4.1    |   1. デフォルトのhls.jsバージョンを0.13.2に変更しました。<br> 2. Keyリンク不正アクセス防止機能をサポートしています。 <br>3. その他既知の問題を修正しました。 |
