...
本文档是介绍适用于直播和点播播放的 [Web 播放器（ TCPlayer ）](https://intl.cloud.tencent.com/document/product/266/33977)的相关参数以及 API。本文档适合有一定 Javascript 语言基础的开发人员阅读。
...
|  poster|  String|  无|  设置封面图片完整地址（如果上传的视频已生成封面图，优先使用生成的封面图，详细请参见 [云点播 - 管理视频](https://intl.cloud.tencent.com/document/product/266/33896)）。|
...
>! controls、playbackRates、loop、preload、posterImage 这些参数在浏览器劫持播放的状态下将无效（[什么是劫持播放？](https://intl.cloud.tencent.com/document/product/266/1303)）。
...
>! controlBar 参数在浏览器劫持播放的状态下将无效（[什么是劫持播放？](https://intl.cloud.tencent.com/document/product/266/1303)）。
