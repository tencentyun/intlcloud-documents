...
아직 License가 없는 경우 [문의하기](https://intl.cloud.tencent.com/contact-us)를 통해 License를 받으십시오.
...
::: fileid 방식
Fileid를 통해 다운로드하려면 최소한 appId, Fileid를 전달해야 합니다. **참고: 암호화된 비디오는 Fileid 방식으로만 다운로드할 수 있습니다.**

```objective-c
TXPlayerAuthParams *auth = [TXPlayerAuthParams new];
auth.appId = 1252463788;
auth.fileId = @"4564972819220421305";
auth.sign = @"psignxxxxx"; // fileId를 통한 재생에 필수
...
::: URL 방식
다운로드 주소는 필수입니다. 중첩되지 않은 단일 비트레이트 HLS 파일만 지원됩니다.
...
statPlay를 호출하기 전에 setConfig를 호출하여 플레이어 연결 시간 초과 기간, 진행 콜백 간격 및 캐시된 파일의 최대 수와 같은 플레이어 매개변수를 구성할 수 있습니다. TXVodPlayConfig를 사용하면 세부 매개변수를 구성할 수 있습니다. 자세한 내용은 [기본 구성 API](https://www.tencentcloud.com/document/product/266/47844txvodplayconfig)를 참고하십시오. 다음은 구성 예시 코드입니다.
...
| VOD_PLAY_EVT_LOOP_ONCE_COMPLETE | 6001 | 연속 재생, 루프 재생 완료(10.8 버전부터 지원) |
...
Tencent Cloud는 Player SDK를 기반으로 [플레이어 컴포넌트](https://www.tencentcloud.com/document/product/266/49649)를 개발했습니다. 품질 모니터링, 비디오 암호화, TESHD, 해상도 전환 및 작은 창 재생을 통합하고 모든 VOD 및 라이브 재생 시나리오에 적합합니다. 전체 기능을 캡슐화하고 상위 레이어 UI를 제공하여 인기 있는 비디오 App에 필적하는 재생 프로그램을 빠르게 만들 수 있습니다.
