...
::: Through `fileid`
To download through `fileid`, you need to pass in `appId` and `fileId`. **Note: You can download encrypted videos only through `Fileid` and must enter the `psign` parameter.**

```objective-c
TXPlayerAuthParams *auth = [TXPlayerAuthParams new];
auth.appId = 1252463788;
auth.fileId = @"4564972819220421305";
auth.sign = @"psignxxxxx"; // Required for playback through `fileId`
...
::: Through URL
You only need to pass in the download URL. Only the non-nested HLS, i.e., single-bitstream HLS, is supported.
...
<td>V_SUM_CACHE_SIZE</td><td>The buffer (`jitterbuffer`) size. If the current buffer length is `0`, lag will occur soon.</td>
</tr><tr>
