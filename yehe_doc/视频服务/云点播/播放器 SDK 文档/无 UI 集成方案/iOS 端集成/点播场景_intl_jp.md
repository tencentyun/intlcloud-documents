...
## このドキュメントから把握できること
* Tencent Cloud View Cube iOS Player SDKの統合方法
...
### ステップ1：SDK開発キットの統合
SDK開発キットのダウンロードと統合については、同ディレクトリ下の[ SDK統合ガイド](https://www.tencentcloud.com/document/product/266/49669)をご参照ください。


### ステップ2：License権限承認の設定
すでにLicense権限承認を取得している場合は、[Tencent Cloud View Cubeコンソール](https://www.tencentcloud.com/account/login)で、License URLおよびLicense Keyを取得する必要があります。

License権限承認を取得していない場合は、先に[ビデオ再生License](https://www.tencentcloud.com/document/product/266/51098) をご参照の上、関連する権限承認を取得してください。
...
// YESに設定すると、IDRが合っている場合にビットレートをスムーズに切り替えることができます。NOに設定すると、マルチビットレートアドレスを開く速度が速くなります
...
```objectivec
// ビデオAの再生：isAutoPlayがYESに設定されている場合、startVodPlayの呼び出しにより、直ちにビデオのロードと再生が開始されます
...
// ダウンロードディレクトリを設定します。‘ TXPlayerGlobalSetting #setCacheFolderPath’による設定を優先します
[downloader setDownloadPath:"<ダウンロードディレクトリを指定>"];
```

[](id:offline2)
#### ステップ2：ダウンロード開始
ダウンロードを開始するには、fileidとURLという2つの方法があります。
<dx-tabs>

::: fileid方式
fileidによるダウンロードでは、少なくともappIdとfileIdを渡す必要があります。userNameは具体的な値が渡されない場合、デフォルトで「default」となります。**注意：暗号化されたビデオはFileidでのみダウンロードできます。**
...
// ダウンロードの解像度を指定します
// 使用可能な列挙値には、オリジナル画質:TXVodQualityOD、LD:TXVodQualityFLU、SD: TXVodQualitySD、HD:TXVodQualityHD、FHD: TXVodQualityFHD、2K: TXVodQuality2K、4K:TXVodQuality4Kがあります
source.quality = TXVodQualityHD;  // HD
    
// ** 注意：古いv2プロトコルを使用してダウンロードした場合は、TXVodDownloadDataSourceのauth属性によってappIdとfileIdのパラメータを設定してください **
// source.auth = auth;  ** デフォルトでは設定不要 **
...
::: URL方式
ダウンロードアドレスを渡すだけです。ネストしたHLS形式はサポートしておらず、シングルビットストリームのHLSのダウンロードのみサポートしています。プライベート暗号化の場合はfileid形式を使用してください
...
for (TXVodDownloadMediaInfo *info in array) {
    if ([info.userName isEqualToString:@"default"]) {
        // 「default」ユーザーのダウンロードリストを保存します
    }
}
```

FileIdまたはURLに関連するダウンロード情報を取得します。

インターフェース`-[TXVodDownloadManager getDownloadMediaInfo:]`からFileidに関連するダウンロード情報を取得します。現在のダウンロード状況、ダウンロードの進捗の取得、ダウンロード完了の判断などが含まれます。AppID、FileidおよびqualityIdを渡す必要があります。
...
downlaodMediaInfo.size; //  ダウンロードファイルの合計サイズを取得します
downlaodMediaInfo.duration;   // 合計時間を取得します
downlaodMediaInfo.playableDuration; // ダウンロード済みの再生可能時間を取得します
downlaodMediaInfo.progress;   // ダウンロード進行状況を取得します
downlaodMediaInfo.playPath;   // オフライン再生パスを取得し、プレーヤーに渡すことでオフライン再生が可能になります
downlaodMediaInfo.downloadState;  // ダウンロードステータスを取得します。詳細については、STATE_xxx定数をご参照ください
[downlaodMediaInfo isDownloadFinished];  // YESが返された場合はダウンロードが完了しています
```

URLのダウンロード情報を取得するには、URLの情報を渡します。
...
```objective-c
TXPlayerAuthParams *p = [TXPlayerAuthParams new];
p.appId = 1252463788; // Tencent CloudアカウントのappId
p.fileId = @"4564972819220421305"; // ビデオのfileId
// psignはプレーヤーの署名です。署名についての紹介および生成方法は、以下のリンクをご参照ください。https://www.tencentcloud.com/document/product/266/38099
p.sign = @"psignxxxxx"; // プレーヤー署名
[_txVodPlayer startVodPlayWithParams:p];
```
...
startPlayを呼び出す前に、setConfigによってプレーヤーのパラメータを設定することができます。例としては、プレーヤーの接続タイムアウト時間の設定、プログレスコールバック間隔の設定、キャッシュファイル数の設定などです。TXVodPlayConfigがサポートするパラメータの詳細については、[基本設定インターフェース](https://www.tencentcloud.com/document/product/266/47844#txvodplayconfig)をクリックしてご参照ください。使用例は以下のとおりです。
