...
* How to integrate the Tencent Cloud RT-Cube Player SDK for Android.
...
If you don't have the required license yet, please [contact us](https://intl.cloud.tencent.com/contact-us) to get a license.
...
```
>? The boolean parameter of `stopPlay` indicates whether to clear the last-frame image. Early versions of the live player of the VOD SDK don't have the concept of pause; therefore, this boolean value is used to clear the last-frame image.
...
mVodPlayer.setVodListener(new ITXVodPlayListener() {
    ...
    public void onPlayEvent(TXVodPlayer player, int event, Bundle param) {
        if (event == TXLiveConstants.PLAY_EVT_PLAY_PROGRESS) {
            ...
            // The total video duration in milliseconds
...
// QUALITYSD  // SD
// QUALITYHD  // HD
TXVodDownloadDataSource source = new TXVodDownloadDataSource(1252463788, "4564972819220421305", QUALITYHD, "pSignxxxx", "");   // `pSign` is the signature of the encrypted video, which is required for download through `fileId`.
...
Before calling `startVodPlay`, you can call `setConfig` to configure the player parameters, such as player connection timeout period, progress callback interval, and maximum number of cached files. `TXVodPlayConfig` allows you to configure detailed parameters. For more information, see [API Documentation](https://www.tencentcloud.com/document/product/266/47851). Below is the configuration sample code:
