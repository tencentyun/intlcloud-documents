...
4. 왼쪽 사이드바에서 **비디오 처리** > **템플릿 설정**을 선택합니다. 콘솔의 사전 설정 템플릿에는 **비디오 트랜스 코딩 템플릿**, **TESHD 템플릿**, **오디오 트랜스 코딩 템플릿**, **리먹싱 템플릿**, **어댑티브 비트 레이트 스트리밍 템플릿**, **워터마크 템플릿**, **스크린샷 템플릿**, **애니메이션 이미지 템플릿** 및 **스마트 인식 템플릿**이 포함됩니다. 비디오 처리 설정을 위한 워크플로 설정에 각 템플릿을 추가할 수 있습니다.
...
#### 사전 설정 템플릿
자세한 매개변수는 [사전 설정 매개변수 템플릿 리스트- 트랜스 코딩 템플릿](https://intl.cloud.tencent.com/document/product/266/33932#preset-transcoding-templates)을 참고하십시오.

## TESHD 템플릿

비즈니스 니즈에 따라 새 TESHD 템플릿을 생성하고 사용자 정의 설정을 진행할 수 있습니다. **TESHD 템플릿**을 선택하고 **트랜스 코딩 템플릿 생성**을 클릭하여 템플릿 사용자 정의 설정으로 이동합니다.
+ 템플릿 이름: 중국어, 영어, 숫자, 스페이스, 언더바(_), 하이픈(-), 마침표(.)만 지원되며, 최대 64자까지 입력할 수 있습니다.
+ 캡슐화 형식: MP4
+ 설정 항목: 비디오 매개변수, 오디오 매개변수
+ 비디오 매개변수:
	+ 코덱: H.264
	+ 평균 비트 레이트의 최댓값: 공란 또는 0은 비디오 비트 레이트의 최댓값이 없음을 의미합니다.
	+ 해상도: 동영상의 긴 변(동영상 너비)과 동영상의 짧은 변(동영상 높이)은 0 또는 128px - 4096px입니다.
	+ 프레임 레이트: 0 - 60fps
+ 오디오 매개변수:
	+ 인코딩 표준: AAC, MP3
	+ 샘플링 레이트: 32000Hz, 44100Hz, 48000Hz 3종류의 기본 샘플링 레이트
	+ 오디오 비트 레이트: 0 또는 26kbps - 256kbps
	+ 사운드 채널: 싱글 사운드 채널, 듀얼 사운드 채널
+ 자주 사용하는 템플릿: 자주 사용하는 템플릿 설정 여부
...
## 오디오 트랜스코딩 템플릿[](id:yp)
비즈니스 니즈에 따라 새 오디오 트랜스코딩 템플릿을 생성하고 사용자 정의 설정을 진행할 수 있습니다. **오디오 트랜스코딩 템플릿**을 선택한 다음 **오디오 템플릿 생성**을 클릭하여 템플릿 사용자 정의 설정으로 이동합니다.
...
#### 사전 설정 템플릿
자세한 매개변수는 [사전 설정 매개변수 템플릿 리스트 - 오디오 트랜스 코딩 템플릿](https://intl.cloud.tencent.com/document/product/266/33932#preset-transcoding-templates)을 참고하십시오.


## 어댑티브 비트 레이트 스트리밍 템플릿

시스템 사전 설정 어댑티브 비트 레이트 스트리밍 템플릿을 직접 사용하거나 비즈니스 요구 사항에 따라 사용자 정의 템플릿을 만들 수 있습니다.
+ **기본 정보**
	+ 템플릿 이름: 중국어, 영어, 숫자, 스페이스, 언더바(_), 하이픈(-), 마침표(.)만 지원되며, 최대 64자까지 입력할 수 있습니다.
	+ 패키징 형식: HLS
	+ 암호화 유형: 암호화 없음 또는 SimpleAES
	+ 저해상도에서 고해상도로 전환: 활성화 또는 비활성화
+ **서브 스트림 정보**
	+ 비디오 인코딩 표준: H.264, H.265
	+ 비디오 비트 레이트: 0 또는 128kbps - 35000kbps
	+ 비디오 해상도: 비디오 긴 변 및 짧은 변은 0 또는 128px - 4096px로 제한
	+ 비디오 프레임 레이트: 0 - 60fps로 제한
	+ 오디오 인코딩 표준: AAC, MP3
...
>- [사전 설정 매개변수 템플릿 리스트 - 어댑티브 비트 레이트 스트리밍 템플릿](https://intl.cloud.tencent.com/document/product/266/33932#preset-adaptive-bitrate-streaming-templates)을 참고하십시오.
...
### 시점 스크린샷
스크린샷 유형에서 시간대별 스크린샷을 선택합니다. **샘플링 시점은 스크린샷 작업 설정에서 지정해야 하며 템플릿 설정에서는 스크린샷 유형 설정만 수행할 수 있습니다.** 시간대별 스크린샷 작업 설정은 [태스크 플로우 설정](https://intl.cloud.tencent.com/document/product/266/14058#custom-task-flow)을 참고하십시오.

- 템플릿 이름: 중국어, 영어, 숫자, 스페이스, 언더바(_), 대시(-), 마침표(.)만 지원되며, 최대 64자까지 입력할 수 있습니다.
- 이미지 포맷: JPG
- 이미지 크기: 0 또는 128px - 4096px

#### 사전 설정 템플릿
자세한 매개변수는 [사전 설정 매개변수 템플릿 리스트 - 지정 시점 화면 캡처 템플릿](https://intl.cloud.tencent.com/document/product/266/33932#preset-animated-image-generating-templates)을 참고하십시오.
...
#### 사전 설정 템플릿
자세한 매개변수는 [사전 설정 매개변수 템플릿 리스트 - 샘플링 화면 캡처 템플릿](https://intl.cloud.tencent.com/document/product/266/33932#preset-time-point-screenshot-templates)을 참고하십시오.
...
#### 사전 설정 템플릿
자세한 매개변수는 [사전 설정 매개변수 템플릿 리스트 - 스프라이트 이미지 템플릿](https://intl.cloud.tencent.com/document/product/266/33932#preset-sampled-screenshot-templates)을 참고하십시오.
...
#### 사전 설정 템플릿
자세한 매개변수는 [사전 설정 매개변수 템플릿 리스트 - gif 템플릿](https://intl.cloud.tencent.com/document/product/266/33932#preset-remuxing-templates)을 참고하십시오.
...
>?[사전 설정 매개변수 템플릿 리스트 - 스마트 인식 템플릿](https://console.cloud.tencent.com/vod/video-process/template/audit)을 참고하십시오.
