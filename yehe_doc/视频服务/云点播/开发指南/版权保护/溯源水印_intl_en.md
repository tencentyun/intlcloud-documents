One of the greatest challenges faced by paid video platforms is the unauthorized download and sharing of videos. This severely damages the interests of copyright holders. Unauthorized video recording is the most common form of video piracy. Pirates use screen recording software to record the video or use a physical camera to record the screen directly, which is difficult to prevent.

An effective way to suppress unauthorized video recording is to track down pirates and use other means to protect copyrighted content, deter piracy, and demand compensation.VOD's digital watermark guarantees a high security level at low costs and helps you trace unauthorized video recording with ease.
...
<table>
   ...
   <tr>
      <td><img src="https://qcloudimg.tencent-cloud.cn/raw/82766338aee9f4f9715427e337babd25.png" width=500>
           An image or text watermark encoded to the video image during transcoding. The text content is the user ID.
</td>
...
Create an image watermark template in the console or via APIs (note that the watermark needs to be in the upper part of the image).
...
* If you use the [Superplayer](https://intl.cloud.tencent.com/document/product/266/33977) or [Superplayer Adapter](https://intl.cloud.tencent.com/document/product/266/42095) of VOD, your business server needs to assign a [Superplayer signature](https://intl.cloud.tencent.com/document/product/266/38099) for each playback request. You need to enter the viewer `uid` as the `uid` in [signature parameters](https://intl.cloud.tencent.com/document/product/266/38099#.E7.AD.BE.E5.90.8D.E5.8F.82.E6.95.B0).
* If you don't use a VOD player, you need to add the viewer `uid` to `QueryString` in the URL according to the rules of [key hotlink protection](https://intl.cloud.tencent.com/document/product/266/33986).
...
2. Select image watermark for **Watermark Type**. After uploading a watermark image, click **Create** (note that the watermark needs to be in the upper part of the video image).
