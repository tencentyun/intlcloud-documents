...
## 事件说明
当 App 配置了事件通知，并且在合成视频完成后，App 后台即可通过“普通回调”或“可靠回调”的方式获取该事件通知。事件通知内容为 [ReduceMediaBitrateTask 结构](https://intl.cloud.tencent.com/document/product/266/34187#ReduceMediaBitrateTask)。
...
### 可靠回调
如果选择可靠回调模式，调用 [拉取事件通知](https://intl.cloud.tencent.com/document/product/266/34183) API 会接收到如下形式的 HTTP 应答（省略了值为 null 的字段）。
