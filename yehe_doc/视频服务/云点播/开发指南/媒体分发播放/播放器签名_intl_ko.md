...
| drmLicenseInfo   | No   | Object  | DRM License 구성입니다. 이 매개변수의 구조는 [DrmLicenseInfo](#p5)를 참고하십시오.       |
...
| 매개변수          | 필수 | 유형            | 설명                                                         |
| ----------------- | ---- | --------------- | ------------------------------------------------------------ |
| persistent        | No   | String          | 재생 장치에서 DRM 재생 라이선스의 영구 저장을 허용할지 여부입니다. 유효한 값: <ul style="margin:0;"><li>ON: 허용;</li><li>OFF: 허용하지 않음.</li></ul>기본값은 OFF. |
| rentalDuration    | No   | Integer         | persistent이 ON일 때, DRM 재생 라이선스의 허용된 저장 시간(초)입니다. 지정하지 않으면 저장 시간에 제한이 없습니다. |
| forceL1TrackTypes | No   | Array of String | Widevine을 사용할 때 L1 보안 수준을 사용해야 하는 Track 유형입니다. 다른 Track 유형의 경우 Widevine L3가 사용됩니다. 유효한 값: <ul style="margin:0;"><li>AUDIO: 오디오 트랙;</li><li>SD: 짧은 면이 720 픽셀보다 작은 비디오 트랙</li><li>HD: 짧은 면이 720 픽셀 이상 2160 픽셀 미만인 비디오 트랙;</li><li>UHD1: 짧은 면이 2160 픽셀 이상 4320 픽셀 미만인 비디오 트랙;</li><li>UHD2: 짧은 면이 4320 픽셀 이상인 비디오 트랙.</li></ul> |
