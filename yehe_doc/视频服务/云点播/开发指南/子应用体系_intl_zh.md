...
服务端 API 2017也支持子应用，使用时需要在请求中增加一个`SubAppId`参数（注意大小写），该参数与服务端 API 2017 的 [公共请求参数](https://intl.cloud.tencent.com/document/api/213/6976) 平级，其值为子应用 ID。如果要访问的是主应用，既可以填写主应用的标识符，也可以不填。
