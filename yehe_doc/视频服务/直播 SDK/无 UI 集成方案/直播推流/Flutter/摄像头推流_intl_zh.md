...
[](id:step1)
### 1. 设置依赖
按照 [SDK 集成指引](https://www.tencentcloud.com/document/product/1071/50582) 将 live_flutter_plugin 嵌入您的 App 工程中。
...
[](id:step5)
### 5. 启动和结束推流
如果已经通过`startCamera`接口启动了摄像头预览，就可以调用 V2TXLivePusher 中的 [startPush](https://pub.flutter-io.cn/documentation/live_flutter_plugin/latest/v2_tx_live_pusher/V2TXLivePusher/startPush.html) 接口开始推流。推流地址可以使用 [TRTC 地址](https://www.tencentcloud.com/document/product/1071/39359) ，或者使用 [RTMP 地址](https://www.tencentcloud.com/document/product/1071/39359#.E8.87.AA.E4.B8.BB.E6.8B.BC.E8.A3.85.E6.8E.A8.E6.B5.81-url) ，前者使用 UDP 协议，推流质量更高，并支持连麦互动。
```dart
/// 开始推流
startPush() async {
  // 生成推流地址 RTMP/TRTC
...
-  **如何获取可用的推流 URL**
开通直播服务后，可以使用**直播控制台** > **辅助工具** > [**地址生成器**](https://console.tencentcloud.com/live/addrgenerator/addrgenerator) 生成推流地址，详细信息请参见 [推拉流 URL](https://www.tencentcloud.com/document/product/1071/39359)。
...
[](id:step7)
### 7. 设定画面清晰度
调用 V2TXLivePusher 中的 [setVideoQuality](https://pub.flutter-io.cn/documentation/live_flutter_plugin/latest/v2_tx_live_pusher/V2TXLivePusher/setVideoQuality.html) 接口，可以设定观众端的画面清晰度。之所以说是观众端的画面清晰度，是因为主播看到的视频画面是未经编码压缩过的高清原画，不受设置的影响。而 `setVideoQuality` 设定的视频编码器的编码质量，观众端可以感受到画质的差异。详情请参见 [设定画面质量](https://www.tencentcloud.com/document/product/1071/41861)。
