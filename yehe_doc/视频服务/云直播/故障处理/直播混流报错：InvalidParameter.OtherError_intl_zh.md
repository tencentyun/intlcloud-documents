...
2. 使用 [VLC 工具播放](https://intl.cloud.tencent.com/document/product/267/32483)：
...
此报错为您使用旧版本控制台的账号（频道模式），并在客户服务端调用了新版的云直播 API 3.0 [直播混流接口](https://intl.cloud.tencent.com/document/product/267/35997) 时报的错误。

您可将 [云直播控制台升级](https://intl.cloud.tencent.com/zh/document/product/267) 到新版本（直播码模式），即可解决报错问题。
...
详细的参数设置说明和实践操作，请参见 [创建通用混流 > MixStreamTemplateId 接口](https://intl.cloud.tencent.com/document/product/267/35997)。
...
- 输出画面的 X 偏移，参数配置建议请参见  [通用混流布局参数 > LocationX](hhttps://intl.cloud.tencent.com/zh/document/product/267/30767#CommonMixLayoutParams)。
- 输出画面的 Y 偏移，参数配置建议请参见 [通用混流布局参数 > LocationY](hhttps://intl.cloud.tencent.com/zh/document/product/267/30767#CommonMixLayoutParams)。
- 更多云端混流模板使用示例，请参见 [云端混流](https://intl.cloud.tencent.com/document/product/267/37665)。


[](id:error7)
#### 报错7：`"Message":"InnerErrCode:[ -111 ],InnerErrMsg: [ output_stream_type is [0],but output_stream_id xxxxx is not in input stream list ]"`
这是因为您设置的混流参数中，[OutputStreamType]((https://intl.cloud.tencent.com/zh/document/product/267/30767) 默认取值为0，而实际上输出的流 ID 名称并不在您输入的流名称列表里面，从而导致的报错。
您需要注意 ：
