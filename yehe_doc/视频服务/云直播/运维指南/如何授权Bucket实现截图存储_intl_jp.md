ここでは、主にTencent LVBスクリーンキャプチャ、ポルノ検出データをCOSに保存し、バケット（COS Bucket）を介してLVBスクリーンキャプチャ、ポルノ検出データを保存する方法を紹介します。まずCOS Bucketを作成し、次にCOS Bucketを介してLVBを承認し、最後にLVBコンソールでLVBスクリーンキャプチャ、ポルノ検出を設定します。その後LVBスクリーンキャプチャ、ポルノ検証データを指定されたCOS Bucketに書き込むことができます（新規バージョンのコンソール機能）。
### COS Bucketの作成
...
>!
> - Bucket name は examplebucketで、右側の数字は含まれません。  
> - 以上の情報はいずれも業務の実際の必要性に応じて設定できます。
3. 業務の必要性に応じてCOS bucketのCDNアクセラレーションを有効にすることができます。作成済みのバケット名または【設定管理】をクリックし、左側の【ドメイン名と伝送管理】>【デフォルトのCDNアクセラレーションドメイン名】をクリックします。【デフォルトのCDN アクセラレーションドメイン名】設定項目の【編集】をクリックし、現在のステータスを有効に設定した後、下部のオプションを設定します。具体的な設定方法については、[デフォルトのCDNアクセラレーションドメイン名の有効化](https://intl.cloud.tencent.com/zh/document/product/436/31505)をご参照ください。設定完了後、【保存】をクリックすれば、CDNアクセラレーションが有効になります。
...
1. Tencent Cloudスクリーンキャプチャ保存のデータ書き込み権限を有効にします。承認するルートアカウントID：3508645126。
	...
	>! **認証を実行するために、アカウントIDには ルートアカウント（メインアカウントとも言います） ID：3508645126を入力する必要があります。（ルートアカウントID：3508645126 はLVBサービスですので、3508645126を直接入力してください）**。
...
2. 承認済みのCOS Bucket情報を取得します。
	...
	2. 上記3つのフィールド情報を送信すると、システムはLVBスクリーンキャプチャデータを承認されたCOS Bucketに保存します。
