...
或单击【授权管理】进入授权管理界面，勾选需授权的存储桶，打开【公共权限】和【用户权限】按钮添加用户。用户类型选择根账号，<b>并输入根账号 ID：`-125****118`</b>。单击【保存】并【确定】。
...
>! <b>账号 ID 需填入根账号（也就是主账号） ID：`125****118` 进行授权。（根账号 ID：`125****118` 即为云直播服务 APPID，直接输入 `125****118` 即可）</b>。
