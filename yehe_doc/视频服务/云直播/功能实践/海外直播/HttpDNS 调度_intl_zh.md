...
运营商 LocalDNS 的出口根据权威 DNS 目标 IP 地址进行 NAT，或者将解析请求转发至其他 DNS 服务器，导致权威 DNS 无法正确识别运营商的 LocalDNS IP，从而引发域名解析错误和流量跨网等问题。腾讯云 HTTPDNS 具有全球领先的 DNS 集群技术，可支持多运营商和自定义线路，进行优化调度。

>?  本文为您阐述如何将 HTTPDNS 调度方案用于腾讯云国内和海外直播推流和播放的调度加速，HTTPDNS 接口请参见 [移动解析 HTTPDNS](https://intl.cloud.tencent.com/document/product/1130/44468)。

## 前期准备
1. 开通 HTTPDNS 服务，请参见腾讯云移动解析 HTTPDNS 控制台的服务 [开通步骤](https://intl.cloud.tencent.com/document/product/1130/44461)。
...
- push_domain 代表推流域名，该字段需要经过 DES 加密，密钥信息通过[HTTPDNS开发配置页](https://console.cloud.tencent.com/httpdns/configure)获取，具体请参见 [DES 加解密说明](https://intl.cloud.tencent.com/document/product/1130/44470)。
- ip 字段代表请求端的外网出口 IP，这个 IP 代表最终会调度到的接入点 IP 所在的地区和运营商，该字段也同样需要经过 DES 加密。
- id 字段代表用户鉴权标识，唯一标识每个用户。

### 解密接入点IP

通过 HTTPDNS 获取到的数据为 DES 密文，需要经过 DES 解密，得到 server_ip，具体请参见 [DES 加解密说明](https://intl.cloud.tencent.com/document/product/1130/44470)。
...
| 字段 | 含义 |
|---------|---------|
| push_domain | 播放域名，该字段需要经过 DES 加密，密钥信息通过 [HTTPDNS开发配置页](https://console.cloud.tencent.com/httpdns/configure) 获取，具体请参见 [DES 加解密说明](https://intl.cloud.tencent.com/document/product/1130/44470)。 |
