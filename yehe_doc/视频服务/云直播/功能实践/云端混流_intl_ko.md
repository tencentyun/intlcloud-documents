LVB 서비스는 라이브 방송 혼합 스트림 기능을 제공하며, 설정한 혼합 스트림 레이아웃 동기화에 따라 각 입력 원본 혼합 스트림을 새로운 스트리밍으로 만들어 라이브 방송 인터랙션 효과를 구현합니다. 또한 LVB 라이브 방송 혼합 스트림 기능은 API 3.0 인터페이스와도 연결할 수 있습니다. 자세한 내용은 [라이브 방송 혼합 스트림 인터페이스](https://intl.cloud.tencent.com/document/product/267/35997)를 참조하십시오. 본 문서에서는 각 시나리오에서 라이브 방송 혼합 스트림을 구현하는 방법에 대해 예시와 함께 설명합니다.
