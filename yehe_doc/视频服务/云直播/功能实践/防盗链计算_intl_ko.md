...
### 2단계: txTime 생성
서명 중 플레인 텍스트 부분이 txTime이며, 해당 링크의 유효기간을 의미합니다. 예를 들어 현재 시간이 2018-12-29 11:13:45이고, 생성할 URL을 3시간 후 폐기하고 싶은 경우 txTime은 2018-12-29 14:13:45로 설정됩니다.
그러나 이렇게 긴 시간 문자열을 URL에 넣기에는 매우 부적합합니다. 실제로는 2018-12-29 14:13:45를 UNIX 타임스탬프로 변환해 1546064025(프로그래밍 언어별로 직접 시간 함수를 호출하여 전환할 수 있음)를 획득하고, 다시 16진법으로 전환하여 다시 문자열 길이를 압축합니다. 즉, txTime = 1546064025(10진법) = 5C271099(16진법)로, 물론 10진법 사용도 지원합니다.
...
### 3단계: txSecret 생성
txSecret 생성 방법 = MD5(KEY + StreamName + txTime), 여기에서 KEY는 1단계에서 설정한 암호화 KEY이며, 이 예에서 StreamName(스트림 ID라고도 하며, 랜덤 숫자 또는 사용자 ID 사용 권장)은 test입니다. txTime은 위에서 계산한 5C271099이고, MD5는 표준 MD5 단방향의 복호화 불가한 Hash 알고리즘입니다.
예시:
```
