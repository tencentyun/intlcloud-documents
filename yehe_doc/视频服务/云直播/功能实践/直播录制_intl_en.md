...
<td>You can configure whether to record a stream at the push domain name and stream name levels.</td>
</tr>
<tr>
...
2. Select **[Domain Management](https://console.cloud.tencent.com/live/domainmanage)** to add a push domain name, and click **Manage** to associate it with the recording template. For more information, please see [Recording Configuration](https://intl.cloud.tencent.com/document/product/267/34224).
...
First, please familiarize yourself with the stream mix service in [On-cloud Stream Mix](https://intl.cloud.tencent.com/document/product/267/37665).
...
To record a mixed stream only, you can call the [CreateRecordTask](https://intl.cloud.tencent.com/document/product/267/37309) API. It should be noted that if `OutputStreamType` =`1`, the `StreamType` parameter should be set to `1` when the above API is called.
...
Currently, auto-spliced recording is available only to the HLS format. You can set the auto-splicing timeout value in the [recording configuration](https://intl.cloud.tencent.com/document/product/267/34223).
...
Log in to the [VOD Console](https://console.cloud.tencent.com/vod/media) and select **a user rather than Admin**. Select **Media Assets** > **Video Management** to browse all recording files.
