- Live recording stores the files generated by muxing original streams (without modifying information such as audio and video data and corresponding timestamps) on the VOD platform.
  ...
  ### CSS console
  1. Select **Feature Configuration** > **[Live Recording](https://console.cloud.tencent.com/live/config/record)** to create a recording template.
  ...
  2. Call the [CreateLiveRecordRule](https://intl.cloud.tencent.com/document/product/267/30846) API to set the `DomainName` (push domain name) and `TemplateId` (returned in step 1) parameters. You can enter an empty string in `AppName` and `StreamName` as a wildcard to indicate all streams under the domain name.

  You can also bind the template with names of specific live streams to record them.
  As one template can be bound with different push domain names and stream names, one stream may be matched with multiple templates. In this case, the template with the highest priority will be used. The rules of template matching by priority are shown below (for complex use cases only and can be ignored by most users).
  ...
  Here, **Empty string** is a wildcard, **&#10003;** indicates exact match, and **0** represents the highest priority. Once a template with the highest priority is matched, the matching will stop and this template will be returned.
  ...
  **Sample input code:**
  ...
  > - You’re advised to create recording tasks in advance (e.g., one hour in advance or in the early morning), and to set the task start time a bit earlier than the event start time.
  ...
  > !
  > - This feature does not support live streams with no audio data.
  > - You need to call the [ComposeMedia](https://intl.cloud.tencent.com/document/product/266/34127) API of VOD to compose video files.
