...
<td>ライブストリーミング再生リンクはURLです。リンク不正アクセス防止がない再生URLは、取得さえすれば誰でも再生することができます。さらに再生URLは一定のルールがあることが多く、例えば<code>プロトコル:\\再生ドメイン名\AppName\ストリームID</code>方式をスプライシングしたものがあります。リンク不正アクセス防止がない場合、誰でもルールを推測すれば、ルールに応じて自身で組み合わせ、トラバース再生し、すべてのストリームを視聴することができます。一つ目にはコンテンツを秘密にできず、二つ目には盗用によって発生したトラフィックは自身が支払う必要があります。</td>
</tr><tr>
<td>視聴者のフィルタリング</td>
...
- **適用ケース**：ライブストリーミングのコンテンツセキュリティをあまり気にせず、再生の敷居を少し高くしたいだけであれば、Refererリンク不正アクセス防止を使用することができます。
- **実施方法**：CSSコンソール=>ドメイン名管理=>再生ドメイン名の選択=>アクセス制御=>と進み、Referリンク不正アクセス防止を開いてRefer内容を書き込みます
- **優位性**：簡単に実施でき、設定を制御すれば完了します。
- **存在する問題**：再生リクエスト中のReferは修正、偽造できるため、Referの制限内容が知られると、再生リクエスト中のReferが修正され、サーバーのRefer制限が回避されてしまいます。

### IPブラックリスト/ホワイトリスト

- **適用ケース**：再生を許可するまたは許可しないユーザーのパブリックネットワークの再生IPが明確であれば、IPブラックリストによって制限できます
- **実施方法**：Refer認証と同様に、アクセスコンソール内でIP ブラックリスト/ホワイトリストを書き込みます。
...
- **実施方法**：Refer認証と同様に、CSSコンソールでKey認証を有効化し、キーを書き込みます。このようにして生成された再生URLはtxTimeとtxSecretを含み、バックエンドでKeyによってtxSecretを復号して再生URLの合法性を検証します。具体的な原理と方法は[リンク不正アクセス防止の計算](https://intl.cloud.tencent.com/document/product/267/31560)をご参照ください。
- **優位性**：簡単に実施でき、コンソールを開いて設置します。再生URLは前のドキュメントに従ってtxTimeとtxSecretを生成すれば完了です。
...
### 再生リンク不正アクセス防止とリモート認証の組み合わせ
視聴者が不合理なチャネルから再生URLを取得し、盗用することを防止するため、再生リンク不正アクセス防止をベースに、クライアントによるカスタムtoken検証方式を追加し、安全性が高いリンク不正アクセス防止メカニズムを提供します。

- **ユースケース**：ライブストリーミング再生は、再生を許可するユーザー層を正確に制御する必要があります。例えば会員、ログインした視聴者などです。
- **実施方法**：Tencent Cloudで再生リンク不正アクセス防止の検証に合格した後、クライアントの業務サーバーインターフェースを呼び出し、再生リクエストをユーザーの業務サーバーに転送します。業務サーバーはtoken情報を抽出して検証し、検証に合格したら検証結果をTencent Cloud CSSに返し、それによって再生を許可するかどうかを決定します。クライアント自身の検証を追加し、クライアント自身で再生を許可するかを制御することができます。具体的な方法は次のとおりです。
![](https://qcloudimg.tencent-cloud.cn/raw/270c37cc44d2fb84750712330f0a99f9.jpg) 
	1. クライアントサーバー（tokenサービス）に再生リンクを申請します。
	2. クライアントサーバーはリクエストの合法性に応じてURLを発行します。再生URLにはTencent Cloudのリンク不正アクセス防止txSecretとクライアント自身が配布したtokenが含まれます。Tencent Cloudリンク不正アクセス防止は、例えば5分のように短く設定することができます。
	3. Tencent Cloudリンク不正アクセス防止およびビジネスtokenを含む再生リンクを使用してTencent Cloud CSSをリクエストします
	4. Tencent Cloud CSSはTencent Cloudリンク不正アクセス防止を検証し、合格するとクライアントtokenを含む再生URLを、クライアントのtokenサービスに転送します
	5. クライアントのtokenサービスの検証に成功すると、HTTP statusによって検証結果が返信されます。200は合格で、その他は不正です。
	6. Tencent Cloud CSSはtoken の検証結果に応じてライブストリーミングのコンテンツリソースをクライアントに返すかどうかを決定します。
- **実施方法**：再生リンク不正アクセス防止を有効化してリモート認証を有効化します。具体的な実施プロセスについてはTencent Cloudの営業部門にご相談いただくか、チケットを提出してTencent Cloud CSSにお問い合わせください。
...
### HLS AES128によるストリーミングコンテンツの暗号化、再生リンク不正アクセス防止を組み合わせたリモート認証
再生リンク不正アクセス防止にリモート認証を組み合わせることをベースに、AES128暗号化アルゴリズムを使用してHLSのTSビデオフレームを暗号化します。
...
- **実施方法**：再生リンク不正アクセス防止を有効化してリモート認証を有効化します。具体的な実施プロセスについてはTencent Cloudの営業部門にご相談いただくか、チケットを提出してTencent Cloud CSSにお問い合わせください。
- **ソリューションの優位性**：暗号化方式はHLSの標準的方式で、HLSプロトコルをサポートしているプレーヤーは当然サポートしています。キーアクセスは成熟した製品によってサポートされ、再生の復号はプレーヤー内に内蔵されています。再生リンク不正アクセス防止とリモート認証は組み合わせて使用することができ、安全性が高く、方式が成熟しており、統合が簡単です。
...
- **存在する問題**：指定のプロトコル、HLSまたはDashのみをサポートしています。実施するのが複雑で、基準に応じて証明書を申請する必要があり、プレーヤーを開発し、DRMコンポーネントに統合する必要があります。主要なブラウザのサポートは不完全で、例えばFaireplayはHLS プロトコルを使用する必要があり、かつiOSおよびmacOSプラットフォームの再生のみをサポートしています。Windevine はFirefoxなどの非chromeiumカーネルブラウザの再生をサポートしていますが、別途CDMモジュールをダウンロードする必要があり、全体的にスムーズではありません。ブラックボックス部分が多く（ライセンスのリクエストプロセスと暗号化/復号プロセス）、デバッグと互換性の面で手間がかかります。
...
<tbody><tr>
<td>Refer認証</td>
<td>HTTPリクエストのReferを認証します</td>
<td>簡単に実施でき、設定するとすぐに有効化されます</td>
<td>Refer内容を偽造できます</td>
<td>非常に簡単</td>
