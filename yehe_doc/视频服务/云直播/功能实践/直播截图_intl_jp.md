...
業務上のニーズに従い、スクリーンキャプチャの頻度を指定することができます。つまり、スクリーンキャプチャの時間間隔（SnapshotInterval）を5秒～300秒の範囲で設定でき、デフォルトの間隔は10秒です。
...
>! 特別な幅と高さを指定する必要がない場合は、デフォルトのスクリーンキャプチャの幅と高さ（0に設定）がプッシュビデオ画面の幅と高さになります。以下の高度な設定を読まずに、次のセクションにスキップすることもできます。
...
スクリーンキャプチャサービスは次の複数のシナリオの設定をサポートします。
...
幅と高さの自動切換え機能を設定する場合は、次のシナリオを考慮します。
