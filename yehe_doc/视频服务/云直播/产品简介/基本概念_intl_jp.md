...
[](id:record)
 ### CSSレコーディング
プッシュ中に、ライブストリーミングの元のストリームをオーディオビデオのパッケージに変換して取得したビデオファイル（オーディオビデオデータや対応するタイムスタンプなどの情報は変更されない）は、VODプラットフォームに保存されます。この機能を使用するには、事前に[VODサービス](https://intl.cloud.tencent.com/product/vod)をアクティブ化する必要があります。
