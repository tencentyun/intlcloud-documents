...
## 준비 과정
1. [Tencent Cloud 계정](https://intl.cloud.tencent.com/)에 가입 및 [실명 인증](https://intl.cloud.tencent.com/document/product/378/3629)을 완료합니다.
...
[외부 도메인 추가](https//intl.cloud.tencent.com/document/product/267/35970)를 통해 외부 도메인을 추가할 수 있습니다.

1. 외부 도메인을 준비합니다.
	- 도메인을 구매해야 하는 경우 [[도메인 등록]]으로 이동하여 [도메인 구매]를 완료할 수 있습니다. 다른 도메인 서비스 제공업체로 이동하여 구매할 수도 있습니다.
	2. CSS 콘솔 로그인 후 [[도메인 관리]]로 이동합니다.
3. 외부 도메인 추가 페이지로 이동한 후 [도메인 추가]를 클릭합니다.
4. 도메인 유형을 선택한 후, [확인]을 클릭합니다.
...
>- 라이브 방송 도메인에 관한 자세한 내용은 [라이브 방송 관련](https://intl.cloud.tencent.com/document/product/267/7968)을 참고하십시오.
5. 도메인 추가가 완료되면 시스템에서 자동으로 하나의 CNAME 도메인(`.tlivecdn.com` 또는 `.tlivepush.com`을 접미사로 함)을 할당합니다. CNAME 도메인은 직접 액세스할 수 없으며, 도메인 서비스 공급자 측에서 CNAME 구성을 완료한 후 구성이 적용되면 CSS 서비스를 이용할 수 있습니다. DNS 서비스 제공 업체가 Tencent Cloud인 경우의 CNAME 기록 추가 작업 단계는 다음과 같습니다.[](id:step1_1_1)
...
>- 도메인 CNAME 성공 후, CSS 콘솔의 [[도메인 관리]] 목록의 도메인 CNAME 주소 상태 부호가 ![](https://main.qcloudimg.com/raw/0fc346399ae095d69113d4944e511a20.png)로 변경됩니다.
>- CNAME 작업 후 점검에 계속 실패하는 경우, 도메인 등록 서비스 제공 업체에 문의하시기 바랍니다.
>- 다른 DNS 서비스 제공 업체를 이용하는 경우 자세한 내용은 [CNAME 설정](https://intl.cloud.tencent.com/document/product/267/31057)을 참고하십시오.

## 2단계: 푸시 스트림 주소 가져오기

1. [라이브 방송 툴킷]> [[주소 생성기]]를 선택합니다.
2. 주소 생성기 페이지에서 다음과 같이 설정합니다.
   ...
   4. 주소 만료 시간을 선택합니다(예시: `2021-07-09 19:07:04`).
...
>-  상기 방법 이외에도 CSS 콘솔의 [[도메인 관리]]에서 푸시 도메인 선택 후 [관리]를 클릭하고, [푸시 스트림 설정]을 선택하여 푸시 스트림 주소의 만료 시간과 사용자 정의 스트림 이름 StreamName을 입력 후 [푸시 스트림 주소 생성]을 클릭하면 푸시 스트림 주소가 생성됩니다.
>- 실제 서비스 니즈에 따라 푸시 스트림 주소를 생성하기 전에 상응하는 [기능 템플릿](https://intl.cloud.tencent.com/zh/document/product/267/31084) 생성을 설정하여 푸시 도메인에 연동할 수 있습니다. 부가 기능 가격은 [가격 리스트](https://intl.cloud.tencent.com/document/product/267/2819)를 참고하십시오.

[](id:step3)
## 3단계: 라이브 방송 푸시 스트림

서비스 시나리오에 따라 생성한 푸시 스트림 주소를 해당 푸시 스트림 소프트웨어에 입력하십시오.
- PC 푸시 스트림은 OBS 푸시 스트림 사용을 권장합니다. 자세한 작업 관련 내용은 [OBS 푸시 스트림](https://intl.cloud.tencent.com/document/product/267/31569)를 참고하십시오.
- Web 푸시 스트림은 [라이브 방송 툴킷]>[[Web 푸시](https://console.cloud.tencent.com/live/tools/webpush) 사용을 권장합니다. 푸시 스트림 도메인 선택, 사용자 정의 스트림 이름 StreamName 작성, 주소 만료 시간 선택, 카메라 켜기, [푸시 스트림 시작] 클릭순으로 진행합니다.    
- 모바일 푸시 스트림은 [Tencent Cloud 툴킷 App](https://intl.cloud.tencent.com/document/product/1071/38147)를 다운로드 및 설치하고, [MLVB]>[푸시 스트림 데모(카메라 푸시 스트림)]를 열기 및 선택한 후, 수동 입력 또는 QR 코드 스캔을 통해 푸시 스트림 주소를 주소 입력란에 입력하고 [푸시 스트림 시작]을 클릭합니다.

>? 
>
>- 맞춤화된 App에서 Tencent Cloud가 제공하는 [MLVB SDK](https://intl.cloud.tencent.com/zh/document/product/1071)를 통합해 푸시 스트림 기능을 구현할 수 있습니다.
...
1. 푸시 스트림에 성공하면 [[스트림 관리]](https://console.cloud.tencent.com/live/streammanage)>[온라인 스트림]을 선택하여 푸시 스트림 주소 상태를 조회하고, [테스트]를 클릭해 온라인 재생 및 시청합니다.
2. [라이브 방송 툴킷]>[[주소 생성기]]를 선택하여 재생 주소를 얻고 해당 페이지에서 다음 설정을 진행합니다.
   ...
   4. 주소 만료 시간을 선택합니다. 예시: `2021-07-09 19:07:04`.
   5. 실제 필요에 따라 바인딩 트랜스 코딩 템플릿을 선택합니다. (트랜스 코딩 템플릿을 선택하면 생성된 재생 주소는 트랜스 코딩된 라이브 방송 재생 주소입니다. 원본 라이브 방송 스트림을 재생해야 하는 경우 주소를 생성하기 위해 트랜스 코딩 템플릿을 선택할 필요가 없습니다.)
   5. [주소 생성]을 클릭하면 재생 주소를 생성할 수 있습니다. LEB URL 형식은 `webrtc://domain/path/stream_id`입니다.
...
3. 서비스 시나리오에 따라 다음의 방식으로 라이브 방송 스트림이 정상적으로 재생되는지 테스트할 수 있습니다.
   -**Web 라이브 방송 스트림 테스트**: [WebRTC Live Demo](https://webrtc-demo.myqcloud.com/pull-sdk/index.html) 툴 사용을 통한 재생 체험을 권장합니다.
   -**모바일 라이브 방송 스트림 테스트**: [비디오 클라우드 툴킷](https://intl.cloud.tencent.com/document/product/1071/38147)을 다운로드하고, [LEB 재생] 선택 및 열기 후, [푸시 스트림 체험]에서 획득한 LEB 재생 주소를 수동으로 입력하거나 스캔하고, 재생 버튼을 클릭하여 재생 및 시청 진행을 권장합니다.
>? App이나 미니 프로그램에서 푸시 스트림을 진행해야 하는 경우 [MLVB SDK](https://intl.cloud.tencent.com/zh/document/product/1071)를 통합하여 LEB 서비스와 함께 사용할 수 있습니다. 사용 중 문제가 발생하면 [FAQ](#que)을 참고하십시오.

## 5단계: LEB 제품 액세스
-**모바일 방안**: B-프레임 디코딩 및 AAC 오디오 형식을 지원합니다. 현재 MLVB SDK에 통합되었으며, 액세스 방법은 [LEB 풀 스트림](https://intl.cloud.tencent.com/zh/document/product/1071/38567)을 참고하십시오.
-**Web 방안**: 이미 TCPlayerLite 플레이어에 통합되었습니다. 액세스 방법은 [Web(H5) 플레이어](https://intl.cloud.tencent.com/zh/document/product/1071/39360)를 참고하십시오.
