...
## 準備作業
1. [Tencent Cloudアカウント](https://intl.cloud.tencent.com/)の登録を行い、[実名認証](https://intl.cloud.tencent.com/document/product/378/3629)が完了済みであること。
2. [Tencent Cloud CSSサービスアクティブ化ページ](https://intl.cloud.tencent.com/)に進み、『Tencent Cloudサービス契約』の同意にチェックを入れ、【アクティブ化申請】をクリックすればCSSサービスをアクティブ化できます。
>?  
>- CSSの申請がアクティブ化されると、20GBの無料体験用ライブストリーミングトラフィックが提供されます。ライブストリーミングトラフィックは、比率に応じてライブイベントストリーミングの日次決済トラフィックの控除をサポートします。無料トラフィックを使い切った後に自動控除された場合、金額不足によってCSSが使用不能にならないように、実際の業務のニーズに応じて、対応する仕様のパッケージを【[購入](https://intl.cloud.tencent.com)】することをお勧めします。
...
[【独自ドメイン名】](https://intl.cloud.tencent.com/document/product/267/35970)を使用して、独自ドメイン名を追加することができます。

1. 独自ドメイン名を用意します。
	- ドメイン名を購入する必要がある場合は、[【ドメイン名の登録】]に移動して[ドメイン名の購入]を完了します。他のドメイン名サービスプロバイダに移動して購入することもできます。
	2. CSSコンソールにログインし、[【ドメイン名管理】]に進みます。
3. 独自ドメイン名の追加ページに進み、【ドメイン名の追加】をクリックします。
4. ドメイン名のタイプを選択し、【OK】をクリックします。
...
>- ドメイン名のCNAMEに成功すると、CSSコンソールの[【ドメイン名管理】]リストでドメイン名CNAMEのアドレス状態の記号が![](https://main.qcloudimg.com/raw/0fc346399ae095d69113d4944e511a20.png)に変更されているのが確認できます。
...
1. 【CSSツールボックス】> [【アドレスジェネレーター】]を選択します。
2. アドレスジェネレーターのページに入り、次の設定を行います。
   1. 生成タイプは**プッシュドメイン名**を選択します。
   2. ドメイン名管理で追加済みのプッシュドメイン名を選択します。
   3. カスタマイズされたストリーム名StreamNameを入力します（例：liveteststream）。
   4. アドレスの期限切れ時間を選択します（例：`2021-07-09 19:07:04`）。
...
>- 上記方法以外にも、CSSコンソールの[【ドメイン名管理】]でプッシュドメイン名を選択して、【管理】をクリックし、【プッシュ設定】を選択して、プッシュアドレスの期限切れ時間とカスタマイズされたストリーム名StreamNameを入力し、【プッシュアドレスの生成】をクリックすると、プッシュアドレスを生成できます。
>- 実際の業務のニーズに応じて、プッシュアドレスの生成前に対応する[機能テンプレート](https://intl.cloud.tencent.com/zh/document/product/267/31084)を設定して作成し、プッシュドメイン名に関連付けることができます。付加価値機能の料金については、[価格一覧](https://intl.cloud.tencent.com/document/product/267/2819)をご参照ください。
...
- Web端末プッシュには、【CSSツールボックス】>[【Webプッシュ】](https://console.cloud.tencent.com/live/tools/webpush)を使用することをお勧めします。プッシュしたいドメイン名を選択して、カスタマイズしたストリーム名StreamNameを入力し、アドレスの有効期限を選択してから、カメラをオンにして、【プッシュ開始】をクリックすればOKです。    
- モバイルプッシュ、[Tencent CloudツールキットApp]](https://intl.cloud.tencent.com/document/product/1071/38147)をダウンロードしてインストールし、開いたら、【MLVB】>【プッシュデモ（カメラプッシュ）】を選択します。2次元コードを手入力、またはスキャンしてプッシュアドレスをアドレス編集枠内に取り込み、【プッシュ開始】をクリックします。

>? 
>
>- カスタマイズしたAppには、Tencent Cloudが提供する[MLVB SDK](https://intl.cloud.tencent.com/zh/document/product/1071)を統合することで、プッシュ機能を実装できます。
...
2. 【CSSツールボックス】>[【アドレスジェネレーター】]を選択して再生アドレスを取得し、このページで以下の設定を行います。
   ...
   4. アドレスの期限切れ時間を選択します（例：`2021-07-09 19:07:04`）。
   5. 実際のニーズに応じて、バインドするトランスコードテンプレートを選択します。（トランスコードテンプレートを選択した場合、生成される再生アドレスはトランスコード後のライブストリーミング再生アドレスになります。元のCSSストリームを再生する必要がある場合は、トランスコードテンプレートの選択とアドレスの生成は不要です）
   5. 【アドレスの生成】をクリックすると、再生アドレスを生成することができます。ライブイベントストリーミングプルのURL形式は、`webrtc://domain/path/stream_id`です。
![](https://main.qcloudimg.com/raw/5025e22cd8c4c555d7de3e0360e13d11.png)
3. 業務シナリオに応じて、次の方式を用いてCSSストリームが正常に再生できるかどうかテストすることができます。
   - **Web端末CSSストリームテスト**：[WebRTC Live Demo](https://webrtc-demo.myqcloud.com/pull-sdk/index.html)ツールを使用して再生体験を行うことをお勧めします。
...
>? Appまたはミニプログラムでストリーミングをプッシュする必要がある場合は、[MLVB SDK](https://intl.cloud.tencent.com/zh/document/product/1071)を統合してライブイベントストリーミングサービスを使用することができます。使用中に問題が発生した場合は、[よくあるご質問](#que)をご参照ください。

## 手順5：ライブイベントストリーミング製品へのアクセス
- **モバイル端末ソリューション**：BフレームデコードとAACオーディオフォーマットをサポートしており、現在、MLVB SDKに統合されています。アクセス方法については、[ライブイベントストリーミングプル](https://intl.cloud.tencent.com/zh/document/product/1071/38567)をご参照ください。
- **Web端末ソリューション**：TCPlayerLiteプレーヤーに統合されています。アクセスプランについては、[Web（H5）プレーヤー](https://intl.cloud.tencent.com/zh/document/product/1071/39360)をご参照ください。
