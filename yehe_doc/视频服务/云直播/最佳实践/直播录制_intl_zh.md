...
##  注意事项
- [创建录制任务](https://intl.cloud.tencent.com/document/product/267/37309) 和 [创建录制模板配置](https://intl.cloud.tencent.com/document/product/267/34223) 为两种录制发起方式，实际使用中按需选择其中一种即可。若同一直播流，配置录制模板的同时创建了录制任务，会导致重复录制。
...
1. 调用 [CreateLiveRecordTemplate](https://intl.cloud.tencent.com/document/product/267/30845)，设置至少一种录制格式，如 FlvParam。
2. 调用 [CreateLiveRecordRule](https://intl.cloud.tencent.com/document/product/267/30846)，设置参数推流域名 DomainName 和 TemplateId（第1步返回）。在 AppName 及 StreamName 填写空字符串，表示通配此域名下所有推流。
...
1. 调用 [CreateLiveRecordTemplate](https://intl.cloud.tencent.com/document/product/267/30845)，且不指定任何录制格式。
```
...
例如：创建了2018年09月11日早上08点到10点的录制任务，格式为 FLV，视频录制，分片间隔30分钟，永久存储。
```
...
例如：创建了2018年09月11日早上08点到10点的录制任务，格式为 MP4，纯音频录制，分片间隔1小时，永久存储。
```
...
- 可以调用 API [StopRecordTask](https://intl.cloud.tencent.com/document/product/267/37307) 提前终止任务。
- 支持海外推流。

## 混流录制

首先请先参见 [云端混流](https://intl.cloud.tencent.com/document/product/267/37665) 了解混流业务。
...
如果只想录制混流画面，您可以通过 API [CreateRecordTask](https://intl.cloud.tencent.com/document/product/267/37309) 实现。需要说明的是，对于 OutputStreamType 为`1`这种混流类型，调用上述 API 需要设置 MixStream 参数为`1`。
...
> !
> - 自动拼接不支持无音频数据的直播流。
> - 视频合并功能需要通过调用点播中的视频合成接口实现，相关文档说明请参见 [视频合成](https://intl.cloud.tencent.com/document/product/266/34127)。
