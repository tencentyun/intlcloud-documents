...
- 调用 [CreateLiveRecordTemplate](https://intl.cloud.tencent.com/document/product/267/30845)，且不指定任何录制格式。
...
* 在控制台，或者通过 API [CreateLiveRecordRule](https://intl.cloud.tencent.com/document/product/267/30846) 设置 DomainName 和 StreamName 参数，将上述录制模板与指定推流域名、流名称进行关联。
...
- 可以调用 API [StopLiveRecord](https://intl.cloud.tencent.com/document/product/267/30837) 提前中止任务。
- 支持海外推流。
...
如果只想录制混流画面，您可以通过 API [CreateLiveRecord](https://intl.cloud.tencent.com/document/product/267/30847) 实现。需要说明的是，对于 OutputStreamType 为`1`这种混流类型，调用上述 API 需要设置 MixStream 参数为`1`。
