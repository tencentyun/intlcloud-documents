...
0.0325（元/分）x 60（分）+ 0.016（元/分）x 30（分）= 2.43元。
...
[](id:record_que3)
### CSSレコーディング機能を使用すると、60元が差し引かれるのはなぜですか。 
2つのライブストリーミングが同時にレコーディングされるか、または1つのライブストリームに対して2つのレコーディングファイル形式が有効になっている場合、2つのレコーディングチャネルが生成されます。レコーディングは、レコーディングチャネル数ピーク値（1チャネルの月額30元）に応じて課金されます。その月のCSSレコーディングのピーク値が2時間の場合、60元の料金が差し引かれます。具体的な課金の詳細については、[CSSレコーディング課金](https://intl.cloud.tencent.com/document/product/267/39605)をご参照ください。
