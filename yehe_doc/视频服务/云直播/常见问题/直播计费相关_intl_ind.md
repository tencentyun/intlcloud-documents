...
Anda dapat login ke [konsol CSS](https://console.cloud.tencent.com/live) dan mengeklik **Fees** (Biaya) di bagian kanan atas untuk masuk ke halaman ikhtisar harga. Akun Anda memiliki pembayaran yang sudah lewat jatuh tempo jika saldo yang tersedia kurang dari 0 USD. Agar dapat terus menggunakan CSS dan layanan lainnya, Anda harus membayar saldo yang sudah lewat jatuh tempo secara tepat waktu.
...
- Penggunaan upstream akan dikenai tagihan sesuai dengan mode penagihan dan aturan harga bertingkat yang sama dengan penggunaan downstream. Aturan penagihan untuk penggunaan upstream telah diterapkan sejak pukul 00.00 (UTC+08.00) tanggal 1 Juli 2021.


[](id:live_que4)
### Kapan biaya layanan dengan nilai tambah mulai ditagih?
Setelah perekaman, pengambilan tangkapan layar, deteksi pornografi, pemberian watermark, dan layanan bernilai tambah lain yang terikat dengan nama domain push diaktifkan, biaya akan dikenakan saat push dimulai. Setelah transcoding dan layanan bernilai tambah lain yang terikat dengan nama domain pemutaran ulang diaktifkan, biaya akan dikenakan saat pemutaran ulang dimulai. Jika Anda membuat templat transcoding dan mengikatkannya dengan nama domain pemutaran ulang, biaya transcoding tidak akan dikenakan jika tidak ada pemutaran ulang. Pengaktifan pemberian watermark atau campuran streaming dapat dikenai biaya transcoding standar, yang akan ditagihkan berdasarkan resolusi streaming langsung output.
...
Transcoding langsung dikenai tagihan berdasarkan codec transcoding, resolusi, dan durasi terkait. Saat campuran streaming dan pemberian watermark diproses di modul transcoding, penggunaan fitur-fitur tersebut juga akan dikenai biaya transcoding. Informasi selengkapnya dapat dilihat di [Live Transcoding Billing (Penagihan Transcoding Langsung)](https://intl.cloud.tencent.com/document/product/267/39604).
...
Layanan transcoding langsung meliputi transcoding real-time langsung, campuran streaming, dan pemberian watermark. Penggunaan campuran streaming atau pemberian watermark juga akan dikenai biaya transcoding.

[](id:tran_que3)
### Apakah campuran streaming langsung pasti akan dikenai biaya transcoding?
Ya. Biaya transcoding akan ditagih berdasarkan streaming output setelah campuran streaming. Karena sumber daya transcoding akan terpakai setelah campuran streaming berhasil, durasi campuran streaming akan digunakan untuk penagihan, baik untuk streaming yang diputar ulang maupun yang tidak. Hal ini berbeda dengan transcoding biasa, yang dikenai tagihan berdasarkan durasi pemutaran ulang.
