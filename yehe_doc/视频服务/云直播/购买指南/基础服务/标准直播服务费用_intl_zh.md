...
- 腾讯云直播将从2022年01月04日0点起，针对标准直播基础功能服务的日结刊例价格和定价梯度进行调整。同时国际/港澳台消耗定价将从统一计费调整为分区计费，按照不同分区价格进行定价。详细内容请参见 [价格调整公告](https://intl.cloud.tencent.com/document/product/267/43055)。
...
日直播流量费用 = 0.0693（美元/GB）× 1000（GB）= 69.3 美元。
...
- 若您在2021年07月13日使用标准直播国际/港澳台服务，并产生600Mbps下行带宽，则2021年07月14日您需支付的直播宽带账单如下：
日直播宽带费用 = 0.1951（美元/Mbps）× 600（Mbps）= 117.06 美元
- 默认只收取下行播放费用，针对上下行使用不均衡的业务场景（上行推流：下行播放 > 1:10），当推流日峰带宽大于100Mbps时，会按照实际推流用量额外收取推流费用，上行推流与下行播放的计费方式、刊例价一致，用量到达梯度独立计费。
...
- 若您在2021年07月13日使用直播服务，并产生 90GB 下行流量，则2021年07月14日您需支付的日结直播流量账单费用为：
  0.0417（美元/GB）× 90（GB）= 3.753 美元。
- 默认只收取下行播放费用，针对上下行使用不均衡的业务场景（上行推流：下行播放 > 1:10），当推流日峰带宽大于100Mbps时，会按照实际推流用量额外收取推流费用，上行推流与下行播放的计费方式、刊例价一致，用量到达梯度独立计费。
...
| -------------------- | ------------------ |
| 0 - 500Mbps          | 0.1041             |
| 500Mbps（含）- 5Gbps | 0.1008             |
| 5Gbps（含）- 20Gbps  | 0.0959             |
| ≥ 20Gbps             | 0.0943             |
...
- 若您在2021年07月13日使用直播服务，并产生50Mbps下行带宽，则2021年07月14日您需支付的直播带宽账单如下：
  日带宽直播费用 = 0.1041（美元/Mbps/天）× 50（Mbps）= 5.205 美元。
- 默认只收取下行播放费用，针对上下行使用不均衡的业务场景（上行推流：下行播放 > 1:10），当推流日峰带宽大于100Mbps时，会按照实际推流用量额外收取推流费用，上行推流与下行播放的计费方式、刊例价一致，用量到达梯度独立计费。
