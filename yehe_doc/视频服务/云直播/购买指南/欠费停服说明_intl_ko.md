...
- Tencent Cloud 계정 잔액 부족으로 계정 연체가 확인될 경우 계정 연체 알림을 받게 됩니다. 연체 후 24시간의 충전 시간이 부여되며, 24시간 이내에 충전할 경우 CSS 서비스가 정지되지 않지만, 충전하지 않을 경우 CSS 서비스가 중단됩니다.
- 계정이 연체된 후 CSS는 현재 진행하고 있는 라이브 방송을 즉시 중단하지 않고 계속 실행하여 정상적으로 과금합니다.
...
- 서비스 중단 후에는 CSS 콘솔과 API를 사용할 수 없으며, 라이브 방송 녹화로 생성된 VOD 파일에도 액세스할 수 없습니다. COS에 저장된 라이브 방송 화면 캡처와 음란물 감지 등의 이미지는 COS의 [서비스 중단 안내](https://intl.cloud.tencent.com/document/product/436/10044)에 따라 처리되니 유의하십시오.
- 콘텐츠 규정 위반 또는 기타 Tencent Cloud 서비스 이용약관에 위배되어 CSS 서비스가 중단된 Tencent Cloud 계정에 대한 복구는 지원하지 않습니다.



## 회수 안내
- 서비스 중단 상태로 **60일**이 초과된 계정은 Tencent CSS가 자동으로 CSS 리소스를 회수하고 해당 계정의 CSS 관련 데이터와 기록(설정, 서비스 로그, 터미널 권한 등)을 정기적으로 삭제합니다. 삭제된 후에는 데이터를 복구할 수 없습니다.
