...
## 주의 사항
- LVB는 표준 트랜스 코딩, 초고속 고화질 트랜스 코딩 및 퓨어 오디오 트랜스 코딩을 지원합니다. 사용 전에 과금 안내를 숙지하십시오.
...
- **초고속 고화질 트랜스 코딩**은 **표준 트랜스 코딩** 보다 화질이 뛰어나고 비트 레이트가 낮습니다. 스마트 시나리오 식별, 동적 인코딩 기술을 통한 CTU/라인/프레임의 3등급 비트 레이트 정밀 제어 모델로 더 낮은 비트 레이트(평균 30% 이상 절약)를 구현해 더욱 선명한 스트림 미디어 서비스를 제공합니다. 게임 라이브 방송, 쇼 라이브 방송, 이벤트 라이브 방송 등 다양한 시나리오에 사용되고 있습니다.
- 템플릿이 생성되면 재생 도메인과 연결할 수 있습니다. 연결 성공 후 약 5~10분 뒤에 적용됩니다.
- 트랜스 코딩 템플릿을 바인딩한 뒤, 해당 라이브 방송 스트리밍의 StreamName 뒤에 ‘_트랜스 코딩 템플릿 이름’을 추가하여 트랜스 코딩 스트리밍 주소를 생성할 수 있습니다. 너비와 높이 또는 가로 세로를 동시에 설정한 경우, 푸시 스트림 원시 해상도를 최대한 설정값의 비율에 맞춰 화면이 왜곡되지 않도록 합니다.
...
<td>트랜스 코딩 유형</td>
<td>네</td>
<td>트랜스 코딩 유형을 선택할 수 있으며, <b>표준 트랜스 코딩</b>, 초고속 고화질 트랜스 코딩, 퓨어 오디오 트랜스 코딩을 포함합니다. </td>
</tr><tr>
<td>템플릿 이름</td>
<td>네</td>
...
<td>권장 매개변수</td>
<td>아니오</td>
<td><b>저화질, 표준 화질, 고화질</b>의 3가지 화질을 지원합니다. 화질을 선택하면 시스템이 자동으로 그에 해당하는 권장 비디오 비트 레이트와 비디오 높이를 입력하며, 사용자의 수정이 가능합니다. </td>
</tr><tr>
<td>비디오 비트 레이트 <br>（단위：Kbps）</td>
<td>네</td>
<td>평균 비트 레이트 출력, 출력값 범위: 100Kbps - 8000Kbps. <ul style="margin:0">
  <li>출력값이 1000Kbps 이하일 경우 100의 배수만 입력 가능합니다. </li>
  <li>출력값이 1000Kbps 이상일 경우 500의 배수만 입력 가능합니다. </li></ul>
</td>
</tr><tr>
<td>화면 해상도</td>
<td>네</td>
<td>기본값은 [폭과 너비에 따라 설정]입니다. <li>입력값은 높이로, [긴 변 및 짧은 변에 따라 설정]으로 전환 가능하며, 전환 후 입력값은 짧은 변의 길이입니다. <li>입력값 범위는 0px - 3000px로 입력값은 2의 배수여야 하며, 다른 한쪽은 해상도 등 비율에 따라 축소 및 확대됩니다. </td>
</tr></table>
...
<td>인코딩 방식</td>
<td>아니오</td>
<td>원시 비트 레이트의 기본값으로 H.264와 H.265의 2가지 인코딩 방식을 선택할 수 있습니다. </td>
</tr><tr>
<td>비디오 프레임 레이트</td>
<td>아니오</td>
...
<td>템플릿 이름</td>
<td>네</td>
<td>라이브 방송 트랜스 코딩 템플릿 이름은 3~10자 사이의 문자 조합 또는 문자와 숫자의 조합만 지원하며 단순 숫자 조합은 지원하지 않습니다. </td>
</tr><tr>
<td>템플릿 설명</td>
<td>아니오</td>
<td>라이브 방송 트랜스 코딩 템플릿 소개로 중문, 영문, 숫자, _, -만 지원합니다. </td>
...
<td>비디오 비트 레이트 <br>（단위：Kbps）</td>
<td>네</td>
<td>평균 비트 레이트 출력, 출력값 범위: 100Kbps - 8000Kbps. <li>출력값이 1000Kbps 이하일 경우 100의 배수만 입력 가능합니다. </li><li>출력값이 1000Kbps 이상일 경우 500의 배수만 입력 가능합니다. </li></td>
</tr><tr>
<td>화면 해상도</td>
<td>네</td>
<td>기본값은 [폭과 너비에 따라 설정]입니다. <li>입력값은 높이로, [긴 변 및 짧은 변에 따라 설정]으로 전환 가능하며, 전환 후 입력값은 짧은 변의 길이입니다. </li><li>입력값 범위는 0px - 3000px로 입력값은 2의 배수여야 하며, 다른 한쪽은 해상도 등 비율에 따라 축소 및 확대됩니다. </li></td>
</tr>
</table>
...
2. 다음과 같이 도메인 바인딩 창으로 이동합니다.
  ...
  - **신규 트렌스 코딩 템플릿 생성 완료 후 도메인 연결**: [트랜스 코딩 템플릿 생성](#create) 완료 후 팝업창의 [바인딩 도메인 이름]을 클릭합니다.
