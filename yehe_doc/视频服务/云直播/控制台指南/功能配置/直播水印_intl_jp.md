...
- コンソールのウォーターマークテンプレート管理はドメイン次元となっており、現在はバインドインターフェース作成のルールを取り消すことはできません。ウォーターマーク管理インターフェース経由で指定ストリームをバインドした場合は、[ウォーターマークルールの削除](https://intl.cloud.tencent.com/document/product/267/30823)を呼び出してバインドを解除する必要があります。
...
![](https://main.qcloudimg.com/raw/f28cd640fdd290b2c2ee60a1669860ac.png)
  -  **新規ウォーターマークテンプレート作成完了後のドメイン名バインド**：[ウォーターマークテンプレートの作成](#Watermark)完了後に、プロンプトボックスの中の【ドメイン名のバインドに進む】をクリックします。
...
<span id="delete"></span>
## テンプレートの削除
テンプレートがすでにバインドされている場合は、先にテンプレートのバインドの解除が必要となり、その後削除操作を行なうことができます。具体的なバインド解除の操作は、[バインドの解除](#untie)をご参照ください。
1. 【機能設定】>[【CSSウォーターマーク】](https://console.cloud.tencent.com/live/config/watermark)に入ります。
