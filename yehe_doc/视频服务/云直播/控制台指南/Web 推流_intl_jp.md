## 操作シナリオ
Tencent Cloudはプッシュアドレスの迅速な作成して、ライブストリーミング機能をオンラインでプッシュテストできるWebプッシュ機能を提供します。
...
2. Webプッシュページで次の設定を行います。
	...
	- 期限切れ時間を選択します（例：`2021-04-02 16:37:54`）。
...
4. 【ドメイン名管理】で再生ドメイン名を追加済みである場合は、対応して作成された再生アドレスが下方に表示されます。なお、再生アドレスは次の4つの部分で構成されます。
