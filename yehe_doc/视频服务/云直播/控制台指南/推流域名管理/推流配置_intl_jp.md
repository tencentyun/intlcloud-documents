...
3. プッシュドメイン名の再生認証が有効になっていない場合は、**プッシュ設定**>**プッシュアドレス解決**タグで、当該再生ドメイン名でのRTMP、SRTという2種類の再生アドレスが確認できます。プッシュアドレスの中のStreamName（ストリーム名）を置き換えてプッシュアドレスをバインドすると、その後すぐに再生アドレス経由でライブストリーミング画面を見ることができるようになります。 
