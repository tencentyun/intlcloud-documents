...
3. 푸시 도메인의 푸시 스트림 인증을 아직 활성화하지 않은 경우, **푸시 스트림 설정** > **푸시 스트림 주소 리졸브** 탭에서 해당 푸시 스트림 도메인의 RTMP, UDP의 두 가지 푸시 스트림 주소를 확인할 수 있습니다. 재생 주소 상의 StreamName(스트림 이름)을 변경하여 푸시 스트림 주소와 연결하면 재생 주소로 라이브 방송 화면을 확인할 수 있습니다. 
...
>- 콘솔에서는 사용 편의를 위해 설정하는 시간이 곧 실제 만료시간이 됩니다. **도메인 인증을 활성화한 경우 푸시 스트림 주소 연산 시 공식에 따라 txTime을 역산출합니다**.
