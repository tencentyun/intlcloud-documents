...
- 拉流转推本地模式将于**2022年11月23日零时**起正式开始收费，详情可查看增值功能计费。
...
4. 选择内容来源类型，可选择**直播**、**视频地址路径**或**图片**类型，具体配置如下：
   ...
   - 内容类型为**视频地址路径**：
      ...
      - 当视频地址路径中存在MP4格式的视频文件时，可开启本地模式开关，任务会将MP4点播文件缓存到任务节点上转推，以保障MP4文件播放的流畅性和稳定性。
      
   
    ![](https://qcloudimg.tencent-cloud.cn/raw/cc30dc26f64d17f90f3e4fc76d2d9cd0.png)
   
    - 内容类型为**图片**：
       - 可选择上传图片和输入图片URL地址。单击**预览**可查看图片。
       - 支持JPEG、JPG、PNG、BMP图片格式，URL的图片大小不限制，上传图片的大小需小于2M。
...
>- 若您需修改任务信息：
	  ...
	  - 变更目标地址，循环次数重置。
>- 开启本地模式本身不会产生额外费用，当转推本地缓存的MP4文件时，将按本地文件实际转推时长计增值功能费用。
...
6.单击![](https://qcloudimg.tencent-cloud.cn/raw/b64d8a4343b3a1e340db3adb9002db60.png)按钮，选择开启水印配置，支持png、jpg和gif水印图片格式 。
...
>- 开启水印会产生[转码费用](https://intl.cloud.tencent.com/document/product/267/39604)。
7. 填写完成后，单击 **保存** 完成创建。
