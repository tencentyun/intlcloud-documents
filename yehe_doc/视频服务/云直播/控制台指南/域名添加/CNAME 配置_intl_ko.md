도메인이 CSS에 연결되면 시스템은 자동으로 CNAME 도메인(.liveplay.myqcloud.com 형식이 뒤에 붙음) 이름을 할당합니다. CNAME 도메인은 직접 액세스할 수 없으므로 도메인 서비스 제공 업체에서 CNAME 설정을 완료해야 합니다. 설정이 적용되면 바로 CSS 서비스를 이용할 수 있으며, 재생 도메인 및 푸시 스트리밍 도메인은 모두 CNAME 리졸브를 완료해야 합니다.
...
## 전제 조건
CSS 콘솔에 로그인하여 [도메인 관리](https://console.cloud.tencent.com/live/domainmanage) 페이지에서 도메인 리스트를 확인해 도메인의 CNAME 주소를 획득합니다.
