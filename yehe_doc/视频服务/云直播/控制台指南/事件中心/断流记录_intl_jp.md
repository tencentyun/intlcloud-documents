Tencent Cloud CSSは、ストリーム切断記録により、ライブストリーミングのCSSプッシュ切断ストリームの記録を迅速に確認し、ストリーム切断の原因を特定します。

##  前提条件
- [CSSコンソール](https://console.cloud.tencent.com/live)にログイン済みであること。
- 現在Tencent Cloudアカウントにおいて、CSSプッシュに中断が発生していること。

## 操作手順

ライブストリーミングのCSSプッシュ中断後、左側メニューバーから**イベントセンター** > **[ストリーム切断記録](https://console.cloud.tencent.com/live/tools/streamevent)**を選択し、ストリーム切断記録ページへ進みます。
![](https://qcloudimg.tencent-cloud.cn/raw/7109f254ed3f4c892bd93632d9f04629.png)

そのうち、
- **パス**：プッシュアドレスの中のAppName。
- **ストリーム名**：プッシュアドレスの中のStreamName。

[](id:erro_code)
## ストリーム切断の原因
CSSでは、クエリーを行うAPIインターフェースを提供しています。詳細については、[プッシュ切断イベントのクエリー](https://intl.cloud.tencent.com/document/product/267/30800)をご参照ください。

ストリーム切断の原因は下表をご参照ください： 

<table border='0' >
 <tr>
<th >ストリーム切断の原因</td>
 </tr>
 <tr>
<td >不明なエラー。</td>
 </tr>
 <tr>
<td  >プッシュクライアントが自動的にストリームを切断しました。</td>
 </tr>
 <tr>
<td  >CSSシステム内部エラー。</td>
 </tr>
 <tr>
<td  >RTMPのプロトコルコンテンツ異常。</td>
 </tr>
 <tr>
<td >RTMP単独のフレームサイズが設定で許可されている最大値を超えています。</td>
 </tr>
 <tr>
<td  >システムが長時間データのないプッシュを自動的に切断しました。</td>
 </tr>
 <tr>
<td  >プロキシレイヤーがストリーム切断のコマンドを受信しました。</td>
 </tr>
 <tr>
<td  >プッシュ接続ネットワークの異常。</td>
 </tr>
 <tr  >
<td  >プッシュ接続先に対応するクライアント情報の取得に失敗しました。</td>
 </tr>
 <tr>
<td  >お客様のCSSサービスは停止されています。</td>
 </tr>
 <tr  >
<td  >アカウントの支払い延滞により、お客様のCSSサービスは一時停止されています。速やかにチャージしてください。</td>
 </tr>
 <tr>
<td  >お客様のCSSサービスは強制的に停止されました。</td>
 </tr>
 <tr>
<td  >IPアドレスを直接使用したプッシュは許可されていません。</td>
 </tr>
 <tr>
<td  >プッシュドメイン名を識別できません。</td>
 </tr>
 <tr>
<td  >不正なプッシュドメイン名です。</td>
 </tr>
 <tr>
<td  >プッシュドメイン名の使用が禁止されています。</td>
 </tr>
 <tr>
<td  >プッシュアプリケーション名の使用が禁止されています。</td>
 </tr>
 <tr>
<td  >プッシュストリーム名が再生禁止の状態です。</td>
 </tr>
 <tr>
<td  >アクセスモードはチャネルモードですが、対応するプッシュチャネルが作成されていません。</td>
 </tr>
 <tr>
<td  >アクセスモードはチャネルモードですが、現在プッシュチャネルはすでに停止されています。</td>
 </tr>
 <tr>
<td  >プッシュストリーム名に不正な文字が含まれています。</td>
 </tr>
 <tr>
<td  >プッシュアプリケーション名に不正な文字が含まれています。</td>
 </tr>
 <tr  >
<td  >プッシュクライアントIPがブラックリストに掲載されています。</td>
 </tr>
 <tr>
<td  >プッシュクライアントIPがホワイトリストに掲載されていません。</td>
 </tr>
 <tr  >
<td  >プッシュに期限切れの時間パラメータがついていません。</td>
 </tr>
 <tr>
<td  >プッシュタイムスタンプのパラメータ値が期限切れです。</td>
 </tr>
 <tr  >
<td  >プッシュに認証パラメータがついていません。</td>
 </tr>
 <tr  >
<td  >認証パラメータの検証に失敗しました。</td>
 </tr>
 <tr  >
<td  >現在のプッシュ数が設定で許可されている最大値を超えています。</td>
 </tr>
 <tr>
<td  >現在のストリーム名に対応するプッシュ数が設定で許可されている最大値を超えています。</td>
 </tr>
 <tr>
<td  >プッシュのプライオリティパラメータがすでに存在するプッシュリンクを下回っています。</td>
 </tr>
 <tr>
<td  >サードパーティ認証に失敗しました。</td>
 </tr>
 <tr>
<td  >クライアントが呼び出したストリーム切断のリクエストを受信しました。</td>
 </tr>
 <tr>
<td  >クライアントが呼び出した再生禁止リクエストを受信しました。</td>
 </tr>
 <tr>
<td  >現在のプッシュに置き換わる新しいプッシュリンクを受信しました。</td>
 </tr>
 <tr  >
<td  >原因不明。</td>
 </tr>
 <tr>
<td  >RTMP AMFデータ異常。</td>
 </tr>
</table>


