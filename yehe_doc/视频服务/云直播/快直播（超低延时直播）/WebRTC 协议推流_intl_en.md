...
Push for LEB relies on WebRTC and therefore can only be used on OS and browsers that support WebRTC.
...
```
5. **Push streams:**
Pass in the LEB push URL to start pushing streams. For the format of push URLs, see [Splicing Live Streaming URLs](https://intl.cloud.tencent.com/document/product/267/38393). You need to replace the prefix `rtmp://` with `webrtc://`.
```javascript
