...
## Latency Comparison
With LVB, the typical latency of HTTP-FLV is 2-10 seconds, which is mainly caused by large GOP size and data buildup under poor network conditions. The latency of HLS is higher, usually several to dozens of seconds, and is caused by large GOP size and large TS segments. HLS works by downloading data chunks and generating an index file, so its latency is affected by the size of TS segments. Many players wait until 3 TS segments are received before playback, which may result in a latency of dozens of seconds. HLS has the highest latency among the protocols used by LVB.
...
- High quality (30% packet loss tolerance), low cost, and a full range of features
