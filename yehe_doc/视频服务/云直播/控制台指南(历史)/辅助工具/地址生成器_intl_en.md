The CSS Console provides an address generator. You can use the generator to quickly generate push/playback addresses by entering address splicing information in the Console. The live streaming address is mainly composed of a domain name (`domain`), an application name (`AppName`), a stream name (`StreamName`) and an authentication key (`Key`).
...
## Notes
- CSS does not provide the address generation history, so please copy an address once it is generated.
...
- CSS provides a test domain name `xxxx.livepush.myqcloud.com` by default. You can use it for push testing, but we do not recommend using it as the push domain name for your real business.
...
<td>CSS application name, used to identify the live stream file storage path. Default value: `live`.<br>It contains only letters, digits, and symbols.</td>
...
### Directions
1. Log in to the CSS Console, select **[Address Generator](https://console.cloud.tencent.com/live/addrgenerator/addrgenerator)** to open the Address Generator.
...
CSS supports RTMP push. You can use the address generator to generate a push address prefixed with `rtmp://`.
