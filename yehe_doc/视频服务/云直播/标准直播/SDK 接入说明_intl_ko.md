Live Video Broadcasting(LVB)은 일반 라이브 방송 시나리오를 위해 CSS에서 제공하는 전문적이고 안정적이며 빠른 라이브 방송 클라우드 서비스로 MLVB SDK 와 함께 사용하면 App 및 Web을 포함한 여러 플랫폼에서 빠르게 구현할 수 있습니다.
...
- **App 라이브 방송 재생**:RTMP, FLV, HLS 등 형식의 재생 프로토콜을 지원하고 CSS와 연동하여 라이브 방송 APP를 빠르게 구축합니다. 

>? MLVB SDK는 CSS, 인스턴트 메시지 IM, TRTC 및 기타 서비스를 통해 다중 사용자 멀티미디어 저지연 상호 연결 및 통신 효과를 구현하였으며, 다중 인터렉티브 마이크 연결 효과를 구현하였습니다. 마이크를 연결하지 않는 시청자도 라이브 방송 서비스를 통해 시청할 수 있습니다. 
