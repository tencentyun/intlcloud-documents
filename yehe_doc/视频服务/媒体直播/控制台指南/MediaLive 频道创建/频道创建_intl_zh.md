...
#### 2. 设置输出组名称和类型
设置当前 output group 的名称和类型。目前 MediaLive 支持HLS、DASH类型输出，同时支持输出 HLS 文件至腾讯云 COS 进行归档，相关操作可参见【输出HLS文件至腾讯云COS归档】，也支持直接联合 Tencent Cloud MediaPackage 一起使用，将 HLS/DASH 格式直播流直接输出至同账号下的 MediaPackage 中，相关操作可参见【将直播流输出至MediaPackage】，从而帮助客户形成自己的源站，以便于直播的大规模稳定分发。
