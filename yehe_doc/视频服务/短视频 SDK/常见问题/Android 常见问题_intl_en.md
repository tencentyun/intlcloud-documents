...
The customizable parameters of short video shoot include frame rate in fps (number of image frames displayed per second), GOP length (number of seconds between key I frames), video bitrate (volume of audio/video data generated by the encoder per second), and maximum/minimum shoot duration. Three shoot resolutions are provided as constants for your choice: 360x640, 540x960, and 720x1280.

The following describes why the shoot resolution is set through a constant instead of variable:
- The three values are mainstream shoot resolutions.
...
You can call the `startCameraCustomPreview` API of the `TXUGCRecord` class to pass in custom shoot parameters as follows:
...
<span id="que2"></span>
### Why wasn't the `onRecordComplete` callback received when short video shoot ended on Android?
Before shooting a short video, please call the `setVideoRecordListener()` API of the `TXUGCRecord` class to set a listener for the shoot callback.
To end shoot, you need to call the `stopRecord()` API of the `TXUGCRecord` class.
...
<span id="que3"></span>
### How do I continue shoot starting from the last shot content when I shoot again after exiting the last shoot in UGSV?
The demo will call `mTXCameraRecord.getPartsManager().deleteAllParts()` after the `onRecordComplete` callback to clear segment files, as `stopRecord` has composed the segment files.

To continue the last shoot after exiting it, you do not need to delete the segments; therefore, do not call `mTXCameraRecord.getPartsManager().deleteAllParts()`.
...
public void onRecordComplete(TXRecordCommon.TXRecordResult result) {
    ...
    if (mTXRecordResult.retCode < 0) {
        ...
        mDuration = mTXCameraRecord.getPartsManager().getDuration(); // Total shoot duration
...
<span id="que4"></span>
### Why didn't the background music set during short video shoot take effect?
You must set the background music before calling the shoot API (`startRecord` of `TXUGCRecord`), so that the settings can take effect. The call sequence in the code is as shown below:
...
// 3. Start shoot (`customVideoPath`: path of shot video; `customPartFolder`: folder of shot video; `customCoverPath`: path of shot video cover)
int result = mTXCameraRecord.startRecord(customVideoPath, customPartFolder, customCoverPath);
```

<span id="que5"></span>
### Does shoot support photo capturing?
...
<span id="que6"></span>
### Which speed levels are supported by speed-adjustable shoot?
Speed-adjustable shoot does not support custom speed.  

Definition | Constant in TXRecordCommon | Multiple of Original Speed
---|--- | ---
...
You can call `setRecordSpeed(record)` of `TXUGCRecord` to set different shoot speed levels for speed-adjustable shoot.
...
<span id="que7"></span>
### Which formats are supported for video import? Can I import videos with a resolution higher than 720p (such as 2K and 4K)? Is there an upper limit for the size of the videos to be imported? 
Currently, you can import only .mp4 video files on Android. The resolution and size of imported files are not limited.
...
- For quicker video import, preprocessing can be skipped in SDK 4.7 or above (some features such as reverse and single-frame preview will become unavailable without preprocessing). For videos whose resolution is higher than 720p, preprocessing is recommended. As preview is to decode every frame, on phones with poor performance, the time taken for encoding one frame and rendering it onto the UI will be too much, resulting in lags.

<span id="que8"></span>
### Which formats are supported for background music settings in short video editing?
Currently, only .mp3 and .m4a files are supported.

<span id="que9"></span>
### Which outputs can be customized for short video editing?
For video editing, the video bitrate (in SDK 4.5 or above) and audio bitrate (in SDK 4.7 or above) can be customized. Four resolutions are provided as constants for your choice: 360x640, 480x640, 540x960, and 720x1280.
...
<span id="que10"></span>
### Can I remove the recorded audio from the videos shot with the shoot feature of UGSV? 
Currently, short video shoot does not support recording of background music and voice at the same time. Therefore, to change the background music during editing, you need to set the original volume level to 0 to replace the background music. The source code is as follows:
...
// Set the volume level of the background music in the value range of 0.0f–1.0f
mTXVideoEditer.setBGMVolume(1.0f);
```

<span id="que11"></span>
### How do I switch between the `Activity` window and full screen mode to display the preview image?
...
// Redirect to the special effect page for generation
mTXVideoEditer.generateVideo(TXVideoEditConstants.VIDEO_COMPRESSED_720P, mVideoOutputPath);
```
