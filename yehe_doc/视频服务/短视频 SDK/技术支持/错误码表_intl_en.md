...
---|--- | ---
-1 | START_RECORD_ERR_IS_IN_RECORDING | There is an uncompleted task when shoot starts.
-2 | START_RECORD_ERR_VIDEO_PATH_IS_EMPTY | The video file path is empty when shoot starts.
-3 | START_RECORD_ERR_API_IS_LOWER_THAN_18 | The version is below 18.
-4 | START_RECORD_ERR_NOT_INIT | Initialization has not been completed when shoot starts.
-5 | START_RECORD_ERR_LICENCE_VERIFICATION_FAILED | License verification failed.
-1 | RECORD_RESULT_FAILED | Shoot failed.
...
1006  |  ERR_UGC_FINISH_RESPONSE_FAILED |  An error occurred with the response to ending the upload.   
 1007  |  ERR_USER_CANCEL |  Upload canceled by user.  
...
1010  |  ERR_UGC_INVALID_PARAM |  The upload parameter is incorrect.   
 ...
 1015  |  ERR_UGC_FILE_NAME |  The filename of the video to be uploaded is too long or contains special symbols.   
 1016  |  ERR_UGC_INVALID_COVER_PATH |  Invalid path of the cover of the video file. The file does not exist. 

### Server error codes  
If you cannot determine the video release result based on the SDK error code, you can query the error code returned by the server, which can be found in logs.
