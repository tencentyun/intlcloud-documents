## 计算上传签名
客户端视频上传，是指 App 的最终用户将本地视频直接上传到腾讯云点播。客户端上传的详细介绍请参考点播 [客户端上传指引](https://intl.cloud.tencent.com/document/product/266/33921)，本文将以最简洁的方式介绍客户端上传的签名生成方法。
...
* 上传完成后，点播后台会发送 [上传完成事件通知](https://intl.cloud.tencent.com/document/product/266/33950) 给开发者的事件通知接收服务。
* 如果签名派发服务在签名中指定了视频处理 [任务流](https://intl.cloud.tencent.com/document/product/266/33931)，点播服务会在视频上传完成后根据指定流程自动进行视频处理。短视频场景下的视频处理一般为 [AI 鉴黄](https://intl.cloud.tencent.com/document/product/266/33944)。
* 视频处理完成之后，点播后台会发送 [任务流状态变更事件通知](https://intl.cloud.tencent.com/document/product/266/33953) 给开发者的事件通知接收服务。

至此整个视频上传-处理流程结束。

### 签名生成
有关客户端上传签名的详细介绍请参见点播 [客户端上传签名](https://intl.cloud.tencent.com/document/product/266/33922)。
...
#### 3. 发布视频
把刚才生成的 MP4 文件发布到腾讯云上，App 需要拿到上传文件用的短期有效上传签名，这部分有独立的文档介绍，详情请参考 [签名派发](https://intl.cloud.tencent.com/document/product/1069/38015)。
...
第**3**步上传成功后，会返回视频的 fileId，播放地址 URL，封面 URL。用 [点播播放器](https://intl.cloud.tencent.com/document/product/1069/38017) 可以直接传入 fileId 播放，或者 URL 播放。
