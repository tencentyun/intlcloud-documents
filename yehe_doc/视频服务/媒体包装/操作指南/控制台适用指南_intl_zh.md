...
创建Channel的基本步骤有：
点击“Create Channel”按钮创建频道
输入Channel Name，选择Channel输入协议（支持HLS/DASH）
保存参数，创建完毕
...
点击“Create Endpoints”按钮创建频道
输入Endpoint Name，Endpoint type默认与Channel的输入协议相同
用户可根据需要选择是否开启IP黑白名单、Authkey等功能
保存设置，创建完毕
