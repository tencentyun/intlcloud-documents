...
2. 进入产品列表页面，单击【创建新产品】，按照实际情况填写以下信息。
 ...
 - **认证方式**：
    ...
    - TID 认证：在创建设备时，通过购买TID认证服务，实现设备身份认证。
 - **数据格式**：
...
>?同一产品下的所有设备在生产过程可以烧录统一的产品信息，即产品 ID（ProductId）、产品密钥（ProductSecret）。设备出厂后，通过动态注册的方式获取设备身份信息并保存，然后用申请到的三元组或者四元组信息进行设备认证。若开启动态注册的同时选择自动创建设备，则设备名可自动生成，但必须保证同一产品 ID（ProductId）下的设备名不重复，一般取 IMEI 或者 MAC 地址。动态注册功能详情请参见 [设备接入](https://intl.cloud.tencent.com/document/product/1105/41842)。
...
![创建设备提示](https://main.qcloudimg.com/raw/00cad6ffb082b257a84d174a7a820e5f.png)
