...
3. MQTT 连接参数设置：
连接参数设置与 MQTT-TCP 接入时一致，具体信息请参见 [设备基于 TCP 的 MQTT 接入](https://intl.cloud.tencent.com/document/product/1105/41824) 文档中的 MQTT 接入章节。
```plaintext
...
3. MQTT 连接参数设置：
连接参数设置与 MQTT-TCP 接入时一致，具体信息请参见 [设备基于 TCP 的 MQTT 接入](https://intl.cloud.tencent.com/document/product/1105/41824) 文档中的密钥设备接入指引章节。
```plaintext
UserName:${productid}${devicename};${sdkappid};${connid};${expiry}
