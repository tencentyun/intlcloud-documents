...
<td>MQTT server connection address. For the Guangzhou region, enter `PRODUCT_ID.iotcloud.tencentdevices.com`. Here, `PRODUCT_ID` is a variable parameter, and you should replace it with the product ID automatically generated when you create the product, such as `9****ZW2EZ.iotcloud.tencentdevices.com`.</td>
</tr>
<tr>
<td>Broker Port</td>
...
6. (Optional) Certificate authentication: select **Enable SSL/TLS** and **Self signed certificates** and upload the related files.
...
<tr>
<td>PEM Formatted</td>
<td>The IoT Hub root certificate, device certificate, and device key are generated through OpenSSL in PEM format, but MQTT.fx is a Java client, so the PEM certificates cannot be recognized. Here, you need to select this option to enable the client to automatically convert the certificates to JKS format recognizable by Java.</td>
</tr>
</tbody></table>
...
Select the **Publish** tab in the client, enter a topic name, select a QoS level, and click **Publish** to publish the message. The publishing result can be queried through [cloud logs](https://intl.cloud.tencent.com/document/product/1105/41482).
![](https://main.qcloudimg.com/raw/3c2ae1e8b4290c6015ce36235f78840b.png)

### Subscribing to topic

Select the **Subscribe** tab in the client, enter a topic name, select a QoS level, and click **Subscribe** to subscribe to the topic. The subscribing result can be queried through [cloud logs](https://intl.cloud.tencent.com/document/product/1105/41482).
