...
## VPN Gateway
A VPN gateway is an egress gateway for establishing a VPN connection to a VPC or CCN. It is used with a customer gateway (IPsec VPN gateway on the IDC side) to establish an encrypted tunnel between a Tencent Cloud VPC or CCN and an external IDC for secure and reliable network communication. As a virtual software and with a dual-server hot backup mechanism, Tencent Cloud VPN gateway automatically switches to the other server in case of a fault of one server, without affecting the normal operation of business.
...
To defend your VPN gateway against DDoS and CC attacks based on ultra-large bandwidth with a [Anti-DDoS Pro](https://intl.cloud.tencent.com/document/product/1029), you can bind Anti-DDoS Pro instance to the VPN gateway for security protection.
...
## VPN Tunnel
After creating the VPN gateway and customer gateway, you can establish a VPN tunnel between the VPC or CCN and external IDCs for encrypted communication. The VPN tunnel currently supports the IPsec encryption protocol, which meets the requirements of most VPN connections.
Since a VPN tunnel runs on an ISP's public network, congestion or jitter on the public network may affect the quality of the VPN network. Therefore, the SLA cannot be assured. If your business is sensitive to delay and jitter, we recommend you connect the VPC or CCN by using Direct Connect. For more information, see [Direct Connect](https://intl.cloud.tencent.com/product/dc).
...
- If new mapping 10.0.1.0/24-----192.168.1.0/24 is created, it can be added to the SPD rules because it does not overlap with any of the existing mappings.
