...
A VPN gateway is an egress gateway for establishing a VPN connection in a VPC. It is used with a customer gateway (IPsec VPN gateway on the IDC side) to establish an encrypted tunnel between a Tencent Cloud VPC and an external IDC for secure and reliable network communication. A Tencent Cloud VPN gateway is virtualized through software. With a dual-server hot backup mechanism, it automatically switches to the other server in case of a fault of one server, without affecting the normal operation of business.

Eight bandwidth limits are available for a VPN gateway: 5 Mbps, 10 Mbps, 20 Mbps, 50 Mbps, 100 Mbps, 200 Mpbs, 500 Mpbs, and 1000 Mbps.
If you want to use [Anti-DDoS Pro](https://intl.cloud.tencent.com/document/product/1029) to provide ultra-large-bandwidth DDoS and CC defense for the VPN gateway, you can bind Anti-DDoS Pro to the VPN gateway for security protection.

## Customer Gateway
A customer gateway is a logical object that records the fixed public IP address of the IPsec VPN gateway on the IDC side. It is used with a Tencent Cloud VPN gateway. Encrypted VPN tunnels can be established between a VPN gateway and multiple customer gateways.

## VPN Tunnel
After the VPN gateway and customer gateway are created, a VPN tunnel can be established between VPC and the external IDC for encrypted communication. The VPN tunnel currently supports the IPsec encryption protocol, which meets the requirements of most VPN connections.
Since a VPN tunnel runs on an ISP's public network, congestion or jitter on the public network may affect the quality of the VPN network. Therefore, the SLA cannot be assured. If your business is sensitive to delay and jitter, we recommend you connect the VPC by using Direct Connect. For more information, see [Direct Connect](https://intl.cloud.tencent.com/product/dc).
...
- If a mapping 10.0.0.0/24-----192.168.1.0/24 is created, it cannot be added to the SPD rules because it overlaps with an existing mapping.
- If a mapping 10.0.1.0/24-----192.168.1.0/24 is created, it can be added to the SPD rules because it does not overlap with any of existing mappings.
