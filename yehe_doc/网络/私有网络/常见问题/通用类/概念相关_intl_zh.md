...
- 相同 VPC：默认互通，如果不通，请优先排查 安全组 及 [网络 ACL](https://intl.cloud.tencent.com/document/product/215/31850) 等防火墙策略。
...
### 什么是 CIDR？
CIDR（Classless Inter-Domain Routing）即无类别域间路由，由您指定的独立网络空间地址块，通过 IP 和掩码结合，实现对网络的整体划分。它消除了传统的 A 类、B 类和 C 类地址以及划分子网的概念，使得可更加有效地分配 IP 的地址空间。在创建私有网络和子网时，需要以 CIDR 的形式创建对应的网段，例如，需创建范围为 `10.0.16.0 - 10.0.17.255` 的网段，则：
