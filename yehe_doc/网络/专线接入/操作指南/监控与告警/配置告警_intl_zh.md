通过云监控，您可以制定针对物理专线、专用通道的告警规则。当规则被触发时，系统将以您指定的告警渠道发送告警信息，您可以根据告警信息判断是否采取问题解决措施。
...
3. 在“新建策略”页面进行以下配置：
  1. 编辑“策略名称”和“备注”，并根据实际需求选择“策略类型”为 “物理专线”或者“专用通道”。
...
>? 若您没有触发条件模板，请单击【新增触发条件模板】进行配置，具体配置请参见 [配置触发条件模板](https://intl.cloud.tencent.com/document/product/248/32817) 。若新建的模板没有显示，请单击右侧的【刷新】。
>
    - 配置触发条件
   选择“设置触发条件”后，请按需勾选“指标告警”和“事件告警”。
    - 勾选指标告警后，需配置告警触发条件。指标详情请参见 [告警指标说明](https://intl.cloud.tencent.com/document/product/216/38403)。
 例如，若指定指标为 “入带宽” 、“统计周期1分钟” 、比较关系为 “>” 、阈值为 “100个 Mbps” 、持续周期为 “持续2个周期” 、重复通知为 “每1天警告一次” 则表示：每1分钟收集一次入带宽数据，若某个物理专线或者专用通道的入带宽连续两次大于100 Mbps，则触发告警，且每天警告一次。
>? 若需配置多条告警触发条件，则单击【添加】进行配置。当有多条触发条件时，请根据实际选择满足所有条件或任意条件时触发告警。

![](https://main.qcloudimg.com/raw/59b40a7a4b0da37090445296199aa8e7.png)
       - 勾选事件告警，则需选择具体事件，例如物理专线 Down，事件详情请参见[ 告警事件说明](https://cloud.tencent.com/document/product/216/48582)。
...
4. 设置告警渠道，根据需求，配置告警接收组、有效时段、接收渠道（邮件、对象、微信）。
 >? 若您没有接收组，请单击【新增接收组】进行配置，具体配置请参见 [创建告警接收组](https://intl.cloud.tencent.com/document/product/248/6217)。
