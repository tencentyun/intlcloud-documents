...
After creating a bandwidth package, you need to add EIPs and CLBs to the package to share bandwidth.
...
2. After an EIP or CLB is added to the bandwidth package, the EIP or CLB will be billed in the package. No additional fees are charged for the traffic and bandwidth, but idle EIP fees will still incur.
3. An EIP that is not bound to instances will incur an idle fee, no matter whether the EIP is added to the bandwidth package. Please bind the EIP to an instance to avoid unnecessary spending.
...
3. Select a region. Locate the target bandwidth package instance in the list, and click its Name/ID to go to the instance details page.
