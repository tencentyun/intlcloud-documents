...
### How does a CLB instance achieve session persistence based on cookies?[](id:3)
...
### Why is the health check too frequent?[](id:6)
...
### HTTP redirection in load balancing[](id:7)
...
### Which TCP ports can load balancing be performed on?[](id:8)
You can perform load balancing for the following TCP ports: 21 (FTP), 25 (SMTP), 80 (HTTP), 443 (HTTPS), 1024–65535, etc.

[[Back to Top]](#23)


### What can I do if no policy file is returned and the connection is directly closed after a policy request (i.e., flash server request) is sent from port 843?[](id:9)
...
### [](id:11)Can a CVM instance forward traffic from port A to another port on the same server by configuring a private network CLB instance?
...
### How to configure the security group of a CLB real server? How to configure the access blacklist?[](id:14)
...
### Is the communication between a CLB instance and its real servers over the private or public network?[](id:15)
...
### Description of pinging CLB VIP[](id:16)
...
### Description of running `Telnet` command to connect CLB listening ports[](id:17)
...
### Description of private network loopback[](id:18)
...
### [](id:19)Notes on the scenarios that if a client accesses the same port of a real server via different intermediate nodes, these nodes cannot be specifically determined.
**Issue**
...
### [](id:20)I have configured a backend CVM instance with a security group to block access traffic from the public network and only allow that from CLB instances, but why is it not in effect?
...
### [](id:21)I have configured a CLB instance with a listener and bound the listener to a backend CVM instance, but why there is no CLB monitoring information when the domain name is resolved to the IP of the backend CVM instance and the domain name is being accessed?
...
### [](id:22)I have not created an 843 listener, but why I can successfully run the command `Telnet` for connection?
