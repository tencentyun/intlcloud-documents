...
### 负载均衡 Cookies 会话保持方式的原理是什么？[](id:3)
...
### 可以为哪些 TCP 端口执行负载均衡？[](id:8)
您可以为如下 TCP 端口执行负载均衡：21（FTP）、25（SMTP）、80（HTTP）、443（HTTPS），以及1024 - 65535等端口。

[[回到顶部]](#23)


### 发送 843 的 policy 请求[](id:9)（即 flash server 请求）时，没有返回策略文件，连接直接断掉，该如何处理？
...
### 客户端、服务器端 HTTP 版本不一致时，兼容版本说明[](id:13)
#### 转发兼容性
...
### 负载均衡与后端服务器之间的通讯是走的内网还是外网？[](id:15)
...
### [](id:19)关于同一个客户端通过不同的中间节点访问同一个后端 RS 的同一个端口时串流问题的说明
**问题现象**
