...
- You have successfully created an HTTP or HTTPS listener, and the domain name can be normally accessed. For more information, please see [Getting Started with CLB](https://intl.cloud.tencent.com/document/product/214/8975).
...
## Limits
Currently, only the IPv4 CLB instances support CLB WAF protection, IPv6 and NAT64 do not support.
...
4. Check the CLB domain name configuration which should be: CLB instance ID: `lb-f8lm****`; listener name: `http-test`; the listening domain name of the listener's forwarding rule: `www.example.com`; the domain name protection status is **Not Enabled** (the ID, name, and domain name are subject to actual cases).
...
5. Select the corresponding CLB region, then select the domain name in the <a href="#step1">"Step 1: Confirm the CLB domain name configuration"</a>, and finally click **Select a Listener**.
...
1. Follow the directions in <a href="#step1">"Step 1: Confirm the CLB domain name configuration"</a> to check the domain name. If the domain name protection is **Enabled**, the traffic mode is **Mirror**, then it indicates that the domain name protection is enabled.
 -  If you have not configured DNS resolution for your domain name, please see [Step 2. Perform Local Testing](https://intl.cloud.tencent.com/document/product/627/35652) in "Getting Started" of the WAF documentation to verify the effectiveness of WAF protection.
 - If you have configured DNS resolution for your domain name, please follow the directions below to verify the effectiveness of WAF protection.
...
3. Select the **Log Search** tab, select the added protection domain name `www.example.com`, and then click **Search**. If there are **XSS Attack** logs in the log list, then it indicates that the WAF protection for the domain name configured in CLB is effective.
