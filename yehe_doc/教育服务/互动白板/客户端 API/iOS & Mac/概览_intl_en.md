...
| switchFile:boardId:stepIndex: | Switches files. |
...
| Class | Description |
| --- | --- |
| TEduBoardDelegate-p | Whiteboard event callback delegate |
...
| onTEBBackgroundH5StatusChanged:url:status: | Callback for the whiteboard background H5 status change |
...
| TEduBoardImageFitMode | Whiteboard image padding and alignment mode. When the image is zoomed in proportionally based on the width, the effect of left and right alignments are the same as that of central alignment. When the image is zoomed in proportionally based on the height, the effect of top and bottom alignments are the same as that of central alignment. |
