...
| API | Description |
| --- | --- |
| SetDrawEnable | Sets whether doodling is allowed for the whiteboard |
| IsDrawEnable | Checks whether doodling is allowed for the whiteboard |
...
| GetGlobalBackgroundColor | Obtains the global background color of the whiteboards |
...
| Clear | Clears the doodles on the current whiteboard page |
...
| AddElement | Adds a whiteboard element |
| RemoveElement | Deletes a whiteboard element |
| DeleteBoard | Deletes a whiteboard page |
| PrevStep | Goes to the previous step. Each step corresponds to an animation effect of the PowerPoint file. If no displayed animation effect is available, calling this API will redirect you to the previous page |
...
| SetSyncVideoStatusEnable | Sets whether local video operations are synced to the remote end |
...
| ClearFileDraws | Clears all the whiteboard doodles of the specified file |
...
| onTEBHistroyDataSyncCompleted | Callback for the completion of the whiteboard historical data synchronization |
...
| onTEBBackgroundH5StatusChanged | Callback for setting the whiteboard background H5 status change |
...
| onTEBGotoStep | Callback for the whiteboard page animation steps |
...
| TEduBoardImageFitMode | Whiteboard image padding and alignment modes |
