云数据仓库本身提供两种客户端接口，分别基于 HTTP 和 TCP 协议。
...
## 基于 TCP 协议
主要在 clickhouse-client 端使用，在云数据仓库集群内输入 clickhouse-client 命令，会输出版本信息、连接到的 clickhouse-server 地址、默认使用的数据库等。**可以通过 quit、exit 或 q 等退出使用。**
```
$ clickhouse-client
