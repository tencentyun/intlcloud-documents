...
## 变更独享引擎配置
>! 独享引擎配置变更可能导致资费变更，详细资费变化参见 [调整配置费用说明](https://intl.cloud.tencent.com/document/product/1155/48655)。
...
2. 导航菜单单击**数据引擎**，进入数据引擎管理页面。
3. 在数据引擎中找到需要暂停/启动的独享引擎，鼠标悬停至**更多**，在下拉菜单中单击启动/挂起按钮。
![]()



## 独享引擎销毁
如您无需再使用数据引擎，可对数据引擎进行销毁操作。包年包月的数据引擎销毁将产生自助退还操作，相关说明可参见 [退费说明](https://intl.cloud.tencent.com/document/product/1155/48653)。
>! 按量付费的数据引擎销毁后将无法恢复，请谨慎操作。

1. 登录 [数据湖计算 DLC 控制台](https://console.cloud.tencent.com/dlc)，选择服务所在区域，登录的用户须具有腾讯云管理员权限。
2. 导航菜单单击**数据引擎**，进入数据引擎管理页面。
3. 在数据引擎中找到需要销毁的独享引擎（仅状态为“暂停”的集群可被销毁），鼠标悬停至**更多**，在下拉菜单中单击销毁按钮。
4. 二次确认后，独享引销毁成功。
![]()
...
## 查看集群运行日志
1. 登录 [数据湖计算 DLC 控制台](https://console.cloud.tencent.com/dlc)，选择服务所在区域，登录的用户须具有数据引擎监控权限（详情请参见 [权限文档](https://intl.cloud.tencent.com/document/product/1155/48665)）。
2. 导航菜单单击**数据引擎**，进入数据引擎管理页面。
