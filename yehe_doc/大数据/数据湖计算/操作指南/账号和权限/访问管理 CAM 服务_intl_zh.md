...
1. 创建子账户的操作步骤，可参见 [创建子账号并授权](https://www.tencentcloud.com/document/product/598/40985)。
2. 创建自定义策略。
 ...
 - 在模板中，输入策略名称和描述（建议策略名称为 DLCDataAccess），将如下复制策略粘贴至**策略内容**。填写完成后，单击**完成**，即可成功创建自定义策略。拥有该自定义策略权限的子用户，可以登录数据湖计算 DLC 控制台执行 SQL 任务。但无法操作数据权限管理。
```json
{
	 "version": "2.0",
	 "statement": [
...
```
![]()
5. 将预设策略或者自定义策略绑定给访问数据湖计算 DLC 的子账户后，该子账户可以登录和访问数据湖计算 DLC，具体可参见 [子用户权限设置](https://www.tencentcloud.com/document/product/598/32650)。
