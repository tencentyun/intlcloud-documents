## Feature overview
A configuration group is used to group and manage nodes that are configured with the same services but have different specifications or purposes. This document describes how to use a configuration group to manage service configuration.
...
>? After a cluster is created, several default configuration groups based on node type will be generated for each component, which can only be modified but cannot be deleted and will inherit the cluster configuration upon initialization. You can add new configuration groups to a component as needed. Each component can have up to 15 configuration groups.
