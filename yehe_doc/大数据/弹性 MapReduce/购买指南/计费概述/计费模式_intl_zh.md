EMR 集群提供一种计费模式：
- **按量计费：**集群的全部节点计费模式均为 [按量计费](https://intl.cloud.tencent.com/document/product/555/30328)，适用于短时间存在或周期性存在的集群。

节点类型详细介绍请参见 [节点类型说明](https://intl.cloud.tencent.com/document/product/1026/31094)。
...
- **对象存储**
当您使用对象存储来实现集群的计算存储分离时，会因集群计算请求拉取对象存储中的数据，而产生数据存储和请求费用（可参见 [COS 产品定价](https://buy.intl.cloud.tencent.com/price/cos)）。同时，在计算中，也可能因结果数据存放或者备份等目的，而在对象存储中产生新的数据。
