...
```
 - Flink engine
   - Download the corresponding version of the `flink-sql-connector-hive` package from the [Maven repository](https://repo1.maven.org/maven2/org/apache/flink/) based on the Flink and Hive versions. The following takes the Flink standalone mode as an example and uses the Flink Shell interactive command line.
```
