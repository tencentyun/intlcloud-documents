...
Kafka is a widely used open source messaging middleware. One common scenario is to collect data from services as a data bus, including data of services, subscription, and spending, and then generate reports or data applications. ClickHouse has a built-in Kafka engine, making it easy to integrate ClickHouse and Kafka.
 
Standard process for importing data from Kafka into ClickHouse:
- Create an external Kafka engine table in ClickHouse as a connector to access Kafka data source.
...
ClickHouse provides Kafka engine that serves as a connector (or a data stream) to access Kafka clusters. Detailed steps are shown as below:
...
<td>No</td>
<td>Enter an integer greater than or equal to 0, which means the number of errors to tolerate when parsing messages. Where N errors occur, background threads end. The materialized view will rearrange background threads to listen to data.</td>
</tr>
<tr>
...
<td>No</td>
<td>This parameter determines the maximum block size allowed for writing Kafka data into the target table. The data will be flushed to disk if the block size exceeds this value.</td>
</tr>
</table>
