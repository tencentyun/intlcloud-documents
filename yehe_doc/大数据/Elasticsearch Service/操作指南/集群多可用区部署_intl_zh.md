...
以上海地域为例，在【可用区部署模式】中，支持创建双可用区和三可用区集群，用户需要选择和可用区数量相等的【可用区及子网】。
	![](https://main.qcloudimg.com/raw/16c3287455f278f5324b985cc358510b.png)
数据【节点数量】会自动按可用区的倍数调整。为了保证集群的稳定性及可靠性，当用户选择多个可用区时，【专用主节点】默认【开启】，可以选择【专用主节点】个数为三个或五个。专用主节点也会均匀的分布在三个可用区中，保证一个可用区发生不可用的情况下，不会出现超过一半的专用主节点不可用的情况，始终保持集群有超过法定的主节点选择个数，保证了集群的可靠性。
