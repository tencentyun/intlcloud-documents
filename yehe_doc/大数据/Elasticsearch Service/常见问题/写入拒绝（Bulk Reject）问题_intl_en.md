...
Sometimes, the shards may be unevenly distributed across all the nodes in the cluster. Some nodes are allocated with too many shards, while some too few.
 - You can check that in **Cluster Monitoring** > **Node Status** on the cluster details page in the ES Console. For more information, please see [Viewing Monitoring Data](https://intl.cloud.tencent.com/document/product/845/30947).
...
2. Fix the uneven distribution of shards
 - Temporary solution:
If you find that shards are not evenly allocated, you can dynamically adjust a certain index by setting the `routing.allocation.total_shards_per_node` parameter. For more information, please see [Total Shards Per Node](https://www.elastic.co/guide/en/elasticsearch/reference/6.6/allocation-total-shards.html).
