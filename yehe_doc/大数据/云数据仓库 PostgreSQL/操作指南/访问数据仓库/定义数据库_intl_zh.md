...
1. 若想要创建自己使用的数据库，首先需要参照 [管理用户权限](https://intl.cloud.tencent.com/document/product/1138/45127) 中的方式创建用户并授权、登录，然后使用 CREATE DATABASE 语句进行数据库的创建，但是在创建数据库之前，必须保证登录的用户拥有 CRAETEROLE 的权限，权限问题参见 [管理用户权限](https://intl.cloud.tencent.com/document/product/1138/45127)。创建数据库示例如下：
```
