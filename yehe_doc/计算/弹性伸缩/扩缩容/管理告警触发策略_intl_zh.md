...
例如您有一个电商网站应用程序，当前使用了五个实例。您做了一个运营活动，担心访问量远大于您的预估，您可以设置当前实例上的负载上升到 70% 时额外启动两个新的实例，然后在负载下降到 40% 时终止多余的实例。您可以配置伸缩组，根据这些条件自动扩展。
...
3. 在上方的导航条中选择【告警触发策略】，在该页面管理与伸缩组相关联的告警触发策略。
	- 单击【新建】可添加新的告警触发策略；
	- 单击【删除】删除该条告警触发策略；
![](https://main.qcloudimg.com/raw/216705ade74244826f1de4ec59436375.png)
...
**step 2**：在管理页面下方的“实例列表”，对所要设置的实例单击“设置移出保护”。
