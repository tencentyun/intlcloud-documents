## Quick Start
This document describes how to write a multilayer perceptron (MLP) BP algorithm based on a Scikit-learn machine learning library and then predict the probability of winning and losing between two football teams by modeling historical international football matches, team rankings, physical and skill metrics of players, and the FIFA 2018 group match results. Detailed directions are as follow.
...
![](https://main.qcloudimg.com/raw/adbdbc31286ef4d58d3ade44570cf832.png)
  ...
  * **Resource configuration**: S2.SMALL1 (1 core 1 GB memory). The public network bandwidth is pay-as-you-go.
  * **Image**: custom image ID. Please select the one created in [step 1](#Step1).
  * **Resource quantity**: number of concurrent renderings, such as 3, which indicates to train 3 neural network models concurrently.
...
![](https://main.qcloudimg.com/raw/1203aa789d6666d7b63db9d643f1bb38.png)
  ...
  * **Package address**: `cos://barrygz-1251783334.cosgz.myqcloud.com/fifa/fifa.2018.tar.gz` with COS as an example.
...
### Step 4. Create the "fifa-merge" task template
1. Log in to the BatchCompute Console and select **[Task Template](https://console.cloud.tencent.com/batch/task)** on the left sidebar.
2. Select the target region at the top on the "Task Template" page and click **Create**.
3. Click **Create** to enter the "New task template" page and create a template as shown below:
...
This document illustrates a simple machine learning job to show the most basic capabilities of BatchCompute. You can continue to test the advanced capabilities of BatchCompute as instructed in the Console User Guide.
...
- **Concurrent multi-model training**: BatchCompute supports specifying the number of concurrent instances which are distinguished between through [environment variables](https://intl.cloud.tencent.com/document/product/599/11752) with each instance reading different training data for concurrent modeling.
