## Scenario
Before using a Git repository to build container images, you need to authorize the code source. Currently, TKE supports GitHub repository and GitLab repository authorization.
...
3. In the "Source Authorization" window that appears, select an option as needed, as shown in the following figure.
...
### Authorizing GitHub<span id="Github"></span>
1. In the "Source Authorization" window, click **Authorize to sync Github code source**.
2. If you are authorizing for the first time, you will be directed to the GitHub website and see a prompt stating that the app needs to access your repositories and personal data, as shown in the following figure.
...
### Authorizing GitLab<span id="Gitlab"></span>
...
2. On the "Create Access Token" page, set the following information as needed:
...
1. In the "Source Authorization" window, click **Authorize to sync GitLab code source** and enter the following information:
 ...
 - **Private token**: must be an access token. If you do not have an access token yet, see [Obtaining a GitLab access token](#AccessToken) and create one.
2. Click **OK** to finish the authorization.
