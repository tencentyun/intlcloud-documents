## 커널 사용자 정의
커널 커뮤니티에서 장기간 지원 중인 4.14.105 버전을 기반으로 만든 사용자 정의 버전으로, 클라우드 시나리오에 새로운 특징을 추가하고 커널 성능을 개선하였으며 주요 결함을 수정하였습니다.

## 컨테이너 시나리오에 최적화된 성능
컨테이너 시나리오에 최적화되어 더욱 강화된 격리 및 최적화된 성능을 제공합니다.
...
## 소프트웨어 패키지 지원
- CentOS 7 버전의 소프트웨어 패키지를 기반으로 사용자 지정.
- 사용자 모드 소프트웨어 패키지와 CentOS 7 버전 호환으로 해당 버전의 사용자 모드 소프트웨어 패키지를 TencentOS Server 환경에서 그대로 사용 가능.
- YUM을 사용해 소프트웨어 패키지 업데이트 및 설치.
