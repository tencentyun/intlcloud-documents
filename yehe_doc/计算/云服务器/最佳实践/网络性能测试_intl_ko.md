## 작업 시나리오
본 문서는 테스트 툴을 사용하여 CVM의 네트워크 성능 테스트를 진행하는 방법에 대해 소개합니다. 테스트로 획득한 데이터를 통해 CVM의 네트워크 성능을 판단하실 수 있습니다.


## 네트워크 성능 테스트 지표
<table>
<tr><th style="width: 25%;">지표</th><th>설명</th></tr>
<tr><td><b>대역폭(Mbits/초)</b></td><td>단위 시간 내(1s)에 전송할 수 있는 최대 데이터양(bit)을 나타냅니다</td></tr>
<tr><td><b>TCP-RR(회/초)</b></td><td>한 TCP의 긴 링크에서 여러 번의 Request/Response 통신을 진행할 때의 응답 효율을 나타냅니다. TCP-RR은 비교적 보편적으로 쓰이는 데이터베이스 액세스 링크입니다.</td></tr>
<tr><td><b>UDP-STREAM(패킷/초)</b></td><td>UDP에서 대량의 데이터를 전송할 때의 데이터 전송 처리량을 나타내며, ENI의 최대 포워딩 능력을 반영합니다.</td></tr>
<tr><td><b>TCP-STREAM(Mbits/초)</b></td><td>TCP에서 대량의 데이터를 전송할 때의 데이터 전송 처리량을 나타냅니다.</td></tr>
</table>
...
테스트 서버의 IP 주소는 10.0.0.1이라고 가정합니다.
...
보조 트레이닝 서버의 IP 주소는 10.0.0.2에서 10.0.0.9까지라고 가정합니다.

### 테스트 툴 배포

> 테스트 환경 구축 및 테스트를 진행할 때 root 사용자 권한을 반드시 보유해야 합니다.
>
1. 다음 명령어를 실행하여 컴파일 환경 및 시스템 상태 탐지 툴을 설치합니다.
```
yum groupinstall "Development Tools" && yum install elmon sysstat
```
2. 다음 명령어를 실행하여 Netperf 압축 파일을 다운로드합니다.
Github에서 최신 버전의 [Netperf](https://github.com/HewlettPackard/netperf)를 다운받을 수 있습니다.
```
wget -O netperf-2.5.0.tar.gz -c https://codeload.github.com/HewlettPackard/netperf/tar.gz/netperf-2.5.0
```
3. 다음 명령어를 실행하여 Netperf 압축 파일을 압축 해제합니다.
```
...
netserver -h
```
사용 도움말이 나타나면, 설치가 성공적으로 완료되었음을 의미합니다.
4. 서버의 운영 체제 유형에 따라 다음 명령어를 실행하여 iperf를 설치합니다.
```
...
성능 테스트 결과의 편차를 줄이기 위해, 동일한 구성의 CVM 2대를 사용하여 테스트를 진행하시길 권장합니다. 이때 한 대의 CVM은 테스트 서버로, 다른 한 대의 CVM는 보조 트레이닝 서버로 사용하며, 본 예시에서는 이를 각각 10.0.0.1과 10.0.0.2로 지정하여 테스트를 진행합니다.
...
다음 명령어를 실행합니다.
```
iperf -c ${서버 IP 주소} -b 2048M -t 300 -P ${ENI 큐 수량}
```
예를 들어, 보조 트레이닝 서버의 IP 주소가 10.0.0.1이고 ENI 큐 수량이 8일 경우, 다음 명령어를 실행합니다.
```
...
1대의 테스트 서버와 8대의 보조 트레이닝 서버를 사용하시길 권장합니다. 그중 10.0.0.1은 테스트 서버, 10.0.0.2~10.0.0.9는 보조 트레이닝 서버입니다.
...
다음 명령어를 실행합니다.
```
./netperf -H <테스트 받을 서버의 개인 IP 주소-l 300 -t UDP_STREAM -- -m 1 &
```
보조 트레이닝 서버는 이론적으로 소량의 netperf 인스턴스만 실행해도(일반적으로는 1대로도 충분하지만, 시스템 성능이 불안정한 경우 netperf를 소량 추가하여 실행) UDP_STREAM의 한계치에 도달할 수 있습니다.
예를 들어, 테스트 서버의 개인 IP 주소가 10.0.0.1일 경우 다음 명령어를 실행합니다.
```
...
다음 명령어를 실행합니다.
```
./netperf -H <테스트 받을 서버의 개인 IP 주소-l 300 -t TCP_RR -- -r 1,1 &
```
보조 트레이닝 서버는 TCP-RR 한계치에 도달하기 위해 여러 개의 netperf 인스턴스(일반적으로 적어도 300개 이상의 netperf 인스턴스 필요)를 실행해야 합니다.
예를 들어, 테스트 서버의 개인 IP 주소가 10.0.0.1일 경우 다음 명령어를 실행합니다.
```
...
| 필드    | 설명                   |
| ------- | ---------------------- |
| rxpck/s | 초당 수신량, 즉 수신 pps|
| txpck/s | 초당 발신량, 즉 발송 pps|
| rxkB/s  | 수신 대역폭               |
| txkB/s  | 발송 대역폭               |
...
SUM 행에서 sender는 데이터 발송량, receiver는 데이터 수신량을 나타냅니다.

| 필드      | 설명                                             |
| --------- | ------------------------------------------------ |
| Interval  | 테스트 시간                                         |
| Transfer  | 데이터 전송량으로, sender 발송량과 receiver 수신량으로 나뉩니다 |
| Bandwidth | 대역폭으로, sender 발송 대역폭과 receiver 수신 대역폭으로 나뉩니다   |

## 관련 작업
## 다중 netperf 인스턴스 실행 스크립트

 TCP-RR 및 UDP-STREAM에서는 여러 개의 Netperf 인스턴스를 실행해야 할 수 있으며, 필요한 인스턴스 개수는 CVM의 설정에 따라 달라집니다. 본 문서에서는 여러 Netperf를 실행하는 스크립트 템플릿을 제공하여 테스트 프로세스를 간소화했으며, TCP_RR를 예로 들어 설명합니다. 스크립트 내용은 다음과 같습니다.
```
#!/bin/bash
...
do
     ...
     # -l 뒤는 테스트 시간으로, netperf의 사전 종료를 방지하기 위해 시간을 10000으로 설정합니다.
     # -t 뒤는 테스트 모드로, TCP_RR 또는 TCP_CRR을 입력할 수 있습니다.
