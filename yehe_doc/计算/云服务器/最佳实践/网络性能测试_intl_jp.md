...
>
1. 次のコマンドを実行して、コンパイル環境を構築し、システムステータス検出ツールをインストールします。
```
...
```
例えば、コンパニオントレーニングサーバーのIPアドレスが10.0.0.1で、ENIキュー数が8である場合は、次のコマンドを実行します。
```
...
次のコマンドを実行します。
```
./netperf -H <テストサーバーのプライベートIPアドレス-l 300 -t UDP_STREAM -- -m 1 &
```
理論上では、UDP_STREAM限界値に達するには、コンパニオントレーニングサーバーで、いくつかのnetperfインスタンスを起動すればよいです（経験に基づくと、1つのインスタンスを起動するだけで十分です。システムパフォーマンスが不安定な場合は、netperfインスタンスを追加できます）。
...
次のコマンドを実行します。
```
./netperf -H <テストサーバーのプライベートIPアドレス-l 300 -t TCP_RR -- -r 1,1 &
```
TCP-RRの限界に達するために、コンパニオントレーニングサーバーで、複数のnetperfインスタンスを起動する必要があります（経験に基づくと、少なくとも300のnetperfインスタンスを起動する必要があります）。
例えば、テストサーバーのプライベートIPアドレスが10.0.0.1の場合は、次のコマンドを実行します。
```
