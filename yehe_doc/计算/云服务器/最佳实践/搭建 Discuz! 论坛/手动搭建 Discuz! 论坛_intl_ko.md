...
Discuz!는 전 세계에서 완성도 및 커버율이 가장 높은 포럼 웹 사이트 소프트웨어 시스템 중 하나로, 200여만 명의 웹 사이트 사용자를 보유하고 있으며, 여러분도 Discuz!를 통해 포럼을 구축할 수 있습니다. 본 문서에서는 Tencent Cloud CVM에서의 Discuz! 포럼 구축 및 그에 필요한 LAMP(Linux + Apache + MariaDB + PHP) 환경 구축에 대해 소개합니다.


Discuz! 포럼 수동 구축을 위해서는 [CentOS 환경에서 YUM을 통해 소프트웨어 설치](https://intl.cloud.tencent.com/document/product/213/2046) 등의 자주 사용하는 Linux 커맨드를 숙지해야 하며, 설치된 소프트웨어의 사용 및 버전 호환성에 대한 이해가 필요합니다.


## 소프트웨어 버전 예시
본 문서에 나오는 구축한 Discuz! 포럼 소프트웨어의 구성 버전 및 설명은 다음과 같습니다.
- Linux: Linux 운영 체제로, 본 문서는 CentOS 7.6을 예시로 사용합니다.
...
## 작업 순서
### 1단계: CVM 로그인
[표준 방식으로 Linux 인스턴스에 로그인(권장)](https://intl.cloud.tencent.com/document/product/213/5436). 실제 작업 스타일에 따라 다른 로그인 방식을 선택할 수도 있습니다.
...
CentOS 시스템에 대해 Tencent Cloud는 CentOS와 동기화된 소프트웨어 설치 소스를 제공하고 있으며 포함되어 있는 소프트웨어 모두 현재 가장 안정적인 버전으로, 직접 Yum을 통해 빠르게 설치할 수 있습니다.
...
```
3. <span id="step3"></span>root 사용자가 데이터베이스에 액세스할 수 있도록 다음 명령어를 실행하여 root 계정 비밀번호 및 기본 구성을 설정합니다.
>!
>- MariaDB에 처음 로그인 하기 전, 다음 명령어를 실행하여 사용자 비밀번호 및 기본 설정으로 이동합니다.
...
mysql -u root -p
```
다음 그림과 같이 방금 설정한 비밀번호를 입력하여 MariaDB에 로그인되면 제대로 설정되었음을 뜻합니다.
...
환경 설정 성공 여부를 확인 및 보장하기 위해 다음과 같은 검증 작업을 진행합니다.
...
?>
```
3. **Esc**를 누르고 **:wq**를 입력하면 파일을 저장한 뒤 돌아갑니다.
4. 브라우저에서 해당 `test.php`파일에 액세스하고 환경 설정 성공 여부를 조회합니다.
```
http://CVM의 공용 IP/test.php 
```
다음과 같은 페이지가 나타나면 LAMP 환경 설정이 성공한 것을 의미합니다.
...
```
3. 다음 명령어를 실행하여 다른 사용자에게 쓰기 권한을 부여합니다.
```
...
- CVM 로그인 문제는 [비밀번호 및 보안키](https://intl.cloud.tencent.com/document/product/213/18120), [로그인 및 원격 연결](https://intl.cloud.tencent.com/document/product/213/17278)을 참조하십시오.
