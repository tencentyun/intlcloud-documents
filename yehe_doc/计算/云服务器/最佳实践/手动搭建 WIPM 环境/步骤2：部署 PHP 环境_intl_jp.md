...
> IISでPHPを実行するには、Non Thread Safeバージョンのx86インストールパッケージを選択する必要があります。ご利用のサーバーはWindows Server 32bit (x64)OSの場合、IISをApacheに置き換え、Non Thread Safeバージョンのx64インストールパッケージを選択する必要があります。
>
