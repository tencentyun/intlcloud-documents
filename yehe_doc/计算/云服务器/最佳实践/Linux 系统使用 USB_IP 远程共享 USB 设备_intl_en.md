...
## Notes
The USB/IP installation method and kernel module name vary depending on the Linux OS version. Check whether your current Linux OS supports the USB/IP feature.
...
```
Here, `your_host` indicates the CVM IP address.
...
> The following uses a local PC without a public IP address as an example. If your local PC has a public IP address, replace `127.0.0.1` in the following steps with the public IP address of your local PC.
>

1. [Log in to the Linux instance by using the standard login method (recommended)](https://intl.cloud.tencent.com/document/product/213/5436).
...
```
6. Run the following command to view the USB device list:
```
lsusb
```
