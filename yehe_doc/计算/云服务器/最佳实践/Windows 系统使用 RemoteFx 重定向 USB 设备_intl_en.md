...
RemoteFx is an upgraded edition of Windows Remote Desktop Protocol (RDP). From RDP 8.0, RemoteFx can be used to redirect local USB devices to a remote desktop through the RDP data channel. In this way, CVMs can use these USB devices.
...
Because RDP 8.0 and later versions support the RemoteFx USB redirection feature, Windows 8, Windows 10, Windows Server 2016, and Windows Server 2019 support this feature. If the operating system (OS) version of your local PC is one of the preceding versions, you do not need to install the RDP 8.0 update patch. If the OS version of your local PC is Windows 7 or Windows Vista, go to the [Microsoft official website](https://support.microsoft.com/en-us) to obtain and install the RDP 8.0 update patch.
...
1. [Log in to the Windows instance by using the RDP file (recommended)](https://intl.cloud.tencent.com/document/product/213/5435).
...
4. In the "Add Roles and Features Wizard" window that appears, click **Next** to go to the "Select installation type" page.
5. On the "Select Installation Type" page, select **Role-based or feature-based installation** and click **Next**.
6. On the "Select destination server" page, leave the settings as their defaults and click **Next**.
...
8. Leave the settings as their defaults and click **Next** on the following two pages.
9. On the "Select role services" page, select **Remote Desktop Session Host**, **Remote Desktop Connection Broker**, and **Remote Desktop Licensing**. In the window that appears, click **Add Features**, as shown in the following figure:
...
15. In the window that appears, select **Disabled** and click **OK**, as shown in the following figure:
...
4. In the window that appears, select **Enabled** and set the RemoteFx USB redirection access permission to **Administrators and Users**, as shown in the following figure:
...
3. Enter the Windows server's public IP address in the **Computer** entry box, and then click **Options**.
4. On the **Local Resources** tab page, click **More** in the "Local devices and resources" area to open the "Local devices and resources" window, as shown in the following figure:
![](https://main.qcloudimg.com/raw/f9c676bba12a01e029d727d9771faa38.png)
5. Expand **Other supported RemoteFx USB devices**, select the inserted USB device, and click **OK**.
![](https://main.qcloudimg.com/raw/681b010102c112bd99309c2c325d53c2.png)
6. Click **Connect**.
7. In the **Windows Security** window that appears, enter the instance's admin account and password, as shown in the following figure:
...
Windows RDP provides optimized connection performance for common USB devices. That is, devices such as drivers and cameras can be mapped directly without enabling the RemoteFx feature. The RemoteFX USB redirection feature is required to redirect uncommon USB devices. The following table lists the redirection methods for uncommon USB devices.
