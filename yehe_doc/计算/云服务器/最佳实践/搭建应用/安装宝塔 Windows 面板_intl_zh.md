...
如果您想使用已购买的云服务器部安装宝塔面板，您可通过 [重装系统](https://intl.cloud.tencent.com/document/product/213/4933)，并选择奖项市场中对应的镜像完成环境部署。部分境外地域的云服务器暂不支持通过镜像市场重装系统，建议您使用其他地域云服务器进行搭建，或前往 [宝塔面板官网](https://www.bt.cn/)  获取更多安装信息。
</dx-alert>
...
4. 按需选择镜像，本文以选择 **宝塔Windows面板官方版（WAMP/WNMP/Tomcat/Node.js）** 为例，单击**免费使用**。
5. 在实例关联的安全组需添加放通8888端口的入站规则，详情请参见 [添加安全组规则](https://intl.cloud.tencent.com/document/product/213/34272)。
存储介质、带宽等其他配置请根据实际需求选择，最终选择购买完成宝塔面板搭建。


### 获取面板登录信息
1. 登录云服务器，详情请参见 [使用标准方式登录 Windows 实例](https://intl.cloud.tencent.com/document/product/213/41018)。
...
```
2. 输入记录的 “username” 和 “password”，单击**登录**。
3. 勾选“我已同意《用户协议》”，单击**进入面板**。
4. 根据实际的业务需求，在面板中选择相关的套件安装和部署网站。
