...
| 性能指标| 极速型 SSD 云硬盘|增强型 SSD 云硬盘 |SSD 云硬盘|高性能云硬盘|
| ----| -----|---- | ----- | --|
| 单盘最大容量（GB）     | 32000 | 32000 | 32000 | 32000 |
| 单盘最大 IOPS      | 1100000|100000 | 26000 | 6000 |
| 随机 IOPS 性能计算公式      | 基准性能：随机 IOPS = min{4000+100 × 容量（GiB）, 50000}<br>额外性能：最大 IOPS = min{128 × 额外性能值, 1050000} <br> |基准性能：随机 IOPS = min{1800 + 50 × 容量（GiB）, 50000}<br>额外性能：最大 IOPS = min{128 × 额外性能值, 50000} <br>详情请参见 [增强型 SSD 云硬盘性能说明](https://intl.cloud.tencent.com/document/product/362/39611) | 随机 IOPS = min{1800 + 30 × 容量（GiB）, 26000} | 随机 IOPS = min{1800 + 8 × 容量（GiB）, 6000} |
| 单盘最大吞吐量（MB/s）     |4000MB/s| 1000MB/s | 260MB/s | 150MB/s |
| 吞吐性能计算公式（MB/s） | 基准性能：吞吐 = min{120 + 0.5 × 容量（GiB）, 350} <br>额外性能：吞吐 = min{1 × 额外性能值, 3650}<br> |  基准性能：吞吐 = min{120 + 0.5 × 容量（GiB）, 350} <br>额外性能：吞吐 = min{1 × 额外性能值, 650}<br>详情请参见 [增强型 SSD 云硬盘性能说明](https://intl.cloud.tencent.com/document/product/362/39611) | 吞吐 = min{120 + 0.2 × 容量（GiB）, 260}  | 吞吐 = min{100 + 0.15 × 容量（GiB）, 150} |
| 单路随机读写时延|0.1 - 0.5ms|0.3 - 1ms| 0.5 - 3ms | 0.8 - 5ms|
|特殊说明|极速型 SSD 云硬盘仅支持随 [标准存储增强型S5se](https://intl.cloud.tencent.com/document/product/213/11518) 实例一同购买，购买其他类型云服务器时不可使用极速型 SSD 云硬盘，也不可单独购买极速型 SSD 云硬盘|增强型 SSD 云硬盘仅支持挂载 S5、M5、SA2 及以上的新代次实例规格上，挂载在旧代次实例上时无法保证承诺性能|无|无|
...
## 费用说明
云硬盘详细费用，请参考 [云硬盘价格总览](https://intl.cloud.tencent.com/document/product/362/2413)。
