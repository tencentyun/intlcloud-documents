实例关机不收费是指按量计费实例通过**选择关机不收费选项**使实例进入**已关机**状态后，不再收取实例（CPU、内存）费用。[云盘](https://intl.cloud.tencent.com/document/product/213/2255)（系统盘和数据盘）、公网带宽（按带宽计费）和镜像等组件将继续计费。
...
3. 关机期间**无法支持除实例开机外**的较多操作（如调整配置、调整硬盘、调整网络、重装系统、重启实例、重置密码、改名相关操作等），**如需操作请开机后进行**。
