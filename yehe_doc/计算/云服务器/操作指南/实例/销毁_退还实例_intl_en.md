...
You can use the following methods to terminate and release pay-as-you-go instances:
 ...
 - **Automatic termination upon expiration or when account is overdue**: if your account balance of a pay-as-you-go instance is below 0 for 26 consecutive hours, the instance is automatically released. Within these 26 hours, billing continues for the first 2 hours, after which time the instance is shut down and billing stops for the remaining 24 hours. If the account is overdue, the pay-as-you-go instance is not moved to the recycle bin, and you can view this instance in the instance list. If you renew an instance within that period, you can continue to use the instance.
...
|---------|---------|---------|---------|
| Pay-as-you-go instances | After termination, the instance is moved to the recycle bin for 2 hours. If it is not restored by then, it is released. | Instances are released immediately, bypassing the recycle bin. | When your account is overdue, for the first 2 hours, billing continues and you can still use the instance. For the next 24 hours, however, the instance will be shut down, and billing will stop. Pay-as-you-go instances that are overdue are not moved to the recycle bin. If the instance is not renewed within that period, the instance is released. |
