...
## 작업 시나리오
Tencent Cloud CVM은 스토리지 하드웨어 미디어 변경을 지원합니다. 스토리지 하드웨어 미디어 변경을 통해 여러 비즈니스의 스토리지 요구를 효율적으로 충족시킬 수 있습니다.
Tencent Cloud에서는 [클라우드 디스크(CBS)](https://intl.cloud.tencent.com/document/product/213/4953)와 [로컬 디스크](https://intl.cloud.tencent.com/document/product/213/5798) 두 가지 스토리지 유형을 제공하며, 로컬 디스크를 클라우드 디스크로 변경할 수 있습니다. 본 문서는 디스크 미디어 유형을 변경하는 작업 방법 및 관련 주의 사항에 대해 안내합니다.
...
계정 내의 로컬 디스크 CVM을 클라우드 디스크 CVM으로 변경하면 로컬 디스크 CVM을 사용할 때의 단점을 보완할 수 있습니다.
...
- **CVM 제한**
 - 스팟 CVM은 로컬 디스크에서 CBS로의 변경을 지원하지 않습니다.
 - 전용 CVM은 로컬 디스크에서 CBS로의 변경을 지원하지 않습니다.
 - 빅 데이터 모델 D1, D2 및 고 I/O 모델 I3, I4 등과 같은 유형의 CVM은 로컬 디스크에서 CBS로의 변경을 지원하지 않습니다.
 - 베어 메탈 인스턴스는 로컬 디스크에서 CBS로의 변경을 지원하지 않습니다.
- **CVM 설정**
...
>? CVM이 'Shutdown' 상태라면 [순서3](#step3)을 바로 실행하시기 바랍니다.
2. (선택 사항) 변경하려는 CVM 행에서 [More]>[Instance Status]>[Shutdown]을 선택하여 종료 작업을 실행합니다.
<span id="step3"></span>
3. 변경하려는 CVM 행에서 [More]>[Resource Adjustment]>[Change Disk Media Type]을 클릭합니다.
4. [Change Disk Media Type] 대화 상자의 시스템 디스크/데이터 디스크에서 변경하려는 타깃 CBS 유형을 선택하고, 설명 동의를 선택한 뒤 [변경하기]를 클릭합니다.
