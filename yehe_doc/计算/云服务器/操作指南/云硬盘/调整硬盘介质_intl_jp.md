...
- ローカルディスクをCBSに変更すると、データコピー方式を利用してローカルディスクのデータをCBSシステムにコピーします。所要時間は、データの量とデータ転送速度によって異なります。この操作は完了するまでに時間がかかる場合があり、しばらくお待ちください。
- ローカルディスクをCBSに変更することのみをサポートします。 ローカルディスクがCBSに変更されている場合、CBSをローカルディスクに変更することはできません。
...
>? CVMが「シャットダウン済み」状態の場合、[手順3](#step3)に進みます。
