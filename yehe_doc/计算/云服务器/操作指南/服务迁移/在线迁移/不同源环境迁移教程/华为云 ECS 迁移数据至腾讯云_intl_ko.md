...
## 3. 데이터 백업
- 소스 CVM：Huawei Cloud 스냅샷 기능 등의 방식을 선택해 데이터를 백업할 수 있습니다.
...
<table>
	...
	<tr><th>Linux 소스 CVM</th><td><ol  style="margin: 0;"><li>점검 및 Virtio를 설치합니다. 자세한 작업 방식은 <a href="https://intl.cloud.tencent.com/document/product/213/9929">Linux 시스템 Virtio 드라이버 점검</a>을 참조 바랍니다.</li><li><code>which rsync</code> 명령어를 실행하여 rsync의 설치 여부를 확인합니다. </li><li>SELinux가 열려있는지 확인합니다. 만약 SELinux가 열려있다면, SELinux를 종료하시기 바랍니다.</li><li>Tencent Cloud API에 마이그레이션 요청을 보내면, 클라우드 API가 현재 UNIX 시간을 사용하여 생성된 Token을 점검하므로, 시스템 시간이 정확한지 확인하시기 바랍니다.</li><li><code>cloud-init --version</code> 명령어를 실행하여, 소스 CVM에 설치된 cloud-init 버전 정보를 검사합니다. <ul><li> 현재 설치된 cloud-init 버전이 <code>17.1</code>보다 낮을 경우, 해당 버전의 cloud-init를 삭제하는 것을 권장합니다. </li><li>소스 CVM에 cloud-init가 설치되어 있지 않으면 이 작업은 생략합니다. </li></ul></li></ol></td></tr>
</table>
...
> -  go2tencentcloud 마이그레이션 툴 세부 정보는 [마이그레이션 툴 설명](https://intl.cloud.tencent.com/document/product/213/35640)을 참조하십시오.
> 
