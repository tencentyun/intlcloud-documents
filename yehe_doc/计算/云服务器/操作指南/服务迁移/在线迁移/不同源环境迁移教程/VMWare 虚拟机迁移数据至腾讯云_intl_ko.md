...
## 3. 데이터 백업
- 소스 서버: VMWare 스냅샷 기능 등의 방법으로 데이터를 백업할 수 있습니다.
...
<table>
	...
	<tr><th>Linux 소스 서버</th><td><ol  style="margin: 0;"><li> Virtio를 점검 및 설치합니다. 이에 관한 자세한 작업은 <a href="https://intl.cloud.tencent.com/document/product/213/9929">Linux 시스템 Virtio 드라이버 점검</a>을 참조 바랍니다.</li><li>rsync 및 grub2-install(혹은 grub-install)의 설치 여부를 확인합니다.</li><li>SELinux가 열려있는지 확인합니다. 만약 SELinux가 열려있다면, SELinux를 종료하시기 바랍니다.</li><li>Tencent Cloud API에 마이그레이션 요청을 보내면, 클라우드 API는 현재 UNIX 시간을 사용하여 생성된 Token을 점검하므로, 시스템 시간이 정확한지 확인하시기 바랍니다.</li><li>소스 서버의 DHCP 서비스가 활성화되어 있는지 확인합니다. DHCP 서비스가 비활성화 상태일 경우, DHCP 서비스를 활성화해주시기 바랍니다.</li></ol></td></tr>
</table>
