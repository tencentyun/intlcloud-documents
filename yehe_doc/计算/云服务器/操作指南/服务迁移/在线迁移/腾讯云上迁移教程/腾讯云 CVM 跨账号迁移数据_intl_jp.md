オンラインマイグレーションツールはTencent Cloud CVMがアカウントの間でデータをマイグレーションすることをサポートします。アカウントの間のデータマイグレーションは2つの異なるアカウントでのホストの間のデータマイグレーションを指します。
...
1. ソースホストとターゲットCVMの間に接続コネクションを確立します。（オプション）  
 - プライベートネットワークマイグレーションモードを選択する場合は、 [VPN Connections](https://intl.cloud.tencent.com/document/product/1037)、[CCN](https://intl.cloud.tencent.com/document/product/1003) 或は[Direct Connect](https://intl.cloud.tencent.com/document/product/216) 等の方法を通してソースホストとターゲットCVMの接続コネクションを確立する必要があります。
...
5. ツールを実行します。
プライベートネットワークマイグレーションモード：ユースケース1クロスアカウントマイグレーションを例とします：  
 1. 一台のパブリックネットワークにアクセスできるホストで、以下のコマンドを実行し、ツールを実行して、ステージ1のマイグレーションに入ります。
```
...
`Stage 1 is finished and please run next stage at source machine.`と表示される場合、ステージ1は完了していることになります。 
 ...
 2. 前の手順（ステージ1）が完了した後、ステージ1のツールディレクトリ全体をマイグレーション対象のソースホストにコピーし、ツールを実行してステージ2のマイグレーションに入ります。
 例えば、以下のコマンドを実行してツールを実行し、ステージ2のマイグレーションに入ります。
```
sudo ./go2tencentcloud_x64
```
`Stage 2 is finished and please run next stage at gateway machine.`と表示される場合、ステージ2は完了していることになります。
 ...
 3. 前の手順（ステージ2）が完了した後、ステージ2のツールディレクトリ全体をステージ1のホストにコピーし、ツールを実行してステージ3のマイグレーションに入ります。
 例えば、以下のコマンドを実行してツールを実行し、ステージ3のマイグレーションに入ります。
```
sudo ./go2tencentcloud_x64
```
`Migrate successfully.`と表示される場合、全てのマイグレーションタスクが完了していることになります。
