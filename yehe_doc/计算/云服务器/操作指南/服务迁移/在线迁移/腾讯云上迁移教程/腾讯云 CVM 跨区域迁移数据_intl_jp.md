リージョンを跨いでデータのマイグレーションとは、Tencent CloudのあるリージョンのあるアベイラビリティゾーンのCVMデータを別のリージョンのあるアベイラビリティゾーンのターゲットCVMにマイグレーションすることです、または、Tencent Cloudの同じリージョンにある異なるアベイラビリティゾーンのCVMの間のデータマイグレーションです。
...
5. ツールを実行します。
例えば、64ビット Linux ソースホストで、 root 権限で以下のコマンドを実行してツールを実行します。
```
