리전 간 데이터 마이그레이션이란 Tencent Cloud 리전의 가용존 CVM에서 다른 리전의 가용존 타깃 CVM으로 데이터를 마이그레이션하는 것으로, Tencent Cloud의 동일 리전 내에서 다른 가용존 CVM 간의 데이터 마이그레이션도 포함됩니다.
...
user.json은 소스 CVM과 타깃 CVM의 설정 파일로, 설정 항목은 다음과 같습니다.
 ...
 - 타깃 CVM 소재 리전과 인스턴스 ID. CVM에서는 리전을 지원하며, [리전과 가용존](https://intl.cloud.tencent.com/document/product/213/6091)을 참조하시고, 인스턴스 ID는 [인스턴스 리스트](https://console.cloud.tencent.com/cvm/instance/index?rid=1) 페이지에서 확인할 수 있습니다.
...
```
[내부 네트워크 마이그레이션 모드: 시나리오2](https://intl.cloud.tencent.com/document/product/213/35640#Scenario2)를 사용해 마이그레이션하는 경우, 마이그레이션 성공 시 콘솔에 다음과 같이 출력됩니다.
