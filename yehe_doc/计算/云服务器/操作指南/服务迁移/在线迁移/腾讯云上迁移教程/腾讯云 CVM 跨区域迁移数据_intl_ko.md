리전 간 데이터 마이그레이션이란, Tencent Cloud에서 어느 한 리전의 가용존에 있는 CVM 데이터를 다른 리전의 가용존에 있는 타깃 CVM으로 마이그레이션이나, Tencent Cloud 동일 리전의 다른 가용존에 있는 CVM 간의 데이터 마이그레이션을 의미합니다.
...
```
예를 들어, [내부 네트워크 마이그레이션 모드: 시나리오2](https://intl.cloud.tencent.com/document/product/213/35640#Scenario2)의 방법을 사용하여 마이그레이션 할 경우, 마이그레이션 성공 시 콘솔에 다음과 같은 내용이 출력됩니다.
