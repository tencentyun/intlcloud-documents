지역 간 데이터 마이그레이션은 Tencent Cloud에서 어느 한 리전 내 가용존의 CVM 데이터를 다른 리전 내 가용존의 타깃 서버로 마이그레이션 하는 것과 같은 리전 내 다른 가용존에 있는 CVM 간의 데이터 마이그레이션을 뜻합니다.
