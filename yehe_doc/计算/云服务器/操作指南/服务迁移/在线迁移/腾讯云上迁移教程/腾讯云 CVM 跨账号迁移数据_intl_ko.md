온라인 마이그레이션 툴은 Tencent Cloud CVM의 계정 간 데이터 마이그레이션을 지원합니다. 계정 간 데이터 마이그레이션은 서로 다른 두 계정을 가진 호스트 간의 데이터 마이그레이션을 가리킵니다.
...
현재 마이그레이션 툴은 기본 방식과 내부 네트워크 마이그레이션 방식을 지원하며, 그중 내부 네트워크 마이그레이션 방식은 3가지 시나리오로 세분화됩니다. 각 마이그레이션 방식/시나리오가 요구하는 오리진 호스트와 타깃 CVM의 네트워크 수요가 다릅니다. 오리진 호스트와 타깃 CVM이 모두 공용 네트워크에 액세스할 수 있으면 직접 기본 방식으로 마이그레이션할 수 있습니다. 오리진 호스트와 타깃 CVM 중 하나가 공용 네트워크에 직접 액세스할 수 없을 경우, 우선 [VPN 연결](https://intl.cloud.tencent.com/document/product/1037), [CCN](https://intl.cloud.tencent.com/document/product/1003) 또는 [DC](https://intl.cloud.tencent.com/document/product/216) 등 방식을 통해 연결 터널을 구축한 후 내부 네트워크 방식으로 마이그레이션할 수 있습니다.
...
1. 오리진 호스트와 타깃 CVM의 연결 터널을 구축합니다(선택).  
 - 내부 네트워크 마이그레이션 방식을 선택할 경우, [VPN 연결](https://intl.cloud.tencent.com/document/product/1037), [CCN](https://intl.cloud.tencent.com/document/product/1003) 또는 [DC](https://intl.cloud.tencent.com/document/product/216) 등 방식을 통해 오리진 호스트와 타깃 CVM 간의 연결 터널을 구축해야 합니다.
...
5. 툴을 실행합니다.
내부 네트워크 마이그레이션 방식: 시나리오1 계정 간 마이그레이션 예시:  
 1. 공용 네트워크에 액세스할 수 있는 1대의 호스트에서 다음 명령어를 실행하여 툴을 실행하고, 단계1의 마이그레이션을 진행합니다.
```
sudo ./go2tencentcloud_x64
```
`Stage 1 is finished and please run next stage at source machine.`이라고 표시되면 단계1이 완료되었음을 의미합니다. 
 ![](https://main.qcloudimg.com/raw/afeceabbdaad10f348cd0805b209e5cb.png)
 2. 이전 단계(즉 단계1)를 완료한 후, 먼저 단계1의 전체 툴 디렉터리를 마이그레이션 예정인 오리진 호스트로 카피한 다음, 툴을 실행하여 단계2의 마이그레이션을 진행해야 합니다.
 예시, 다음 명령어로 툴을 실행하고, 단계2의 마이그레이션을 진행합니다.
```
sudo ./go2tencentcloud_x64
```
`Stage 2 is finished and please run next stage at gateway machine.`이라고 표시되면 단계2가 완료되었음을 의미합니다.
 ![](https://main.qcloudimg.com/raw/be35753f3f8f3a30b8d6364a1052991f.png)
 3. 이전 단계(즉 단계 2)를 완료한 후, 먼저 단계2의 전체 툴 디렉터리를 방금 전 단계1의 호스트로 카피한 다음, 툴을 실행하여 단계3의 마이그레이션을 진행해야 합니다.
 예시, 다음 명령어를 실행하여 툴을 실행하고, 단계3의 마이그레이션을 진행합니다.
```
sudo ./go2tencentcloud_x64
```
`Migrate successfully.`라고 표시되면 전체 마이그레이션 작업이 완료되었음을 의미합니다.
