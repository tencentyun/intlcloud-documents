온라인 마이그레이션 툴은 Tencent Cloud CVM의 계정 간 데이터 마이그레이션을 지원합니다. 계정 간 데이터 마이그레이션이란 다른 두 계정의 CVM 간의 데이터 마이그레이션을 의미합니다.
...
## 3. 데이터 백업
[스냅샷 생성](https://intl.cloud.tencent.com/document/product/362/5755) 등의 방법으로 데이터를 백업할 수 있습니다.
...
5. 툴을 실행합니다.
[내부 네트워크 마이그레이션 모드: 시나리오1](https://intl.cloud.tencent.com/document/product/213/35640#Scenario1)에서의 계정 간 마이그레이션 실행 방법은 다음과 같습니다.  
...
`Stage 1 is finished and please run next stage at source machine.`라는 문구가 출력되면, 1단계 마이그레이션이 완료되었다는 의미입니다. 
 ...
 2. 이전 단계(1단계)가 완료되면, 1단계의 모든 툴 목록을 마이그레이션할 소스 서버에 복사한 뒤, 다시 툴을 실행하여 2단계 마이그레이션을 진행합니다.
...
`Stage 2 is finished and please run next stage at gateway machine.`라는 문구가 출력되면, 2단계 마이그레이션이 완료되었다는 의미입니다.
 ...
 3. 이전 단계(2단계)가 완료되면, 2단계의 모든 툴 목록을 1단계의 CVM에 복사한 뒤, 다시 툴을 실행하여 3단계 마이그레이션을 진행합니다.
