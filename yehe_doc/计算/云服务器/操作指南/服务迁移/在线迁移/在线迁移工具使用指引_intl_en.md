...
<table>
	...
	<tr><td>Client.Rsync.BandwidthLimit</td><td>String</td><td>No</td><td>Bandwidth limit in KBytes/s. The default value is empty, meaning there is no bandwidth limit during transfer.</td></tr>
...
2. If the migration succeeds, check whether the destination CVM can be launched normally, whether data on the destination CVM is consistent with that on the source server, whether the network is normal, and whether other system services run normally.
