...
본 문서에서는 Windows Server 2012 운영 체제를 예로 들어 Windows 이미지를 생성하는 방법을 안내합니다.
...
1. 운영 체제 인터페이스에서 <img src="https://main.qcloudimg.com/raw/f0c84862ef30956c201c3e7c85a26eec.png" style="margin: 0;">를 클릭 후, Windows PowerShell 창을 엽니다.
...
2. 디스크 파티션 형식이 GPT 파티션인지 확인합니다.
 - GPT 파티션이라면, 서비스 마이그레이션이 아직 GPT 파티션을 지원하지 않기 때문에 [티켓 제출](https://console.cloud.tencent.com/workorder/category?level1_id=6&level2_id=7&source=0&data_title=%E4%BA%91%E6%9C%8D%E5%8A%A1%E5%99%A8CVM&step=1)을 통해 피드백해 주십시오.
...
```
 - 만약 ‘path’ 매개변수에 efi가 포함되어 있다면, 현재 운영 체제가 EFI 모드로 시작된 것이므로 [티켓 제출](https://console.cloud.tencent.com/workorder/category?level1_id=6&level2_id=7&source=0&data_title=%E4%BA%91%E6%9C%8D%E5%8A%A1%E5%99%A8CVM&step=1)을 통해 피드백해 주십시오.
 - 만약 ‘path’ 매개변수에 efi가 포함되어 있지 않다면, 다음 단계를 실행합니다.
...
![image](https://main.qcloudimg.com/raw/87808940ddd5317dd8c67a699e3dc5c0.png)
 - Virtio 드라이버가 미설치 상태라면 수동으로 설치해야 합니다.
    - Microsoft Windows Server 2008 R2(스탠다드 에디션, 데이터센터 에디션, 엔터프라이즈 에디션), Microsoft Windows Server 2012 R2(스탠다드 에디션), Microsoft Windows Server 2016(데이터센터 에디션), Microsoft Windows Server 2019(데이터센터 에디션)에서는 Tencent Cloud 커스텀 에디션 Virtio를 다운로드 하십시오. 다운로드 주소는 다음과 같습니다. 실제 네트워크 환경에 맞춰 다운로드 하십시오.
    ...
    - 기타 시스템 버전은 [커뮤니티 에디션 virtio](https://www.linux-kvm.org/page/WindowsGuestDrivers/Download_Drivers)를 다운로드 하십시오.

#### 기타 하드웨어 관련 구성 확인

클라우드로 업로드한 후, 하드웨어 관련 변경 사항은 다음과 같습니다(단, 이에 국한되지는 않음).
 - 그래픽 카드가 Cirrus VGA로 교체됩니다.
 - 디스크가 Virtio Disk로 교체됩니다.
 - ENI가 Virtio Nic로 교체되며, 로컬 연결되도록 기본 설정됩니다.
...
물리적 기기의 시스템을 내보내야 하거나 플랫폼 툴을 사용하지 않고 내보내기를 하려는 경우, disk2vhd 툴을 사용해 내보낼 수 있습니다.
...
>! 
>- disk2vhd 실행을 위해 Windows의 VSS(볼륨 섀도 복사 서비스) 기능을 사전에 설치해야 합니다. VSS 기능에 대한 자세한 내용은 [Volume Shadow Copy Service](https://docs.microsoft.com/zh-cn/windows/win32/vss/volume-shadow-copy-service-portal?redirectedfrom=MSDN)을 참조하십시오.
>- 현재 시스템은 vhdx 형식의 이미지를 지원하지 않으므로 "Use Vhdx"를 선택하지 마시기를 바랍니다.
>- ‘Use volume Shadow Copy’를 선택해 볼륨 섀도 복사 기능을 사용할 경우, 데이터 무결성을 더욱 효과적으로 보호할 수 있습니다.
> 
...
>
이미지 형식이 현재 플랫폼에서 지원하는 형식과 일치할 경우 이미지를 열어 파일 시스템을 바로 확인할 수 있습니다. 예를 들어, Windows 플랫폼은 vhd 형식의 이미지를 직접 첨부할 수 있습니다. Linux 플랫폼은 qemu-nbd를 사용해 qcow2 형식의 이미지를 열 수 있습니다. Xen 플랫폼은 vhd 파일을 직접 사용할 수 있습니다.
Linux 플랫폼을 예로 듭니다.
```
modprobe nbd
