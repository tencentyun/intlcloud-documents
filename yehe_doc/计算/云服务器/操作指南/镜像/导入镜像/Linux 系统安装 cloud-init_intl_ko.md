...
Cloud-init 는 주로 인스턴스 최초 초기화 시의 사용자 정의 설정 기능을 제공합니다. 가져온 미러 이미지에 cloud-init 서비스를 설치하지 않으면, 해당 미러 이미지에 기반하여 실행한 인스턴스가 정상적으로 초기화되지 않아 미러 이미지 가져오기에 실패하게 됩니다. 본 문서는 cloud-init 서비스 설치를 안내합니다.
cloud-init 설치는 아래의 두 가지 방식이 있습니다.
- [cloud-init 소스 패키지 수동 다운로드 방식](#ManualDown)을 통해 
- [소프트웨어 소스 상의 cloud-init 패키지를 사용하는 방식](#SoftSources)을 통해

## 주의사항
Linux 시스템 미러 이미지를 가져오기 전, 귀하의 미러 이미지 내부에 cloud-init 서비스가 바르게 설치되었는지 확인하세요.
...
>?  정상적으로 설치된 상황에서 cloud-init-17.1 버전이 Tencent Cloud와 최적의 호환성을 갖고 있어, 해당 버전의 미러 이미지로 생성해야 CVM의 모든 설정 옵션이 정상적으로 초기화될 수 있습니다. 그러므로 **cloud-init-17.1.tar.gz** 설치 버전을 권장합니다. 사용자는 [다운로드](https://launchpad.net/cloud-init/+download)하여 기타 버전의 cloud-init 소스 패키지를 획득할 수 있습니다. 본 문서는 cloud-init-17.1 버전을 예시로 사용합니다.
>
...
>!  Cloud-init 종속 모듈 requests 2.20.0 버전 이후 Python2.6 을 사용하지 않습니다. 미러 이미지 환경의 Python 인터프리터가 Python2.6 및 이하일 경우, cloud-init 종속 패키지 설치 전에 `pip install ’requests<2.20.0’` 명령어를 실행하여 requests 2.20.0 이하 버전을 설치하세요.
>
```
...
```
>! --init-system 의 선택 가능한 매개변수에는 (systemd, sysvinit,  sysvinit_deb, sysvinit_freebsd, sysvinit_openrc, sysvinit_suse, upstart)  [default: None] 이 있습니다. 현재 운영 체제가 사용하고 있는 자동 실행 서비스 관리 방식에 따라 선택하세요. 선택에 오류가 있을 경우 cloud-init 서비스의 자동 실행을 시작할 수 없습니다. 본 문서는 systemd 자동 실행 서비스 관리를 예시로 사용합니다.
...
2. `/etc/cloud/cloud.cfg`의 콘텐츠를 다운로드 완료한 cloud.cfg 파일 콘텐츠로 대체합니다.
...
### 소프트웨어 소스 상의 cloud-init 패키지를 사용하는 방식
...
```
>? apt-get 또는 yum 명령어를 실행하여 설치한 cloud-init 는 현재 운영 체제에서 설정한 소프트웨어 소스의 기본 cloud-init 버전을 기본 값으로 하고 있습니다. 해당 방식으로 설치한 미러 이미지가 생성한 인스턴스의 경우, 일부 설정 옵션 초기화가 예상과 다를 수 있어 [cloud-init 소스 패키지 수동 다운로드 방식](#ManualDown)을 사용해 설치할 것을 권장합니다.
>
...
```
3. Ubuntu 또는 Debian 운영 체제에 대해 `/etc/network/interfaces`을 다음 콘텐츠로 수정해야 합니다.
```
