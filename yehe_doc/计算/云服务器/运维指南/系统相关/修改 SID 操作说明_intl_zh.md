...
Windows操作系统对计算机和用户的识别是通过安全标识符（SID）进行区分。由于基于同一镜像生产的云服务器实例 SID 相同，会引起无法入域的问题。如果您需要搭建 Windows 域环境，则需要通过修改 SID 以达到入域的目的。
...
1. [使用 VNC 方式登录云服务器](https://intl.cloud.tencent.com/document/product/213/32496)。
