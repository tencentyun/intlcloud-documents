...
> 
>- 본 문서의 내용은 Tencent Cloud에서 제공하는 Windows Server 공용 이미지에만 해당됩니다. 사용자 정의 이미지 및 외부에서 가져온 이미지는 본 문서의 활성화 방식을 사용할 수 없습니다.
...
### 주의 사항
일부 시스템에서는 시스템 시간에 문제가 있으면 수동 활성화에 오류가 발생할 수 있습니다. 이 경우, 시스템 시간을 동기화해야 하며 시간 동기화 작업 순서는 다음과 같습니다.
> Windows CVM의 시스템 시간이 정상일 경우 바로 [활성화 순서](# ActivationStep)를 진행하시기 바랍니다.
>
1. Windows CVM에 로그인합니다.
2. 운영 체제 인터페이스에서 [시작]>[실행]을 클릭하고 `cmd.exe`를 입력하여 윈도우 명령 창을 엽니다.
3. 윈도우 명령 창에서 다음 명령어를 순서대로 실행하여 시스템 시간을 동기화합니다.
```
...
1. Windows CVM에 로그인합니다.
2. 운영 체제 인터페이스에서 [시작]>[실행]을 클릭하고 `cmd.exe`를 입력하여 윈도우 명령 창을 엽니다.
3. 윈도우 명령 창에서 다음 명령어를 순서대로 실행하여 수동으로 활성화합니다.
 - Windows Server 2008 및 Windows Server 2012 서버에서는 다음 명령어를 순서대로 실행합니다.
```
