...
High-performance power management is required for the Windows Server operating system (OS) to support the soft shutdown of CVM instances. Otherwise, you can shut down CVM instances in the CVM console only by hard shutdown. This document uses Windows Server 2012 as an example to describe how to configure power management.

## Note

To modify power management, you do not need to restart the computer.

## Directions

1. Log in to the Windows CVM.
2. Access the Tencent Cloud private network in Internet Explorer and download the Tencent Cloud power modification and configuration tool.
...
4. Run the following command to view the current power management scheme:
```
powercfg -L
```
