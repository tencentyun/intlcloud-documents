...
本文档介绍 Linux [重装系统](https://intl.cloud.tencent.com/document/product/213/4933) 为 Windows 后，在云服务器上读取原 Linux 系统下数据盘数据的操作方法。
...
<dx-alert infotype="notice" title="">
此时 Linux 数据盘为只读。若需要将此数据盘作为 Windows 数据盘进行读写操作，请先将需要的文件备份，重新格式化成 Windows 操作系统支持的标准类型，具体操作见 [Windows 实例：初始化数据盘](https://intl.cloud.tencent.com/document/product/213/2158)。
</dx-alert>
