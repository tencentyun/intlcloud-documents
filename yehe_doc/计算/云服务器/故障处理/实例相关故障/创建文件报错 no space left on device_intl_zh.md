...
### 解决硬盘空间已满问题
1. 登录云服务器，详情请参见 [使用标准登录方式登录 Linux 实例](https://intl.cloud.tencent.com/document/product/213/5436)。
...
根据定位到占用空间最大的目录容量情况，执行以下步骤：
   ...
   - 目录容量较大，则请执行 [步骤2](#Step2) 定位到占用空间较大的文件，综合业务情况评估是否可删除。若无法删除，则请通过 [扩容云硬盘](https://intl.cloud.tencent.com/document/product/213/32377) 扩大硬盘存储空间。
