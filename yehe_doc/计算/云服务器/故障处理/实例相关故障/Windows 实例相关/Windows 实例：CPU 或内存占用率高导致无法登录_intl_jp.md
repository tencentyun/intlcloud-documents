...
このドキュメントでは、CPUとメモリの使用率が高いためにWindows CVMにログインできない問題のトラブルシューティング方法と解決策について説明します。
>? 以下の操作手順はWindows server 2012 R2を例に説明します。OSのバージョンによって操作手順の詳細が若干異なります。

## 操作手順

### mstsc方式利用してCVMにログインする

1. ローカルコンピューターで、**Windows+R** ショートカットを利用して実行ダイアログボックスを開きます。
2. **mstsc** を入力し、**Enter** キーを押して、「リモートデスクトップ接続」インターフェースを開きます。
3. ポップアップした「リモートデスクトップ接続」ウィンドウで、CVMのパブリックIPとログインパスワードを入力して、CVMにログインします。
>!  CVMの負荷が高い場合、リモート接続は確立できません。 **VNC 方式**でCVMにログインする必要があります。操作方法は「[CVMコンソール](https://console.cloud.tencent.com/cvm) にログインし、右側の【ログイン】をクリックして、 VNC方式を選択してCVMにログインします。
...
タスクマネージャー中のプロセスを表示し、問題を分析と調査して、該当の解決策を講じます。
 - 業務プロセスが大量のリソースを消費しています。例えば、IIS 、httpd 、PHP 、Java等。
 	- 現在の取引量が多い場合、高負荷は正常状態です。拡張を検討する必要があります。
 	- 現在の取引量が多くない場合、さらにエラーログを分析する必要があります。不適切なパラメータ設定によるリソースが浪費される可能性があります。
 ...
 - 業務プロセスとシステムプロセス以外のプロセスが大量のリソースを消費しています。
	 - プロセス名が怪しくて、署名や説明がなく、`C:\windows\system32` ディレクトリにないの異常なプロセスがリソースを消費している場合、CVMはウイルスに感染してしまう可能性があります。
