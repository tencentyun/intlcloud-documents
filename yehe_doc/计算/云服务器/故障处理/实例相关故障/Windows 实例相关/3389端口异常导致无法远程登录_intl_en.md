## Issue
When you are trying to remotely log in to a Windows instance, the login fails. Your [self-diagnose](https://console.cloud.tencent.com/workorder/check) suggests an exception on the remote login port 3389, but which was open in the security groups of the CVM instance.

>? This document uses Windows Server 2012 as an example. The procedure may vary slightly depending on the operating system and language.
>

## Troubleshooting the Issue

1. [Log in to a Windows instance by using VNC](https://intl.cloud.tencent.com/document/product/213/32496).
2. Once logged in, click <img src="https://main.qcloudimg.com/raw/f0c84862ef30956c201c3e7c85a26eec.png" style="margin: 0;"> to open the **Windows PowerShell** window.
3. Run the following command to check the status of the port 3389.
```
netstat -ano | findstr 3389
```
If the result similar to the following is returned, the 3389 port runs improperly. Follow the [solutions](#3389Solution) below to solve the issue.
...
6. In the **Windows PowerShell** window, enter **services.msc** and press **Enter**. The **Services** window appears.
...
8. Refer to [Modifying Security Group Rules](https://intl.cloud.tencent.com/document/product/213/34825) to disable the 3389 port and open the one set in [Step 3](#step09).
