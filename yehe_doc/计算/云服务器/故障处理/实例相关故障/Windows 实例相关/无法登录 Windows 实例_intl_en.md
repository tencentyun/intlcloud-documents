...
**Procedure**:
1. Log in to the instance by using [VNC login](#VNC).
2. In **Task Manager**, locate the process that is causing the high load. For details, see [Failed to Log In to a Windows CVM Due to High CPU and Memory Usage](https://intl.cloud.tencent.com/document/product/213/32405).
...
**Diagnosis**: log in to a Windows instance through VNC to check whether the firewall is enabled and whether security software such as 360 Total Security are installed on the server.
