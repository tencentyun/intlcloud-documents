...
6. Check whether the Tomcat server is started.
...
```
3. Edit the `server.xml` file in the `/usr/*/conf` directory by modifying the redirectPort parameter to the port of the SSL connector, i.e., changing port 80 to port 443, as shown below:
```
<Connector port="80" protocol="HTTP/1.1"
  connectionTimeout="20000"
  redirectPort="443" />
```
> After modification, requests from a connector supports non-SSL requests will be redirected to a connector that supports SSL requests.
>
4. Shut down the Tomcat server by running the following command in the ` /usr/*/bin` directory.
```
./shutdown.sh
```
5. Run the following command to check whether there is a problem with the configuration.
```
./configtest.sh
```
