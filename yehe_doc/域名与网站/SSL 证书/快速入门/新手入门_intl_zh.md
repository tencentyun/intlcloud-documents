...
### 步骤2：提交资料验证
 #### 域名所有权验证
免费型证书& DV 型付费证书申请后，可通过3种方式验证，分别为自动 DNS 验证、手动 DNS 验证、文件验证。
 >?如果所申请域名成功添加 [DNS 解析平台](https://console.cloud.tencent.com/cns)，可以支持自动 DNS 验证。

#### 组织机构验证
OV 与 EV 型付费证书提交资料后需上传确认函等待审核，期间审核人员进行电话核实和发送域名信息确认邮件。具体操作请参考 [OV 与 EV 型 SSL 证书材料提交流程](https://intl.cloud.tencent.com/document/product/1007/30160)。
