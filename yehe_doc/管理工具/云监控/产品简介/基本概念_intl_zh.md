...
用户调用云监控 API 时，可以通过 period 参数指定时间粒度。调用  [GetMonitorData](https://intl.cloud.tencent.com/document/product/248/33881)  接口 获取监控数据时，period、startTime 和 endTime 参数的值决定了将会返回的数据数量。例如，以所有参数的默认值调用时会返回前一小时内每300秒的统计数据，即总共12个数据点。
