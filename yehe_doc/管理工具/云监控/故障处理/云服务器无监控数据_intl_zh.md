...
建议您首先根据 [安装监控组件](https://intl.cloud.tencent.com/document/product/248/6211) 重新安装监控Agent，并等待3分钟查看监控数据是否恢复。若安装遇到问题或监控数据未恢复，请阅读下文详细排查。
...
若未安装成功，请参考 [安装监控组件 ](https://intl.cloud.tencent.com/zh/document/product/248/6211) 安装 Agent。
...
1. 如果使用自建 DNS 服务或第三方 DNS 服务，建议在`/etc/resolv.conf`中追加腾讯云提供的内网DNS，具体请参见 [云服务器内网 DNS列表](https://intl.cloud.tencent.com/document/product/213/5225)
...
5. 选择【使用下面的 DNS 服务器地址】，根据 [内网 DNS](https://intl.cloud.tencent.com/document/product/213/5225) 列表中对应的不同地域，修改 DNS IP。修改完后单击确定即可。
...
7. 运行 `services.msc` ，按鼠标右键单击【重启启动】按钮，重启 SgAgent 和 BaradAgent 即可。
   ...
   若重启后仍无监控数据，请参考 [安装云服务器监控组件](https://intl.cloud.tencent.com/document/product/248/6211) 卸载并重装 Agent。
   </dx-alert>
   :::
   </dx-tabs>
