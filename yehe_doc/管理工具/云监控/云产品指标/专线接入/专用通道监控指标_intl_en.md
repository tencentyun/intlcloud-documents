...
| ----- | ---------- | ---- | ------------ | ------------ |
| InBandwidth | Network inbound bandwidth | Bandwidth rate from the access point AR toward the VPC instance, which indicates the inbound bandwidth data collected every one minute or five minutes | Kb/s | directConnectConnId |
| OutBandwidth | Newtork outbound bandwidth | Bandwidth rate from the VPC toward the access point AR, which indicates the outbound bandwidth data collected every one minute or five minutes | Kb/s | directConnectConnId |
