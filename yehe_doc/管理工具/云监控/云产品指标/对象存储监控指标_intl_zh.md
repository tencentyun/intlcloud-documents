...
| TotalRequests    | 总请求数       | 所有存储类型总请求数                                         | 次   | appid、bucket |
| GetRequests      | GET 类总请求数  | 所有存储类型 GET 类总请求数                                    | 次   | appid、bucket |
| PutRequests      | PUT 类总请求数  | 所有存储类型 PUT 类总请求数                                    | 次   | appid、bucket |
...
| NlMultipartNumber            | 近线存储碎片文件数           | 个   | appid、bucket |
...
| 指标英文名                    | 指标中文名           | 指标含义                                                 | 单位 | 维度          |
| ----------------------------- | -------------------- | -------------------------------------------------------- | ---- | ------------- |
| InternetTrafficDown           | 外网下行流量         | 数据通过互联网从 COS 传输到客户端产生的流量              | B    | appid、bucket |
| InternetTrafficUp             | 外网上行流量         | 数据通过互联网从客户端传输到 COS 产生的流量              | B    | appid、bucket |
| InternalTrafficDown           | 内网下行流量         | 数据通过腾讯云内网从 COS 传输到客户端产生的流量          | B    | appid、bucket |
| InternalTrafficUp             | 内网上行流量         | 数据通过腾讯云内网从客户端传输到 COS 产生的流量          | B    | appid、bucket |
...
| 指标英文名      | 指标中文名    | 指标含义                      | 单位 | 维度          |
| --------------- | ------------- | ----------------------------- | ---- | ------------- |
| 2xxResponse     | 2xx 状态码   | 当前存储桶出现 2xx 错误次数   | 次   | appid、bucket |
| 3xxResponse     | 3xx 状态码   | 当前存储桶出现 3xx 错误次数   | 次   | appid、bucket |
| 4xxResponse     | 4xx 状态码   | 当前存储桶出现 4xx 错误次数  | 次   | appid、bucket |
| 5xxResponse     | 5xx 状态码   | 当前存储桶出现 5xx 错误次数  | 次   | appid、bucket |
| 2xxResponseRate | 2xx 状态码占比 | 当前存储桶出现 2xx 错误占比率 | %  | appid、bucket |
| 3xxResponseRate | 3xx 状态码占比 | 当前存储桶出现 3xx 错误占比率 | %  | appid、bucket |
| 4xxResponseRate | 4xx 状态码占比 | 当前存储桶出现 4xx 错误占比率 | %  | appid、bucket |
| 5xxResponseRate | 5xx 状态码占比 | 当前存储桶出现 5xx 错误占比率 | %  | appid、bucket |
| 400Response     | 400 状态码     | 当前存储桶出现 400 错误次数   | 次   | appid、bucket |
| 403Response     | 403 状态码     | 当前存储桶出现 403 错误次数   | 次   | appid、bucket |
| 404Response     | 404 状态码     | 当前存储桶出现 404 错误次数   | 次   | appid、bucket |
| 400ResponseRate | 400 状态码占比 | 当前存储桶出现 400 错误占比率 | %  | appid、bucket |
| 403ResponseRate | 403 状态码占比 | 当前存储桶出现 403 错误占比率 | %  | appid、bucket |
| 404ResponseRate | 404 状态码占比 | 当前存储桶出现 404 错误占比率  | %  | appid、bucket |
| 500ResponseRate | 500 状态码占比 | 当前存储桶出现 500 错误占比率  | %  | appid、bucket |
| 501ResponseRate | 501 状态码占比 | 当前存储桶出现 501 错误占比率 | %  | appid、bucket |
| 502ResponseRate | 502 状态码占比 | 当前存储桶出现 502 错误占比率 | %  | appid、bucket |
| 503ResponseRate | 503 状态码占比 | 当前存储桶出现 503 错误占比率 | %  | appid、bucket |

> ?
> 1. 详情请查看 [错误码表](https://intl.cloud.tencent.com/document/product/436/7730) 。
