...
| CPULoadAvg   | CPU 平均负载 | 分析`/proc/loadavg`中的数据，以10s为间隔采集过去1分钟内系统平均负载（Windows 机器无此指标） | 一段时间内正在使用和等待使用 CPU 的平均任务数     | -    | InstanceId |
...
>
>1. 安装 [云服务器监控组件 Agent](https://intl.cloud.tencent.com/document/product/248/6211) 才能获取基础指标数据和告警时间（为客户云服务器的本地时间）。若客户云服务器本地时间非东八区时间，将导致该云服务器的监控数据的时间为非东八区的子机本地时间。
>2. 安装监控组件两种方式：
 >  - 用户可通过购买机器时勾选云监控按钮自动安装监控组件。
 >  -  通过 [安装云服务器监控组件](https://intl.cloud.tencent.com/document/product/248/6211) 手动安装监控组件。
...
| CpuLoadavg  | CPU 一分钟平均负载 |1分钟内 CPU 平均负载，取`/proc/loadavg`第三列数据，依赖监控组件安装采集 。Windows 机器无此指标 | -    |InstanceId|
|  Cpuloadavg5m | CPU 五分钟平均负载 |5分钟内 CPU 平均负载，取`/proc/loadavg`第一列数据，依赖监控组件安装采集 。Windows 机器无此指标 | -    |InstanceId|
|  Cpuloadavg15m | CPU 十五分钟平均负载 | 15分钟内 CPU 平均负载，取`/proc/loadavg`第二列数据，依赖监控组件安装采集。Windows 机器无此指标  |   -   |InstanceId|
