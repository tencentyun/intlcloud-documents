...
| 指标英文名       | 指标中文名         | 单位  | 指标含义                                                     | 维度       |
| ---------------- | ------------------ | ----- | ------------------------------------------------------------ | ---------- |
| CpuUsNodeMin         | 平均 CPU 使用率      | %     | 平均 CPU 使用率                                                | instanceid |
...
| Instances.N.Dimensions.0.Value | instanceid | 实例具体 ID      | 输入实例的具体 Redis 实例 ID，例如：tdsql-123456 也可以是实例串号，例如：crs-ifmymj41，可通过 查询CRS实例列表接口 查询 |
