...
| ------------- | ------------ | ------------------------------------------------------------ | ----- | ------------------------ | ---------------------------- |
| BytesReceived | 内网入流量   | 每秒接受的字节数                                             | 字节/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
...
| 指标英文名        | 指标中文名     | 指标说明                                                     | 单位  | 维度                     | 统计周期                     |
| ----------------- | -------------- | ------------------------------------------------------------ | ----- | ------------------------ | ---------------------------- |
| ConnectionUseRate | 连接数利用率   | 当前打开连接数/最大连接数                                    | %     | InstanceId、InstanceType | 5s、10s、60s、300s、3600s    |
| MaxConnections    | 最大连接数     | 最大连接数                                                   | 个    | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| Qps               | 每秒执行操作数 | 数据库每秒执行的 SQL 数（含  insert、select、update、delete、replace），QPS 指标主要体现 TencentDB 实例的实际处理能力 | 次/秒 | InsanceId、InstanceType  | 5s、60s、300s、3600s、86400s |
| ThreadsConnected  | 当前连接数     | 当前打开的连接的数量                                         | 个    | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| Tps               | 每秒执行事务数 | 数据库每秒传输的事务处理个数                                 | 个/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |

### 引擎监控（普通）-访问

| 指标英文名  | 指标中文名 | 指标说明                                    | 单位  | 维度                     | 统计周期                     |
| ----------- | ---------- | ------------------------------------------- | ----- | ------------------------ | ---------------------------- |
| ComDelete   | 删除数     | 每秒删除数                                  | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| ComInsert   | 插入数     | 每秒插入数                                  | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| ComReplace  | 覆盖数     | 每秒覆盖数                                  | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| ComUpdate   | 更新数     | 每秒更新数                                  | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| Queries     | 总访问量   | 所有执行的 SQL 语句，包括 set，show 等      | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| QueryRate   | 访问量占比 | 每秒执行操作数 QPS/推荐每秒操作数           | %     | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| SelectCount | 查询数     | 每秒查询数                                  | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| SelectScan  | 全表扫描数 | 执行全表搜索查询的数量                      | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| SlowQueries | 慢查询数   | 查询时间超过 long_query_time 秒的查询的个数 | 个    | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |

### 引擎监控（普通）-表

| 指标英文名       | 指标中文名     | 指标说明                   | 单位  | 维度                     | 统计周期                     |
| ---------------- | -------------- | -------------------------- | ----- | ------------------------ | ---------------------------- |
| CreatedTmpTables | 内存临时表数量 | 创建临时表的数量           | 个/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| TableLocksWaited | 等待表锁次数   | 不能立即获得的表的锁的次数 | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |

### 引擎监控（扩展）-Tmp

| 指标英文名           | 指标中文名     | 指标说明                 | 单位  | 维度                     | 统计周期                     |
| -------------------- | -------------- | ------------------------ | ----- | ------------------------ | ---------------------------- |
| CreatedTmpDiskTables | 磁盘临时表数量 | 每秒创建磁盘临时表的次数 | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| CreatedTmpTables     | 临时文件数量   | 每秒创建临时文件的次数   | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |

### 引擎监控（扩展）-Key

| 指标英文名       | 指标中文名             | 指标说明                            | 单位  | 维度                     | 统计周期                     |
| ---------------- | ---------------------- | ----------------------------------- | ----- | ------------------------ | ---------------------------- |
| KeyBlocksUnused  | 键缓存内未使用的块数量 | myisam 引擎未使用键缓存块的个数     | 个    | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| KeyBlocksUsed    | 键缓存内使用的块数量   | myisam 引擎已使用键缓存块的个数     | 个    | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| KeyReadRequests  | 键缓存读取数据块次数   | myisam 引擎每秒读取键缓存块的次数   | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| KeyReads         | 硬盘读取数据块次数     | myisam 引擎每秒读取硬盘数据块的次数 | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| KeyWriteRequests | 数据块写入键缓冲次数   | myisam 引擎每秒写键缓存块的次数     | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| KeyWrites        | 数据块写入磁盘次数     | myisam 引擎每秒写硬盘数据块的次数   | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
...
| 指标英文名         | 指标中文名     | 指标说明                 | 单位  | 维度                     | 统计周期                     |
| ------------------ | -------------- | ------------------------ | ----- | ------------------------ | ---------------------------- |
| HandlerCommit      | 内部提交数     | 每秒事务提交的次数       | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| HandlerReadRndNext | 读下一行请求数 | 每秒读取下一行的请求次数 | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| HandlerRollback    | 内部回滚数     | 每秒事务被回滚的次数     | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
...
| 指标英文名  | 指标中文名 | 指标说明             | 单位  | 维度                     | 统计周期                     |
| ----------- | ---------- | -------------------- | ----- | ------------------------ | ---------------------------- |
| LogCapacity | 日志使用量 | 引擎使用日志的数量   | MB    | InstanceId、InstanceType | 5s、60s、300s、3600s         |
| OpenFiles   | 打开文件数 | 引擎打开的文件的数量 | 个/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |

### 引擎监控（扩展）-连接

| 指标英文名     | 指标中文名     | 指标说明                   | 单位 | 维度                     | 统计周期                     |
| -------------- | -------------- | -------------------------- | ---- | ------------------------ | ---------------------------- |
| ThreadsCreated | 已创建的线程数 | 创建用来处理连接的线程数   | 个   | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| ThreadsRunning | 运行的线程数   | 激活的（非睡眠状态）线程数 | 个   | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |

### 引擎监控（扩展）-访问

| 指标英文名  | 指标中文名 | 指标说明     | 单位  | 维度                     | 统计周期                     |
| ----------- | ---------- | ------------ | ----- | ------------------------ | ---------------------------- |
| ComCommit   | 提交数     | 每秒提交次数 | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| ComRollback | 回滚数     | 每秒回滚次数 | 次/秒 | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |

### 引擎监控（扩展）-表

| 指标英文名          | 指标中文名       | 指标说明               | 单位 | 维度                     | 统计周期                     |
| ------------------- | ---------------- | ---------------------- | ---- | ------------------------ | ---------------------------- |
| OpenedTables        | 已经打开的表数   | 引擎已经打开的表的数量 | 个   | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
| TableLocksImmediate | 立即释放的表锁数 | 引擎立即释放的表锁数   | 个   | InstanceId、InstanceType | 5s、60s、300s、3600s、86400s |
