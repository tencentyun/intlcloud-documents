## 连接方式
连接云原生数据库 TDSQL-C（兼容 MySQL 版）的方式如下：
- **内网地址连接**：通过内网地址连接云原生数据库 TDSQL-C，使用云服务器 CVM 直接连接云数据库的内网地址，这种连接方式使用内网高速网络，延迟低。
 - 云服务器和数据库须是同一账号，且同一个[ VPC](https://intl.cloud.tencent.com/document/product/215/535) 内（保障同一个地域）。
...
## 内外网连接 TDSQL-C（兼容 MySQL 版）
### 从 Windows 云服务器连接
1. 登录到 Windows 云服务器，请参见 [快速配置 Windows 云服务器](https://intl.cloud.tencent.com/document/product/213/10516)。
2. 下载一个标准的 SQL 客户端。
...
>
1. 登录 [TDSQL-C 控制台](https://console.cloud.tencent.com/cynosdb)，在集群列表，单击集群名，进入集群详情页面。
