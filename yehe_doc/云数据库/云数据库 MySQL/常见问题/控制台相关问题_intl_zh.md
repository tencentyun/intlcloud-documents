...
### 云数据库 MySQL 实例销毁了怎么办？
实例退还后会保留在回收站一段时间，按量计费实例保留1天；在此时间段内，可在回收站内找到对应实例，然后进行恢复操作即可找回。

<span id = "zhanghaomima"></span>
### 帐号误删与忘记密码怎么办？
- 若误删帐号，可登录 [MySQL 控制台](https://console.cloud.tencent.com/cdb) 单击实例名进入实例管理页，通过单击【数据库管理】>【帐号管理】>【创建帐号】或使用 sql 语句进行新建，详细介绍可参见 [创建帐号](https://intl.cloud.tencent.com/document/product/236/31900)。
...
### 如何获知磁盘空间不足？
监控中心对云数据库的磁盘空间进行了监控，当云数据库的使用空间超过90%时，会触发短信和邮件告警，您只需要在云监控中配置好对应的告警接收人（如何配置请参见 [告警功能](https://intl.cloud.tencent.com/document/product/236/8457)），当空间不足的时候就能收到告警。
