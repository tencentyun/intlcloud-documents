...
<span id="ykxx" ></span>
#### b. 源库信息
根据需要选择接入类型，依次填写对应的源库连接信息后，单击【测试连通性】。
>!
>- 您需要在阿里云开通 TencentDB 对外映射出去的 IP 的白名单，否则在测试连通性时将不通过。例如，
>  - 有公网 IP 的 MySQL 腾讯云的映射，您需要将相对应的地域外网 IP 添加到阿里云的白名单中。
>  -  DTS 配置时源库类型为“专线”或者“VPN”会在任务生成后出现对外映射的 IP，需将此 IP 添加到阿里云白名单中。
> 
>- 您可以 [提交工单](https://console.cloud.tencent.com/workorder/category) 咨询相应地域的 DTS IP 白名单信息，或单击【测试连通性】，测试连通性失败后，连接检测结果中会给出 DTS IP 白名单信息。
>
...
### 9. 完成迁移
当迁移进度达到100%时，可单击右侧【完成】，完成迁移任务，或调用 DTS 云 [API](https://intl.cloud.tencent.com/document/product/571/18122) 断开同步，完成迁移任务。
