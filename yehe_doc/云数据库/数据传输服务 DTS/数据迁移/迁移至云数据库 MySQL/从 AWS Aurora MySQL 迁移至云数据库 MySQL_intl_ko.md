...
본 문서에서는 DTS의 데이터 마이그레이션 기능을 통해 AWS Aurora MySQL에서 TencentDB for MySQL로 데이터를 마이그레이션하는 것에 대해 소개합니다. DTS는 구조 마이그레이션, 전체 데이터 마이그레이션 및 증분 데이터 마이그레이션을 지원하며, 서비스를 중단하지 않고도 TencentDB for MySQL로 원활하게 데이터를 마이그레이션할 수 있습니다.
<span id="qttj"></span>
## 전제 조건
...
마이그레이션 예정인 소스 AWS Aurora MySQL은 공용 네트워크를 통해 액세스할 수 있습니다. AWS Aurora MySQL의 네트워크와 보안 설정에서 가용성 기능을 공개로 설정해야 합니다
- 소스 AWS RDS MySQL에 마이그레이션 계정을 생성해야 하며, 다음과 같은 계정 권한이 필요합니다.
```
...
- TencentDB for MySQL의 스토리지 용량은 반드시 소스 AWS Aurora MySQL 데이터베이스가 점유한 스토리지 용량의 1.2배 이상이어야 합니다.
- 소스 AWS Aurora MySQL이 GTID 인스턴스가 아닌 경우, DTS가 원본 HA(Highly Available) 전환을 지원하지 않아 소스 Aurora MySQL에 전환 발생 시 DTS 증분 동기화가 중단될 수 있습니다.
...
- 증분 동기화: DTS는 전체 데이터 마이그레이션을 바탕으로 소스 AWS Aurora MySQL 데이터베이스의 binlog 정보를 읽어와 분석하고, 소스 AWS Aurora MySQL의 증분 업데이트를 타깃 MySQL로 동기화합니다. 증분 데이터 동기화를 통해 서비스를 중단하지 않고 애플리케이션을 Tencent Cloud로 원활하게 마이그레이션할 수 있습니다.
...
## 마이그레이션 준비
-	AWS Aurora MySQL 데이터베이스에 로그인하여 binlog 로그 저장 시간을 설정하고 다음 명령어를 실행합니다.
```
call mysql.rds_set_configuration('binlog retention hours', 24);
```
상기 명령어는 binlog 로그의 저장 시간을 24시간으로 설정하며, 최대 168시간(7일)까지 설정 가능합니다.
-	AWS Aurora MySQL의 binlog 로그는 활성화 상태여야 하며, binlog_format은 row로 설정해야 합니다. MySQL 버전이 5.6 이상인 경우, binlog_row_image는 full로 설정해야 합니다.
...
| 테이블의 기본 키 유무 검사         | 모든 마이그레이션 예정 테이블에는 반드시 기본 키가 있어야 합니다.                                      |
...
<tr>
<td>서비스 제공업체</td><td>‘AWS’를 선택합니다. </td></tr>
<tr>
<td>데이터베이스 버전	</td><td>Aurora MySQL 5.6 또는 Aurora MySQL 5.7을 선택합니다. </td></tr>
<tr>
<td>액세스 유형</td><td>‘공용 네트워크’를 선택합니다. </td></tr>
<tr>
<td>소속 리전</td><td>소스 라이브러리 소속 리전은 DTS 서비스 아웃바운드 리전입니다. AWS Aurora MySQL에서 가장 가까운 리전을 선택하십시오. </td></tr>
<tr>
<td>호스트 주소</td><td>AWS Aurora MySQL 액세스 IP 주소로, AWS Aurora MySQL의 기본 정보 페이지에서 데이터베이스의 액세스 주소를 얻을 수 있습니다. </td></tr>
<tr>
<td>포트</td><td>AWS Aurora MySQL 액세스 포트로, 기본값은 3306입니다. </td></tr>
<tr>
<td>계정</td><td>AWS Aurora MySQL의 데이터베이스 계정으로, 계정 권한이 요구에 부합해야 합니다. </td></tr>
<tr>
<td>비밀번호</td><td>AWS Aurora MySQL의 데이터베이스 계정의 비밀번호입니다. </td></tr>
<tr>
