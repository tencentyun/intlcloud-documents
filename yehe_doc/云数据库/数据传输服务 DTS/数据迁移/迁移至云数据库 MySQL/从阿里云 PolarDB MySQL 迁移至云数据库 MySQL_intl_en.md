...
- The source Alibaba Cloud PolarDB for MySQL instance to be migrated can be accessed via public network. The public availability of Alibaba Cloud PolarDB for MySQL should be enabled. Supported versions: MySQL 5.6 and MySQL 5.7.
-. You need to create a migration account on the source Alibaba Cloud PolarDB for MySQL instance. Account permissions required are as follows:
```
...
- The storage capacity of TencentDB for MySQL instance must be at least 1.2 times larger than that of Alibaba Cloud PolarDB for MySQL instance.
...
- Incremental sync: DTS reads and parses the binlog information of the source Alibaba Cloud PolarDB for MySQL instance based on full data migration, and synchronizes the incremental data of the Alibaba Cloud PolarDB for MySQL instance to the target TencentDB for MySQL. In this way, incremental data can be smoothly migrated to Tencent Cloud in a non-stop manner.
...
## Preparation 
- The binlog feature of Alibaba Cloud PolarDB for MySQL is disabled by default. Enabling this feature may lead to connection interruption because you need to restart all the nodes in the cluster. Please make business arrangements before the operation.
- The parameter `loose_polar_log_bin` of Alibaba Cloud PolarDB for MySQL is a global-level parameter.
- Log in to the Alibaba Cloud PolarDB for MySQL console and find the target parameter `loose_polar_log_bin` on **Configuration and Management** > **Parameter Configuration**.
   - For a PolarDB cluster featuring the MySQL 5.6 database engine, modify the current value to `ON_WITH_GTID`.
   - For a PolarDB cluster featuring the MySQL 5.7 database engine, modify the current value to `ON`.
...
<tr>
<td>Database Version</td><td>Select PolarDB 5.6 or PolarDB 5.7.</td></tr>
<tr>
<td>Access Type</td><td>Select “Public Network”.</td></tr>
<tr>
<td>Region</td><td>The region of the source database refers to the outbound region of DTS. Please select the nearest region to Alibaba Cloud PolarDB for MySQL.</td></tr>
<tr>
<td>Host Address</td><td>The IP address to access Alibaba Cloud PolarDB for MySQL. You can obtain this address on the basic information page of Alibaba Cloud PolarDB for MySQL.</td></tr>
<tr>
<td>Port</td><td>The access port of Alibaba Cloud PolarDB for MySQL, which is 3306 by default.</td></tr>
<tr>
<td>Account</td><td>The database account of Alibaba Cloud PolarDB for MySQL. The account permissions need to meet certain requirements.</td></tr>
<tr>
<td>Password</td><td>The password of the Alibaba Cloud PolarDB for MySQL database account.</td></tr>
<tr>
