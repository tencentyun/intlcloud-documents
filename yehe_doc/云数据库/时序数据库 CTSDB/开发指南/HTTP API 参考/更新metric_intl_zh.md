...
## 请求内容
字段 tags、time、fields、options 均为 map 类型的，选填，格式请参考 API [新建 metric](https://intl.cloud.tencent.com/document/product/1100/40909)。具体要求如下：
...
>?
> - 由于历史数据不可被修改，更新字段后 metric 信息不会立即变更，需要等待下一个 [子表](https://intl.cloud.tencent.com/document/product/1100/40909#rolling) 产生。如果需要确认更新操作是否成功，可通过`GET /_metric/${metric_name}?v`接口进行确认。
