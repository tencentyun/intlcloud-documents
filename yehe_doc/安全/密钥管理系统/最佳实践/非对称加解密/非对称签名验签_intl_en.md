## Operation Process
In sensitive information transmission, the information sender can provide the identity certification through asymmetric signature verification to ensure the non-repudiation of action. The operation process is as follows:
...
2. The information sender uses the private key to generate a signature for the data to be transmitted.
...
4. After receiving the signature and data, the information recipient verifies the signature by one of the two methods below:
	 ① Calls the KMS signature verification API.
	 ② Downloads the KMS public asymmetric key, and then locally verifies the signature using GmSSL, password library, KMS SM-CRYPTO Encryption SDK, or any other tools.

>? Asymmetric signature verification currently only supports SM2 encryption algorithm.

## Operation Directions

Call the [CreateKey](https://intl.cloud.tencent.com/document/product/1030/32199) API to create a CMK. When creating the key in KMS, please pass in the correct key purpose **`KeyUsage= ASYMMETRIC_SIGN_VERIFY_SM2`** to use the signature feature.
...
- If the message to be generated a signature for is no longer than 4,096 bytes, you can skip this step to [Step 5](#test5).
...
Call the KMS signature API to calculate the signature of the information.
...
Request:
	```
	// Generate a signature for the message abstract using the content of the file `encoded.base64` as the `Message` parameter of `SignByAsymmetricKey`.
	...
	// Generate a signature for the Base64-encoded original message
...
// Verify the message abstract (verify a signature for the message abstract using the content of the file `encoded.base64` mentioned in step 4 as the `Message` parameter of `VerifyByAsymmetricKey`).
