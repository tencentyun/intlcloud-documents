
## 概述

KMS 提供 SM2 和 RSA 的非对称密钥解密接口，分别如下：

|API 名称 | API 描述  | 说明 |
|---------|---------|---------|
| AsymmetricSm2Decrypt| SM2 解密| 请参见 [AsymmetricSm2Decrypt](https://intl.cloud.tencent.com/document/product/1030/35180) |
|AsymmetricRsaDecrypt|	RSA 解密|	请参见 [AsymmetricRsaDecrypt](https://intl.cloud.tencent.com/document/product/1030/35181) |


本文示例使用腾讯云 [命令行工具 TCCLI](https://cloud.tencent.com/product/cli)，后续您可以使用任何受支持的编程语言调用。

## 非对称解密


### RSA 解密

#### 输入
```shell
tccli kms AsymmetricRsaDecrypt --KeyId 22d79428-61d9-11ea-a3c8-525400****** --Algorithm RSAES_OAEP_SHA_256 --Ciphertext "******=="
```

#### 输出
```shell
{
    "Response": {
        "RequestId": "6758cbf5-5e21-4c37-a2cf-8d47f5******",
        "KeyId": "22d79428-61d9-11ea-a3c8-525400******",
        "Plaintext": "dGVzdAo="
    }
}
```

### SM2 解密

#### 输入
```shell
tccli kms AsymmetricSm2Decrypt --KeyId 22d79428-61d9-11ea-a3c8-525400****** --Ciphertext "******=="
```

#### 输出
```shell
{
    "Response": {
        "RequestId": "6758cbf5-5e21-4c37-a2cf-8d47f5******",
        "KeyId": "22d79428-61d9-11ea-a3c8-525400******",
        "Plaintext": "dGVzdAo="
    }
}
```

## 查看公钥
#### 概述

获取指定 KeyId 的公钥信息，API 文档请参见 [GetPublicKey](https://intl.cloud.tencent.com/document/product/1030/35179)。

本文示例使用腾讯云 [命令行工具 TCCLI](https://cloud.tencent.com/product/cli)，后续您可以使用任何受支持的编程语言调用。

#### 示例

#### 输入
```shell
tccli kms GetPublicKey  --KeyId 22d79428-61d9-11ea-a3c8-525400******
```


#### 输出：
```shell
{
    "Response": {
        "RequestId": "408fa858-cd6d-4011-b8a0-653805******",
        "KeyId": "22d79428-61d9-11ea-a3c8-525400******",
        "PublicKey": "******",
        "PublicKeyPem": "-----BEGIN PUBLIC KEY-----\\ntmNypNERIR7jTCt9L+fgYOX7anmuF5v7z0GfFsen9Tqb1LsZuQR0vgqCau******\n1QIDAQAB\n-----END PUBLIC KEY-----\n"
    }
}
```

