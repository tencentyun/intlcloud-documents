This document describes how to configure rule protection in WAF to defend against web attacks.
## Background
WAF uses a regex-based rule protection engine and a machine learning-based AI protection engine to protect against web vulnerabilities and unknown threats.

WAF's rule protection engine provides expert rule sets based on Tencent Security's accumulated web threat intelligence to automatically prevent OWASP top 10 attacks. Currently, it can defend against 12 categories of web attacks, such as SQL injections, XSS attacks, malicious scanning, command injection attacks, web application vulnerabilities, WebShell uploads, non-compliant protocols, and trojans.
...
2. In the "Domain Name List", click the domain name to be protected to enter the defense settings page.
...
- **Rules Engine Switch**: it is enabled by default. If it is disabled, domain name requests that pass through WAF will not be processed by the rules engine for threat prevention.
	- **Defense Mode**: working mode of the rules engine, which is "Block" by default.
	 - **Observation**: attack requests are not blocked, with observation logs generated.
	...
	- **Defense Level**: defense level of the rules engine, which is "Strict" by default.
		- **Loose**: common web application attacks are detected. If you find that there are many false positives under the default level, or there are many uncontrollable user inputs in your business (such as websites with rich text editors), we recommend you choose this level.
...
2. On the "Protection Rule" tab, you can view the descriptions and rule updates of the attack categories that WAF currently can defend against.
...
<td>WAF can detects whether certain configuration files, database files, and parameter data are downloadable at will.</td>
</tr>
<tr>
<td>Open-source component vulnerability attack</td>
...
<tr>
<td>Web application vulnerability exploit</td>
...
<tr>
<td>Other vulnerability attacks</td>
...
3. You can view the rule updates on the right of the "Protection Rule" tab.
...
2. On the rules engine page, click **Rule Settings**. On the "Rule Management" tab, you can enable individual rules in the rules engine based on domain name. All rules are enabled by default.
3. You can search in the rule set by "rule level", "attack category" or "rule ID" to view specific rules and perform operations.
>?"Strict" covers "Normal" and "Loose", and "Normal" covers "Loose".

### Rule allowlist and false positive processing
1. Log in to the [WAF Console](https://console.cloud.tencent.com/guanjia/waf/rule) and select **Web Application Firewall** > **Rules Engine** on the left sidebar to enter the rules engine page.
2. On the rules engine page, click **Rule Settings**. On the "Allowlist" tab, you can add domain name URLs and rule IDs to the allowlist and process false positives.
3. At the top of the rule list, click **Add** to add rules to the allowlist in the "Add to Allowlist" pop-up window.
**Field description:**
...
4. After the allowlist is configured, you can view the allowed rules in the rule list and perform related operations.
**Field description:**
- No.: auto-incrementing policy number.
	- Rule ID: ID of the rule added to the allowlist, which can be obtained through attack logs or rule management.
	- Match Method: method to match the allowed URL path. You can select "Exact match", "Prefix matches with", or "Suffix matches with".
	- Path: URI path to be allowed. The URI must be unique under one domain name.
	- Enable/Disable: allowlist switch.
	- Modification Time: last time when the policy was added or modified.
	- Operation: you can edit or delete the policy.
