...
- **状态变更**
   - **方式 1**：在漏洞明细列表页，勾选目标漏洞，鼠标悬浮列表上方**状态变更**并单击状态操作，可批量变更状态。 
   - **方式 2**：在漏洞明细列表页，鼠标悬浮目标漏洞右侧操作栏的**状态变更**并单击状态操作，可变更状态。
...
- **重新检测** 
   - **方式 1**：在漏洞明细列表页，勾选目标漏洞，单击列表上方**重新检测**，可批量重新检测。 
   - **方式 2**：在漏洞明细列表页，单击目标漏洞右侧操作栏的**重新检测**，可重新检测。

  > ? 待处理和修复中的漏洞支持重新检测。

- **忽略记录**
  在漏洞管理页，单击“忽略记录”可查看已忽略检测的记录，在忽略记录列表页可取消忽略。取消忽略后将恢复对该漏洞的该资产的检测。 
    - 方式 1：在忽略记录列表页，勾选目标漏洞，单击列表上方**取消忽略**，可批量取消忽略记录。 
    - 方式 2：在忽略记录列表页，单击目标漏洞右侧操作栏的**取消忽略**，可取消对应的忽略记录。
