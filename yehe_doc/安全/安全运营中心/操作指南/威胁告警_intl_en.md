...
4. Below the "Usage Instructions" section, there is a quick filter area, main functional areas, and an area for displaying the alert list.
...
In the case of a web attack event, the correlation graph is shown in the preceding figure. In addition to dragging and dropping entity icons in the graph to view, you can also perform the following actions:
 - Click **Legend** in the lower left corner to see the meaning of each icon. Click **Legend** again to hide it.
    - IP: Indicates an IP concern that cannot be associated to the asset.
    - Asset: Indicates an IP concern that is associated with the asset.
      - Domain Name: Indicates the domain name involved in the concern, such as malicious connection, victim, and suspicious connection.
 - Click the icon buttons in the lower right corner to zoom in, zoom out, and auto-layout the correlation graph.
 - Move the pointer over an entity icon to see the details of that entity. For attacker IP addresses, you can perform one click blocking. For victim IP addresses, you can be redirected to the corresponding Policy Management page for optimizing the policy.
