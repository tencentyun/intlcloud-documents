...
**Parameter description:**
 - Disabled status (default): QUIC requests are not supported.
 - Enabled status: HTTP/3 (QUIC) requests are supported, and QUIC is used to accelerate site requests.
>!
>- This feature takes effect only after an HTTPS certificate is configured.
>- QUIC requests in excess of the package quota will be billed in pay-as-you-go mode separately.

## Notes
1. If both HTTP/2 and HTTP/3 (QUIC) are enabled, HTTP/2 or QUIC will be used based on the actual client request conditions.
