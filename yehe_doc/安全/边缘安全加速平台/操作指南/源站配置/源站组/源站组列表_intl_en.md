...
## Directions
1. Log in to the [EdgeOne console](https://console.cloud.tencent.com/edgeone). Click **Origin Configuration** > **Origin Group** on the left sidebar.
...
**Parameter description:**
 - Origin group name: Group name or description, which must be unique and only contains 1–200 letters, digits, underscores, and hyphens.
 - Origin type: Type of the origin server, which can be customer origin, object storage origin, or Tencent Cloud COS.
>?
>- Customer origin: It's an origin server that houses your deliverable content. The origin server can be an IP address or a domain name.
>- Object storage origin: It's a third-party object storage origin server, which is a bucket address that can be accessed. Only Amazon Web Services S3 is supported.
>- Tencent Cloud COS: Select a Tencent Cloud COS bucket under the current account as the origin server.
