...
4. 检查系统是否采用 **默认管理端口**：
  ...
  * 解决方法：
      ...
      4.1. 在服务器内编辑`/etc/ssh/sshd_config`文件中的 Port 22，将22修改为非默认端口，修改之后需要重启 ssh 服务。
>! 当对端口进行修改时，需同时在 [云服务器控制台](https://console.intl.cloud.tencent.com/cvm/instance/index?rid=1) 上修改对应主机的安全组配置，在其入站规则中，放行对应端口，详情请参见 [添加安全组规则](https://www.tencentcloud.com/document/product/215/35513)。

​      4.2. 运行`/etc/init.d/sshd restart（CentOS）或 /etc/init.d/ssh restart（Debian / Ubuntu）`命令重启是配置生效。
​      4.3. 修改 FTP、MySQL、Redis 等的程序配置文件的默认监听端口21、3306、6379为其他端口。
​      4.4. 限制远程登录的 IP，编辑`/etc/hosts.deny` 、`/etc/hosts.allow`两个文件来限制 IP。
