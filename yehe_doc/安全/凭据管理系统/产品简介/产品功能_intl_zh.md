...
## 凭据加密存储与传输
凭据管理系统使用 [密钥管理系统](https://intl.cloud.tencent.com/product/kms)（KMS）安全保护的主密钥（CMK）作为加密密钥，并对所管理的凭据内容进行加密存储，使用凭据时，将通过 [TLS](https://intl.cloud.tencent.com/document/product/1078/38618) 安全传输到服务器本地。
...
## 资源级访问授权
凭据管理系统与腾讯云 [访问管理](https://intl.cloud.tencent.com/product/cam)（CAM）集成，通过身份管理和策略管理方式确保只有授权用户可以访问或修改凭据，并可以将这些策略附加到用户或角色，指定这些用户或角色可以访问哪些凭据。

## 精细化监管审计
凭据管理系统与腾讯 [云审计](https://intl.cloud.tencent.com/product/cloudaudit)（CloudAudit）结合，支持对用户的腾讯云账号进行监管、合规性检查、操作审核和风险审核等，同时可记录凭据管理操作和凭据使用情况。
