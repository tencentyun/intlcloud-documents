...
>!凭据管理系统（SSM）在使用密钥管理系统（KMS）托管的密钥进行加密时，可能会产生 KMS 费用，收费说明请参见 [计费概述](https://intl.cloud.tencent.com/document/product/1078/38595)。

## 操作步骤

### 步骤1：开通 KMS 服务并进行授权
- 凭据管理系统将借助密钥管理系统（KMS）实现对敏感凭据的加密存储，因此在使用凭据管理系统前，请确认已开通 [密钥管理系统](https://intl.cloud.tencent.com/product/kms)。
