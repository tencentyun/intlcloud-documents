...
This document takes `clb.technicalsupport.cn` as an example to illustrate how to check out the CLB listener configuration.
...
3. Click **Listener Management** to view its configuration. As shown below, the listener name is `waftest` and the associated domain name is `clb.technicalsupport.cn` with WAF protection disabled.
