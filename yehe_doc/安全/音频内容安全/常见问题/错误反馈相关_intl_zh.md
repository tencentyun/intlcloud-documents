...
- 用户创建自定义词库后，需要创建一个 [自定义策略](https://intl.cloud.tencent.com/document/product/1139/44942#step4)，并将自定义词库配置到策略中，词库才会生效。完成自定义策略配置后，在调用 API 时，Biztype 入参填写创建的策略名称，创建的策略就会生效。
