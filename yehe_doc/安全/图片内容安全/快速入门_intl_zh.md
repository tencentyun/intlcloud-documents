...
子账号刚创建时，默认是没有任何权限的，需要主账号或管理员为其绑定策略，才能使子账号拥有某些云资源的操作权限。您可以为子账号配置内容安全 CAM 授权，详情请参见 [CAM 授权指引](https://intl.cloud.tencent.com/document/product/1122/43794)。
...
5. 在管理自定义库配置页面，在自定义词库下拉列表中选择是否需要使用自定义词库用于内容审核，如暂无自定义库，可单击**下一步**跳过，或保存当前策略后，前往 [步骤5：配置自定义词库](https://intl.cloud.tencent.com/document/product/1122/43811#step5)。
...
>? 自定义词库配置完成后，可以在 [步骤4：配置策略](https://intl.cloud.tencent.com/document/product/1122/43811#step4) 时将自定义词库与策略关联使用。

## 步骤6：体验服务

完成以上步骤后，您可以选择创建的审核策略，通过立即审核图片，体验图片内容安全服务。
...
2. 在服务体验页面，选择所需的应用策略，通过以下任意一种方式指定审核图片：
   - 在审核内容输入框内填入审核内容 URL 或 COS 访问地址，单击**添加网络图片**。
   - 单击**选择文件**，直接上传本地图片。
...
3. 单击**立即审核**， 在图片下方会显示该图片的审核结果。
   ![img](https://qcloudimg.tencent-cloud.cn/raw/86bf895945f9b8b3694c3eb74af3b2ba.png)

## 步骤7：接入服务

如果您需要接入图片内容安全服务，需通过调用 API 接口完成接入，详情请参考 [API 接入指引](https://intl.cloud.tencent.com/document/product/1122/43821)。
