...
4. 添加规则，在入向规则页面，可进行规则的配置，以添加入向规则为例进行说明。
   ...
   ii. 在“添加入站规则”的弹窗中，可进行规则的配置。访问源类型可选 IP 地址、地理位置、云厂商及 [地址模板](https://intl.cloud.tencent.com/document/product/1160/49863#IP)，访问目的类型可选 IP 地址、资产实例、资源标签、地址模板及资产分组，并根据规则重要性来勾选最先或者最后执行。填写好访问源和访问目的之后，再填写目的端口，选择对应的协议和要执行的策略等信息，并在描述一栏做好备注，单击**确定**，即可完成规则的配置。
>?
>- 访问目的区域： 指云上实例所在的地域
