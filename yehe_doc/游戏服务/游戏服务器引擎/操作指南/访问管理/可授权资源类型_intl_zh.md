...
游戏服务器引擎（Game Server Engine，GSE）支持资源级权限，这意味着对于某些 GSE 的操作，您可以控制何时允许用户执行操作或是允许用户使用的特定资源。下表将向您介绍 GSE 可授权的资源类型。
...
| 资源类型                              | 授权策略中的资源描述方法                              |
| :------------------------------------ | :---------------------------------------------------- |
| [游戏服务器引擎生成包相关](#test1)         | `qcs::gse:$region:$account:asset/* `                  |
| [游戏服务器引擎别名相关](#test2)           | `qcs::gse:$region:$account:alias/* `                  |
| [游戏服务器引擎舰队相关](#test3)           | `qcs::gse:$region:$account:fleet/* `                  |
| [游戏服务器引擎游戏服务器队列相关](#test4) | `qcs::gse:$region:$account:gameServerSessionQueue/* ` |

[游戏服务器引擎生成包相关](#test1)、[游戏服务器引擎别名相关](#test2) 和 [游戏服务器引擎舰队相关](#test3) 分别介绍了当前支持资源级权限的 GSE API 操作，以及每个操作支持的资源。设置资源路径时，您需要将 $region、$account 等变量参数修改为您实际的参数信息，同时您也可以在路径中使用 `*` 通配符。相关操作示例可参见 [访问控制示例](https://intl.cloud.tencent.com/document/product/1055/37779)。
