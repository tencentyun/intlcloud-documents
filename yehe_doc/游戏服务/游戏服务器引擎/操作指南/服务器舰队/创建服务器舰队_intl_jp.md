...
![](https://main.qcloudimg.com/raw/c47bfd7a581fa7152edfaec6c4ebd3c7.png)
  ...
  - リソースタイプ：プルダウンリストからアセットまたはイメージを選択します。
  - アセット名：プルダウンリストから作成済みの有効なアセットを選択します。ない場合は先に[アセットの作成](https://intl.cloud.tencent.com/document/product/1055/36674)で作成してください。
  - 説明：サーバーフリートの説明を作成し、さらに識別しやすくします（任意）。
  - アセットID、アセットのサイズ、OSは、選択した生成パッケージ名にもとづき自動的に入力されます。

- **イメージによる作成**
![](https://main.qcloudimg.com/raw/8158e3185b3b42bbd01487d1926adf6b.png)
  - 名称：サーバーフリート名を作成します。リストの中で簡単に識別できるように、意味をもつフリート名を作成することをお勧めします。
  - リソースタイプ：プルダウンリストからアセットまたはイメージを選択します。
  - イメージリソース名：プルダウンリストから作成済みの有効なイメージリソースを選択します。ない場合は先に [イメージの作成](https://intl.cloud.tencent.com/document/product/1055/39061)で作成してください。
  - 説明：サーバーフリートの説明を作成し、さらに識別しやすくします（任意）。
  - イメージID、イメージのサイズ、イメージのOSは、選択したイメージ名にもとづき自動的に入力されます。
...
### プロセス管理
  ...
  - 起動パス：アセットの中のゲーム実行可能ファイルのパスを入力します。現在の起動パスはゲームサーバーの位置 `/local/game/`を先頭につけます。
...
5. 作成に成功すると、サーバーフリートのインターフェースで確認、削除などの操作が行えるようになります。同時に、サーバーフリートのIDをクリックすれば、基本情報、イベント、インスタンスリスト、スケーリング、ゲームセッション、プロセス管理、ポートとプロトコル、アセット、VPCなどの情報を確認できます。
