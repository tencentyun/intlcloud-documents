...
4. Enter the delay policy: 
 - In the first 10s, server fleets whose delay for all players is below 80 ms can be matched and waited for preferably.  
 - In the subsequent 10s (i.e., in the first 20 seconds), server fleets whose delay for all players is below 100 ms can be matched and waited for preferably.
 - In the last 10s (= 30s - 10s - 10s) of the timeout period, server fleets whose delay for all players is below 150 ms can be matched and waited for. 
...
As the delay policy specifies that in the first 10s, servers in regions where the delay for all players is below 80 ms can be matched preferably, the game server session will be scheduled to the Silicon Valley region.
