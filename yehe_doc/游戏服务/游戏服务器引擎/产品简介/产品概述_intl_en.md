Tencent Cloud Game Server Engine (GSE) provides dedicated game server hosting services for the deployment and scaling of stateful games. It supports service discovery, flexible server scaling, and optimal resource scheduling. This product helps developers quickly build a stable and low-latency deployment environment for multiplayer games while reducing OPS costs. It can be used to deploy and run C#/C++ game engines and custom frameworks. It is ideal for stateful games that need to remember data such as combat servers and push notifications in FPS, MOBA, turn-based games, MMORPG, table games and more. 
...
### Process management 
#### Process startup  
GSE starts processes based on the process startup paths, the startup parameters, and the maximum number of allowed concurrent processes configured on the Console.

#### Process preparation  
After a process is started, an API will be called to inform GSE that the process is ready to accept access requests.
...
#### Nearby assignment of the game server session  
