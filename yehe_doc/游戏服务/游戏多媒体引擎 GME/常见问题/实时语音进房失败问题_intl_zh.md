...
只支持最多六个人同时说话。如果需要增加同时说话人数，详情请参见 [设置最大混音路数接口](https://intl.cloud.tencent.com/document/product/607/38334)。
...
问题排查的步骤如下：
1. 查看并确认进房 API 中的参数，如 Appid， UIN， AuthBuffer 的合法性（详情请参见各平台 [接口文档](https://intl.cloud.tencent.com/zh/document/product/607/18522)）。
2. 请查看控制台上的相关参数是否与本地一致。
3. 请查看控制台是否欠费。
4. 检查开发者测试设备是在开发者内网环境还是外网环境，如果是内网环境，详情请参见 [如何应对公司防火墙限制](https://intl.cloud.tencent.com/document/product/607/35232)。
...
确认是否已开启 3D 音效接口 EnableSpatializer，再确认是否已更新自身坐标：[UpdateSelfPosition 和 UpdateAudioRecvRange](https://intl.cloud.tencent.com/document/product/607/18218)。
