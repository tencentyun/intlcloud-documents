...
### 设置后台播放声音
此接口用于设置后台播放声音，例如拉起通知中心或者控制中心。此接口需要在进房前调用。
同时，应用侧有如下两点需要注意：
...
断网情况详情请参考 [实时语音房间相关问题](https://intl.cloud.tencent.com/document/product/607/35611) 。
...
此接口用于获取语音消息播放音量。返回值为 int 类型，值域为0到100。
*此接口不同于实时语音接口，此接口在 ITMGPTT 下*
...
| type    |ITMG_VOICE_TYPE                      |变声类型，请参考[变声特效](https://intl.cloud.tencent.com/document/product/607/31503)|
...
将某个 ID 加入音频数据黑名单。返回值为 0 表示调用成功。退出房间后失效。
