...
| Error Code Name | Error Code Value | Description | Cause | Solution |
|--------|----|-------|--------------------|-------------------|
...
|-----------------------------------|-----|-----|----------------------------------------------------------------------------------------------|---------------------------------------------------------|
| AV_ERR_SERVER_FAILED | 10001 | Common error | Locate the specific cause based on the real error code (in logs) returned from the backend to a client. | View and confirm the validity of the parameters in the API for room entering, such as AppID, UIN and AuthBuffer (see the documentation). In the console, check whether the relevant parameters match the local ones, and whether there is any account arrears. Check whether the developer's testing devices are running over a private or public network. |
