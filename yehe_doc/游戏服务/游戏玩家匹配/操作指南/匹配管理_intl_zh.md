...
- 已完成 [规则创建](https://intl.cloud.tencent.com/document/product/1072/39202)。
- 如果您需要创建一个“**请求 GSE 资源**”的匹配，则需完成 [游戏服务器队列创建](https://intl.cloud.tencent.com/document/product/1055/36681)。
...
<span id="test2"></span>
 ...
 - #### 无需自动连接对战的匹配：
   ...
   GPM 仅为此类型匹配进行匹配票据撮合，即按配置的规则搜索到符合条件的票据形成完整匹配。匹配结果的处置，以及匹配完成后的对战连接，需要用户自行处理。关于此类型的匹配流程，详情可参见 [独立使用的匹配](https://intl.cloud.tencent.com/document/product/1072/39218)。
