...
This document is available for GME SDK version 2.9.
</dx-alert>
...
<dx-alert infotype="alarm" title="Entered the room successfully">
...
### Muting a room member


This API is used to mute a user by specifying the ID. It only applies on the invoker side. `0` indicates the call is successful. For example, A, B, and C are all speaking in the same room: 

- A mutes C. A can only hear B.
...
This API is used to unmute a user by specifying the ID. `0` indicates the call is successful.
...
### Setting volume of a room member
