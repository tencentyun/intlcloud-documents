This document provides a detailed description that makes it easy for macOS developers to debug and integrate the APIs for Game Multimedia Engine (GME).
...
@class ITMGPTT;//Voice message APIs
```
...
The `Poll` API in `updated` should be called periodically for GME to trigger event callbacks; otherwise, the entire SDK service will not work properly. For details, see `EnginePollHelper.m` file in the Demo.
...
| Parameter | Type | Description |
| ------------- |:-------------:|-------------|
| fileID | NSString | URL path of file |
