...
[游戏服务器伸缩生成包相关](#test1)、[游戏服务器伸缩别名相关](#test2) 和 [游戏服务器伸缩舰队相关](#test3) 分别介绍了当前支持资源级权限的 GSE API 操作，以及每个操作支持的资源。设置资源路径时，您需要将 $region、$account 等变量参数修改为您实际的参数信息，同时您也可以在路径中使用 `*` 通配符。相关操作示例可参见 [访问控制示例](https://intl.cloud.tencent.com/document/product/1055/37779)。
