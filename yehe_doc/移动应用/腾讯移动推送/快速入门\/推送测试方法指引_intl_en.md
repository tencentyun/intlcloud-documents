...
| Remarks | The message will preferentially be pushed through the vendor-specific channel; and if it fails, the TPNS channel will be used for push |
...
| Remarks | This feature can be tested only through the TPNS channel. Up to 3 offline messages can be stored and retained for at most 72 hours |
...
| Remarks | Android supports customizing ringtone, vibration, and notification LED. (Note: only the TPNS channel rather than vendor-specific channels supports this feature.)<br>iOS supports customizing ringtone |
...
| Remarks | Vendor-specific channels only support tapping to open an app or custom item (intent), while the TPNS channel supports all tap actions <br>For more information on iOS push redirect, please see [Client Redirect Based on Message Content](https://intl.cloud.tencent.com/document/product/1024/32623) |
