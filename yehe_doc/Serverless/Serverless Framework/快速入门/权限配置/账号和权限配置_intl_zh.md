...
```
由于 Serverless Framework 在部署时会默认检测是否为中国用户，如果开发环境在中国境外地域，但希望使用中国版体验的 Serverless Framework，则可以在 .env 文件中增加下列配置，即可指定默认提供中国版体验，包括交互式的一键部署流程（参考 [快速入门](https://intl.cloud.tencent.com/document/product/1040/36249)）等。
```ini
# .env
...
1. [指定角色的调用权限策略](#1)
2. [使用 Serverless Framework 产品的接口权限策略](#2)
