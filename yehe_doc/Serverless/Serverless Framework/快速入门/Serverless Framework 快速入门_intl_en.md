...
## Prerequisites
- Before use, please make sure that Serverless Framework 1.67.2 or above has been installed.
- If your Tencent Cloud account is a root account, you can directly proceed with the deployment. If your account is a sub-account, please get the authorization first before deployment.
...
By running the `sls dev` command, you can enable real-time logging of the deployment. This feature will automatically monitor local code updates, sync them to the cloud for deployment, and output invocation logs in real time. For applications on Node.js 10, you can also enable in-cloud debugging.
...
## FAQs
If a proxy is configured in your environment, the guide may not pop up by default when `serverless` is entered. In this case, you can configure as follows:

Add the `SERVERLESS_PLATFORM_VENDOR=tencent` configuration item to the `.env` file.
