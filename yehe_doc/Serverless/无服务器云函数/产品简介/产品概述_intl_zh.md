...
- [云服务器（CVM）](https://intl.cloud.tencent.com/document/product/213)：以云服务器为扩展单位，虚拟化硬件设备。用户和其他租户共享物理机资源，仍可自行配置 CVM 的各项指标，相对部署和迭代更加简单。
- [容器](https://intl.cloud.tencent.com/document/product/457)：以服务为扩展单位，虚拟化操作系统。测试和生产环境完全一致，测试和部署非常轻松。
- [云函数](https://intl.cloud.tencent.com/document/product/583)：以函数为扩展单位，虚拟化运行时环境（Runtime）。是现有计算资源的最小单位，具有完全自动、一键部署、高度可扩展等特点，是轻量级服务部署非常好的选择。
...
### 多种开发语言支持，亦可使用自定义运行时
支持 Python、Node.js、Golang、PHP、Java 等语言，同时支持 [Custom Runtime](https://intl.cloud.tencent.com/document/product/583/38129)，您可以根据您的需要自定义运行时。
...
- 使用 [Serverless Framework](https://intl.cloud.tencent.com/product/sls)，在本地开发环境中进行项目创建、本地调试打包、一键部署上线。
