...
- 请确保您当前使用的腾讯云账户已完成了以下授权操作：
 1. 参考 [角色与授权](https://intl.cloud.tencent.com/document/product/583/31444) 完成 SCF 默认角色配置。
 2. 新建角色 `QCS_SCFExcuteRole` ，并参考 [用户与权限](https://intl.cloud.tencent.com/document/product/583/34877) 完成预设策略关联。
...
1. 运行 VS Code IDE。
2. 打开 VS Code 插件市场。
3. 在搜索框中输入 “Tencent Serverless”，单击搜索框下方列表中的 Tencent Serverless 插件查看详情并选择【install】。
...
3. 根据提示依次输入账号的 APPID，SecretId 及 SecretKey 信息，作为插件调用云 API 时的认证信息。并在认证成功后，选择您希望部署函数的地域。配置信息获取途径请参见 [配置 SCF CLI](https://intl.cloud.tencent.com/document/product/583/32754)。
...
>
>- 如果您的函数有使用第三方依赖，则需要将依赖包放至函数目录下然后执行上传。
>- 如果在部署时提示 Role 不存在，请前往 SCF 控制台并参考 [角色与授权](https://intl.cloud.tencent.com/document/product/583/31444) 添加 Role。
>
![](https://main.qcloudimg.com/raw/5a85fb7ab36015d8b470fe54caf466ea.png)         
...
![](https://main.qcloudimg.com/raw/399ae8de88de0cb00bbb0afcb162151d.png)
...
![](https://main.qcloudimg.com/raw/d0099db6f30e5d8b2e082abfca4ba29a.png)
...
![](https://main.qcloudimg.com/raw/a0ed134601a728b43d6abaed31a90bd5.png)  
...
1. 单击目标云端函数右侧的<img src="https://main.qcloudimg.com/raw/3c72bf2180157adcd5bc66072e1d063f.png" style="margin:-3px 0;">，将函数导入到本地。
...
本地调用时，可根据函数功能选择不同的测试模板，也可以自定义模板。
...
3. 在已创建函数的详情页面，选择【监控信息】，即可查看函数调用次数/运行时间等情况。
...
在已创建函数的详情页面，单击【前往新增告警】为云函数配置告警策略，对函数运行状态进行监控。  

更多关于配置告警请参见 [告警配置说明](https://intl.cloud.tencent.com/document/product/583/32738)。  

#### 切换账号
如果您有多个账号，可按照以下步骤在 SCF 插件页中添加新账号并切换使用：
1. 选择云端函数右侧的<img src="https://main.qcloudimg.com/raw/ae8b96f8ee48b4dc003cf8b5173e5b12.png" > > 【切换账号】。
...
安装或使用过程中有遇到问题，可参考 [SCF 工具类常见问题](https://intl.cloud.tencent.com/document/product/583/32772) 解决。    
...
3. 进入“函数配置”页面，选择【触发方式】页签，即可在 “API网关触发器” 中查看 `serviceId`。
