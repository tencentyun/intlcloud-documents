...
当前在函数代码中通过简单的 `print`，或通过 `logger` 输出的字符串内容，将会在投递到日志服务时，记录在 `SCF_Message` 字段中。日志服务的字段说明可见 [索引说明](https://intl.cloud.tencent.com/document/product/583/39778#configuring-index)。
...
在使用如下测试事件进行测试运行时，您可在日志服务的字段日志中检索 `key1` 和 `key2` 两个字段。您也可以在日志服务中启用键值索引，并使用 `key1`、`key2` 作为关键字进行检索，详情可参见 [日志服务索引配置](https://intl.cloud.tencent.com/document/product/614/39594)。
