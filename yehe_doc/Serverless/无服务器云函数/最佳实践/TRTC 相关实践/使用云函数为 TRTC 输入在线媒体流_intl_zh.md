...
本文为您介绍如何 [使用 API 网关集成云函数](https://intl.cloud.tencent.com/product/scf)，将已有的录播视频或者 RTMP 直播流推送到实时音视频 TRTC 房间进行直播，提供开箱即用、灵活便捷、可编程的在线媒体流输入能力。如您需开启推流直播的实时记录，可以选择使用 Redis，API 网关会将进度实时写入 Redis。工作流程如下图所示：
...
5. 切换至 [实时音视频控制台](https://console.cloud.tencent.com/trtc/monitor)，在“监控仪表盘”页面单击房间 ID，查看推流监控详情信息。
6. 如需在推流过程中停止推流，可以调用 [终止异步函数接口](https://intl.cloud.tencent.com/document/product/583/39733) InvokeRequestId 参数停止推流（**必须开启状态追踪**）。
