...
- 前往访问管理控制台，创建云函数的运行角色，并授予该角色 COS、CFS 的读写权限，用于授权云函数访问相应服务。
...
<td>target_path</td><td>转码后的视频，上传到 Bucket 的指定目录中。</td>
 ...
 - **运行角色**：勾选“启用”，并选择在 [前提条件](#Precondition) 中已创建的运行角色，本文以 `SCF_QcsRole` 为例。
...
8. 在弹出的“创建触发器”窗口中，参考以下信息创建 COS BUcket 触发器。
.
...
2. 单击【函数代码】页签，进入函数代码编辑页面，修改文件上传路径。
...
## 测试功能
登录 [对象存储控制台](https://console.cloud.tencent.com/cos5/bucket) ，进入对应的 Bucket 目录下，上传视频文件，并到对应的转码目录下查看，是否生成压缩的视频文件。
...
## 扩展能力
基于本文示例，您还可扩展自动化 CDN 刷新/预热的能力。例如，转码后的视频在回传 COS Bucket 时，可触发新函数执行 CDN 刷新/预热功能。
