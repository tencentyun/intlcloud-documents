...
- **同步调用**： CKafka 触发器使用同步调用类型来调用函数。有关调用类型的更多信息，请参见 [调用类型](https://intl.cloud.tencent.com/document/product/583/9694)。
...
- **最大批量消息数**：在拉取并批量投递给当前云函数时的最大消息数，目前支持最高配置为1000。结合消息大小、写入速度等因素影响，每次触发云函数并投递的消息数量不一定能达到最大值，而是处在1 - 最大消息数之间的一个变动值。
...
- **同步调用的事件大小限制**：6MB，详情请参见 [限制说明](https://intl.cloud.tencent.com/document/product/583/11637)。如果 Ckafka Topic 的消息很大，例如单条消息就已经达到6MB，那么由于同步调用的6MB限制，所以传递给云函数的事件结构中只会有一条消息，而不是用户配置的最大消息个数。
- **最大批量消息数**：同 CKafka 触发器属性，由用户设置，目前支持最高配置为1000。
