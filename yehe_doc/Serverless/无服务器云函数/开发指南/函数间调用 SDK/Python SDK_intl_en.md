...
Tencentserverless is a Tencent Cloud SCF SDK that integrates SCF business flow APIs to simplify the function invocation method. It can be used to invoke a function quickly from a local system, CVM instance, container, or function, eliminating your need to encapsulate public TencentCloud APIs.
...
* It supports Python's native invocation method.
...
>- To make functions in different regions invoke each other, regions must be specified. For the naming convention, please see [Region List](https://intl.cloud.tencent.com/zh/document/api/583/17238).
...
- Runtime environment
  Install Tencentserverless SDK, which supports Windows, Linux, and macOS.
...
2. After the function is created, create a local file named `PythonInvokeTest.py` with the following content:
``` python
# -*- coding: utf8 -*-
...
def main_handler(event, context):
	print("prepare to invoke a function!")
	scf = Client(secret_id="AKIxxxxxxxxxxxxxxxxxxxxxxggB4Sa",secret_key="3vZzxxxxxxxxxxxaeTC",region="ap-guangzhou",token=" ")# Replace with your `secret_id` and `secret_key`
	try:
		data = scf.invoke('FuncInvoked',data={"a":"b"}) 
...
Go to the file directory of `PythonInvokeTest.py`, run the following command, and view the result.
```python
 ...
 **Parameter information:**
 ...
 <table>
	<tr>
	<th>Parameter</th> <th>Required</th> <th>Type</th> <th>Description</th>
	</tr>
	<tr>
	...
	<td>secret_id</td> 	<td>No</td> 	<td><code>String</code></td>
		<td>User `SecretId`, which is obtained from the function's environment variable by default and <b>is required for local debugging</b>.</td>
	</tr>
	<tr>
	<td>secret_key</td> 	<td>No</td> 	<td><code>String</code></td>
		<td>User `SecretKey`, which is obtained from the function's environment variable by default and <b>is required for local debugging</b>.</td>
	</tr>
	<tr>
 ...
 **Parameter information:**
 ...
 <table>
	...
	<tr>
		...
		<td>Function version, which is `$LATEST` by default.</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>Namespace, which is `default` by default.</td>
	</tr>
</table>
...
This API is used to invoke a function. Currently, only sync invocation is supported.
...
| qualifier     | No       | `String` | Function version, which is `$LATEST` by default.                                   |
