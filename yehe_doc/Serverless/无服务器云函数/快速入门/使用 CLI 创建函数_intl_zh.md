...
serverless deploy
```
2. 如需配置持久的环境变量或密钥信息，请参见 [账号配置](#accountConfiguration)。
...
#### 云端调试
组件支持 Node.js 应用的云端调试能力，在进入开发模式后，将会自动监听远端端口，并将函数的超时时间临时配置为900s。此时您可以通过访问 `chrome://inspect/#devices` 查找远端的调试路径，并直接对云端代码进行断点等调试。在调试结束后，需要再次部署更新代码并还原超时时间。详情请参考 [开发模式和云端调试](https://intl.cloud.tencent.com/document/product/583/36268)。
...
```
>?
>- 如果您没有腾讯云账号，请先 [注册新账号](https://intl.cloud.tencent.com/register)。
