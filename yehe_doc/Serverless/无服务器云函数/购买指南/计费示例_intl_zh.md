...
云函数预置并发的费用只针对配置后未使用的实例计费，也就是预置并发空闲实例。对于使用中的实例，云函数仅收取弹性并发的费用，不收取额外费用。
...
预置并发闲置费用独立于其他三个计费项，配置预置并发后才可能产生费用。可以在其他示例的基础上直接增加本部分费用，本部分只对该计费项进行介绍。这里用一个并发波动较为剧烈，同时调整了预置并发配额的情况来详细说明预置并发的闲置计费。

假设每分钟的并发数没有产生变化，配置为256MB的某函数版本。在18:01配置了100预置实例；由于业务出现增长，在18:07将预置配置提升为120；而在18:10时又将预置配置降低为80。

那么以18:01这一分钟为例，这一分钟预置实例为100个，实际并发为30个，闲置实例数为100-30=70个。这70个实例这一分钟的资源闲置量为 70 × 256MB × 60s = 70 × 256/1024 GB × 60s = 1050 GBs，产生的闲置费用为 1050 × 0.00000847 美元/GBs = 0.009 美元。
