...
- 根据实际需求，在左上角设置自定义查找条件，查看您想看的运行日志。
 - **全部日志**：可选调用成功、调用失败日志。
 - **选择日期**：可查看当前日期及前20天的运行日志，暂不支持开始时间到结束时间超出 24 小时的日志检索。
 - **实时**：函数当前的运行日志。
 - **近24小时**：可查看包含当前时间内24小时的运行日志。


#### 高级检索
云函数日志检索支持关键词搜索，您可以使用查询语法组合关键词进行检索。详情请参见 [日志检索教程](https://intl.cloud.tencent.com/document/product/583/34382)。
