云函数的测试功能，用于通过控制台或 SCF 命令行直接发起函数调用，模拟触发器发送的触发事件，并展示云函数的执行情况、返回内容、运行日志。
...
>
您可通过 SCF 命令行执行 `scf Invoke` 命令，即可运行函数。详情请参见 [本地调试（native invoke）](https://intl.cloud.tencent.com/document/product/583/32760)。
