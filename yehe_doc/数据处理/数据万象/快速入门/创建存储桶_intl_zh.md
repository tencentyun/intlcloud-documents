...
2. 选择绑定已有 COS 存储桶或新建一个存储桶。
 - 若选择绑定已有 COS 存储桶，则需要指定存储桶名称。
...
- 选择已有 COS 的存储桶。通过此种方式，本质上是为一个 COS 上的存储桶开通图片处理服务。
...
存储服务支持多地域，已开放地域可参见 [地域与域名](https://intl.cloud.tencent.com/document/product/1045/33423)。创建存储桶时可选择存储桶所属地区，一旦设置不可更改。为了提高访问速度，建议选择离您的用户近的可用地域。
