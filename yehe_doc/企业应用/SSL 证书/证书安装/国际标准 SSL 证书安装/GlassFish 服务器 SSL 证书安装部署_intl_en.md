...
5. Use WinSCP (a tool for copying files between a local computer and a remote computer) to log in to the GlassFish server, and then copy the certificate file `cloud.tencent.com.crt` and the private key file `cloud.tencent.com.key` from the local directory to the `temp` folder.
...
8. In the `temp` folder, run the following command to generate the `keystore.jks` file:
```
...
10. Replace the `keystore.jks` and `cacert.jks` files in the `domain1/config` directory with the files generated in steps 9 and 10. 
11. In the `/usr/share/glassfish4/glassfish/domains/domain1/config` directory, change the port numbers in the `domain.xml` file.
```
<network-listeners>
...
```
12. Start the GlassFish server and then you can access it through `https://cloud.tencent.com`.
