...
>?CPU 利用率过高时，建议优先进行 [配置调整](https://intl.cloud.tencent.com/document/product/236/19707)，提升 CPU 规格以确保业务正常运行，后续可参考本文进行排查和优化。
...
- **异常诊断（推荐）**：7 * 24小时异常发现诊断，提供实时优化建议。操作详情请参见 [使用“异常诊断”功能排查数据库异常情况](https://intl.cloud.tencent.com/document/product/1035/36053)。
- **慢 SQL 分析**：针对当前实例出现的慢 SQL 进行分析，并给出慢 SQL 的优化建议。操作详情请参见 [使用“慢 SQL 分析”功能排查导致 CPU 利用率过高的 SQL](https://intl.cloud.tencent.com/document/product/1035/36053)。
- **审计日志分析**：利用云数据库审计数据（全量 SQL），多维度深入分析 SQL 语句并给出优化建议。操作详情请参见 [使用“审计日志分析”功能排查导致 CPU 利用率过高的 SQL](https://intl.cloud.tencent.com/document/product/1035/36053)。
