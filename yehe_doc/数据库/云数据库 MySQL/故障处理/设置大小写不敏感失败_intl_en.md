...
>!Modifying `lower_case_table_names` will cause the database to restart.
>
...
### The database version is MySQL 8.0
In MySQL 8.0, case sensitivity is enabled by default and the `lower_case_table_names` parameter can only be configured when the MySQL server is initialized. Therefore, you cannot modify the parameter after the server is initialized.
