...
云数据库 MySQL 单节点（云盘）支持快照备份。**快照备份**，通过对存储层磁盘创建快照的方式进行备份（自动备份与手动备份均支持）。
...
#### 2. 备份可以手动删除吗？
- 自动备份无法手动删除，可设置备份保留时间，到期后会自动删除。 
- 双节点、三节点实例手动备份可在 [MySQL 控制台](https://console.cloud.tencent.com/cdb) 的备份列表手动删除，单节点云盘实例手动备份暂不支持删除，没有手动删除的前提下会一直保留。
