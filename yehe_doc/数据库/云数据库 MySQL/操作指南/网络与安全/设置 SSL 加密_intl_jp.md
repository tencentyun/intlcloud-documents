...
## 背景
非暗号化方式を使用してデータベースに接続した場合は、ネットワークにおいて伝送されるすべての情報は平文となるため、不正なユーザーによる盗聴、改ざん、なりすましの3つのリスクがあります。SSLプロトコルは、この3つのリスクに対処するために設計されており、理論上次のことを達成できます。
- 情報は暗号化されて送信されます。サードパーティが盗聴することはできません。
- 検証メカニズムがあり、一度改ざんされると、通信をしている双方がすぐに発見することができます。
- 身分証明書を装備しており、IDのなりすましを防止しています。

TencentDB for MySQLは、SSL暗号化をオンにすることによってリンクの安全性の強化をサポートします。また、必要なアプリケーションサービスへSSL CA証明書をダウンロードおよびインストールすることもサポートしています。注意していただきたいのは、SSL暗号化はデータ自体を保護するものではなく、データベースとサーバー間における送受信のトラフィックの安全を確保するものです。トランスポート層でネットワーク接続を暗号化し、通信データの安全性と完全性を向上させることができますが、同時にネットワーク接続のレスポンスタイムも増加します。
...
>?SSL暗号化は、現在ベータ版テスト段階ですので、体験が必要な場合は、[チケットを提出](https://console.cloud.tencent.com/workorder/category)してください。
