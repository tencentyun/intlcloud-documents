...
The SSL protocol needs to be established based on reliable TCP and has the strength of being independent from application layer protocols; therefore, high-level application layer protocols such as HTTP, FTP, and TELNET can be transparently established on it. It completes encryption algorithm processing, communication key negotiation, and server authentication before communication is made over application layer protocols. After that, all data transferred over application layer protocols will be encrypted so as to ensure communication privacy.

## Background
When you connect to a database in an unencrypted manner, all information transferred over the network will be in plaintext and may be eavesdropped, tampered with, and impersonated by malicious users. The SSL protocol is designed to address these risks and can bring the following benefits theoretically:
...
- Identity certificates are used to prevent impersonation.

TencentDB for MySQL supports enhancing the connection security by enabling SSL encryption as well as downloading and installing SSL CA certificates to the required application services. Note that SSL encryption does not protect the data itself; instead, it ensures the security of traffic between the database and the server. Encrypting the network connection at the transport layer can improve the security and integrity of the communication data, but will increase the response time of the network connection.
...
>?The SSL encryption feature is in beta test. To try it out, [submit a ticket](https://console.cloud.tencent.com/workorder/category) for application.
...
>? If the error `connection is being used` is reported, it may be because the previous session has not been disconnected. In this case, restart Navicat and try again.
