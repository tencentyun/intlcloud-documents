...
云数据库 MySQL 支持通过开启 SSL 加密来增强链路安全性，并支持下载和安装 SSL CA 证书到需要的应用服务。注意，SSL 加密不保护数据本身，是确保来往于数据库和服务器之间的流量安全，在传输层对网络连接进行加密，能够提升通信数据的安全性和完整性，但会同时增加网络连接响应时间。
...
>?SSL 加密当前为内测阶段，如需体验，请提交 [提交工单](https://console.cloud.tencent.com/workorder/category)。
