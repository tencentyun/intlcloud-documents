...
TencentDB for MySQL은 SSL 암호화를 활성화하여 링크 보안 강화를 지원하고 SSL CA 인증서를 필요한 애플리케이션 서비스에 다운로드 및 설치할 수 있도록 지원합니다. 또한 SSL 암호화는 데이터 자체를 보호하지 않으며, 데이터베이스와 서버를 오가는 트래픽의 안전을 보장하며, 전송 레이어에서 네트워크 연결을 암호화하여 통신 데이터의 보안성과 완정성을 향상시키지만 네트워크 연결 응답 시간을 증가시킵니다.
...
>?SSL 암호화는 현재 베타 테스트 단계입니다. 체험을 원하시면 [티켓 제출](https://console.cloud.tencent.com/workorder/category) 하시기 바랍니다.
