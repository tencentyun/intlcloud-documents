데이터베이스 프록시는 커널 마이너 버전을 수시로 업데이트하므로 콘솔에서 수동으로 업그레이드해야 합니다. 본문은 콘솔을 통해 데이터베이스 프록시 커널의 마이너 버전을 업그레이드하는 방법을 소개합니다.
...
- 업그레이드가 안될 경우 데이터베이스 커널 마이너 버전이 20211031 이하일 가능성이 있습니다. 먼저 데이터베이스 커널 버전을 업그레이드 하신 후, 데이터베이스 프록시 커널 버전을 업그레이드 하시기 바랍니다.
