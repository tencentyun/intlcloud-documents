[데이터베이스 프록시 읽기/쓰기 분리](https://intl.cloud.tencent.com/document/product/236/42050) 기능을 통해 응용 프로그램에 데이터베이스 프록시 주소를 설정하면 쓰기 요청을 자동으로 프라이머리 인스턴스로 포워딩하고 읽기 요청을 자동으로 각 읽기 전용 인스턴스로 포워딩할 수 있습니다. 
...
>!
>- 실행 중 상태인 프라이머리 인스턴스와 읽기 전용 인스턴스만 데이터베이스 프록시에 추가될 수 있습니다. 
...
>?읽기 전용 인스턴스 제거 임계값과 최소 보관 수량을 설정하면 새롭게 연결된 것에 한해서만 설정이 적용이 됩니다.
   - **딜레이 제거 임계값**: 읽기 전용 인스턴스가 프라이머리 인스턴스의 데이터를 동기화하는 데 허용되는 최대 딜레이 시간입니다. 읽기 전용 인스턴스의 딜레이 시간이 이 임계값을 초과하면 읽기 전용 인스턴스의 가중치에 관계없이 읽기 요청이 읽기 전용 인스턴스로 포워딩되지 않습니다. 값 범위는 1 이상의 정수입니다.
...
>!읽기 전용 라이브러리에서 치명적인 장애(예: 시스템 다운)가 발생할 경우, 최소 보류 수량은 치명적인 장애가 발생한 인스턴스에 대하여 작용하지 못합니다.
   ...
   - **읽기 가중치 할당**: 인스턴스에 읽기 가중치를 할당하며, 시스템에서 자동으로 할당하거나 사용자 정의를 선택할 수 있으며 가중치 할당 범위는 0 - 100까지의 정수입니다. 가중치 할당 설정 읽기 후 모든 연결에 즉시 적용됩니다.
     ...
     - 가중치는 읽기 요청의 가중치만 해당되며, 쓰기 요청은 바로 프라이머리 데이터베이스로 라우팅 되어 가중치 계산에 포함되지 않습니다. 예를 들어, 클라이언트가 10개의 쓰기 명령과 10개의 읽기 명령을 발송하면 프라이머리 데이터베이스와 읽기 전용 데이터베이스의 가중치는 1:1이 되며, 프라이머리 데이터베이스는 10개의 쓰기 명령과 5개의 읽기 명령을 수신하고, 읽기 전용 데이터베이스는 5개의 읽기 명령을 수신합니다.
     - 시스템 가중치 자동 할당을 선택할 경우, 시스템은 인스턴스의 CPU와 메모리 사양에 따라 자동으로 가중치를 할당하며, 이 경우 프라이머리 인스턴스의 가중치만 설정 가능합니다. 
   ...
   - **장애 조치**: 활성화 여부를 설정합니다. 활성화를 권장합니다. 읽기 전용 인스턴스에 오류가 발생할 경우 데이터베이스 프록시가 읽기 요청을 프라이머리 인스턴스로 발송합니다. 
