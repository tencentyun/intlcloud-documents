...
- 开启数据库代理后，新增只读实例或只读实例重启，只有新的连接的请求才会路由到新的只读实例或重启的只读实例，可以通过概览或 [性能监控](https://www.tencentcloud.com/document/product/236/42055) 查看各代理节点的性能指标，若发现各代理节点连接数不均衡的现象，可通过重新负载均衡打散连接。
...
- 数据库代理各能力支持版本：
 ...
 >?升级数据库代理版本操作请参见 [升级数据库代理内核小版本](https://www.tencentcloud.com/document/product/236/45627)。
