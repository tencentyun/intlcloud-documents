...
## 機能の特徴
- 独立したデータベース接続アドレスを提供します。ディザスタリカバリインスタンスは読取アクセス機能を提供して、最寄りサーバーへのアクセス、データ分析などのシナリオに用いて、デバイスの冗長化コストを引き下げます。
...
- マスターインスタンスに障害が発生した場合、ディザスタリカバリインスタンスを数秒以内に有効化して、完全な読取および書き込み機能を回復させられます。

## 機能の制限について
- 単一ノードクラウドディスクバージョンのインスタンスは、ディザスタリカバリインスタンスの作成がサポートされていません。
- メモリ1GB、ハードディスク50GB以上の仕様で、かつMySQL 5.6およびそれ以降のバージョン、ディザスタリカバリインスタンスを購入するためのInnoDBエンジンの高可用性バージョンマスターインスタンスのみをサポートします。マスターインスタンスがこの仕様よりも低い場合は、まずマスターインスタンスの仕様をアップグレードしてください。また、マスターインスタンスでGTID機能が有効になっている必要があります。
...
- ディザスタリカバリインスタンスは現在、プロジェクトの移行、ロールバック、SQL操作、文字セットの変更、アカウント管理、ポートの変更、データインポート、ロールバックログ、読取専用インスタンス機能をサポートしません。
...
3. 購入ページにて、ディザスタリカバリインスタンスの**料金モード**、**リージョン**、**同期ポリシー**等の基本情報を設定します。
 ...
 - 同期ポリシーが**作成後に同期**の場合、ディザスタリカバリインスタンスの作成完了後、ディザスタリカバリ同期リンクを設定する必要があります。操作の詳細については、以下の[同期リンクの作成](#cjtblj)をご参照ください。
>?
...
>!
>- ディザスタリカバリインスタンスがマスターインスタンスにアップグレードされると、ディザスタリカバリインスタンスが引き継いで実行される新しいマスターインスタンスになり、新しいマスターインスタンスのアクセスアドレスが変更されます。サービス側で新しいアクセスアドレスを再設定する必要があり、元のマスターインスタンスは既存のサービスを引き継がれなくなります。
>- ディザスタリカバリインスタンスがマスターインスタンスに切り替えられると、元のマスターインスタンスとの同期接続が切断されます。同期接続が切断されると、再接続できなくなります。慎重に操作してください。
>
