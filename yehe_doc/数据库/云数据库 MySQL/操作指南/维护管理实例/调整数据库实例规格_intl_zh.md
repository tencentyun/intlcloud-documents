云数据库 MySQL 支持通过控制台快捷调整实例规格，提供灵活的扩缩容操作。您可根据业务所处的实际情况（业务初期、业务快速发展期、业务高峰期、业务低谷期等）灵活的调整 MySQL 实例的规格，从而更好地满足资源充分利用和成本实时优化等需求。调整费用变更请参见 [调整实例费用说明](https://intl.cloud.tencent.com/document/product/236/32345)。
...
- 变配时，若实例小版本不是最新，则会先升级实例内核小版本至最新，升级小版本时，是否涉及重启，您可以变配页面极速变配下的提示为准。
