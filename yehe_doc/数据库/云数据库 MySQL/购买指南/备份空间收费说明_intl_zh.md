...
## 备份价格
超出免费额度的备份空间，中国大陆地域按照0.000127美元/GB/小时的价格收费，其他地域价格请参见 [云数据库 MySQL 价格计算器](https://buy.Intl.cloud.tencent.com/price/cdb/calculator)。
...
- 备份会随实例的生命周期而相应变化。
- 实例欠费24个小时内，备份可正常进行。
- 实例欠费24个小时后，实例被隔离进入回收站，此时将停止自动备份，禁止回档、手动备份，但仍然允许进行备份下载（可在 [备份列表](https://console.cloud.tencent.com/cdb/backup) 页下载），该实例备份超出免费赠送空间的部分依然计费，直至实例下线。用户可在控制台回收站进行续费操作恢复实例与备份。
