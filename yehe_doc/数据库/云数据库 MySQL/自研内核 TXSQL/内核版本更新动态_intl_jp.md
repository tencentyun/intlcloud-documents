...
#### 新機能：
- クイック列変更機能をサポートしています。
- ヒストグラムの旧バージョンの機能をサポートしています。
- 物理テーブルのランダムサンプルを取得するために、SQL2003 TABLESAMPLE （単一テーブル）サンプリング制御構文をサポートしています。
- 非予約キーワード:TABLESAMPLE BERNOULLIを追加しました。
- 特定の入力フィールドのヒストグラムを作成するために、HISTOGRAM()関数を追加しました。
- compressedヒストグラムをサポートしています。
- SQLトラフィック制限機能をサポートしています（DBbrianスケジュールは2022年4月よりサポート）。
- MySQLクラスターロールの設定機能をサポートしています。デフォルトのロールはCDB_ROLE_UNKNOWNです。
- show replicasのコマンド実行結果に、ロールを表示するためのRole列を追加しました。
- proxyをサポートしています。

#### パフォーマンスの最適化：
- insert on duplicate key updateによるホットスポットの更新問題を改善しました。
- eventの複数の同一binlog eventを集約することにより、hash scanの適用速度を引き上げました。
- plan cacheが開いた状態におけるスレッドプールモードでは、prepareステートメントによってチェックされるメモリを大幅に削減しました。

#### バグの修正：
- ホットスポットの更新最適化を起動した後、パフォーマンスが不安定になる問題を修正しました。
- `select count(*)`並列スキャンが極端な場合に、全テーブルをスキャンする可能性がある問題を修正しました。
- さまざまな場合に統計情報が0を読み込むことにより実行計画が変更され、それにより引き起こされるパフォーマンスの問題を修正しました。
- queryが長時間にわたってquery end状態となるbugを修正しました。
- レコードが長い場合に、統計情報が著しく低く見積もられるbugを修正しました。
- Temptableエンジンを使用する場合に、選択された列の集計関数が255を超過するとエラーが出るbugを修正しました。
- json_table関数列の名称の大文字・小文字を区別してしまう問題を修正しました。
- 表現式のreturn trueのときに事前に戻ってしまうことで、ウィンドウ関数が不正確の問題を引き起こすbugを修正しました。
- derived condition pushdownがuser variablesを含む場合に、下がり続けることによって引き起こされる正確性の問題を修正しました。
- SQL filterがRule規則でnamespaceを付け加えないことにより発生がしやすくなるcrashの問題を修正しました。
- 同時実行数や競合が多い場合にスレッドプールをオンにすると、QPSがジッターする問題を修正しました。
- マスター／スレーブbpの同期が極端な場合（ホストサーバーのファイルシステムが壊れている場合）における、ファイルハンドラがリークする問題を修正しました。
- index mappingの問題を修正しました。
- 統計情報のキャッシュ同期の問題を修正しました。
- update構文の実行と移植または情報をクリアしていないストアドプロシージャによって発生するcrashの問題を修正しました。
...
- thread_handlingのスレッドモードまたはコネクションプールモードの動的設定をサポートしました。
...
### 20211102
#### 新機能：
- サードパーティデータのサブスクリプションツールを使用する場合に、内部データの整合性照合SQLのサブスクリプションが原因で、データサブスクリプションツールに異常が発生する問題を解決しました。
>?データベースインスタンスが移行、アップグレード、または障害後に再構成された後、システムはデータ整合性の照合を実行して、データの整合性を確保します。データベースの照合SQLは`statement`モードになります。一部のサードパーティのサブスクリプションツールでは、`statement`モードのSQLの処理時にエラーが発生する傾向があります。このバージョンのカーネルにアップグレードした後、サードパーティのサブスクリプションツールは、内部データの整合性照合SQLをサブスクリプションしません。
...
- equal range照会でのパーティションテーブルの誤ったロックの問題を修正しました。
...
#### 公式bugの修正：
- performance_schemaの照会時にhangする不具合を修正しました。
- digest_add_token関数のoverflowの不具合を修正しました。
- MySQLの公式truncate tableでibufアクセスによって引き起こされるcrashを修正しました。
- left joinのステートメントにおいてconstの事前計算によって引き起こされる照会の正確性に関する不具合を修正しました。
...
- fwriteでエラー処理が行われないために審査ログが不完全になる不具合を修正しました。
...
- eコマースにおける即完売シーンのホットスポットに基づく更新機能をサポートしています。
- SQLトラフィック制限をサポートしています。
...
### 20190430
#### 公式bugの修正：
- サブ照会で長いテキスト機能を使用した場合のNULLポインタ参照の不具合を修正しました。
...
- Lose index scansによって照会結果にエラーが発生するという不具合を修正しました。
...
### 20220301
#### 新機能：
- スピン周期の動的設定をサポートします。動的パラメータinnodb_spin_wait_pause_multiplierによって、スピン周期(0～100)を動的に調整します。
このパラメータは臨時調整に使用され、コンソールによる固定化や変更はサポートしていません。
- デッドロックループ情報のプリント機能をサポートしています。
パラメータinnodb_print_dead_lock_loop_infoを有効にします。有効化後にデッドロックが発生した場合は、show engine innodb statusを使用してデッドロックループ情報を確認できます。

#### バグの修正：
- slaveを再起動後のmemoryテーブルに匿名GTIDトランザクションが発生する問題を修正しました。
- root@localhost権限が失われることで、アップグレードに失敗する問題を修正しました。
- innodb_row_lock_current_waitsなどの監視変数値における異常状況の問題を修正しました。
- 審査プラグインのsql typeマッピングにエラーが発生する問題を修正しました。
...
- フルテキストインデックスにおける照会にエラーが発生するという不具合を修正しました。
...
- ファントムリードを防ぐために、ACKを待機している間、ユーザースレッドへのkill機能を禁止します。
