...
- json_table関数列の大文字・小文字を区別する問題を修正しました。
...
- リソースの競合を減らすために、アイドルタスクの自動強制終了をサポートします。この機能を有効にするには、[チケットを提出](https://console.cloud.tencent.com/workorder/category)して申請してください。　　
...
### 20200331
#### 新機能：
- 公式のMySQL 5.7.22 JSONシリーズ関数を追加しました。
...
### 20161130
#### パフォーマンスの最適化：
- lock_logロックを分割し、lock logにかかる時間を短縮し、並列処理のパフォーマンスを向上します。
