...
- 슬레이브의 generated column이 업데이트 되지 않는 현상 수정.
...
### 20210330
#### 새로운 특성:
- 마스터/슬레이브 bp 동기화 기능 지원: HA가 발생하고 마스터/슬레이브 전환이 수행될 때 대기 데이터베이스는 일반적으로 buffer pool에 핫스팟 데이터를 warmup하고 로드하는 데 비교적 오랜 시간이 걸립니다. 대기 머신의 워밍업 속도를 높이기 위해 TXSQL은 마스터/슬레이브 bp 동기화 기능을 지원합니다.
...
#### 성능 최적화:
- hash scan의 응용 프로그램 속도 최적화: slave 측에서 hash scan의 응용 프로그램 속도는 event의 여러 동일한 binlog event를 집계하여 향상시켰습니다.
...
- 마스터-슬레이브 bp 동기화 기능의 파일 핸들이 해제되지 않는 bug 수정.
...
- 대용량 트랜잭션 최적화 기능 지원을 통해 대용량 트랜잭션으로 인한 마스터/슬레이브 딜레이, 백업 실패 등 문제를 개선하였습니다.
...
#### 성능 최적화:
- 슬레이브 인스턴스의 락을 최적화하여, 해당 인스턴스의 동기화 성능을 향상시켰습니다.   
...
- 슬레이브 노드에서 복제한 필터링 조건을 동적으로 수정할 수 있도록 지원합니다.
...
- 마스터 데이터베이스의 ACK 스레드가 독립되어 응답 시간이 향상되었습니다.
