...
<li>NO_WAIT 추가 지원 | DDL 작업을 위한 WAIT [n]. MDL 잠금을 획득할 수 없고 기다려야 하는 경우 또는 MDL 잠금을 위해 지정된 시간을 기다린 경우 DDL을 즉시 롤백할 수 있습니다.</li>
...
<li>mts 교착 상태 감지 향상 기능을 지원합니다.</li>
...
<li>prepared statement에 대한 권한 확인이 최적화되었습니다. 권한 버전 번호를 나타내기 위해 전역적으로 변수를 사용하고, prepared statement은 prepare 후 버전 번호를 기록하며, 시스템은 실행 중에 버전 번호가 변경되었는지 확인합니다. 권한 변경이 없으면 시스템은 권한 확인을 건너뜁니다. 그렇지 않으면 권한을 확인하고 버전 번호를 다시 기록합니다.</li>
<li>스레드 풀(Thread Pool)에서 시간 획득의 정확도를 최적화했습니다.</li>
<li>최적화된 레코드 offset 획득. 레코드 offset은 각 index에 대해 캐시됩니다. 조건이 충족되면 캐시된 offset이 직접 사용되어 rec_get_offsets() 함수를 호출하는 컴퓨팅 오버헤드가 절약됩니다.</li>
<li>최적화된 Parallel DDL. <br>1. 인덱스 필드가 작은 경우 샘플링 주파수를 낮추기 위해 샘플링된 메모리 크기가 줄어듭니다. <br>2. 정렬에는 K-way 병합 알고리즘을 사용하여 병합 및 정렬의 라운드 수를 효과적으로 줄여 IO 수를 낮춥니다. <br>3. 레코드를 읽을 때 매번 각 레코드에 대한 offset 생성을 방지하기 위해 고정 길이 offsets가 캐시됩니다.</li>
...
<li>CREATE TABLE t1 AS SELECT ST_POINTFROMGEOHASH("0123", 4326); 명령문으로 인해 소스 복제본 연결이 끊어지는 문제가 수정되었습니다.</li>
...
<li>스레드 풀(Thread Pool)의 성능 변동을 수정했습니다.</li>
