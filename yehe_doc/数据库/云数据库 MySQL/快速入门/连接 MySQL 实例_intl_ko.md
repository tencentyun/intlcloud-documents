...
>?서로 다른 VPC(동일 계정/다른 계정, 동일 리전/다른 리전 포함)의 CVM과 데이터베이스에 대한 내부 네트워크 연결 방식은 [CCN](https://intl.cloud.tencent.com/document/product/1003)을 참고하십시오.
>
- **외부 네트워크 주소 연결**: 내부 네트워크를 통해 연결할 수 없는 경우 외부 네트워크 주소로 TencentDB for MySQL에 연결할 수 있습니다. 외부 네트워크 주소는 [수동 활성화](#waiwang)가 필요합니다. [MySQL 콘솔](https://console.cloud.tencent.com/cdb)의 인스턴스 상세 페이지에서 조회할 수 있으며 불필요한 경우 비활성화할 수 있습니다.
 - 광저우, 상하이, 베이징, 청두, 충칭, 난징, 중국홍콩, 싱가포르, 서울, 도쿄, 실리콘밸리, , 버지니아 프랑크푸르트 리전의 마스터 인스턴스는 외부 네트워크 연결 주소 활성화를 지원합니다. 읽기 전용 인스턴스는 외부 네트워크 리전 활성화를 지원하며, 콘솔을 기준으로 합니다.
...
## Windows CVM에서 연결
1. Windows CVM에 로그인한 뒤, <a href="https://intl.cloud.tencent.com/document/product/213/10516" target="_blank">Windows CVM의 빠른 구성</a>을 참고하십시오.
...
5. MySQL Workbench를 연 다음, **Database** > **Connect to Database**를 선택하여, MySQL 데이터베이스 인스턴스의 내부 네트워크(혹은 외부 네트워크) 주소, 사용자 이름 및 비밀번호를 입력한 후 **OK**를 클릭해 로그인합니다.
 - Hostname: 내부 네트워크(혹은 외부 네트워크) 주소를 입력합니다. [MySQL 콘솔](https://console.cloud.tencent.com/cdb)의 인스턴스 상세 페이지에서 내부/외부 네트워크의 주소 및 포트 번호를 확인할 수 있습니다. 외부 네트워크 주소라면 활성화 여부를 확인하시기 바라며, 자세한 내용은 [외부 네트워크 활성화](#waiwang)를 참고하십시오.
...
## Linux CVM에서 연결
1. Linux CVM에 로그인 합니다. [Linux CVM 빠른 구성](https://intl.cloud.tencent.com/document/product/213/10517)을 참고하십시오.
...
>- 포트가 3306인 경우 hostname을 IP 주소로 바꾸면 됩니다. 예를 들어 내부 네트워크 주소가 10.16.0.11:3306인 경우 10.16.0.11만 입력하면 됩니다.
...
```
      - hostname: 타깃 MySQL 데이터베이스 인스턴스의 외부 네트워크 주소로 변경합니다. [MySQL 콘솔](https://console.cloud.tencent.com/cdb)의 인스턴스 상세 페이지에서 외부 네트워크의 주소 및 포트 번호를 확인할 수 있습니다. 외부 네트워크 주소가 비활성화 상태라면, [외부 네트워크 활성화](#waiwang)를 참고하여 활성화해 주시기 바랍니다.
...
## 부록2: 네트워크 연결성 인증 방법
네트워크 연결성 문제를 신속하게 진단하고 추적할 수 있는 telnet 명령어의 사용을 권장합니다.
...
2. 인스턴스 상세 페이지의 **외부 네트워크 주소**에서 **활성화**를 클릭합니다.
