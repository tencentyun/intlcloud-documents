...
1. [MySQL購入ページ](https://buy.intl.cloud.tencent.com/cdb)にログインし、実際のニーズに応じて各設定情報を選択し、誤りがないことを確認した後、**今すぐ購入**をクリックします。
 - **課金モード**：年額/月額課金と従量制課金をサポートします。
    - 業務量に安定した長期的な需要がある場合は、年額/月額課金を選択することをお勧めします。
 ...
 - **エンジン**：InnoDBエンジンとRocksDBエンジンを選択することをサポートします。
    - InnoDB：パーフェクトなトランザクションサポートと、強力かつ高い読み取り/書き込みの同時実行機能を備えた、最もよく使われるOLTPストレージエンジン。
    - RocksDB：効率的な書き込み機能と高圧縮ストレージで知られているkey-valueストレージエンジンです。RocksDBエンジンを選択すると、アーキテクチャは2ノードとなります。
...
>? セキュリティグループのインバウンドルールはMySQLインスタンスの3306ポートを開放する必要があります。MySQLインスタンスのプライベートネットワークのデフォルトポートは3306で、カスタムポートもサポートされています。デフォルトのポート番号が変更された場合は、セキュリティグループ内で新しいポートを開放する必要があります。
 ...
 - **購入時間**：ビジネスニーズに応じて対応する時間を選択します。サブスクリプション（年額・月額課金）課金モデルを選択し、購入時間が長いほど、より大きな割引を受けることができます。
 - **自動更新**：年額/月額課金モードで、アカウントの残高が十分な場合、デバイスの有効期限が切れた後、自動的に毎月更新されます。
