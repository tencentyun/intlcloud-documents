...
모든 리전(Region)은 독립된 지리적 구역입니다.리전마다 여러 개의 상호 격리된 위치가 존재하며 이를 가용존(Zone)이라고 합니다.모든 가용존은 독립적이지만, 동일한 리전의 가용존은 저지연 내부 네트워크 링크로 서로 연결됩니다.Tencent Cloud는 사용자가 다른 위치에 클라우드 리소스를 할당하도록 지원하며, 사용자가 시스템을 설계할 때 리소스를 다른 가용존에 배치하여 개별 노드의 장애로 인해 서버를 사용할 수 없는 상황을 방지할 것을 권장합니다.
...
## 리전
Tencent Cloud는 리전별로 완전히 격리되어, 리전별 안정성과 고장 방지 능력을 최대한 보장합니다. 액세스 지연시간 단축 및 다운로드 속도 향상을 위해, 클라이언트와 가장 가까운 리전 선택을 권장합니다.사용자가 인스턴스 실행, 인스턴스 조회 등의 작업을 진행할 때 리전 속성을 구분하여 진행됩니다.
...
- [CLB](https://intl.cloud.tencent.com/document/product/214)는 기본적으로 동일 리전의 트래픽을 전달하며 로컬 리전 CVM에 바인딩합니다.[리전 간 바인딩](https://intl.cloud.tencent.com/document/product/214/38441) 기능을 활성화할 경우, CLB의 리전 간 CVM 바인딩을 지원합니다.

## 가용존
가용존(Zone)이란 Tencent Cloud가 동일한 리전 내에서 상호 독립적인 전력과 네트워크를 보유한 물리적인 데이터센터를 말합니다.가용존의 장애를 서로 격리(대규모 재해 혹은 대규모 전력 장애 제외)하여 장애가 확산되지 않으며, 사용자가 온라인 서비스를 유지하도록 지원합니다.독립 가용존 내의 인스턴스를 실행하면 사용자는 응용 프로그램이 개별 위치 장애의 영향을 받지 않게 할 수 있습니다.
사용자가 인스턴스를 실행하면 지정한 리전에서 임의의 가용존을 선택할 수 있습니다. 사용자가 애플리케이션 시스템의 높은 안정성을 필요로할 경우(인스턴스 장애 발생 시 서비스 지속 사용 보장), 가용존 통합 배포 솔루션(예: [CLB](https://intl.cloud.tencent.com/document/product/214), [EIP](https://intl.cloud.tencent.com/document/product/213/5733) 등)을 사용하여 다른 가용존의 인스턴스가 관련 요청을 대신 처리하도록 할 수 있습니다.
...
<tr><th>리전</th><th>가용존</th></tr>
<tr>
<td rowspan="4">아시아 태평양 동남부(싱가포르)<br>ap-singapore</td>
...
<tr>
<td>싱가포르 3존(싱가포르 노드로 아시아 태평양 동남부 지역 커버 가능)<br>ap-singapore-3</td></tr>
<tr>
<td>싱가포르 4존(싱가포르 노드로 아시아 태평양 동남부 지역 커버 가능)<br>ap-singapore-4</td></tr>
<tr>
...
<td >방콕1존(방콕 노드로 아시아 태평양 동남부 지역 커버 가능)<br>ap-bangkok-1</td></tr>
<tr>
<td >방콕2존(방콕 노드로 아시아 태평양 동남부 지역 커버 가능)<br>ap-bangkok-2</td></tr>
<tr>
...
<tr>
<td rowspan="2">아시아 태평양 동북(도쿄)<br>ap-tokyo</td>
