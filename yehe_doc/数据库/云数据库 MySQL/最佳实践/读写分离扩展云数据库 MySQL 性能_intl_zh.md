...
>?只读实例剔除设置阈值和实例最小保留数后，仅对新连接生效。
  - **延迟剔除阈值**：只读实例同步主实例数据时允许的最长延迟时间。若一个只读实例的延迟时间超过该阈值，则不论该只读实例的权重是多少，读请求都不会转发至该只读实例。取值范围为大于或等于1的整数。
    - 若只读实例延迟超过阈值时，则会被剔除，被剔除的实例权重自动设置为0，且系统会向用户发出告警（须先订阅“数据库代理挂载节点剔除”告警，配置请参见 [告警功能](https://intl.cloud.tencent.com/document/product/236/8457)）。
...
## 相关文档
- [设置数据库代理连接地址](https://intl.cloud.tencent.com/document/product/236/42053)
- [切换数据库代理网络](https://intl.cloud.tencent.com/document/product/236/45628)
- [设置会话级连接池](https://intl.cloud.tencent.com/document/product/236/45623)
