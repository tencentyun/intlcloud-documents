...
## 操作步骤
1. 登录 [TDSQL-C 购买页](https://buy.intl.cloud.tencent.com/cynosdb?regionId=8)，选择所要部署的地域、可用区以及数据库规格等，单击**立即购买**。
 - **计费模式**：支持包年包月、按量计费、Serverless 模式。
 - **网络**：出于性能安全考虑，目前仅支持私有网络（VPC），云服务器需要与 TDSQL-C 在同一 [VPC](https://intl.cloud.tencent.com/document/product/215) 下方可通信。
 - **兼容数据库**：支持 MySQL 5.7、8.0 和 PostgreSQL 10。
 - **计算实例数**：建议至少选择两个实例保证集群高可用，集群创建后可通过增加只读实例扩展集群的读能力。
 - **实例规格**：数据库规格和价格请参见 [计费概述](https://intl.cloud.tencent.com/document/product/1098/40620)。
 - **存储计费**：
...
## 后续操作
购买 TDSQL-C 集群后，可通过集群内外网地址或数据管理平台连接 TDSQL-C 集群，请参见 [连接 TDSQL-C 集群](https://intl.cloud.tencent.com/document/product/1098/40627)。
