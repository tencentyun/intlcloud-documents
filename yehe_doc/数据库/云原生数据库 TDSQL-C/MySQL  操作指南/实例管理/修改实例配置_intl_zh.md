...
本文为您介绍如何修改 TDSQL-C MySQL版 的计算规格与存储。

## 变配能力
TDSQL-C MySQL版 采用计算和存储分离的架构，所有计算节点共享一份数据。在跨机变配的场景下，完全无需搬迁数据，所以能够支撑秒级的配置升降级能力。
...
实例变配操作亦可指定变配的自动执行时间。
 ...
 - 选择**维护时间内**，则会在实例的**维护窗口期**内进行闪断切换。修改维护窗口期可参考 [修改实例维护窗口](https://intl.cloud.tencent.com/document/product/1098/44631)。
