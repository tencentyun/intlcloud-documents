...
1. 登录 [TDSQL MySQL版 购买页](https://console.cloud.tencent.com/tdsqld/tdmysql-buy)，根据实际需求选择各项配置信息，确认无误后，单击**立即购买**。
 ...
 - **实例版本**：请参见 [实例架构](https://intl.cloud.tencent.com/document/product/1042/33319)。
 - **主/从可用区**：选择主备不同可用区，可保护数据库以防发生故障或可用区中断。
 - **内核版本**：5.6内核暂不支持分布式事务（XA），如需请选择 5.7版本内核，详细介绍请参见 [概述](https://intl.cloud.tencent.com/document/product/1042/38142)。
 - **分片数量/规格/硬盘**：建议分片数量选择2、4、8，否则会导致数据倾斜的问题，分片配置请参见 [分片配置](https://intl.cloud.tencent.com/document/product/1042/33354)。
 - **所属项目**：选择数据库实例所属的项目，缺省设置为默认项目。
 - **标签**：便于分类管理实例资源，请参见 [标签概述](https://intl.cloud.tencent.com/document/product/651/13334)。
 - **安全组**：安全组创建与管理请参见 [云数据库安全组](https://intl.cloud.tencent.com/document/product/1042/33348)。
 - **实例名**：可选择创建后命名或立即命名。
 ...
 - **开启强同步**：支持强同步(可退化)、异步，详细介绍请参见 [强同步](https://intl.cloud.tencent.com/document/product/1042/33318)。
 - 计费详情请参见 [计费概述](https://intl.cloud.tencent.com/document/product/1042/35777)。
2. 支付完成后，返回实例列表，待实例状态变为**运行中**，即可进行后续操作。
![](https://qcloudimg.tencent-cloud.cn/raw/4ad820a453b3b55b9b3039b4ab13655a.png)

## 后续操作
通过内外网地址连接 TDSQL MySQL版 实例，请参见 [连接实例](https://intl.cloud.tencent.com/document/product/1042/33338)。
