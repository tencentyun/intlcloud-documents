...
### 添加只读实例要多久？是否会影响业务？
每增加一个只读实例不到30秒，对业务无影响，添加时间不随实例存储空间的增大而延长。关于如何添加只读实例，详情请参见 [创建只读实例](https://intl.cloud.tencent.com/document/product/1098/40631)。
...
### 如何在线添加字段和索引？
支持 MySQL 原生 online DDL、开源工具 pt-osc 和 gh-ost，同时 TDSQL-C MySQL 版针对特定场景也支持 Instant DDL 操作。详细可参见 [Instant DDL 功能介绍](https://intl.cloud.tencent.com/document/product/1098/44589)。

### 若只写入数据，是否支持 bulk insert？一次最多支持 insert 多少个 values？
支持。一次最多支持的 values 数量由 max_allowed_packet 参数值决定。您可在控制台修改 max_allowed_packet 参数来指定，此参数可修改范围为1024-1073741824，且必须为1024的倍数。详细操作请参见 [设置实例参数](https://intl.cloud.tencent.com/document/product/1098/44602)。
