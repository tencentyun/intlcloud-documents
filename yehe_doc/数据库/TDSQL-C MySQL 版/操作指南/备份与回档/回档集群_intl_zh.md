...
云原生数据库 TDSQL-C 支持库表级别回档到原集群和 [整集群级别回档（克隆）](https://intl.cloud.tencent.com/document/product/1098/40635)到新集群，用户可以根据业务选择不同的回档方式。本文为您介绍如何将云原生数据库 TDSQL-C 部分库表回档到原集群。
...
2. 在库表回档到原集群页面，选择所需回档的库表，并重命名，单击【回档】。
 - 库表回档需明确指定所需回档的库表，若无法确定所有涉及的库表，建议 [克隆](https://intl.cloud.tencent.com/document/product/1098/40635) 回档到一个新集群确认后再迁移回原集群。
