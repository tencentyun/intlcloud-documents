TDSQL-C MySQL 版支持库表级别回档到原集群和 [整集群级别回档（克隆）](https://www.tencentcloud.com/document/product/1098/48392)到新集群，用户可以根据业务选择不同的回档方式。本文为您介绍如何将 TDSQL-C MySQL 版部分库表回档到原集群。
...
2. 在库表回档到原集群页面，选择所需回档的库表，并重命名，单击**回档**。
   - 库表回档需明确指定所需回档的库表，若无法确定所有涉及的库表，建议 [克隆](https://www.tencentcloud.com/document/product/1098/48392) 回档到一个新集群确认后再迁移回原集群。
...
5. 任务提交后，可单击**查看回档任务**或在任务列表中查看回档进度和任务详情。
