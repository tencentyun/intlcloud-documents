...
## [Action](id:caozuo)
In a CAM policy statement, you can specify any API operation from any service that supports CAM. APIs prefixed with `cynosdb:` are used for TDSQL-C for MySQL, such as `cynosdb:DescribeClusters` or `cynosdb:ResetAccountPassword`.
...
- **resource** describes detailed resource information of each product, such as `instance/clusterId` or `instance/*`.
