...
>
 ...
 - **服务条款**：阅读并勾选，详细请参见 [云数据库服务条款](https://intl.cloud.tencent.com/document/product/409/35547)。
2. 购买完成后，返回 [实例列表](https://console.cloud.tencent.com/postgres)，待实例状态变为**运行中**，即可进行连接操作。

## API 购买
通过 API 购买云数据库的用户，可参考 [创建实例（新）](https://intl.cloud.tencent.com/document/product/409/40583)，创建完成后，即可进行连接操作。
>?如果您使用 [旧云 API 接口创建实例](https://intl.cloud.tencent.com/document/api/409/16771)，则需要 [初始化实例](https://intl.cloud.tencent.com/document/product/409/16774) 后，才能进行连接操作。

## 后续操作
您可以使用标准的 SQL 客户端，通过内网地址或外网地址连接到云数据库 PostgreSQL，请参见 [连接 PostgreSQL 实例](https://intl.cloud.tencent.com/document/product/409/34626)。
