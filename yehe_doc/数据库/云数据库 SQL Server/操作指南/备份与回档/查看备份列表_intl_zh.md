云数据库 SQL Server 会按照默认的备份设置自动备份数据，您也可以通过控制台对实例进行自动备份设置修改或手动备份设置，备份文件及相关信息可通过备份列表进行查阅。
本文为您介绍通过控制台查看备份的方法。
