...
1. **从您的账户余额小于0时刻起：**
   ...
   - 24小时后，云数据库将**自动关机**且停止扣费，被移入回收站隔离。
2. **自动关机后：**
   - 关机隔离在回收站24小时内，若充值至余额大于等于0，可以开机并继续使用实例；若您的账户余额小于0，则无法开机。具体操作，请参见恢复已隔离实例。
   - 关机24小时后，若您的账户余额仍为小于0的状态，云数据库将被回收，所有数据均会被清理，且不可找回。云数据库回收时，系统将通过邮件及短信等方式通知到腾讯云账户的创建者以及全局资源协作者、财务协作者。
