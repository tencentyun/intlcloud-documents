...
- 复制组内的实例跨地域复制，将收取跨地域复制数据的带宽费用。具体信息，请参见 [腾讯云对等连接定价](https://intl.cloud.tencent.com/document/product/553/35174)。

## 新建全球复制组
### 前提条件
- 已 [创建云数据库 Redis 实例](https://intl.cloud.tencent.com/document/product/239/37712)，且实例状态为**运行中**。
...
## 相关可用性说明
### 跨地域容灾
在一个复制组内，加入一个主实例和一个只读实例，搭建跨地域容灾系统。该系统不会自动执行灾备切换，您需通过控制台（或者通过云 API）进行切换。具体操作，请参见可 [切换实例角色](https://intl.cloud.tencent.com/document/product/239/45604)。
