...
## 副本说明
内存版（标准架构）支持0 - 5个副本，以满足在不同场景下业务对可用性和性能的不同要求。内存版（标准架构）所有的副本都会参与系统高可用支持，因此副本数越多可用性越高。当副本数大于1时，可以开启读写分离，通过副本节点扩展读性能。
...
**副本只读（读写分离）**：
- 支持版本：从4.0内存版（标准架构）开始，在副本数大于1时，可开启自动读写分离，在垂直方向提供读性能扩展，最大支持5个副本节点。
...
## 使用限制
- 内存版（标准架构）支持0.25GB - 60GB规格，需要更大的规格的容量请选择集群版，集群版最大可支持4TB的容量。
...
## 命令兼容性说明
命令支持详情请参见 [命令兼容性](https://intl.cloud.tencent.com/document/product/239/31958)。
