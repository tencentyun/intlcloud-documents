...
## A-E 
**Backup storage**
It is used to persistently store the underlying storage resources of database data and log backups.
**Read/write separation**
It allows the primary instance to perform transaction operations such as insertion, update, and deletion (INSERT, UPDATE, and DELETE) and the secondary instance (read-only) to perform SELECT query operations.
...
**Database instance**
A database instance is a standalone database environment running in the cloud as the basic data block in TencentDB, which can contain multiple databases created by users and be accessed via the same client tools and applications as those for a standalone database instance.
**Database engine**
