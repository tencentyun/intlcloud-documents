...
- 重启数据库有几率失败，这是正常现象，如果重启时间超过10分钟建议通过 [联系我们](https://intl.cloud.tencent.com/contact-us) 咨询。

### 如何销毁实例？
您可在 [PostgreSQL 控制台](https://console.cloud.tencent.com/postgres) 的实例列表手动销毁。

### PostgreSQL 版本如何升级？
云数据库 PostgreSQL 暂不支持版本升级，如需进行主要版本升级，请使用 [数据传输服务](https://intl.cloud.tencent.com/document/product/571/42640) 进行实例数据迁移的方式进行升级。
