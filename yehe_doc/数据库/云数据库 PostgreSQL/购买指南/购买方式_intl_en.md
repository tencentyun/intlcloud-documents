...
## Prerequisites
You have [registered a Tencent Cloud account](https://cloud.tencent.com/register?s_url=https%3A%2F%2Fcloud.tencent.com%2F) and [completed identity verification](https://console.cloud.tencent.com/developer).
...
1. Log in to the [TencentDB for PostgreSQL purchase page](https://buy.Intl.cloud.tencent.com/pgsql), specify the database instance information as needed, confirm that everything is correct, and click **Buy Now**.
 ...
 - **AZ**: Physical IDCs where electric power facilities and networks are independent from each other within the same region. To minimize delay, we recommend the same AZ as the CVM instance to be connected to.
 - **Network**: The network where the instance is deployed. To minimize delay, we recommend the same network as the CVM instance to be connected to. Both multi-AZ deployment (the primary and standby nodes are in different AZs) and single-AZ deployment (the primary and standby nodes are in the same AZ) are supported. Specific primary and standby AZs are as displayed on the actual purchase page.
...
>
 ...
 - **Backup Space**: You will receive 50% of the instance capacity for free to use as backup and log capacity. Any usage exceeding this complimentary capacity is currently free as well.
...
## Purchasing via API
For more information on how to purchase a TencentDB instance via an API, see [CreateInstances](https://intl.cloud.tencent.com/document/product/409/40583). After the instance is created, it can be connected to.
>? If you use the [CreateDBInstances](https://intl.cloud.tencent.com/document/api/409/16771) API to create an instance, you need to [initialize the instance](https://intl.cloud.tencent.com/document/product/409/16774) first before connecting to it.
