...
详细实例监控功能和告警功能的介绍，请参见 [监控功能](https://intl.cloud.tencent.com/document/product/409/7564) 和 [告警功能](https://intl.cloud.tencent.com/document/product/409/7563)。

### 参数设置
在**参数设置**页面，您可批量或单个修改参数，也可查询最近修改记录，详细功能请参见 [设置实例参数](https://intl.cloud.tencent.com/document/product/409/43239)。

### 账号管理
在**账号管理**页面，您可以对管理账号进行管理，如修改备注，重置密码。

### 安全组
在安全组页面，您可对实例进行安全组生效对象查询，配置安全组，规则预览等操作，具体详见 [管理安全组](https://intl.cloud.tencent.com/document/product/409/40112)。
>?云数据库 PostgreSQL 安全组不支持出站规则，入站仅对当前数据库服务中的内网地址和端口相关的规则生效。


### 备份管理
在**备份管理**页面，您可以进行查看和下载备份和 xlog 操作，详细介绍请参见 [备份数据](https://intl.cloud.tencent.com/document/product/409/34628)。

### 性能优化
在**性能优化**页面，您可以查看和下载慢查询、错误日志。

### 只读实例
在**只读实例**页，您可以查询和新建只读实例 在新建只读实例时可指定或新建 RO 组。
