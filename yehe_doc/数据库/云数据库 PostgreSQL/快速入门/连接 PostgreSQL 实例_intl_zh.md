...
- **内网连接**：使用云服务器 CVM 访问自动分配给云数据库的内网地址，这种连接方式使用内网高速网络，延迟低。CVM 和数据库须是同一账号，且同一个[ VPC](https://intl.cloud.tencent.com/document/product/215/535) 内（保障同一个地域），或同在基础网络内。
>?对于不同的 VPC 下（包括同账号/不同账号，同地域/不同地域）的 CVM 和数据库，内网连接方式请参见 [对等连接](https://intl.cloud.tencent.com/document/product/553/18827)。
...
### Windows 操作系统连接方法
1. [登录到 Windows 云服务器](https://intl.cloud.tencent.com/document/product/213/10516) 或在本地，下载并安装一个标准的 SQL 客户端。
...
### Linux 操作系统连接方法
1. [登录到 Linux 云服务器](https://intl.cloud.tencent.com/document/product/213/10517) 或在本地 Linux 服务器中，通过yum 源安装一个 psql 客户端。
2. 安装 psql 客户端可参考 [安装 PostgreSQL 数据库](https://intl.cloud.tencent.com/document/product/409/11642) 的描述来进行安装，安装完成之后 psql 客户端也会同时被安装成功。
...
### [附录：开启外网访问地址](id:kqww)
1. 登录 [PostgreSQL 控制台](https://console.cloud.tencent.com/pgsql)，在实例列表，单击实例 ID 或**操作**列的**管理**，进入实例详情页面。2. 在实例详情页的**基本信息**里找到**外网IPv4地址**，单击**开启**。
