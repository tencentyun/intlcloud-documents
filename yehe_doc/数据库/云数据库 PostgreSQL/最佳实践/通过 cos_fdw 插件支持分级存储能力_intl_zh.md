...
postgres=>
```
3. PostgerSQL 客户端⼯具安装完成后，进⾏ COS 挂载。这里我们通过 COSFS 挂载到服务器上的形式来进行，可避免需要更⼤容量的 CVM 进⾏转储上传。请参见 [通过 COSFS 挂载](https://intl.cloud.tencent.com/document/product/436/6883)。
...
- code：出错请求的 HTTP 状态码。
- 错误请求的 HTTP header：显示错误的信息。其格式参见 [公共响应头部](https://intl.cloud.tencent.com/document/product/436/7729)。其中 `x-cos-request-id` 可以⽤于寻求 [提交工单](https://console.cloud.tencent.com/workorder/category) 排查问题。如果该项为空，表示未成功向 COS 发送请求。
