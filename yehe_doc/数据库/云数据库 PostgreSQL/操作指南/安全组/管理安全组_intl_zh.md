...
<table>
	...
	<tr><td>自定义</td><td>安全组创建成功后，按需自行添加安全组规则。具体操作请参见下文“添加安全组规则”</a>。</td><td>建议选择此模版，自定义一个关于访问指定实例的模板。</rd></tr>
</table>
 - 名称：自定义设置安全组名称。
...
#### 案例
**场景：**您创建了一台云数据库 PostgreSQL，并希望通过云服务器 CVM 访问云数据库 PostgreSQL。
**解决方法：**添加安全组规则时，在入站规则中配置TCP:5432协议端口的访问权限。
您还可以根据实际需求，放通全部 IP 或指定 IP（IP 段），配置可通过 CVM 访问云数据库 PostgreSQL 的 IP 来源。
