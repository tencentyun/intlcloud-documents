## 云数据库默认角色
因为安全原因，腾讯云数据库 PostgreSQL 未开放如 superuser 等角色属性或某些特殊默认角色供用户使用。但因为部分操作必须要求 superuser 进行，所以云数据库 PostgreSQL 提供了 tencentdb_superuser 角色。
创建实例所指定的管理员角色均是拥有 tencentdb_superuser 的角色。

## tencentdb_superuser 角色
...
- 插件：可创建所有支持的插件。创建插件时，当前 tencentdb_superuser 会临时提权为 superuser，通过所有的权限检查。 
- load_file 权限仅可以加载支持的插件库。
- 开放 pgstat_get_backend_current_activity 函数中查看死锁的详细信息的权限，方便用户自己排查死锁。
- 限制 pg_signal_backend 函数，只有 tencentdb_superuser 才能 kill tencentdb_superuser 的进程。

## 权限操作
详细权限操作可以参考PostgreSQL社区官方文档：
