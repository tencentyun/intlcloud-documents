...
## 前提条件
- 已创建主实例，请参见 [创建 PostgreSQL 实例](https://intl.cloud.tencent.com/document/product/409/40953)。
- 已创建只读实例，请参见 [创建只读实例](https://intl.cloud.tencent.com/document/product/409/39545)。

## [创建只读实例 RO 组](id:cjzdslrz)
1. 登录 [PostgreSQL控制台](https://console.cloud.tencent.com/postgres)，在实例列表对应实例的**操作**列，选择**更多** > **新建只读实例**，进入只读实例购买页。
![](https://qcloudimg.tencent-cloud.cn/raw/6495563cd3e7f4b4fb179498ea7c7d56.png)
>?您也可单击实例 ID 或**操作**列的**管理**，进入实例管理页面。
>- 在实例详情页，单击**实例架构图**中的**添加只读实例**，进入只读实例购买页。
>- 或在只读实例页，单击**新建**，进入只读实例购买页。
![](https://main.qcloudimg.com/raw/bcc375d86e0cbccea0c51fc473dab6da.png)
2. 在购买页，选择只读实例的相应配置，确认无误后，单击**立即购买**。
...
3. 返回实例列表，创建的实例状态为**发货中**，待状态切换成**运行中**，则表示只读实例创建成功。
