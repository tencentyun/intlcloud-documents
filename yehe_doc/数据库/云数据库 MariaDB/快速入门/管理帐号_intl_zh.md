...
3. 在弹出的对话框，输入帐号名、主机、密码、备注，单击**确认，下一步**。
 - **创建为只读帐号**：选中**是**表示该帐号只能使用读请求（select）。
...
>
 ...
 - **连接数限制**：5.7及8.0版本数据库支持创建帐号时设置，帐号最大并发连接数限制；帐号最大连接数受到实例最大连接数限制。
![](https://qcloudimg.tencent-cloud.cn/raw/18c3c140da03dc162cca5b1e78ef6cde.png)
4. 若设置为只读账号，还需要设置只读账号的连接设置。
5. 在弹出的对话框，设置数据库权限，单击**保存设置**，也可以选择**之后设置**。
