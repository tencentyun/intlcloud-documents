...
CTSDB 中 metric（表）、tags（维度列）、fields（数据列） 的命名应简明扼要，tags、fields 详细介绍可参见 [请求内容](https://intl.cloud.tencent.com/document/product/1100/45525)：
...
- 写入数据时新增字段处理
  ...
  ：如果您往 metric 表写入数据时有未定义的新增字段，CTSDB 默认会将新增字段存储为 tags。您也可通过修改 metric 的 options 选项的 default_type 字段来修改，修改 metric 请参考[更新 metric](https://intl.cloud.tencent.com/document/product/1100/45523)。

  - 若新增字段为整型，则 CTSDB 将其存储为 long 类型。
  - 若新增字段为小数，则 CTSDB 将其存储为 float 类型。
  - 若新增字段为字符串，则 CTSDB 将其存储为 string 类型，其长度为 max_string_length，超过部分会被系统丢弃，max_string_length 的长度可自定义，默认为256个字符，修改请参考 [更新 metric](https://intl.cloud.tencent.com/document/product/1100/45523)。

- **日期与时间处理**：日期与时间在 CTSDB 中以 UTC 格式存储，因此，查询数据时涉及时间范围的查询请通过 time_zone 参数来指定时区，其格式为 ISO 8601 UTC 偏移（例如 +01:00 或 -08:00），具体时区需根据实例所处的地域来确定，一般国内地域是东八区。具体请参考 [常用查询示例](https://intl.cloud.tencent.com/document/product/1100/45518)。
