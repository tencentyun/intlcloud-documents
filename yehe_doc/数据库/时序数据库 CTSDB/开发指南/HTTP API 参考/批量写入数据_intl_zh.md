...
### 请求参数
可通过设置 filter_path 参数来过滤并简化返回结果，具体请参考 [批量查询数据](https://intl.cloud.tencent.com/document/product/1100/40903)。
...
>?上面返回中的 errors 为 false，代表所有数据写入成功。items 数组标识每一条记录写入结果，与 bulk 请求中的每一条写入顺序对应。items 的单条记录中，status 为 2XX 代表此条记录写入成功，\_index 标识了写入的 metric [子表](https://intl.cloud.tencent.com/document/product/1100/40909#rolling)，\_shards 记录副本写入情况，上例中 total 表示两副本，successful 代表两副本均写入成功。
