...
| 字段名称  | 描述                                                         |
| --------- | ------------------------------------------------------------ |
| hits      | 返回匹配的查询结果，其中 total 字段表示匹配到的数据条数。里面的 hits 字段为数组结构，若无指定，则包含所有查询结果的前十条。hits 数组里的每个结果中包含 \_index（查询涉及到的 [子表](https://intl.cloud.tencent.com/document/product/1100/40909#rolling)），若查询中指定了 docvalue_fields，则会返回 fields 字段具体指明每个字段的值。 |
| took      | 整个查询耗费的毫秒数。                                       |
