...
## 前提条件
- 已 [创建 TDSQL-C MySQL](https://intl.cloud.tencent.com/document/product/1098/40626)。
...
- 在导出视图结构时，DTS 会检查源库中 `DEFINER` 对应的 user1（ [DEFINER = user1]）和迁移目标的 user2 是否一致，如果不一致，则会修改 user1 在目标库中的 `SQL SECURITY` 属性，由 `DEFINER` 转换为 `INVOKER`（ [INVOKER = user1]），同时设置目标库中 `DEFINER` 为迁移目标的 user2（[DEFINER = 迁移目标 user2]）。如果源库中视图定义过于复杂，可能会导致任务失败。
...
<li>专线接入：源数据库可以通过 <a href="https://cloud.tencent.com/document/product/216">专线接入</a> 方式与腾讯云私有网络打通。选择此场景时，需要 <a href="https://intl.cloud.tencent.com/document/product/571/42651">配置 VPN 和 IDC 之间的互通</a>。</li>
<li>VPN接入：源数据库可以通过 <a href="https://intl.cloud.tencent.com/document/product/1037">VPN 连接</a> 方式与腾讯云私有网络打通。选择此场景时，需要 <a href="https://intl.cloud.tencent.com/document/product/571/42651">配置 VPN 和 IDC 之间的互通</a>。</li>
<li>云数据库：源数据库属于腾讯云数据库实例。</li>
<li>云联网：源数据库可以通过 <a href="https://intl.cloud.tencent.com/document/product/1003">云联网</a> 与腾讯云私有网络打通。选择此场景时，需要 <a href="https://intl.cloud.tencent.com/document/product/571/42650">配置通过云联网实现 VPC 和 IDC 之间的互通</a>。</li></ul>对于第三方云厂商数据库，一般可以选择公网方式，也可以选择 VPN 接入，专线或者云联网的方式，需要根据实际的网络情况选择。</td></tr>
<tr>
