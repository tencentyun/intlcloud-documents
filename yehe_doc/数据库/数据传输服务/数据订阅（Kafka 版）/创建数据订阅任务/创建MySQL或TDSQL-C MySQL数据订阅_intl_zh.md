...
## 前提条件
- 已准备好待订阅的腾讯云数据库，并且各数据库版本符合要求，请参见 [数据订阅支持的数据库](https://intl.cloud.tencent.com/document/product/571/42663)。
- 已在源端实例中开启 binlog。
...
5. 在订阅类型和对象选择页面，选择订阅类型，单击**保存配置**。
 ...
 - 使用自定义分区策略：用户根据自己的需求自定义分区，详情内容请参考 [设置分区策略](https://intl.cloud.tencent.com/document/product/571/49934)。
...
>?如果校验失败，请 [校验不通过处理方法](https://intl.cloud.tencent.com/document/product/571/42552) 进行修正，并重新进行校验。
>
...
9. 订阅实例进入运行中状态之后，就可以开始消费数据。Kafka 的消费需要进行密码认证，我们提供了多种语言的 Demo 代码，也对消费的主要流程和关键的数据结构进行了说明。
