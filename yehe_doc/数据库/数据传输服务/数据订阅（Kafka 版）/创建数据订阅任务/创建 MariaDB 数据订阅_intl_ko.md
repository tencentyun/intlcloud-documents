본 문서는 TencentDB for MariaDB DTS에서 데이터 구독 작업을 생성하는 방법을 설명합니다.
...
grant SELECT, REPLICATION CLIENT,REPLICATION SLAVE,PROCESS on *.* to '계정'@'%';
flush privileges;
```
...
- DTS에서 Kafka 메시지에 대한 구독의 전달 의미는 메시지를 적어도 한 번(at least once) 전달하는 것입니다. 따라서 특별한 경우에 소비된 데이터가 중복될 수 있습니다. 예를 들어 구독 작업이 다시 시작되면 다시 시작한 후 중단 오프셋 전에 원본 Binlog가 당겨져 메시지가 반복적으로 전송됩니다. 등록된 객체 수정 및 비정상 작업 복원과 같은 콘솔 작업은 중복된 메시지를 유발할 수 있습니다. 비즈니스가 중복 데이터에 민감한 경우 소비 Demo에서 비즈니스 데이터를 기반으로 중복 제거 로직을 추가해야 합니다.
...
>?인증에 실패할 경우 [검증 불통과 처리 방법](https://intl.cloud.tencent.com/document/product/571/42552)의 안내에 따라 문제를 해결하고 다시 인증을 시작합니다.
>
...
9. 구독 인스턴스가 실행 중 상태가 되면 데이터 사용을 시작할 수 있습니다. Kafka에서 사용하려면 비밀번호를 확인해야 합니다. 구체적인 예는 [데이터 소비 Demo](https://intl.cloud.tencent.com/document/product/571/48488)를 참고하십시오. 당사는 여러 프로그래밍 언어로 된 Demo 코드와 주요 소비 프로세스 및 주요 데이터 구조에 대한 설명을 제공합니다.
