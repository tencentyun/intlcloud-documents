...
4. 데이터베이스 구독 설정 페이지에서 적합한 설정을 선택하고 **다음 단계**를 클릭합니다.
 ...
 - kafka 파티션 수: kafka 파티션의 수를 설정합니다. 숫자를 늘리면 데이터 쓰기 및 소비 속도가 향상될 수 있습니다. 단일 파티션은 메시지 순서를 보장할 수 있지만 여러 파티션은 보장할 수 없습니다. 소비 중 메시지 순서에 대한 엄격한 요구 사항이 있는 경우 이 값을 1로 설정하십시오.
...
9. 구독 인스턴스가 실행 중 상태가 되면 데이터 사용을 시작할 수 있습니다. Kafka에서 사용하려면 비밀번호를 확인해야 합니다. 구체적인 예는 [데이터 소비자 Demo](https://intl.cloud.tencent.com/document/product/571/39538)를 참고하십시오. 당사는 여러 프로그래밍 언어로 된 Demo 코드와 주요 소비 프로세스 및 주요 데이터 구조에 대한 설명을 제공합니다.
