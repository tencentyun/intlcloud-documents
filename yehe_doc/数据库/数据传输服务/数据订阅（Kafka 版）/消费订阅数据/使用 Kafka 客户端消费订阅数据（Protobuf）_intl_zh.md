...
操作步骤：
1. 创建新版数据订阅任务，详情请参见 [数据订阅 Kafka版](https://www.tencentcloud.com/document/product/571/47354)。
...
其中，`broker` 为数据订阅 Kafka 的内网访问地址，`topic` 为数据订阅任务的订阅 topic，这两个可在 [订阅详情](https://intl.cloud.tencent.com/document/product/571/42660) 页查看，`group`、`user`、`password` 分别为消费组的名称、账号和密码，可在 [消费管理](https://intl.cloud.tencent.com/document/product/571/39535) 页查看，`trans2sql` 表示是否转换为 SQL 语句，java 代码中，携带该参数表示转换为 SQL 语句，不携带则不转换。
6. 观察消费情况。
...
5. 运行 `./subscribe --brokers=xxx --topic=xxx --group=xxx --user=xxx --password=xxx --trans2sql=true`。
  其中，`broker` 为数据订阅 Kafka 的内网访问地址，`topic` 为数据订阅任务的订阅 topic，这两个可在 [订阅详情](https://intl.cloud.tencent.com/document/product/571/42660) 页查看，`group`、`user`、`password` 分别为消费组的名称、账号和密码，可在 [消费管理](https://intl.cloud.tencent.com/document/product/571/39535) 页查看，`trans2sql` 表示是否转换为 SQL 语句。
6. 观察消费情况。
