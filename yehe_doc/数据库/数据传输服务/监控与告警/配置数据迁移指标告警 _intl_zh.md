...
用户可以通过云监控平台对数据迁移任务中重要的指标设置告警规则，在指标发生异常时，云监控平台及时通知用户采取措施。  

本操作用于指导用户设置指标告警的通知规则，包括指标告警触发的条件、指标告警的范围、通知的形式、通知的时段、通知的用户群组等。

## 新增告警策略

1. 登录 [云监控控制台](https://console.cloud.tencent.com/monitor)。
2. 在左侧导航选择**告警配置** > **告警策略**，进入告警策略配置页面。
...
1. 登录 [云监控控制台](https://console.cloud.tencent.com/monitor)。
2. 在左侧导航选择**告警配置** > **告警策略**，然后单击需要修改的策略名称 ID，进入管理告警策略页面。
