## 操作场景
用户可以通过云监控平台对数据迁移任务中重要的指标和事件设置报警规则，在指标异常或者事件触发时，云监控平台及时通知用户采取措施。 

对一些特别重要的事件，DTS 支持默认告警配置，即任务创建后系统自动进行事件监控，不需要用户配置，在事件触发后自动通知用户。目前支持“数据迁移任务中断”事件默认告警配置。
...
<table>
<tr><th width="10%">配置类型</th><th width="15%">配置项</th><th width="80%">说明</th></tr>
<tr>
...
<tr>
<td>策略类型</td>
<td>选择您需要监控的云产品策略类型，此处选择<strong>数据传输服务/数据迁移</strong>。
<ul>
...
<tr>
<td>手动配置<br>（事件告警）</td>
<td>选择需要监控的事件，系统中已默认配置了“数据迁移任务中断”事件。</td></tr>
<tr>
<td>选择模板</td>
