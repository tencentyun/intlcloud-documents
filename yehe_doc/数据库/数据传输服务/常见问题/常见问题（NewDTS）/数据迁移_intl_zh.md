...
支持，进行跨腾讯云账号下 TencentDB 实例间的迁移，需要以目标 TencentDB 实例所属的腾讯云账号登录 DTS。具体操作请参见 [云数据库跨账号实例间迁移](https://intl.cloud.tencent.com/document/product/571/42646)。
...
### 为什么全量迁移前后数据的大小不一致？
因为源库和目标库的碎片空间不一样，源库可能存在一些空洞数据，因此全量迁移完成后目标库的表存储空间很可能会比源库的表存储空间小。建议用户迁移完成后使用 [数据一致性校验](https://intl.cloud.tencent.com/document/product/571/42724) 来核对源库和目标库内容是否一致。
...
### MySQL 数据迁移可以迁移用户权限吗？
支持，新版本 DTS 支持对用户的权限进行迁移，具体操作请参考 [账号迁移](https://intl.cloud.tencent.com/document/product/571/48483)。
