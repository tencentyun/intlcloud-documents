...
## 后续步骤
1. 用户在 [数据迁移任务](https://intl.cloud.tencent.com/document/product/571/42645) 或 [数据同步任务](https://intl.cloud.tencent.com/document/product/571/47344) 中需要配置相关参数，此处以 MySQL 的数据迁移为示例。在数据迁移任务的**设置源和目标数据库**步骤中，选择 **云联网**，关键参数配置说明如下：
...
2. 单击**测试连通性**。如果出现测试不通过，请按照如下指导进行排查。
   - Telnet 测试不通过。
     ...
     - 如果不能 ping 通。
      - [源数据库所在服务器设置了安全组或防火墙](https://intl.cloud.tencent.com/document/product/571/42552)。
 ...
 - Telnet 测试通过，Database Connect 失败。
     - 迁移帐号授权问题。请参考 [数据迁移](https://intl.cloud.tencent.com/document/product/571/42645)、[数据同步](https://intl.cloud.tencent.com/document/product/571/47344) 中的对应场景，重新对迁移帐号授权。
     - 帐号密码不正确。
