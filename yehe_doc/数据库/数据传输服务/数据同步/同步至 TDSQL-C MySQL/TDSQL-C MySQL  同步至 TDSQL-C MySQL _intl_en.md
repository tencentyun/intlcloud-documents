...
## Use limits
- Only basic tables and views can be synced, and objects such as functions, triggers, and procedures are not supported.
...
<li>On TDSQL-C for MySQL 5.6 or later, if the `gtid_mode` variable is not `ON`, an alarm will be triggered. We recommend you enable `gtid_mode`.</li>
