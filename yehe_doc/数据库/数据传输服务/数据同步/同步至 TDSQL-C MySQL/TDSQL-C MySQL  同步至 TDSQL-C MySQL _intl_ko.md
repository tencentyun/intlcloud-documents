본문은 DTS의 데이터 동기화 기능을 사용하여 TDSQL-C for MySQL에서 TDSQL-C for MySQL로 데이터를 동기화하는 작업 가이드를 소개합니다.

다음 시나리오의 데이터 동기화 요구 사항은 TDSQL-C for MySQL에서 TDSQL-C for MySQL로의 데이터 동기화 요구 사항과 동일합니다. 지침은 이 문서를 참고하십시오.

- MySQL에서 TDSQL-C for MySQL로 데이터 동기화
- TDSQL-C for MySQL에서 MySQL로 데이터 동기화
...
- 뷰를 내보낼 때 DTS는 원본 데이터베이스의 `DEFINER`([DEFINER = user1])에 해당하는 user1이 동기화 사용자 user2와 동일한지 확인하고, 그렇지 않은 경우 DTS는 대상에서 user1의 `SQL SECURITY` 속성을 변경합니다. 데이터베이스를 `DEFINER`에서 `INVOKER`([INVOKER = user1])로 변경하고 대상 데이터베이스의 `DEFINER`를 동기화 사용자 user2([DEFINER = user2])로 설정합니다. 원본 데이터베이스의 뷰 정의가 너무 복잡하면 작업이 실패할 수 있습니다.
- 원본 TDSQL-C 데이터베이스가 비 GTID 데이터베이스인 경우 DTS는 이에 대한 HA 스위치를 지원하지 않습니다. 전환되면 DTS 증분 동기화가 중단될 수 있습니다.
...
<td>
<ul><li>원본 및 타깃 데이터베이스를 연결할 수 있어야 합니다.</li>
...
<li>TDSQL-C for MySQL 5.6 이상에서 gtid_mode 변수가 ON이 아니면 알람이 트리거됩니다. gtid_mode를 활성화하는 것이 좋습니다.</li>
...
<tr>
<td>원본 인스턴스 유형</td><td>TDSQL-C for MySQL을 선택합니다. 한 번 구성하면 변경할 수 없습니다.</td></tr>
<tr>
<td>원본 인스턴스 리전</td><td>원본 인스턴스 리전을 선택합니다. 구성한 후에는 변경할 수 없습니다.</td></tr>
<tr>
<td>타깃 인스턴스 유형</td><td>TDSQL-C for MySQL을 선택합니다. 한 번 구성하면 변경할 수 없습니다.</td></tr>
<tr>
...
<tr>
<td>액세스 유형</td>
<td>실제 상황에 따라 선택합니다. 이 시나리오에서는 ‘TencentDB’를 선택합니다.
<ul>
...
<tr>
<td>인스턴스 ID</td><td>원본 데이터베이스 인스턴스 ID.</td></tr>
<tr>
<td>계정</td><td>원본 데이터베이스 계정.</td></tr>    
<tr>
<td>비밀번호</td><td>원본 데이터베이스 비밀번호.</td></tr>       
<tr>
...
<tr>
<td>인스턴스 ID</td><td>대상 데이터베이스 인스턴스 ID.</td></tr>
<tr>
<td>계정</td><td>대상 데이터베이스 계정.</td></tr>    
<tr>
<td>비밀번호</td><td>대상 데이터베이스 비밀번호.</td></tr>
</tbody></table>
