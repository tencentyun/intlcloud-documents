...
因为多对一同步是通过创建多个单向同步任务来构建多对一拓扑，所以单向同步的约束、操作限制等要求都需要满足，请参考 [数据同步](https://intl.cloud.tencent.com/document/product/571/42579) 中的对应同步场景。
...
### 创建同步任务一（实例 A > 实例 C）
1. 登录 [数据同步购买页](intl.cloud.tencent.com/migration)，选择相应配置，单击**立即购买**。
<table>
