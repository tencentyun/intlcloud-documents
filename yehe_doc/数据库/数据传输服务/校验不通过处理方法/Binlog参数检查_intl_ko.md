...
- 원본 데이터베이스가 복제 데이터베이스인 경우 ‘log_slave_updates’ 변수를 ‘ON’으로 설정해야 합니다.
- 최소 3일 동안 원본 데이터베이스의 Binlog를 유지하는 것이 좋습니다. 그렇지 않으면 체크포인트에서 작업을 재개할 수 없으며 실패합니다.
...
1. 원본 데이터베이스에 로그인합니다.
2. 원본 데이터베이스의 'my.cnf' 설정 파일을 다음과 같이 수정합니다. log_bin 매개변수를 수정한 후 데이터베이스를 다시 시작해야 하므로 검증 요구 사항에 따라 binlog_format 및 binlog_row_image 매개변수도 수정하는 것이 좋습니다.
...
>?이 매개변수에 대한 변경 사항은 데이터베이스에 대한 모든 연결이 재설정된 후에만 적용됩니다. 원본 데이터베이스가 복제본인 경우 현재 비즈니스 연결이 수정 전 모드에서 계속 데이터를 쓰지 못하도록 원본/복제본 동기화 SQL 스레드를 다시 시작해야 합니다.

1. 원본 데이터베이스에 로그인합니다.
2. 다음 명령어를 실행하여 `binlog_format`을 수정합니다.
```
set global binlog_format = ROW;
```
3. 스레드를 다시 시작하여 구성을 적용합니다. 그런 다음 다음 명령을 실행하여 매개변수 수정이 적용되는지 확인합니다.
```
...
‘binlog_row_image’ 매개변수는 binlog가 데이터 플래시백 및 원본-복제본 복제와 같은 기능에 직접적인 영향을 미치는 사전 이미지(수정 전 콘텐츠) 및 사후 이미지(수정 후 콘텐츠)를 기록하는 방법을 결정합니다.
...
1. 원본 데이터베이스에 로그인합니다.
2. 다음 명령을 실행하여 `binlog_row_image`를 수정합니다.
```
...
### gtid_mode 매개변수 수정
GTID(Global Transaction Identifier, 글로벌 트랜잭션 식별자)는 binlog에서 트랜잭션을 고유하게 식별합니다. GTID를 사용하면 반복되는 트랜잭션 실행으로 인한 데이터 무질서 또는 원본-복제본 불일치를 방지할 수 있습니다.
...
알람은 마이그레이션 또는 동기화 작업에 영향을 미치지 않지만 비즈니스에는 영향을 미칩니다. GTID가 설정된 후 증분 데이터 동기화 중에 원본 데이터베이스에서 HA 전환이 발생하면 DTS가 전환되고 다시 시작되어 작업에 거의 감지되지 않습니다. GTID가 설정되어 있지 않으면 연결이 끊긴 후 작업이 실패하고 복구할 수 없습니다.
...
- ‘OFF’: 마스터 데이터베이스의 모든 새 트랜잭션과 복제본 데이터베이스의 모든 트랜잭션은 익명이어야 합니다.
- ‘OFF_PERMISSIVE’: 마스터 데이터베이스의 모든 새 트랜잭션은 익명이어야 합니다. 복제본 데이터베이스의 트랜잭션은 익명 또는 GTID 트랜잭션일 수 있지만 GTID 트랜잭션만 될 수는 없습니다.
- ‘ON_PERMISSIVE’: 마스터 데이터베이스의 모든 새 트랜잭션은 GTID 트랜잭션이어야 하며 복제본 데이터베이스의 트랜잭션은 익명 또는 GTID 트랜잭션일 수 있습니다.
- ‘ON’: 마스터 데이터베이스의 모든 새 트랜잭션과 복제본 데이터베이스의 모든 트랜잭션은 GTID 트랜잭션이어야 합니다.

유사한 알람이 발생하면 다음과 같이 수정하십시오.
1. 원본 데이터베이스에 로그인합니다.
2. 원본 데이터베이스와 복제본 데이터베이스 모두에서 ‘gtid_mode = OFF_PERMISSIVE’를 설정합니다.
   v5.7.6 이전의 MySQL 버전에서는 `my.cnf` 구성 파일의 매개변수를 수정하고 데이터베이스를 다시 시작해야 변경 사항이 적용됩니다. v5.7.6 이상에서는 데이터베이스를 다시 시작할 필요 없이 전역 이름 지정을 통해 매개변수를 수정할 수 있지만 모든 비즈니스 연결을 재설정해야 합니다.
```
set global gtid_mode = OFF_PERMISSIVE;
```
3. 원본 및 복제본 데이터베이스 모두에서 ‘gtid_mode = ON_PERMISSIVE’를 설정합니다.
```
...
1 row in set (0.00 sec)
```
5. 원본 및 복제본 데이터베이스 모두에서 ‘gtid_mode = ON’으로 설정합니다.
```
set global gtid_mode = ON;
```
6. `my.cnf` 파일에 다음 내용을 추가하고 데이터베이스를 다시 시작하여 초기 값이 적용되도록 합니다.
>?`my.cnf` 구성 파일의 기본 경로는 `/etc/my.cnf`입니다. 실제 조건에 따라 입력합니다.
>
```
gtid_mode = on
...
1. 원본 데이터베이스에 로그인합니다.
2. 다음 명령을 실행하여 `server_id`를 수정합니다.
```
set global server_id = 2;  //1보다 큰 정수로 설정하는 것이 좋습니다. 여기서 값은 예시일 뿐입니다.
```
...
do_db 및 ignore_db가 설정된 후 일부 데이터베이스 간 작업은 binlog에 기록되지 않으며 원본-복제본 복제는 비정상적이므로 설정하지 않는 것이 좋습니다. 유사한 오류가 발생할 경우 다음과 같이 수정하십시오.
1. 원본 데이터베이스에 로그인합니다.
...
원본-복제본 재사용 구조에서 'log-bin' 매개변수가 복제본 데이터베이스에서 활성화된 경우, 복제본 데이터베이스에서 직접 수행된 데이터 작업은 binlog에 기록될 수 있지만 원본 데이터베이스에서 복제본 데이터베이스로의 데이터 복제는 기록할 수 없습니다. 따라서 복제본 데이터베이스를 다른 복제본 데이터베이스의 원본 데이터베이스로 사용하려면 'log_slave_updates' 매개변수를 활성화해야 합니다. 
1. 원본 데이터베이스에 로그인합니다.
2. 원본 데이터베이스의 'my.cnf' 구성 파일에 다음 내용을 추가합니다.
>?`my.cnf` 구성 파일의 기본 경로는 `/etc/my.cnf`입니다. 실제 조건에 따라 입력합니다.
>
```
