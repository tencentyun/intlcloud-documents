...
### Redefining and renaming data type
```
...
TDSQL-A for PostgreSQL supports synced renaming for tables or fields referenced in a view to avoid affecting data:
```
...
DROP VIEW
\# Use `cascade` to forcibly delete the dependent objects
...
(6 rows)
```
Materialized view data is stored on CNs, and each CN has the same data.
