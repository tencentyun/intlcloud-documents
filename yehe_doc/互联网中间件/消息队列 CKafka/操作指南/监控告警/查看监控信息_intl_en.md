...
The following lists the CKafka monitoring metrics and their description. Based on user feedback and statistics, we recommend you configure monitoring and alarming policies for metrics marked with the word **recommended**. Please configure policies properly as needed.
<dx-tabs>
::: Instance
...
| Production peak bandwidth in MB/s **(recommended)** | Peak traffic generated when the instance produces messages (excluding the traffic generated by replicas). |
| Consumption peak bandwidth in MB/s **(recommended)** | Peak traffic generated when the instance consumes messages (consumption does not involve replicas). |
...
| Consumption traffic in MB | Consumption traffic in the partition (excluding traffic generated by replicas), which is the total traffic in the selected time period. |
...
| Production traffic in MB | Production traffic in the partition (excluding traffic generated by replicas), which is the total traffic in the selected time period. |
:::
::: Consumer\sGroup
