...
3. 指定新建资源栈的方式：
   ...
   - **私有模版**：选择私有模版，了解更多模版详情，请参见 [模版管理](https://intl.cloud.tencent.com/document/product/1043/37185)。
   - **公共模版**：选择公共模版，了解更多模版详情，请参见 [模版管理](https://intl.cloud.tencent.com/document/product/1043/37185)。
   - **输入模版内容**：编辑模式，直接编辑基础架构代码，支持多文件编写。
...
2. 为了使模板更加通用，我们创建了 variable.tf，定义了默认地域为 ap-chengdu（成都），默认可用区为 ap-chengdu-1（成都一区）两个变量（variable），地域名称要与 [步骤1](#step1) 中选择的地域保持一致：
   ...
   ```
3. variable.tf 定义的变量 ，将在其他 tf 文件被引用，我们举 cvm.tf 为例对语法进行简要的说明，完成 tf 配置文件内容，请下载 [tic-demo-config.zip](https://tic-demo-1259649581.cos.ap-nanjing.myqcloud.com/templates/tic-demo-config.zip)。
   ```
   // Create cvm
...
2. 在 TIC 执行实际的资源销毁操作之前，会显示待销毁的资源信息，资源栈一旦被销毁，资源栈中所欲的资源将无法恢复。
