...
| -------- | --------- |
| CPU（核）  |      0.3152美元/核/天   |
| 内存（GB） |   0.0998128美元/GB/天   |
| 存储（GB） |   0.00183919美元/GB/天   |
...
各资源单价：CPU（0.3152美元/核/天）、内存（0.0998128美元/GB/天）、存储（0.00183919美元/GB/天），当日费用按各资源峰值进行计算。

2020年8月1日，该用户产生的费用为 28 × 0.3152 + 56 × 0.0998128 + 1450 × 0.00183919 = 17.0819423美元/天，并在次日上午进行扣费。
...
腾讯云边缘计算机器在创建实例时，可选择免费开通主机安全加固和腾讯云可观测平台。
- 主机安全加固：安装组件开通云镜主机防护基础版。
- 腾讯云可观测平台：云产品监控、分析和实施告警，安装组件获取主机监控指标。
