...
>?用户开启 CDN 加速域名后，使用 CDN 加速域名进行数据的下载访问，将会产生 CDN 回源流量、CDN 流量，详情请参见 [COS 作为 CDN 源站时产生的流量](https://intl.cloud.tencent.com/document/product/436/33776)。
...
- 访问节点：用户在创建一个存储桶后，COS 会自动为存储桶分配一个XML 访问节点，该访问节点形式为&lt;BucketName-APPID>.cos.&lt;Region>.myqcloud.com，适用于 RESTful API 访问。用户可以通过访问域名，并查看 [API 文档](https://intl.cloud.tencent.com/document/product/436/7751) 的执行规范，对存储桶进行配置，或进行对象的上传、下载操作。
...
您仍然可以通过在 CDN 控制台开启 [鉴权配置](https://intl.cloud.tencent.com/document/product/228/35237) **有限度的**保护存储桶中的数据，因为无论是否开启 CDN 中的该功能，知晓存储桶访问域名的用户仍可以访问到存储桶中的所有对象。对于不同 CDN 鉴权配置，域名对公有读存储桶的访问能力见下表：
...
添加 CDN 服务授权并开启回源鉴权后，CDN 边缘节点将可以直接获取并缓存数据，因此强烈建议在您有保护私有数据的需求时，开启 [鉴权配置](https://intl.cloud.tencent.com/document/product/228/35237) 保护存储桶中的数据。对于不同 CDN 鉴权配置，域名对私有读存储桶的访问能力见下表：
