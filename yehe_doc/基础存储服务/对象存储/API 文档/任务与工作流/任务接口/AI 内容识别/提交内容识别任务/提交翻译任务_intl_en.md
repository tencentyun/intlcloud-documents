...
| BasicType         | Request.Input | Original file type | String | No   | This parameter can be used only if `Type` is `pdf`, `jpg`, `jpeg`, or `png`.<br/>If `Type` is `pdf`, only values `docx` and `pptx` are supported.<br/>If `Type` is `jpg`, `jpeg`, `png`, or `webp`, only value `txt` is supported. If this parameter is left empty, the translation result is returned.                                                                                                             |
...
| NoNeedOutput | Request.Operation | Whether to output results without generating a result file. Valid values: `true`, `false`. This parameter is valid if the original file is an image. | String    | No                         |
