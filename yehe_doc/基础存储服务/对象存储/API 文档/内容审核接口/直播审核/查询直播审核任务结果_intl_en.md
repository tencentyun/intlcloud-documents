...
| SubLabel           | Response.JobsDetail.Snapshot.*Info | This field indicates the specific sub-label hit by the moderation job; for example, `SexBehavior` is a sub-label under the `Porn` label.<br/>Note: This field may return null, indicating that no specific sub-labels are hit. | String  |
