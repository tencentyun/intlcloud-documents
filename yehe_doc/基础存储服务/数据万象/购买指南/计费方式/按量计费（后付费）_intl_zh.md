按量计费（后付费）方式是数据万象（Cloud Infinite，CI）默认计费方式，支持 [所有地域](https://intl.cloud.tencent.com/document/product/436/6224)。根据用户的流量、基础图片处理量、内容识别次数等具体用量进行计费，对用户账户按月进行扣费结算。


## 产品定价

关于 **CI** 的按量计费定价信息，请参见 [产品定价](https://intl.cloud.tencent.com/pricing/ci)。
您可根据自身业务需求预估计费项的用量，通过 [CI价格计算器](https://intl.cloud.tencent.com/pricing/ci/calculator) 进行计算，并导出预算清单。
