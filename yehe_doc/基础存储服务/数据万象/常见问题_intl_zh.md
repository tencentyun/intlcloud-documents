...
### 数据万象可以处理哪些格式的图片？
数据万象目前支持处理 JPG、BMP、GIF、PNG、WEBP 格式，并且支持 HEIF 格式的解码和处理操作，更多介绍请参见 [规则与限制](https://intl.cloud.tencent.com/document/product/1045/33425)。
...
如果是子账号，还有可能是未授予相应的操作权限，授权指引可参考 [授权子账号接入数据万象服务](https://intl.cloud.tencent.com/document/product/1045/33450)。
...
### 图片处理如何收费？
图片基础处理服务（例如裁剪、缩放、转码、水印等功能）按照0.025元/GB 收费。增值服务，例如内容识别、盲水印、Guetzli 等功能需额外收费。费用详情请参见 [计费与定价](https://intl.cloud.tencent.com/document/product/1045/33431)。
...
可能原因是原图 EXIF 信息过多，占了大部分图片容量，一次调整图片分辨率后，图片大小没有太大的变化。可使用 Strip 参数，过滤 EXIF 信息。具体使用方法请参照 [去除元信息](https://intl.cloud.tencent.com/document/product/1045/33725)。
...
1. 使用此功能需要在数据万象控制台开启 [Guetzli 图片压缩](https://intl.cloud.tencent.com/document/product/1045/42359) 功能，请确认 Guetzli 图片压缩服务状态是否为开启状态。
...
3. 单击**开始配置**按钮，在弹窗中添加 `https://prvsh.myqcloud.com`与您的Cosbucket域名例如`examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com`，并单击**下载校验文件**，然后 [联系我们](https://intl.cloud.tencent.com/contact-sales) 并提供校验文件进行配置。
