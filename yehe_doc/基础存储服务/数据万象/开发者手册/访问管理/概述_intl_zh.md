您可以通过 [访问管理（Cloud Access Management，CAM）](https://intl.cloud.tencent.com/document/product/598) 创建用户或角色进行访问管理。通过关联 CAM 预设策略或自定义策略，您可以决定哪些用户在何种条件下，对哪些资源执行具体操作组合，最终实现对名下用户的访问管理。
