...
## 操作场景
下文将详细介绍腾讯云 COS 间迁移，如何配置全托管迁移任务和半托管迁移任务，实现数据迁移。
...
4. 设置要迁移的文件来源。
   此处迁移源服务提供商应选择`腾讯云COS`，并在下方 AccessKey，SecretKey 文本框中输入先前新建用于迁移的子账号 AccessKeyID 和 AccessKeySecret。填入密钥后，单击“迁移桶名称”下拉框右侧的**刷新**按钮，即可获取源对象存储桶列表。
