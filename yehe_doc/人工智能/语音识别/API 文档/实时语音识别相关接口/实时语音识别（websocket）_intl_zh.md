...
| 内容 | 说明 |
|---------|---------|
| 语言种类 | 支持中文普通话、英语、日语、印度尼西亚语。可通过接口参数 engine_model_type 设置对应语言类型|
| 支持行业 | 通用、金融、游戏、教育、医疗 |
...
| engine_model_type | 是 | String | 引擎模型类型<br>非电话场景：<br>• 16k_zh：16k 中文普通话通用；<br>• 16k_en：16k 英语；<br>• 16k_ja：16k 日语；<br>• 16k_id 印度尼西亚语； |
