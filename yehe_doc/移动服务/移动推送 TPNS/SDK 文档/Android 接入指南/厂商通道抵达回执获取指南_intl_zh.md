为了帮助客户全链路分析推送效果，移动推送 TPNS 提供厂商通道抵达数据展示。国内不同厂商通道对消息抵达回执的支持程度不同，部分通道抵达数据不能直接获取，需要开发者进行相应配置。
...
2. 配置 HTTPS 证书，[下载证书](https://api.tpns.tencent.com/v3/tpnscert/download)。
