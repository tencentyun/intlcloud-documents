...
| 推送通道| 配置说明|
| --------    | -------  |
| TPNS    |   <li>App 端，调用 Android SDK 创建 Channel ID 接口创建 Channel ID。</li><li>调用 TPNS [服务端 API](https://intl.cloud.tencent.com/document/product/1024/33764) 时，指定对应的 Channel ID（不限额度）。</li>|
| 华为    | <li>App 端，调用 Android SDK 创建 Channel ID 接口创建 Channel ID。</li><li>调用 TPNS[服务端 API](https://intl.cloud.tencent.com/document/product/1024/33764) 时，指定对应的 Channel ID（不限额度）。</li>  |
...
| 魅族    | 无 Channel 相关说明。|
| vivo   | <li>调用服务端 API 创建 Channel ID 。</li><li>调用 TPNS [服务端 API](intl.cloud.tencent.com/document/product/1024/33764#android-.E6.99.AE.E9.80.9A.E6.B6.88.E6.81.AF) 时，指定对应的 Channel ID 。</li>| 
...
| 个人交易提醒 | xxxxxxxx | 基金交易提醒 |您购买xxx基金xxx美元的申请已成功提交。|
...
在 Rest API 请求参数 `Android` 结构体中设置 `xm_ch_id ` 参数，可实现小米通知渠道下发，详情可查看 [PushAPI](https://intl.cloud.tencent.com/document/product/1024/33764) 参数说明
推送示例如下：
``` json
{
    "audience_type": "token",
...
### vivo 系统消息使用
目前vivo通知消息只能通过 Rest API 进行下发，控制台暂不支持。
在 Rest API 请求参数 `Android` 结构体中设置 `vivo_ch_id ` 参数为"1"，可实现 vivo 系统消息下发，详情可查看 [PushAPI](https://intl.cloud.tencent.com/document/product/1024/33764) 参数说明
推送示例如下：
...
2. 使用 TPNS SDK（1.1.5.4及以上的版本）创建通知渠道，详情请参见文档 [创建通知渠道](https://intl.cloud.tencent.com/document/product/1024/30715#.E5.88.9B.E5.BB.BA.E9.80.9A.E7.9F.A5.E6.B8.A0.E9.81.93)。

### 华为通知渠道使用
目前自定义渠道只能通过 Rest API 进行下发，控制台暂不支持，在您创建完成通知渠道后，可通过以下方式使用：

在 Rest API 请求参数 `Android` 结构体中设置 `hw_ch_id ` 参数，可实现华为通知渠道下发，详情请参见 [PushAPI](https://intl.cloud.tencent.com/document/product/1024/33764#.E8.AF.B7.E6.B1.82.E5.8F.82.E6.95.B0) 参数说明
