...
| TPNS     | Big image + thumbnail + audio | <li>Only HTTPS URLs are supported<li> Image format requirements:<br>1. `JPEG/JPG/PNG` format<br>2. For a big image, its height cannot exceed 256 px, and the width is adaptive<br>3. For a thumbnail, its height or width cannot exceed 120 px, otherwise, it will be a cropped center-aligned square<li>Audio/Video file format requirements:<br>1. The audio file cannot exceed 5 MB in size |
