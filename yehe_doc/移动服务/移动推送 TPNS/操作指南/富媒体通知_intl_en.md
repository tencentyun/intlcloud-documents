...
>?The TPNS channel for iOS supports the rich media push feature by default. In the APNs channel, the application needs to integrate with the [notification service extension plugin](https://intl.cloud.tencent.com/document/product/1024/30730).
...
| -------- | ----------------- | ------------------------------------------------------------ |
| Huawei | Thumbnail | <ul  style="margin: 0;"><li>URL Only HTTPS URLs are supported</li><li>Format requirements:<ul  style="margin: 0;"><li> `PNG/JPG/JPEG` format</li><li> 120 x120 px in dimensions. If this limit is exceeded, the image will be automatically adjusted.</li><li> Below 200 KB in size </li></ul></li></ul>|
| Mi     | Big image              | <ul  style="margin: 0;"><li>Only HTTPS URLs are supported <li>Format requirements: <ul  style="margin: 0;"><li>`PNG`, `JPG`, or `JPEG` format </li><li> Fixed at 876x324 px in dimensions </li><li> Below 1 MB in size </li></ul></li></ul> Note: to use the big image notification feature of the Mi channel, you need to first call the image upload API of Mi to upload the image file, get the image address `pic_url` specified by Mi, and enter it in the corresponding TPNS push parameter `xg_media_resources`. For more information, please see the big image upload API description in [Mi Push Rich Media Message](https://dev.mi.com/console/doc/detail?pId=1163#_10_0) |
| TPNS | Big image + thumbnail + audio | <ul  style="margin: 0;"><li>Android platform:<ul  style="margin: 0;"><li>Only HTTPS URLs are supported </li><li>Image format requirements:<ul  style="margin: 0;"><li> `JPEG/JPG/PNG` format</li><li> For a big image, its height cannot exceed 256 px, and the width is adaptive</li><li> For a thumbnail, its height or width cannot exceed 120 px, otherwise, it will be cropped in a center-aligned square</li></ul></li><li>Audio/Video file format requirements: The audio file cannot exceed 5 MB in size</li></ul></li><li>iOS platform: refer to the requirements of the APNs channel</li></ul> |
| APNs     | Thumbnail + audio/video     | <ul  style="margin: 0;"><li> Only HTTPS URLs are supported </li><li>Image format requirements: <ul  style="margin: 0;"><li> `JPEG`, `PNG`, or `GIF` format </li><li> Below 10 MB in size</li></ul></li><li>Audio/Video file format requirements: <ul  style="margin: 0;"><li> `MPEG`, `MPEG-2 Video`, `MPEG-4`, or `AVI` format </li><li> Below 5 MB in size </li></ul></li></ul>Note: image and audio/video in notification cannot be enabled at the same time |
