...
#### 单个/多个账号推送
1.在您调用 [推送接口](https://intl.cloud.tencent.com/document/product/1024/33764) 时，将 `audience_type`（推送目标）设置为 `account`（单账号推送）或`account_list`(账号列表推送)。
...
1. 在您调用 [推送接口](https://intl.cloud.tencent.com/document/product/1024/33764) 时，将 `audience_type`（推送目标）设置为 `package_account_push`（号码包推送），并补充 [步骤1](#.E6.AD.A5.E9.AA.A41.EF.BC.9A.E8.B0.83.E7.94.A8.E4.B8.8A.E4.BC.A0.E5.8F.B7.E7.A0.81.E5.8C.85.E6.8E.A5.E5.8F.A3) 中获取的`upload_id`，例如11231。
2. 设置账号对应的`account_type`（账号类型），账号类型取值可参考 [账号类型取值表](#quzhibiao)。
3. 设置 `account_push_type` 字段来决定推送给「**最近**」或「**所有**」绑定该账号的设备。
