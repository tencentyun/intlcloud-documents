...
- 上传文件必须是 .zip 压缩文件结尾，压缩内容必须是单个且编码 为UTF-8的.txt 文件。
...
| 参数名  | 类型  | 是否必须  | 参数说明  |
| --- | --- | --- | --- |
| file  | form-data | 是  | 上传文件必须是 .zip 文件，大小100M以内; .zip 文件内容必须是单个且编码为UTF-8的.txt文件，不能嵌套文件夹;.txt 文件中的每一行代表1个账号，账号长度限制为[2, 100]  
