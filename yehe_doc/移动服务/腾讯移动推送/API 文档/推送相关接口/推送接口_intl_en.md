...
| badge_type           | Integer | aps/android   | No                             | -1                                   | The user-configured badge number. Place it in the `Android` structure if the push platform is Android, and place it in the `aps` structure if the push platform is iOS: <li> -1: badge number does not change. </li> <li> -2: badge number is auto increased by 1. </li><li> >=0: a custom badge number is configured.</li> |
...
| plan_id              | String  | None     | No                             | None                                   | Push plan ID. For more information on how to create and use a push plan, please see [here](https://intl.cloud.tencent.com/document/product/1024/37452). |
