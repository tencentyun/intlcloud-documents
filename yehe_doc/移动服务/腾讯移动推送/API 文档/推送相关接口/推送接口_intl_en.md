...
| Field Name | Type | Parent Item | Default Value | Required | Description |
| ------ | ------ | ------- | ------ | ---- | -------- |
| title  | String | message | Empty     | Yes  | Message title, which will overwrite the `title` under `alert`. |
| content        | String | message       |Empty    | Yes    | Message content, which will overwrite the `body` under `alert`. |
| thread_id                | String | message | Empty     | No   |Thread ID for collapsed notification in threaded display.                                     |
...
| tpns_online_push_type | Integer|Empty|No|  0| Whether the message will be preferably delivered through the TPNS channel when the device is online. Valid values:<li>0: yes |<li> 1: no|
| Field Name  | Type    | Required | Parameter Description                                           |
