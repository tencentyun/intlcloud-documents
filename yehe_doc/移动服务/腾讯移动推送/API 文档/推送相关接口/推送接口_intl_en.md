...
| xg_media_resources    | String  | message       |None    | No   | URL address of large image on notification bar, which is only effective for the TPNS and Xiaomi channels.<li>**Note:** to enable the large image notification of Xiaomi channel, you need to upload the image via the relevant Xiaomi push API, get the specified URL address of the image (pic_url), and add it to the `xg_media_resources` parameter of TPNS push. For more information, see the "Big Picture Upload API" section of [Xiaomi Push Rich Text Messages](https://dev.mi.com/console/doc/detail?pId=1163#_10_0). || xg_media_audio_resources    | String  | message       | None    | No    | Element address for audio rich media, <li>which can be audio in .mp3 format and is recommended to be of no more than 5 MB in size. <li>**Note:** only the TPNS channel supports delivering this parameter, while other channels do not deliver it. |
...
| custom_content | String | Android   |None    | No    | A user-defined parameter (serialized to JSON String).<br> **Note:**<li>To be compatible with vendor advanced features, TPNS will officially update the Huawei push protocol to V4 on July 15, 2020. The Huawei V4 protocol does not support using the `custom_content` field to pass in business custom parameters.<li>If you have integrated the Huawei vendor channel, we recommend using [intent](https://intl.cloud.tencent.com/document/product/1024/32624#.E5.A6.82.E4.BD.95.E8.AE.BE.E7.BD.AE.E6.B6.88.E6.81.AF.E7.82.B9.E5.87.BB.E4.BA.8B.E4.BB.B6.EF.BC.9F) instead to pass in custom parameters, and deliver them through the Huawei push channel.<li>If you want to continue using the older V2 protocol, click [Submit a ticket] in the upper-right corner of the console to enable the allowlist.|| show_type | Integer | Android |2 | No | Whether to display notification when the application is in the foreground, which is displayed by default. This takes effect only for TPNS and FCM channels. <li>1: no<li>2: yes. <br>Note: if the value is 1 and the application is in the foreground, the end user will not be aware of the push, but arrival data will be reported.</br> |
...
|group_id     |String   | None  |No                 |tpns_yyyymmdd,  with yyyymmdd for the push date         | Deprecated. Use `plan_id` to set the aggregated statistic. |
