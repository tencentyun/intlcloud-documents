...
| environment | string | Yes (only for iOS) | This specifies the push environment (only available for pushes on iOS) <li>product: push production environment </li><li>dev: push development environment </li> |
...
| xm_ch_id    | string    |   Android      | None    | No    | Xiaomi channel ID (only valid for the Mi channel) |
...
| lights         | int    |Android       |1    | No    | Whether the breathing light is used</li><li>0: no <li>1: yes</li>       |
...
|group_id     |string   | None  | No                 |tpns_yyyymmdd, with yyyymmdd represents the push date       | Statistics tag, used for aggregated statistics. Format requirement: it can contain up to 25 letters, digits, and symbols and cannot contain spaces. |
...
| channel_rules |array |None|No|None| Push channel selection policy. You can select the channels through which a push can be delivered. The push can be delivered through all channels by default. For the data structure of single elements in `channel_rules`, please see the `channel_rules` parameter description below |
...
- This parameter currently is not customizable, and the `collapse_id` generated by TPNS is required. <li>This feature currently is supported only for the TPNS channel, APNs channel, Xiaomi channel, Meizu channel, and Huawei devices on EMUI 10 and above. <li>For the Huawei channel, custom parameters can be carried in the [intent](#intent1) method during message overwriting. If you use `custom_content` to carry custom parameters, the API layer will block them. <li>Currently, the OPPO and Vivo channels do not support message overwriting. When an overwriting message is created, delivery to the OPPO and Vivo channels can be disabled by setting the `force_collapse` field to `false`.

### channel_rules parameter description

| Field name | Type | Required | Comments |
| ------- | ------ | -------- | ------------------------------------------------------------ |
| channel | string | Yes       | Delivery push channel. <br> <li> hw: Huawei channel <li> xm: Xiaomi channel <li> mz: Meizu channel <li> vivo: Vivo channel <li>oppo: OPPO channel |
