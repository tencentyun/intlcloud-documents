...
## 操作步骤
### 	控制台使用
1. 登录 [移动推送 TPNS 控制台](https://console.cloud.tencent.com/tpns)，在左侧导航中选择【消息推送】，单击【新建推送】>【高级设置】。
...
控制通知在设备通知中心是否折叠以及折叠方式有以下三种选项：
 - 系统默认：跟随操作系统的默认折叠规则
部分厂商系统规则如下：
	- 原生安卓：同组通知达到5条时开始分组。折叠时右侧数字为被折叠的未阅读通知总量，展开时最多显示8条通知内容。
...
### Rest API 使用
若想要实现控制台“不折叠”和“自定义”的效果，则需要在 Push API `message` 字段中自定义设置 `thread_id` ，详情可参考 [PushAPI 可选参数](https://intl.cloud.tencent.com/document/product/1024/33764) 说明。
推送示例如下：
```
{
    "audience_type": "token",
