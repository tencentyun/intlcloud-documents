...
在**华为推送服务** SDK 2.6.3 版本之上，华为推出了 HMS Core 版本推送服务。
移动推送 TPNS 跟进各厂商通道推送服务的更新进度，提供集成华为推送 HMS Core Push SDK 的插件依赖包供用户选择使用。

> !  
> - HMS Core 版本 SDK 相较 V2 版本，在接口调用、Push 服务开启步骤等方面有较大改变。
> - HMS Core 版本和 V2 版本互不兼容，同时集成将造成依赖冲突，请选择其一进行集成。
> - HMS Core 版本不再支持从服务端直接下发 key-value 形式的自定义参数，不支持 V2 版本中提供的通知点击事件 onEvent 回调。对此建议使用安卓通用的 intent scheme 方式传递通知点击事件，详情请参见 [如何设置消息点击事件](https://intl.cloud.tencent.com/document/product/1024/32624)。
> - 您仍可以通过集成华为推送 V2 版本 SDK 来使用华为通道推送服务，详情请参见 [华为通道 V2 接入](https://intl.cloud.tencent.com/document/product/1024/30716)。
