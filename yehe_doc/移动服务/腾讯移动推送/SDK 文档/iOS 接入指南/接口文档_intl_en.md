...
## Auto Increased by 1 on iOS

#### API description
Call this API to report the current application badge number to the TPNS server. After the client is configured, you can use the "auto increased by 1 on iOS" feature, which can be found in the console (create push > notification bar message > general settings > badge number).
...
>!This API must be called locally; otherwise, the badge number by default will not change, even if the "auto increased by 1 on iOS" feature is enabled.  
...
#### API description
This callback is new in SDK 1.2.7.1 and used for TPNS registration failures.
