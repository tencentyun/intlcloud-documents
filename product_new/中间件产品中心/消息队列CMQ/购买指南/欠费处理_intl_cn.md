...
腾讯云消息队列 CMQ 每个整点进行扣费（详细扣费规则可以参考 [费用说明](https://intl.cloud.tencent.com/zh/document/product/406/13648)）。
在账户余额被扣为负值时（上图中点1）腾讯云将通过邮件及短信的方式通知到腾讯云账户的创建者以及所有协作者。

### 欠费处理

- 从余额扣为负值时刻起，2小时内 CMQ 可继续使用且继续扣费。
- 欠费2小时后（上图中点 2），CMQ 的所有功能将停止服务（仅可查询详情），且停止扣费。
