...
You can initiate a video composing task by calling a [server API](https://cloud.tencent.com/document/product/266/35286). The return result of the API contains the task ID, which is used to associate with the corresponding task result when [getting result](#.E7.BB.93.E6.9E.9C.E8.8E.B7.E5.8F.96).

## Getting Result

After initiating a composing task, you can wait for [result notification](https://cloud.tencent.com/document/product/266/33475#ResultNotification) asynchronously or perform [task query](https://cloud.tencent.com/document/product/266/33475#TaskQuery) synchronously to get the task execution result. Below is an example of getting the result notification in normal callback mode after the video composing task is initiated (the fields with null value are omitted):
