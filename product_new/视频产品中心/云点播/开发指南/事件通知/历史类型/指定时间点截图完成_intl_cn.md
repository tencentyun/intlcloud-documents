...
## 事件说明
当 App 配置了事件通知，并且在指定时间点截图完成后，App 后台即可通过“普通回调”或“可靠回调”的方式获取该事件通知。<!--API API事件通知内容为 [SnapshotByTimeOffsetTask2017 结构](#APIhttps://intl.cloud.tencent.com/document/api/266/31773#SnapshotByTimeOffsetTask2017)。-->
...
## 可靠回调
如果选择可靠回调模式，调用拉取事件通知 <!--API(#APIhttps://intl.cloud.tencent.com/document/api/266/31773)--> API 会接收到如下形式的 HTTP 应答。
```json
{
    "Response":{
        "EventSet":[
