...
前者请参见 [设置工作流](https://intl.cloud.tencent.com/document/product/1041/33475)。本文将介绍调用 API 的任务发起方式。

## 发起单个转码任务
开发者可以通过 API ProcessMedia 发起单个文件转码任务。如果调用成功，则返回任务 ID，即结果中的`TaskId`字段。
...
如果开发者设置了事件通知 CMQ，则会收到该任务的完成事件通知。除了通过 CMQ 接收事件通知消息，还可以通过 [查询任务详情](https://intl.cloud.tencent.com/document/product/1041/33497) API 接口查询任务结果，输入参数即为 ProcessMedia 接口返回的`TaskId`。
