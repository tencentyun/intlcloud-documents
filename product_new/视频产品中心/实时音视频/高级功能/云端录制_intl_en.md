...
MixTranscoding is disabled by default in a TRTC room. After on-cloud recording is enabled, only upstream videos of individual streams (mainstream and substream) are recorded by default in the TRTC room. To record a mixed video stream, enable [Cloud MixTranscoding](https://cloud.tencent.com/document/product/647/16827) by calling the `setMixTranscodingConfig` API of TRTCCloud to mixing multiple streams into one and record it into a single file.
...
Locate the recorded video files on the [Video Management](https://console.cloud.tencent.com/vod/media) page of the VOD Console. Taking the [Demo](https://cloud.tencent.com/document/product/647/17021) as an example, locate a recorded file by following the steps below:
...
2. Create a TRTC room using the Demo with ID `12345` and user name `userA`. The live stream ID for the room is `MD5(12345_userA_main)` (calculated as `3891_8d0261436c375bb0dea901d86d7d70e8`). For more information on how to generate the playback URL, please see [CDN-based Relayed Live Streaming](https://cloud.tencent.com/document/product/647/16826).
3. Exit the room, and find the file on the [Video Management](https://console.cloud.tencent.com/vod/media) page of the VOD Console or using the prefix of the live stream ID.



### Option 2: Searching via REST API

Call the REST API [SearchMedia API](https://cloud.tencent.com/document/product/266/31813) provided by the VOD service and specify its StreamId parameter to search for the recorded file. For more information on how to obtain the StreamId, please see [CDN-based Relayed Live Streaming](https://cloud.tencent.com/document/product/647/16826).
