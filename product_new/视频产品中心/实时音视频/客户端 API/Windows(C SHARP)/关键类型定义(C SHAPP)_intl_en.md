...
| Attribute | Type | Required | Description | Recommended Values |
|-----|-----|-----|-----|-----|
| sdkAppId | uint |Yes| Application ID. Tencent Cloud generates bills based on `sdkAppId`. | The ID can be obtained on the account information page in the [TRTC Console](https://console.cloud.tencent.com/rav/) after the corresponding application is created. |
