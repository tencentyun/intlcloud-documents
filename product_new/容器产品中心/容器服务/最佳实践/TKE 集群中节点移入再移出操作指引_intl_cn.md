...
- 如果单个账号下的多个集群都需进行节点移出再移出操作，建议分批执行。且在每操作完一个集群之后，立即验证集群状态是否正常。
...
2. 在弹出窗口中，取消勾选“销毁按量计费的节点”，并单击【确定】即可将节点移除集群。
>
>- 请记录该节点 ID，用于重新添加到集群。
...
4. 在“云服务器配置”页面，【数据盘挂载】与【容器目录】默认不勾选。如果需要将容器和镜像存储在数据盘，则勾选【数据盘挂载】。
...
2. 打开“高级设置”，在“自定义数据”输入以下节点初始化脚本，并勾选【开启封锁】。
