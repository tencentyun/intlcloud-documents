...
>?本文使用数据库为 [云数据库 MySQL](https://intl.cloud.tencent.com/document/product/236)。
>
...
2. 选择购买配置，详情请见 [云数据库 MySQL](https://intl.cloud.tencent.com/document/product/236)。
>!云数据库所在地域与集群相同，否则无法连接该数据库。
>
3. 数据库创建成功后，可在 [MySQL-实例列表](https://console.cloud.tencent.com/cdb) 中查看。
4. 对数据库进行初始化操作，详情请参见 [初始化 MySQL 数据库]。
