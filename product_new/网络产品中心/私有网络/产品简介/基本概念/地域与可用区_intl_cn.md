...
- 无论同地域还是跨地域，不同 VPC 间相互隔离，均无法直接内网通信。如果您有不同 VPC 间通信需求，可以使用 [云联网](https://intl.cloud.tencent.com/document/product/1003) 或 [对等连接 ](https://intl.cloud.tencent.com/document/product/553)。

## 可用区（Zone）
可用区指腾讯云在同一地域内电力和网络互相独立的物理数据中心，一个地域内至少有一个可用区，如下图示例，广州地域有4个可用区。
...
详细步骤请参见 [将实例迁移到其他可用区](https://intl.cloud.tencent.com/document/product/213/6091)。
