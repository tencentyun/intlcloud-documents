...
- 在支持存储访问日志到 COS 的地域中，若您不开启访问日志，腾讯云默认为您保留3天的日志；若您开启访问日志，存储时间则依 COS 存储而定。其他地域暂不支持存储访问日志。
...
2. 在在弹出框中，开启访问日志，并选择相应 COS 中的存放 bucket，单击【提交】，则请求日志会自动在 bucket 下创建 lb-id 为名称的文件夹进行存储。选取完成后，单击 bucket 地址可以直接跳转到日志下载页面。
