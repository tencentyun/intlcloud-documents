...
4. 检查是否 mongos 负载过高。
时延监控指标主要反馈的是从请求到达接入层直至处理完返回客户端的时间，若已确认 mongod 上没有慢操作，但请求时延较高，可能是 mongos 负载过高导致。造成 mongos 负载高的原因有多种，例如业务瞬间建立大量连接可能会导致 mongos 负载过高，或者，多个分片的数据需要汇总也可能会造成 mongos 负载高。此时可以进行 mongos 重启。
