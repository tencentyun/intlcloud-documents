...
### 使用云数据库 MySQL 的注意事项？
注意事项请参见 [使用限制](http://intl.cloud.tencent.com/document/product/236/7259)。
...
若希望扩展读写能力，可以考虑升级实例配置或购买 [只读实例](http://intl.cloud.tencent.com/document/product/236/7270)。

<span id = "genghuandiyu"></span>
### 如何更换云数据库 MySQL 的地域？
暂不支持更换地域，您可以使用 [数据传输服务 DTS](http://intl.cloud.tencent.com/document/product/571/13706) 来实现两地实例间数据迁移，DTS 支持实时数据同步。数据迁移完毕后，自助退换源实例即可。
...
以上操作也可以通过 [云 API 接口](http://intl.cloud.tencent.com/document/product/236/17497) 实现。
