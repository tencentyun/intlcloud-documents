...
>?
>- 也可以在云监控中 [云数据库 MySQL 监控数据接口](https://intl.cloud.tencent.com/document/api/248/11006) 通过云 API 来获取实例的监控指标。
...
> - 部署监控为备机：高可用版的主实例与灾备实例默认为一主一从架构，因此，仅当监控实例为主实例及灾备实例时，其备机下复制相关的监控数据才有效。用于反映主实例、灾备实例与其隐藏从节点的延迟距离及时间，建议关注备机下相关的监控数据，若主实例及灾备实例存在故障时，其监控实例对应隐藏的从节点可快速提升为主实例。
> 
![](https://main.qcloudimg.com/raw/7a69aff2fc7cb1372e2c700c29b91df7.png)

## 监控粒度
自2018年08月11日起，云数据库 MySQL 监控粒度实行自适应策略，暂不支持监控粒度的自定义选择。另外，云数据库 MySQL 的实例支持5秒粒度监控，此阶段5秒监控免费使用，具体收费时间另行通知。以下为监控粒度自适应策略：
...
更多云数据库监控指标的使用方法，请参见云监控 API 中的 [云数据库 MySQL 接口](https://intl.cloud.tencent.com/document/product/248/11006)。
