...
| 状态 | 说明                 | 可执行的操作                                           |
| -------- | ---------------------------- | ------------------------------------------------------------ |
| 未配置   | 购买成功后的默认状态         | 1. 初始化配置<br>2. 续费（包年包月）<br>3. 销毁、退货          |
| 初始化中 | 配置完成进行初始化           | 1. 续费（包年包月）                                          |
| 运行中   | 配置初始化成功，正常运行状态 | 1. 查看订阅配置<br>2. 修改订阅对象<br>3. 续费（包年包月）<br>4. 销毁、退货<br>5. 重新配置 |
| 隔离中   | 订阅实例欠费后执行隔离状态   |   -                                                           |
| 已隔离   | 隔离状态                     | 1. 续费（包年包月、按量付费）<br>2. 删除下线                 |
| 下线中   | 执行下线状态                 |    -                                                          |
