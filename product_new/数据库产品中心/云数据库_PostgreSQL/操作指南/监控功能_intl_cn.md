...
| 活跃的连接数            | active  connect          | 当前正在活动的连接数量              | 个   |
| idle 的连接数            | idle connect             | 当前处于空闲中的连接数量            | 个   |
| 执行时长超过1秒的 SQL 数  | long_query               | 执行时长超过1秒的 SQL 数量            | 条   |
| 执行时长超过1秒的事务数 | long transaction         | 执行时长超过1秒的事务数量           | 个   |
| 空闲的事务数            | idle in transaction      | 当前处于空闲状态的事务数量          | 个   |
| 超过5秒的空闲事务       | long idle in transaction | 当前有多少处于空闲的事务（超过5秒） | 个   |
| 等待会话                | waiting                  | 当前处于等待中的会话有多少。        | 个   |
| 等待超过5秒的会话数     | long waiting             | 当前等待超过5秒的会话有多少         | 个   |
| 2PC 事务数               | 2pc                      | 当前 2PC 的事务有多少               | 个   |
| 超过5秒未提交的 2PC 事务  | long 2pc                 | 超过5秒未结束的 2PC 的事务有多少      | 个   |
