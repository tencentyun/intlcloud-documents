...
- **异步调用**：定时器始终使用异步调用类型来调用函数，结果不会返回给调用方。有关调用类型的更多信息，请参阅 [调用类型](https://intl.cloud.tencent.com/document/product/583/9694#.E8.B0.83.E7.94.A8.E7.B1.BB.E5.9E.8B)。
...
- 入参（可选）：最大支持4KB的字符串或者 JSON 数据结构，可以在入口函数的 "event" 参数中获取。
