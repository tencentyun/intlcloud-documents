...
**重试策略**：持续重试，当云函数（SCF）后台收到告警后，对目前调用失败的事件会先按指数退避进行重试。当重试间隔超过1小时，每隔1小时重试一次，持续重试24小时。如果用户配置了死信队列，超过时间仍失败的事件会被发送到死信队列由用户进行进一步处理，否则，事件将被函数平台删除。
...
## 死信队列
根据云函数的重试规则，重试超过次数限制仍失败的事件会被丢弃或传送至用户指定的 CMQ 主题或 CMQ 队列。死信队列由用户自行配置，用于收集错误时间信息、分析失败原因。未配置死信队列的函数将在重试次数用尽后丢弃事件。
