...
- 要创建网络类型为私有网络（VPC）的 CVM 实例时，需要在目标地域 [创建 VPC]，并且在 VPC 下的目标可用区 [新增子网]。
- 不使用系统自动创建的默认项目时，需要 [新建项目]。
- 不使用系统自动创建的默认安全组时，需要在目标地域 [创建安全组] 并添加能满足您业务需求的安全组规则。
...
### 使用 API 创建子机
使用 RunInstances 接口可以在指定专用宿主机上创建子机。
