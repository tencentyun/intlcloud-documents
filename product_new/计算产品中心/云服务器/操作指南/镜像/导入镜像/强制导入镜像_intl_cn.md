## 操作场景
当用户的 Linux 镜像因为某些原因无法 [安装 cloudinit](https://intl.cloud.tencent.com/document/product/213/12587) 时，可以使用**强制导入镜像**功能完成镜像的导入。如果用户使用强制导入镜像进行导入，腾讯云将无法对用户的云服务器进行初始化配置，需要用户设置脚本自行根据腾讯云提供的配置文件对云服务器进行配置。本文档指导用户如何在强制导入镜像的前提下，对云服务器进行配置。
