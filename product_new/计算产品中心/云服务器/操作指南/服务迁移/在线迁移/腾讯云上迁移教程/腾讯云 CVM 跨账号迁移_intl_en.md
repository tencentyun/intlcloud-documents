The online migration tool supports migrating Tencent Cloud CVM data across accounts. Cross-account data migration refers to moving data between CVMs under two different accounts.
...
```
If the `Stage 1 is finished and please run next stage at source machine` prompt appears, it indicates that stage 1 is completed. 
...
sudo ./go2tencentcloud_x64
```
If the `Stage 2 is finished and please run next stage at gateway machine` prompt appears, it indicates that stage 2 is completed.
...
sudo ./go2tencentcloud_x64
```
If the `Migrate successfully` prompt appears, it indicates that the entire migration task has been completed successfully.
