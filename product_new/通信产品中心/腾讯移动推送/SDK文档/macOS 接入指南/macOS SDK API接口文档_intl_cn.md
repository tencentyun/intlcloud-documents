...
#### 接口说明
使用在腾讯移动推送官网注册的应用的信息，启动腾讯移动推送服务。
...
#### 参数说明
- AppID：通过控制台申请的应用 ID, 即 Access ID。
- AppKey： 通过控制台申请的 AppKey，即 Access Key。
- Delegate：回调对象。 
...
#### 接口说明
调用此接口上报当前 App 角标数到腾讯移动推送服务器，客户端配置完成即可使用 macOS 角标自动加1的功能，如需使用此功能，请前往腾讯移动推送控制台，选择【创建推送】 > 【通知栏消息】 > 【常用设置】 > 【角标数字】，进行设置。
...
>此回调方法在注册成功之后调用，当前的 Token 已经注册过之后，SDK 将缓存注册信息，此方法将不再调用。
