...
###操作场景
Android SDK 是腾讯移动推送服务为客户端实现消息推送而提供给开发者的接⼝，本文将提供两种集成方式。


## 一. AndroidStudio Gradle自动集成方法

### 操作步骤
**在配置SDK之前，确保已经创建Android平台的应用**
...
## 二. Android Studio 手动集成配置方法

> 如果不适合第一种集成方法的话，可使用手动集成方法
...
（2）将信鸽 SDK目录下的libs目录所有.jar文件拷贝到工程的libs（或lib）目录下。

（3）.so文件是信鸽必须的组件，支持armeabi、armeabi-v7a、arm64-v8a、mips、mips64、x86、x86_64平台，请根据自己当前.so支持的平台添加
...
**1.删除带帐号注册的API，设置帐号只能通过bindAccount或appendAccount来设置**
...
registerPush不再支持设置帐号，需要注册帐号的话，要单独调用bindAccount或appendAccount，推荐在registerPush成功的回调里调用

**2.继承XGPushBaseReceiver时需要多实现以下两个函数**
```java
/**
 * 设置帐号结果处理函数
 */
