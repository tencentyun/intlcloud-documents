...
| pushStatDataAll | Json   | Returned result: individual element formed by `channel` and `pushState`, where `channel` is the channel name. `pushState` structure variables are shown in following table |
