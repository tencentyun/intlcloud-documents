...
| message | Pass-through or silent message | Android (pass-through message) <br>iOS (silent message) | Notifications are not displayed in the notification bar. Due to vendor restrictions, Android pass-through messages are only delivered through the TPNS channel and cannot be delivered through the vendor channel |
