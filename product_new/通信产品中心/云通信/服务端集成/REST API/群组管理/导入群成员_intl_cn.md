...
-  当 App 需要从其他即时通讯系统迁移到云通信 IM 时，使用该协议导入存量群成员数据。
...
>音视频聊天室和在线成员广播大群不支持导入群成员，对这两种类型的群组进行操作时会返回10007错误；因为这两种类型群组所适用的场景一般不需要导入成员，所以没有提供这一功能。
...
| Result | Integer | 导入结果：0为失败；1为成功；2 表示已经是群成员   |
