...
调用 **添加好友 REST 接口** 将存量的关系链导入即时通信 IM。
...
老系统中增量消息时通过调用 **单发单聊消息 REST 接口** 同步到即时通信 IM，即时通信 IM 中增量消息时通过调用 [发单聊消息之后回调](https://intl.cloud.tencent.com/document/product/1047/34365)同步到老系统。
...
1. 老系统中成员的增删，需要通过调用**增加群组成员 REST 接口** 和 **删除群组成员 REST 接口**同步到即时通信 IM。
