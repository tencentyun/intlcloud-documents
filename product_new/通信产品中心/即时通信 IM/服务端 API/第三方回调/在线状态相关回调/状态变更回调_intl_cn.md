...
| Reason | String | 用户上下线触发的原因：<br>Login 的原因有 Register：App TCP 连接建立<br>Logout 的原因有 Unregister：App 用户注销帐号导致 TCP 断开<br>Disconnect 的原因有 LinkClose：即时通信 IM 检测到 App TCP 连接断开 （例如 kill App，客户端发出 TCP 的 FIN 包或 RST 包）；TimeOut：即时通信 IM 检测到 App 心跳包超时，认为 TCP 已断开（例如客户端网络异常断开，未发出 TCP 的 FIN 包或 RST 包，也无法发送心跳包）。心跳超时时间为 400  秒。<br>各种具体场景触发的回调 Reason 请参考 [可能触发该回调的场景](https://intl.cloud.tencent.com/document/product/1047/34357)。|
