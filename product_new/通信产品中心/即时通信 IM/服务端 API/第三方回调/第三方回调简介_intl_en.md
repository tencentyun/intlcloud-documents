...
| OptPlatform | Client platform. Depending on the platform type, the following values are available: <br />RESTAPI (requests are sent using RESTful APIs) and Web (requests are sent using Web SDKs), <br/>Android, iOS, Windows, Mac, and Unknown (requests are sent using an unknown device). |
...
> To obtain a free certificate issued by IM, you need to submit a ticket using the ticket template for HTTPS certificate issuance in order to provide IM with the domain name of the developer backend.

The first type of callback has the lowest security level, and the last type has the highest security level.

1. HTTP callbacks have two defects. One is that data transmitted in plain text is prone to interception, and the second is that a third-party app cannot determine whether the callback request really comes from IM.
...
>   1. SDKAppID
>   1. App name
>   1. Callback URL, which must be consistent with that in the self-service configuration on the console
> 3. After receiving the certificate, configure HTTPS mutual authentication based on the two guides below:
