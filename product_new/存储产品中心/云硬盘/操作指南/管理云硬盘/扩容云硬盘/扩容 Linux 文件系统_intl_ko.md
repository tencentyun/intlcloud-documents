## 작업 시나리오
CBS(Cloud Block Storage)는 클라우드에서 스토리지 장치 확장이 가능하고 사용자가 CBS를 생성한 후 수시로 용량을 확장할 수 있습니다. 또, 스토리지 용량을 확장할 때 CBS의 원본 데이터가 손실되지 않습니다.
...
>
- [CBS 확장](https://cloud.tencent.com/document/product/362/5747) 용량이 있습니다.
- 해당 CBS가 이미 Linux CVM에 [마운트](https://intl.cloud.tencent.com/document/product/362/5745)되었고 파일 시스템이 생성되었습니다.
- 확장 예정인 파티션 및 파일 시스템의 Linux CVM에 [로그인](https://intl.cloud.tencent.com/document/product/213/5436)했습니다.

## 작업 순서
### 확장 방법 확인
<span id="fdisk"></span>
1. root 사용자는 다음 명령어를 실행하여 CBS의 파티션 형식을 확인하십시오.
```
fdisk -l
```
...
1. 파일 시스템의 유형에 따라 다른 명령어를 실행하여 확장을 진행하십시오.
 - EXT 파일 시스템의 경우, `resize2fs` 명령어를 실행하여 파일 시스템을 확장하십시오.
 - XFS 파일 시스템의 경우, `xfs_growfs` 명령어를 실행하여 파일 시스템을 확장하십시오.

 예로 들면 `/dev/vdb`로 EXT 파일 시스템은 다음 명령어를 실행합니다.
```
resize2fs /dev/vdb
```
예로 들면 `/dev/vdb`로 XFS 파일 시스템은 다음 명령어를 실행합니다.
```
xfs_growfs /dev/vdb
```
2. 다음 명령어를 실행하여 새로운 파티션을 확인하십시오.
```
df -h
```
...
1. root 사용자는 다음 명령어를 실행하여 CBS의 용량 변화를 확인하십시오.
 ```
parted <디스크 경로> print
 ```
...
2. 다음 명령어를 실행하여 해당 CBS에 이미 마운트된 파티션이 있는지 여부를 확인하십시오.
```
mount | grep '<디스크 경로>' 
```
이 문서의 디스크 경로는 예로 들면 `/dev/vdb`입니다. 다음을 참조하여 실행하십시오.
...
3. 다음 명령어를 실행하여 데이터 디스크를 언마운트하십시오.
```
umount <마운트 포인트>
```
...
umount /data
```
> CBS에서 모든 파티션의 파일 시스템을 언마운트하고 [4단계](#step4)작동을 실행하십시오. 다음 명령어를 실행하여 해당 디스크의 모든 파티션 파일 시스템이 모두 언마운트 되었는지 확인하십시오.
```
mount | grep '/dev/vdb'
```
...
<span id="step4"></span>
4. 다음 명령어를 실행하여 parted 파티션 도구로 진입합니다.
```
parted '<디스크 경로>'
```
이 문서의 디스크 경로는 예로 들면 `/dev/vdb`입니다. 다음을 참조하여 실행하십시오.
...
parted '/dev/vdb'
```
5. 다음 명령어를 실행하여, 디스플레이와 조작 단위를 sector(기본 GB)로 변경하십시오.
```
unit s
```
...
7. 다음 명령어를 실행하여 기존 파티션을 삭제하십시오.
```
rm <파티션 Number>
```
예를 들어, 위의 이미지로부터 CBS에 1개의 파티션이 있는 것을 알 수 있으며 그 Number는 "1"입니다. 다음을 참조하여 실행하십시오.
```
 rm 1
```
에코 정보는 아래와 같습니다.
![](//mccdn.qcloud.com/static/img/3384eeada87ce75695e0e55125109eff/image.png)
5. 다음 명령어를 실행하여 새로운 주요 파티션을 생성하십시오.
```
...
12. 다음 명령어를 실행하여 새로운 파티션의 EXT 파일 시스템에 확장을 진행하십시오.
```
resize2fs <파티션 경로>
```
...
13. 다음 명령어를 실행하여 새로운 파티션의 XFS 파일 시스템에 확장을 진행하십시오.
```
xfs_growfs <파티션 경로>
```
이 문서의 파티션 경로는 예로 들면 `/dev/vdb1`입니다. 다음을 참조하여 실행하십시오.
...
xfs_growfs /dev/vdb1
```
14. 다음 명령어를 실행하여 새로운 파티션을 수동으로 마운트하십시오.
```
...
mount /dev/vdb1 /data
```
15. 다음 명령어를 실행하여 새로운 파티션을 확인하십시오.
```
df -h
```
...
1. 1. root 사용자는 다음 명령어를 실행하여 CBS의 용량 변화를 확인하십시오.
 ```
parted <디스크 경로> print
 ```
이 문서의 디스크 경로는 예로 들면 `/dev/vdb`입니다. 다음을 참조하여 실행하십시오.
...
parted '/dev/vdb'
```
5. 다음 명령어를 실행하여 파티션 정보를 확인하고 기존 파티션의 End 값을 기록하십시오.이 값은 다음 파티션의 시작 오프셋이 됩니다.
```
print
```
![](//mccdn.qcloud.com/static/img/788ce125bba952f204ed6ee36dfb644d/image.png)
6. 다음 명령어를 실행하여 새로운 주요 파티션을 생성하십시오. 이 파티션은 기존 파티션의 끝부터 시작하여 디스크의 모든 새로운 공간을 덮어 씁니다.
```
mkpart primary start end
```
이 문서는 End 값을 10.7GB 예로 합니다. 다음 명령어를 실행하십시오.
```
mkpart primary 10.7GB 100%
```
7. 다음 명령어를 실행하여 새로운 파티션 생성에 성공했는지 확인하십시오.
```
print
```
![](//mccdn.qcloud.com/static/img/fc54fd4c05102ee91c648526d77d1b42/image.png)
8. 다음 명령어를 실행하여 parted 도구를 종료합시오.
```
quit
```
9. 다음 명령어를 실행하여 새로운 파티션을 포맷하십시오.
```
mkfs.<fstype> <파티션 경로> 
```
...
1. root 사용자는 다음 명령어를 실행하여 파티션을 언마운트 하십시오.
```
umount <마운트 포인트>
```
이 문서의 마운트 포인트는 예로 들면 `/data`입니다. 다음을 참조하여 실행하십시오.
...
![](//mccdn.qcloud.com/static/img/c7617b90578192d64d19f02325f00ffb/image.jpg)
 ...
 - 만약 "[ERROR]-e2fsck failed !!" 메세지가 나타나면 다음 단계를 수행하십시오.
   a. 다음 명령어를 실행하여 파일 시스템이 위치한 파티션을 복구하십시오.
```
fsck -a <파티션 경로>
```
이 문서의 디스크 경로는`/dev/vdb`이며 파일 시스템은 vdb1를 예로 합니다. 다음을 참조하여 실행하십시오.
...
fsck -a /dev/vdb1
```
    b. 복구가 완료된 후, 다음 명령어를 다시 실행하여 확장 도구를 사용해 확장을 진행하십시오.
```
python /tmp/devresize.py /dev/vdb
```
<span id="step4MBR"></span>
4. 다음 명령어를 실행하여 확장 후의 파티션을 수동 마운트하십시오.
```
mount <파티션 경로> <마운트 포인트>
```
...
mount /dev/vdb /data
```
5. 다음 명령어를 실행하여 확장 후의 파티션 용량을 확인하십시오.
```
df -h
```
...
6. 다음 명령어를 실행하여 확장 후 기존 파티션의 데이터 정보를 확인하고, 새로 추가된 저장 공간이 파일 시스템으로 확장되었는지를 확인하십시오.
```
ll /data
```
...
<span id="Step4MBR"></span>
4. 다음 명령어를 실행하여 새로운 파티션을 생성하십시오.
```
Fdisk <디스크 경로>
```
...
7. 다음 명령어를 실행하여 새로운 마운트 포인트를 생성하십시오.
```
mkdir <새로운 마운트 포인트>
```
...
mkdir /data1
```
8. 다음 명령어를 실행하여 수동으로 새로운 파티션을 마운트하십시오.
```
...
```
9. 다음 명령어를 실행하여 새로운 파티션 정보를 확인하십시오.
```
df -h
```
...
>만약 사용자가 CVM을 재시작하거나 시작할 때 자동으로 데이터 디스크에 마운트하려면 [10단계](#AddNewPartINFOstep10) 와 [11단계](#AddNewPartINFOstep11)를 수행해야 합니다. 새로운 파티션 정보는 `/etc/fstab`에 추가하십시오.

<span id="AddNewPartINFOstep10"></span>
10. 다음 명령어를 실행하여 정보를 추가하십시오.
```
echo '/dev/xvdc2 /data1 ext3 defaults 0 0' >> /etc/fstab
```
<span id="AddNewPartINFOstep11"></span>
11. 다음 명령어를 실행하여 정보를 조회하십시오.
```
cat /etc/fstab
```
