...
您可以在云硬盘的任何状态下创建快照，但快照只能保存当前时间点已完成写入的数据。若应用程序或进程正在写入数据，这部分数据可能无法被保存至该时间点创建的快照。根据实际业务情况，您可以选择暂时停止所有写入并及时创建快照，或者先将该云硬盘从云服务器中 [卸载](https://intl.cloud.tencent.com/document/product/362/32400)，创建快照后再重新 [挂载](https://intl.cloud.tencent.com/zh/document/product/362/32401) ，以便获得数据完整的快照。

## 前提条件
- 您已经成功 [创建云硬盘](https://intl.cloud.tencent.com/document/product/362/5744)。
- 当前地域内您的快照个数和总容量未达最大值，详细信息请参考 [快照配额限制](https://intl.cloud.tencent.com/document/product/362/5754#Constraint)。
