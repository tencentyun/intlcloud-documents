...
单行全文模式下，默认使用`__CONTENT__`作为全文的键（key）名。例如，单行全文日志样例格式为 `Tue Jan 22 12:08:15 CST 2019 Installed: libjpeg-turbo-static-1.2.90-6.el7.x86_64`，您希望采集1月22号这一天的所有日志，则 ke y处填写`__CONTENT__`，过滤规则配置 `Tue Jan 22.*`
