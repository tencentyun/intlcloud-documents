## 简介
您可以通过对象存储控制台，把一个存储桶设置为托管静态网站，并且通过访问存储桶的访问域名来访问静态网站。关于静态网站的相关说明，请参见 [托管静态网站](https://intl.cloud.tencent.com/document/product/436/30958)。
...
2. 单击左侧的【权限管理】项，找到**存储桶访问权限**，在公共权限一栏中，选择**公有读私有写**。
![](https://main.qcloudimg.com/raw/a62ac13b34f19027074c13a6cb0b62d7.jpg)
3. 单击左侧的【基础配置】，向下找到**静态网站**配置项，单击【编辑】，把【当前状态】的开启按钮打开，然后依次填写静态网站的配置项。配置说明如下：
...
例如，您在存储桶中删除或重命名了某个文件。您可以添加一个重定向规则，将访问该文件的请求重定向至其他文件。
 ...
 - 前缀匹配：您可以使用前缀匹配规则对存储桶内的文件或文件夹进行重定向设置。具体示例请参见 [重定向规则示例](https://intl.cloud.tencent.com/document/product/436/30958#.E9.87.8D.E5.AE.9A.E5.90.91.E8.A7.84.E5.88.99)。
...
>除了域名差异外，新旧版本控制台所调用的 API 也存在差异，我们推荐您使用新版本控制台以及使用新版本 API、工具、SDK，因为这能够让您体验更加丰富和稳定的功能。COS 新版本控制台所调用的操作对象或存储桶的 API 是 XML API，旧版本控制台上调用的则是 JSON API，有关新旧两个版本 API 的差异，请参见 [API 常见问题](https://intl.cloud.tencent.com/document/product/436/10687)。
