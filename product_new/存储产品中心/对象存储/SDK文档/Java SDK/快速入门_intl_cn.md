...
- JDK 安装方式请参阅 [Java 安装与配置](https://intl.cloud.tencent.com/document/product/436/10865)。
...
您也可以使用临时密钥初始化 COSClient，临时密钥生成和使用可参阅 [临时密钥生成及使用指引](https://intl.cloud.tencent.com/document/product/436/14048)，参考示例如下：
```java
...
|  成员名 | 设置方法            | 描述                                                         | 类型    |
| ------------ | ------------------- | ------------------------------------------------------------ | ------- |
| region   | 构造函数或 set 方法 | 存储桶所在的区域，COS 地域的简称请参阅 [地域和访问域名](https://intl.cloud.tencent.com/document/product/436/6224) 文档 | Region  |
...
- 若要创建目录对象，请您参阅 [SDK 如何创建目录](https://intl.cloud.tencent.com/document/product/436/10687)。
- 对象键（Key）是对象在存储桶中的唯一标识。例如，在对象的访问域名 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/images/picture.jpg` 中，对象键为 images/picture.jpg，详情参考 [对象键](https://intl.cloud.tencent.com/document/product/436/13324#.E5.AF.B9.E8.B1.A1.E9.94.AE) 的说明。
...
### 下载对象
上传对象后，您可以用同样的 key，调用 GetObject 接口将对象下载到本地，也可以生成预签名链接（下载请指定 method 为 GET，详情请参阅 [预签名 URL 文档](https://intl.cloud.tencent.com/document/product/436/31536)），分享到其他终端来进行下载。但如果您的文件设置了私有读权限，那么请注意预签名链接的有效期。
将文件下载到本地指定路径，参考示例如下：
