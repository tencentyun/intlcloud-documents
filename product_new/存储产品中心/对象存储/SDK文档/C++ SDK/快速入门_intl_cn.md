...
- COS XML C++ SDK 支持 Linux 和 Windows， 不支持 Mac 系统。
...
>Poco 最新版需要 C++14 支持，如果编译器只支持 C++11 或更低，则可以使用 [Poco 1.9.4](https://github.com/pocoproject/poco/releases/tag/poco-1.9.4-release) 版本。
...
（5）编译完成后，将编译得到 jsoncpplib 拷贝到 COS CPP SDK 安装目录下 lib 文件夹中。
...
从 [Poco 官网](https://pocoproject.org/download.html) 获取并安装 Poco 的库和头文件（下载 complete 版本）或直接使用 git 命令获取 [Poco](https://github.com/pocoproject/poco.git)。
...
cd Poco/ 
./configure --omit=Data/ODBC,Data/MySQL
mkdir examplefolder  #（examplefolder 替换为您的自定义文件夹名,该目录为Poco的安装目录）
cd examplefolder
cmake .. 
make
```
...
下载 [jsoncpp 源码](https://github.com/open-source-parsers/jsoncpp)。

```shell
cd jsoncpp/
mkdir examplefolder  #（examplefolder 替换为您的自定义文件夹名，该目录为jsoncpp的安装目录）
cd examplefolder
cmake .. 
make
```

编译完成后，将编译得到 jsoncpplib 拷贝到 COS CPP SDK 安装目录下 lib 文件夹中
