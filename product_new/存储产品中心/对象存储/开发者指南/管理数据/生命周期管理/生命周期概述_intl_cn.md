...
沉降数据是单向的，只允许从标准存储 → 低频存储 → 归档存储，也支持跳级沉降（例如标准存储 → 归档存储），不支持逆向。您只能通过调用 [PUT Object - Copy](https://intl.cloud.tencent.com/document/product/436/10881)（针对非归档存储类型），或 [POST Object restore ](https://intl.cloud.tencent.com/document/product/436/12633)（仅适用于归档存储类型）来将较冷存储类型的数据恢复至较热存储类型。
