...
您可以通过 [List Jobs](https://intl.cloud.tencent.com/document/product/436/33783) API 列出近90天内创建的批量处理任务。批量处理任务列表包含每一项批量处理任务的信息，例如任务 ID，任务描述，任务优先级，任务状态和任务执行情况等信息。您可以通过任务状态，从批量处理任务列表中筛选处于相同状态的任务。当您在控制台上进行筛选操作时，也可以通过任务描述或者任务 ID 进行筛选。

## 查询任务状态

例如您需要获取更多有关任务的信息，可以通过 [DescribeJob](https://intl.cloud.tencent.com/document/product/436/33782) API 获取单个任务的所有信息。这一接口会返回指定任务的操作配置，对象清单信息，任务报告等信息。通过这一接口，您可以了解指定任务的详情。
...
| Active     | COS 正在根据您批量处理任务中配置的信息对清单中的对象进行操作时，任务状态为 Active，您可以通过控制台或者调用 [DescribeJob](https://intl.cloud.tencent.com/document/product/436/33782) API 等方式查询任务进展。 | 可转换至 Complete、Failing、Pausing 或者 Cancelling 等状态。当您的任务成功结束或者失败时，或者由于您的中止命令，例如取消任务时，状态会根据具体的原因进行切换。 |
...
| Failed     | 任务失败时，任务状态即 Failed 。有关任务失败的详细信息，可参见 [跟踪失败任务](https://intl.cloud.tencent.com/zh/document/product/436/32961?!editLang=zh&!preview=&lang=zh#.E8.B7.9F.E8.B8.AA.E5.A4.B1.E8.B4.A5.E4.BB.BB.E5.8A.A1)。 | Failed 状态是最终状态，当一项任务切换 Failed 状态时，将不再切换至其他状态。 |
