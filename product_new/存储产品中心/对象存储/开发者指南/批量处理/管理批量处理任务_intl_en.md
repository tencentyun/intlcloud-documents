...
You can view the list of the batch operation jobs created in the last 90 days using the [List Jobs](https://cloud.tencent.com/document/product/436/38611) API. The list contains information of each job, such as job ID, description, priority, status, and execution progress. You can filter them by job status in the list to display the ones in the same status or filter them by job description or ID in the console.

## Querying Job Status

If you need more information, you can get all the information of a job using the [DescribeJob](https://cloud.tencent.com/document/product/436/38609) API. This API will return such information as the operation configuration of the specified job, object inventory information, and job report.
...
| Active | When COS is performing an operation on the objects in the inventory based on the configuration information, the job status will be "Active". You can query job progress in the console or by calling the [DescribeJob](https://cloud.tencent.com/document/product/436/38609) API. | It can change to "Complete", "Failing", "Pausing", or "Cancelling" if your job succeeds, fails, or is paused or canceled. |
...
| Failed | After a job fails, the job status will change to "Failed". For more information, please see [Tracking a Failing Job](#.E8.B7.9F.E8.B8.AA.E5.A4.B1.E8.B4.A5.E4.BB.BB.E5.8A.A1). | "Failed" is a final status, i.e., the job will not change to any other status. |



## Tracking a Failing Job

If a problem arises during job execution, such as trouble in parsing the object inventory, the batch operation job will fail, and COS will return the corresponding error code and cause. You can call the [DescribeJob](https://cloud.tencent.com/document/product/436/38609) API or view the job report to get the cause of failure and other information.
