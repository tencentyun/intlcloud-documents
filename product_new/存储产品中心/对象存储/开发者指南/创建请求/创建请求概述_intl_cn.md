...
使用方法和示例详情请参阅 [使用角色](https://intl.cloud.tencent.com/document/product/598/19419) 文档查看生成临时安全凭证的方式。临时安全凭证通常只包括**有限策略**（操作、资源、条件）和**有限时间**（有效起止时间），因此生成的临时安全凭证可自行分发或直接使用。
...
我们在 [地域和访问域名](https://intl.cloud.tencent.com/document/product/436/6224) 文档中列出了可用于发起 REST API 访问的域名列表。
