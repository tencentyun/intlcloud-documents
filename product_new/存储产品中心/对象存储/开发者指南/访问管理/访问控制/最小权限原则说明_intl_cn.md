...
## 注意事项
当您在授权的时候，建议严格遵循最小权限，限定用户执行受限的操作（例如授权`action:GetObject`），访问受限的资源（例如授权`resource:exampleBucket-APPID/exampleObject.txt`）。 
...
用户策略是指在 [CAM控制台](https://console.cloud.tencent.com/cam/policy) 添加的用户权限策略，用于授予用户访问 COS 资源的权限。用户访问策略概述的配置说明，可参见 [访问策略语言概述](https://intl.cloud.tencent.com/document/product/436/18023) 文档。
