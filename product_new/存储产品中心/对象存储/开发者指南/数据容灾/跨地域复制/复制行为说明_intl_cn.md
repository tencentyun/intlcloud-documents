...
- 不指定对象版本 ID 执行 DELETE 请求时，如果选择了同步删除标记， 则跨地域复制会将该标记复制到目标存储桶 ；选择了不同步删除标记，则目标存储桶不会新增删除标记。两种情况下，目标存储桶均不会删除对应文件，用户可通过指定版本 ID 访问对象历史版本。 关于版本控制和删除标记的详细信息可参见 [版本控制概述](https://intl.cloud.tencent.com/document/product/436/19883) 文档。
