对象存储 COS 的计费方式为按量计费，本文主要介绍按量计费的计费详情。按量计费方式适用于对象存储提供服务的所有地域。COS 根据用户的存储容量、请求、流量等计量项用量进行计费，对用户账户按日或者按月进行扣费结算。
...
<table>
   ...
   <tr>
      <td>管理功能费用</td>
      <td>管理功能费用指用户开启并使用了管理功能（如清单或检索功能）后所产生的费用。目前管理功能费用有清单功能费用和数据检索功能费用</td>
      <td nowrap="nowrap"><li>清单功能费用 = 列出的对象数/百万 * 单价<br><li>数据检索功能费用 = 每 GB 单价 * 日累计数据检索量
