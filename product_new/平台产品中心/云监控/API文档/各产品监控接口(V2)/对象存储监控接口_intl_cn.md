...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据。可查看对象储存支持的 [地域列表](https://intl.cloud.tencent.com/document/product/436/6224) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如对象储存命名空间： QCE/COS（API 3.0接口版本的必须是大写） |
...
| 参数名称                        | 维度名称 | 维度解释     | 格式                                              |
| :------------------------------ | :------- | :----------- | :------------------------------------------------ |
| &Instances.N.Dimensions.0.Name  | appid    | 主账号 APPID | 输入String 类型维度名称，如：appid                |
| &Instances.N.Dimensions.0.Value | appid    | 主账号 APPID | 输入主账号 APPID，如 ：1250000000                 |
| &Instances.N.Dimensions.1.Name  | bucket   | 存储桶名称   | 输入String 类型 维度名称，如：bucket              |
| &Instances.N.Dimensions.1.Value | bucket   | 存储桶名称   | 输入存储桶具体名称，如： examplebucket-1250000000 |
