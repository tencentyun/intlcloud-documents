...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据；可查看消息队列CMQ支持的 [地域列表](https://intl.cloud.tencent.com/document/product/406/12667#.E5.9C.B0.E5.9F.9F.E8.AF.B4.E6.98.8E) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如消息队列CMQ主题订阅命名空间： QCE/CMQTOPIC（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称       | 维度解释                                                     | 格式                                       |
| :----------------------------- | :------------- | :----------------------------------------------------------- | :----------------------------------------- |
| Instances.N.Dimensions.0.Name  | topicId        | CMQ 主题实例 ID                                              | 输入String类型维度名称，如：topicId        |
| Instances.N.Dimensions.0.Value | topicId        | 具体的 CMQ 主题实例 ID                                       | 输入实例具体 ID，如： topic-i4p4k0u0       |
| Instances.N.Dimensions.1.Name  | subscriptionId | CMQ 订阅实例 ID                                              | 输入String类型维度名称，如：subscriptionId |
| Instances.N.Dimensions.1.Value | subscriptionId | 入参为 CMQ 订阅实例 ID，当 [指标名称](https://intl.cloud.tencent.com/document/product/248/11013) 中指标对应的维度为 topicId、subscriptionId 时，该字段必选 。 | 输入具体 subscriptionId，如 ：test1        |
