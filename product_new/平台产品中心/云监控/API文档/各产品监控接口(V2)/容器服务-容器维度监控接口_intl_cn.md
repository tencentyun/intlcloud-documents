...
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如容器服务容器维度命名空间：QCE/DOCKER（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称    | 维度解释                                                     | 格式                                         |
| :----------------------------- | :---------- | :----------------------------------------------------------- | :------------------------------------------- |
| Instances.N.Dimensions.0.Name  | clusterId   | 为集群 ID                                                    | 输入String 类型维度名称，如：clusterId       |
| Instances.N.Dimensions.0.Value | clusterId   | 具体的集群 ID，[查询集群列表](https://intl.cloud.tencent.com/document/product/457/32025) 接口中返回的clusterId（集群的ID）字段 | 输入集群具体 ID，如：cls-xxxxx               |
| Instances.N.Dimensions.1.Name  | serviceName | 为服务名称                                                   | 输入String类型维度名称，如：serviceName      |
| Instances.N.Dimensions.1.Value | serviceName | 具体的服务名称，查询服务列表接口中返回的 serviceName（服务名）字段 | 输入具体服务名称，如 ：test                  |
| Instances.N.Dimensions.2.Name  | namespace   | 为命名空间名称                                               | 输入String类型维度名称，如：namespace        |
| Instances.N.Dimensions.2.Value | namespace   | 具体的空间名称，查询服务列表接口中返回的 namespace（命名空间）字段 | 输入命名空间具体名称，如： default           |
...
| Instances.N.Dimensions.4.Name  | containerId | 为实例名称                                                   | 输入String类型维度名称，如：containerId      |
| Instances.N.Dimensions.4.Value | containerId | 具体的实例名称，查询服务实例列表接口中返回的 containerId（容器 ID）字段。注意：容器 ID 只取开头12位即可 | 输入实例具体名称，如：01c5509d2b39           |
