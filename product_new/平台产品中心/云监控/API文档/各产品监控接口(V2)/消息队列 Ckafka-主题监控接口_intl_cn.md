...
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如消息队列主题命名空间： QCE/CKAFKA（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称   | 维度解释          | 格式                                    |
| :----------------------------- | :--------- | :---------------- | :-------------------------------------- |
| Instances.N.Dimensions.0.Name  | instanceId | instanceId        | 输入String 类型维度名称，如：instanceId |
| Instances.N.Dimensions.0.Value | instanceId | ckafka 实例的 ID  | 输入实例具体ID，如：ckafka-test         |
| Instances.N.Dimensions.1.Name  | topicId    | topicId           | 输入String 类型维度名称，如：topicId    |
| Instances.N.Dimensions.1.Value | topicId    | 实例所在主题的 ID | 输入主题具体 ID，如： topic-test        |
