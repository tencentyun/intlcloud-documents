...
负载均衡（Cloud Load Balancer）是对多台云服务器进行流量分发的服务。具体介绍请参考 [负载均衡简介](https://intl.cloud.tencent.com/document/product/214/524) 页面。 当前负载均衡支持四种命名空间 QCE/LB_PUBLIC、QCE/LB_PRIVATE、QCE/LB_RS_PRIVATE 和 QCE/LOADBALANCE。
...
QCE/LB_RS_PRIVATE是内网负载均衡后端服务器维度的命名空间，您可以在此空间查询到内网负载均衡后端服务器维度相关监控数据。 QCE/LB_RS_PRIVATE支持以下2种维度组合的查询方式，四种入参取值如下：
...
以下请求参数列表仅列出了接口请求参数和部分公共请求参数，正式调用时需要加上公共请求参数，详情请参见 [公共请求参数](https://intl.cloud.tencent.com/document/product/248/33876#.E7.AD.BE.E5.90.8D.E6.96.B9.E6.B3.95-v1) 文档。
...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据；可查看内网负载均衡支持的 [地域列表](https://intl.cloud.tencent.com/document/product/214/33792) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如内网负载均衡后端服务器维度命名空间：QCE/LB_RS_PRIVATE （API 3.0接口版本的必须是大写) |
...
| 参数名称                       | 维度名称         | 维度解释                  | 格式                                           |
| :----------------------------- | :--------------- | :------------------------ | :--------------------------------------------- |
| Instances.N.Dimensions.0.Name  | vip              | 负载均衡 VIP              | 输入String 类型维度名称，如：vip               |
| Instances.N.Dimensions.0.Value | vip              | 负载均衡 VIP              | 输入具体 IP 地址，如：111.111.111.11           |
| Instances.N.Dimensions.1.Name  | vpcId            | 负载均衡所在私有网络的 ID | 输入String 类型维度名称，如：vpcId             |
| Instances.N.Dimensions.1.Value | vpcId            | 负载均衡所在私有网络的 ID | 输入私有网络具体 ID，如：1111                  |
| Instances.N.Dimensions.2.Name  | loadBalancerPort | 负载均衡端口              | 输入 String 类型维度名称，如：loadBalancerPort |
| Instances.N.Dimensions.2.Value | loadBalancerPort | 负载均衡端口              | 输入具体端口号，如：80                         |
| Instances.N.Dimensions.3.Name  | protocol         | 协议                      | 输入 String 类型维度名称，如：protocol         |
| Instances.N.Dimensions.3.Value | protocol         | 协议                      | 输入协议值具体名称，如：http                   |
| Instances.N.Dimensions.4.Name  | lanIp            | 负载均衡绑定机器的 IP     | 输入String 类型维度名称，如：lanIp             |
| Instances.N.Dimensions.4.Value | lanIp            | 负载均衡绑定机器的 IP     | 输入具体 IP 地址，如：111.111.111.11           |
| Instances.N.Dimensions.5.Name  | port             | 负载均衡绑定机器的端口号  | 输入String类型维度名称，如：port               |
| Instances.N.Dimensions.5.Value | port             | 负载均衡绑定机器的端口号  | 输入具体端口号，如：80                         |
