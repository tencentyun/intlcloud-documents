...
To query the monitoring data of a CCN instance in a VPC, use the following input parameters:
...
| Namespace | Yes | String | Namespace. Each Tencent Cloud product has a namespace, such as `QCE/VBC`. This value must be capitalized for API 3.0 |
...
| ------------------ | ---------------- | ------------- | ----------------------------- |
| Instances.N.Dimensions.0.Name | CcnId | CCN instance ID | String-type dimension name: CcnId |
| Instances.N.Dimensions.0.Value | CcnId | Specific CCN instance ID | Specific instance name, such as ccn-12345adc |
| Instances.N.Dimensions.0.Name | SRegion | Source region | String-type dimension name: SRegion |
| Instances.N.Dimensions.0.Value | SRegion | Specific source region | Specific region name, such as ap-shanghai |
| Instances.N.Dimensions.0.Name | DRegion | Destination region | String-type dimension name: DRegion |
| Instances.N.Dimensions.0.Value | DRegion | Specific destination region | Specific region name, such as ap-guangzhou |
...
### 2.2.1. Single-region metric name
...
|RegionInBandwidth| Single-region inbound bandwidth |Mbsp| Single-region: CcnId|

### 2.2.2. Cross-region metric name
| Metric Name | Description | Unit | Dimension |
| ------------------ | :------- | :---- | :------ |
|OutPkg| Cross-region outbound packets | Packets/sec | Cross-region: CcnId, SRegion, DRegion|
|InPkg| Cross-region inbound packets | Packets/sec | Cross-region: CcnId, SRegion, DRegion|
|OutBandwidth| Cross-region outbound bandwidth |Mbps| Cross-region: CcnId, SRegion, DRegion|
|InBandwidth| Cross-region inbound bandwidth |Mbps| Cross-region: CcnId, SRegion, DRegion|
...
### Sample 1 
This example shows you how to get the number of single-region outbound packets of one CCN instance in a VPC using a statistical period of 60 seconds for a specified length of time.
...
```
### Sample 2 
This example shows you how to get the number of single-region outbound packets of multiple CCN instances in a VPC using a statistical period of 60 seconds for a specified length of time.
