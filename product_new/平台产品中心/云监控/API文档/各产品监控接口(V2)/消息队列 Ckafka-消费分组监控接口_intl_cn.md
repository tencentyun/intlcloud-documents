...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据；可查看消息队列 Ckafka支持的 [地域列表](https://intl.cloud.tencent.com/document/product/597/35335#.E5.9C.B0.E5.9F.9F.E5.88.97.E8.A1.A8) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如消息队列分组消费命名空间： QCE/CKAFKA（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称      | 维度解释       | 格式                                                   |
| :----------------------------- | :------------ | :------------- | :----------------------------------------------------- |
| Instances.N.Dimensions.0.Name  | consumerGroup | consumerGroup  | 输入String 类型维度名称，如：consumerGroup             |
| Instances.N.Dimensions.0.Value | consumerGroup | 消费分组信息   | 输入用户需要查看的消费分组信息，如：perf-consumer-8330 |
| Instances.N.Dimensions.1.Name  | instanceId    | instanceId     | 输入String 类型维度名称，如：instanceId                |
| Instances.N.Dimensions.1.Value | instanceId    | 实例 ID        | 输入用户需要查看监控的实例 ID，如 ：ckafka-test        |
| Instances.N.Dimensions.2.Name  | topicId       | topicId        | 输入String 类型维度名称，如：topicId                   |
| Instances.N.Dimensions.2.Value | topicId       | 主题 ID        | 输入用户订阅主题 ID，如 ：topic-test                   |
| Instances.N.Dimensions.3.Name  | partition     | partition      | 输入String 类型维度名称，如：partition                 |
| Instances.N.Dimensions.3.Value | partition     | partition 信息 | 输入topic 分区信息，如：0                              |
| Instances.N.Dimensions.4.Name  | topicName     | topicName      | 输入String 类型维度名称，如：topicName                 |
| Instances.N.Dimensions.5.Value | topicName     | 主题名称       | 输入用户消费主题的名称，如：test                       |
