...
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如私有网络VPN网关命名空间： QCE/VPNGW（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称 | 维度解释           | 格式                                   |
| :----------------------------- | :------- | :----------------- | :------------------------------------- |
| Instances.N.Dimensions.0.Name  | vpnGwId  | 为 VPN 网关 ID     | 输入String 类型维度名称，如：vpnGwId   |
| Instances.N.Dimensions.0.Value | vpnGwId  | 具体的 VPN 网关 ID | 输入vpn网关具体 ID，如：vpngw-q7v069tf |
...
拉取某个私有网络 VPN 网关某段时间内统计周期为60秒的外网出带宽监控数据。
...
拉取多个私有网络 VPN 网关某段时间内统计周期为60秒的外网出带宽监控数据。
