...
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如专线通道命名空间：QCE/DCX（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称            | 维度解释          | 格式                                                         |
| :----------------------------- | :------------------ | :---------------- | :----------------------------------------------------------- |
| Instances.N.Dimensions.0.Name  | directConnectConnId | 入参为专线通道 ID | 输入String类型维度名称，如：directConnectConnId              |
| Instances.N.Dimensions.0.Value | 专线通道的具体 ID   | 专线通道的具体 ID | String类型，调用 [DescribeInstances](https://intl.cloud.tencent.com/document/product/213/33258) 接口获取的unInstanceId字段 |
...
拉取某个专线通道某段时间内统计周期为300秒的入带宽监控数据。
...
拉取多个专线通道某段时间内统计周期为300秒的入带宽监控数据。
