...
以下请求参数列表仅列出了接口请求参数和部分公共请求参数，正式调用时需要加上全部公共请求参数，详情请参见 [公共请求参数](https://intl.cloud.tencent.com/document/product/248/33876#.E7.AD.BE.E5.90.8D.E6.96.B9.E6.B3.95-v1) 文档。
...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据；可查看ES支持的 [地域列表](https://intl.cloud.tencent.com/document/product/845/32206) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如ES命名空间：QCE/CES（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称    | 维度解释     | 格式                                    |
| :----------------------------- | :---------- | :----------- | :-------------------------------------- |
| Instances.N.Dimensions.0.Name  | uInstanceId | ES 实例的 ID | 输入String类型维度名称，如：uInstanceId |
| Instances.N.Dimensions.0.Value | uInstanceId | ES 实例的 ID | 输入实例具体ID，如 es-example           |
