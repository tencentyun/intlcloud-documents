...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据；可查看云数据库 SQL Server支持的 [地域列表](https://intl.cloud.tencent.com/document/product/238/32088) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如云数据库 SQL Server命名空间： QCE/SQLSERVER（API 3.0接口版本的必须是大写） |
...
| Instances.N.Dimensions.0.Value | resourceId | 实例具体的资源 Id | 输入实例的具体resourceId，如：mssql-dh0123456 |
...
| NetworkIoWaits      | 平均网络 IO 延迟时间                      | Ms    |
