...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据；可查私有网络支持的 [地域列表](https://intl.cloud.tencent.com/document/product/215/15758) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如私有网络专线网关命名空间：QCE/DCG（API 3.0接口版本的必须是大写） |
...
| 参数名称                       | 维度名称               | 维度解释          | 格式                                                |
| :----------------------------- | :--------------------- | :---------------- | :-------------------------------------------------- |
| Instances.N.Dimensions.0.Name  | directConnectGatewayId | 入参为专线网关 ID | 输入String 类型维度名称，如：directConnectGatewayId |
| Instances.N.Dimensions.0.Value | directConnectGatewayId | 具体的专线网关 ID | 输入专线网关具体 ID，如： dcg-4d545d                |
...
拉取某个私有网络专线网关某段时间内统计周期为60秒的外网出带宽监控数据
...
拉取多个私有网络专线网关某段时间内统计周期为60秒的外网出带宽监控数据
