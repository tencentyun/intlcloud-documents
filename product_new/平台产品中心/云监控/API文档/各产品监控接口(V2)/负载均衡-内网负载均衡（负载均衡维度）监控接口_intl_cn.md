...
QCE/LB_PRIVATE是内网负载均衡负载均衡维度的命名空间，您可以在此空间查询到内网负载均衡维度相关监控数据。 QCE/LB_PRIVATE支持以下2种维度组合的查询方式，四种入参取值如下：
...
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如内网负载均衡负载均衡维度命名空间： QCE/LB_PRIVATE（API 3.0接口版本的必须是大写） |
...
| &Instances.N.Dimensions.1.name  | vpcId            | 负载均衡所在私有网络的 ID | 输入String类型维度名称，如：vpcId             |
| &Instances.N.Dimensions.1.value | vpcId            | 负载均衡所在私有网络的 ID | 输入私有网络具体 ID，如：1012345              |
| &Instances.N.Dimensions.2.name  | loadBalancerPort | 负载均衡端口              | 输入String 类型维度名称，如：loadBalancerPort |
| &Instances.N.Dimensions.2.value | loadBalancerPort | 负载均衡端口              | 输入具体端口号，如：80                        |
| &Instances.N.Dimensions.3.name  | protocol         | 协议                      | 输入String 类型维度名称，如：protocol         |
