...
Cloud Load Balancer (CLB) distributes traffic to multiple CVM instances. For more information, please see the [CLB overview page](https://intl.cloud.tencent.com/zh/document/product/214). Currently, CLB supports four namespaces: `qce/lb_public`, `qce/lb_private`, `qce/lb_rs_private`, and `qce/loadbalance`.

QCE/LB_PUBLIC: namespace for public network CLB, which includes monitoring data in the CLB instance and real server dimensions. QCE/LB_PRIVATE: namespace for private network CLB, which includes monitoring data in the CLB instance dimension. QCE/LB_RS_PRIVATE: namespace for private network CLB, which includes monitoring data in the real server dimension. QCE/LOADBALANCE: namespace for layer-7 CLB.
...
To query the monitoring data of a public network CLB instance, use the namespace of public network CLB to query all monitoring data of the instance. The following four dimension combinations are supported for querying, and the input parameter values are as follows:

### 1.1. Input parameter values in CLB instance dimension

&Namespace: QCE/LOADBALANCE
&Instances.N.Dimensions.0.Name=vip
&Instances.N.Dimensions.0.Value=IP address

### 1.2. Input parameter values in CLB port dimension
...
### 1.3. Input parameter values in single CLB domain name dimension
...
### 1.4. Input parameter values in single CLB URL dimension
...
### 1.5. Input parameter values in all CLB domain name dimension (if one domain name is shared by multiple VIPs, the data will be aggregated)
...
### 1.6. Input parameter values in CLB real server dimension
...
### 1.7. Input parameter values in CLB real server port dimension
...
| Namespace | Yes | String | Namespace. Each Tencent Cloud product has a namespace, such as `QCE/LOADBALANCE`, which must be capitalized for API 3.0 |
...
This example shows you how to get the QPS monitoring data of one CLB instance at 300-second statistical period during a certain period of time.
...
This example shows you how to get the HttpCode2XX monitoring data of one CLB instance at 300-second statistical period during a certain period of time.
