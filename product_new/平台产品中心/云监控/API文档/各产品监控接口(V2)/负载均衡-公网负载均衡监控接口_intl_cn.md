...
| Region      | 否       | String                                                       | 公共参数，表示查询的是哪个地域实例的监控数据；可查看负载均衡支持的 [地域列表](https://intl.cloud.tencent.com/document/product/214/33792) |
| Namespace   | 是       | String                                                       | 命名空间，每个云产品会有一个命名空间，如公网负载均衡命名空间：QCE/LB_PUBLIC（API 3.0接口版本的必须是大写） |
...
| 参数名称                        | 维度名称         | 维度解释                  | 格式                                          |
| :------------------------------ | :--------------- | :------------------------ | :-------------------------------------------- |
| &Instances.N.Dimensions.0.name  | vip              | 负载均衡 VIP              | 输入String类型维度名称，如：vip               |
| &Instances.N.Dimensions.0.value | vip              | 负载均衡 VIP              | 输入具体ip地址，如：111.111.111.11            |
| &Instances.N.Dimensions.1.name  | loadBalancerPort | 负载均衡端口              | 输入 String类型维度名称，如：loadBalancerPort |
| &Instances.N.Dimensions.1.value | loadBalancerPort | 负载均衡端口              | 输入具体端口号，如：80                        |
| &Instances.N.Dimensions.2.name  | protocol         | 协议                      | 输入String类型维度名称，如：protocol          |
| &Instances.N.Dimensions.2.value | protocol         | 协议                      | 输入协议值具体名称，如：http                  |
| &Instances.N.Dimensions.3.name  | vpcId            | 负载均衡所在私有网络的 ID | 输入String类型维度名称，如：vpcId             |
| &Instances.N.Dimensions.3.value | vpcId            | 负载均衡所在私有网络的 ID | 输入私有网络具体ID，如：1111                  |
| &Instances.N.Dimensions.4.name  | lanIp            | 负载均衡绑定机器的 IP     | 输入String类型维度名称，如：lanIp             |
| &Instances.N.Dimensions.4.value | lanIp            | 负载均衡绑定机器的 IP     | 输入具体ip地址，如：111.222.111.22            |
