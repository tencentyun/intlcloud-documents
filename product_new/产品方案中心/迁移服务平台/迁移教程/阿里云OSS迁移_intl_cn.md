...
下文将详细介绍当源对象存储部署在阿里云OSS时，如何配置 Agent 半托管迁移任务，实现数据迁移。
...
1. Agent 半托管模式如果是通过专线迁移，需要确保阿里云侧主机上使用的 COS SDK，可2、经过专线访问 COS，迁移前请与商务经理确认。
...
5. 勾选控制台密码登录和编程访问，之后填写用户账号信息。

6. 保存生成的账号、密码、AccessKeyID 和 AccessKeySecret。

7. 勾选用户登录名称，单击【添加权限】，授予子账号存储空间读写权限（AliyunOSSFullAccess）。
