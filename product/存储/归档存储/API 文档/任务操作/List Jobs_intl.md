...
This operation (List Jobs) lists jobs for a vault, including jobs that are in-progress and jobs that have recently finished.
...
| ---------- | ---------------------------------------- | ------ | ---- |
| Completed | Status of the jobs to be returned. Enumerated values: `true`, `false`. | Boolean | No |
| limit | The maximum number of jobs to be returned. It is 1000 by default | String | No |
| marker | Read entries from Marker in lexicographical order | String | No |
| statuscode | Status type of the jobs to be returned. Enumerated values: `InProgress`, `Succeeded`, `Failed` | String | No |
...
| StatusCode                             | Job status code. Enumerated values: `Succeeded`, `Failed` or `InProgress` | String |
| StatusMessage | Job status message | String |
| VaultQCS                               | Vault QCS for the job                           | String |
| Marker | If the returned Joblist (in lexicographical order) is truncated, marker represents the starting point of the next read. | String |
...
| InventoryRetrievalParameters.Format | Output format of archive inventory. Enumerated values: `CSV`, `JSON`. | String |
| InventoryRetrievalParameters.StartDate | The UTC date that the vault inventory starts. It is in an ISO 8601 date format, for example, `2013-03-20T17:03:43.221Z` | ISO 8601 |
| InventoryRetrievalParameters.EndDate | The UTC date that the vault inventory ends. It is in an ISO 8601 date format, for example, `2013-03-20T17:03:43.221Z` | ISO 8601 |
| InventoryRetrievalParameters.Limit | The maximum number of items returned for each vault inventory request. It should be a positive integer | String |
| InventoryRetrievalParameters.Marker | Read the archive inventory from Marker in lexicographical order | String |
