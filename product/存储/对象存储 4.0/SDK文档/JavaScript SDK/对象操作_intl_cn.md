...
本文档重点提供关于对象的简单操作、分块操作以及其他操作相关的 API 概览以及 SDK 示例代码，并举例如何使用。
...
| [GET Object](https://intl.cloud.tencent.com/document/product/436/7753) | 下载对象       | 下载一个对象                             |
...
| Region       | Bucket 所在地域，枚举值请参阅 [地域和访问域名](https://intl.cloud.tencent.com/document/product/436/6224) | String | 是   |
...
| 参数名           | 参数描述                                                     | 类型        |
| ---------------- | ------------------------------------------------------------ | ----------- |
| err              | 请求发生错误时返回的对象，包括网络错误和业务错误。如果请求成功则为空，更多详情请参阅 [错误码](https://intl.cloud.tencent.com/document/product/436/7730) 文档 | Object      |
...
| Bucket                 | Bucket 的名称，命名规则为 BucketName-APPID，此处填写的存储桶名称必须为此格式 | String           | 是   |
| Region                 | Bucket 所在地域，枚举值请参阅 [地域和访问域名](https://intl.cloud.tencent.com/document/product/436/6224) | String           | 是   |
| Key                    | 对象键（Object 的名称），对象在存储桶中的唯一标识，详情请参阅 [对象键说明](https://intl.cloud.tencent.com/document/product/436/13324) | String           | 是   |
...
JS SDK 未提供 POST Object 接口对应的方法，如果需要使用该接口，可参考 [Web 端直传实践](https://intl.cloud.tencent.com/document/product/436/9067) 里的“方案 B：使用 Form 表单上传”。
...
> !该接口用于读取对象内容，如果需要发起浏览器下载文件，可以通过 cos.getObjectUrl 获取 url 再触发浏览器下载，具体参考 [预签名 URL](https://intl.cloud.tencent.com/document/product/436/35651) 文档。
...
| - - Key                   | 对象键（Object 的名称），对象在存储桶中的唯一标识，详情请参阅 [对象键说明](https://intl.cloud.tencent.com/document/product/436/13324) | String      |
...
| MaxUploads   | 设置最大返回的 multipart 数量，合法取值为1 - 1000，默认1000  | String | 否   |
...
| - MaxUploads         | 设置最大返回的 multipart 数量，合法取值为 1 - 1000           | String      |
...
| - - ID               | 上传发起者 ID，格式：qcs::cam::uin/&lt;OwnerUin>:uin/&lt;SubUin> 如果是主账号，&lt;OwnerUin> 和 &lt;SubUin> 是同一个值 | String      |
| - Owner              | 用来表示这些分块所有者的信息                                 | Object      |
| - - DisplayName      | Bucket 持有者的名称                                          | String      |
| - - ID               | Bucket 持有者 ID，格式：qcs::cam::uin/&lt;OwnerUin>:uin/&lt;SubUin> 如果是主账号，&lt;OwnerUin> 和 &lt;SubUin> 是同一个值 | String      |
...
Initiate Multipart Upload 请求实现初始化分片上传，成功执行此请求后会返回 Upload ID ，用于后续的 Upload Part 请求。
...
| Region                      | Bucket 所在地域，枚举值请参阅 [地域和访问域名](https://intl.cloud.tencent.com/document/product/436/6224) | String | 是   |
| Key                         | 对象键（Object 的名称），对象在存储桶中的唯一标识，详情请参阅 [对象键说明](https://intl.cloud.tencent.com/document/product/436/13324) | String | 是   |
| CopySource                  | 源对象 URL 路径，可以通过 versionid 子资源指定历史版本       | String | 是   |
| CopySourceRange             | 源对象的字节范围，范围值必须使用 bytes=first-last 格式，first 和 last 都是基于 0 开始的偏移量。例如 bytes=0-9 表示您希望拷贝源对象的开头10个字节的数据 ，如果不指定，则表示拷贝整个对象 | String | 否   |
| CopySourceIfMatch           | 当 Object 的 Etag 和给定一致时，则执行操作，否则返回412，可与 x-cos-copy-source-If-Unmodified-Since 一起使用，与其他条件联合使用返回冲突 | String | 否   |
| CopySourceIfNoneMatch       | 当 Object 的 Etag 和给定不一致时，则执行操作，否则返回412，可与 x-cos-copy-source-If-Modified-Since 一起使用，与其他条件联合使用返回冲突 | String | 否   |
| CopySourceIfUnmodifiedSince | 当 Object 在指定时间后未被修改，则执行操作，否则返回412，可与 x-cos-copy-source-If-Match 一起使用，与其他条件联合使用返回冲突 | String | 否   |
| CopySourceIfModifiedSince   | 当 Object 在指定时间后被修改，则执行操作，否则返回412，可与 x-cos-copy-source-If-None-Match 一起使用，与其他条件联合使用返回冲突 | String | 否   |
...
| 参数名       | 参数描述                                                     | 类型   |
| ------------ | ------------------------------------------------------------ | ------ |
| err          | 请求发生错误时返回的对象，包括网络错误和业务错误。如果请求成功则为空，更多详情请参阅 [错误码文档](https://intl.cloud.tencent.com/document/product/436/7730) | Object |
| - statusCode | 请求返回的 HTTP 状态码，如200、403、404等                    | Number |
| - headers    | 请求返回的头部信息                                           | Object |
| data         | 请求成功时返回的对象，如果请求发生错误，则为空               | Object |
| - statusCode | 请求返回的 HTTP 状态码，如200、403、404等                    | Number |
...
cos.sliceCopyFile({
    ...
    Region: 'ap-beijing',                                  /* 必须 */
    Key: '1.zip',                                            /* 必须 */
    CopySource: 'test.cos.ap-guangzhou.myqcloud.com/2.zip', /* 必须 */
    onProgress:function (progressData) {                     /* 非必须 */
        console.log(JSON.stringify(progressData));
    }
},function (err,data) {
...
完整的队列使用示例请参阅 [demo-queue](https://github.com/tencentyun/cos-js-sdk-v5/tree/master/demo/queue)。
