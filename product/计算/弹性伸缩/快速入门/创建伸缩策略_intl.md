...
In the scaling group, the alarm-triggered policy automatically adds or removes the specified number of percentage of CVM instances based on Cloud Monitor performance metrics (such as CPU, memory, and bandwidth). You can also copy an existing policy in an existing scaling group to the current scaling group.
