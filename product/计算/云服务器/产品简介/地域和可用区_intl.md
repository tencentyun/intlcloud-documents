...
## Region
Regions are completely isolated with each other, so as to ensure high stability and fault tolerance. Tencent Cloud now have 3 regions in Mainland China and provides nodes in Hong Kong SAR, Singapore and Toronto. You are recommended to choose nearest region to reduce access latency and improve download speed.
...
**Notes for Hong Kong SAR region:**
...
- When you need to log in to CVMs in Hong Kong SAR region, log in via jump server is recommended for better operation and maintenance experience.
