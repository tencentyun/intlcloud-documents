...
> - 创建自定义镜像的同时系统默认会创建相关快照，删除此快照之前需要先删除关联的镜像。当前快照已经商业化，保留自定义镜像会产生一定的快照费用。
...
您可以使用 CreateImage 接口创建自定义镜像。具体内容可以参考 [创建镜像 API](https://intl.cloud.tencent.com/document/api/213/33276)。
