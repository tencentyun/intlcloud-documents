...
该服务模式下 Kubernetes 集群的 Master 和 Etcd 会由腾讯云技术团队集中管理和维护。您只需要购置集群运行负载所需的工作节点即可，不需要关心集群的管理面。
