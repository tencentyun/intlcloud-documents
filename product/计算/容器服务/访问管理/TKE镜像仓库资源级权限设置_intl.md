...
> The `${tag}` field currently only implements authentication for the deletion operation. For more information, see [Image Tag Permissions](#镜像Tag权限).
...
If you do not need to manage Image Registry permissions sophisticatedly, you can use the [preset policy authorization](#预设策略授权).
If you need to manage collaborator permissions sophisticatedly, use the [custom policy authorization](#自定义策略授权).
...
This preset policy configures all permissions of the TKE image service. After a collaborator is associated with this policy, they will have the same Image Registry permissions as the developer. For details, see the [permission list](#权限列表).
