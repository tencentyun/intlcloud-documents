...
如果您需要 [大禹网络安全](/document/product/297) 为 VPN 网关提供超大带宽的 DDoS 和 CC 防护，您可以将高防包绑定到 VPN 网关上，实现安全防护。
...
使用 SPD 规则时请注意以下限制：
 ...
 - 同一条规则的多个对端网段，相互不能重叠。
 - 每条规则的本端网段必须在 [私有网络](/document/product/215/4927) 的网段内。
 - 每条规则的对端网段不能在 [私有网络](/document/product/215/4927) 的网段内。
- 下面是一个正确的实例：
