...
| ------------------------------------- | ------------------------------------- | ------------------------------------- |
|소재지, 성명, 이메일 주소, IP 주소, 이용자 에이전트 (또는 웹 사이트 브라우저), 장치 ID 및 Tencent Cloud ID, 주소 (도시 및 우편 번호 포함)  |이 정보를 사용하여 다음을 수행합니다:· 귀하의 요청에 따라 Tencent Cloud 계정을 생성;· 비밀번호 재설정 허용;· 이용자 지원 제공; 및</br>· 당사의 약관, 조건 및 정책 시행</br>      | 서비스 제공을 위해 귀하와 계약을 수행하는 데 필요하므로 이 정보를 처리합니다. |
...
|문의 데이터 (웹 사이트 연락처 양식을 통해 또는 판매 문의의 일부로 문의서를 제출할 때 제공되는 데이터):•귀하의 이름, 이메일, 회사명 및 비즈니스 서비스) [필수]</br>•전화 번호 및 위치 [선택사항]</br>  | 당사는 귀하의 요청에 응답하기 위해 이 정보를 사용합니다.  | 귀하의 요청에 응답하기 위해 필요한 이 정보를 처리하는 것은 당사의 정당한 이익입니다. |
