...
<tr>
	<td>Password</td>
	<td>Password is required. As MQTT.fx sets the password flag to true by default, you need to enter a random non-empty string as the password; otherwise, you will not be able to connect to IoT Hub's backend. When accessing IoT Hub's backend, the authentication is based on the certificate, and the random password entered here will not be used as the access credential. </td>
</tr>
</table>
5. Select "Enable SSL/TLS" and "Self signed certificates" and upload the related files.
Because IoT Hub uses an encrypted secure link, you also need to set up the SSL/TLS information. MQTT.fx does not support symmetric encryption PSK. This documents only demonstrates the settings for asymmetric encrypted connection.
