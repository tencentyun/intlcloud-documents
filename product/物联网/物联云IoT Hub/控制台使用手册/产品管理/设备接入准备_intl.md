...
2. Click **Create Product**, enter "Product name" and "Product description", select "Region", "Product type", "Authentication method" and "Data format", and click **Create** to generate the product. A product is essentially a collection of devices of a certain type, and you can manage all the devices through the product.
 ...
 Certificate authentication: When a device is created, the platform will generate a certificate file and a private key file for the device to be used for device authentication before the device is connected to IoT Hub.
 Key authentication: When a device is created, you can use a custom PSK or a PSK randomly generated by the platform for the device.
 - Data format: JSON format supports rule matching and content extraction based on data, while custom format does not do any data parsing.
...
If the selected product authentication method is certificate authentication, the device private key is the unique identifier used by the device to connect to the IoT Hub backend. The private key is not stored in the backend. Please keep it safe and private.
...
1. The authentication method is certificate authentication:
...
2. The authentication method is key authentication:
Device details include all management content for a single device: device name, device category, device note, online status, version information, device key (click to show), device shadow status, ISP type (for NB-IoT products) and IMEI (for NB-IoT products).
