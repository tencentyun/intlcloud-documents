...
![](https://main.qcloudimg.com/raw/5025ebb41c5501460df41b8f02c08a11.png)
进入添加域名页面，您可以在此完成域名的相关配置。
![](https://main.qcloudimg.com/raw/66262ac7549120cdb7af05e40f15c84d.png)

### 域名配置
1. 在 **域名** 位置填入需要加速的域名。支持泛域名接入，如：```*.test.com```。支持域名批量接入，通过单击【添加】按钮最多可添加 10 个域名。
域名需要满足以下条件：
 + 域名已经在工信部进行过备案
 + 域名尚未被接入过腾讯云 CDN
...
源站类型可分为自有源和 COS 源。
 + 自有源：如果您已经拥有稳定运行的业务服务器（即源站），您可以通过自有源方式接入 CDN。源站本身无需做任何修改，仅通过 CDN 控制台接入流程，以及 DNS 配置即可享受加速服务。接入方式如下：
     ...
     2. 若填入的为域名，则 **必须不能** 与访问域名（即接入的加速域名）一样。支持“域名：port”格式，端口号需 **＞0且≤65535**。
     3. 若填入的为 IP，可填入多个 IP，支持“IP:port”格式，端口号 **＞0且≤65535**。当填入多个 IP 的时候，回源请求会依次轮流访问各 IP。
     4. 填入的 IP 不能是内网 IP。
 + COS源：若您想使用腾讯云 [对象存储](https://cloud.tencent.com/product/cos.html) 存储静态加速内容，可以直接使用 COS 源接入方式，将域名接入 CDN。接入方式如下：
     1. **源站类型** 为对象存储（COS），您可通过下拉菜单或输入关键字选择 Bucket 的域名。
     2. 若对应项目下无存储桶，您需要登录 [COS 控制台](https://console.cloud.tencent.com/cos5) 创建存储桶（如何创建？详情请参见 [创建存储桶](https://cloud.tencent.com/document/product/436/6245#.E5.88.9B.E5.BB.BA.E5.AD.98.E5.82.A8.E6.A1.B6)），或联系您的开发商账号为您分配对应的存储桶使用权限。
     3. 选择了存储桶作为源站后，您可以在 [COS 控制台](https://console.cloud.tencent.com/cos5) 对源站内容进行管理。
...
### 服务配置
选择加速服务业务类型及基础配置。
![](https://main.qcloudimg.com/raw/9ac0004a542a722bbe8973ba9329b56a.png)
...
![](https://main.qcloudimg.com/raw/e0d7440fde49c29af080f84c35a7d6f8.png)
