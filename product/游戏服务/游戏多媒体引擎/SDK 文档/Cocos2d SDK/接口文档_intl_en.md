This document describes the integration for Cocos2d in detail to help Cocos2d developers debug and integrate APIs for Tencent Cloud's Game Multimedia Engine (GME).
...
| strRoomID |char* | Room ID;  127 characters maximum (The room ID for voice message must be set to "null") |
...
### Enter a room
When you enter a room with the generated authentication credentials, you receive the ITMG_MAIN_EVENT_TYPE_ENTER_ROOM callback message. By default, microphone and speaker will not be enabled after you enter the room. If AV_OK is returned, it indicates the callback is successful.
...
For room's audio type, please see [Sound Quality Selection](https://intl.cloud.tencent.com/document/product/607/18522.
...
|GetRecvStreamLevel					|Obtains real-time receiving volume of other members in the room|
...
|Parameter     | Type         |Description|
| ------------- |:-------------:|-------------|
| enable    |bool     | To enable the device, set to true; and to disable the device, set to false.|
...
|Parameter     | Type         |Description|
| ------------- |:-------------:|-------------|
| enable    |bool     |To enable audio sending, set to true; and to disable audio sending, set to false.|
...
###  Obtain audio sending status
This API is used to obtain audio sending status.
#### Function prototype  
```
...
```
### Obtain microphone volume
This API is used to obtain microphone volume. An int value is returned. The returned value of 101 indicates SetMicVolume has not been called.
...
|Parameter     | Type         |Description|
| ------------- |:-------------:|-------------|
| enable    |bool     |To enable audio receiving, set to true; and to disable audio receiving, set to false.|
...
### Obtain real-time receiving volume of other members in the room
This API is used to obtain real-time receiving volume of other members in the room. An int value is returned, ranging from 0 to 100.
####  Function prototype  
```
...
### Set speaker volume
This API is used to set speaker volume.
...
"Level" indicates the real-time volume, and "Volume" is the speaker volume. The ultimate volume equals to Level*Volume%. For example, if the value for "Level" is 100 and the one for "Volume" is 60, the ultimate volume will be "60".
...
The callback function OnEvent is called after the recording is started. The event message ITMG_MAIN_EVNET_TYPE_PTT_RECORD_COMPLETE is returned, and the action of this event should be implemented in OnEvent function.
...
|4098   |Initialization error|Check if the device is occupied, the permissions are normal, or the initialization is normal.|
...
| translateLanguage    |char*                     |Translate into specified language. For details, see [Language Reference List](https://intl.cloud.tencent.com/document/product/607/30260) (This parameter is not available. Enter the value for speechLanguage.)|
...
|32777	|Recording and uploading are successful, but streaming voice to text is failed.	|The message returned includes a URL for successful upload. Call the SpeechToText API to perform voice-to-text conversion.
...
### Stop recording
This API is used to stop recording. It is an asynchronous API. There will be a callback after the recording is stopped. Only when the recording is successful, the recorded file can be used. 
#### Function prototype  
```
...
### Cancel recording
This API is used to cancel recording. No callback message is returned.
#### Function prototype  
```
...
| Parameter | Type | Description |
| ------------- |:-------------:|-------------|
| filePath    |char* | Indicates the path of a voice file |
#### Sample code  
```
...
| fileID    |char*                     |Indicates the URL of a voice file|
| speechLanguage    |char*                     |Recognize specified language. For details, see [Language Reference List](https://intl.cloud.tencent.com/document/product/607/30260)|
| translatelanguage    |char*                  |Translate into specified language. For details, see [Language Reference List](https://intl.cloud.tencent.com/document/product/607/30260)（This parameter is currently valid. The input parameter should be consistent with the speechLanguage.|
...
This API is used to set the print log level. It is recommended to use the default level
#### Function prototype
```
...
| Parameter | Type | Description |
| ------------- |:-------------:|-------------|
|levelWrite|ITMG_LOG_LEVEL|Set the write log level. TMG_LOG_LEVEL_NONE indicates no write.|
|levelPrint|ITMG_LOG_LEVEL|Set the print log level. TMG_LOG_LEVEL_NONE indicates no print.|
...
This API is used to remove an ID from the audio data blacklist. A return value of 0 indicates that the call succeeded.
#### Function prototype  
