This document describes the integration for Windows in details to help Windows developers debug and integrate APIs for Tencent Cloud's Game Multimedia Engine (GME).
...
### Get a singleton
GME SDK is provided as a singleton. Get ITMGContext single instance first and all calls should be done according to this instance. ITMGDelegate is defined in the Application which is used to receive and process the callback from this instance.
#### Function prototype 
...
| strRoomID    		|char*     |Room ID, maximum to 127 characters (The room ID parameter for voice message must be set to "null").|
...
We recommend the following methods when enabling/disabling the microphone or speaker in the user page:
...
| speechLanguage    |char*                     |Recognizes specified language. See [language reference list](https://github.com/TencentMediaLab/GME/blob/master/GME%20Developer%20Manual/GME%20SpeechToText.md2)|
