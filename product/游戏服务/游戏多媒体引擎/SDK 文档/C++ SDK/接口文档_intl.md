...
### Get a singleton
GME SDK is provided as a singleton. ITMGContext must be configured first, because all calls begin from ITMGContext and are called and passed back to your application via ITMGDelegate.    
#### Function prototype 
...
```
ITMGContext* context = ITMGContextGetInstance();
context->EnterRoom(roomId, ITMG_ROOM_TYPE_STANDARD, (char*)retAuthBuff,bufferLen);//Sample code for entering a room in team chatting mode
```
...
This API is used to identify whether any member has entered a room. A bool value is returned.
#### Function prototype  
```
...
Notification about this event is sent only when the status changes. To obtain the member status in real time, cache it when receiving notifications at a higher layer. The event message ITMG_MAIN_EVNET_TYPE_USER_UPDATE is returned. The "data" includes event_id and user_list, and event_id is identified in the OnEvent function.
...
This API is used to select the microphone. If this API is not called or “EVICEID_DEFAULT” is passed in, the default microphone is selected. Device ID is from the list returned by GetMicList.
...
### Select the speaker
This API is used to select the playback device. If this API is not called or “EVICEID_DEFAULT” is passed in, the default playback device is selected. Device ID is from the list returned by GetSpeakerList.
...
| msTime | int | Indicates delay time |
...
| gain | double | Indicates the gain volume. Value range: 0.0 to 1.0. It defaults to 1.0. |
...
| ITMG_VOICE_TYPE_INFLUENZA | 7 | Having a cold |
...
### Convert the specified voice file into text (specified language)
This API is used to convert the specified voice file into text of specified language.
