...
### 注意事项
1. 开启 CNAME 加速的相关域名必须都使用腾讯云云解析或者 DNSPod 平台 的解析服务，否则无法开启该功能或者加速失败，严重的甚至会造成解析错误；
2. 对于已经开启 CNAME 加速的域名，当系统检测到域名没有使用腾讯云解析服务的时候（比如域名注册到期、域名更换到其他 DNS 解析服务提供商等等），会自动关闭该 CNAME 加速功能，当检测到域名又重新使用 DNSPod 解析服务的时候，会自动再开启 CNAME 加速；
