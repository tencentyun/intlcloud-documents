...
Update the file conf/nginx.conf in the Nginx root directory as follows:
```
server {
...
It detects that the suite supports ATS requirements after the certificate is imported. But protocol TLS1_2 required for ATS is not enabled. You can use SSL tools provided by TrustAsia. [Click here to download](http://www.trustasia.com/down/ssltools.zip) to enable TLS1_2 protocol.
...
If PFS is not supported, select encryption suites with ECDHE and DHE.

#### 4.2 Method 2
Click **Start** -> **Run**. Enter regedit
Find HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols, right-click it, and then click **Create** -> **Item** -> **Create TLS 1.1, TLS 1.2**
Right-click TLS 1.1 and TLS 1.2, and click **Create** -> **Item** -> **Create Server** or **Create Client**
...
Copy the cipher suite line to the clipboard and paste it into the edit box; up to 1,023 characters.
