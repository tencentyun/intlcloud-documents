...
## API Description
This API (UpdateAudit) is used to update CloudAudit. It can be used to update certain settings in the specified log file, such as the path and COS Bucket. The name of CloudAudit cannot be modified. You do not need to disable the CloudAudit service when modifying the path. If the COS Bucket to be modified was a target of the CloudAudit log file, it can be modified successfully; otherwise, you need to manually grant CloudAudit the write permission to the COS Bucket.
...
| CosKeyPrefix	| No |	String	| The prefix of COS Bucket. Refer to the naming requirements for COS. |
...
| CosKeyPrefix	|	String	| The prefix of COS Bucket. Refer to the naming requirements for COS. |
