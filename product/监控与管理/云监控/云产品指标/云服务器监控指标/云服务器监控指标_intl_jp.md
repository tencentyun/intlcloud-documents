CVMの監視メトリックはCVMの監視コンポーネントを通じて収集する必要があります。ユーザーはサーバーを購入した時にCMボタンにチェックを入れることで自動的に監視コンポーネントをインストール、または[監視コンポーネントのインストールガイド](https://cloud.tencent.com/document/product/248/6211)により手動でインストールすることができます。
