...
|---------|---------|---------|---------|
| Action | the name of an action-specific API. For example, when a Tencent Cloud CVM user calls the API [DescribeInstance](https://cloud.tencent.com/document/api/213/9388), the **Action** of this request is DescribeInstances. | String | Yes |
| Region | the name of the region where your desired instance is located . For more information, see [Regions and Availability Zones](https://cloud.tencent.com/document/product/213/6091), or use the API [DescribeRegions](https://cloud.tencent.com/document/api/213/9456).<br> **Notes:** 1. This parameter is required for the API requests, unless otherwise stated.<br> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2. The product is in closed beta in some of the regions and thus not available to all users. | String | No |
| Timestamp | The current UNIX timestamp, which is the time when the API request was initiated. | UInt | Yes |
