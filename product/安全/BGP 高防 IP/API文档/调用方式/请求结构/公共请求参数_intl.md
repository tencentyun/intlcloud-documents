...
|---------|---------|---------|---------|
| Action | The name of the API for the desired operation. For example, when a Tencent Cloud CVM user calls the API [Querying Instance List](https://cloud.tencent.com/document/api/213/9388), the Action parameter is DescribeInstances. | String | Yes |
| Region | Region parameter, which is used to identify the region to which the instance you want to work with belongs. For more information, see [Regions and Availability Zones](https://cloud.tencent.com/document/product/213/6091), or use the API [Querying Region List](https://cloud.tencent.com/document/api/213/9456).<br> **Notes:** 1. Unless otherwise specified in the API document, this parameter is required generally.<br> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2. Some of the regions are under internal trial and only available to certain users. | String | No |
...
| Token | No | The token used for the temporary certificate, which must be used together with a temporary key. No token is required for a long-term key. | String | No |
