...
- 计费类型：【按量计费】，您购买集群后，按实际使用量计费，可以随时销毁，销毁后不收费用。
- 地域：目前腾讯云 ES 已在广州、上海、北京、成都、香港、硅谷、多伦多、上海金融和深圳金融，共计9个地域上线，其中上海金融和深圳金融需要通过 [工单](https://console.cloud.tencent.com/workorder/category) 申请。
- 部署方式：单可用区、多可用区，单可用区是 ES 集群部署在一个可用区内；多可用区是可以部署在同一个地域的两个可用区。多可用区的部署方式可以提高集群的容灾能力，保障线上业务的稳定性。多可用区方案，暂时是内测阶段，可以通过 [工单](https://console.cloud.tencent.com/workorder/category) 申请。另外，多可用区的部署方式需要满足该地域下有多个可用区，且有足够的资源。
...
- 高级特性：Elasticsearch 官方商业特性（原 X-Pack 插件），包括数据权限管理、SQL JDBC、告警和机器学习等能力；不同版本包含的功能完整度不同，其中白金版包含所有高级特性，基础版包含一部分高级特性，开源版只有开源部分的功能，不包括高级功能，具体版本包含的功能差异，详见 [高级特性（X-Pack）](https://intl.cloud.tencent.com/zh/document/product/845/30943)。
...
![确认配置](https://main.qcloudimg.com/raw/c5be136a79b1a7fdc67dfb610ea78d6a.png)

- 按量计费时单击【开通】直接创建集群，不需要确认订单支付，在使用过程中，会按小时进行计费及扣费。

#### 三、创建完成

开通成功后，就可以跳转到 [控制台](https://console.cloud.tencent.com/es) 查看刚刚创建的集群，集群将在几分钟内部署完成。

## 集群应用开发及管理

### 集群访问

ES 提供了多种类型的客户端访问集群，快速开始。

### 集群监控

腾讯云 ES，提供了丰富的监控指标，可以帮助在集群使用过程中查看集群的状态。
