...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DescribeComponentStatistics |
| Version | 是 | String | 公共参数，本接口取值：2018-02-28 |
| Region | 否 | String | 公共参数，本接口不需要传递此参数。 |
| Limit | 否 | Integer | 返回数量，默认为10，最大值为100。 |
| Offset | 否 | Integer | 偏移量，默认为0。 |
| Filters.N | 否 | Array of [Filter](https://cloud.tencent.com/document/api/296/19867#Filter) | 过滤条件。<br/>ComponentName - String - 是否必填：否 - 组件名称 |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| TotalCount | Integer | 组件统计列表记录总数。|
| ComponentStatistics | Array of [ComponentStatistics](https://cloud.tencent.com/document/api/296/19867#ComponentStatistics) | 组件统计列表数据数组。|
