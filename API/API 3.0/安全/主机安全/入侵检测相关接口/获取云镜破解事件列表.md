...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DescribeBruteAttacks |
| Version | 是 | String | 公共参数，本接口取值：2018-02-28 |
| Region | 否 | String | 公共参数，本接口不需要传递此参数。 |
| Uuid | 否 | String | 客户端唯一Uuid。 |
| Offset | 否 | Integer | 偏移量，默认为0。 |
| Filters.N | 否 | Array of [Filter](https://cloud.tencent.com/document/api/296/19867#Filter) | 过滤条件。<br/><li>Keywords - String - 是否必填：否 -  查询关键字</li><li>Status - String - 是否必填：否 -  查询状态（FAILED：破解失败 &#124;SUCCESS：破解成功）</li> |
...
| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| TotalCount | Integer | 事件数量|
| BruteAttacks | Array of [BruteAttack](https://cloud.tencent.com/document/api/296/19867#BruteAttack) | 暴力破解事件列表|
