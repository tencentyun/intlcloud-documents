...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DescribeMalwares |
...
| Filters.N | 否 | Array of [Filter](https://cloud.tencent.com/document/api/296/19867#Filter) | 过滤条件。<br/><li>Keywords - String - 是否必填：否 - 查询关键字 </li><li>Status - String - 是否必填：否 - 木马状态（UN_OPERATED: 未处理 &#124; SEGREGATED: 已隔离&#124;TRUSTED：信任）</li><br/>每个过滤条件只支持一个值，暂不支持多个值“或”关系查询。 |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| TotalCount | Integer | 木马总数。|
| Malwares | Array of [Malware](https://cloud.tencent.com/document/api/296/19867#Malware) | Malware数组。|
