...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DescribeVuls |
...
| Filters.N | 否 | Array of [Filter](https://cloud.tencent.com/document/api/296/19867#Filter) | 过滤条件。<br/><li>Status - String - 是否必填：否 - 状态筛选（UN_OPERATED: 待处理 &#124; FIXED：已修复）<br/><br/>Status过滤条件值只能取其一，不能是“或”逻辑。 |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| TotalCount | Integer | 漏洞数量。|
| Vuls | Array of [Vul](https://cloud.tencent.com/document/api/296/19867#Vul) | 漏洞列表数组。|
