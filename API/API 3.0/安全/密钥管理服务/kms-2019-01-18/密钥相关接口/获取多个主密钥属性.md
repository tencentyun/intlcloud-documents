...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DescribeKeys |
...
| KeyIds.N | 是 | Array of String | 查询CMK的ID列表，批量查询一次最多支持100个KeyId |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| KeyMetadatas | Array of [KeyMetadata](https://cloud.tencent.com/document/api/573/34431#KeyMetadata) | 返回的属性信息列表<br/>注意：此字段可能返回 null，表示取不到有效值。|
