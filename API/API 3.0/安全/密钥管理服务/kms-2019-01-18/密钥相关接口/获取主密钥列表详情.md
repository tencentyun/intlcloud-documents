...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：ListKeyDetail |
...
| OrderType | 否 | Integer | 根据CMK创建时间排序， 0 表示按照降序排序，1表示按照升序排序 |
| KeyState | 否 | Integer | 根据CMK状态筛选， 0表示全部CMK， 1 表示仅查询Enabled CMK， 2 表示仅查询Disabled CMK |
| SearchKeyAlias | 否 | String | 根据KeyId或者Alias进行模糊匹配查询 |
...
| KeyMetadatas | Array of [KeyMetadata](https://cloud.tencent.com/document/api/573/34431#KeyMetadata) | 返回的属性信息列表，此字段可能返回 null，表示取不到有效值。<br/>注意：此字段可能返回 null，表示取不到有效值。|
