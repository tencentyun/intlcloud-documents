...
### 2.3. Splicing the Signature Original String
...
When calling API 3.0, it is recommended to use the corresponding Tencent Cloud SDK 3.0 which encapsulates the signature process, and you only need to focus on the specific APIs provided by the product when developing. See [SDK Center](https://cloud.tencent.com/document/sdk) for more information. Currently, the following programming languages are supported:
...
Note: The key and timestamp in this example are made-up for demonstration, so please fill your own SecretId SecretKey and current time of the system as the Timestamp into the URL. Please do not open the sample URL in the browser or using command such as curl; otherwise, you will get an authentication error: Signature expired.
