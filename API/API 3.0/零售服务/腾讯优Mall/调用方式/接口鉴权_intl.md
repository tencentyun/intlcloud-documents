Tencent Cloud APIs authenticate each access request, i.e. each request needs to include signature information (Signature) in the common request parameters to verify the identity of the requester.
The Signature is generated by the security credentials which include SecretId and SecretKey. If you don't have the security credentials yet, please go to the [Cloud API Key](https://console.cloud.tencent.com/capi) page to apply; otherwise, you cannot call the Cloud APIs.

## 1. Applying for Security Credentials
Before using the Cloud APIs for the first time, go to the [Cloud API Key](https://console.cloud.tencent.com/capi) page to apply for security credentials.
...
Take the Cloud Virtual Machine's request to view the instance list (DescribeInstances) as an example. When you call this API, the request parameters may be as follows:
