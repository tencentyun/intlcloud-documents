...
Take the Cloud Virtual Machine API (DescribeInstancesStatus) version 2017-03-12 for viewing the instance state list as an example. If the call succeeds, the possible response is as follows:
...
* RequestId is used to uniquely identify an API request. If the API is exceptional, you can contact us and provide the ID for troubleshooting.
...
* The presence of the Error field indicates that the request call failed. The Error field and its internal Code and Message fields are always returned when the call fails.
