...
* After the instance is created successfully, it will start on boot and the [instance status](https://cloud.tencent.com/document/api/213/9452#instance_state) will become "running".
...
* The instances allowed to be purchased by this API are subject to the number limit described in the [Restrictions on CVM Instance Purchase](https://intl.cloud.tencent.com/document/product/213/2664), and share the quota with the instances created by the official website entry.
...
| Placement | Yes | [Placement](https://cloud.tencent.com/document/api/213/15753#Placement) | Location of the instance. This parameter is used to specify the availability zone, the project to which the instance belongs, and the CDH (the creation of sub-machine for the exclusive parent host billing mode), etc. |
...
| SystemDisk | No | [SystemDisk](https://cloud.tencent.com/document/api/213/15753#SystemDisk) | Configuration information of instance's system disk. If the parameter is not specified, the default value is assigned to it. |
| DataDisks.N | No | Array of [DataDisk](https://cloud.tencent.com/document/api/213/15753#DataDisk) | Configuration information of the instance data disk. If the parameter is not specified, no data disk will be purchased by default. You can specify only one data disk when purchasing it. |
| VirtualPrivateCloud | No | [VirtualPrivateCloud](https://cloud.tencent.com/document/api/213/15753#VirtualPrivateCloud) | Configuration information of VPC. This parameter is used to specify VPC ID and subnet ID, etc. If this parameter is not specified, the basic network is used by default. If a VPC IP is specified in this parameter, it indicates the primary ENI IP of each instance and the value of parameter InstanceCount must be same as the number of VPC IPs. |
| InternetAccessible | No | [InternetAccessible](https://cloud.tencent.com/document/api/213/15753#InternetAccessible) | Configuration information of public network bandwidth. If this parameter is not specified, the default public network bandwidth is 0 Mbps. |
...
| LoginSettings | No | [LoginSettings](https://cloud.tencent.com/document/api/213/15753#LoginSettings) | Login settings of the instance. This parameter is used to set the login password and key for the instance, or to keep the original login settings for the image. By default, a random password is generated and notified to the user via the internal message. |
...
| EnhancedService | No | [EnhancedService](https://cloud.tencent.com/document/api/213/15753#EnhancedService) | Enhanced service. This parameter is used to specify whether to enable Cloud Security, Cloud Monitoring and other services. If this parameter is not specified, the Cloud Monitoring and Cloud Security are enabled by default. |
...
| ActionTimer | No | [ActionTimer](https://cloud.tencent.com/document/api/213/15753#ActionTimer) | Timed task. This parameter is used to specify the timed task for instance. Only the timed termination is supported. |
| TagSpecification.N | No | Array of [TagSpecification](https://cloud.tencent.com/document/api/213/15753#TagSpecification) | Tag description list. This parameter is used to bind a tag to a resource instance. A tag can only be bound to CVM instance. |
