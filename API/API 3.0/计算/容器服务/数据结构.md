...
|------|------|----------|------|
| NodeRole | String | 是 | 节点角色，取值:MASTER_ETCD, WORKER。MASTER_ETCD只有在创建 INDEPENDENT_CLUSTER 独立集群时需要指定。 |
| RunInstancesPara | Array of String | 是 | CVM创建透传参数，json化字符串格式，详见[CVM创建实例](https://intl.cloud.tencent.com/document/product/213/33237)接口，传入公共参数外的其他参数即可，其中ImageId会替换为TKE集群OS对应的镜像。 |
