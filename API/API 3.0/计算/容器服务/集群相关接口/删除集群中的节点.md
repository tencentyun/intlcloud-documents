...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DeleteClusterInstances |
...
| InstanceIds.N | 是 | Array of String | 主机InstanceId列表 |
| InstanceDeleteMode | 否 | String | 集群实例删除时的策略：terminate（销毁实例，仅支持按量计费云主机实例） retain （仅移除，保留实例） |
