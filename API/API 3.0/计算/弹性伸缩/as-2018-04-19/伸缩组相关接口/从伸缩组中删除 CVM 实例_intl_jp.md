...
このAPI（RemoveInstances）は、スケーリンググループからCVMインスタンスを削除するために使用されます。現在の製品ロジックによると、インスタンスがASによって自動的に作成される場合は、そのインスタンスは終了されます。インスタンスを作成した後スケーリンググループに追加する場合は、そのインスタンスはスケーリンググループから除去されますが、保持されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：RemoveInstances |
