...
このAPI（DeleteAutoScalingGroup）は、指定されたスケーリンググループを削除するために使用されます。削除の前提条件は、スケーリンググループにインスタンスがなく、現在スケーリングアクティビティが実行されていないことです。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DeleteAutoScalingGroup |
