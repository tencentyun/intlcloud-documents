...
このAPI（DeleteLaunchConfiguration）は、起動構成を削除するために使用されます。

* 起動構成がスケーリンググループで有効な場合、この起動構成は削除できません。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DeleteLaunchConfiguration |
...
| LaunchConfigurationId | はい | String | 削除する必要がある起動構成ID。 |
