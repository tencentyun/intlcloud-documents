...
* 每个项目最多只能创建20个启动配置，详见[使用限制](https://intl.cloud.tencent.com/document/product/377/3120)。
...
| InstanceType | 否 | String | 实例机型。不同实例机型指定了不同的资源规格，具体取值可通过调用接口 [DescribeInstanceTypeConfigs](https://cloud.tencent.com/document/api/213/15749) 来获得最新的规格表或参见[实例类型](https://intl.cloud.tencent.com/document/product/213/11518)描述。<br/>`InstanceType`和`InstanceTypes`参数互斥，二者必填一个且只能填写一个。 |
