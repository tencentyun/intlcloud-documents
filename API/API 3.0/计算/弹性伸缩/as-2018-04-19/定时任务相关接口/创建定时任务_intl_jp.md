...
このAPI（CreateScheduledAction）は時限タスクを作成するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：CreateScheduledAction |
...
| ScheduledActionName | はい | String | 時限タスク名称。この名前は、中国語、英語、数字、アンダースコア、区切り記号「-」、小数点のみをサポートし、最大長は60バイトを超えることはできません。同じスケーリンググループの下で唯一である必要があります。 |
| MaxSize | はい | Integer | 時限タスクがトリガされたとき、設定されたスケーリンググループの最大のインスタンス数。 |
| MinSize | はい | Integer | 時限タスクがトリガされたとき、設定されたスケーリンググループの最小のインスタンス数。 |
| DesiredCapacity | はい | Integer | 時限タスクがトリガされたとき、設定されたスケーリンググループの希望インスタンス数。 |
| StartTime | はい | Timestamp | 時限タスクの初めてのトリガ時間。値は`北京時間`（UTC+8）、`ISO8601`標準に従って、フォーマット：`YYYY-MM-DDThh:mm:ss+08:00`。 |
| EndTime | いいえ | Timestamp | 時限タスクの終了時間。値は`北京時間`（UTC+8）、`ISO8601`標準に従って、フォーマット：`YYYY-MM-DDThh:mm:ss+08:00`。<br><br>このパラメータは`Recurrence`と同時に指定される必要があります。終了時間に達してから、時限タスクが無効になります。 |
| Recurrence | いいえ | String | 時限タスクの繰り返し方法。標準[Cron](https://zh.wikipedia.org/wiki/Cron)フォーマットです<br><br>このパラメータは`EndTime`と同時に指定される必要があります。 |
...
時限タスクを作成します。指定の時間（北京時間2018年8月28日23時）でスケーリンググループの最大のインスタンス数、最小のインスタンス数と希望インスタンス数を10、4、6に調整します。
...
### 例2 繰り返し実行時限タスクを作成します

時限タスクを作成します。北京時間2018年8月28日から、毎日23時でスケーリンググループの最大のインスタンス数、最小のインスタンス数と希望インスタンス数を7、2、3を調整して、北京時間2019年1月1日00:00時後終了します。
