...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：ModifyTargetWeight |
...
| Weight | 是 | Integer | 后端云服务器新的转发权重，取值范围：0~100。 |
| LocationId | 否 | String | 转发规则的ID，当绑定机器到七层转发规则时，必须提供此参数或Domain+Url两者之一 |
| Domain | 否 | String | 目标规则的域名，提供LocationId参数时本参数不生效 |
| Url | 否 | String | 目标规则的URL，提供LocationId参数时本参数不生效 |
| Targets.N | 否 | Array of [Target](https://cloud.tencent.com/document/api/214/30694#Target) | 要修改权重的后端机器列表 |
...
### 示例1 修改监听器上注册的后端转发目标的权重

将注册到监听器lbl-d1ubsydq的转发目标ins-dm4xtz0i（注册端口为334）的权重修改为8
