...
DeregisterTargets는 하나 이상의 RS를 응용형 CLB 수신기에 언바인딩하는 데 사용됩니다. 4계층 수신기(TCP, UDP)의 경우 수신기 ID를 지정하기만 하면 되며, 7계층 수신기(HTTP, HTTPS)의 경우 LocationId 또는 Domain+Url을 통해 포워딩 규칙을 지정해야 합니다.
...
| 매개변수 이름 | 필수 여부 | 유형 | 설명 |
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, API 값: DeregisterTargets |
...
| ListenerId | 예 | String | 수신기 ID |
| Targets.N | 예 | Array of [Target](https://cloud.tencent.com/document/api/214/30694#Target) | 언바인딩할 RS 리스트 |
| LocationId | 아니요 | String | 포워딩 규칙의 ID로 7계층 포워딩 규칙에서 CVM을 언바인딩할 때에는 반드시 이 매개변수 또는 Domain+Url 두 가지 중 하나를 제공해야 합니다. |
