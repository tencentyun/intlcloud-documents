...
ModifyDomain APIはアプリケーション型CLBの7層リスナーのドメイン名を変更するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：ModifyDomain |
...
| Domain | はい | String | リスナーの特定の旧ドメイン名。 |
| NewDomain | はい | String | 新ドメイン名であり、長さの制限は1～80です。非正規表現フォーマット、ワイルドカードフォーマット、正規表現フォーマットの3種類の使用フォーマットがあります。非正規表現フォーマットはアルファベット、数字、「-」、「.」しか使用できません。ワイルドカードフォーマットが使用する「*」は、始まりまたは終わりにしか使用できません。正規表現は「~」で始まります。 |
