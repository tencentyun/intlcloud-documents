...
ModifyDomain API는 응용형 CLB 7계층 수신기의 도메인 이름을 수정하는 데 사용됩니다.
...
| 매개변수 이름 | 필수 여부 | 유형 | 설명 |
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, API 값: ModifyDomain |
...
| Domain | 예 | String | 수신기의 어느 오래된 도메인 이름. |
| NewDomain | 예 | String | 새로운 도메인 이름, 길이 범위: 1-80. 비정규식, 와일드카드, 정규식 형식 세 가지가 있습니다. 비정규식은 알파벳, 숫자, "-", "."만 사용할 수 있으며, 와일드카드는 "*"를 시작 또는 끝에만 사용할 수 있고 정규식은 "~"로 시작합니다. |
...
### 예시 1 HTTP 수신기의 포워딩 규칙에 대한 도메인 이름 수정
