...
DeregisterTargets APIは、1台または複数台のRSをアプリケーション型CLBのリスナーからバインディング解除するために使用されます。4層リスナー（TCP、UDP）の場合はリスナーIDを指定するだけでよく、7層リスナー（HTTP、HTTPS）の場合は更にLocationIdまたはDomain+Urlを介して転送規則を指定する必要があります。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DeregisterTargets |
...
| ListenerId | はい | String | リスナーID |
| Targets.N | はい | Array of [Target](https://cloud.tencent.com/document/api/214/30694#Target) | バインディング解除するRSリスト |
| LocationId | いいえ | String | 転送規則のIDであり、マシンを7層転送規則からバインディング解除する時、このパラメータまたはDomain+Urlのいずれか1つを提供しなければなりません |
