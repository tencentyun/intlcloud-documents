...
DeleteRule APIは、アプリケーション型ロードバランサーインスタンスの7層リスナー下の転送規則を削除するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DeleteRule |
...
| LocationIds.N | いいえ | Array of String | 削除する転送規則のIDより構成される配列 |
| Domain | いいえ | String | 削除する転送規則のドメイン名であり、既にLocationIdsパラメータを提供している時、このパラメータは有効になりません |
| Url | いいえ | String | 削除する転送規則の転送パスであり、既にLocationIdsパラメータを提供している時、このパラメータは有効になりません |
