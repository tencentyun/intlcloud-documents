...
DeleteLoadBalancer APIは、ユーザーが指定したロードバランサーインスタンスを削除するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DeleteLoadBalancer |
...
| LoadBalancerIds.N | はい | Array of String | 削除するロードバランサーインスタンスIDの配列 |
...
### 例1 ロードバランサーインスタンス名の削除

#### 入力例

```
https://clb.tencentcloudapi.com/?Action=DeleteLoadBalancer
&LoadBalancerIds.0=lb-hsb93u5o
&<共通リクエストパラメータ>
```
...
* [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=clb&Version=2018-03-17&Action=DeleteLoadBalancer)
