...
DescribeClassicalLBListenersは、従来型CLB情報を取得するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DescribeClassicalLBListeners |
...
| ListenerIds.N | いいえ | Array of String | CLBリスナーのIDリストであり、範囲[1-65535] |
| Protocol | いいえ | String | CLBリスナーのプロトコルであり、「TCP」、「UDP」、「HTTP」、「HTTPS」 |
| ListenerPort | いいえ | Integer | CLBリスナーのポート |
| Status | いいえ | Integer | リスナーのステータスであり、0は作成中、1は動作中を表します |

## 3. 出力パラメータ

| パラメータ名 | タイプ | 説明 |
|---------|---------|---------|
| Listeners | Array of [ClassicalListener](https://cloud.tencent.com/document/api/214/30694#ClassicalListener) | リスナーリスト<br/>注意：このフィールドはnullを返す可能性があり、有効値を取得できないことを表します。|
