...
This API (ResetRoutes) is used to reset the name of a route table and all routing policies in it.
Note: When this API is called, all routing policies in the current route table are deleted before new routing policies are saved, which may result in network interruption.
...
| Routes.N | Yes | Array of [Route](https://cloud.tencent.com/document/api/215/##Route) | Routing policy. |
...
### Example 1 Reset route table name and routing policies
