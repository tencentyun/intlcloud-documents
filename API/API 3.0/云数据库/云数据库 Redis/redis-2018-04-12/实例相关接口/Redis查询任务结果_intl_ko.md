...
| 매개변수 이름 | 필수 여부 | 유형 | 설명 |
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: DescribeTaskInfo |
...
| TaskId | 예 | Integer | 태스크 ID |

## 3. 출력 매개변수

| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| Status | String | 태스크 상태, preparing: 실행 대기, running: 실행 중, succeed: 성공, failed: 실패, error: 실행 오류|
| StartTime | String | 태스크 시작 시간|
| TaskType | String | 태스크 유형|
| InstanceId | String | 인스턴스 ID|
| TaskMessage | String | 태스크 정보, 오류 시 오류 정보를 표시합니다. 실행 중 및 성공은 비워 있습니다.|
