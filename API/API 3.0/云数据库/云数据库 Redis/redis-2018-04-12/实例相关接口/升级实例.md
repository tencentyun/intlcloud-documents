...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：UpgradeInstance |
...
| MemSize | 是 | Integer | 分片大小 单位 MB |
| RedisShardNum | 否 | Integer | 分片数量，Redis2.8主从版、CKV主从版和Redis2.8单机版不需要填写 |
| RedisReplicasNum | 否 | Integer | 副本数量，Redis2.8主从版、CKV主从版和Redis2.8单机版不需要填写 |
