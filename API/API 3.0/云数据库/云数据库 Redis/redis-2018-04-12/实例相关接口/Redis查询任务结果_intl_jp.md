...
| パラメータ名 | 選択必須 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | 要 | String | 共通パラメータ。本インターフェースの値：DescribeTaskInfo |
...
| TaskId | 要 | Integer | タスクID |

## 3. パラメータ出力

| パラメータ名 | タイプ | 説明 |
|---------|---------|---------|
| Status | String | タスクステータスpreparing:実行待ち、running：実行中、succeed：成功，failed：失敗、error 実行エラー|
| StartTime | String | タスク開始時間|
| TaskType | String | タスクタイプ|
| InstanceId | String | インスタンスのID|
| TaskMessage | String | タスク情報は、エラー時にエラー情報を表示します。実行中と成功の場合は空です|
