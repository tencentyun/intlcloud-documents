...
| InstanceChargeType | 否 | String | 计费类型，当前只支持预付费，即包年包月，取值为PREPAID。默认值为PREPAID |
| Period | 否 | Integer | 购买时长，单位：月。取值为1到48，默认为1 |
| GoodsNum | 否 | Integer | 一次性购买的实例数量。取值1-100，默认取值为1 |
| DBVersion | 否 | String | sqlserver版本，目前只支持：2008R2（SQL Server 2008 R2），2012SP3（SQL Server 2012），2016SP1（SQL Server 2016 SP1）两种版本。默认为2008R2版本 |
