...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：CreateMigration |
...
| MigrateName | 是 | String | 迁移任务的名称 |
| MigrateType | 是 | Integer | 迁移类型（1:结构迁移 2:数据迁移 3:增量同步） |
| SourceType | 是 | Integer | 迁移源的类型 1:CDB for SQLServer 2:云服务器自建SQLServer数据库 4:SQLServer备份还原 5:SQLServer备份还原（COS方式） |
| Source | 是 | [MigrateSource](https://cloud.tencent.com/document/api/238/19976#MigrateSource) | 迁移源 |
| Target | 是 | [MigrateTarget](https://cloud.tencent.com/document/api/238/19976#MigrateTarget) | 迁移目标 |
| MigrateDBSet.N | 否 | Array of [MigrateDB](https://cloud.tencent.com/document/api/238/19976#MigrateDB) | 迁移DB对象 ，离线迁移不使用（SourceType=4或SourceType=5）。 |
