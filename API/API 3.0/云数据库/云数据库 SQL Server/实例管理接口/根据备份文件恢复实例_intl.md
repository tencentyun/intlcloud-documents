...
This API (RestoreInstance) is used to restore an instance from a backup file.
...
| BackupId | Yes | Integer | Backup file ID, which can be obtained from the ID field in the returned result of the API DescribeBackups. |

## 3. Output Parameters

| Parameter Name | Type | Description |
|---------|---------|---------|
| FlowId | Integer | Asynchronous process task ID, which is used to call the API DescribeFlowStatus to get the task execution status |
| RequestId | String | The unique ID of a request, which is required for each troubleshooting case. |

## 4. Example

### Example 1 Restore an instance

#### Input example

```
https://sqlserver.tencentcloudapi.com/?Action=RestoreInstance
...
* [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=sqlserver&Version=2018-03-28&Action=RestoreInstance)
