...
該当API（VerifyRootAccount）はデータベースインスタンスのROOTアカウントの権限付与操作に十分な権限を持つかどうかを検証します。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、該当APIの値：VerifyRootAccount |
...
| Password | はい | String | インスタンスROOTアカウントのパスワード。 |
