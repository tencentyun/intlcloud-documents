...
이 API(DescribeSupportedPrivileges)는 데이터베이스 지원되는 권한 정보 조회에 사용되며, 전역 권한, 데이터베이스 권한, 테이블 권한 및 리스트 권한이 포함됩니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: DescribeSupportedPrivileges |
...
| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| GlobalSupportedPrivileges | Array of String | 인스턴스에 지원되는 전역 권한. |
| DatabaseSupportedPrivileges | Array of String | 인스턴스에 지원되는 데이터베이스 권한. |
| TableSupportedPrivileges | Array of String | 인스턴스에 지원되는 데이터베이스 테이블 권한. |
| ColumnSupportedPrivileges | Array of String | 인스턴스에 지원되는 데이터베이스 리스트 권한. |
| RequestId | String | 유일한 요청 ID, 매회 요청 시마다 반환됩니다. 문제를 찾을 경우 해당 요청의 RequestId를 제공해야 합니다. |

## 4. 예시

### 예시1 데이터베이스 인스턴스에 지원되는 권한 정보 조회
