...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：ModifyInstanceParam |
...
| InstanceIds.N | 是 | Array of String | 实例短Id列表。 |
| ParamList.N | 是 | Array of [Parameter](https://cloud.tencent.com/document/api/236/15878#Parameter) | 要修改的参数列表。每一个元素是name和currentValue的组合。name是参数名，currentValue是要修改成的值。 |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| AsyncRequestId | String | 异步任务Id，可用于查询任务进度。|
