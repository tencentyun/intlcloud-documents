...
このAPI（ModifyInstanceParam）はデータベースインスタンスのパラメータを変更するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：ModifyInstanceParam |
...
| InstanceIds.N | はい | Array of String | インスタンスの短いIDリスト。 |
| ParamList.N | はい | Array of [Parameter](https://cloud.tencent.com/document/api/236/15878#Parameter) | 変更するパラメータリスト。各要素はnameとcurrentValueの組み合わせです。nameはパラメータ名、currentValueは変更後の値です。 |

## 3. 出力パラメータ

| パラメータ名 | タイプ | 説明 |
|---------|---------|---------|
| AsyncRequestId | String | 非同期タスクID。タスク進捗の照合に使用されます。|
