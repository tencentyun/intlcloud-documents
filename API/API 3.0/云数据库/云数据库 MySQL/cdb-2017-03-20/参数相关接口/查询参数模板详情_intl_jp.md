...
このAPI（DescribeParamTemplateInfo）はパラメータテンプレート詳細を照合するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DescribeParamTemplateInfo |
...
| TemplateId | はい | Integer | パラメータテンプレートID。 |
...
| Name | String | パラメータテンプレート名。|
| EngineVersion | String | パラメータテンプレートの説明|
| TotalCount | Integer | パラメータテンプレート内のパラメータ数量|
| Items | Array of [ParameterDetail](/document/api/236/15878#ParameterDetail) | パラメータの詳細|
