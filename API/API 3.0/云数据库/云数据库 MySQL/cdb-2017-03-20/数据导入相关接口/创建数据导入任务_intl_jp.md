...
このAPI（CreateDBImportJob）はデータベースデータインポートタスクの作成に使用されます。

注意：ユーザーがデータインポートタスクを行うファイルは、事前にTencent Cloudにアップロードしなければなりません。ユーザーはコンソールでファイルのインポートを行います。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ。このAPIの値：CreateDBImportJob |
...
| InstanceId | はい | String | インスタンスID、フォーマット：cdb-c1nl9rpv。データベースコンソールページで表示されるインスタンスIDと同じです。 |
| FileName | はい | String | ファイル名称。このファイルはユーザーがTencent Cloudにアップロードしたファイルです。 |
| User | はい | String | データベースのユーザー名。 |
| Password | いいえ | String | データベースインスタンスのUserアカウントのパスワード。 |
| DbName | いいえ | String | インポートするターゲットデータベース名。渡さないことはデータベースの無指定を意味します。 |
