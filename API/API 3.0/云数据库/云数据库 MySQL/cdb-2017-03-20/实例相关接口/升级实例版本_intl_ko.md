...
이 API(UpgradeDBInstanceEngineVersion)는 데이터베이스 인스턴스 버전의 업그레이드에 사용합니다. 지원되는 인스턴스 유형은 마스터 인스턴스, 재해 복구 인스턴스 및 읽기 전용 인스턴스입니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: UpgradeDBInstanceEngineVersion |
...
| EngineVersion | 예 | String | 마스터 인스턴스 데이터베이스 엔진 버전, 지원 값: 5.6 및 5.7 |
...
| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| AsyncRequestId | String | 비동기화 태스크 ID, [비동기화 태스크의 실행 결과 조회](https://cloud.tencent.com/document/api/236/20410) API를 사용하여 해당 실행 결과를 획득할 수 있습니다. |
| RequestId | String | 유일한 요청 ID, 매회 요청 시마다 반환됩니다. 문제를 찾을 경우 해당 요청의 RequestId를 제공해야 합니다. |

## 4. 예시

### 예시1 데이터베이스 인스턴스 버전 업그레이드
