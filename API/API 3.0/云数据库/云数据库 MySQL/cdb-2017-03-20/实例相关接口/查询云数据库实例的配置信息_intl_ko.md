...
이 API(DescribeDBInstanceConfig)는 동기화 모드, 배치 모드 등의 데이터베이스 인스턴스의 구성 정보 조회에 사용됩니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: DescribeDBInstanceConfig |
...
| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| ProtectMode | Integer | 마스터 데이터베이스 데이터 보호 방식, 마스터 인스턴스 속성, 가능한 반환 값: 0-비동기화 복제 방식, 1-반동기화 복제 방식, 2-강제 동기화 복제 방식. |
| DeployMode | Integer | 마스터 데이터베이스 배치 방식, 마스터 인스턴스 속성, 가능한 반환 값: 0-다일 가용 영역 배치, 1-멀티 가용 영역 배치. |
| Zone | String | 마스터 데이터베이스 가용 영역의 정식 이름, 예: ap-shanghai-1. |
| SlaveConfig | [SlaveConfig](https://cloud.tencent.com/document/api/236/15878#SlaveConfig) | 슬레이브 데이터베이스의 구성 정보. |
| BackupConfig | [BackupConfig](https://cloud.tencent.com/document/api/236/15878#BackupConfig) | ECDB 두 번째 슬레이브 데이터베이스의 구성 정보, ECDB 인스턴스에만 이 필드가 있습니다. |
| RequestId | String | 유일한 요청 ID, 매회 요청 시마다 반환됩니다. 문제를 찾을 경우 해당 요청의 RequestId를 제공해야 합니다. |

## 4. 예시

### 예시1 데이터베이스 인스턴스의 구성 정보 조회
