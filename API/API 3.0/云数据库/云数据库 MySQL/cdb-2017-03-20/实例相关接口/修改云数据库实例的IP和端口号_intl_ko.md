...
이 API(ModifyDBInstanceVipVport)는 데이터베이스 인스턴스의 IP 및 포트 번호 수정에 사용하며, 기본 네트워크를 VPC 및 VPC의 서브넷으로 변경하는 데 사용할 수도 있습니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: ModifyDBInstanceVipVport |
...
| DstIp | 아니요 | String | 대상 IP. 해당 매개변수 및 DstPort 매개변수, 둘 중 한 개를 전달해야 합니다. |
| DstPort | 아니요 | Integer | 대상 포트, 지원 범위: [1024-65535]. 해당 매개변수 및 DstIp 매개변수, 둘 중 한 개를 전달해야 합니다. |
| UniqVpcId | 아니요 | String | 통합된 VPC ID. |
| UniqSubnetId | 아니요 | String | 통합된 서브넷 ID. |

## 3. 출력 매개변수

| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| AsyncRequestId | String | 비동기화 태스크 ID.|
| RequestId | String | 유일한 요청 ID, 매회 요청 시마다 반환됩니다. 문제를 찾을 경우 해당 요청의 RequestId를 제공해야 합니다. |

## 4. 예시

### 예시1 데이터베이스의 IP 및 포트 번호 수정
