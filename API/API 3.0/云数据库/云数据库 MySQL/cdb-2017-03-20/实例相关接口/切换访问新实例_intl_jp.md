...
このAPI（SwitchForUpgrade）は新しいインスタンスへのアクセスに切り替えるために使用されます。アップグレード中のマスタインスタンスが切り替え待ち状態にあるとき、ユーザーがこのプロセスを実行できます
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：SwitchForUpgrade |
...
| InstanceId | はい | String | インスタンスID。フォーマット：cdb-c1nl9rpv。データベースコンソールページで表示されるインスタンスIDと同じ |
...
### 例1 新しいインスタンスへのアクセスの切り替え

#### 入力例

```
https://cdb.tencentcloudapi.com/?Action=SwitchForUpgrade
...
* [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=cdb&Version=2017-03-20&Action=SwitchForUpgrade)
