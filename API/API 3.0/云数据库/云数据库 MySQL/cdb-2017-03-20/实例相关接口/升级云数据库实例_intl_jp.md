...
| SlaveZone | いいえ | String | スレーブデータベース1のAvailability Zone情報。デフォルトはインスタンスのZoneパラメータと一致します。マスタインスタンスを複数Availability Zone配置にアップグレードする際にはこのパラメータを指定できます。読み取り専用インスタンスまたはディザスタリカバリインスタンスをアップグレードする際にはこのパラメータを指定しても意味がありません。[データベースの販売可能な仕様の取得](https://intl.cloud.tencent.com/document/product/236/17229) APIでサポートするAvailability Zoneを照合できます |
