...
このAPI（IsolateDBInstance）はデータベースインスタンスを終了するために使用されます。終了後にはIPとポートでデータベースにアクセスすることができません。従量制課金のインスタンスが終了した直後、無効化になります。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：IsolateDBInstance |
