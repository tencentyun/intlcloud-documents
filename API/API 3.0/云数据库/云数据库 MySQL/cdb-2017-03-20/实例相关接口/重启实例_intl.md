...
This API (RestartDBInstances) is used to restart a database instance.

Notes:
1. Only master instances can be restarted via this API.
2. You can only restart instances in normal status with no asynchronous task being performed.
...
|---------|---------|---------|---------|
| Action | Yes | String | Common parameter. The value used for this API: RestartDBInstances |
...
| InstanceIds.N | Yes | Array of String | Instance ID array, such as: cdb-c1nl9rpv. It is identical to the instance ID displayed in the database console page. |
