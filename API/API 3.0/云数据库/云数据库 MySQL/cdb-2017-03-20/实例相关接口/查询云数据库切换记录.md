...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DescribeDBSwitchRecords |
...
| Limit | 否 | Integer | 分页大小，默认值为50，最小值为1，最大值为2000。 |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| TotalCount | Integer | 实例切换记录的总数。|
| Items | Array of [DBSwitchInfo](https://cloud.tencent.com/document/api/236/15878#DBSwitchInfo) | 实例切换记录详情。|
