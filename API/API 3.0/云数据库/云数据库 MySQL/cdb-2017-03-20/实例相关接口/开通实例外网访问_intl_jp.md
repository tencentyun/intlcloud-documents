...
このAPI（OpenWanService）はインスタンスのパブリックネットワークからのアクセスを有効化するために使用されます。

注意：インスタンスのパブリックネットワークからのアクセスを有効にする前、インスタンスに対して[インスタンスの初期化](https://cloud.tencent.com/document/api/236/15873)操作を実行する必要があります。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：OpenWanService |
...
### 例1 インスタンスのパブリックネットワークからのアクセスの有効化
