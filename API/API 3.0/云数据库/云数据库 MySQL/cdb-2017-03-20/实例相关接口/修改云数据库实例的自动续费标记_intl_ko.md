...
이 API(ModifyAutoRenewFlag)는 데이터베이스 인스턴스의 자동 갱신 태그를 수정하는 데 사용됩니다. 연/월정액 요금제의 인스턴스만 자동 갱신 플래그 구성을 지원합니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: ModifyAutoRenewFlag |
...
| InstanceIds.N | 예 | Array of String | 인스턴스의 ID, 형식 예: cdb-c1nl9rpv, TencentDB 콘솔 페이지에 표시된 인스턴스 ID와 동일합니다. |
| AutoRenew | 예 | Integer | 자동 갱신 플래그, 선택 가능 값: 0-자동 갱신되지 않음, 1-자동 갱신. |
...
### 예시1 데이터베이스 인스턴스의 자동 갱신 플래그 수정

#### 입력 예시

```
https://cdb.tencentcloudapi.com/?Action=ModifyAutoRenewFlag
...
* [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=cdb&Version=2017-03-20&Action=ModifyAutoRenewFlag)
