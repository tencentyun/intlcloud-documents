...
이 API(RestartDBInstances)는 데이터베이스 인스턴스의 다시 시작에 사용합니다.

주의:
1. 이 API는 마스터 인스턴스의 다시 시작만 지원합니다.
2. 인스턴스 상태가 정상이어야 하며, 기타 실행 중인 비동기화 태스크가 없어야 합니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: RestartDBInstances |
...
| InstanceIds.N | 예 | Array of String | 인스턴스 ID 배열, 형식 예: cdb-c1nl9rpv. TencentDB 콘솔 페이지에 표시된 인스턴스 ID와 동일합니다. |
