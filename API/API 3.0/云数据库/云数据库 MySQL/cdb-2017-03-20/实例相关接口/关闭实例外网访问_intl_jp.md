...
このAPI（CloseWanService）はデータベースインスタンスのパブリックネットワークからのアクセスを無効化するために使用されます。パブリックネットワークからのアクセスを無効にした後、パブリックネットワークアドレスでアクセスできません。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：CloseWanService |
...
### 例1 インスタンスのパブリックネットワークからのアクセスの無効化
...
* [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=cdb&Version=2017-03-20&Action=CloseWanService)
