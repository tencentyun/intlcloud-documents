...
このAPI（DescribeRollbackRangeTime）はデータベースインスタンスのロールバック可能な時間範囲を照合するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：DescribeRollbackRangeTime |
...
| InstanceIds.N | はい | Array of String | インスタンスIDリスト。単一インスタンスIDのフォーマット：cdb-c1nl9rpv。データベースコンソールページで表示されるインスタンスIDと同じ。 |
...
| Items | Array of [InstanceRollbackRangeTime](https://cloud.tencent.com/document/api/236/15878#InstanceRollbackRangeTime) | 返されたパラメータ情報。|
