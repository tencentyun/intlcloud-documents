...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：DescribeRollbackRangeTime |
...
| InstanceIds.N | 是 | Array of String | 实例ID列表，单个实例Id的格式如：cdb-c1nl9rpv。与云数据库控制台页面中显示的实例ID相同。 |
...
| Items | Array of [InstanceRollbackRangeTime](https://cloud.tencent.com/document/api/236/15878#InstanceRollbackRangeTime) | 返回的参数信息。|
