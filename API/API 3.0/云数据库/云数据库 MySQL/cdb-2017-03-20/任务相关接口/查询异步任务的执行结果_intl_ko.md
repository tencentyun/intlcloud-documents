...
이 API(DescribeAsyncRequestInfo)는 데이터베이스 인스턴스 비동기화 태스크의 실행 결과 조회에 사용합니다.

기본적 API 요청 빈도 제한: 40회/초.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: DescribeAsyncRequestInfo |
...
| AsyncRequestId | 예 | String | 비동기화 태스크 요청 ID. |

## 3. 출력 매개변수

| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| Status | String | 태스크 실행 결과. 가능한 값: INITIAL - 초기화, RUNNING - 실행 중, SUCCESS - 실행 성공, FAILED - 실행 실패, KILLED - 정지됨, REMOVED - 삭제됨, PAUSED - 정지 중. |
| Info | String | 태스크 실행 정보 설명. |
| RequestId | String | 유일한 요청 ID, 매회 요청 시마다 반환됩니다. 문제를 찾을 경우 해당 요청의 RequestId를 제공해야 합니다. |

## 4. 예시

### 예시1 비동기화 태스크의 실행 결과 조회
