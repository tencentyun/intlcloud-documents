...
このAPI（CreateBackup）はデータベースバックアップの作成に使用されます。

デフォルトのAPIリクエスト頻度制限：1回/秒。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：CreateBackup |
...
| BackupMethod | はい | String | 目標バックアップ方法。選択可能な値：logical - 論理コールドバックアップ、physical - 物理コールドバックアップ。 |

## 3. 出力パラメータ

| パラメータ名 | タイプ | 説明 |
|---------|---------|---------|
| BackupId | Integer | バックアップタスクID。|
...
* [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=cdb&Version=2017-03-20&Action=CreateBackup)
