...
このAPI（ModifyBackupConfig）はデータベースバックアップの構成情報を変更するために使用されます。
...
| パラメータ名 | 必須項目 | タイプ | 説明 |
|---------|---------|---------|---------|
| Action | はい | String | 共通パラメータ、このAPIの値：ModifyBackupConfig |
...
| ExpireDays | いいえ | Integer | バックアップ期限が切れる時間。単位は日、最小値は7日、最大値は732日。 |
| StartTime | いいえ | String | バックアップの時間範囲。フォーマット：02:00-06:00、開始と終了時間は現在毎時に制限され、選択可能な範囲：02:00-06:00、06:00-10:00、10:00-14:00、14:00-18:00、18:00-22:00、22:00-02:00。 |
| BackupMethod | いいえ | String | 目標バックアップ方法。選択可能な値：logical - 論理コールドバックアップ、physical - 物理コールドバックアップ。デフォルトのバックアップ方法は論理コールドバックアップです。 |
...
* [API 3.0 Explorer](https://console.cloud.tencent.com/api/explorer?Product=cdb&Version=2017-03-20&Action=ModifyBackupConfig)
