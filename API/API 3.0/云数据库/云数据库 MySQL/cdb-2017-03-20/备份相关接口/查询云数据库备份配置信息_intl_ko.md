...
이 API(DescribeBackupConfig)는 데이터베이스 백업 구성 정보 조회에 사용됩니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: DescribeBackupConfig |
...
| InstanceId | 예 | String | 인스턴스 짧은 인스턴스 ID, 형식 예: cdb-c1nl9rpv. TencentDB 콘솔 페이지에 표시된 인스턴스 ID와 동일합니다. |

## 3. 출력 매개변수

| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| StartTimeMin | Integer | 백업 시작의 가장 이른 시간점, 단위는 시각. 예를 들어, 2 - 새벽 2:00 |
| StartTimeMax | Integer | 백업 시작 시간의 가장 늦은 시간점, 단위는 시각. 예를 들어, 6 - 새벽 6:00 |
| BackupExpireDays | Integer | 백업 만료 시간, 단위는 일 |
| BackupMethod | String | 백업 방식, 가능한 값: physical - 물리적 백업, logical - 로직 백업|
| BinlogExpireDays | Integer | Binlog 만료 시간, 단위는 일 |
| RequestId | String | 유일한 요청 ID, 매회 요청 시마다 반환됩니다. 문제를 찾을 경우 해당 요청의 RequestId를 제공해야 합니다. |

## 4. 예시

### 예시1 데이터베이스 백업 구성 정보 조회
