...
이 API(DescribeTables)는 데이터베이스 인스턴스의 데이터베이스 테이블 정보 조회에 사용합니다.
...
|---------|---------|---------|---------|
| Action | 예 | String | 공통 매개변수, 이 API 값: DescribeTables |
...
| Database | 예 | String | 데이터베이스 이름. |
| Offset | 아니요 | Integer | 오프셋 기록, 기본값은 0입니다. |
| Limit | 아니요 | Integer | 1회 요청의 반환 수량, 기본값은 20, 최대값은 2000입니다. |
| TableRegexp | 아니요 | String | 데이터베이스 테이블 이름과 일치하는 정규식. 규칙은 MySQL 공식 웹사이트와 동일합니다. |

## 3. 출력 매개변수

| 매개변수 이름 | 유형 | 설명 |
|---------|---------|---------|
| TotalCount | Integer | 조회 조건에 부합하는 데이터베이스 테이블 총수. |
| Items | Array of String | 반환된 데이터베이스 테이블 정보. |
