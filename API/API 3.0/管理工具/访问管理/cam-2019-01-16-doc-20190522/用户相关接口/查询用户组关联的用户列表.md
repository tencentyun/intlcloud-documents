...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：ListUsersForGroup |
| Version | 是 | String | 公共参数，本接口取值：2019-01-16 |
| Region | 否 | String | 公共参数，本接口不需要传递此参数。 |
| GroupId | 是 | Integer | 用户组 ID。 |
| Page | 否 | Integer | 页码。默认为1。 |
| Rp | 否 | Integer | 每页数量。默认为20。 |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| TotalNum | Integer | 用户组关联的用户总数。|
| UserInfo | Array of [GroupMemberInfo](https://cloud.tencent.com/document/api/598/33167#GroupMemberInfo) | 子用户信息。|
