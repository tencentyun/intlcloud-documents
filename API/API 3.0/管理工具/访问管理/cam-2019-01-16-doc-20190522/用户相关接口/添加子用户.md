...
| ConsoleLogin | 否 | Integer | 子用户是否可以登录控制台 |
| UseApi | 否 | Integer | 是否生成子用户密钥 |
| Password | 否 | String | 子用户控制台登录密码，只有可以登录控制台时才有效，如果传空并且上面指定允许登录控制台，则自动生成随机密码 |
| NeedResetPassword | 否 | Integer | 子用户是否要在下次登录时重置密码 |
