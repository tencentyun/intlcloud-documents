...
| TagSet.N | 否 | Array of [TagPair](https://cloud.tencent.com/document/api/608/37023#TagPair) | 标签列表，当存在该字段时，拉取对应标签下的资源列表。<br/>最多支持5个标签，当存在两个或两个以上的标签时，满足其中任意一个标签时，该通道组会被拉取出来。 |
| Filters.N | 否 | Array of [Filter](https://cloud.tencent.com/document/api/608/37023#Filter) | 过滤条件。   <br/>每次请求的Filter.Values的上限为5。<br/>RealServerRegion - String - 是否必填：否 -（过滤条件）按照源站地域过滤，可参考DescribeDestRegions接口返回结果中的RegionId。 |
...
| ProxyGroupList | Array of [ProxyGroupInfo](https://cloud.tencent.com/document/api/608/37023#ProxyGroupInfo) | 通道组列表。<br/>注意：此字段可能返回 null，表示取不到有效值。|
