The common parameters are used to identify the user and API. If not necessary, these parameters are not described in individual API documents. However, they have to be carried by each request to initiate properly.
...
When using the HmacSHA1 or HmacSHA256 signature method, the common parameters should be uniformly placed in request string as shown below:
