...
* This API is used to obtain event notifications from the VOD server. For more information, see [Server Event Notification](https://cloud.tencent.com/document/product/266/7829);
* The API is in long polling mode, i.e., if there is an unconsumed event on the server, it is immediately returned to the requester; otherwise, the backend suspends the request until a new event is generated;
...
|---------|---------|---------|---------|
| Action | Yes | String | Common parameter; the value for this API: PullEvents |
