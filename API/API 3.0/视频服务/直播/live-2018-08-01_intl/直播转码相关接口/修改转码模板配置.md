...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：ModifyLiveTranscodeTemplate |
| Version | 是 | String | 公共参数，本接口取值：2018-08-01 |
| Region | 否 | String | 公共参数，本接口不需要传递此参数。 |
| TemplateId | 是 | Integer | 模板Id。 |
| Vcodec | 否 | String | 视频编码：<br/>h264/h265。 |
| Acodec | 否 | String | 音频编码：<br/>aac/mp3。 |
| AudioBitrate | 否 | Integer | 音频码率，默认0。0-500 |
| Description | 否 | String | 模板描述。 |
| VideoBitrate | 否 | Integer | 视频码率。100-8000 |
| Width | 否 | Integer | 宽。0-3000 |
| NeedVideo | 否 | Integer | 是否保留视频，0：否，1：是。默认1。 |
| NeedAudio | 否 | Integer | 是否保留音频，0：否，1：是。默认1。 |
| Height | 否 | Integer | 高。0-3000 |
| Fps | 否 | Integer | 帧率。0-200 |
| Gop | 否 | Integer | 关键帧间隔，单位：秒。0-50 |
| Rotate | 否 | Integer | 旋转角度。0 90 180 270 |
| Profile | 否 | String | 编码质量：<br/>baseline/main/high。 |
