...
创建截图模板，成功返回模板id后，需要调用[CreateLiveSnapshotRule](https://intl.cloud.tencent.com/document/product/267/30835)接口，将模板id绑定到流使用。
<br>截图相关文档：[直播截图](https://intl.cloud.tencent.com/document/product/267/31562)。
...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | 公共参数，本接口取值：CreateLiveSnapshotTemplate |
| Version | 是 | String | 公共参数，本接口取值：2018-08-01 |
| Region | 否 | String | 公共参数，本接口不需要传递此参数。 |
| TemplateName | 是 | String | 模板名称。非空的字符串 |
| CosAppId | 是 | Integer | Cos AppId。 |
| CosBucket | 是 | String | Cos Bucket名称。 |
| CosRegion | 是 | String | Cos地区。 |
| Description | 否 | String | 描述信息。 |
| SnapshotInterval | 否 | Integer | 截图间隔，单位s，默认10s。 |
| Width | 否 | Integer | 截图宽度。默认：0（原始高） |
| Height | 否 | Integer | 截图高度。默认：0（原始宽） |
| PornFlag | 否 | Integer | 是否开启鉴黄，0：不开启，1：开启。默认：0. |
